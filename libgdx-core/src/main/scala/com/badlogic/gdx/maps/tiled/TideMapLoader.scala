...
class TideMapLoader extends com.badlogic.gdx.assets.loaders.SynchronousAssetLoader[com.badlogic.gdx.maps.tiled.TiledMap, Parameters] {
  ...
  def load(assetManager: com.badlogic.gdx.assets.AssetManager, fileName: java.lang.String, tideFile: com.badlogic.gdx.files.FileHandle, parameter: Parameters): com.badlogic.gdx.maps.tiled.TiledMap = {
    try {
  ...
  def getDependencies(fileName: java.lang.String, tmxFile: com.badlogic.gdx.files.FileHandle, parameter: Parameters): com.badlogic.gdx.utils.Array[com.badlogic.gdx.assets.AssetDescriptor] = {
    val dependencies: com.badlogic.gdx.utils.Array[com.badlogic.gdx.assets.AssetDescriptor] = new com.badlogic.gdx.utils.Array[com.badlogic.gdx.assets.AssetDescriptor]()
    try {
      this.root = this.xml.parse(tmxFile)
      for (image <- this.loadTileSheets(this.root, tmxFile)) {
        dependencies.add(new com.badlogic.gdx.assets.AssetDescriptor(image.path(), classOf[java.lang.Class]))
      }
      return dependencies
    } catch {
      case e: java.io.IOException => {
      ...
      { var row: scala.Int = 0; val rowCount: scala.Int = rows.size; while (row < rowCount) { {
        ...
        { var child: scala.Int = 0; val childCount: scala.Int = currentRow.getChildCount(); while (child < childCount) { {
          ...
          if (name.equals("TileSheet")) {
            ...
            firstgid = currentTileSet.getProperties().get("firstgid", classOf[java.lang.Class])
          } else {
            if (name.equals("Null")) {
    ...
    if (element.getName().equals("Properties")) {
      for (property <- element.getChildrenByName("Property")) {
        ...
        if (`type`.equals("Int32")) {
          properties.put(key, java.lang.Integer.parseInt(value))
        } else {
