...
object GL32 {
  export com.badlogic.gdx.graphics.GL31.{DebugProc => _, GL_BUFFER => _, GL_CLAMP_TO_BORDER => _, GL_COLORBURN => _, GL_COLORDODGE => _, GL_COMPRESSED_RGBA_ASTC_10x10 => _, GL_COMPRESSED_RGBA_ASTC_10x5 => _, GL_COMPRESSED_RGBA_ASTC_10x6 => _, GL_COMPRESSED_RGBA_ASTC_10x8 => _, GL_COMPRESSED_RGBA_ASTC_12x10 => _, GL_COMPRESSED_RGBA_ASTC_12x12 => _, GL_COMPRESSED_RGBA_ASTC_4x4 => _, GL_COMPRESSED_RGBA_ASTC_5x4 => _, GL_COMPRESSED_RGBA_ASTC_5x5 => _, GL_COMPRESSED_RGBA_ASTC_6x5 => _, GL_COMPRESSED_RGBA_ASTC_6x6 => _, GL_COMPRESSED_RGBA_ASTC_8x5 => _, GL_COMPRESSED_RGBA_ASTC_8x6 => _, GL_COMPRESSED_RGBA_ASTC_8x8 => _, GL_COMPRESSED_SRGB8_ALPHA8_ASTC_10x10 => _, GL_COMPRESSED_SRGB8_ALPHA8_ASTC_10x5 => _, GL_COMPRESSED_SRGB8_ALPHA8_ASTC_10x6 => _, GL_COMPRESSED_SRGB8_ALPHA8_ASTC_10x8 => _, GL_COMPRESSED_SRGB8_ALPHA8_ASTC_12x10 => _, GL_COMPRESSED_SRGB8_ALPHA8_ASTC_12x12 => _, GL_COMPRESSED_SRGB8_ALPHA8_ASTC_4x4 => _, GL_COMPRESSED_SRGB8_ALPHA8_ASTC_5x4 => _, GL_COMPRESSED_SRGB8_ALPHA8_ASTC_5x5 => _, GL_COMPRESSED_SRGB8_ALPHA8_ASTC_6x5 => _, GL_COMPRESSED_SRGB8_ALPHA8_ASTC_6x6 => _, GL_COMPRESSED_SRGB8_ALPHA8_ASTC_8x5 => _, GL_COMPRESSED_SRGB8_ALPHA8_ASTC_8x6 => _, GL_COMPRESSED_SRGB8_ALPHA8_ASTC_8x8 => _, GL_CONTEXT_FLAGS => _, GL_CONTEXT_FLAG_DEBUG_BIT => _, GL_CONTEXT_FLAG_ROBUST_ACCESS_BIT => _, GL_CONTEXT_LOST => _, GL_DARKEN => _, GL_DEBUG_CALLBACK_FUNCTION => _, GL_DEBUG_CALLBACK_USER_PARAM => _, GL_DEBUG_GROUP_STACK_DEPTH => _, GL_DEBUG_LOGGED_MESSAGES => _, GL_DEBUG_NEXT_LOGGED_MESSAGE_LENGTH => _, GL_DEBUG_OUTPUT => _, GL_DEBUG_OUTPUT_SYNCHRONOUS => _, GL_DEBUG_SEVERITY_HIGH => _, GL_DEBUG_SEVERITY_LOW => _, GL_DEBUG_SEVERITY_MEDIUM => _, GL_DEBUG_SEVERITY_NOTIFICATION => _, GL_DEBUG_SOURCE_API => _, GL_DEBUG_SOURCE_APPLICATION => _, GL_DEBUG_SOURCE_OTHER => _, GL_DEBUG_SOURCE_SHADER_COMPILER => _, GL_DEBUG_SOURCE_THIRD_PARTY => _, GL_DEBUG_SOURCE_WINDOW_SYSTEM => _, GL_DEBUG_TYPE_DEPRECATED_BEHAVIOR => _, GL_DEBUG_TYPE_ERROR => _, GL_DEBUG_TYPE_MARKER => _, GL_DEBUG_TYPE_OTHER => _, GL_DEBUG_TYPE_PERFORMANCE => _, GL_DEBUG_TYPE_POP_GROUP => _, GL_DEBUG_TYPE_PORTABILITY => _, GL_DEBUG_TYPE_PUSH_GROUP => _, GL_DEBUG_TYPE_UNDEFINED_BEHAVIOR => _, GL_DIFFERENCE => _, GL_EXCLUSION => _, GL_FIRST_VERTEX_CONVENTION => _, GL_FRACTIONAL_EVEN => _, GL_FRACTIONAL_ODD => _, GL_FRAGMENT_INTERPOLATION_OFFSET_BITS => _, GL_FRAMEBUFFER_ATTACHMENT_LAYERED => _, GL_FRAMEBUFFER_DEFAULT_LAYERS => _, GL_FRAMEBUFFER_INCOMPLETE_LAYER_TARGETS => _, GL_GEOMETRY_INPUT_TYPE => _, GL_GEOMETRY_OUTPUT_TYPE => _, GL_GEOMETRY_SHADER => _, GL_GEOMETRY_SHADER_BIT => _, GL_GEOMETRY_SHADER_INVOCATIONS => _, GL_GEOMETRY_VERTICES_OUT => _, GL_GUILTY_CONTEXT_RESET => _, GL_HARDLIGHT => _, GL_HSL_COLOR => _, GL_HSL_HUE => _, GL_HSL_LUMINOSITY => _, GL_HSL_SATURATION => _, GL_IMAGE_BUFFER => _, GL_IMAGE_CUBE_MAP_ARRAY => _, GL_INNOCENT_CONTEXT_RESET => _, GL_INT_IMAGE_BUFFER => _, GL_INT_IMAGE_CUBE_MAP_ARRAY => _, GL_INT_SAMPLER_2D_MULTISAMPLE_ARRAY => _, GL_INT_SAMPLER_BUFFER => _, GL_INT_SAMPLER_CUBE_MAP_ARRAY => _, GL_ISOLINES => _, GL_IS_PER_PATCH => _, GL_LAST_VERTEX_CONVENTION => _, GL_LAYER_PROVOKING_VERTEX => _, GL_LIGHTEN => _, GL_LINES_ADJACENCY => _, GL_LINE_STRIP_ADJACENCY => _, GL_LOSE_CONTEXT_ON_RESET => _, GL_MAX_COMBINED_GEOMETRY_UNIFORM_COMPONENTS => _, GL_MAX_COMBINED_TESS_CONTROL_UNIFORM_COMPONENTS => _, GL_MAX_COMBINED_TESS_EVALUATION_UNIFORM_COMPONENTS => _, GL_MAX_DEBUG_GROUP_STACK_DEPTH => _, GL_MAX_DEBUG_LOGGED_MESSAGES => _, GL_MAX_DEBUG_MESSAGE_LENGTH => _, GL_MAX_FRAGMENT_INTERPOLATION_OFFSET => _, GL_MAX_FRAMEBUFFER_LAYERS => _, GL_MAX_GEOMETRY_ATOMIC_COUNTERS => _, GL_MAX_GEOMETRY_ATOMIC_COUNTER_BUFFERS => _, GL_MAX_GEOMETRY_IMAGE_UNIFORMS => _, GL_MAX_GEOMETRY_INPUT_COMPONENTS => _, GL_MAX_GEOMETRY_OUTPUT_COMPONENTS => _, GL_MAX_GEOMETRY_OUTPUT_VERTICES => _, GL_MAX_GEOMETRY_SHADER_INVOCATIONS => _, GL_MAX_GEOMETRY_SHADER_STORAGE_BLOCKS => _, GL_MAX_GEOMETRY_TEXTURE_IMAGE_UNITS => _, GL_MAX_GEOMETRY_TOTAL_OUTPUT_COMPONENTS => _, GL_MAX_GEOMETRY_UNIFORM_BLOCKS => _, GL_MAX_GEOMETRY_UNIFORM_COMPONENTS => _, GL_MAX_LABEL_LENGTH => _, GL_MAX_PATCH_VERTICES => _, GL_MAX_TESS_CONTROL_ATOMIC_COUNTERS => _, GL_MAX_TESS_CONTROL_ATOMIC_COUNTER_BUFFERS => _, GL_MAX_TESS_CONTROL_IMAGE_UNIFORMS => _, GL_MAX_TESS_CONTROL_INPUT_COMPONENTS => _, GL_MAX_TESS_CONTROL_OUTPUT_COMPONENTS => _, GL_MAX_TESS_CONTROL_SHADER_STORAGE_BLOCKS => _, GL_MAX_TESS_CONTROL_TEXTURE_IMAGE_UNITS => _, GL_MAX_TESS_CONTROL_TOTAL_OUTPUT_COMPONENTS => _, GL_MAX_TESS_CONTROL_UNIFORM_BLOCKS => _, GL_MAX_TESS_CONTROL_UNIFORM_COMPONENTS => _, GL_MAX_TESS_EVALUATION_ATOMIC_COUNTERS => _, GL_MAX_TESS_EVALUATION_ATOMIC_COUNTER_BUFFERS => _, GL_MAX_TESS_EVALUATION_IMAGE_UNIFORMS => _, GL_MAX_TESS_EVALUATION_INPUT_COMPONENTS => _, GL_MAX_TESS_EVALUATION_OUTPUT_COMPONENTS => _, GL_MAX_TESS_EVALUATION_SHADER_STORAGE_BLOCKS => _, GL_MAX_TESS_EVALUATION_TEXTURE_IMAGE_UNITS => _, GL_MAX_TESS_EVALUATION_UNIFORM_BLOCKS => _, GL_MAX_TESS_EVALUATION_UNIFORM_COMPONENTS => _, GL_MAX_TESS_GEN_LEVEL => _, GL_MAX_TESS_PATCH_COMPONENTS => _, GL_MAX_TEXTURE_BUFFER_SIZE => _, GL_MIN_FRAGMENT_INTERPOLATION_OFFSET => _, GL_MIN_SAMPLE_SHADING_VALUE => _, GL_MULTIPLY => _, GL_MULTISAMPLE_LINE_WIDTH_GRANULARITY => _, GL_MULTISAMPLE_LINE_WIDTH_RANGE => _, GL_NO_RESET_NOTIFICATION => _, GL_OVERLAY => _, GL_PATCHES => _, GL_PATCH_VERTICES => _, GL_PRIMITIVES_GENERATED => _, GL_PRIMITIVE_BOUNDING_BOX => _, GL_PRIMITIVE_RESTART_FOR_PATCHES_SUPPORTED => _, GL_PROGRAM => _, GL_PROGRAM_PIPELINE => _, GL_QUADS => _, GL_QUERY => _, GL_REFERENCED_BY_GEOMETRY_SHADER => _, GL_REFERENCED_BY_TESS_CONTROL_SHADER => _, GL_REFERENCED_BY_TESS_EVALUATION_SHADER => _, GL_RESET_NOTIFICATION_STRATEGY => _, GL_SAMPLER => _, GL_SAMPLER_2D_MULTISAMPLE_ARRAY => _, GL_SAMPLER_BUFFER => _, GL_SAMPLER_CUBE_MAP_ARRAY => _, GL_SAMPLER_CUBE_MAP_ARRAY_SHADOW => _, GL_SAMPLE_SHADING => _, GL_SCREEN => _, GL_SHADER => _, GL_SOFTLIGHT => _, GL_STACK_OVERFLOW => _, GL_STACK_UNDERFLOW => _, GL_TESS_CONTROL_OUTPUT_VERTICES => _, GL_TESS_CONTROL_SHADER => _, GL_TESS_CONTROL_SHADER_BIT => _, GL_TESS_EVALUATION_SHADER => _, GL_TESS_EVALUATION_SHADER_BIT => _, GL_TESS_GEN_MODE => _, GL_TESS_GEN_POINT_MODE => _, GL_TESS_GEN_SPACING => _, GL_TESS_GEN_VERTEX_ORDER => _, GL_TEXTURE_2D_MULTISAMPLE_ARRAY => _, GL_TEXTURE_BINDING_2D_MULTISAMPLE_ARRAY => _, GL_TEXTURE_BINDING_BUFFER => _, GL_TEXTURE_BINDING_CUBE_MAP_ARRAY => _, GL_TEXTURE_BORDER_COLOR => _, GL_TEXTURE_BUFFER => _, GL_TEXTURE_BUFFER_BINDING => _, GL_TEXTURE_BUFFER_DATA_STORE_BINDING => _, GL_TEXTURE_BUFFER_OFFSET => _, GL_TEXTURE_BUFFER_OFFSET_ALIGNMENT => _, GL_TEXTURE_BUFFER_SIZE => _, GL_TEXTURE_CUBE_MAP_ARRAY => _, GL_TRIANGLES_ADJACENCY => _, GL_TRIANGLE_STRIP_ADJACENCY => _, GL_UNDEFINED_VERTEX => _, GL_UNKNOWN_CONTEXT_RESET => _, GL_UNSIGNED_INT_IMAGE_BUFFER => _, GL_UNSIGNED_INT_IMAGE_CUBE_MAP_ARRAY => _, GL_UNSIGNED_INT_SAMPLER_2D_MULTISAMPLE_ARRAY => _, GL_UNSIGNED_INT_SAMPLER_BUFFER => _, GL_UNSIGNED_INT_SAMPLER_CUBE_MAP_ARRAY => _, GL_VERTEX_ARRAY => _, *}
