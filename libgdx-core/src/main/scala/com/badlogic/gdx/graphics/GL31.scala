...
object GL31 {
  export com.badlogic.gdx.graphics.GL30.{GL_VERTEX_SHADER_BIT => _, GL_FRAGMENT_SHADER_BIT => _, GL_COMPUTE_SHADER_BIT => _, GL_ALL_SHADER_BITS => _, GL_VERTEX_ATTRIB_ARRAY_BARRIER_BIT => _, GL_ELEMENT_ARRAY_BARRIER_BIT => _, GL_UNIFORM_BARRIER_BIT => _, GL_TEXTURE_FETCH_BARRIER_BIT => _, GL_SHADER_IMAGE_ACCESS_BARRIER_BIT => _, GL_COMMAND_BARRIER_BIT => _, GL_PIXEL_BUFFER_BARRIER_BIT => _, GL_TEXTURE_UPDATE_BARRIER_BIT => _, GL_BUFFER_UPDATE_BARRIER_BIT => _, GL_FRAMEBUFFER_BARRIER_BIT => _, GL_TRANSFORM_FEEDBACK_BARRIER_BIT => _, GL_ATOMIC_COUNTER_BARRIER_BIT => _, GL_SHADER_STORAGE_BARRIER_BIT => _, GL_ALL_BARRIER_BITS => _, GL_TEXTURE_WIDTH => _, GL_TEXTURE_HEIGHT => _, GL_TEXTURE_INTERNAL_FORMAT => _, GL_STENCIL_INDEX => _, GL_TEXTURE_RED_SIZE => _, GL_TEXTURE_GREEN_SIZE => _, GL_TEXTURE_BLUE_SIZE => _, GL_TEXTURE_ALPHA_SIZE => _, GL_TEXTURE_DEPTH => _, GL_PROGRAM_SEPARABLE => _, GL_ACTIVE_PROGRAM => _, GL_PROGRAM_PIPELINE_BINDING => _, GL_MAX_COMPUTE_SHARED_MEMORY_SIZE => _, GL_MAX_COMPUTE_UNIFORM_COMPONENTS => _, GL_MAX_COMPUTE_ATOMIC_COUNTER_BUFFERS => _, GL_MAX_COMPUTE_ATOMIC_COUNTERS => _, GL_MAX_COMBINED_COMPUTE_UNIFORM_COMPONENTS => _, GL_COMPUTE_WORK_GROUP_SIZE => _, GL_MAX_UNIFORM_LOCATIONS => _, GL_VERTEX_ATTRIB_BINDING => _, GL_VERTEX_ATTRIB_RELATIVE_OFFSET => _, GL_VERTEX_BINDING_DIVISOR => _, GL_VERTEX_BINDING_OFFSET => _, GL_VERTEX_BINDING_STRIDE => _, GL_MAX_VERTEX_ATTRIB_RELATIVE_OFFSET => _, GL_MAX_VERTEX_ATTRIB_BINDINGS => _, GL_MAX_VERTEX_ATTRIB_STRIDE => _, GL_TEXTURE_COMPRESSED => _, GL_TEXTURE_DEPTH_SIZE => _, GL_READ_ONLY => _, GL_WRITE_ONLY => _, GL_READ_WRITE => _, GL_TEXTURE_STENCIL_SIZE => _, GL_TEXTURE_RED_TYPE => _, GL_TEXTURE_GREEN_TYPE => _, GL_TEXTURE_BLUE_TYPE => _, GL_TEXTURE_ALPHA_TYPE => _, GL_TEXTURE_DEPTH_TYPE => _, GL_TEXTURE_SHARED_SIZE => _, GL_SAMPLE_POSITION => _, GL_SAMPLE_MASK => _, GL_SAMPLE_MASK_VALUE => _, GL_MAX_SAMPLE_MASK_WORDS => _, GL_MIN_PROGRAM_TEXTURE_GATHER_OFFSET => _, GL_MAX_PROGRAM_TEXTURE_GATHER_OFFSET => _, GL_MAX_IMAGE_UNITS => _, GL_MAX_COMBINED_SHADER_OUTPUT_RESOURCES => _, GL_IMAGE_BINDING_NAME => _, GL_IMAGE_BINDING_LEVEL => _, GL_IMAGE_BINDING_LAYERED => _, GL_IMAGE_BINDING_LAYER => _, GL_IMAGE_BINDING_ACCESS => _, GL_DRAW_INDIRECT_BUFFER => _, GL_DRAW_INDIRECT_BUFFER_BINDING => _, GL_VERTEX_BINDING_BUFFER => _, GL_IMAGE_2D => _, GL_IMAGE_3D => _, GL_IMAGE_CUBE => _, GL_IMAGE_2D_ARRAY => _, GL_INT_IMAGE_2D => _, GL_INT_IMAGE_3D => _, GL_INT_IMAGE_CUBE => _, GL_INT_IMAGE_2D_ARRAY => _, GL_UNSIGNED_INT_IMAGE_2D => _, GL_UNSIGNED_INT_IMAGE_3D => _, GL_UNSIGNED_INT_IMAGE_CUBE => _, GL_UNSIGNED_INT_IMAGE_2D_ARRAY => _, GL_IMAGE_BINDING_FORMAT => _, GL_IMAGE_FORMAT_COMPATIBILITY_TYPE => _, GL_IMAGE_FORMAT_COMPATIBILITY_BY_SIZE => _, GL_IMAGE_FORMAT_COMPATIBILITY_BY_CLASS => _, GL_MAX_VERTEX_IMAGE_UNIFORMS => _, GL_MAX_FRAGMENT_IMAGE_UNIFORMS => _, GL_MAX_COMBINED_IMAGE_UNIFORMS => _, GL_SHADER_STORAGE_BUFFER => _, GL_SHADER_STORAGE_BUFFER_BINDING => _, GL_SHADER_STORAGE_BUFFER_START => _, GL_SHADER_STORAGE_BUFFER_SIZE => _, GL_MAX_VERTEX_SHADER_STORAGE_BLOCKS => _, GL_MAX_FRAGMENT_SHADER_STORAGE_BLOCKS => _, GL_MAX_COMPUTE_SHADER_STORAGE_BLOCKS => _, GL_MAX_COMBINED_SHADER_STORAGE_BLOCKS => _, GL_MAX_SHADER_STORAGE_BUFFER_BINDINGS => _, GL_MAX_SHADER_STORAGE_BLOCK_SIZE => _, GL_SHADER_STORAGE_BUFFER_OFFSET_ALIGNMENT => _, GL_DEPTH_STENCIL_TEXTURE_MODE => _, GL_MAX_COMPUTE_WORK_GROUP_INVOCATIONS => _, GL_DISPATCH_INDIRECT_BUFFER => _, GL_DISPATCH_INDIRECT_BUFFER_BINDING => _, GL_TEXTURE_2D_MULTISAMPLE => _, GL_TEXTURE_BINDING_2D_MULTISAMPLE => _, GL_TEXTURE_SAMPLES => _, GL_TEXTURE_FIXED_SAMPLE_LOCATIONS => _, GL_SAMPLER_2D_MULTISAMPLE => _, GL_INT_SAMPLER_2D_MULTISAMPLE => _, GL_UNSIGNED_INT_SAMPLER_2D_MULTISAMPLE => _, GL_MAX_COLOR_TEXTURE_SAMPLES => _, GL_MAX_DEPTH_TEXTURE_SAMPLES => _, GL_MAX_INTEGER_SAMPLES => _, GL_COMPUTE_SHADER => _, GL_MAX_COMPUTE_UNIFORM_BLOCKS => _, GL_MAX_COMPUTE_TEXTURE_IMAGE_UNITS => _, GL_MAX_COMPUTE_IMAGE_UNIFORMS => _, GL_MAX_COMPUTE_WORK_GROUP_COUNT => _, GL_MAX_COMPUTE_WORK_GROUP_SIZE => _, GL_ATOMIC_COUNTER_BUFFER => _, GL_ATOMIC_COUNTER_BUFFER_BINDING => _, GL_ATOMIC_COUNTER_BUFFER_START => _, GL_ATOMIC_COUNTER_BUFFER_SIZE => _, GL_MAX_VERTEX_ATOMIC_COUNTER_BUFFERS => _, GL_MAX_FRAGMENT_ATOMIC_COUNTER_BUFFERS => _, GL_MAX_COMBINED_ATOMIC_COUNTER_BUFFERS => _, GL_MAX_VERTEX_ATOMIC_COUNTERS => _, GL_MAX_FRAGMENT_ATOMIC_COUNTERS => _, GL_MAX_COMBINED_ATOMIC_COUNTERS => _, GL_MAX_ATOMIC_COUNTER_BUFFER_SIZE => _, GL_ACTIVE_ATOMIC_COUNTER_BUFFERS => _, GL_UNSIGNED_INT_ATOMIC_COUNTER => _, GL_MAX_ATOMIC_COUNTER_BUFFER_BINDINGS => _, GL_UNIFORM => _, GL_UNIFORM_BLOCK => _, GL_PROGRAM_INPUT => _, GL_PROGRAM_OUTPUT => _, GL_BUFFER_VARIABLE => _, GL_SHADER_STORAGE_BLOCK => _, GL_TRANSFORM_FEEDBACK_VARYING => _, GL_ACTIVE_RESOURCES => _, GL_MAX_NAME_LENGTH => _, GL_MAX_NUM_ACTIVE_VARIABLES => _, GL_NAME_LENGTH => _, GL_TYPE => _, GL_ARRAY_SIZE => _, GL_OFFSET => _, GL_BLOCK_INDEX => _, GL_ARRAY_STRIDE => _, GL_MATRIX_STRIDE => _, GL_IS_ROW_MAJOR => _, GL_ATOMIC_COUNTER_BUFFER_INDEX => _, GL_BUFFER_BINDING => _, GL_BUFFER_DATA_SIZE => _, GL_NUM_ACTIVE_VARIABLES => _, GL_ACTIVE_VARIABLES => _, GL_REFERENCED_BY_VERTEX_SHADER => _, GL_REFERENCED_BY_FRAGMENT_SHADER => _, GL_REFERENCED_BY_COMPUTE_SHADER => _, GL_TOP_LEVEL_ARRAY_SIZE => _, GL_TOP_LEVEL_ARRAY_STRIDE => _, GL_LOCATION => _, GL_FRAMEBUFFER_DEFAULT_WIDTH => _, GL_FRAMEBUFFER_DEFAULT_HEIGHT => _, GL_FRAMEBUFFER_DEFAULT_SAMPLES => _, GL_FRAMEBUFFER_DEFAULT_FIXED_SAMPLE_LOCATIONS => _, GL_MAX_FRAMEBUFFER_WIDTH => _, GL_MAX_FRAMEBUFFER_HEIGHT => _, GL_MAX_FRAMEBUFFER_SAMPLES => _, *}
