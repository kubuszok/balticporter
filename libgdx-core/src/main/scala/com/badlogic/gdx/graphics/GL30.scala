...
object GL30 {
  export com.badlogic.gdx.graphics.GL20.{GL_READ_BUFFER => _, GL_UNPACK_ROW_LENGTH => _, GL_UNPACK_SKIP_ROWS => _, GL_UNPACK_SKIP_PIXELS => _, GL_PACK_ROW_LENGTH => _, GL_PACK_SKIP_ROWS => _, GL_PACK_SKIP_PIXELS => _, GL_COLOR => _, GL_DEPTH => _, GL_STENCIL => _, GL_RED => _, GL_RGB8 => _, GL_RGBA8 => _, GL_RGB10_A2 => _, GL_TEXTURE_BINDING_3D => _, GL_UNPACK_SKIP_IMAGES => _, GL_UNPACK_IMAGE_HEIGHT => _, GL_TEXTURE_3D => _, GL_TEXTURE_WRAP_R => _, GL_MAX_3D_TEXTURE_SIZE => _, GL_UNSIGNED_INT_2_10_10_10_REV => _, GL_MAX_ELEMENTS_VERTICES => _, GL_MAX_ELEMENTS_INDICES => _, GL_TEXTURE_MIN_LOD => _, GL_TEXTURE_MAX_LOD => _, GL_TEXTURE_BASE_LEVEL => _, GL_TEXTURE_MAX_LEVEL => _, GL_MIN => _, GL_MAX => _, GL_DEPTH_COMPONENT24 => _, GL_MAX_TEXTURE_LOD_BIAS => _, GL_TEXTURE_COMPARE_MODE => _, GL_TEXTURE_COMPARE_FUNC => _, GL_CURRENT_QUERY => _, GL_QUERY_RESULT => _, GL_QUERY_RESULT_AVAILABLE => _, GL_BUFFER_MAPPED => _, GL_BUFFER_MAP_POINTER => _, GL_STREAM_READ => _, GL_STREAM_COPY => _, GL_STATIC_READ => _, GL_STATIC_COPY => _, GL_DYNAMIC_READ => _, GL_DYNAMIC_COPY => _, GL_MAX_DRAW_BUFFERS => _, GL_DRAW_BUFFER0 => _, GL_DRAW_BUFFER1 => _, GL_DRAW_BUFFER2 => _, GL_DRAW_BUFFER3 => _, GL_DRAW_BUFFER4 => _, GL_DRAW_BUFFER5 => _, GL_DRAW_BUFFER6 => _, GL_DRAW_BUFFER7 => _, GL_DRAW_BUFFER8 => _, GL_DRAW_BUFFER9 => _, GL_DRAW_BUFFER10 => _, GL_DRAW_BUFFER11 => _, GL_DRAW_BUFFER12 => _, GL_DRAW_BUFFER13 => _, GL_DRAW_BUFFER14 => _, GL_DRAW_BUFFER15 => _, GL_MAX_FRAGMENT_UNIFORM_COMPONENTS => _, GL_MAX_VERTEX_UNIFORM_COMPONENTS => _, GL_SAMPLER_3D => _, GL_SAMPLER_2D_SHADOW => _, GL_FRAGMENT_SHADER_DERIVATIVE_HINT => _, GL_PIXEL_PACK_BUFFER => _, GL_PIXEL_UNPACK_BUFFER => _, GL_PIXEL_PACK_BUFFER_BINDING => _, GL_PIXEL_UNPACK_BUFFER_BINDING => _, GL_FLOAT_MAT2x3 => _, GL_FLOAT_MAT2x4 => _, GL_FLOAT_MAT3x2 => _, GL_FLOAT_MAT3x4 => _, GL_FLOAT_MAT4x2 => _, GL_FLOAT_MAT4x3 => _, GL_SRGB => _, GL_SRGB8 => _, GL_SRGB8_ALPHA8 => _, GL_COMPARE_REF_TO_TEXTURE => _, GL_MAJOR_VERSION => _, GL_MINOR_VERSION => _, GL_NUM_EXTENSIONS => _, GL_RGBA32F => _, GL_RGB32F => _, GL_RGBA16F => _, GL_RGB16F => _, GL_VERTEX_ATTRIB_ARRAY_INTEGER => _, GL_MAX_ARRAY_TEXTURE_LAYERS => _, GL_MIN_PROGRAM_TEXEL_OFFSET => _, GL_MAX_PROGRAM_TEXEL_OFFSET => _, GL_MAX_VARYING_COMPONENTS => _, GL_TEXTURE_2D_ARRAY => _, GL_TEXTURE_BINDING_2D_ARRAY => _, GL_R11F_G11F_B10F => _, GL_UNSIGNED_INT_10F_11F_11F_REV => _, GL_RGB9_E5 => _, GL_UNSIGNED_INT_5_9_9_9_REV => _, GL_TRANSFORM_FEEDBACK_VARYING_MAX_LENGTH => _, GL_TRANSFORM_FEEDBACK_BUFFER_MODE => _, GL_MAX_TRANSFORM_FEEDBACK_SEPARATE_COMPONENTS => _, GL_TRANSFORM_FEEDBACK_VARYINGS => _, GL_TRANSFORM_FEEDBACK_BUFFER_START => _, GL_TRANSFORM_FEEDBACK_BUFFER_SIZE => _, GL_TRANSFORM_FEEDBACK_PRIMITIVES_WRITTEN => _, GL_RASTERIZER_DISCARD => _, GL_MAX_TRANSFORM_FEEDBACK_INTERLEAVED_COMPONENTS => _, GL_MAX_TRANSFORM_FEEDBACK_SEPARATE_ATTRIBS => _, GL_INTERLEAVED_ATTRIBS => _, GL_SEPARATE_ATTRIBS => _, GL_TRANSFORM_FEEDBACK_BUFFER => _, GL_TRANSFORM_FEEDBACK_BUFFER_BINDING => _, GL_RGBA32UI => _, GL_RGB32UI => _, GL_RGBA16UI => _, GL_RGB16UI => _, GL_RGBA8UI => _, GL_RGB8UI => _, GL_RGBA32I => _, GL_RGB32I => _, GL_RGBA16I => _, GL_RGB16I => _, GL_RGBA8I => _, GL_RGB8I => _, GL_RED_INTEGER => _, GL_RGB_INTEGER => _, GL_RGBA_INTEGER => _, GL_SAMPLER_2D_ARRAY => _, GL_SAMPLER_2D_ARRAY_SHADOW => _, GL_SAMPLER_CUBE_SHADOW => _, GL_UNSIGNED_INT_VEC2 => _, GL_UNSIGNED_INT_VEC3 => _, GL_UNSIGNED_INT_VEC4 => _, GL_INT_SAMPLER_2D => _, GL_INT_SAMPLER_3D => _, GL_INT_SAMPLER_CUBE => _, GL_INT_SAMPLER_2D_ARRAY => _, GL_UNSIGNED_INT_SAMPLER_2D => _, GL_UNSIGNED_INT_SAMPLER_3D => _, GL_UNSIGNED_INT_SAMPLER_CUBE => _, GL_UNSIGNED_INT_SAMPLER_2D_ARRAY => _, GL_BUFFER_ACCESS_FLAGS => _, GL_BUFFER_MAP_LENGTH => _, GL_BUFFER_MAP_OFFSET => _, GL_DEPTH_COMPONENT32F => _, GL_DEPTH32F_STENCIL8 => _, GL_FLOAT_32_UNSIGNED_INT_24_8_REV => _, GL_FRAMEBUFFER_ATTACHMENT_COLOR_ENCODING => _, GL_FRAMEBUFFER_ATTACHMENT_COMPONENT_TYPE => _, GL_FRAMEBUFFER_ATTACHMENT_RED_SIZE => _, GL_FRAMEBUFFER_ATTACHMENT_GREEN_SIZE => _, GL_FRAMEBUFFER_ATTACHMENT_BLUE_SIZE => _, GL_FRAMEBUFFER_ATTACHMENT_ALPHA_SIZE => _, GL_FRAMEBUFFER_ATTACHMENT_DEPTH_SIZE => _, GL_FRAMEBUFFER_ATTACHMENT_STENCIL_SIZE => _, GL_FRAMEBUFFER_DEFAULT => _, GL_FRAMEBUFFER_UNDEFINED => _, GL_DEPTH_STENCIL_ATTACHMENT => _, GL_DEPTH_STENCIL => _, GL_UNSIGNED_INT_24_8 => _, GL_DEPTH24_STENCIL8 => _, GL_UNSIGNED_NORMALIZED => _, GL_DRAW_FRAMEBUFFER_BINDING => _, GL_READ_FRAMEBUFFER => _, GL_DRAW_FRAMEBUFFER => _, GL_READ_FRAMEBUFFER_BINDING => _, GL_RENDERBUFFER_SAMPLES => _, GL_FRAMEBUFFER_ATTACHMENT_TEXTURE_LAYER => _, GL_MAX_COLOR_ATTACHMENTS => _, GL_COLOR_ATTACHMENT1 => _, GL_COLOR_ATTACHMENT2 => _, GL_COLOR_ATTACHMENT3 => _, GL_COLOR_ATTACHMENT4 => _, GL_COLOR_ATTACHMENT5 => _, GL_COLOR_ATTACHMENT6 => _, GL_COLOR_ATTACHMENT7 => _, GL_COLOR_ATTACHMENT8 => _, GL_COLOR_ATTACHMENT9 => _, GL_COLOR_ATTACHMENT10 => _, GL_COLOR_ATTACHMENT11 => _, GL_COLOR_ATTACHMENT12 => _, GL_COLOR_ATTACHMENT13 => _, GL_COLOR_ATTACHMENT14 => _, GL_COLOR_ATTACHMENT15 => _, GL_FRAMEBUFFER_INCOMPLETE_MULTISAMPLE => _, GL_MAX_SAMPLES => _, GL_HALF_FLOAT => _, GL_MAP_READ_BIT => _, GL_MAP_WRITE_BIT => _, GL_MAP_INVALIDATE_RANGE_BIT => _, GL_MAP_INVALIDATE_BUFFER_BIT => _, GL_MAP_FLUSH_EXPLICIT_BIT => _, GL_MAP_UNSYNCHRONIZED_BIT => _, GL_RG => _, GL_RG_INTEGER => _, GL_R8 => _, GL_RG8 => _, GL_R16F => _, GL_R32F => _, GL_RG16F => _, GL_RG32F => _, GL_R8I => _, GL_R8UI => _, GL_R16I => _, GL_R16UI => _, GL_R32I => _, GL_R32UI => _, GL_RG8I => _, GL_RG8UI => _, GL_RG16I => _, GL_RG16UI => _, GL_RG32I => _, GL_RG32UI => _, GL_VERTEX_ARRAY_BINDING => _, GL_R8_SNORM => _, GL_RG8_SNORM => _, GL_RGB8_SNORM => _, GL_RGBA8_SNORM => _, GL_SIGNED_NORMALIZED => _, GL_PRIMITIVE_RESTART_FIXED_INDEX => _, GL_COPY_READ_BUFFER => _, GL_COPY_WRITE_BUFFER => _, GL_COPY_READ_BUFFER_BINDING => _, GL_COPY_WRITE_BUFFER_BINDING => _, GL_UNIFORM_BUFFER => _, GL_UNIFORM_BUFFER_BINDING => _, GL_UNIFORM_BUFFER_START => _, GL_UNIFORM_BUFFER_SIZE => _, GL_MAX_VERTEX_UNIFORM_BLOCKS => _, GL_MAX_FRAGMENT_UNIFORM_BLOCKS => _, GL_MAX_COMBINED_UNIFORM_BLOCKS => _, GL_MAX_UNIFORM_BUFFER_BINDINGS => _, GL_MAX_UNIFORM_BLOCK_SIZE => _, GL_MAX_COMBINED_VERTEX_UNIFORM_COMPONENTS => _, GL_MAX_COMBINED_FRAGMENT_UNIFORM_COMPONENTS => _, GL_UNIFORM_BUFFER_OFFSET_ALIGNMENT => _, GL_ACTIVE_UNIFORM_BLOCK_MAX_NAME_LENGTH => _, GL_ACTIVE_UNIFORM_BLOCKS => _, GL_UNIFORM_TYPE => _, GL_UNIFORM_SIZE => _, GL_UNIFORM_NAME_LENGTH => _, GL_UNIFORM_BLOCK_INDEX => _, GL_UNIFORM_OFFSET => _, GL_UNIFORM_ARRAY_STRIDE => _, GL_UNIFORM_MATRIX_STRIDE => _, GL_UNIFORM_IS_ROW_MAJOR => _, GL_UNIFORM_BLOCK_BINDING => _, GL_UNIFORM_BLOCK_DATA_SIZE => _, GL_UNIFORM_BLOCK_NAME_LENGTH => _, GL_UNIFORM_BLOCK_ACTIVE_UNIFORMS => _, GL_UNIFORM_BLOCK_ACTIVE_UNIFORM_INDICES => _, GL_UNIFORM_BLOCK_REFERENCED_BY_VERTEX_SHADER => _, GL_UNIFORM_BLOCK_REFERENCED_BY_FRAGMENT_SHADER => _, GL_INVALID_INDEX => _, GL_MAX_VERTEX_OUTPUT_COMPONENTS => _, GL_MAX_FRAGMENT_INPUT_COMPONENTS => _, GL_MAX_SERVER_WAIT_TIMEOUT => _, GL_OBJECT_TYPE => _, GL_SYNC_CONDITION => _, GL_SYNC_STATUS => _, GL_SYNC_FLAGS => _, GL_SYNC_FENCE => _, GL_SYNC_GPU_COMMANDS_COMPLETE => _, GL_UNSIGNALED => _, GL_SIGNALED => _, GL_ALREADY_SIGNALED => _, GL_TIMEOUT_EXPIRED => _, GL_CONDITION_SATISFIED => _, GL_WAIT_FAILED => _, GL_SYNC_FLUSH_COMMANDS_BIT => _, GL_TIMEOUT_IGNORED => _, GL_VERTEX_ATTRIB_ARRAY_DIVISOR => _, GL_ANY_SAMPLES_PASSED => _, GL_ANY_SAMPLES_PASSED_CONSERVATIVE => _, GL_SAMPLER_BINDING => _, GL_RGB10_A2UI => _, GL_TEXTURE_SWIZZLE_R => _, GL_TEXTURE_SWIZZLE_G => _, GL_TEXTURE_SWIZZLE_B => _, GL_TEXTURE_SWIZZLE_A => _, GL_GREEN => _, GL_BLUE => _, GL_INT_2_10_10_10_REV => _, GL_TRANSFORM_FEEDBACK => _, GL_TRANSFORM_FEEDBACK_PAUSED => _, GL_TRANSFORM_FEEDBACK_ACTIVE => _, GL_TRANSFORM_FEEDBACK_BINDING => _, GL_PROGRAM_BINARY_RETRIEVABLE_HINT => _, GL_PROGRAM_BINARY_LENGTH => _, GL_NUM_PROGRAM_BINARY_FORMATS => _, GL_PROGRAM_BINARY_FORMATS => _, GL_COMPRESSED_R11_EAC => _, GL_COMPRESSED_SIGNED_R11_EAC => _, GL_COMPRESSED_RG11_EAC => _, GL_COMPRESSED_SIGNED_RG11_EAC => _, GL_COMPRESSED_RGB8_ETC2 => _, GL_COMPRESSED_SRGB8_ETC2 => _, GL_COMPRESSED_RGB8_PUNCHTHROUGH_ALPHA1_ETC2 => _, GL_COMPRESSED_SRGB8_PUNCHTHROUGH_ALPHA1_ETC2 => _, GL_COMPRESSED_RGBA8_ETC2_EAC => _, GL_COMPRESSED_SRGB8_ALPHA8_ETC2_EAC => _, GL_TEXTURE_IMMUTABLE_FORMAT => _, GL_MAX_ELEMENT_INDEX => _, GL_NUM_SAMPLE_COUNTS => _, GL_TEXTURE_IMMUTABLE_LEVELS => _, *}
