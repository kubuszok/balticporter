...
object MeshBuilder {
  export com.badlogic.gdx.graphics.g3d.utils.MeshPartBuilder.{MAX_INDEX => _, MAX_VERTICES => _, createAttributes => _, indicesMap => _, tmpIndices => _, tmpVertices => _, transformNormal => _, transformPosition => _, vTmp => _, *}
