...
class JsonMatcher extends com.badlogic.gdx.utils.JsonSkimmer {
  ...
  def path(): java.lang.String = {
    ...
    { var i: scala.Int = 0; val n: scala.Int = this.path$field.size; while (i < n) { {
      if (i > 0) {
        (buffer.append: (scala.Char) => java.lang.Appendable)('/')
      } else ()
...
object JsonMatcher {
  export com.badlogic.gdx.utils.JsonSkimmer.{Match => _, Node => _, Pattern => _, Processor => _, `match` => _, array => _, capture => _, debug$field => _, keys => _, none => _, process$field => _, single => _, *}
