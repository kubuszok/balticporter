...
class PolygonTest extends balticporter.runtime.PortedSuite {
  testCase("testZeroRotation", {
    ...
    polygon.rotate(0)
    assertArrayEquals("The polygon's vertices don't correspond.", polygon.getTransformedVertices(), polygon.getVertices(), 1.0f)
  })
  ...
  testCase("testConcavePolygonArea", {
    ...
    assertEquals("The area doesn't correspond.", expectedArea, concavePolygon.area(), 1.0f)
  })
  testCase("testTriangleArea", {
    ...
    assertEquals("The area doesn't correspond.", expectedArea, triangle.area(), 1.0f)
  })
}