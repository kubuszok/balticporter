...
class OctreeTest extends balticporter.runtime.PortedSuite {
  testCase("testInsert", {
    ...
    balticporter.runtime.Asserts.assertTrue(octree.root.isLeaf())
    ...
    octree.add(box2)
    balticporter.runtime.Asserts.assertFalse(octree.root.isLeaf())
    ...
    balticporter.runtime.Asserts.assertTrue(result.contains(box1))
    balticporter.runtime.Asserts.assertTrue(result.contains(box2))
    balticporter.runtime.Asserts.assertEquals(2, result.size)
    octree.remove(box2)
    result.clear()
    octree.getAll(result)
    balticporter.runtime.Asserts.assertEquals(1, result.size)
    balticporter.runtime.Asserts.assertTrue(result.contains(box1))
  })
}