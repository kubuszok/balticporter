...
**Why a WORKTREE of the reference repo and not the live checkout.** The lane deletes files (the
...
the lane's to do. A disposable worktree at the commit `../sge` currently has checked out is
reproducible, idempotent (re-running re-creates it at the same commit), and disposable (it lives
under `.balticporter/dropin/`, which is gitignored and which the measure lanes already use for their
own captures).
