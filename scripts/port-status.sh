...
# last baseline date. Mechanism only; the wording is the `port-status` skill's.
cd "$(dirname "$0")/.." || exit 1
printf "report\tmodule\tjvm\tjs\tnative\tref-suite\ttests\tpass\tfail\tskipped\tfailing\tskipped-names\tdeclared-failures\tdeclared-lost\tlast-baseline\n"
for b in port-report/*/baseline; do
  ...
  printf "%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n" "$r" "$mod" "$jvm" "$js" "$nat" "$ref" "$t" "$p" "$f" "$s" "${fn:--}" "${sn:--}" "$df" "$dl" "$last"
done
