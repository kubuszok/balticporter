...
LIVE="LlsMigrate LlsDifferential LibgdxL0Migrate LibgdxL0TestMigrate DemoCheck"
printf "report\tmodule\tstatus\tjvm\tjs\tnative\tref-suite\ttests\tpass\tfail\tskipped\tfailing\tskipped-names\tdeclared-failures\tdeclared-lost\tlast-baseline\n"
for b in port-report/*/baseline; do
  ...
  printf "%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n" "$r" "$mod" "$st" "$jvm" "$js" "$nat" "$ref" "$t" "$p" "$f" "$s" "${fn:--}" "${sn:--}" "$df" "$dl" "$last"
done
