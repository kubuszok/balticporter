...
| `PrimitiveToOpaqueTransform(OpaqueSpec)` | seed, propagate along pure-move flows, retype (the scalar primitive AND `Array[Prim]`, whose coercion is an ERASURE identity — one container deep, because a deeper one has none), coerce at the boundary | which primitive, what the type is called, where it is minted, which declarations seed it — as a `Set[String]` of exact FQNs, because a PREDICATE has no rendering and the surface fingerprint could not see it (`ENGINE-LIMITS.md` §13 O4) — and how far propagation may reach. A dependent seeds ADDITIONAL declarations via `MergeablePolicy`: `hints` union, `target`/`underlying`/`fqn` must agree, `scope` composes as `NullabilityTransform`'s (`Everywhere` entries union, `Only` entries union, mixed refuses). A formal on a callee this run does NOT emit is read LITERALLY — `coerceArgs` checks `runScope.emits` and unwraps rather than wrapping, because the base emitted `Int` there regardless of what this run's propagation did to the TIR |
