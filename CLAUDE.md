...
| `CollectionsTransform(RuleScope, families, familyScopes, retarget, retargetRewrites, retargetRewritesByDesc, retargetTypeArgs, retargetCoercions, reifiedCarriers, reflectiveSinks)` | retype collections and API-map their call sites — the JDK table is a §1(a) constant in the companion; `families` adds per-library entries alongside it, each getting the same kind-aware call rewrites and boundary counts; `familyScopes` is per-ENTRY `RuleScope` (D12). `retarget` retypes a library type that is usable wherever the java source was (the scala target extends or implements the java source, so no coercion is needed — unlike `families`, which carries kind-aware rewrites and boundary coercions); `retargetRewrites` maps per-member call-site rewrites keyed `(name, arity)`, `retargetRewritesByDesc` the same keyed `(name, Descriptor)` for overloaded members (descriptor wins over arity); `retargetTypeArgs` maps arity-changing retargets (`SourceArg`/`FixedType`); `retargetCoercions` maps `(actualHead, expectedHead)` pairs to template expressions for boundary coercions between retarget targets and shims. `RetargetBoundaryCheck` (`collection-retarget`) counts the direction a subtyping argument does not license — a value the JDK PRODUCES at a type this phase retargeted — with kinds `ExternalProducer`, `CastToTarget`, `IteratorRemove` | WHICH declarations (`scope`); WHICH ADDITIONAL collection types to retype (`families`); WHICH declarations each family entry reaches (`familyScopes`). WHICH library types to retarget (`retarget` — java FQN to scala FQN; empty = no-op); WHICH members to rewrite at call sites (`retargetRewrites`/`retargetRewritesByDesc` — nine variants: `Rename`, `BoolDispatch(flagIndex, onTrue, onFalse)`, `Construct(companion, factory, fillTypeArgs, dropTrailing)`, `ForEach(via, arity)`, `Collect(via, into)`, `Chain(members, parens, dropArgs)`, `FieldWrite(field, method)`, `IndexedField(field)`, `Template(expr)` with `$recv`, `$0`..., `$T0`..., `$Target` holes); WHICH type argument mappings (`retargetTypeArgs`); WHICH boundary coercion templates (`retargetCoercions`); WHICH external generics are reified carriers (`reifiedCarriers` — K20); WHICH external types are reflective sinks (`reflectiveSinks` — K21). A dependent ADDS families and retarget entries via `MergeablePolicy`: independent keys union, same source with a different target refuses, scope disagreement on the same source refuses |
| `PrimitiveToOpaqueTransform(OpaqueSpec)` | seed, propagate along pure-move flows, retype (the scalar primitive AND `Array[Prim]`, whose coercion is an ERASURE identity — one container deep, because a deeper one has none), coerce at the boundary | which primitive, what the type is called, where it is minted, which declarations seed it — as a `Set[String]` of exact FQNs, because a PREDICATE has no rendering and the surface fingerprint could not see it (`ENGINE-LIMITS.md` §13 O4) — and how far propagation may reach. A dependent seeds ADDITIONAL declarations via `MergeablePolicy`: `hints` union, `target`/`underlying`/`fqn` must agree, `scope` composes as `NullabilityTransform`'s (`Everywhere` entries union, `Only` entries union, mixed refuses). A formal on a callee this run does NOT emit is read off the BASE'S PUBLISHED PORT MAP (`RunScope.baseMemberUpstream`, one reader: `PortMap.discover`) — wrap where the base retyped it, unwrap where the base emitted the primitive — never off this run's own propagation over base units and never "literally" off the java: the first re-derives what the base already published (D12), the second unwrapped at every base-retyped formal and regressed four dependents at once (`ENGINE-LIMITS.md` O8, wave 2.11) |
...
| `NullabilityTransform(annotations, target, scope, nullableMembers)` | move a nullability annotation into the type — `T \| Null`, `W[T]` (named wrapper), or `Option[T]` — strip the annotation, coerce at every slot seam, rewrite `== null` / `!= null` | WHICH annotations mark nullability (FQN set), WHICH target shape (`Union` default, `Named(fqn)` closes K13, `OptionTarget`), a `RuleScope` for opt-out (empty annotations = no-op), and `nullableMembers` — exact member FQNs (`Class#member`) matched at run time against `Symbol.fullName`, treated as if their return/field type carried a configured annotation: same target, same coercions, same override propagation, same boundary count. Empty = no-op; non-empty contributes a fingerprint segment (omitted when empty). `MergeablePolicy` union. `.conf` key `nullableMembers = [...]`. `ENGINE-LIMITS.md` K13.6 |
| `BeanPropertyTransform(pairs, targets, scope)` | rename a java accessor pair into a scala property — `getX`/`setX` → `x`/`x_=`, over the whole override component or refused — and DERIVE the pairs java's own convention identifies wherever the scope says to | WHICH pairs are spelled explicitly (and at which `target`), and WHICH types are DERIVED over. Empty pairs + `Only(Set.empty)` = no-op; a configured key WINS at the same property or accessor |
| `NullaryArityTransform(scope)` | drop `()` from a getter-like nullary method — `def x(): R` → `def x: R` — and rewrite every call site, whole-or-none per override component | WHICH declarations, as a `RuleScope`. `Only(Set.empty)` = no-op (§1's ADD rule — the phase MINTS an arity, so its unrestricted form is not a safe default); sge's convention is EMPIRICAL and the predicate is structural and narrower than it |
| `PortManifest.externalParenless` | emit calls to listed external members WITHOUT `()` — a call without parens is legal against both a Java `getX()` (Scala 3 auto-applies) and a Scala `def getX` (parenless). Closes the cross-platform arity gap where a JS/Native platform shim declares a member parenless while the JVM class file has `()` (P11: munit's `Description`). NOT a phase — a `PortManifest` field read by `TirEmitter.applyStr0` | WHICH external members, as exact FQNs (`Owner#member`). Empty = no-op. NOT inherited (a classpath fact). Does NOT implement `SurfacePolicy` (reaches calls only, not emitted signatures). `.conf` key `externalParenless = [...]` |
| `ClassToTraitTransform(specs)` | rewrite a nominated abstract class into a trait: constructors removed from the TIR, mapped constructor parameters become abstract `val` members, `isTrait` set; every DIRECT subclass (named and anonymous `new`) gains `override val` members bound to its `super(args)`, and multi-root super-calling constructors are rewritten into `this(...)` delegations targeting the widest so the funnel promotes it as primary. The nominated type's `ClassDef` is transformed in the TIR even when emission drops it in favour of an injected file, because the `CtorFunnel` then sees a trait parent with no constructor — nothing to replay, nothing to widen. `.conf` key `class-to-trait` | WHICH types to transform and HOW their constructor parameters map to abstract vals — `Map[fqn, List[ParamMapping(index, valName)]]`. Empty map = no-op. `SurfacePolicy` (the parent's kind and the subclass's members are emitted surface). `MergeablePolicy`: independent keys union, same key with different param mappings refuses. `.conf` key `specs { "com.foo.Pool" { params = [ { index = 0, name = "initialCapacity" } ] } }` |
| `AddMembersTransform(members)` | append hand-written Scala members to a mechanically-translated class at the end of its body — the seam for hand-port-added API that `inject` (whole file) and `MethodBodyTransform` (body replacement) cannot express. Each member is verbatim Scala text spliced at statement position; the target compiler is the gate. `Decision.Kind.AddedMember` with `PorterNote.InBody` placement | WHICH owners (upstream FQN) and WHAT members (name, arity, source text, `Reason`). Empty map = no-op, fingerprint segment omitted when empty. `SurfacePolicy` (added members are emitted surface). `MergeablePolicy`: independent owners union, same owner+name refuses. ADD-scoped `Only(Set.empty)` default — the phase MINTS members, so its unrestricted form is not a safe default. `.conf` key `add-members` with `members { "com.foo.Engine" = [ { name = "register", arity = 2, source = "def register..." } ] }` |
...
| `int x = intValue` at a `float` slot — assignment, argument, return | `x` (relying on `int2float`) | Scala's implicit `int2float`/`long2float`/`long2double` are deprecated since 2.13.1 because they lose precision; java widens implicitly (JLS 5.1.2) and every slot compiles cleanly with a deprecation warning that `-Werror` promotes to an error | `.toFloat`/`.toDouble` at the slot, emitted by the FRONTEND's `coerce` function (`ENGINE-LIMITS.md` F10) |
...
| a rename onto a stdlib member that RETURNS instead of MUTATING — `Bits.xor(bits)` (in-place `^=`) | `bitSet.xor(other)` | java's `Bits.xor` xors IN PLACE (mutates `this`); Scala's `BitSet.xor` RETURNS a new set. The port compiles, the original set is unchanged, and the test that checks the original fails silently | the in-place operator (`bitSet ^= other`) or a Template that calls the right method. Read the MUTABILITY of the source member and emit the in-place form |
| an INCLUSIVE range bound translated to an EXCLUSIVE API — `removeRange(start, end)` where java's `end` is inclusive | `target.removeRange(start, end)` | java's `end` is the last index to remove (inclusive); the scala target's is the first index to KEEP (exclusive). `removeRange(2, 4)` removes 3 elements in java and 2 in the port — a silent off-by-one. Compile clean, 0 check counts moved | translate the bound (`end + 1`), and restate java's own REFUSALS (`end >= size`, `start > end`) because the target may silently accept an inverted range as a no-op |
