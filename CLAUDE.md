...
| `CollectionsTransform(RuleScope, families, familyScopes)` | retype collections and API-map their call sites — the JDK table is a §1(a) constant in the companion; `families` adds per-library entries alongside it, each getting the same kind-aware call rewrites and boundary counts; `familyScopes` is per-ENTRY `RuleScope` (D12) | WHICH declarations (`scope` — a bridge that must keep the JDK shape opts out); WHICH ADDITIONAL collection types to retype (`families` — java FQN to (scala FQN, Kind), merged into `typeMap` at construction; JDK default = empty, no-op); WHICH declarations each family entry reaches (`familyScopes`). A dependent ADDS families via `MergeablePolicy`: independent keys union, same source with a different target refuses, scope disagreement on the same source refuses |
