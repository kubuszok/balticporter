...
| `@Rule ExpectedException thrown` + `thrown.expect(E.class)` | the field emitted, nothing applying it | the row above at JUnit's OTHER spelling, and one step LOUDER: the expected throw propagates and the harness records a FAILURE where java recorded a pass — 37 of one suite's 40 failures, every one the port being RIGHT about the library. A general `@Rule` really has no image; THIS one's contract is written down (`ExpectedExceptionStatement.evaluate`), so it is a fact about the two frameworks like every row here | `intercept[E] { the REST of the test }` — and the guard is the whole of it, because java arms the rule to the END OF THE TEST and `intercept` wraps a LEXICAL REGION. Convert at STATEMENT POSITION in the test body and refuse everywhere else, one counted row per declined site: wrapping the rest of an enclosing LOOP BODY fails where java ran the next iteration, and wrapping the loop fails to fail where java did (`ENGINE-LIMITS.md` X5) |
