...
| `MemberRenameTransform(renames, derive)` — rename over the whole override component or refuse | which members, what name (symbolic emits `@targetName`); `derive` reads the reference's `TargetName` and underscore `FieldName` rows |
