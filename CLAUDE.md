...
```
grep -rn --include='*.scala' -E "badlogic|libgdx|liqp|liquid\.parser|earlygrey|simplegraphs|dongbat|jbump|czyzby|noise4j|tommyettinger|anim8|crashinvaders|eskalon|mgsx|vladsch|flexmark|nibor|fasterxml|antlr|strftime" balticporter/api balticporter/engine balticporter/frontend-spoon balticporter/runtime | grep -vE ":\s*(\*|//|/\*)"
```
...
| a test class's INSTANCE FIELD with its own INITIALISER — `private final BehaviorTree<E> t = new BehaviorTree<>()` | the same field on the converted suite | the row above covers the SETUP and not the CONSTRUCTION. JUnit rebuilds the whole test object per `@Test`, so a field initialiser runs N times; MUnit's suite is ONE instance and it runs ONCE, so every test after the first inherits the last one's state. Discharging `@Before` correctly is what makes this VISIBLE, not what causes it — the state java rebuilt is the constructor's, and no `@Before` translation reaches it. Green compile, 0 skipped, every count flat, `outcomes N of N emitted`: only RUNNING the suite sees it. Measured at **4 of one suite's 10**, all four the same `IllegalStateException` from a tree that already had a root | per-test CONSTRUCTION of the suite's instance state. **OPEN** — the blast is every converted suite in the corpus, so it is a wave of its own: `ENGINE-LIMITS.md` X4 |
