...
| `static { … }` on a class | `locally { … }` in the companion `object` | the block is EMITTED and never RUNS. Java initialises the CLASS on the first `new`, the first static access or a subclass's init (JLS 12.4.1); Scala initialises the OBJECT when something touches the OBJECT, and `new Template(…)` does not. Where the block REGISTERS something — an SPI provider, a factory, a codec — every later lookup silently answers "not registered", which a library then turns into a plausible wrong answer rather than an error. 5 test failures on liqp, 0 compile errors, every count flat | reproduce JAVA'S OWN TRIGGER LIST, item by item, and nothing else: `val _ = <Type>` at the head of the CLASS body — ahead of every field initialiser, where java ran `<clinit>` relative to them — which also carries a subclass's `new S` through the super constructor. A static ACCESS needs nothing (`T.member` already touches the object) and a CONSTANT needs nothing (the row above inlines it, exactly as java does — which is also why this cannot re-enter that cycle: the repair adds a trigger where java HAD one and none where java has none). A subclass's own `S.member` read is the second broken item and reflection is the third: the first is closed, the second counted, the third refused — nothing in the program can see a reflective load living in its CONSUMER. **NOT from every use.** liqp `567 -> 572` passing, 8 -> 3 failing (`ENGINE-LIMITS.md` K22, catalog `JS-C07`) |
