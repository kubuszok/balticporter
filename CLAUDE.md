...
details live in a path-scoped file under `.claude/rules/` (the `details:` line), which carries the
measured evidence. User documentation is `docs/user-guide/` (mkdocs), contributor documentation
`docs/contributing/`.
...
- The scope, and any parameter a reader could point at in the emitted signatures, is SURFACE: the
  phase implements `SurfacePolicy`, or two configurations fingerprint EQUAL by NAME. **An
...
- **Every seam a scope creates is COUNTED**, read through the DECLARATION never node types, at
  external callees JDK or third-party alike; a formal that stays JAVA is read LITERALLY; a
  coercion may never precede a rewrite of the same call; the residue's `Rewrite.accountedBy` lanes
  are declared and `rewrite-callsites` reports a phase naming none.
- An obligation the engine's own translation created is the PHASE's, never a port's drop key;
  a re-parenting owes the `override` modifier it moved; a repair asks whether the languages
...
- A class a FRAMEWORK instantiates has no caller to change: a constructor-changing phase owes
  *takes the value without a parameter*; a refusing escape hatch is asked *is there no value,
  or no NAME?*, indexed by where the mechanism ATTACHED.
- Collections/retarget obligations (boundary directions, the third population, reified positions,
  egress sinks): `.claude/rules/collections.md`.
...
the abstraction, `runtime/` refuses it, and only a BASE can ship a support type.
...
| `typeRenames`, `subPackages`, `flattenNestedTypes`, `allowPackageSplit` | **`inject`**, **`platformDirs`** (`jvm`/`js`/`native` → `src_managed/<row>/scala`, only that row compiles it), `rowSources` (a row's upstream java shadowing a main type on that row; surfaces must agree), `providedSources` (read like `inject`, never copied), `targets`, `verdictOverrides`, `dependencies` |

- `targets`: a dependent may narrow, never widen (fatal; widen the BASE instead). All-platform is the
  stated intent; a `Verdict.Depend` is answered by DECLARING the artifact, never a rewrite.
...
- No dependent key may edit what a base EMITS (`SurfaceIntrusion`), screened over the whole
  `subjects` against the base's PUBLISHED MAP, never its `governs` claim.
- A dependent's retyping phase scopes on the ENTRY, and so does a BASE'S — an inherited
  `Everywhere` decides the dependent's own steps, except a replacement for an INHERITED drop,
  which is inherited with it; a base names its own types by dotted `typeRenames`, never a package
  claim, and DECLARES the split (`allowPackageSplit`: the type and its package-private members ship
  public); a member the base RETYPED that the dependent OVERRIDES forces the same step onto the
  dependent's entry (2 erasure clashes); a DROPPED type's members are read literally; a DERIVED
  surface is published and compared as `base-surface`; a dependent FOLLOWS the base's member renames.
- **A base's SOURCE SET is decided before its policy**, against the REFERENCE port's own ownership:
  ...
  out of the overlap — a base never resolves against its dependent.
...
RISES, serially — read `errors.tsv`'s MEMBER column, never the headline. A green compile says
...
- **An idiom transform's safety argument is a REFUSAL ENUMERATION, never a suite result**: every
  ...
  CLASSIFIED, not minimised, and the unattributed residue is EMPTY.
- **"Refuse loudly" is a claim about the emitted text**: where the untranslated form is also valid
  Scala, the refusal is COUNTED when written. A repair at a USE cannot discharge the
  DECLARATION's obligation. A new arm inherits its node's catalog obligations.
...
| a skill | a PROCEDURE, triggered by its description — a task shape an agent will meet again (see the index below). One a CONSUMER repository needs lives in `plugin/skills/` (shipped as the Claude Code plugin); an engine-development one in `.claude/skills/` |
...
numbered entry in a document nobody loads explained nothing to the reader who met its id.
...
somewhere the engine ships** (this file, a rule file, a plugin skill, an agent definition, the
documentation), in the commit that measures it; **every check and rule line says which of §1's three kinds
...
| `break L` / `continue L` | *nothing* | a labelled jump crosses nested loops and switches by definition | a NAMED boundary on the labelled statement. For `break L` the name is a `ControlThrowable` SENTINEL (`val brk$n = new ControlThrowable {}; try { loop } catch { case $e: ControlThrowable if $e eq brk$n => () }`, the jump `throw brk$n`): Scala.js lowers a named `boundary.break` across nested `while`s to a JS `break` of the INNERMOST loop (TimSort's merge read index -1). `continue L` keeps `break(())(using cnt$n)` on the BODY boundary |
...
| any boundary the emitter INTERPOSES | an un-annotated `break(())` under it | `boundary.break` with no `using` resolves the INNERMOST `Label`, so a new boundary silently steals the enclosing loop's jumps | name the enclosing boundary whenever anything inside the body renders with a boundary of its own (a named LOOP boundary is the sentinel above); over-approximate, an unused name costs one identifier |
...
`balticporter.catalog.Differences` is the complete set (`just catalog`).
...
the enclosing SCOPE. **A name clash and an implementation pair look identical** — decide from
the emitted SHAPE. A loose `(name, arity)` key (a `Map` or a `find`) indexes to a `List` and the
CALLER decides; `OverrideGraph.overridden` is FILTERED, never re-derived. A promotion moves
a NAME, never a POSITION (JLS 12.5 step 4 incl. BLOCKS, then 5). details: `.claude/rules/emitter.md`
...
symbol's SIGNATURE is a class-file fact no phase may move**. Cut only at a `Symbol.fullName`
...
of the ANCESTRY; a `match` arm under a supertype arm is dead — one `TypeShape.of` derivation,
answers at the callers; a guard whose evidence can be MISSING states the REFUTATION; a
node-kind test owes every syntax java has (`C::new`); an instrument's filter states the
COMPLEMENT; the survivors are not the DECLARATION (`permits`); a synthesised member carries the
parent's scope through one `ParentSubst`; a refusal lane is a POPULATION and a
two-direction predicate needs two functions. details: `.claude/rules/emitter.md`
...
comment, one home (a CLAIMED identity set; the positional header harvest FIRST); a consumed
...
the SUPERCLASS ctor's); read the parser's synthesis only where a fixture has shown the two
...
- A doc comment states the CONTRACT in at most 5 lines, in plain words, with the why as ONE clause.
  No internal ids, no section numbers, no history, no measurement narrative, no quotation of
  another document in code. The same holds for a finding, a refusal message and a test name.
