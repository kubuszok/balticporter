...
| `type-redirect` | `redirects { "a.B" = "c.D" }`, or the same entry as an object: `"a.B" = { to = "c.D", memberRenames { m = "n" } }` | re-point every reference to one type at another. A `memberRenames` key is a member SEGMENT under its owner — `dispose`, or `dispose()` for one overload — and renames every declaration of that member's override COMPONENT before the redirect, so a target that spells the member differently (`Disposable#dispose` → `AutoCloseable#close`) is expressible. Both entry shapes live in one map; the flat one is not legacy. A rename to a name a KNOWN target does not declare, or one whose component reaches a declaration this program cannot move, is REFUSED and counted — never half-applied. The object form also takes `scope { only = [...] }`: only declarations inside it are retyped, which lets a DEPENDENT retype its own uses of a base-emitted type without an intrusion; a call on a receiver such a redirect moved binds to the TARGET's member, so the base's published renames of the source member do not follow it (declare a parenless target member in `externalParenless`) |
| `method-body` | `bodies { "a.B#m()" = "{ … }" }` | keep the signature, replace the body |
| `call-site-substitution` | `calls { "a.B#m(int)" = "<template>" }`, `scoped = [ { call, template, scope { only = [...] } } ]` | replace a CALL with a template naming `{recv}`, `{this}` (the instance of the nearest named class whose member encloses the call — `Outer.this` inside an anonymous class; a call in a static member is refused and counted; never a literal `this`), `{arg0}`…; a literal brace is `{{`/`}}`. Holes are spliced where the template names them — bind `{recv}` to a local first when the argument must come first. A `scoped` entry rewrites only calls whose enclosing declaration its scope admits; per call the most specific scope wins (member over type over package over the unscoped `calls` entry) and an equal-specificity tie is refused and counted. Scoped outside a base's namespace, a dependent may rewrite calls of the base's members without an intrusion. Runs before `type-redirect` |
...
| `primitive-to-opaque` | `fqn` **required**, `underlying` (default `Int`), `extraHints = […]`, `scope`, `derive = true` (seed from the manifest's `parity` reference), `carriers = […]` (one-type-parameter wrapper FQNs, e.g. the null model's, whose element may be the primitive or its boxed form — retyped to `Carrier[Opaque]`, coerced through the carrier's `map`, one level only); **`hints` REFUSED** | see below |
| `bean-properties` | `pairs`, `targets`, `scope`, `derive = true` | accessor pair → property over the override component; `derive` folds a pair the reference spells as a `var`/`val` (fluent setter included) and leaves a pair alone where the reference port keeps the java accessor name |
...
| `type-class-params` | `rules = [ { members = [...], typeClass = "com.foo.Factory", create = "create", instantiators = ["a.Reflect#newInstance"], spelling = "type-argument" \| "keep-parameter", classValue = "refuse" \| "class-tag" \| "member:<name>", handles = [...] } ]` | a `Class<T>` parameter used to CONSTRUCT a `T` reflectively becomes `(using F[T])` on YOUR type class; every construction (`c.newInstance()`, `c.getConstructor().newInstance()`, a listed instantiator `f(c)`) becomes `summon[F[T]].create()`, so no reflection runs on any platform and a type with no instance is a compile error. The type class and its compile-time derivation are yours to write (the user guide shows one). `keep-parameter` keeps `m(classOf[X])`; `type-argument` spells `m[X]` and answers other reads of the class per `classValue`. Whole override component or none; a `Class` value at a type-argument member, a constructor with arguments, an uncovered read, or a `try` whose handlers `handles` does not name is refused or counted as a `policy` finding. Remove any `registry` entry over the same callee |
| `visibility` | `widen = [...]` (`C#m`, `C#<init>(desc)`), `derive = true`, `narrow = [...]` (`C#m`), `deriveNarrow = true` | `widen` ships the listed members public where java declared them narrower; `narrow` ships a java `protected` member (and every override of it) as plain `protected` instead of `protected[pkg]`, refused and counted where a non-subclass or a foreign receiver references it; the `derive` switches take the reference port's `Public` / `Protected` rows |
...
| `add-members` | `fromReference { "<upstream owner>" = "name, name" }` (verbatim from the manifest's `parity` reference), `members = [ { owner = "com.foo.Engine", specs = [ { name = "register", arity = 2, source = "def register[T](k: Class[T], f: () => T): Unit = ???", why = "factory registry" } ] } ]` | append hand-written Scala members to a mechanically-translated class. Each member is verbatim text spliced at the end of the owner's body. Empty map = no-op. ADD-scoped `Only(Set.empty)` default. `SurfacePolicy` + `MergeablePolicy` |
