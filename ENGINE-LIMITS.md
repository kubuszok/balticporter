...
Third face (2026-09-06): a replayed SUCCESS prints nothing and reads as 0 — the lls port read 0 on JVM/JS/Native across four promotions while a forced real compile reads 11 `E164` (K39); the ladder ports (`port-lls`, `port-sge-l0`) now carry the per-run nonce too.
...
### K39. The DIAMOND FORWARDER sees only INJECTED parents, never a CLASS FILE's DEFAULT method — **11 `E164` rows on the ladder's L0 rung, 0 on every other port. OPEN**
...
Rule: the exit is NOT "add the JDK interfaces to `externalConcrete`" — minting a forwarder wherever an external interface MIGHT carry a default changes emitted text at every class-plus-interface parent pair in the corpus. The exit is the frontend interning an external PARENT's concrete instance members off the class file (`FrontendConfig.internTypes`' mechanism) so the emitter ASKS instead of guessing, measured `members.tsv`-flat on gdx/liqp/textra/md before it lands.
