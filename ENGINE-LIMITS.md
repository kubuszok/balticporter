...
| M (measurement) | M5.12, M10 | — | Metals BSP-connect timeout; a diagnostic id keyed on a mint counter (fix belongs in `Minter.table`, unbuilt) | no card yet |
...
Post-body through parameters (item 4, 2026-09-04): the child's synthesised primary carries the parent secondary's post-body inputs as parameters, guarded by a null check; each secondary evaluates the argument once in its `this(...)` call. The old inlining and its double-evaluation guard are removed. gdx 0 with 95 moved members (30 classes gaining synthesis), omissions 43 -> 45 (DistanceFieldFont: `region` used 2x in delegation head); VisWindow still E134 — `formalsOf` reads `NoType` for a nested-class param, blocking synthesis.
...
Triage (2026-09-04): REFUSED-BY-DESIGN — family P: no shared registry abstraction covers all three reflective-instantiation ports at once (a null-vs-throw miss-policy conflict breaks a single-parameter table) — refused pending a 4th BASE-port instance read only one way
