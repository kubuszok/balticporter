...
### P10. "Reflective instantiation becomes a REGISTRY" is a shape three ports share — refused as a SHARED SUPPORT TYPE, BUILT as a phase that MINTS one INTO each port. **`RegistryTransform` (§1b) SHIPPED; ashley `portability(injected)` 4 -> 0, `registry(jvm-only-miss)` 0 -> 2, 0 errors and 108/2/2 held**
...
Next (wave 2): gdx-gltf `GLTFMaterialExporter#ext` and gdx-ai `Task#cloneTask` + the `BehaviorTreeParser` reflection family are still `MethodBodyTransform` keys (`PROGRESS.md` §13.27); `Task#cloneTask` is a `registry(self-clone)` refusal under a non-reflective `miss` and needs the library's own clone contract, not this mechanism.
