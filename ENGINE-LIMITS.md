...
### K52. A java `static final` SCRATCH instance is one object every thread shares — CLOSED 2026-09-16: `ThreadConfinedStaticsTransform(fields)`
(b) engine, parameterised (which fields). Symptom: `Matrix4`'s ten scratch statics (`l_vex`/`l_vey`/`l_vez`…) were emitted as companion `val`s; two threads inside `setToLookAt` corrupt each other's basis (sge ISS-832, `Matrix4ConcurrentLookAtRedSuite`). The reference confines each to a `ThreadLocal`.
Numbers: L0 10 decisions = 10 fields, 0 refusals; `class-init-trigger 2->0` (the allocations moved inside `initialValue`); lls/L0 errors unchanged.
Rule: the holder is `new ThreadLocal[T] { override def initialValue(): T = e }` as REAL TREES — never text, never `ThreadLocal.withInitial`: the Scala.js javalib ships the class and `initialValue`/`get` and NOT the static factory (the reference port's own ISS-832 note; it links on JVM and Native only). Four counted refusal guards: not static, not final, assigned after initialisation anywhere, initialiser not a fresh allocation.

### K53. A java STATIC the reference spells as an EXTENSION member derived no slot — CLOSED 2026-09-16: the receiver is parameter 0
(a) engine, in `ReferencePolicy`/the parity parser. Symptom: with derive-only seeds ("a DERIVED seed is exact, nothing grows from it"), `Align.isCenterVertical(int)` kept its `int` formal because the reference spells it `extension (a: Align) def isCenterVertical` — arity 0 under the object, receiver unrecorded — and the ladder emitted `Align.isCenterVertical(Align.toInt(align))` where the full port (hand hints, growth) emits `Align.isCenterVertical(align)`.
Numbers: `derived(rows) 6713->6719` (the six `Align` statics); sge core 24 -> 14 errors on the way to 0.
Rule: `SurfaceDecl.receiver` carries an extension group's receiver; a java static one parameter short of its twin reads the receiver as slot 0 (`slotTypes`) — the same reading `StaticForwarderTransform` gives a wrapper's statics. An instance method is never read that way.

### K54. A member rename on an EXTERNAL redirected type bound nothing, and the rename it needed refused on the anchor it replaced — CLOSED 2026-09-16, five layers
(b) engine (`TypeRedirectTransform.memberRenames`) + port policy. Symptom: `Comparable -> Ordered` with `compareTo -> compare` (sge's `Attribute`/`Attributes`/`TextureDescriptor`, their migration notes) silently did nothing: `bindMembers` binds `Owned`, and `java.lang.Comparable` declares nothing this run owns; `MemberRenameTransform` keys refused instead ("the component reaches java.lang.Comparable#compareTo"); a scope on the redirect had to cover the whole override component (`attributes.*`), the hits had to honour that scope (`VertexAttributes` keeps `Comparable`), and `Attributes`' other parent `java.util.Comparator` — deliberately absent from the closed platform table — anchored the rename too.
Numbers: sge g3d 11 -> 14 -> 14 -> 14 -> 1 -> 0 over five publish/regenerate round-trips (~1 h); L0 cannot see it (no `Comparable` class in its subset).
Rule: for a source type the run does not declare, the hits are every owned member whose override closure carries `(source, member)` as an external anchor, WITHIN the redirect's scope, and each request carries `detachedParents = Set(source)` — the redirect detaches that parent, the licence `MemberRenamer` already reads. A second unknown parent's surface is the PORT's to state (`TypeRedirectTransform(external = ExternalSurface.default ++ …)`), never a new closed platform row (`OverrideGraphSpec` records why). Walk such a policy on a testkit spec first (skill `iterate-lane`, step 5).

### K55. A java BOXED primitive at a nullable slot stayed boxed under the wrapper — CLOSED 2026-09-16: `Nullable[Int]`
(a) engine, `NullabilityTransform`. Symptom: `Cell`'s 16 `Integer`/`Float`/`Boolean` fields emitted `Nullable[java.lang.Integer]` etc.; the reference spells every one `Nullable[Int]` — java boxed them only to admit `null`, which the wrapper now carries. sge's `TableCellTest` had been patched to `getOrElse(java.lang.Float.valueOf(-1f)).floatValue()` over it.
Numbers: the 16 slots are all `Cell`'s; engine specs +2 (`NullabilityBoxedSpec`); no lane count moved.
Rule: under a NAMED or `Option` target the element is the primitive (`BoxedToPrimitive`); the union target keeps the box (`Int | Null` is no reference type). A wrapper over the box and over the primitive are ONE slot (`sameSlot`): a call whose recorded type is still the boxed spelling is not ascribed at the unboxed slot (the carrier spec caught the `.asInstanceOf`). Related: a field's derived `NullableMember` row is keyed `fullName:field` so it never retypes a same-named METHOD (`Cell#colspan(int)` returns `Cell[T]`); a QUALIFIED private reference member (`private[ui] var _isDisabled`) is a derivation input (`FieldName`, `KeepName`) and not a compared surface.
