...
(a) engine — a mechanism that does not exist, not a policy anybody can write. Symptom: a `private static final Actor BLOCKER = new Actor();` field beside a `static { BLOCKER.addListener(...); }` block; deferring the FIELD via `sites=LazyInit` DOES fire (contradicting a prior recorded reason) — but the error count doesn't move, it RELOCATES from the field to the `<clinit>` block that reads it, since java runs the field initialiser and the block as ONE JLS step-9 sequence and `lazy-init` can only defer the `ValDef` half.
