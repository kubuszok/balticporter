...
| M (measurement) | M5.12, M5.13, M10 | — | Metals BSP-connect timeout; a diagnostic id keyed on a mint counter (fix belongs in `Minter.table`, unbuilt) | no card yet |
...
### P10. "Reflective instantiation becomes a REGISTRY" is a shape three ports share — refused as a SHARED SUPPORT TYPE, BUILT as a phase that MINTS one INTO each port. **`RegistryTransform` (§1b) SHIPPED on ALL THREE; ashley `portability(injected)` 4 -> 0, gltf's injected factories file DELETED, ai `registry(jvm-only-miss)` 0 -> 2 + `registry(guarded-call)` 0 -> 1 and its hand suite 103/2 -> 104/1; 0 errors held everywhere**
...
Two STOPS wave 2 MEASURED, each a missing shape and not a missing value:
(a) §1(b) ENGINE — `Miss` has no arm that REFLECTS and restates java's own THROW. `Miss.JvmReflect` answers `null` for a type with no nilary constructor where `ClassReflection.newInstance` raised `ReflectionException`; ashley's java `catch` returned null (so eliding was exact), ai's wrapped it in `TaskCloneException` (so `handles` must stay empty and the delta is a counted `registry(guarded-call)`). Measured by the hand suite: `cloneTask clones guard` now PASSES (103/2 -> 104/1, 93/2 -> 94/1 on the differential), `cloneTask creates independent copy` fails with an NPE from `copyTo(null)` because the test's `CountingTask` takes a constructor argument. `Miss.Throw` is NOT the answer — it turns the site into a `registry(self-clone)` refusal, and then the dropped `ClassReflection` does not compile.
(b) §1(b) ENGINE — `newInstance(forName(s))` needs BOTH tables and `ClassTableTransform` cannot be aimed: it takes no `RuleScope` and is no `MergeablePolicy`, and libGDX core already binds `ClassReflection#forName` to `AssetTypeRegistry#classFor` for the whole program, which a dependent may neither narrow nor re-point. So gdx-ai's `BehaviorTreeParser$DefaultBehaviorTreeReader#openTask(String,boolean)` STAYS a `MethodBodyTransform` body. `findMetadata`/`getField`/`setField`/`castValue` are the `TaskField` (c) and were never P10.
Next: (a) then (b); a fourth `Miss` before a fourth port.
...
Do NOT retry: a registry keyed on anything but `Class[T] -> () => T`; a concurrent table (`runtime` ships no threading); pricing an extraction by LINES SAVED in the ports that already have one; `Miss.Throw` at a SELF-CLONE (it refuses the site and the dropped callee then does not compile).
