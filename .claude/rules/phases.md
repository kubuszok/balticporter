...
- An opaque type at a java constants class's OWN name (`OpaqueSpec.Target.OwnClass`) keeps javac's
  constant inlining only as `inline def c: N = <literal>`: scalac rejects `inline val` at an opaque
  type, a typed `final val` initialises the object on read (measured 0 -> 1 init), and the minted
  coercions are `inline` for the same reason. An extension named like an `Any` member (`toString`)
  compiles and is never selected (`a.toString` answered the Int's), so such a static stays a plain
  member, counted (`ExtensionDeclined`); inside the object the opaque IS the primitive, so calls
  there keep the prefix form `N.m(a)(rest)`. A case label needs a stable `final val` (counted).
...
| `TypeClassParamsTransform(rules)` — a `Class<T>` parameter used to CONSTRUCT a `T` becomes `(using F[T])` on a port-supplied type class; the JDK's `newInstance`/nilary-constructor chain and listed instantiators become `summon[F[T]].<create>()`, a `try` whose every handler is a declared failure elided; whole override component or none (decision shared with `ClassTagParamsTransform`) | which members, `typeClass` FQN, `create`, `instantiators`, `handles`; `spelling` (`TypeArgument` drops the parameter, `KeepParameter` keeps `m(classOf[X])` beside the clause and accepts a concrete `Class[X]` value); `classValue` for other reads where the parameter goes (`Refuse`, `Tag` = a `ClassTag` clause on every member of the rule so overrides agree, `Member(name)`). Refused and COUNTED: a `Class` value at a type-argument member, reflection with arguments, an uncovered read, a method reference; a surviving `try` is counted. The type class and its derivation are CONSUMER code — only a base ships a support type |
| `VisibilityTransform(widen, derive, narrow, deriveNarrow)` — ship a listed member PUBLIC where java declared it narrower; or a listed java `protected` member as plain subclass-only `protected` instead of `protected[pkg]`, its whole override component with it (a `SubclassOnly` tag on the symbol, read by `Visibility.plan`) | which members; `derive`: the reference's `Public` rows; `deriveNarrow`: its `Protected` rows (a frozen derived-policy file needs them regenerated or added by hand). A narrowing is REFUSED and counted (`Unverifiable`, the site named) for a static, a constructor, an anchored component (a base's or an unparsed member), and any reference from outside a subclass, from a static context, or through a receiver that is not the accessing subclass |
