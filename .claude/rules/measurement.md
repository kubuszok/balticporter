...
| `just lls-measure` | the libGDX UTILITIES family lls ported — then lls's OWN MUnit suite against them, as a second number (`expected-errors.suite`) |
| `just lls-diff-measure` | the same suite ADAPTED by an enumerated NAME/SHIM table (`ported/lls/diff-shims.tsv`) on comment-masked copies under `src_managed/diff`, with the tests whose assertion cannot survive DECLARED (`ported/lls/diff-incompatible.tsv`) and counted — then RUN |
