...
| `primitive-to-opaque` | `fqn` **required**, `underlying` (default `Int`), `extraHints = […]`, `scope`, `derive = true` (seed from the manifest's `parity` reference); **`hints` REFUSED** | see below |
...
| `nullary-arity` | `scope` (default `Only([])`, the no-op), `force = […]`, `derive = true` | drop `()` from getter-like nullary methods over the whole override component; `derive` also drops it where the reference port's `def` is parenless |
