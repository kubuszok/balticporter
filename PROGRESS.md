...
| `ssg-md` / `-test` | 126 / 126 | `-Wunused` family, largest port (136 -> 126 on 2026-09-05) |
...
that reproduces java's static inheritance (`JS-C3`): five `E120`s, one per subclass. The fix is that a
spliced companion member is one of the class's own static names, so the export excludes it — which is
why `SnapshotArray`, `DelayedRemovalArray`, `OrderedMap`, `IdentityMap` and `OrderedSet` declare the
factories rather than inheriting them.

**`just lls-diff-measure` (`LlsDifferential`)** is the oracle the gauge is not. Census re-derived every
run: **18 files / 423 `test(…)`**. Eight files / 188 tests are ADAPTED into `ported/lls/src_managed/diff/`
...
(188 `test(…)` + 3 scalacheck `property`), 189 passing, 2 failing.** Both failures are one differential
finding and the PORT is right: lls's `Select` throws `IllegalArgumentException` where java's throws
...
outcomes baselined; without it a step's gate is a compile count (§3). BUILT 2026-09-06: `LibgdxL0TestMigrate`. **Lane fact (2026-09-06): the main lane zinc-cleans the port BEFORE its migrator; the test lane never cleans (it would delete the main tree). Before that, six stale sites read as 0 for a day (K51 v–vi).** **RUNS since the sixth step landed the same day: main 0 errors, 220 tests discoverable (221 java; `BezierTest` excluded — JUnit `Parameterized`'s `Collection<Object[]>` filled from `new ArrayList<>()`, the collections seam uncoerced under the merged entry scope, 1 test), 216 pass, 4 fail = the four `JsonTest`s DERIVED from the `Json` drop, 0 unexpected.** The `net` step (7th) drops the JVM-only `NetJavaImpl` (no core reader). Steps landed in order: witness 57->44, collections 44->15 (+K46 15->10), enrich 10->7, reflection 7->3, net 3->2, nullability 2->0 (through 66, K48).
...
| R1 — **LANDED 2026-09-06, 57 -> 44** (seven subjects, six reflective constructors dropped; `witness` 147 -> 142: 129 `OccupancySentinel` refused by design, 10 `NonSubject` = the null-as-empty tables, 3 `ErasedArrayCast`; K44 opened for the 2 `Json`/`Skin` override rows; then **15 -> 10** once the fill paid its two debts, K46: class-literal payload, unchecked raw conversion at a filled formal — 4 `RawConversion` rows) | element arrays through `MkArray` (type classes, not reflection) | `ElementWitnessTransform` + `GlobalsToImplicits.requiredGivens`, subjects = core's element-typed arrays · (b) | ~16 errors (`SnapshotArray`/`DelayedRemovalArray` ctors, `Queue`, `Octree`, `BufferedParticleBatch`); `witness` 147 = 133 `OccupancySentinel` (K41, refused by design: null-as-empty tables keep the bound) + 13 `NonSubject` + 1 | largest seam, first |
| R2 — **LANDED 2026-09-06, 44 -> 15** (`jdk-surface` 50 -> 21; the merge had kept the BASE's scope and silently dropped the fragment, 44 -> 44 — fixed in place, `CollectionsPolicy.mergedWith` composes `Only` scopes, K45) | collections onto lls, `Comparator -> Ordering`, JDK `Iterable`/`Iterator` shims | lls's `CollectionsTransform` policy widened to core's entry · (b) | ~30 errors (`sort(Comparator)` 8, `iterator()`/`Iterable` shims ~22); `jdk-surface` 51 (29 kept-iterable, 22 unhandled), `collection-boundary` 14 | before R4 (engine edge) |
| R3 — enrich part **LANDED 2026-09-06, 10 -> 7** (`LibgdxEnrich` through lls's generators: primitive arrays, value maps, `IntSet`, and the factories `SnapshotArray`/`DelayedRemovalArray`/`IdentityMap` must declare against the inherited-statics export; the `IntMap`/`LongMap` templates wrap `get` in `Nullable` and are GATED on the nullability step — 2 errors when added ahead of it). Properties and parenless getters still to do | properties, parenless getters, lls's added API on core's own collections | `BeanPropertyTransform`, `NullaryArityTransform`, `LlsEnrich`'s pruned kinds re-added on core's side · (b) | ~11 errors (`setItems`, `addPool`, `IdentityMap` export clash, `Tree`); 1,422 pairs / 928 nullaries; K42 operator allowlist priced 408 members | after R2 |
| R4 — **LANDED 2026-09-06** (forced early: `SnapshotArray.replaceFirst` overrides a member lls retyped — 2 `E120` name clashes after erasure — so the step merges onto core's entry; then 66 -> 0 once a DROPPED type's members are read literally, K48; `nullability-boundary` 609 -> 149) | `@Null -> lowlevel.Nullable` | `NullabilityTransform(Named)` scoped to core; `nullableMembers` (K13.6) as core policy · (b) | `nullability-boundary` 685 = 684 `ScopedOut` (the deferred population) + 1 `UncoercibleSeam`; 650 `@Null`; K13.8 open (scope splits an override component) | after R2 |
| R5 — **LANDED 2026-09-06, 7 -> 3** (the full port's policy lifted: `Json`, `ReflectionPool`, `reflect.*` dropped; `ladder-overrides/` injects the reflection-free `Json`, `ReflectionException`, `AssetTypeRegistry`; `ClassReflection` statics forwarded to `java.lang.Class`; `Pools.get`'s reflective fallback is java's own refusal; `portability(emitted)` 153 -> 94) | no runtime reflection; `Json` dropped (sge skipped it; the codec is a hand-written injection) | `ClassTableTransform`, `StaticForwarderTransform`, `MethodBodyTransform`, `dropTypes` + `inject` · (b)+(c) | `portability(emitted)` 153: `reflect.Field` 13, `ClassReflection` 13, `Method` 9, `Modifier` 16, `Constructor` 7, `ArrayReflection` 4; 54 reflective calls / 17 files (§13.27) | before JS/Native; free vs R6 |
...
| R7 — injection-free half **LANDED 2026-09-06, 0 = 0, suite 216/4 held** (`Disposable -> java.lang.AutoCloseable` with `dispose -> close`, `InputEvent.type -> eventType`, `List.toString(T) -> itemToString`); the `Pool.*`/`SgeError`/`Gdx -> Sge` renames ride their own steps' injections Graphics spellings **LANDED 2026-09-06, 0 = 0, suite 216/4 held** (`ShapeRenderer.rect -> rectangle` over its four overloads, the two `drawing { … }` helpers spliced; 19 members moved, all attributed). Properties **LANDED 2026-09-06, 0 = 0, suite 216/4 held** (`BeanPropertyTransform` + `NullaryArityTransform` over core: 1434 properties detected, 150 arities dropped, 1648 arity refusals counted, 161 setters with behaviour kept as methods (sge's convention, K51 xiii); four faces of the base-follow closed on the way, K51 ix–xii; the demos' `input.x`, `input.touched`, `graphics.deltaTime`, `viewport.camera` now spell). Vector spellings **LANDED 2026-09-06, 0 = 0, suite 216/4 held** (`Vector#len/len2/dst/dst2/scl/nor -> length/lengthSq/distance/distanceSq/scale/normalize` over the override component; the classes' own overloads — `dst(x, y)`, static `len(x, y)` — keep java's names: a second key on one component refuses the whole rename, measured 3 -> 23 policy rows and reverted). | `Disposable -> AutoCloseable`; class renames (`Gdx -> Sge`, `DefaultPool -> Pool.Default`, `FlushablePool -> Pool.Flushable`, `QuadTreeFloat -> Pool.QuadTreeFloat`, `Files.FileType -> sge.files.FileType`); member renames | `MemberRenameTransform`, `typeRenames`, `subPackages` · (b) | 147 sites | free among R7–R9 |
| R8 — `Seconds` **LANDED 2026-09-06, 0 = 0, suite 216/4 held** (sge's own file injected; seeded at the two `Graphics` producers, reaches `Screen.render`/`ScreenAdapter` through the new override edge; `opaque-boundary` 1 = `Math.min` unwrapped, K50). `Pixels` **LANDED 2026-09-06, 0 = 0, suite 216/4 held** (`sge.Pixels` injected; seeded at the `Graphics`/`Input` producers and `resize(width, height)`, 128 declarations retyped; four engine faces closed on the way, 12 -> 86 -> 0, K51; `opaque-boundary` 1 -> 15, every row a JDK callee; the test port's `base-surface` 0 -> 10 `unanswered` primaries naming `Pixels`, compiled and run regardless). `WorldUnits` **LANDED 2026-09-06, 0 = 0, suite 216/4 held** (sge's own file injected; seeded at the viewport's and the camera's world-size fields and FENCED to the viewports and cameras by an `Only` scope — sge's thirteen files; unfenced the flow rode `setToOrtho`/`Batch.draw` into `Sprite.scaleX`, 237 then 85 errors; `opaque-boundary` 15 -> 9). Audio (`Volume`/`Pitch`/`Pan`/`SoundId`) and time (`Millis`/`Nanos`) **LANDED 2026-09-06, 0 = 0, suite 216/4 held**, each fenced by an `Only` scope to the files sge keeps it in (8/3/5/2 and 5/13); with them the `helpers` step (sge's `graphics.gl`, `rendering { … }`, class-tag `load`, `Nullable` `AssetManager.get`) — demo check 76 -> 49. GL enums (`EnableCap`/`PrimitiveMode`/`CompareFunc`/`ClearMask`, sge's `GLEnum.scala` injected, seeded at seven `GL20` parameters) **LANDED 2026-09-06, 0 = 0, suite 216/4 held**: a constant variable is never a seed and an enum case's arguments are coerced (K51 xv–xvi); with it the `Nullable` parameters sge takes on `ModelBatch`, `Clipboard.setContents` and `TiledMapTileLayer.setCell`. Still to do: `Align`, GL handles, `Millis`/`Nanos` | opaque domains (`Align`, `Seconds`, GL handles…) | `PrimitiveToOpaqueTransform` · (b) | 32 opaque types in sge, 3 configured today | free |
...
| R10 — **LANDED 2026-09-06, 0 = 0, suite 216/4 held** (the full port's holder policy lifted; `sge.Sge` injected; the GL statics hop through GETTER CALLS and writes through the bean setter until the property step, K49 — 321 -> 10 -> 0; `context-seam` 20 counted: 13 captured-context, 3 residual-global-read, 2 deferred-init, 1 unconstructed-thread; `SgeTestFixture` hand-written in `ported/sge-l0/src/test`) Refined 2026-09-06: `Pixmap` is context-free as in sge (`Pixmap(w, h, format)` in the demos) — its `dispose` is left out of the holder's `scope`, so the class takes no clause, the statics still do, and the `Gdx.app.error` inside stays a counted residual global read. | implicit `Sge` context instead of `Gdx` globals | `GlobalsToImplicitsTransform` · (b); CT7 (framework-instantiated classes) | 267 reads / 79 files + 336 `Gdx.gl` | LATE: constructor signatures move every backend |
| R11 — **LANDED 2026-09-06, 0 = 0, suite 216/4 held** (sge's `Pool.scala`/`Poolable.scala`/`DynamicArrayOps.scala` injected with four adjustments recorded in the copy: java's constructor defaults on the trait's vals, `protected[utils]` as java's protected, java's arity on two lls calls, java's exact `freeAll(Array<T>)` shape; `DefaultPool -> Pool.Default`, `DefaultPool.PoolSupplier -> () => A`; java's `Pool`/`DefaultPool` dropped; `FlushablePool` stays java's CLASS over the trait as in the full port — a subclass with three constructors passing different `super(...)` args cannot map onto one val, 1 test) | `Pool` as a trait, adapters collapsed | `ClassToTraitTransform` (AD-003, CT12) · (b) | `Pool` only | after R7 |
...
| R14 — **census 2026-09-07 of what sge's backend is MADE OF** (`sge/src/main/{scala/sge/platform,scalajvm,scaladesktop,scalanative,scalajs}`): three layers. (1) A platform CONTRACT in core, hand-written: seven traits (`WindowingOps` 576 lines, `AudioOps`, `BufferOps`, `ETC1Ops`, `Gdx2dOps`, `GlOps`, `ConcurrencyOps`; 1,362 lines) that core calls where java called JNI or LWJGL (`Pixmap`, `Gdx2DPixmap`, `BufferUtils`, `ETC1`, `Timer`, `AssetManager`); COPY. (2) Per-platform IMPLEMENTATIONS of that contract, hand-written: JVM 5,276 lines (Panama over GLFW, ANGLE GL 2.0–3.2 bindings, miniaudio, a native ops library `sge_native_ops`/`sge_audio` shipped by `native-bundle.json`), Native 3,966, JS 2,757 (plus image decoders); COPY, with their native libraries. (3) The backend classes DERIVED from libGDX's java (desktop 24 files from `gdx-backend-lwjgl3` + `gdx-backend-headless`; browser 34 from `gdx-backends-gwt`; android 9 from `gdx-backend-android`): every LWJGL static (`GLFW.*` 139 in `DefaultLwjgl3Input` alone, `AL10`, `GL11`) became a call on the contract (`windowing.*`), audio moved from OpenAL to miniaudio, files/preferences/net came from the HEADLESS backend, not LWJGL3. Method-name survival against the java: 100% files/preferences, 64–75% window/sync/file-handle, 50% input, 30% graphics/application, 15–25% config/audio. PORT-AND-MODIFY the high-survival files through the engine with one redirect table (LWJGL static → contract member, a `StaticForwarder`/`ClassTable` policy); the low-survival ones (application, config, audio engine) are cheaper COPIED. **ACCEPTED by the maintainer 2026-09-07 as the route to the first RUNNING demo: (1) copy the contract traits and the JVM implementations with their native libraries, wire `Pixmap`/`Gdx2DPixmap`/`BufferUtils`/`ETC1`/`Timer`/`AssetManager` to the contract, drop the 20 native stubs; (2) copy the low-survival desktop files (application, config, graphics, audio), port the high-survival ones (window, input, cursor, files, preferences, net, sync) with an LWJGL-static-to-contract redirect table; (3) a `demo-run` lane that launches pong for a fixed number of frames and fails on any exception, and a `runs` column in the status table.** | backends: desktop (Panama/GLFW), browser (DOM), android | per-platform source sets from gdx's backends (71 backend-derived files in sge core) · **GAP** `PortManifest.platformDirs` (b) | — | after R13 |
...
re-runs it; the number is the distance to the demos. **2026-09-06, after the Pixels, graphics, properties and WorldUnits steps: pong + shared compile — 30 -> 21 -> 9 -> 4 -> 1 -> 0.** Next: the other eleven demos through the same check. **All twelve demos (the check's default since 2026-09-06): 76 -> 49 -> 26 -> 25 after the helpers, audio, time, GL-enum and vector-rename steps.** **Then 25 -> 22 -> 18 -> 10 -> 3 (2026-09-07): the three small gaps, the enrich value fix, the noise port on the classpath, the four repeated-parameter overloads, the nested resolvers, and sge's `screen`/`projectionMatrix` setter pairs. Then 3 -> 2: a fluent setter collapses under a configured pair when no call reads its result (`cell.tile = …`, K51 xix). Then 2 -> 0 (2026-09-07): `sge.Screen`'s lifecycle members carry sge's empty defaults (`MethodBodyTransform`, `render` stays abstract), and hex-tactics compiles from an ADJUSTED COPY (`ported/demo-check/adjusted/`, differences enumerated in `ported/demo-check/ADJUSTMENTS.tsv`, standing order 1): `ObjectMap[java.lang.Integer, Vector2]` where sge writes `ObjectMap[Int, Vector2]` — java's implicit `Object` bound on lls's map (ENGINE-LIMITS G24; the narrow per-type drop measured lls 0 -> 20 on 2026-09-07 and was reverted: the map's views would need the array witness the open-addressed tables deliberately do not carry, K41). **All twelve demos and shared TYPECHECK against the port (`just demo-check` 0, one adjusted file). That is NOT the goal and was wrongly reported as reached on 2026-09-07. The goal (maintainer, restated 2026-09-07): sge's libraries REPLACED by the ported ones, the demo code taken over with API adjustments, and the demo RUNS. Today nothing runs: the port has no backend, its native calls are stubs, and there is no launcher — a demo would fail at its first native or GL call. The demo check stays as the API-distance instrument; the goal's instrument is a demo that starts and renders frames on the ported stack.** The 25 that remain are not another step; each is a card of its own, in the order they should go:
(1) **java varargs as Scala repeated parameters** — the four types the demos construct carry a spliced repeated-parameter overload each (`Material`, `Mesh` with sge's two lists, `Bezier` as the companion's `apply` — a secondary constructor cannot unify the class's F-bounded `T` with its own inside a self-invocation — and `TextFormatter.format`), 2026-09-06; the MECHANISM is still this card — 8 (`Material(attr)`, `Bezier(p0, …)`/`set`, `Mesh(…, attrs)`): the port emits a `T...` parameter as `Array[T]` by design (CLAUDE.md §4.4, the `String.format` row); sge emits `T*` and the demos call it so. An emitter/frontend decision plus every declared call site's spread — one wave, measured on the frozen family too since it moves every port's surface;
(2) **lls decisions** — 4 (`foreachEntry` DONE 2026-09-06: the enrich templates hand the raw value through `.orNull` as the lls reference does; lls 0 = 0, 189/2 held; `ObjectMap[Int, Vector2]()` stays, G24): `ObjectMap[Int, Vector2]()` (the `K <: java.lang.Object` bound java never wrote, ENGINE-LIMITS G24) and `foreachEntry { (k, v) => v.col }` (the enrich template hands the value as `Nullable`; sge hands `V`) — the base's port, then `just measure-all`'s dependents;
(3) **the noise port** — DONE 2026-09-06 (it is standalone: `demo-check` depends on `port-sge-noise` as emitted) — was: `tile-world` imports `sge.noise.*`, which is the noise4j port (`sge-noise`) rebuilt on `sge-l0` and put on the demo classpath — the first DEPENDENT of the ladder, the shape every other sge module will take;
(4) **small mechanism gaps** — DONE 2026-09-06 (25 -> 22): `TextFormatter` published by its declared split, `Clipboard.hasContents` parenless by the arity phase's `force` list, sge's four-argument `OrthogonalTiledMapRenderer` spliced as a secondary constructor (K51 xvii–xviii).
...
1. *What done means* (restated by the maintainer 2026-09-07, after a compile count was wrongly
   reported as the goal). sge's libraries are REPLACED by the ported ones — the core and the
   extensions a demo uses — the demo code (`../sge/demos`: pong, space-shooter, hex-tactics,
   tile-world, viewer-3d, particle-show, shader-lab, net-chat, game-screens, curve-playground,
   asset-showcase, viewport-gallery, and their `shared` module) is taken over, adjusted where the
   port's API differs, and the demo RUNS on the ported stack on the platforms it targets.
   Typechecking against the port is a distance measure on the way, never done. Nothing else in sge was verified in
