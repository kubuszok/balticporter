...
| R7 — injection-free half **LANDED 2026-09-06, 0 = 0, suite 216/4 held** (`Disposable -> java.lang.AutoCloseable` with `dispose -> close`, `InputEvent.type -> eventType`, `List.toString(T) -> itemToString`); the `Pool.*`/`SgeError`/`Gdx -> Sge` renames ride their own steps' injections | `Disposable -> AutoCloseable`; class renames (`Gdx -> Sge`, `DefaultPool -> Pool.Default`, `FlushablePool -> Pool.Flushable`, `QuadTreeFloat -> Pool.QuadTreeFloat`, `Files.FileType -> sge.files.FileType`); member renames | `MemberRenameTransform`, `typeRenames`, `subPackages` · (b) | 147 sites | free among R7–R9 |
