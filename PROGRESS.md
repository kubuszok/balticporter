...
- **The difference catalog's four coverage lanes** (`DESIGN.md` §2.8). Their first honest numbers,
  ...
  | `catalog(unmechanised)` | **10**, identical on all fifteen (was 20, 47, 88, 111, 112) | rows nothing is instrumented to answer for. Derived from the REGISTRY and not from the run, deliberately, so a reader comparing two ports can see it did not move. The FOURTH SURFACE took it **20 → 10** (`DESIGN.md` §2.8): nine `JS-G` type-reference rows plus `JS-C29`, which was area C's own residue and the same fact one area over. What is left is ten and every one of them is named — `JS-C22`/`JS-C23` waiting on the risk counter, `JS-C30`/`JS-C43` refused or absorbed at their Spoon kind, `JS-C42`'s missing shim, `JS-E06`'s cast half, `JS-G20`'s per-phase discipline, `JS-G41`'s absent counter, and the two `refusedAtTheKind` rows the `markers` lane measures instead |
