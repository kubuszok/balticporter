...
- **The difference catalog's four coverage lanes** (`DESIGN.md` §2.8). Their first honest numbers,
  ...
  |---|---|---|
  | `catalog(consulted)` | **46–63 per port** (was 14–29, and 5–8 before that) | rows the run REACHED, at any of the three surfaces. Area C is what last moved it — libGDX core reads 63 against 29 — and the artifact shows the split rather than a total: `JS-S17` fires 1,365 of 1,366 consults, `JS-S18` 92 of 92, `JS-S13` **211 consults and 0 fires**, which is `ENGINE-LIMITS.md` F5's "the corpus count is ZERO" re-derived by a different instrument on a different day |
  | `catalog(unreached)` | **4–21 per port** (was 3–18) | a row whose surface EXISTS and which this port never reached. Per-port informational, and a real spread: `JS-S02`/`JS-S06`/`JS-S08` are reached on libGDX core and unreached on its TEST source set, which is exactly what a coverage lane should look like. Corpus-wide (`just catalog-coverage`) it is **three** rows: JS-C12, JS-E17 and JS-S23, all `Open` and therefore unconsultable by rule (ii) — the work list seen from the other side. That the corpus-wide set is EXACTLY the open rows is the evidence that no mechanised row is dead code |
  | `catalog(unmechanised)` | **47**, identical on all fifteen (was 88, 111, 112) | rows nothing is instrumented to answer for. Derived from the REGISTRY and not from the run, deliberately, so a reader comparing two ports can see it did not move. Area C's wave took it **88 → 47**: 41 of the 47 `JS-C` rows now attach to a real surface, and the six that do not each NAME the surface they wait for — `JS-C22`/`JS-C23` a risk counter, `JS-C29` a type rendering that is not a `Tree` dispatch, `JS-C30` a construct REFUSED at the statement dispatch, `JS-C42` a shim, `JS-C43` a construct absorbed silently at `CtRecord`. The area needed no new surface at all: a `ClassDef`, a `DefDef` and a `ValDef` are `Statement`s, so `Rendering.of` had reached every declaration since area S built it |
  | `catalog(undischarged)` | **3** on the two libGDX lanes, **2** on the other thirteen | the WORK LIST, and every entry on it is DECLARED. JS-E17, the lvalue's single evaluation (`ENGINE-LIMITS.md` F7), on every port that lowers a compound assignment; **JS-C12**, a forward reference to a later field, which java rejects and scala reads as a default; and **JS-S23**, java's `assert` being off without `-ea` while Scala's always runs, at 136 `Tree.Assert` renderings in libGDX core alone. All three are `Open` rows, so a fatal log counts them rather than raising — a mode that died on the work list would make the work list unrunnable. The rises are the design working: each wave attaches rows nobody handles, and the count is the number of known holes rather than the number somebody remembered. JS-E04 was an entry and is gone; **JS-C44 was an entry for one commit** and is gone the same way — a `WidenedSeal` decision now records the half scala cannot express, and both closings cost **0 member digests on all fifteen ports** because no corpus library has the shape |
