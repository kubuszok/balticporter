...
outcomes baselined; without it a step's gate is a compile count (§3). BUILT 2026-09-06: `LibgdxL0TestMigrate`. **RUNS since the sixth step landed the same day: main 0 errors, 220 tests discoverable (221 java; `BezierTest` excluded — JUnit `Parameterized`'s `Collection<Object[]>` filled from `new ArrayList<>()`, the collections seam uncoerced under the merged entry scope, 1 test), 216 pass, 4 fail = the four `JsonTest`s DERIVED from the `Json` drop, 0 unexpected.** The `net` step (7th) drops the JVM-only `NetJavaImpl` (no core reader). Steps landed in order: witness 57->44, collections 44->15 (+K46 15->10), enrich 10->7, reflection 7->3, net 3->2, nullability 2->0 (through 66, K48).
