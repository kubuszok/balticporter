...
| `ssg-md` | flexmark-java `flexmark` + 11 `flexmark-util-*` | 458 → **468** (0 dropped, 0 injected; 486 in scope, 28 declaration-only) | its suite is a THIRTEENTH module (`flexmark-util`) and a second lane — **723** emitted, **0 lost**, not yet run (§10.6.6) | **38** main + **25** test (243 at first emit; §10.6.3, all classified) |
...
| | |
|---|---|
| scalac errors | **243** at first emit (coded 241 + bare 2), **171** after wave 1, **106** after wave 2, **81** after wave 3, **69** after wave 4, **58** after wave 5, **47** after wave 6, **43** after wave 7, **40** after wave 8, **38** after wave 9 and **35** after wave 10 (coded 35 + bare 0), all `EngineGap`, 0 `Approx`, 0 `Unmapped`. Concentrated in **60 of the 468 emitted files** at first emit and **21** now — 95 % of the port compiles clean |
...
### 10.6.3 The census, classified per §1 — **243 → 171 after wave 1, → 106 after wave 2, → 81 after wave 3, → 69 after wave 4, → 58 after wave 5, → 47 after wave 6, → 43 after wave 7, → 40 after wave 8, → 38 after wave 9, → 35 after wave 10, → 34 after wave 11**
...
**What is left, by mechanism** (the census re-read at 34, every row counted from `errors.tsv`):
...
| 4 | **the SHIM against a scala collection, inside the program** — the RESIDUE of that row after wave 5 closed its rule half (K25) and wave 9 closed its `DeclaredSubtype` half (K26). A `JavaCollection` formal or result meeting a `Buffer`/`Set`/own-`OrderedSet` value at a call the port declares (`ScopedDataSet#getKeys` 2, `Attributes#values`, `BuilderBase#extensions`). What wave 9 took was `OrderedMultiMap#keys`/`#values`, the two the lane could SEE; what is left sits at a callee THIS PHASE MINTED, which carries no signature, and K26 records both the removed operand-only arm and its repair | **(a)** |
| 2 | **a held-back `iterator()` against the class's OWN shim-parented iterator** — `BitFieldSet#iterator()` overrides `java.util.AbstractSet.iterator()`, whose class file returns `java.util.Iterator<E>`, while the nested `EnumBitSetIterator`/`EnumBitFieldIterator` the body returns were given the `JavaIterator` SHIM as a parent by the same phase. NEW at wave 5 and not a regression: before K25 the member returned the shim and silently overrode nothing, which only `RefChecks` could have reported (§3). No `JavaCollections.toJava` exists for an iterator, so the seam is a compile error rather than a bridge; the member's own porter note classifies it at the line  **Wave 10 read the TEST set's six beside these two and they are the SAME refusal from outside**: three `value foreach is not a member of BitFieldSet[…]` and three `Found: java.util.Iterator`, all from a `for` loop over the class. **Wave 11 TRIED the third answer, and it is the one that closes all eight — at a price**: the whole family exists because `java.util.AbstractSet` is not in `typeMap` while `java.util.Set` is, which `collection-closure` had been REPORTING as a lost JDK relation since the port began. Mapping it took the whole-compile figure **40 → 36** and the TEST SET **6 → 0**, and opened **4** new main-set errors — `super.containsAll/addAll/removeAll/retainAll`, the JDK defaults `BitFieldSet` delegates to, which `mutable.Set` does not have. So `md-measure` reads 34 → 36 and the step was REVERTED rather than shipped half-built. The two candidates this row used to name are both superseded by that measurement: the target choice is a two-way bind (`mutable.Set` keeps the subtype edge and drops the defaults; the shim keeps the defaults and breaks the edge, which is the 13-error split `typeMap` already measured), and the real fix is a PHASE obligation — two runtime helpers, one widened, and a `super` → `this` substitution licensed per member by the JDK default's own virtual dispatch. `ENGINE-LIMITS.md` K29 carries the numbers and the recipe. **This is the FLOOR for waves 10 and 11 and is stated as one** | **(a)**, `ENGINE-LIMITS.md` K25, K29 |
| 6 | **`Nothing` at a structural type — and wave 6 read the DECLARATION and re-classified it as a STANDING REFUSAL.** `IRichSequenceBase`'s `?{ append: ? }` / `?{ add: ? }` slots are six calls to one member: `<B extends ISequenceBuilder<B, T>> B getBuilder()`, whose variable appears in no formal and whose result is the RECEIVER of the next selection — G22's shape exactly, and G22's pin declines on its FOURTH condition, correctly and for both of that condition's reasons at once. The bound `ISequenceBuilder<B, T>` is an F-bound *and* names the enclosing interface's `T`, so it is not a type this call site can write down; filling `B` while `T`'s own bound still mentions `B` is `G8`'s "either every formal comes from the enclosing scope or none can", which four measured attempts have already priced. This is not open work — it is the expressiveness limit G8 names, and the honest answer is a REPORT rather than a translation | **(a)**, REFUSED — `ENGINE-LIMITS.md` G8, with `G22`'s pin declining correctly |
...
| 2 | **`null` at a type PARAMETER — the ARGUMENT shape is CLOSED (wave 8, G12) and the two that remain are neither this family nor each other's.** What closed was the three ARGUMENT sites, through the receiver's own instantiation. What is left is a LAMBDA whose body is a bare `null` at a result type of `T` (`NullableDataKey`'s `(options: DataHolder) => null`, where the expected type is the SAM's result and no formal exists to read), and one that was never this family — `HashMap.from(null.getAll())`, a `null` RECEIVER at a structural expected type | **(a)**, two different positions |
...
#### The FLOOR the refusal ledger defines — 21 of the 34, and what the other 13 are

Eleven waves took 243 → 34 and every one of them closed a FAMILY. What is left divides into rows a
named refusal already answers and rows nobody has diagnosed, and the two are worth separating,
because a census that only counts is one an operator reads as *seventeen more waves*. **Twenty-one of
the thirty-five are attributed to a refusal or a limit this repository has already measured and
...
| 4 | **K25**'s residue — the in-program half, its two `collection-internal` rows CLOSED at wave 9 | a `JavaCollection` formal or result meeting a `Buffer`/`Set`/own-`OrderedSet` value at a call the PORT declares. K25 closed the rule half (an override of a class file keeps java's formals) and K26 closed `DeclaredSubtype`; what is left sits at a callee THIS PHASE MINTED, which carries no signature at all, and K26 records both the removed operand-only arm and its repair (mint the helpers with their signatures) |
...
The other **13** are open work with no entry behind them: **2** `null` at a position the wave-8 fix
does not reach (a LAMBDA body at a result type of `T`, and one `null` RECEIVER that was never this
family), **2** the `BitFieldSet#iterator()` pair K25 made loud, **2** raw-generic one-offs, and the
**7**-row residue. So the honest reading of 34 is *21 refused, 13 open*. **Wave 10 moved BOTH
columns, which no earlier wave had**: G28's ascription took one out of the open residue and K23's
built arm took one out of the refused column and one more out of the residue — the second being
`Utils#withDefaults`, which the residue had filed under its error text with a diagnosis that was
wrong (§10.6.3's wave-10 row). **Wave 11 took a third out of the residue the same way** — an
`E006 Not found: type S` on `SequenceBuilder#append(char,int)` whose cause was the EMITTER's own
numeric-overload pin naming the callee's F-bounded result (G12), not the one-off its text suggested.
That is now the fourth census row whose cause turned out to be a different defect from the one its
text names, and it is the argument for reading a residue row before counting it.
...
**And the port has not yet met `RefChecks`** (§3): 34 typer errors means it has never run, so every
...
- **`java.util.AbstractSet` into `typeMap`, ALONE.** It is the right relation and the wrong step on
  its own: `40 → 36` whole-compile with the TEST SET at **0**, at `md-measure` **34 → 36** — four
  `super.<JDK default>` calls that `scala.collection.mutable.Set` cannot answer. The mapping needs
  the phase to supply those defaults FIRST; `ENGINE-LIMITS.md` K29 has the recipe and the numbers,
  and re-deriving them costs a full `measure-all` (every other port is rows-flat, so the only signal
  is here).
...
| scalac errors | **40 total on one compile of both source sets after wave 11 — 34 main, 6 test, 0 elsewhere** (42 = 35 + 7 after wave 10; 63 = 38 + 25 after wave 9; 84 = 40 + 44 before that). Wave 10 closed twenty-one in two commits: G28's ascription took the twelve `tagLine` rows and one of the main set's, and K23's built bound-method-reference arm took six `PlaceholderReplacer` rows and two more of the main set's. Wave 11 closed two more with ONE fix, one on each side. Baselined at 40 against `FlexmarkTestMigrate`, which is the whole-compile figure; §10.6.3's 34 stays `md-measure`'s and is reproduced by that lane alone |
| the 6, ATTRIBUTED after wave 11 | by code: 3 `E007`, 3 `E008` — **ONE family, and it is a main-set residue read from a caller**: `BitFieldSet`, K25's held-back `iterator()` met by a `for` loop over a class whose parent stayed `java.util.AbstractSet` — three `foreach is not a member` and three `Found: java.util.Iterator`. The `PlainSegmentBuilder` row CLOSED at wave 11 and this table's re-diagnosis of it was right: the emitted `(segments.append: (Char, Int) => S)(' ', 2)` was a `MethodType` ascription naming `S`, the CALLEE's own type variable, which G12's rule says the call site cannot name — and it was the EMITTER's numeric-overload pin that wrote it, not an inference failure as the table said before that. `PlaceholderReplacer`'s six CLOSED at wave 10 (K23). **The twelve that closed were NOT what this table first said either**, which is now four waves running: `tagLine`'s rows read as an overload-resolution divergence and were an ordinary poly expression at a slot scala cannot use — `overload-risk` read ZERO at all twelve, correctly, because java's candidate set spans no resolution phase (`ENGINE-LIMITS.md` G28) |
| the 6, by owner | `BitFieldSetTest` 6 — **ONE file of fifty-two**, down from two and from five before that: `HtmlAppendableBaseTest`, `HtmlBuilderTest`, `PlaceholderReplacerTest` and now `PlainSegmentBuilderTest` are clean. The one remaining owner is a caller of a residue §10.6.3 already names |
...
**BOTH WERE DEFERRED AGAIN AT WAVE 10, and the reason is the same for each: neither moves the number
that is blocking the suite.** The test set is at **7** errors, all of them main-set residues read from
a caller (`BitFieldSet` 6, `PlainSegmentBuilder` 1), and until those close nothing here RUNS — so a
conversion's only evidence would be that the emitted text changed shape, which is precisely the
...
a blast worth spending once a suite can validate it and not before. The order is therefore: close the
seven, run the suite, then convert — at which point `ExpectedException`'s five convertible sites
become five tests that stop FAILING, which is a number, rather than five statements that changed.
...
0 errors exactly as `liqp-measure` and `sg-measure` gate theirs; at 40 it prints that 723 tests are
