...
- **The difference catalog's four coverage lanes** (`DESIGN.md` §2.8). Their first honest numbers,
  ...
  | `catalog(unmechanised)` | **8**, identical on all fifteen (was 9, 10, 20, 47, 88, 111, 112) | rows nothing is instrumented to answer for. Derived from the REGISTRY and not from the run, deliberately, so a reader comparing two ports can see it did not move. The FOURTH SURFACE took it **20 → 10** (`DESIGN.md` §2.8): nine `JS-G` type-reference rows plus `JS-C29`, which was area C's own residue and the same fact one area over. `JS-C30` came off it when the frontend learned the method-LOCAL class (T9), and `JS-G41` when its RISK COUNTER shipped — having nothing to TRANSLATE is not having nothing to DECIDE. What is left is eight and every one of them is named — `JS-C22`/`JS-C23` waiting on the risk counter's other half, `JS-C43` absorbed at its Spoon kind, `JS-C42`'s missing shim, `JS-E06`'s cast half, `JS-G20`'s per-phase discipline, and the two `refusedAtTheKind` rows the `markers` lane measures instead |
