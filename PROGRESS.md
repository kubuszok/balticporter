...
re-runs it; the number is the distance to the demos. **2026-09-06, after the Pixels, graphics, properties and WorldUnits steps: pong + shared compile — 30 -> 21 -> 9 -> 4 -> 1 -> 0.** Next: the other eleven demos through the same check. **All twelve demos (the check's default since 2026-09-06): 76 -> 49 -> 26 -> 25 after the helpers, audio, time, GL-enum and vector-rename steps.** The 25 that remain are not another step; each is a card of its own, in the order they should go:
(1) **java varargs as Scala repeated parameters** — 8 (`Material(attr)`, `Bezier(p0, …)`/`set`, `Mesh(…, attrs)`): the port emits a `T...` parameter as `Array[T]` by design (CLAUDE.md §4.4, the `String.format` row); sge emits `T*` and the demos call it so. An emitter/frontend decision plus every declared call site's spread — one wave, measured on the frozen family too since it moves every port's surface;
