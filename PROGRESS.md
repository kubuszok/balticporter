...
(2) **lls decisions** — 4: `ObjectMap[Int, Vector2]()` (the `K <: java.lang.Object` bound java never wrote, ENGINE-LIMITS G24) and `foreachEntry { (k, v) => v.col }` (the enrich template hands the value as `Nullable`; sge hands `V`) — the base's port, then `just measure-all`'s dependents;
(3) **the noise port on the ladder** — 2: `tile-world` imports `sge.noise.*`, which is the noise4j port (`sge-noise`) rebuilt on `sge-l0` and put on the demo classpath — the first DEPENDENT of the ladder, the shape every other sge module will take;
(4) **small mechanism gaps** — 4: a public `TextFormatter` (java package-private; `allowPackageSplit` only publishes a type a rename MOVES — a visibility key is missing), `Clipboard.hasContents` parenless (the arity phase refuses a side-effecting body; sge decided otherwise — a per-member force list), and sge's four-argument `OrthogonalTiledMapRenderer(map, unitScale, batch, ownsBatch)` (an sge addition: an injected overload).
