...
**Where the next wave starts.** Wave 9 took the `collection-internal` lane's `DeclaredSubtype` half
at the SLOT — `coerce` now reads a program-declared value's kind out of the phase's own re-parenting
...
#### The FLOOR the refusal ledger defines — 24 of the 40, and what the other 16 are

Eight waves took 243 → 40 and every one of them closed a FAMILY. What is left divides into rows a
...
The other **16** are open work with no entry behind them: **2** `null` at a position the wave-8 fix
...
**10**-row residue. So the honest reading of 38 is *22 refused, 16 open* — and note which half wave 9
moved: it took TWO OUT OF THE REFUSED COLUMN, which is the shape of a wave that re-opens an entry with
a number rather than one that closes open work. The 16 did not move.
