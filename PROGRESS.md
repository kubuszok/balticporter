...
| | |
|---|---|
| scalac errors | **243** at first emit (coded 241 + bare 2), **171** after wave 1, **106** after wave 2, **81** after wave 3, **69** after wave 4, **58** after wave 5 and **49** after wave 6 (coded 49 + bare 0), all `EngineGap`, 0 `Approx`, 0 `Unmapped`. Concentrated in **60 of the 468 emitted files** at first emit and **25** now — 95 % of the port compiles clean |
...
### 10.6.3 The census, classified per §1 — **243 → 171 after wave 1, → 106 after wave 2, → 81 after wave 3, → 69 after wave 4, → 58 after wave 5, → 49 after wave 6**

Every error is `EngineGap`. Six waves have run. Each table below is the state AFTER its wave, with
...
**What is left, by mechanism** (the census re-read at 49, every row counted from `errors.tsv`):

| n | family | §1 |
|---:|---|---|
| 4 | **the raw-generic family's residue, its arity and cast halves both closed.** Wave 5 took the sites whose arity was already right (G12) and wave 6 took the nine whose arity was wrong (G26). What is left is not one family: **2** where `getAction`/`getHandler(Class<?>)` probes a `Map<Class<? extends N>, H>`. Wave 6 read this one to the predicate and it is NOT K24's `objectProbe` guard, which declines correctly (the argument is `Class[?]`, not `java.lang.Object`): it is `wildcardMapCall`, whose question is *can this key be NAMED at a call site* and whose test is `args.exists(_.isInstanceOf[TypeBounds])` — the map's own type ARGUMENTS, one level. `Class<? extends N>` is a capture one constructor IN, as unnameable as a bare `?` and invisible to that test. §4.56's partial type walk, and nothing reports it because the ordinary `getOrElse` rewrite is good Scala that does not type-check at the argument; **1** `toArray(EMPTY_HANDLERS)` at an OVERLOAD selection on the class's own `addHandlers` (java's `Collection.toArray(T[])` returns `T[]` and the port erased the empty-array argument to `Array[Object]`); and **1** cascade at `new NodeVisitor(…)` | **(a)** — `K24` and one overload |
...
count, and the reason the ascription does not fire there is one predicate: `nullToTypeParam` casts
only where `resolveTypeParam` answers, i.e. where the callee's variable is literally the caller's
own. `OrderedSet<E>.add(E)` called on an `OrderedSet[V]` field inside `OrderedMultiMap<K,V>` is not
that shape — but the RECEIVER's type arguments say `E := V` exactly, which is the rule
`varargPack`'s `recvSubst` branch already states in its own comment (*a callee's own type variables
do not resolve at the call site, but the CLASS's do, through the receiver's type arguments*, G12).
Two things a re-try has to deal with and neither is incidental: `receiverTypeArgs` gates on
`tpConcrete`, which answers `false` for a type PARAMETER, so the substitution `OrderedSet<V>` offers
is currently discarded before anybody can use it — and `tpConcrete`'s own neighbour in the source
already documents that as the case it "excludes wrongly"; and `coerceArgsFixed` does not receive
`recvSubst` at all, so the map has to be threaded from `coerceArgs` rather than re-derived, which is
F8's rule.

**And the cheapest thing on the list is not that row at all**, which is worth saying because a wave
that starts with the largest number starts in the wrong place: the `Type::method` eta-expansion
above is 2 errors decided by ONE structural fact the frontend already holds — the referenced
method's `static` modifier — and the two sites fail in opposite directions, so a fixture with both
is the whole safety argument. Take it first and re-census; it is also the only residue row whose
defect can be stated without reading `CollectionsTransform`.

The other three sites in the `null` row are three different positions (a REWRITTEN call, where
the coercion is discarded by the rewrite — `CLAUDE.md` §1's ordering rule read backwards; a LAMBDA
body at a result type of `T`; and one `null` RECEIVER that is not this family at all), so expect the
...
**And the port has not yet met `RefChecks`** (§3): 49 typer errors means it has never run, so every
