...
| `liqp` | liqp `src/main/java` | 135 → **139** (0 dropped, 4 injected) | — | **3** (§10.5.3, all classified) |
...
**What is left, 6 — 3 main and 3 test**, all of them (a) engine, by family:
...
| 1 | K15 at ANTLR — a collapsed stream whose result feeds an external `Stream.concat`. The chain `xs.stream().map(f)` is NOT terminated by a collector, so the collapse hands a `Buffer` to a `java.util.stream.Stream` formal | `parser/v4/NodeVisitor#getJekyllIncludeInsertionNode` |
