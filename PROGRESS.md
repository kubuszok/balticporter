...
| `ssg-md` | flexmark-java `flexmark` + 11 `flexmark-util-*` | 458 → **468** (0 dropped, 0 injected; 486 in scope, 28 declaration-only) | its suite is a THIRTEENTH module (`flexmark-util`) and a second lane — **723** emitted, **0 lost**, not yet run (§10.6.6) | **40** main + **34** test (243 at first emit; §10.6.3, all classified) |
...
**What is left, by mechanism** (the census re-read at 40, every row counted from `errors.tsv`):
...
**Where the next wave starts.** Wave 8 closed the `null`-at-a-type-PARAMETER row's ARGUMENT shape
(three of five, `ENGINE-LIMITS.md` G12) and gave the collections family's in-program seam its own
lane (`collection-internal`, K26). What that leaves is a residue of **40** whose largest single
family is now the SEVEN rows that lane counts — five `SplitTypeVariable` and two `DeclaredSubtype`,
all of them one fact: `typeMap` sends `java.util.Collection` to a standalone shim and every java
SUBTYPE of it to a `scala.collection.*` type, so java's own `List <: Collection` has no image. Those
are the rows a next wave is working against, and the lane is what makes them countable rather than
re-derivable.
...
**10**-row residue. So the honest reading of 40 is *24 refused, 16 open*, and the 16 is the number a
next wave is working against — note the refused half did not move, which is what a wave that closes
open work looks like.
...
| scalac errors | **74 total on one compile of both source sets after wave 9 — 40 main, 34 test, 0 elsewhere** (84 = 40 + 44 before it; the ten that closed are the TEST set's, `ENGINE-LIMITS.md` K27). Baselined at 74 against `FlexmarkTestMigrate`, which is the whole-compile figure; §10.6.3's 40 stays `md-measure`'s and is reproduced by that lane alone |
| the 34, ATTRIBUTED after wave 9 | by code: 18 `E007`, 12 `E134`, 3 `E008`, 1 `E006`. **Nine of them are `collection-internal` rows** — the `OrderedSet` value handed to its own `addAll`/`retainAll`, whose formal is `Collection`'s standalone target; the lane counts **16 SITES** for those 9 errors, because scalac stops after a few per statement (`ENGINE-LIMITS.md` K26). The rest are the same three main-set families read from their callers: `tagLine`'s overload row (12), `BitFieldSet`'s held-back `iterator()` met by a `for` loop (6, K25), the `PlaceholderReplacer` method reference at a mapped member (6, K23's named row) and one `PlainSegmentBuilder` inference residue. **The 10 `E051` CLOSED at wave 9** and they were NOT §4.4's `remove(Object)`/`remove(int)` row this table used to file them under: java's candidate set at `orderedMap.remove(k)` is ONE member, and the second alternative is the one THIS ENGINE'S OWN MINTED PARENT contributed — `ENGINE-LIMITS.md` K27 |
| the 34, by owner | `HtmlAppendableBaseTest` 9, `PlaceholderReplacerTest` 6, `BitFieldSetTest` 6, `OrderedMapTest` 3, `OrderedMultiMapTest` 3, `OrderedSetTest` 3, `HtmlBuilderTest` 3, `PlainSegmentBuilderTest` 1 — **eight files of fifty-two**, and every one of them is a caller of a residue §10.6.3 already names. `tagLine` (12) is the library's own overload row seen from a caller; the nine `OrderedSet`/`OrderedMap`/`OrderedMultiMap` rows are K26's `DeclaredSubtype` seam at `addAll`/`retainAll`; `BitFieldSet` (6) is K25's held-back `iterator()` met by a `for` loop |
