...
| tests | 639 `@Test` upstream, **575 emitted, 575 RUN — 161 -> 357 -> 364 -> 392 -> 552 -> 554 -> 559 -> 567 passing, 414 -> 218 -> 211 -> 183 -> 23 -> 21 -> 16 -> 8 failing** (§10.5.5; the last three steps are K20's carrier, K21 face 1's egress bridge and K21 face 2's bean accessors, one measured commit each) |
...
- **The difference catalog's four coverage lanes** (`DESIGN.md` §2.8). Their first honest numbers,
  ...
  |---|---|---|
  | `catalog(consulted)` | 5–8 per port | rows the run REACHED. `LibgdxTestMigrate` is the 8 — the one port that reaches the PHASE surface as well as both lowering dispatches, so no discharge site built so far is inert |
  | `catalog(unreached)` | 1–4 per port | a row whose surface EXISTS and which this port never reached. Per-port informational; `just catalog-coverage` is where the question is really asked, and corpus-wide it is now **one** row — JS-E17, `Open` and therefore unconsultable by rule (ii). JS-E04 left this set when its fix landed: it is reached on the ten ports that have a compound assignment in EXPRESSION position and unreached on the five that do not, which is what a coverage lane is supposed to look like |
  ...
  | `catalog(undischarged)` | **1**, identical on all fifteen | the WORK LIST, and the one entry on it is DECLARED: JS-E17, the lvalue's single evaluation (`ENGINE-LIMITS.md` F7), on every port that lowers a compound assignment in either position. It is an `Open` row, so a fatal log counts it rather than raising — a mode that died on the work list would make the work list unrunnable. JS-E04 was the other entry and is gone: `ENGINE-LIMITS.md` F8 is what closing it cost, and the answer is **0 member digests on all fifteen ports**, because no corpus library uses the shape |
