...
**And there is a conformance oracle nobody had ever run — IT RUNS NOW, and the census is §10.6.7.**
Upstream ships six versions of the CommonMark spec as classpath resources (618–652 examples each) and
four `FullOrigSpec*CoreTest` classes drive them through PLAIN `@Test` methods — not
`@RunWith(Parameterized.class)`, so they were inside `TestFrameworkTransform`'s mechanical reach as
...
| `collection-internal` | **0 after wave 14**, from the 7 the lane arrived at (`ENGINE-LIMITS.md` K26). The two `DeclaredSubtype` rows (`OrderedMultiMap#keys`/`#values`) closed at wave 9 with their two compile errors, and the five `SplitTypeVariable` rows — exactly the five `MutableDataHolder.set`/`BuilderBase.set` errors — closed at wave 14, with theirs. Both falls carry the attribution §5 requires of a lane that falls. The lane's own arrival was provably flat: 0 errors moved, 0 member digests on any of the sixteen port reports, and every other port still reads **0** |
...
so the remaining distance is a number rather than an absent source set. **After wave 17 that number
is ONE, in the library's main set, and it is the first error this corpus has ever seen from PAST the
typer** — G21's two closed and the typer count reached ZERO, which let a phase gated on
...
they gate on the typer, so each closed row can reveal the next and the headline says nothing about
how many are left. Only `errors.tsv`'s MEMBER column can, and it moved from `ReferencingNode` to
...
**AND AFTER WAVE 18 THE NEXT WAVE STARTS SOMEWHERE ELSE ENTIRELY**, because the two rows above were
the last typer-era ones and the compiler has now looked at the whole port. The work is the wave-18
census: **100 rows are K28's five verdicts** and K28 says how to answer each (strip the modifier,
strip it again, translate the two `E164` result mismatches, synthesise the three abstract members) —
built now against a NUMBER rather than a probe, which is the order that entry asked for. **31 rows
...
| before → after | family | where |
|---|---|---|
| 60 → 42 | **a DIAMOND FORWARDER minted over a `final` parent member** — `diamondOverrides`' whole argument is that SCALA REFUSES WHERE JAVA DID NOT (a concrete superclass method simply IMPLEMENTS the interface's, and scala linearises and reports a conflict), and at a `final` member scala does not refuse: the inherited member implements the mixin's declaration exactly as java's does, so there is nothing to disambiguate and the forwarder is an override of a `final` member — `E164`, at a member whose body, name and formals are all correct, and the emitter's own doing. Six owners × three members on `IRichSequenceBase`'s `split`/`countLeading`. Where the MIXIN's member is concrete too, scala cannot express java's answer at a `final` member at all, and what is left is scalac's own conflict message naming both parents, which is strictly better than an override it will reject. One conjunct on the superclass member's own flag; every other port BYTE-IDENTICAL | K28 |
...
**What wave 19 did NOT close, and why each is a different family:** 21 `E164` — 17 at a collection
...
FORWARDER minted over a `final` parent member — CLOSED in this same wave, `60 -> 42` — and 4 are an
override edge across a
...
**Closed by wave 21**, engine (a) with no manifest entry anywhere:

| before → after | family | where |
|---|---|---|
| 30 → 27 | **`override` ACROSS A SUBSTITUTION — three `needs \`override\` modifier` rows, and the parser's resolution was only half the answer.** Java writes no `@Override` for most of its overrides, so `SpoonTir.overridesInherited` falls back to a signature comparison over the ancestors that DO resolve — and it compared `getQualifiedName` strings, so `handle(N, …)` above and `handle(Item, …)` below are one member to java (JLS 8.4.2) and two strings to it. The frame is now composed one `extends` edge at a time, each clause's ACTUALS read through the frame in force where they are written — `SpoonTir.actualFor`'s composition at a deliberately coarser value type, since this comparison is over an already-erased name — and a RAW supertype contributes an EMPTY frame, so the match declines where java would erase. **48 member digests for 3 errors**, every one a member that GAINED the modifier through a substituted parent (`Comparable<CharSequence>#compareTo`, `BiFunction#apply`, `Map<String,T>#put`, four visitor `visit`s), which is what a correct answer to a declined question looks like. The fixture CANNOT reproduce the decline — the two answers are OR-ed and Spoon resolves anything spec-sized — so `SubstitutedOverrideSpec` pins the modifier and the two negatives and says so in its header | K28.2 |
| 31 → 30 | **an override edge across a PACKAGE BOUNDARY at a java `protected`, and the mechanism was ALREADY THERE** — `HeaderIdGenerator`'s anonymous `visit` shipped `protected[renderer]` over a parent qualified `protected[util]`, while the SAME body's `preVisit` two lines up shipped the correct `protected[md]` with its `x-pkg-protected-override` decision. The difference is that the parent OVERLOADS `visit` at arity 1, and `Visibility.methodsIn` indexed members as `Map[(name, arity), SymId]` — one key, one value, java's body order deciding which, the public overload winning and the fold then reading the visibility of a member the child does not override. A loose key indexes to a `List` and the caller decides; holding all candidates is the safe direction because the fold takes their COMMON package and an override may be wider and never narrower. **2 member digests in the one owner** | K28 |
...
`needs to be abstract`, which is K28.1's own three-commit design. The wave also SHARPENED that entry
against the port rather than against its probe, in three places worth reading before starting: the 17
are three different scalac sentences and `errors.tsv` shows none of them; the caller adaptation the
entry priced as the middle commit's cost is **one** call site in the whole port, because a class that
DEFINES a java collection is called through java's own interface by everyone else and the phase
already rewrote those; and `iterator()` is a FORK the reduced probe could not show, two of its five
rows returning a type the library declares rather than a shim. **The suite is still not run** — it
is gated on a whole-compile of zero, and 723 tests plus every `CLAUDE.md` §4.4 form in a
character-level markdown parser are behind that gate.
...
— closing any of them means re-opening that entry with a number, not finding a bug. **Waves 13 and 14
did exactly that to ELEVEN of them, and the lesson is the classification rather than the count**: G8's
six were refused against the wrong question (a FILL, where every site wanted a SELECTION's type), and
K26's five said *the formal has no HEAD to coerce against*, which is true of the formal as WRITTEN and
false of the one java RESOLVED. **A refused row is a census row and is re-read like one** — that is
now the table's own standing lesson, arrived at twice:
...
| `TrackedOffsetList#spliterator` | **K23**'s SECOND refusal — `spliterator()` is a DEFAULT METHOD re-declared at three owners with three characteristic sets, so reproducing java's at the owner the receiver was typed by models nothing about streams. **And the NEAR MISS the refusal recorded as its evidence does not reproduce** — the converter's wrapper reports the same `16464` the fix does, so the refusal had been resting on a stale measurement | 4 → 3, 2 member digests, `findings.tsv` byte-identical |
...
**WAVE 17 — 2 → 1 in three commits, and the 1 IS NOT A TYPER ERROR.** Both `resolveDependencies` rows
closed, which is G21's second half finally shipping; what the port ran into on the other side is the
thing §3 has been promising since this repository started.

| closed | entry | measured |
|---|---|---|
| — | **ONE derivation says what kind of type reference this is.** Spoon's `CtWildcardReference` EXTENDS `CtTypeParameterReference`, so thirteen `SpoonTir` matches carried a wildcard arm that never ran; wave 16 priced the obvious repair (reorder the arms) at 5 → 8, twice. The taxonomy is now a value — `TypeShape` / `TypeShape.of` — every match reads it, and each site STATES the wildcard answer it had been giving without saying so. FOURTEEN sites, not thirteen: `tpBoundErased`'s exclusion rides on a GUARD rather than an arm, so the grep census could not see it | 2 = 2, **0 member digests on all seventeen port reports**, every count and every findings line unchanged |
| — | **a `?` is not a type VARIABLE** — the first preserved shadow flipped, so something can finally ask *does this written argument mention a type variable* | 2 = 2, 0 digests everywhere, and exactly ONE findings line per port: `catalog(consulted)`'s JS-G32 `fired` DOWN on all seventeen (libGDX 1328 → 1262, ssg-md 1494 → 1334), which are FALSE firings leaving — a `Foo<?>` formal is written in none of the callee's type variables |
| `DependencyResolver#resolveDependencies` ×2 | **G21** — the erased receiver view decided PER POSITION. `Function<? super D, Class<?>>` leaves position 0 unknown and WRITES position 1, and java's own `apply` returns `Class<?>` there; erased at both, a value java handed straight to a `Class<?>` slot needed a conversion java never performed. Narrowed to positions whose written actual mentions NO TYPE VARIABLE, which is exactly the class the three argument erasures cannot disagree about — the `OrderedMap#putAll` 0 → 1 regression is a variable-mentioning position | **2 → 1**, 2 member digests, and **every other lane byte-identical** |

**And the 1 is `E057 Type argument B does not conform to upper bound Node`, in a file whose emitted
text has not moved for waves.** It appeared only when the two `E007`s went away, and that is not a
coincidence: dotty's `Phase.isRunnable` is `!ctx.reporter.hasErrors`, so the phase that checks an
applied type's bounds runs only at zero typer errors. **Verified rather than inferred** — injecting
one deliberate typer error into the emitted tree makes it vanish again. So the count did not fall
from 2 to 1; **it fell from 2 to ZERO and a post-typer phase reported its first row.**

Three things follow, and none of them is the one this section predicted:

- **the first riser is not a missing `override` and not a variance violation.** It is a RAW BOUND
  name-filled from the enclosing declaration at a type that is not the declaring one, so
  `B extends ReferenceNode` became `B <: ReferenceNode[R, B, ?]` on a NAME coincidence and re-imposed
  a bound java never checked — `CLAUDE.md` §4.56 met at a bound, and `ENGINE-LIMITS.md` G30;
- **K28's collection wall is BEHIND this row, not beside it.** `RefChecks` is later than the phase
  that reported this one, so the 30–50 errors K28 prices are still unmeasured and will arrive when
...
fine, an ORDINARY bounded wildcard capture-converts unaided, and NO spelling of the wildcard repairs
it (java's own `? extends ISegmentBuilder<?>` fails identically) — all three measured at scalac 3.8.4
rather than reasoned about. Java's enhanced `for` reads the ITERABLE SUPERTYPE and not the receiver's
own type (JLS 14.14.2), so putting the operand there is what java itself did:
`ENGINE-LIMITS.md` G31, closed, **4 changed member rows** (one member and its file digest).

**AND THAT ONE ROW WAS THE GATE. `RefChecks` RAN FOR THE FIRST TIME IN THIS CORPUS: 1 -> 131.**
§3 has promised this since the repository started and K28 priced it eighteen waves ago; both were
right about the shape and low about the size. The whole compile is **131 = 131 main + 0 test** — the
test set contributes nothing, which is the same zero it has held since wave 12 — and this is the
census, every row counted from `errors.tsv`:

| n | family | which of K28's verdicts | §1 |
|---:|---|---|---|
| **48** | `E037 overrides nothing` at a MINTED collection parent — `putAll` 5, `forEachRemaining` 5, `entrySet` 5, `containsValue` 5, `containsKey` 5, `retainAll`/`removeAll`/`forEach`/`containsAll` 3 each, and nine more. Every one is a JDK collection member name | **STRIP the `override`** | **(a)** K28 |
| **25** | `E038 different signature` — `get(Object)`, `remove(Object)`: scala declares the NAME at different PARAMETERS, so the pair is an OVERLOAD SET | **STRIP the `override`** | **(a)** K28 |
| **17** | `E164 has incompatible type` at a collection parent — `MapOps.put` (5), `IterableOnce.iterator` (5), `MapOps.values` (4), `MapOps.keys` (2), `SeqOps.size` (1 — the `final` one K28 named by hand, found on exactly the class it said) | **the only arm needing a real translation** | **(a)** K28 |
| **10** | `needs to be abstract` on a concrete collection class — scala's abstract members with no java counterpart (`get(K): Option[V]`, `addOne`, `subtractOne`) | **SYNTHESISE** | **(a)** K28 |
| **22** | `E164` at a PROGRAM-DECLARED parent — **not K28's family at all**: `split`/`countLeading` in `IRichSequenceBase` (18), `processNode` in `AstActionHandler` (3), one `visit`. The port's OWN hierarchy, unmeasured for exactly as long as everything else was | — | **(a)**, undiagnosed |
| **7** | `needs to be abstract, since def getBuilder[B <: ISequenceBuilder[B, T]](): B … is not defined` — an F-BOUNDED GENERIC METHOD `IRichSequence` declares and no implementor defines. Seven sequence classes, one row each | — | **(a)**, undiagnosed |
| **2** | `private variable finalize cannot override method finalize in class Object`, and `chars` against `CharSequence.chars` — a java FIELD whose name is a JDK method, which is §4.55's implementation-pair rule met from the other side | — | **(a)** |

**Thirty-six owners, not seven.** K28 priced a reduced class and multiplied; the collection classes
really are the ones it named (`TrackedOffsetList` 24 rows, `OrderedMap`/`OrderedMultiMap` 12 each,
`ItemFactoryMap` 9, `BitFieldSet`/`OrderedSet`/`IndexedItemSetMapBase` 8 each, `NodeRepository` 7),
and they are 8 of the 36. **100 of the 131 are K28's five verdicts; 31 are three families nobody had
looked at**, which is the honest correction to a probe that prices a shape and cannot price a
population.

**The suite still does not run**, and the distance is no longer one row. Discovery holds at
**723 of 723 with 0 lost**.
...
**And wave 14's second commit is the stronger half, because it drained a LANE.** `collection-internal`
existed precisely to count the seam no other instrument could see, and it read **5** here for six
waves. The five went to **0** with the five errors they named — which is the arithmetic §5 asks of a
lane that falls, and it is the first time this port has had it on the internal lane. Nothing about the
refusal's SENTENCE was wrong; what was wrong was reading a fact about the formal java WROTE as a fact
about the formal java RESOLVED.
...
- **it is not a substitute for the counts — and that objection is now DISCHARGED, TWICE OVER.** Two of
  ...
  external half. Wave 8 gave the internal half its own lane: `collection-internal` arrived at **7**
  here and **16** on the test set, at 0 errors moved and 0 member digests on any port report
  (`ENGINE-LIMITS.md` K26), and it now reads **0** on both. Its two `DeclaredSubtype` rows
  (`OrderedMultiMap#keys`/`#values`) closed at wave 9 and its five `SplitTypeVariable` rows — one type
  variable bound to both sides of a broken edge — at wave 14, each fall carrying exactly the errors it
  named. What it does NOT attribute is stated rather than averaged over: `Attributes#values` and
  `BuilderBase#extensions` sit at a callee THIS PHASE MINTED, which carries no signature, and the arm
  that read such a call from its OPERANDS alone was built, measured at 2 rows of which 1 was false,
  and removed — K26 carries the number and the repair (mint the helpers with their signatures). The
  `ScopedDataSet#getKeys` pair that used to sit beside them closed at wave 12 and, exactly as this
  bullet predicts, **no lane fell with it**.
...
**AND THE PORT HAS NOW MET `RefChecks`** (§3) — at wave 18, and the count ROSE `1 -> 131`, which is
this section's own prediction arriving rather than a regression. Wave 17 emptied the typer; wave 18
closed the two post-typer risers that gated everything after them (`ENGINE-LIMITS.md` G30, G31), and
then every missing `override`, unimplemented member and variance violation in 468 emitted files was
measured at once. §10.6.3's wave-18 row is the census; what follows in this section is the PRICING
that predicted it, kept because scoring it is the only way to know what a reduced probe can buy — and
what it bought is stated in `ENGINE-LIMITS.md` K28's own scoring row: **100 of the 131 are its five
...
**AND WAVE 19 CLOSED BOTH *STRIP THE `override`* VERDICTS TOGETHER — 131 -> 60, then the emitter's
own `final`-parent diamond forwarder, 60 -> 42**, which is 47 `E037`
...
is left is 42 and it is no longer this section's prediction to score**: 21 `E164` (17 at a collection
parent — `MapOps.put` 5, `IterableOnce.iterator` 5, `values` 4, `keys` 2, `SeqOps.size` 1 — and 4 at
a PROGRAM-DECLARED one, `processNode` 3 and one `visit`), 17 `needs to be abstract` (10 collection classes, 7 the `getBuilder` F-bound), 2
...
**AND WAVE 20 TOOK THE `getBuilder` FAMILY WHOLE — 42 -> 34**, the 7 `needs to be abstract` and the
1 `E038` together. It is the family this census had filed under K28 and it was never a minted-parent
question at all: java's JLS 8.4.2 SUBSIGNATURE rule lets `SequenceBuilder getBuilder()` override
`<B extends ISequenceBuilder<B,T>> B getBuilder()` — an ERASURE overriding a generic method, which
javac accepts with an unchecked warning the library writes a `@SuppressWarnings` for — and scala has
no such rule. The fix erases that F-bounded, RESULT-ONLY type parameter at the DECLARATION, which is
the position `ENGINE-LIMITS.md` G8.7 could not reach and the only text G8's four measured attempts
left available (`ENGINE-LIMITS.md` G8.10). Two other numbers moved and both are attributable:
`overload-risk` **563 -> 557**, the six `GenericTieBreak` rows at `getBuilder/0` where two candidates
spanned java's resolution phases and now one does; and `catalog(consulted)` 95 -> 96 for the new
`JS-G49`. Every other port is BYTE-IDENTICAL — 0 member digests on libGDX — and gains only the two
new catalog rows at `fired 0, declarations 0`.

**What is left is 34, in four families, and the three biggest are DIAGNOSED WITH A PROBE rather than
by eye** (`ENGINE-LIMITS.md` K28.1/K28.2): 17 `E164` at a collection parent + 10 collection
`needs to be abstract`, whose whole target shape was put through `scala-cli` and compiles except for
ONE thing — `iterator` colliding between a minted `mutable.Map` and a minted `JavaIterable`, a
DUPLICATE relation rather than a member, and dropping the redundant shim parent compiles the rest;
the 4 rows this census called `protected[visitor]`, which turn out to be **two** families and only
...
`error overriding method … of type (…)`. Read from the full compiler output, **three of them say
`needs \`override\` modifier`** and have nothing to do with visibility at all: java's
`BlockNodeVisitor#processNode` overrides `AstActionHandler`'s without writing `@Override` (its body
calls `super.processNode`), and the frontend's `isOverride` — which rests on Spoon's
`getTopDefinitions` where the java writes no annotation — does not survive the substitution through
an F-bounded four-parameter superclass. Only `HeaderIdGenerator#visit` is the visibility row, and it
reads *has weaker access privileges; it should be at least protected* at an emitted
`protected[renderer]` whose parent lives in another package. Two different fixes, and the census had
one name for both (`ENGINE-LIMITS.md` K28's corrected bullet).
...
lane gates the run on, and the first behavioural evidence this port has ever had arrived immediately.
The census, every row classified and every count DERIVED rather than eyeballed:
...
| **§4.4 family — an assertion over a value whose `toString` matches and whose CLASS does not** | **2, DECLARED at wave 24** | both `test_getLineColumnAtIndex`, `anchor=assert-site`, both reading *values are not equal even if they have the same `toString()`: (0, 0)*. Wave 24 read the emitted code rather than the message and the diagnosis moved one class to the left: the defect is not at the assertion, it is that **`Pair.equals` answers `false` for two equal `Pair`s, library-wide**. `Paired` implements `Map.Entry`, so the phase KEPT java's parent (the counted `InexpressibleParent`, visible as a porter note on the emitted trait) and retyped the reified `o instanceof Map.Entry` and its downcast to `scala.Tuple2` — which no `Pair` is at run time. Both halves are already counted where they happen (`collection-boundary/ReifiedOccurrence`, `Pair.java:65` and `:67`), so a bridge at the ASSERTION would hide an emitted-library defect every consumer sees; declared instead, with the anchor, under the same engine family liqp's `SortTest#testSortMap` is (`ENGINE-LIMITS.md` K18.1, K5.7) |
| **genuine, uncensused** | **1, DIAGNOSED AND CLOSED at wave 24** | `BitFieldSetTest.test_BitSetIteratorRemove`, `anchor=assert-site` — and it was an ENGINE defect in a runtime helper, not a translation one. `JavaCollections.toArray` allocated `new Array[Object](xs.size)` and filled it by iterating; `AbstractCollection.toArray` allocates on `size()` and then RECONCILES it against the iterator in both directions, and `BitFieldSet.size()` returns `totalBits` — the fixed universe — so the port answered `ArraySeq(…4 values…, null × 28)` where java trimmed. The message reads as an ordering problem and the null tail is below the fold, which is why it survived a wave uncensused (`ENGINE-LIMITS.md` K31) |

Two things this census settles that no compile could. **The port's own refusal lane PREDICTED the
whole of the first row**: `@Rule × 6` has been in the table above since wave 7, with the sentence *the
...
- **17 refuse**, every one of them `non-statement-position`: a `thrown.expect` inside a `while` or
  ...
  the lowering that WOULD reach all 37 — modelling the rule — with the price of building it. The
...
The 17 were one family behind one guard, and the guard was POSITION: an `intercept` wraps a LEXICAL
region and java's rule is armed from the `expect` CALL to the end of the test. So the `intercept`
shape is GONE, replaced by junit's own state machine — the matcher list `ExpectedException`
accumulates (`var bpExpected`), one `var bpCaught`, each `expect`/`expectMessage` lowered to an
APPEND where java wrote it, and one `try`/`catch` over the whole converted body, which is
`ExpectedExceptionStatement.evaluate` plus `failDueToMissingException` transcribed. **All 17 flipped;
the 20 wave 24 had already converted stayed passing.**

Three things this is worth reading for, none of them the 17:

- **it is ONE lowering, not two.** The alternative — keep `intercept` for the statement-position
  sites and model the rule only where it refused — is the shape X5 priced as *a second lowering for a
  construct the first one already covers 20 of*, and it is two spellings of one translation
  (`CLAUDE.md` §5's one-policy-one-spelling, read at a phase). The model replaces it;
- **FIVE of the seven enumerated deltas stopped being GUARDS**, which is the real yield.
  `double-expect` is junit's own accumulation and is now a `forall`; `non-contiguous` and
  `expect-message-only` are positions and mean nothing against an in-place arming; `after-ordering`
...
- **the state is a LIST because the cheap version is wrong in the DANGEROUS direction.** An arming
  ...
  loop — so this is a shape argument and not a measurement, and the shape is junit's.

Blast: **43 member digests over six suites** — the 37 converted statements plus their six owning
units, and nothing else (§3's attribution gate). The only check counts that moved are
`catalog(unreached) 18 → 15` and `catalog(consulted) 93 → 96`, all three of them the `try` rows
(`JS-S11`, `JS-S12`, `JS-S13`): before this wave the emitted TEST set contained no `try` at all.
`JS-S11` — a translated `catch` swallowing a translated JUMP — is consulted 37 times and **fires 0**,
so no jump crosses one of these catches and `break-catch` stays 0.

### THE FLOOR AFTER WAVE 25 — **721 passing / 2 failing of 723, 0 lost, 0 compile errors on both source sets, and EVERY failure attributed**
...
|---|---|---|
| **`Pair.equals` over a reified `Map.Entry`** | **2** | DECLARED, with anchors. The fix is *weighed and refused on a number* (`ENGINE-LIMITS.md` K18.1 — 2 sites in the whole corpus), the mechanism is counted twice where it happens (`collection-boundary/ReifiedOccurrence`, `Pair.java:65` and `:67`), and the identical shape is already declared on liqp. That is a stated answer rather than an unbuilt lowering — which is exactly what the 17 were NOT, and why they took a work item and never an `expected-failures.tsv` entry |

So the number to quote for this port is **0 open** — every deviation from java's own suite is
attributed, and the one that remains is a decision the port stands behind rather than a fix nobody
...
is emitted. The 114 `ComboSpecTestCase` subclasses stay out of scope and are the documented refusal
§10.6.1 states; the CommonMark conformance oracle is §10.6.7's, and it is IN now.

### 10.6.7 THE COMMONMARK CONFORMANCE CENSUS — the first spec-level behavioural evidence this project has ever had

§10.6.1 recorded an oracle nobody had run: six CommonMark spec files shipped as classpath resources
and four `FullOrigSpec*CoreTest` classes that drive them through the ONE plain `@Test` that
`FullSpecTestCase` declares. This wave ported the infrastructure and ran it. **The test port's scope
is now THREE upstream trees and one emitted source set** — `flexmark-util/src/test/java` (the 52-file
unit suite), `flexmark-test-util/src/main/java` (31 files of harness, entirely non-reflective) and
five files of `flexmark-core-test/src/test/java` — **88 emitted Scala files, 0 dropped, 0 injected,
...
initialiser to build the HTML5 entity table, so every `&nbsp;` in every document needs it. Three of
the four conformance suites died on it — `ExceptionInInitializerError`, then `NoClassDefFoundError`
...
### 13.2 Wave 0.1 measurements

**`just upstream-pin`** (2026-08-25): 14 vendored trees checked, **0 mismatches**. Every vendored
upstream tree matches the reference repo's submodule pin:
libgdx `4b4d2c4f`, ashley `d63d5422`, simple-graphs `f7e17a83`, anim8-gdx `38634cef`,
noise4j `440388c2`, jbump `6dac3ba9`, gdx-gltf `683054a8`, screenmanager `0c350bfd`,
...
- **gdx-ai declares no `parity` reference** (`api-parity` records 0 lanes on `GdxAiMigrate`
  ...
  opens with `PortManifest.parity` pointing at `../sge/sge-extension/ai/src/main/scala`.
