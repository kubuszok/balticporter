...
class EngineSuite extends munit.FunSuite {
  ...
  // createComponent tests excluded on JS/Native: ported ComponentFactories.create uses JVM
  // reflection (java.lang.reflect.Constructor) as a fallback, which the Scala.js/Native linker
  // cannot resolve. These tests run on JVM only (via the adjusted/ directory).
