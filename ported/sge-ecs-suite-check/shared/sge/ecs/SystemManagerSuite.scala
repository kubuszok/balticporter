...
class SystemManagerSuite extends munit.FunSuite {
  ...
  // Adjusted: ported SystemManager.getSystem returns T (nullable) instead of Nullable[T]
  test("getSystem returns system by class") {
    ...
    assert(manager.getSystem(classOf[SystemA]) != null)
    assert(manager.getSystem(classOf[SystemA]) eq a)
    assert(manager.getSystem(classOf[SystemB]) != null)
    assert(manager.getSystem(classOf[SystemB]) eq b)
    assert(manager.getSystem(classOf[SystemC]) == null)
  }
  ...
  test("addSystem replaces existing system of same class") {
    ...
    assert(manager.getSystem(classOf[SystemA]) eq a1)

    manager.addSystem(a2)
    assertEquals(manager.getSystems.size, 1)
    assert(manager.getSystem(classOf[SystemA]) eq a2)
