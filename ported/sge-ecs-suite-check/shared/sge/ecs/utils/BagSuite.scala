...
class BagSuite extends munit.FunSuite {
  ...
  // array directly, so reading beyond capacity throws (ArrayIndexOutOfBoundsException on JVM,
  // UndefinedBehaviorError on JS/Native). Within-capacity but beyond-size slots still hold their
  // array default (null for reference types). Beyond-capacity case tested on JVM only.
