...
class PooledEngineSuite extends munit.FunSuite {
  ...
  // Adjusted: ported PooledEngine.createComponent falls through to ComponentFactories.create which
  // uses JVM reflection as a fallback for unregistered types with a no-arg constructor. The sge
  // reference throws IllegalArgumentException to enforce explicit factory registration. On JVM the
  // reflective fallback succeeds, so the test verifies the component is created instead.
  test("createComponent for an unregistered component type falls through to reflection on JVM (ISS-723 c12)") {
    val engine    = new PooledEngine()
    val component = engine.createComponent(classOf[PooledComponentA])
    assert(component.isDefined, "expected reflective fallback to produce a component on JVM")
  }
}
