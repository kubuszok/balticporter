// ADJUSTED COPY of sge's demos/hex-tactics/src/main/scala/demos/hextactics/HexTacticsGame.scala for
// the demo check (PROGRESS.md §13.29, standing order 1: adjusted where the port's API differs). The
// differences are enumerated in ported/demo-check/ADJUSTMENTS.tsv; sge itself is never edited.
/*
...
object HexTacticsGame extends DemoScene {
  ...
  private var hexCenters: ObjectMap[java.lang.Integer, Vector2]     = uninitialized
  ...
  override def init()(using Sge): Unit = {
    ...
    hexCenters = ObjectMap[java.lang.Integer, Vector2]()
    generateMap()
    placeUnits()
    cacheHexCenters()
    currentTurn = 0
  ...
  private def getHexCenter(col: Int, row: Int): Vector2 = {
    val key = col * 100 + row
    val cached: Nullable[Vector2] = hexCenters.get(java.lang.Integer.valueOf(key))
