/*
 * ADJUSTED COPY of sge/demos/asset-showcase/src/main/scala/demos/assets/AssetShowcaseGame.scala — see the
 * `setLoader` block in `init` and ported/demo-check/ADJUSTMENTS.tsv; sge itself is never edited.
 *
...
object AssetShowcaseGame extends DemoScene {
  ...
  override def init()(using Sge): Unit = {
    ...
    // ADJUSTED (ported/demo-check/ADJUSTMENTS.tsv): sge writes its .g3db through its own UBJsonCodec —
    // in the current UBJSON draft (`i` = one byte) where java's UBJsonReader defaults to the old draft
    // (`i` = two bytes, libGDX's fbx-conv files), and with every `None` field as a JSON null, which
    // java's G3dModelLoader reads as a PRESENT colour. The binary model loader is registered with a
    // reader that speaks the new draft and drops null members, as sge's codec reads `None`.
    assetManager.setLoader(classOf[Model], ".g3db", {
      val reader = new sge.utils.UBJsonReader() {
        private def dropNulls(v: sge.utils.JsonValue): Unit = {
          var c = v.child.orNull
          while (c != null) {
            val n = c.next.orNull
            val nm = c.name.orNull
            if (c.isNull() && nm != null) v.remove(nm) else dropNulls(c)
            c = n
          }
        }
        override def parse(file: sge.files.FileHandle): sge.utils.JsonValue = {
          val v = super.parse(file)
          dropNulls(v)
          v
        }
      }
      reader.oldFormat = false
      new sge.graphics.g3d.loader.G3dModelLoader(reader, FileHandleResolver.Internal())
    })
