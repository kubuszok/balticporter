...
//   ../sge/sge-extension/textra/src/test/scala/sge/textra/utils/StringUtilsSuite.scala
...
//
// mapping rows applied here: T4, T8
// ---------------------------------------------------------------------------------------------
package sge
package textra
package utils
...
class StringUtilsSuite extends munit.FunSuite {
  ...
  test("join with comma delimiter") {
    assertEquals(StringUtils.join(", ", scala.Array[CharSequence]("a", "b", "c")), "a, b, c")
  }

  test("join with empty items returns empty string") {
    assertEquals(StringUtils.join(", ", scala.Array[CharSequence]()), "")
  }

  test("join with single item returns that item") {
    assertEquals(StringUtils.join(", ", scala.Array[CharSequence]("hello")), "hello")
  }
