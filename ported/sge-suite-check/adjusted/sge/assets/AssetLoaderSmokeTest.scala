...
class AssetLoaderSmokeTest extends FunSuite {
  ...
  test("SoundLoader.loadSync fails when loadAsync has not run") {
    val result = SoundLoader(stubResolver).loadSync(bareManager, "s.ogg", handle("s.ogg"), SoundLoader.SoundParameter())
  ...
  test("MusicLoader.loadSync fails when loadAsync has not run") {
    val result = MusicLoader(stubResolver).loadSync(bareManager, "m.ogg", handle("m.ogg"), MusicLoader.MusicParameter())
    assert(result == null, "port returns null when loadAsync has not run")
  }

  test("PixmapLoader.loadSync fails when loadAsync has not run") {
    val result = PixmapLoader(stubResolver).loadSync(bareManager, "p.png", handle("p.png"), PixmapLoader.PixmapParameter())
    assert(result == null, "port returns null when loadAsync has not run")
  }

  test("I18NBundleLoader.loadSync fails when loadAsync has not run") {
    val result = I18NBundleLoader(stubResolver).loadSync(bareManager, "i18n/msg", handle("i18n/msg"), I18NBundleLoader.I18NBundleParameter())
    assert(result == null, "port returns null when loadAsync has not run")
  }
