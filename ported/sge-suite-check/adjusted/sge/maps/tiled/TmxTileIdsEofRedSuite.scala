...
class TmxTileIdsEofRedSuite extends munit.FunSuite {
  ...
  private def assertPrematureEof(element: XmlReader.Element): Unit =
    Try(BaseTmxMapLoader.getTileIds(element, 4, 4)) match {
      ...
      case Failure(e) =>
        val msg = Option(e.getMessage).getOrElse("") + Option(e.getCause).map(c => " " + c.getMessage).getOrElse("")
        assert(
          msg.contains("Premature end of tile data"),
          s"expected the premature-EOF message, got: ${e.getClass.getName}: $msg"
        )
    }
