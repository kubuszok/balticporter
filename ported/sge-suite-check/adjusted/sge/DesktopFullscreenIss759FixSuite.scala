...
class DesktopFullscreenIss759FixSuite extends munit.FunSuite {
  ...
  /** A do-nothing listener. Port's ApplicationListener has more abstract members than sge's. */
  private def listener: ApplicationListener = new ApplicationListenerDefaults {
  ...
  test(
    ...
    // Port's DesktopDisplayMode carries monitorHandle in case-class equality — compare display properties only
    assertEquals((modeFromA.width, modeFromA.height, modeFromA.refreshRate, modeFromA.bitsPerPixel),
                 (modeFromB.width, modeFromB.height, modeFromB.refreshRate, modeFromB.bitsPerPixel),
                 "fixture sanity: the two monitors must expose modes with equal display properties")
