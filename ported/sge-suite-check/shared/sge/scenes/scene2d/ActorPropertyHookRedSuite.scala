...
class ActorPropertyHookRedSuite extends munit.FunSuite {
  ...
  final private class CountingActor(using Sge) extends Actor() {
    ...
    override def positionChanged(): Unit = positionChangedCalls += 1
    override def sizeChanged():     Unit = sizeChangedCalls += 1
    override def scaleChanged():    Unit = scaleChangedCalls += 1
    override def rotationChanged(): Unit = rotationChangedCalls += 1
  }
