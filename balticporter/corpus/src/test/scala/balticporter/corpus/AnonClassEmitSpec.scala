...
/** A constant declared INSIDE an anonymous class must never be emitted as `inline val` (there is no companion to put it in), and a reference to it must never go through the class's synthetic FQN
...
class AnonClassEmitSpec extends munit.FunSuite:
  ...
  test("a reference to an anonymous class instance constant uses the bare name, not a FQN") {
    ...
    assert(!emitted.matches("(?s).*\\b\\d+\\.K\\b.*"), s"FQN reference through numeric anonymous class suffix:\n$emitted")
  }
