...
/** `type-redirect` + `memberRenames` END TO END — java in, emitted Scala out, through the pipeline a port runs.
  */
class TypeRedirectRenamePortSpec extends PortSuite:

  /** The shape a `Disposable`-style redirect meets: an interface, two implementors, one of them subclassed, a caller that uses the interface as a TYPE, and a caller that calls the member through each
    * of the three declarations — the case where a call through an implementor's symbol is a different `SymId` from a call through the interface's.
    */
  private val java =
    """package com.demo;
      |
  ...
  private def ported(renames: Map[String, String] = Map("dispose" -> "close")) =
    val phase        = new TypeRedirectTransform(redirects = Map("com.demo.Disposable" -> "java.lang.AutoCloseable"), memberRenames = Map("com.demo.Disposable" -> renames))
  ...
  test("the emitted SURFACE is the TARGET's, in every file — the interface, both implementors, the subclass") {
    ...
    assertEquals(
      c.linesIterator.count(_.contains("def close()")),
      4,
  ...
  test("every CALL moved with it, through all three receiver shapes") {
    ...
    val c              = code(out)
    List("b.close()", "p.close()", "s.close()", "any.close()", "this.held.close()").foreach(call => assert(c.contains(call), s"$call is not in:\n$c"))
  ...
  test("emitted probe is written for a real compiler — ONE FILE PER UNIT, as a port writes it") {
    ...
    val dir                    = _root_.java.nio.file.Path.of(sys.props.getOrElse("balticporter.dumpProbe", s"${sys.props("user.dir")}/target/probe"), "type-redirect-rename")
