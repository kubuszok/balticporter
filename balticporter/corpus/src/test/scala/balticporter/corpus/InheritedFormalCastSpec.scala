...
/** JAVA'S UNCHECKED CONVERSION AT AN *INHERITED* FORMAL — the one place a callee's type variable really does resolve at the call site.
  */
class InheritedFormalCastSpec extends munit.FunSuite:
  ...
  test("a RAW argument at an ancestor's type-variable formal takes java's unchecked cast") {
    ...
    assert(
      out.contains("super.take(h.asInstanceOf[demo.Box[java.lang.String]])"),
  ...
  test("…and it reaches INSIDE an array whose dimension already agrees") {
    val out = emitted(single)
    assert(
      out.contains("asInstanceOf[scala.Array[scala.Array[demo.Box[java.lang.String]]]]"),
  ...
  test("a DIMENSION mismatch is PACKED, and the cast lands on the ELEMENT — never on the array") {
    ...
    assert(
      flat.head.contains("scala.Array[scala.Array[demo.Box[java.lang.String]]](hs.asInstanceOf[scala.Array[demo.Box[java.lang.String]]])"),
  ...
  test("NEGATIVE: two ancestors' same-named variables are two keys, not one") {
    ...
    assert(
      out.contains("super.up(b.asInstanceOf[demo.Box[java.lang.String]])"),
      s"`Up`'s T must resolve to String, not to `Side`'s Integer\n--- emitted ---\n$out"
    )
    assert(
      !out.contains("super.up(b.asInstanceOf[demo.Box[java.lang.Integer]])"),
