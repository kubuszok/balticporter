...
class BaseSurfaceSpec extends munit.FunSuite:
  ...
  /** the units whose Java lives under `dep/` — what `PortRun.partitionUnits` computes, spelled the same way (by ORIGIN, realpathed) so the fixture and the run agree about ownership.
    */
  ...
  private def typeOf(p: Program, name: String): SymId =
    p.units.find(u => fqn(p, u.symbol) == name).map(_.symbol).getOrElse(fail(s"no unit named $name in ${p.units.map(u => fqn(p, u.symbol))}"))

  /** a contract carrying exactly the rows a test names — the artifact, assembled the way a base publishes it, so a row here is a row a real run writes.
    */
  private def contract(module: String, rows: (String, Surface.TypeShape)*): PortMap.Map0 =
    PortMap.of(
      module,
      "eng",
      rows.map(_._1).toList,
  ...
  test("a PUBLISHED row is read, and it is read EMITTED-name to EMITTED-name") {
    val (p, root) = model(basePkg, Map("q/Mine.java" -> "package q; public class Mine extends p.Base { }"))
    val shape     = Surface.TypeShape(
      form = "class",
      primary = Some(balticporter.tir.Descriptor(List(balticporter.tir.Param.Prim("int")))),
      primaryKind = "unique-root",
      primaryVis = "public"
    )
    val surface = new PublishedSurface(p, ownedUnits(p, root), List("base-mod" -> contract("base-mod", "p.Base" -> shape)))
  ...
  test("D15: a non-wall class whose published row disagrees is NON-FATAL — the dependent follows the base") {
    ...
    val wrong     = Surface.TypeShape(
      form = "class",
      primary = Some(balticporter.tir.Descriptor(List(balticporter.tir.Param.Named("String")))),
      primaryKind = "unique-root"
    )
    val surface = new PublishedSurface(p, ownedUnits(p, root), List("base-mod" -> contract("base-mod", "p.Base" -> wrong)))
  ...
  test("D4 CROSS-CHECK NEGATIVE: an AGREEING row is silent — the check is not noise") {
    val (p, root) = model(basePkg, Map("q/Mine.java" -> "package q; public class Mine { }"))
    val right     = Surface.TypeShape(
      form = "class",
      primary = Some(balticporter.tir.Descriptor(List(balticporter.tir.Param.Prim("int")))),
      primaryKind = "unique-root"
    )
    val surface = new PublishedSurface(
      p,
      ownedUnits(p, root),
      List(
        "base-mod" -> contract("base-mod", "p.Base" -> right, "p.Holder" -> right.copy(primary = Some(balticporter.tir.Descriptor(Nil))))
      )
    )
    val plans = CtorFunnel.Plans(p, Some(surface))
    plans(p.units.head)
  ...
  test("D6 NEGATIVE: a base type published as a `class` produces NO finding") {
    ...
    val surface   = new PublishedSurface(p, ownedUnits(p, root), List("base-mod" -> contract("base-mod", "p.Holder" -> kept)))
    val emitter   = new TirEmitter(p, surfaceView = Some(surface))
    emitter.emit
  ...
  /** Two roots reaching DIFFERENT parent constructors, so the synthesis refuses and the class keeps a nilary primary — which is what makes `Mine(int)`'s `super(a)` a SECONDARY's super call, the only
    * shape `replayFor` exists for (`ModelInstanceHack` in gdx-gltf, exactly).
    */
  ...
  /** a published MEMBER row, keyed the way the source map keys one — `owner#name(params)` for an executable, `owner#name` for a field.
    */
  ...
  private def memberRow(key: String, vis: String): PortMap.Entry =
    PortMap.Entry("member", key, key, PortMap.Disposition.Ported, shape = Surface.render(Surface.MemberShape(vis = vis)))
  ...
  test("D5: a replay reaching a BASE's `private` member is REFUSED, and the refusal is attributed") {
    ...
    val published = new PublishedSurface(
      ...
      ownedUnits(p, root),
      List(
        "base-mod" -> withMembers(contract("base-mod", "p.Base" -> Surface.TypeShape(form = "class")), List(memberRow("p.Base#touch()", "private")))
      )
    )
  ...
  test("D5 NEGATIVE: a member the base published PUBLIC is reachable, and no gap is recorded") {
    ...
    val published = new PublishedSurface(
      ...
      ownedUnits(p, root),
      List(
        "base-mod" -> withMembers(contract("base-mod", "p.Base" -> Surface.TypeShape(form = "class")), List(memberRow("p.Base#touch()", "public")))
      )
    )
  ...
  test("§4.55: a DEPENDENT's method does not rename the BASE's field — and the CLASH still moves") {
    ...
    val published = new PublishedSurface(
      ...
      ownedUnits(p, root),
      List(
        "base-mod" -> withMembers(contract("base-mod", "p.Base" -> Surface.TypeShape(form = "class")), List(memberRow("p.Base#x", "public")))
      )
    )
    ...
    val ds = new TirEmitter(p, surfaceView = Some(published)).ownDecisions.filter(_.detail.get("clash").contains("field-vs-method-in-base"))
    assertEquals(clue(ds).map(d => (d.detail("from"), d.detail("to"), d.detail("field"))), List(("x", "x$method", "p.Base#x")))
  ...
  test("an UNKNOWN base keeps the local derivation and RECORDS the question") {
    val (p, root) = model(clashBase, clashHeir)
    val published = new PublishedSurface(p, ownedUnits(p, root), List("base-mod" -> contract("base-mod", "p.Base" -> Surface.TypeShape(form = "class"))))
  ...
  /** What `export Parent.{… => _, *}` must exclude is the set of names the PARENT'S COMPANION actually delivers, and for a base parent that is a fact about the base's EMITTED output — a static it
    * renamed, a static the manifest dropped. Re-derived from the base's java the dependent gets java's names back, which is right exactly when nothing moved.
    */
  ...
  test("a base whose statics the manifest DROPPED delivers none, so no `export` is written at all") {
    ...
    val published = new PublishedSurface(
      p,
      ownedUnits(p, root),
      List("base-mod" -> contract("base-mod", "p.Holder" -> Surface.TypeShape(form = "class", companion = false, statics = Nil)))
    )
    val out = emittedWith(p, published)
  ...
  test("…and a base that DID emit them is re-exported, with the names it actually emitted") {
    ...
    val published = new PublishedSurface(
      ...
      ownedUnits(p, root),
      List(
        "base-mod" -> contract("base-mod", "p.Holder" -> Surface.TypeShape(form = "class", companion = true, statics = List("LIMIT", "twice")))
      )
    )
  ...
  /** `Base()` is a SECONDARY whose body runs AFTER its `this(1)` delegation and reads no parameter, so `Guarded`'s synthesised primary carries the boolean guard slot `via$pb` — a slot whose type a
    * PHASE minted rather than the frontend interning it from java. `Boxed`'s primary is java's own and takes the WRAPPER, which must not be spelled as a primitive.
    */
  ...
  /** the shapes a BASE publishes for its own tree, with the text it emitted — the same recording a real base run writes.
    */
  private def published(files: Map[String, String]): (Map[String, Surface.TypeShape], String) =
    ...
    val p       = SpoonTir.fromTypes(SpoonTir.buildModel(FrontendConfig(root, files.keys.toList.sorted, Nil), lenient = true))
  ...
  test("D15: a synthesised primary's PRIMITIVE slot publishes as a primitive, and re-derives equal") {
    ...
    val surface   = new PublishedSurface(p, ownedUnits(p, root), List("base-mod" -> contract("base-mod", rows.toList*)))
    val plans     = CtorFunnel.Plans(p, Some(surface))
    plans(p.units.head)
    assertEquals(
      clue(surface.gaps.filter(_.subject == "p.Guarded")),
      Nil,
