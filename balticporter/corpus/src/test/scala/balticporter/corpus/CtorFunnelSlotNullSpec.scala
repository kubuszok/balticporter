...
class CtorFunnelSlotNullSpec extends munit.FunSuite:
  ...
  /** the shape, reduced from a real port: `Data()` and `Data(Handle, String)` both reach the same (nilary) parent constructor, so the primary is SYNTHESISED; `path` is assigned in the leading run of
    * one root, so it is a FIELD SLOT; and `Data(Handle)` is a real one-argument constructor at a reference type, which is what `this(null)` is also applicable to.
    */
  private val src =
    """package demo;
      |public class Handle {}
  ...
  test("the MINTED delegation ascribes its `null`, so scalac has one applicable candidate") {
    assert(
      clue(out).contains("this((null: java.lang.String))"),
