...
class TryResourceBehaviourSpec extends munit.FunSuite:
  ...
  final class R(name: String, log: collection.mutable.ListBuffer[String], failOnClose: Boolean = false) extends java.lang.AutoCloseable:
    def close(): Unit =
      log += name
  ...
  private def one[A](r: R)(body: => A): A =
    ...
    try body
      ...
      if r != null then {
        if primary$1 != null then {
          try r.close()
          catch { case suppressed$1: java.lang.Throwable => primary$1.addSuppressed(suppressed$1) }
        } else r.close()
      }
  ...
  test("EVERY close() is attempted — an inner one throwing does not skip the outer") {
    val log = collection.mutable.ListBuffer.empty[String]
    val e   = intercept[IllegalStateException](two(new R("first", log), new R("second", log, failOnClose = true))(()))
  ...
  test("…and the inner close()'s exception becomes the OUTER level's primary") {
    val log = collection.mutable.ListBuffer.empty[String]
    val e   = intercept[IllegalStateException](two(new R("first", log, failOnClose = true), new R("second", log, failOnClose = true))(()))
    assertEquals(e.getMessage, "close:second")
