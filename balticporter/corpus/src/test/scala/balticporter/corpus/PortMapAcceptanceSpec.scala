...
class PortMapAcceptanceSpec extends munit.FunSuite:
  ...
  /** libGDX core's published map — `run-latest` when a run has produced one, falling back to the COMMITTED BASELINE.
    */
  private val reportDir   = repoRoot.resolve("port-report/LibgdxCoreMigrate")
  private val baseMapPath =
    List("run-latest", "baseline").iterator.map(d => reportDir.resolve(d).resolve("port-map.tsv")).find(Files.isRegularFile(_)).getOrElse(reportDir.resolve("baseline/port-map.tsv"))

  private def baseMap: PortMap.Map0 =
    if !Files.isRegularFile(baseMapPath) then
      fail(
        s"the base's map is missing: neither run-latest nor baseline at $reportDir — " +
  ...
  test("ACCEPTANCE: the base's published map reports the forwarder BEFORE emission, naming the base") {
    ...
    val files = Files
      ...
      .toList
      .sorted
    val types = SpoonTir.buildModel(FrontendConfig(ashley, files, Nil, resolutionRoots = List(gdxSrc)), lenient = true)
    ...
    assertEquals(clue(dropped).map(f => (f.symbol, f.base)), List(("com.badlogic.gdx.utils.Array#toArray(Class)", "sge")))
