...
class CtorFunnelContextClauseSpec extends munit.FunSuite:
  ...
  /** The census's dominant shape: TWO paramful roots reaching the same (implicit, nilary) parent constructor, no nilary root and no hoistable field — `syntheticPrimary` has nothing to synthesise and
    * declines, which is `Plan.none` (`IndexBufferObject`, `Mesh`, `VertexBufferObject`). `Buffer(int)` is the deep secondary chain, and `Buffer.Nested` is the same shape one level down — a walk over
    * top-level classes finds neither.
    */
  ...
  /** The `BitmapFont` / `DistanceFieldFont` shape, which reaches `Plan.none` the other way. `Font` has ONE root, so the funnel promotes it — and `Sub` reaches `Font` with an argument-free `extends`,
    * so the withholding fixpoint takes the paramful promotion back. `nilaryPlan` cannot promote `Font()` in its place either: its delegation argument is a call (not re-evaluable) and the target uses
    * `size` twice, so inlining it would evaluate `seed()` twice.
    */
  ...
  test("…and a delegation that CARRIES ARGUMENTS is a counted omission, never a silent drop") {
    ...
    assert(
      !clue(shapeOf(fontSrc, "demo.Font").secondaries).contains(Descriptor.empty),
      shapeOf(fontSrc, "demo.Font").secondaries.map(_.render).mkString(";")
    )
  }

  /** the OTHER half of the same predicate: a delegation that passes NOTHING is genuinely degenerate — scala's implicit primary already is that constructor — so it is dropped and NOT counted.
    */
  ...
  /** A clause on SOME constructors and not others. No single primary can carry one every secondary's `this(...)` resolves against, so `CtorFunnel.classGivens` refuses — and the refusal is the shape a
    * check has to see, because the emitted class compiles wherever its body summons nothing.
    */
  ...
  private def probe(label: String, source: String, main: String): Unit =
    ...
    val dir        = java.nio.file.Path.of(sys.props.getOrElse("balticporter.dumpProbe", s"${sys.props("user.dir")}/target/probe"), s"ctor-clause-$label")
...
object CtorFunnelContextClauseSpec:
  ...
  /** A context clause put on constructors DIRECTLY, with no closure and no policy — the shapes above are ones a real holder's closure would refuse before reaching, and the question here is what the
    * CONSTRUCTOR REGION does with a clause, whoever attached it and however wrongly. It mints through the phase's own `Minter`, so the parameter is anonymous and `isGiven` exactly as the threading's
  ...
  final class Clause(on: Set[String], firstOnly: Boolean = false) extends Phase:
    ...
    override def run(program: Program): Program =
        ...
        override def transformClassDef(t: Tree.ClassDef)(using p: Program): Tree.ClassDef =
          if !p.symbolOf(t.symbol).map(_.fullName).exists(on) then t
            ...
            if ctors.isEmpty then
              ...
              val ctor = mint.member("<init>", MemberKey(fqn, "<init>").render, t.symbol, TypeRepr.MethodType(Nil, TypeRepr.NoType), Flags())
              t.copy(
                body = Tree.DefDef(
                  ctor,
                  List(List(mint.usingParam(ctor, "demo.Ctx", ctxRef, t.origin))),
                  TypeTree(TypeRepr.NoType, t.origin),
                  Some(Tree.Block(Nil, Tree.Literal(Constant.UnitC, TypeRepr.NoType, t.origin), TypeRepr.NoType, t.origin)),
                  t.origin
                ) :: t.body
              )
            else
      ...
      given Program = p1
      p1.rebuilt(units = p1.units.map(u => StandardTraversal.mapClassDef(edit, u)), symbols = SymbolTable(p1.symbols.all ++ mint.minted))
