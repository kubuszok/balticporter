...
class RewriteCallSitesSpec extends PortSuite:
  ...
  /** a phase that runs and moves NOTHING — the honest baseline, and the one that proves the check's population is derived from what a phase DID rather than from a list of phase names.
    */
  private class Silent extends Phase:
    def name: String = "silent"

  /** the rewrite itself: every occurrence of `java.lang.String` is re-pointed at a type the fixture declares. What it retypes TO is irrelevant — nothing is emitted here — and re-pointing at a symbol
    * the fixture certainly has keeps the test from depending on how the frontend happens to intern a primitive.
    */
  ...
  /** …and the SAME retype written the OTHER way a phase can write it: at the DECLARATION IN THE TREE, through the definition hooks, with `transformType` left alone so the symbol table never moves.
    */
  ...
  test(
    "a phase that moves nothing produces NO row at all — the check is about retyping phases, and the list of those is derived"
  ) {
  ...
  test("…and a phase that moves the DECLARATION IN THE TREE and not the symbol table is observed too") {
    ...
    assert(p.rewrites.all.head.retyped.nonEmpty, "a declaration whose EMITTED type moved is a declaration this phase retyped")
    assertEquals(RewriteCallSitesCheck.check(p.rewrites, Some(Set.empty)).map(_.issue), List(Issue.Unaccounted))
  }
  ...
  test(
    "…and the SAME rewrite naming a lane that recorded is NOT a finding — the check is about the account, not about retyping"
  ) {
  ...
  test("CollectionsTransform names all THREE of its lanes — three residues, three different fixes") {
    assertEquals(
      new CollectionsTransform().accountedBy,
  ...
  test("…and it really does move declarations, so its account is not vacuous") {
    ...
    assertEquals(
      RewriteCallSitesCheck.check(p.rewrites, Some(Set(CollectionClosureCheck.Name, CollectionBoundaryCheck.Name, RetargetBoundaryCheck.Name))),
      Nil
    )
  }
  ...
  test(
    ...
    val predicted = before.owned.filter(id => before.symbolOf(id).exists(s => StandardTraversal.mapType(ph, s.info) != s.info))
    assertEquals(predicted.size, 0, "if this is ever non-zero the phase became predictable and K5.10's refusal should be re-measured")
    // …and what it actually moved, once it ran.
    val p = port(src, ph)
    assert(
      p.rewrites.all.exists(_.retyped.nonEmpty),
