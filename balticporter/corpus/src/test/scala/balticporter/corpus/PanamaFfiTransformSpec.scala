...
/** JNI → Panama: `native` methods become generated `java.lang.foreign` downcall bindings — a MethodHandle over a FunctionDescriptor built from the signature. Asserts the emitted Scala; the generated
  * bindings compile against java.lang.foreign (JDK 22+).
  */
class PanamaFfiTransformSpec extends munit.FunSuite:
  ...
  test("generates a downcall MethodHandle per native, with a signature-derived descriptor") {
    ...
    assert(
      out.contains(
        "java.lang.foreign.FunctionDescriptor.of(java.lang.foreign.ValueLayout.JAVA_INT, java.lang.foreign.ValueLayout.JAVA_INT, java.lang.foreign.ValueLayout.JAVA_INT)"
      )
    )
    // double scale(double,long) → mixed layouts
    assert(
      out.contains(
        "FunctionDescriptor.of(java.lang.foreign.ValueLayout.JAVA_DOUBLE, java.lang.foreign.ValueLayout.JAVA_DOUBLE, java.lang.foreign.ValueLayout.JAVA_LONG)"
      )
    )
  }
  ...
  test("replaces the native body with a handle invocation") {
    assert(
      clue(out).contains("def add(a: scala.Int, b: scala.Int): scala.Int = add$handle.invokeExact(a, b).asInstanceOf[scala.Int]")
    )
  }
  ...
  test("M10 — OVERLOADED natives get distinct handles, ordered by the erased signature") {
    ...
    List("copyJni$0$handle", "copyJni$1$handle", "copyJni$2$handle").foreach(n => assert(clue(o).contains(s"private val $n:"), s"missing $n"))
  ...
  test("M10 — the key is counter-free ALL THE WAY DOWN: an unreadable signature falls back to POSITION") {
    ...
    def byPosition(src: String): Map[(String, Int), String] =
      ...
      val blind   = p0.rebuilt(symbols = SymbolTable(p0.symbols.all.map(s => if natives(s.id) then s.copy(info = TypeRepr.NoType) else s)))
