...
class CollectionsSinkSpec extends PortSuite:
  ...
  private def ported(source: String, sinks: Set[String] = Set(Sink)): (DecisionLog, Program, CollectionsTransform, String) =
  ...
  /** liqp's `Json.apply` and `Template.putStringKey` at the size a spec can hold: one argument whose static type this phase MOVED, and one whose static type is `Object` and says nothing.
    */
  private val Shape =
    """package demo;
  ...
  test("a declared sink's `Object` argument is bridged — with the value's static type KNOWN") {
    ...
    assert(
      clue(out).contains("sb.append(balticporter.runtime.JavaCollections.Reified.toJavaValue(m))"),
  ...
  test("a NON-opaque formal at the same sink is left alone — the bridge is about the SLOT") {
    ...
    assert(!clue(out).contains("toJavaValue"), "`append(String)` is a typed slot; nothing about it can be a retyped collection")
  }
  ...
  test("an UNDECLARED sink bridges nothing — an empty set is the no-op, with no code path") {
    ...
    assert(
      out.contains("sb.append(balticporter.runtime.JavaCollections.toJava(m))"),
    ...
    assert(out.contains("sb.append(value)"), "and an `Object`-typed argument had nothing at all — the seam K21 is about")
  }
