...
class PreviewModeSpec extends munit.FunSuite:
  ...
  /** a labelled `break` whose label the emitter cannot see — the residue shape, minted directly rather than fished out of the corpus, which has zero of them (`break residue: 0`).
    */
  private val src =
    """package demo;
      |class Jumpy {
  ...
  /** the emitter cannot be asked for a residue from Java that has none, so the node is built by hand — which is also the honest test: a `Break` with a label nothing registered.
    */
  ...
  /** replace the method body with a stray labelled `break` — the one form neither the frontend nor the emitter can translate.
    */
  ...
  test("the count goes to its OWN lane — a declared refusal is not an engine gap") {
    val err = Correlate.ScalacError(
      "E???",
      "Error",
      "src_managed/main/scala/demo/Jumpy.scala",
      4,
      5,
