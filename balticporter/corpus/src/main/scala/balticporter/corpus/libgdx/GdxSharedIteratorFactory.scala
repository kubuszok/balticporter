...
/** A §1(c) rule reaching the CONFIG front door via `META-INF/services`, so a `.conf` can name it (`{ transform = "gdx-shared-iterator" }`) without the engine reflectively instantiating a class name
  * from data (CLAUDE.md §1.5). Lives beside [[GdxSharedIteratorRule]] in `corpus`, not the engine.
  */
