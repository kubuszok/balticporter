...
object LibgdxLadder:
  ...
  def stepPlatformInjects(repoRoot: Path): Map[String, Map[String, List[Path]]] = Map(
    ...
    "backend-jvm" -> Map(
      "jvm" -> List(
        repoRoot.resolve("balticporter/corpus/ladder-overrides-backend-jvm/jvm"),
      ...
      "native" -> List(
        repoRoot.resolve("balticporter/corpus/ladder-overrides-backend-jvm/native"),
        repoRoot.resolve("balticporter/corpus/ladder-overrides-backend-jvm/desktop")
      )
    ),
    "backend-desktop" -> Map(
      "jvm" -> List(
        repoRoot.resolve("balticporter/corpus/ladder-overrides-backend-desktop/jvm"),
  ...
  /** Per step, the members the step makes dead: the reflective `Class`-typed constructors the witness replaces (each has a portable twin; the full port dropped the same, `LibgdxPolicy`).
    */
  ...
  def universal(repoRoot: Path, steps: Set[String] = DefaultSteps, reference: Option[Path] = None): PortManifest =
    ...
    LlsPolicy
      .core(repoRoot, LlsPolicy.DefaultRungs)
      .extendedBy(
        PortManifest(
          name = "sge-l0",
