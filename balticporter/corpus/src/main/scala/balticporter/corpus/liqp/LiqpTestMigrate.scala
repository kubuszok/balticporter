...
/** Port liqp's own JUnit suite (`src/test/java`) through the same pipeline as `src/main/java`.
  ...
  *
  * The port is `balticporter/corpus/ports/liqp/test.conf`; this `main` names it, ensures the test classpath the
...
/** liqp's TEST frontend classpath: everything [[LiqpClasspath]] resolves, plus JUnit.
  ...
  *     lookups read. It is hand-written under `ported/ssg-liquid/src/main/resources` (CLAUDE.md §5.5 —
