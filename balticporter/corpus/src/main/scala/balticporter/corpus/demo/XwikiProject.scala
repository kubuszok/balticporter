...
/** M6 cold-port assembly: the flexmark-ext-xwiki-macros dependency closure (ssg never ported it) flattened into one sbt module and driven through the whole-module scalac gate. Constructor-level
  * overrides ([[XwikiOverrides]]) handle the irreducible ctors; everything else is engine-translated — the real M6 gate, a library with no hand-port corpus.
  */
object XwikiProject:
  ...
  def main(args: Array[String]): Unit =
    ...
    val moduleRoots = XwikiSurvey.closureModules.map(m => fmRoot.resolve(m).resolve("src/main/java")).filter(Files.isDirectory(_))
    val files       = moduleRoots.flatMap { root =>
      Files.walk(root).iterator().asScala.filter(p => p.toString.endsWith(".java") && !p.toString.endsWith("package-info.java")).map(p => fmRoot.relativize(p).toString)
    }.sorted
    println(s"[xwiki] ${files.length} closure files across ${moduleRoots.length} modules")

    val annotationsJar =
      val pb   = new ProcessBuilder("cs", "fetch", "--classpath", "org.jetbrains:annotations:24.1.0").redirectErrorStream(true)
      val proc = pb.start()
    ...
    val overridesRoot   = repoRoot.resolve("balticporter/corpus/xwiki-overrides")
    var cacheHits       = 0
    var translated      = 0
    var overridden      = 0
    ...
    println(
      s"[xwiki] cache: $cacheHits hits, $translated translated, $overridden whole-file overrides; comment failures=$commentFailures"
    )
