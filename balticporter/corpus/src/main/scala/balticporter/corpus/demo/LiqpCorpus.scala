...
/** M1 coverage baseline: translate every `done` Liqp file from ssg's migration manifest, per-file tolerant, and report OK (preserved) / COMMENT_LOSS / UNSUPPORTED (with reason) / NO_COUNTERPART.
  * Writes `out/liqp-corpus-report.tsv` (sorted, deterministic) and prints a histogram of unsupported reasons — that histogram is the M1 worklist.
  */
object LiqpCorpus:
  ...
  def main(args: Array[String]): Unit =
    ...
    val rows = Files.readAllLines(ssgRoot.resolve(".rescale/data/migration.tsv")).asScala.toList.filterNot(_.startsWith("#")).map(_.split('\t').toList).collect {
      case "liqp" :: sourcePath :: _ :: status :: _ if status == "done" => sourcePath
    }
    ...
    // manifest mapping entries beyond the path convention: renamed
        ...
        r.status match
          ...
          case SD.Status.Diff =>
            Ledger.lookup(rel) match
              ...
              case None =>
                val detail =
                  (r.missingInHand.take(3).map(m => s"engine-only:$m") ++ r.extraInHand.take(3).map(m => s"hand-only:$m")).mkString("; ")
...
/** Accepted-divergence ledger: corpus/liqp-divergences.tsv (rel_path \t diff-fingerprint \t verified reason). An entry only suppresses the exact diff it was written for — fingerprint drift resurfaces
  * as SKEL_DIFF.
  */
object Ledger:
  private lazy val entries: Map[String, (String, String)] =
    val p = Path.of(sys.props.getOrElse("balticporter.root", ".")).resolve("balticporter/corpus/liqp-divergences.tsv")
