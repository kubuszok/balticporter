...
/** M6 candidate survey: translate-readiness of the flexmark-ext-xwiki-macros dependency closure (the cold-port target + everything its build needs). Reports per-module status counts and every blocker
  * file.
  */
object XwikiSurvey:
  ...
  def main(args: Array[String]): Unit =
    ...
    val moduleRoots = closureModules.map(m => m -> fmRoot.resolve(m).resolve("src/main/java")).filter((_, p) => Files.isDirectory(p))
    println(s"[xwiki] ${moduleRoots.length} closure modules present")

    val files = moduleRoots.flatMap { (m, root) =>
      Files.walk(root).iterator().asScala.filter(p => p.toString.endsWith(".java")).map(p => fmRoot.relativize(p).toString)
    }.sorted
    ...
    val results = parsed.map { case (rel, e) =>
      val status = if rel.endsWith("package-info.java") then "PACKAGE_INFO"
        ...
        e.flatMap(u => scala.util.Try(ScalaPrinter.print(u, prov, sentinels, ctorReg, XwikiOverrides.map)).toEither.map(u -> _)) match
          case Right((u, out)) =>
    ...
    results.collect {
      case (f, s) if s.startsWith("UNSUPPORTED") || s.startsWith("ERROR") || s == "COMMENT_LOSS" =>
