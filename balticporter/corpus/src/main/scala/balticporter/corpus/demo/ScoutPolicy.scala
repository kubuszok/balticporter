...
object ScoutPolicy:
  ...
  /** annotation packages these surveys carry through to the output verbatim: `org.junit.`/`junit.` (a suite's annotations ARE the suite), `com.fasterxml.jackson.` (behaviour-bearing on the JVM —
    * liqp's custom serializers drive its eager-render path off exactly these). Nothing else survives: a nullness hint is advisory, and an unclaimed annotation is reported by the frontend rather than
    * silently kept.
    */
