...
/** M5 bump gate (DESIGN.md §3.9): an upstream pin move must trigger SCOPED regeneration — retranslate exactly the changed units plus the interface-ripple, everything else served from cache. The
  * vendored tree is never touched; the "pin move" is simulated on staged copies under `out/bump-demo`: v2 differs from v1 by a body-only edit (no ripple) and a signature addition (must ripple). Exit
  * != 0 on any violated expectation.
  */
object BumpDemo:
  ...
  def main(args: Array[String]): Unit =
    ...
    edit(v2.resolve(bodyEditRel), "        return 0;", "        // bump-demo v2: body-only upstream drift\n        return 0;")
    edit(
      v2.resolve(sigEditRel),
      "        return params[index];\n    }",
    ...
    println(
      s"[bump]   retranslated: ${bump.translated.sorted.take(8).mkString(", ")}${if bump.translated.length > 8 then s" … (${bump.translated.length} total)" else ""}"
    )
    ...
    expect(
      warm.translated.isEmpty && warm.hits == warm.units.length,
      s"warm v1 sweep must be all hits (translated=${warm.translated.length})"
    )
    expect(cold.commentFailures == 0 && bump.commentFailures == 0, "comment invariant must hold on both versions")

    expect(bump.ifaceHash(bodyEditRel) == cold.ifaceHash(bodyEditRel), "body-only edit must not change the unit's interface hash")
    expect(bump.ifaceHash(sigEditRel) != cold.ifaceHash(sigEditRel), "signature addition must change the unit's interface hash")
    ...
    expect(
      bump.translated.sorted == expected,
    ...
    if failures.nonEmpty then
      ...
      sys.exit(1)
    println(
      s"[bump] scoped regen: ${bump.translated.length}/${bump.units.length} units (1 body edit + 1 signature edit + ${expectedRipple.length} ripple)"
    )
  ...
  private def sweep(root: Path, cache: ActionCache, cp: List[Path]): Sweep =
    val files          = Files.walk(root).iterator().asScala.filter(p => p.toString.endsWith(".java") && !p.toString.endsWith("Examples.java")).map(p => root.relativize(p).toString).toList.sorted
