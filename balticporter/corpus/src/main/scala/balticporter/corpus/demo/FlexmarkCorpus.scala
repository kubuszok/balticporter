...
/** M5 coverage baseline over the flexmark corpus (845 ported files, ~30 Maven modules) — the measured worklist for the scale-up arc, exactly as LiqpCorpus was for M1. Translate-only sweep; hand-port
  * comparison comes once coverage stabilizes.
  */
object FlexmarkCorpus:
  ...
  def main(args: Array[String]): Unit =
    ...
    val rows = Files.readAllLines(ssgRoot.resolve(".rescale/data/migration.tsv")).asScala.toList.filterNot(_.startsWith("#")).map(_.split('\t').toList).collect {
      case "flexmark" :: sourcePath :: _ :: status :: _ if status == "ported" => sourcePath
    }
    ...
    val moduleRoots   = portedModules.toList.sorted.map(m => fmRoot.resolve(m).resolve("src/main/java")).filter(Files.isDirectory(_))
    ...
    val results = parsed.map { case (rel, e) =>
      val status = if rel.endsWith("package-info.java") then "PACKAGE_INFO"
        ...
        e.flatMap(u => scala.util.Try(ScalaPrinter.print(u, prov, sentinels, ctorReg)).toEither.map(u -> _)) match
          case Right((u, out)) =>
