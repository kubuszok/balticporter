...
/** M0 gate: 20 hand-picked Liqp files → compiling Scala 3, comments preserved, byte-identical across runs. The set is the smallest one exercising every M0 translation path while resolving with only
  * two shims. Out-of-set externals follow the Shim disposition: handwritten minimal implementations under `corpus/shims`.
  */
object LiqpM0:
