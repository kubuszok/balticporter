...
/** Emit a single Java file through the TIR to Scala source, to eyeball the emission backend. `corpus/runMain balticporter.corpus.demo.SpoonTirEmit [path/to/File.java]` — default is a small liqp
  * filter. Uses noClasspath parsing (structure over full resolution).
  */
object SpoonTirEmit:
