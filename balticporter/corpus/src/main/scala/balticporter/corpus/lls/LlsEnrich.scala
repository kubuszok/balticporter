...
object LlsEnrich:
  ...
  private[corpus] def spec(owner: String, name: String, arity: Int, src: String, why: String, static: Boolean = false): (String, MemberSpec) =
  ...
  /** @param owner
    ...
    */
  private[corpus] case class ArrayKind(
    owner:      String,
    self:       String,
    elem:       String,
    tparams:    String = "",
    removeOne:  String = "",
    removeMany: String = "",
    /** the FACTORY's witness clause when the `witness` rung is on; the emitted constructors take one and a factory must supply it.
      */
    mk: String = ""
  )
  ...
  /** @param entryValue
    ...
    */
  private[corpus] case class MapKind(
    owner:      String,
    key:        String,
    value:      String,
    entryValue: String,
  ...
  /** owner FQN -> specs, the shape `AddMembersTransform` takes — for any port's table (the ladder's core collections use the same generators, `LibgdxEnrich`).
    */
