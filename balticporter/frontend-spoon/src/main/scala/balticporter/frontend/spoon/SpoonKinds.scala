...
import balticporter.catalog.{ Area, DiffId }

/** WHAT THE FRONTEND CLAIMS ABOUT EACH SPOON NODE KIND — the half of totality scalac cannot enforce, since `CtElement` has no sealedness. `NodeKindTotalitySpec` asserts this registry equals the
  * derived kind set from the spoon-core jar, so an upgrade adding a kind fails the build until classified here. Carries NO OBLIGATIONS — only what happens today.
  */
object SpoonKinds:
  ...
  enum Claim:
    ...
    /** never dispatched on: consumed inside a PARENT's arm, which is correct and is not a gap. `by` is the consuming symbol, so the claim can be checked rather than believed
      */
    case Positional(by: String)

    /** nothing lowers it. [[Absence]] says what happens instead, which is the whole diagnostic value — a construct that is refused loudly and one that is silently absorbed are the same size on this
      * list and nothing like each other in a port
      */
    case Absent(how: Absence, detail: String)

  /** the three ways a kind can be unhandled, in descending order of how loudly it fails. */
  enum Absence:
    /** reaches `SpoonTir.unsupported`, which throws -- the whole COMPILATION UNIT fails to translate, not one node. Still right for blind spots INSIDE dispatching arms (a `Constant` shape the literal
      * arm doesn't know, an unresolvable try-with-resources declaration, a bodyless lambda) -- none of which is a node KIND this census can hold.
      */
    case RefusedLoudly
    ...
    /** a SUPERTYPE's arm takes it and the construct degrades with no error, no moved count and nothing in the emitted file to say a Java construct had ever been there. The dangerous one
      */
    case AbsorbedSilently

    /** the walk never reaches it: the builder enters at the top-level types, so nothing under the package tree or the module tree is visited at all
      */
    case NeverVisited

  /** a kind, its claim, and the catalog row that names the difference — a POINTER, not a consult. Nothing is discharged by writing one down; it exists so the two artifacts can be joined when a run
    * reports "this kind was refused" and a reader asks which known difference that is.
    */
  ...
  /** JUDGEMENT CALLS: each has a concrete `Impl` and a `CoreFactory.create*`, but no Java parse ever mints one (programmatic-insertion API / refactoring containers). Listed apart from a strict
    * Impl-concreteness test, which would have wrongly included them.
    */
  ...
  /** kinds the frontend never dispatches on because a PARENT's arm consumes them. Not a gap — but each one names the consuming symbol, because "handled positionally" is otherwise a claim with nothing
    * behind it, and one of these turned out to be weaker than it reads (see `CtJavaDocTag`).
    */
  val positional: List[Kind] = List(
    ...
    Kind(
      "CtEnumValue",
      Positional(
        "SpoonTir.enumCase, from classDef's getEnumValues — the constant's ctor ARGUMENTS bypass coerceArgs and the call dispatch entirely"
      ),
      scala.None
    ),
  ...
  val lowered: List[Kind] = List(
    "CtArrayRead" -> "SpoonTir.exprNoCast",
    "CtArrayWrite" -> "SpoonTir.exprNoCast",
    "CtAssert" -> "SpoonTir.stmtKind",
    "CtAssignment" -> "SpoonTir.exprNoCast / stmtKind",
    "CtBinaryOperator" -> "SpoonTir.exprNoCast",
    "CtBlock" -> "SpoonTir.blockOf",
    "CtBreak" -> "SpoonTir.stmtKind",
    "CtComment" -> "SpoonTir.triviaOf",
    "CtConditional" -> "SpoonTir.exprNoCast",
    "CtConstructorCall" -> "SpoonTir.exprNoCast",
    "CtContinue" -> "SpoonTir.stmtKind",
    "CtDo" -> "SpoonTir.stmtKind",
    "CtExecutableReferenceExpression" -> "SpoonTir.methodRef",
    "CtFieldRead" -> "SpoonTir.fieldAccess",
    "CtFieldWrite" -> "SpoonTir.fieldAccess",
    "CtFor" -> "SpoonTir.stmtKind",
    "CtForEach" -> "SpoonTir.stmtKind",
    "CtIf" -> "SpoonTir.stmtKind",
    "CtInvocation" -> "SpoonTir.invocation",
    "CtLambda" -> "SpoonTir.exprNoCast",
    "CtLiteral" -> "SpoonTir.literal",
    "CtLocalVariable" -> "SpoonTir.defineLocal",
    ...
    "CtNewArray" -> "SpoonTir.newArray",
    "CtNewClass" -> "SpoonTir.anonClass",
    "CtOperatorAssignment" -> "SpoonTir.stmtKind / exprNoCast",
    "CtReturn" -> "SpoonTir.stmtKind",
    "CtSuperAccess" -> "SpoonTir.exprNoCast",
    "CtSwitch" -> "SpoonTir.switchStmt",
    "CtSynchronized" -> "SpoonTir.stmtKind",
    "CtThisAccess" -> "SpoonTir.exprNoCast",
    "CtThrow" -> "SpoonTir.stmtKind",
    "CtTry" -> "SpoonTir.tryStmt",
    "CtTryWithResource" -> "SpoonTir.tryStmt",
    "CtTypeAccess" -> "SpoonTir.exprNoCast",
    "CtUnaryOperator" -> "SpoonTir.exprNoCast",
    "CtVariableRead" -> "SpoonTir.exprNoCast",
    "CtVariableWrite" -> "SpoonTir.exprNoCast",
    "CtWhile" -> "SpoonTir.stmtKind",
    "CtAnnotationType" -> "SpoonTir.typeFlags",
    "CtAnonymousExecutable" -> "SpoonTir.classDef",
    ...
    "CtEnum" -> "SpoonTir.classDef / enumCase",
    "CtField" -> "SpoonTir.classDef / fieldFlags",
    ...
    Kind(
      "CtRecord",
      Lowered(
        "SpoonTir.classDef — typeFlags.isRecord, recordComponents, createCanonicalConstructorIfMissing, " +
    ...
    Kind(
      "CtRecordPattern",
      Lowered(
        "SpoonTir.recordPattern, from caseLabel — Tree.RecordPattern, with Tree.BindPattern " +
  ...
  /** kinds NOTHING reaches. Four are `AbsorbedSilently`: each extends a kind the frontend DOES dispatch on, so an ordered `match` takes it at the supertype's arm and the construct degrades with a
    * green compile. `CtTextBlock` is the sharpest example.
    */
  val absent: List[Kind] = List(
    Kind(
      "CtAnnotationFieldAccess",
      Absent(AbsorbedSilently, "extends CtVariableRead, not CtFieldAccess, so the variable arm takes it and the TARGET is dropped"),
      scala.None
    ),
    ...
    Kind(
      ...
      Absent(
        MarkedUnportable,
        "reached as the instanceof right operand (JLS 15.20.2); the binding is FLOW-scoped and is refused, and the marker stands at the enclosing boolean expression"
      ),
      Some(g(21))
    ),
    ...
    Kind(
      ...
      Absent(
        NeverVisited,
        "no source this parser accepts builds one: `_` in a TYPE PATTERN position is a CtTypePattern named `_`, and `_` nested in a record pattern raises spoon's own JLSViolation (instanceof) or yields no node (case label)"
      ),
      Some(s(10))
    ),
    ...
    Kind(
      "CtPackage",
      Absent(NeverVisited, "the builder enters at the top-level types; package names are recovered from qualified names"),
      scala.None
    ),
    Kind(
      "CtPackageDeclaration",
      Absent(NeverVisited, "not read; its comments survive only through the positional file-header harvest"),
      scala.None
    ),
    Kind(
      "CtReceiverParameter",
      Absent(
        NeverVisited,
        "execDef reads getParameters, which excludes it; any annotation written on the receiver is dropped with it"
      ),
      scala.None
    ),
  ...
  /** the reference types that are supertypes or mixins, so no parse produces one.
    *
    * Hand-maintained for [[excluded]]'s reason and grouped by the test that put each here: two have an `Impl` that is declared `abstract` and one has no `Impl` at all.
    */
  ...
  /** the FIVE reference kinds `SpoonTir.tpe` dispatches on, and the nine it does not. Same [[Claim]] vocabulary: `Lowered` is a `tpe` arm, `Positional` is read by a parent's arm without reaching
    * `tpe`, `Absent` is unreached. No `MarkedUnportable`/`RefusedLoudly` here (a reference has no body to refuse).
    */
  val references: List[Kind] = List(
    Kind(
      "CtTypeReference",
      Lowered("SpoonTir.tpe's final arm — a class type, raw-filled when its actuals are empty"),
      scala.None
    ),
    ...
    Kind(
      ...
      Positional("SpoonTir.invocation / methodSym / coerceArgs — read for its declaration and its formals, never lowered as a type"),
      scala.None
    ),
    ...
    Kind(
      ...
      Positional(
        "SpoonTir.resolveVar's fallback, which mints an external `?var$<name>` symbol — the reference Spoon builds when it cannot resolve the declaration"
      ),
      scala.None
    ),
    Kind(
      "CtPackageReference",
      Positional("SpoonTir.typeKey reads getQualifiedName, which carries the package; the reference itself is never visited"),
      scala.None
    ),
    ...
    Kind(
      ...
      Absent(
        NeverVisited,
        "an `import static X.*` target; SpoonTir.harvestHeader reads a CtImport only for its comments, and fully-qualified emission removes the question the import answered"
      ),
      Some(c(5))
    )
  )
  ...
  /** the REGISTRY name for a node the parser actually built. Keyed on Spoon's INTERFACE names (`CtSwitchExpression`), not the impl class (`...Impl`) — resolved structurally, not by stripping `Impl`.
    * Among the interfaces a class carries, the MOST SPECIFIC registered one wins. Falls back to the class's own simple name outside the registry, which `NodeKindTotalitySpec` exists to fail on.
    */
  def nameOf(cls: Class[?]): String = nameIn(cls, byName)

  /** the same rule against the REFERENCE registry -- SpoonTir.tpe's key. Its own resolver rather than a widened [[nameOf]]: a merged map would let a node kind answer for a reference, and
    * CtWildcardReferenceImpl extends CtTypeParameterReferenceImpl, so the most-specific rule matters here too.
    */
  ...
  private def nameIn(cls: Class[?], known: Map[String, Kind]): String =
    ...
    if known.contains(stripped) then stripped
      ...
      candidates.find(c => candidates.forall(d => (d eq c) || !c.isAssignableFrom(d))).map(_.getSimpleName).getOrElse(stripped)

  /** the kinds nothing reaches, by how loudly they fail. Printed by the spec; the ONLY honest way to state the size of the gap, because a total that mixes a loud refusal with a silent absorption is a
    * number that hides the half that matters.
    */
