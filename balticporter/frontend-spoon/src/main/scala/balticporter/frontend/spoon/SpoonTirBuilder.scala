...
final private[spoon] class Builder(
  ...
  /** the run's obligation log, in scope for every `Lowering.of` in this builder — `given` rather than threaded through forty signatures that only the dispatch method needs it.
    */
  ...
  /** an executable's own type parameters that are ERASED rather than declared — name → the type every occurrence renders as. Consulted AHEAD of `tpScopes` since such a name has no binder. One frame
    * per executable. See [[unwritableResultVars]].
    */
  ...
  /** Type params LEGALLY in scope at the current point, respecting static-nested boundaries (a static nested type cannot see its enclosing type's params). Distinct from `tpScopes`, which keeps every
    * enclosing frame for reference resolution.
    */
  private[spoon] val tpAccessible = collection.mutable.ArrayDeque[Map[String, SymId]]()

  /** names contributed by EXECUTABLES, parallel to `tpAccessible` (which merges each level into one map, so a frame cannot simply be skipped). Hidden under [[atDeclScope]].
    */
  private[spoon] val tpExecNames = collection.mutable.ArrayDeque[Set[String]]()

  /** The instantiation this class gives its ANCESTORS' type parameters, by name — needed so an overriding member can fill a raw inherited type the same way the inherited declaration did, rather than
    * independently landing on `[?]` and disagreeing with it.
    */
  private[spoon] val inheritedInst = collection.mutable.ArrayDeque[Map[String, (TypeRepr, CtTypeReference[?])]]()

  /** …the same instantiation keyed by DECLARATION — see [[instantiationByDecl]]. Read only by [[inheritedFormal]], at a call to a member an ANCESTOR declares.
    */
  private[spoon] val inheritedByDecl = collection.mutable.ArrayDeque[Map[(String, String), CtTypeReference[?]]]()

  /** FQNs of the enclosing class and its ancestors — a raw type nested in any of them is filled from the names in scope, since those are the names it was declared against.
    */
  private[spoon] val enclosingFqns = collection.mutable.ArrayDeque[Set[String]]()

  /** FQNs of this class's ancestors — the only declarations whose formals are written in type variables the inherited instantiation can speak about.
    */
  private[spoon] val ancestorFqns  = collection.mutable.ArrayDeque[Set[String]]()
  private[spoon] var noInheritFill = false

  /** true while translating a member this class INHERITS (an override), so the inherited instantiation applies only there — a member the class declares for itself has no such obligation, and an
    * unrelated ancestor's same-named type param must not leak into it.
    */
  private[spoon] var inOverridingMember = false

  /** The map is keyed by NAME, so an unrelated ancestor's `T` can collide with the type being filled; require the candidate to satisfy the formal's own BOUND to make the match safe.
    */
  private[spoon] def inheritedTp(f: CtTypeParameter): Option[TypeRepr] =
    if true || noInheritFill || !inOverridingMember then scala.None // sge design: no inherited fill
      ...
      inheritedInst.headOption.flatMap(_.get(f.getSimpleName)).collect {
  ...
  private[spoon] def withStatic[A](s: Boolean)(f: => A): A =
    ...
    try f
    finally inStatic = prev

  /** Every executable this walk considered, INCLUDING ones policy removed — published as [[MemberIndex]], since a dropped executable has no symbol elsewhere to recover it from.
    */
        ...
        acc.getOrElse(
          fqn, {
            ...
            val here = typeDeclarationOf(ref).filter(_.isInterface) match
              case Some(ct) =>
                val declared = ct.getMethods.asScala.iterator.filter(isDefaultMethod).map(m => (m.getSimpleName, List(m.getParameters.size))).toSet
  ...
  /** Every comment handed out, by IDENTITY (not equality — two `// TODO`s are two comments), so `deepComments`'s net cast over a subtree does not re-emit one a closer harvest already took.
    */
  ...
  /** The compilation unit's original text, for slicing. `""` when Spoon has no buffer (the normal case for an in-memory `VirtualFile`). Guard against `Some(null)`: a bare `.map` over the unit yields
    * it and NPEs downstream.
    */
  ...
  /** the in-memory buffer this position belongs to, by file name. Falls back to the only source when exactly one exists; falls back to `""` (never guesses the wrong file) otherwise.
    */
  private[spoon] def inMemoryFor(pos: spoon.reflect.cu.SourcePosition): String =
    Option(pos)
      .filter(_.isValidPosition)
      .flatMap(p => Option(p.getFile))
      .map(_.getName)
  ...
  /** WHERE a comment is: file + start offset. `claimed` uses object identity, which a comment the parser attached nowhere cannot provide — the file header instead claims by span.
    */
  ...
  /** a comment the file header did NOT take — the filter every finer harvest applies, so a leading block that Spoon ALSO attached to the type is not emitted twice.
    */
  private[spoon] def unheaded(c: CtComment): Boolean = spanOf(c).forall(!headerSpans.contains(_))

  /** Comments Spoon attached to expression-level descendants, hoisted to the nearest enclosing harvest point (the TIR carries trivia only on declarations/statements). MUST be called AFTER the
    * element's children have translated, or it swallows their comments too.
    */
  ...
  private[spoon] def originOf(el: CtElement): Origin =
    ...
    if p != null && p.isValidPosition then Origin(Option(p.getFile).map(_.getPath).getOrElse("<unknown>"), p.getLine, columnOf(p))
    else Origin.synthetic

  /** the position's COLUMN, or 0 where the unit has no source buffer to search (an in-memory unit may have no `getOriginalSourceCode`, and Spoon's own column search then crashes). ZERO is honest here
    * — every `Origin` reader keys on FILE and LINE, never on the column.
    */
  ...
  /** An external MEMBER always knows its owner — the key is derived from it. Passing it on is what lets `owner#name` be reconstructed downstream (see `Minter.external`).
    */
  ...
  private[spoon] def erasedSig(m: CtExecutable[?]): String =
    val ps = m.getParameters.asScala.toList.map(p => scala.util.Try(p.getType.getQualifiedName).getOrElse("?")).mkString(",")
    s"($ps)"

  /** The member's DESCRIPTOR — its source-level parameter spelling, read from the PARSER (not from the retyped `MethodType`, so `equals(Object)` stays `Object` rather than `scala.Any`). Spelling
    * matches `isDropped`'s (`dropMethods` keys against it). ALL parameters or none ([[Descriptor.total]]) — a partial descriptor matches the wrong overload.
    */
  private[spoon] def descriptorOf(m: CtExecutable[?]): Option[Descriptor] =
    def paramOf(r: CtTypeReference[?]): Param = r match
      ...
      case a: CtArrayTypeReference[?] =>
        paramOf(a.getComponentType) match
  ...
  /** Is this executable's declaration a SHADOW — reconstructed from a class file rather than parsed from a source this run owns? The one spelling of "is this external" (via [[isExternalCallee]]); no
    * declaring type means external, an unreadable parent means conservatively NOT external (suppresses casts/spreads rather than guessing).
    */
  ...
  /** …the same question asked of a call's REFERENCE: no declaration at all means external, since this program's own members are always parsed and therefore have one.
    */
  ...
  /** one SLOT of [[externalSignature]] — a parameter's or result's declared type, or `NoType` where no scope-free name exists. A slot that cannot render is unknown; a type ARGUMENT that cannot render
    * is `?` (Spoon's reflective reconstruction loses the exact bound).
    */
  private[spoon] def externalSlot(tr: CtTypeReference[?]): TypeRepr = TypeShape.of(tr) match
    ...
    case s @ TypeShape.Named(r, _)   =>
      ...
      if args.isEmpty then if arity <= 0 then head else AppliedType(head, List.fill(arity)(TypeBounds(NoType, NoType)))
      else AppliedType(head, args.map(externalArg))

  /** a type ARGUMENT of [[externalSlot]] — the same rendering, with "cannot be named here" spelled `?` instead of refused.
    */
  ...
  /** parallel to `tpScopes`: the SPOON declarations behind the ids, read by [[resolveTypeParamDecl]]. Pushed and popped with `tpScopes` — the two are indexed together.
    */
  private[spoon] val tpDecls = collection.mutable.ArrayDeque[Map[String, CtTypeParameter]]()

  /** Render a type as its DECLARATION site would have, not as the current reading scope would — the name-directed raw fill is scope-dependent by design, so re-rendering in the reading scope would
    * silently disagree with the declared type. Hides executable frames only: a field's type cannot mention a method's type parameters, so this is exact, not approximate.
    */
  private[spoon] def atDeclScope[A](f: => A): A =
    val saved = declScopeOnly
    declScopeOnly = true
    try f
    finally declScopeOnly = saved
  private[spoon] var declScopeOnly = false

  /** WHICH frame a name resolves in — stated once, because [[resolveTypeParam]] and [[resolveTypeParamDecl]] must answer about the SAME declaration or the raw fill's licence (`licensedFills`) is read
    * off one variable and applied to another.
    */
  ...
  /** the type an ERASED type-parameter name renders as, or `None` for an ordinary one. Consulted AHEAD of [[resolveTypeParam]]: an erased parameter was never minted and has no id.
    */
  ...
  /** the SPOON DECLARATION behind [[resolveTypeParam]]'s id — its bound is what a raw fill's licence is read from, and no `Symbol` carries java's own spelling of it.
    */
  ...
  /** Mint ids for all formals FIRST (so bounds can self-reference — F-bounds), then translate each bound with the frame in scope. Returns the frame and the TypeDefs.
    */
  ...
  /** the `tpDecls` frame for a declaration's formals — one derivation, so the three sites that push a `tpScopes` frame cannot disagree about which declaration a name stands for.
    */
          ...
          if fs.sizeIs == as.size then
            fs.zip(as).foreach { (f, a) =>
              ...
              val nameable = a match
                ...
                case _ => true
              if !a.isInstanceOf[CtWildcardReference] && nameable then out += ((decl.getQualifiedName, f.getSimpleName, a))
            }
  ...
  /** …the same instantiations keyed by DECLARATION rather than name — `(owner FQN, formal name)`, `ParentSubst`'s own identity — so two ancestors' same-named `T`s cannot collide.
    */
    ...
    var ok = formals
      .filter { f =>
        resolveDecl(f.getSimpleName) match
          ...
          case Some(p) =>
            (p eq f) || ((spelled(f), spelled(p)) match
  ...
  /** A bound as JAVA WROTE IT, with type variables by their own name — the comparison [[licensedFills]] makes. Not a rendering: it is only ever compared to another one of these.
    */
  ...
  /** The ERASURE of a type variable — its first bound with nested variables erased, else Object. Used ONLY to build CASTS at wildcard-receiver call sites — must never drive a DECLARATION's type (that
    * breaks assignment of concrete generic values, measured catastrophic).
    */
  ...
  private[spoon] def erasedType(b: CtTypeReference[?], seen: Set[String], depth: Int): TypeRepr =
    if depth <= 0 then objectT
      ...
      TypeShape.of(b) match
        ...
        case TypeShape.Arr(_, c) =>
          AppliedType(TypeRef(NoPrefix, minter.external("scala.Array", "Array")), List(erasedType(c, seen, depth)))
  ...
  /** the erasure a DECLARED formal is seen at through an erased receiver: resolves through `subst` (receiver's own erased arguments, by formal NAME) or, failing that, its declaration. A RAW generic
    * formal is emitted name-FILLED, so it too must resolve through `subst`.
    */
  ...
  private[spoon] def erasedFormal(f: CtTypeReference[?], subst: Map[String, TypeRepr] = Map.empty, named: Map[String, TypeRepr] = Map.empty): TypeRepr = f match
    ...
    case r if subst.nonEmpty && rawFormalsOf(r).exists(n => subst.contains(n)) =>
      AppliedType(
        TypeRef(NoPrefix, typeSym(r)),
        rawFormalNodes(r).map { ff =>
  ...
  private[spoon] def rawFormalNodes(r: CtTypeReference[?]): List[CtTypeParameter] =
    if r == null || r.isPrimitive || r.isInstanceOf[CtWildcardReference] ||
      ...
      (r.getActualTypeArguments.size) > 0
    then Nil
  ...
  /** [[mentionsTypeVar]], but aware that a RAW generic use is emitted name-FILLED from the same-named in-scope parameters — so `ResourceData` inside `ParticleBatch<T>` really does depend on `T`, even
    * though nothing in the Spoon type says so.
    */
  ...
  /** A METHOD type variable declared in terms of the receiver's depends on the receiver just as a bare formal does. Bound only, never the variable's own NAME (a same-named callee `<T>` is a different
    * variable — the confusion [[tpConcrete]] avoids). Depth-limited on BOUND HOPS, and the limit is THREADED THROUGH [[mentionsTypeVarFilled]]: a bound reached through a type ARGUMENT re-entered this
    * function at full fuel, so a cyclic F-bound recursed until the `catch` below swallowed a `StackOverflowError` — 41 s of migration became 19 min.
    */
  private[spoon] def boundMentions(tv: CtTypeParameterReference, names: Set[String], fuel: Int = 2): Boolean =
    fuel > 0 && (try
      Option(tv.getDeclaration).flatMap(d => Option(d.getSuperclass)).filter(_.getQualifiedName != "java.lang.Object").exists {
  ...
  /** does this type involve a RAW use of a generic type — itself, or anywhere in its arguments (`Class`, `ObjectMap<String, AssetLoader>`)? A raw use is exactly where Java stops checking and where
    * our rendering is CONTEXT-dependent (wildcards, or name-directed fill), so the two ends of an assignment need not agree even when Java's do.
    */
  ...
  /** the ONE Spoon lookup in the arity family where an absent value is normal — see [[formalArity]] for why nothing else in that computation may share its `catch`.
    */
  private[spoon] def typeDeclarationOf(r: CtTypeReference[?]): Option[spoon.reflect.declaration.CtType[?]] =
    if r == null then scala.None
    else
      try Option(r.getTypeDeclaration)
    ...
    typeDeclarationOf(r) match
      ...
      case Some(_: CtInterface[?]) =>
        samAbstracts(r) match
          ...
          case several =>
            Sam.Answer.No(
              s"${several.size} abstract methods (${several.map(_.getSimpleName).sorted.distinct.mkString(", ")}) — java's own SAM rule (JLS 9.8) admits exactly one"
            )
      case Some(_) => Sam.Answer.No("the target is a CLASS, not an interface")

  /** JLS 9.8's COUNT, as a list — the one place the rule is spelled, so [[samAnswerOf]] and [[samResultTpt]] cannot disagree. Counted MODULO OVERRIDING (a re-declared inherited method, e.g. a JVM
    * bridge shape, is ONE method to java and two to `getSignature`): same simple name, same arity, dropped one declared by a STRICT SUPERTYPE. Unrelated supertypes stay two.
    */
  ...
  private[spoon] def adaptedToTarget(target: CtTypeReference[?], t: CtTypeReference[?]): CtTypeReference[?] =
    ...
    else Option(new TypeAdaptor(target).adaptType(t)).getOrElse(t)

  /** does THIS lambda hold a `return` with a VALUE — stopping at a nested lambda/anonymous method, whose `return`s are that construct's own (JLS 15.27.2). Asked of the java, before the body
  ...
  private[spoon] def returnsAValue(l: CtLambda[?]): Boolean =
    ...
    body != null &&
    body
      .getElements(new spoon.reflect.visitor.filter.TypeFilter(classOf[CtReturn[?]]))
      .asScala
      .exists(r =>
        r.getReturnedExpression != null &&
          (r.getParent(classOf[spoon.reflect.declaration.CtExecutable[?]]) eq l)
      )

  /** a type that mentions a TYPE VARIABLE anywhere — the declaration's own `T`, an array of one, or one inside an argument. Fuel-bounded, because a bound can be recursive (`T extends Comparable<T>`)
    * and an unbounded walk on a class file is a hang.
    */
  private[spoon] def mentionsTypeVariable(r: CtTypeReference[?], fuel: Int): Boolean =
    if r == null || fuel <= 0 then false
    else
      r.isInstanceOf[CtTypeParameterReference] ||
      (r match
  ...
  /** does this type's ancestry reach `java.io.Serializable`? Unreadable ancestor answers `false` (the SAM answer for it is `Unreadable`, refused under its own guard). Bounded by `seen` (each
    * qualified name walked at most once) rather than a fuel counter — a hierarchy is as deep as somebody wrote it.
    */
  ...
  /** does every type variable this type mentions resolve HERE? `tpe` renders an unresolved one as a `?T` stub, which is not valid Scala — so a synthesized cast must never target such a type.
    */
  ...
  /** free of type VARIABLES entirely. A callee's formal must satisfy this before we may render it at a CALL SITE: `resolveTypeParam` is name-based, so a callee's `<T>` would silently bind to an
    * unrelated in-scope `T` (`ResourceData<T>` vs `Json.readValue<T>`) and emit a wrong cast.
    */
  ...
  /** [[tpConcrete]] with its one over-exclusion repaired: a type VARIABLE passes when it is LITERALLY the one in scope here ([[sameVarInScope]] — same declaration, not just same name). A separate
    * function, not a flag, since every existing `tpConcrete` caller must keep its current answer (one derivation per question).
    */
  ...
  /** Is this callee type VARIABLE literally the one in scope here — same declaration, hence the same minted symbol, not merely the same NAME (the case [[tpConcrete]] excludes wrongly, e.g. a
    * self-call inside the declaring class). Class formals mint at `<FQN>$$<name>`.
    */
  private[spoon] def sameVarInScope(tv: CtTypeParameterReference): Boolean =
    Option(tv.getDeclaration).map(_.getParent) match
      case Some(ct: CtType[?]) =>
        resolveTypeParam(tv.getSimpleName).contains(minter.resolve(ct.getQualifiedName + "$$" + tv.getSimpleName))
      case _ => false

  /** Concrete, or mentioning only type variables OWNED BY THE CALLEE — never in scope at the call site, so Java's view of the formal is its erasure (an unbounded `<T>` is `Object`), which is what an
    * unchecked cast must name for Scala to infer `T`. Unbounded variables must NOT be excluded: measured false, costs `AssetManager.load` casts sge also writes by hand.
    */
  ...
  private[spoon] def tpBoundErased(tr: CtTypeReference[?]): TypeRepr = TypeShape.of(tr) match
    ...
    case TypeShape.Variable(tv)                       =>
      typeParamDeclOf(tv).map(erasureOfFormal(_, Set.empty, 2)).getOrElse(objectT)
    case TypeShape.Arr(_, c) =>
      AppliedType(TypeRef(NoPrefix, minter.external("scala.Array", "Array")), List(tpBoundErased(c)))
    case TypeShape.Named(r, as) if as.nonEmpty =>
      AppliedType(TypeRef(NoPrefix, typeSym(r)), as.map(tpBoundErased))
    case s => tpe(s.ref)

  /** `tpe` of a callee's DECLARED formal with the receiver's type variables replaced by the receiver's own (known) type ARGUMENTS. `None` whenever any part cannot be named here — a raw generic, an
    * intersection, or a variable outside `subst` that does not resolve in this scope — so a cast is only ever built out of a type the emitted Scala can actually see.
    */
  private[spoon] def substFormal(f: CtTypeReference[?], subst: Map[String, TypeRepr]): Option[TypeRepr] =
    ...
    f match
      ...
      case arr: CtArrayTypeReference[?] =>
        substFormal(arr.getComponentType, subst).map(c => AppliedType(TypeRef(NoPrefix, minter.external("scala.Array", "Array")), List(c)))
      case w: CtWildcardReference =>
        Option(w.getBoundingType).filter(_.getQualifiedName != "java.lang.Object") match
          ...
          case Some(b) => substFormal(b, subst).map(u => if w.isUpper then TypeBounds(NoType, u) else TypeBounds(u, NoType))
      case tv: CtTypeParameterReference =>
  ...
  /** [[mentionsTypeVarFilled]], but also descending into WILDCARD BOUNDS (`Array<? extends T>` does depend on `T`). Kept separate: the existing predicate gates the erased-receiver path, whose
    * measured behaviour must not shift.
    */
  ...
  /** [[tpResolvable]], but through the BARRIER-aware frame: `resolveTypeParam` sees every enclosing scope's parameters by name, while a `static` nested class cannot actually name the outer class's —
    * emitting one there is `Not found: type T`.
    */
  ...
  /** Is `actual` the same type as `want` with some type ARGUMENTS collapsed to `Object` or a wildcard? Exactly the shape of an UNCHECKED CONVERSION java performs silently. Compares RENDERED types,
    * requires the same type constructor and arity — narrow deliberately.
    */
  ...
  /** Is this type-parameter reference THE SAME parameter as the one its simple name resolves to here — compared by minted id (`<owner FQN>$$T`), not by name, since a callee's `<T>` could otherwise
    * silently bind to an unrelated in-scope `T`. Method-level params never match.
    */
  private[spoon] def sameTypeParamHere(tv: CtTypeParameterReference): Boolean =
    val owner = typeParamDeclOf(tv).flatMap(d => Option(d.getParent)).collect { case ct: CtType[?] => ct.getQualifiedName }
    owner.exists(o => accessibleTp(tv.getSimpleName).exists(id => minter.fullNameOf(id) == o + "$$" + tv.getSimpleName))

  /** Can this DECLARED formal be named verbatim at the current call site? Concrete parts always; a type variable only when it is literally the same parameter ([[sameTypeParamHere]]).
    */
  ...
  /** Translate a type-parameter bound. A RAW generic bound (`N extends Node`) is Java's idiom for a self-referential (F-)bound; name-directed fill (see [[nameFilledArgs]]) rebuilds it — the decl's
    * own params are already in scope here (minted before bounds are translated) — rather than erasing to `Node[?, ?, ?]`. Non-raw / array / intersection / type-var bounds defer to `tpe`.
    */
  ...
  // ---- types ----
  /** THE TYPE-REFERENCE DISPATCH — the frontend half of the catalog's fourth obligation surface. Wraps HERE rather than per-arm, so no arm can opt out of the consult. `CtTypeReference` is neither
    * statement nor expression, so neither other wrapper reaches it. `null` enters no scope and owes nothing; answers `objectT`.
    */
  ...
  private[spoon] def typeSym(r: CtTypeReference[?]): SymId =
    val (anns, flags) = try
      ...
      if td != null then
        val a =
          if td.getAnnotations.asScala.exists(_.getAnnotationType.getQualifiedName == "java.lang.FunctionalInterface")
  ...
  // ---- declarations ----
  /** @param owner
    *   overrides the DECLARING TYPE as owner; set only for a METHOD-LOCAL class (JS-C30).
    * @param sourceName
    *   overrides the symbol's name; set only for a local class (Spoon's qualified name holds java's binary disambiguator, not a legal Scala identifier).
    * @param selfClass
    *   , outerVars the ANONYMOUS-class wiring, reused verbatim for local classes.
    */
  private[spoon] def classDef(
    t:          CtType[?],
    owner:      Option[SymId] = scala.None,
    sourceName: Option[String] = scala.None,
    selfClass:  SymId = SymId.None,
    outerVars:  Map[String, SymId] = Map.empty
  ): Tree.ClassDef =
    ...
    def isDropped(e: CtExecutable[?], name: String): Boolean =
      subs.dropsMethod(t.getQualifiedName, name, e.getParameters.asScala.toList.map(p => Option(p.getType).map(_.getSimpleName).getOrElse("?")))
    ...
    /** Record what this walk SAW, then translate what survives — the last place the dropped half exists ([[seenMembers]]). Interleaved so minting order stays exactly what it was, since a run's
      * `SymId` assignment is deterministic-artifact-load-bearing.
      */
  ...
  /** JS-C43 — an IMPLICIT record accessor RETURNS ITS COMPONENT'S FIELD (JLS 8.10.3), written out explicitly for EVERY implicit accessor (a nested record's, from Spoon, calls itself forever in
    * scala's one namespace). An accessor the record WROTE is untouched (`isImplicit` gate).
    */
  private[spoon] def accessorBodies(t: CtType[?], id: SymId, comps: List[RecordComponent], methods: List[Tree.DefDef]): List[Tree.DefDef] =
    if comps.isEmpty then methods
    else
      val derived: Map[SymId, RecordComponent] = comps.flatMap { c =>
        t.getMethods.asScala.find(m => m.isImplicit && m.getSimpleName == c.name && m.getParameters.isEmpty).flatMap(m => minter.defined(memberKey(id, c.name + erasedSig(m))).map(_._1 -> c))
      }.toMap
      methods.map { d =>
        derived.get(d.symbol) match
          ...
          case Some(c)    =>
            ...
            d.copy(rhs = Some(Tree.Block(List(Tree.Return(Some(read), nothing, at)), Tree.Literal(Constant.UnitC, unitT, at), unitT, at)))
      }

  /** java's RECORD COMPONENTS, in DECLARATION ORDER, each joined to its FIELD and ACCESSOR by the same keys `fieldDef1`/`execDef` use — a name-keyed join cannot work once the emitter renames. SORTED
    * BY POSITION, not Spoon's `Set` iteration order. ALL OR NOTHING: any component missing its field or accessor answers `None` for the WHOLE record (never a short list, which would silently compute
    * `equals`/`hashCode` over a subset). `Some(Nil)` for a genuinely empty record.
    */
  ...
  /** THE CANONICAL CONSTRUCTOR AS JLS 8.10.4 DECLARES IT — two parser defects repaired: (1) a compact constructor's implicit trailing field assignments, appended after Spoon's modeled body,
    * components matched to parameters BY POSITION; (2) the implicit constructor's parameter order, reordered to the HEADER's order by reading which field each parameter assigns to.
    */
  private[spoon] def canonicalised(t: CtType[?], id: SymId, comps: List[RecordComponent], ctors: List[Tree.DefDef]): List[Tree.DefDef] =
    if comps.isEmpty then ctors
    else
      val byId: Map[SymId, CtConstructor[?]] = t match
        case c: CtClass[?] =>
          c.getConstructors.asScala.toList.flatMap(k => minter.defined(memberKey(id, "<init>" + erasedSig(k))).map(_._1 -> k)).toMap
        case _ => Map.empty
      ...
      ctors.map(reordered(comps, implicitCanonical, _)).map { d =>
        if !compact.contains(d.symbol) then d
          ...
          val assigns = comps.zipWithIndex.flatMap { (c, k) =>
            ps.lift(k).map { p =>
              val ft = minter.defined(memberKey(id, c.name)).map(_._2.info).getOrElse(NoType)
              Tree.Assign(Tree.Select(Tree.This(id, TypeRef(NoPrefix, id), at), c.field, ft, at), Tree.Ident(p.symbol, p.tpt.tpe, at), unitT, at)
            }
          }
          d.copy(
            rhs = d.rhs.map {
  ...
  /** …repair 2 of [[canonicalised]] — the implicit canonical constructor's parameter ORDER, read off the assignments in its own body. Untouched where the body does not account for EVERY component
    * exactly once, which is both the java-written constructor and any shape this rule has not seen.
    */
  private[spoon] def reordered(comps: List[RecordComponent], implicitCanonical: Set[SymId], d: Tree.DefDef): Tree.DefDef =
    if !implicitCanonical.contains(d.symbol) then d
    else
      val ps = d.paramss.headOption.getOrElse(Nil)

      /** parameter -> the component field this constructor assigns it to. */
      val assignedTo: Map[SymId, SymId] = d.rhs.toList
        .flatMap {
          case b: Tree.Block => b.stats
          case other => List(other)
        }
        .collect { case Tree.Assign(Tree.Select(_: Tree.This, f, _, _), Tree.Ident(p, _, _), _, _, _) =>
          p -> f
        }
        .toMap
  ...
  /** a Java enum constant → `EnumCase`: ctor args, and any per-constant method/field overrides from its anonymous-class body, keyed under the CONSTANT so they don't collide with the enum's abstract
    * members. FIELDS included (JLS 8.1.3 permits `static final` constants in an anonymous class) — harvesting only `CtMethod` silently dropped them (4 errors, uncounted).
    */
  private[spoon] def enumCase(enumId: SymId, v: CtEnumValue[?]): Tree.EnumCase =
    val vlead  = leadingOf(v)
    val caseId = minter.define(memberKey(enumId, v.getSimpleName))(sid => Symbol(sid, v.getSimpleName, qualified(enumId, v.getSimpleName), Flags(isStatic = true), enumId, TypeRef(NoPrefix, enumId)))
  ...
  /** distinguishes anonymous classes within one enclosing type; traversal order is deterministic (every member list is `sortBy(posKey)`), so the minted keys are stable across runs.
    */
  private[spoon] var anonSeq = 0

  /** A Java ANONYMOUS CLASS — `new Base(args) { members }`. Members are owned by a SYNTHETIC symbol (so two listeners' `clicked` cannot collide), but bodies translate with `this` bound to the
    * ENCLOSING class (Spoon's own reading of the implicit `this`; emitter renders `Outer.this.m`). Captured locals seeded by NAME, closed over directly.
    */
  private[spoon] def anonClass(nc: CtNewClass[?], enclosing: SymId, outerVars: Map[String, SymId]): Option[Tree.AnonClass] =
    Option(nc.getAnonymousClass).map { ac =>
      ...
      val id  = minter.define(key)(sid => Symbol(sid, "<anon>", minter.fullNameOf(enclosing) + "$" + anonSeq, Flags(), enclosing, TypeRef(NoPrefix, sid)))
      ...
      val sam = try samAnswerOf(nc.getType)
      catch { case _: Throwable => Sam.Answer.Unreadable }
      Tree.AnonClass(id, members, originOf(nc), dropped.result(), sam)
    }

  /** Does this anonymous-class method OVERRIDE an inherited one? Scala requires `override` when concrete, permits it when abstract — marking every genuine override is safe either way. Spoon answers
    * it from the resolved hierarchy where it can; falls back to name+arity over resolvable supertypes; `java.lang.Object`'s members need [[universalMember]] separately since Spoon has no `Object` in
    * the model under noClasspath.
    */
  ...
  /** Does this redeclare one of `java.lang.Object`'s members? Matched on the full SIGNATURE, not name and arity: `equals(VertexAttribute)` is an OVERLOAD that overrides nothing, and marking it
    * `override` is an error scala reports and java has no opinion on.
    */
  private[spoon] def universalMember(m: CtMethod[?]): Boolean =
    val ps = m.getParameters.asScala.toList.map(p => Option(p.getType).map(_.getQualifiedName).getOrElse("?"))
      ...
      def declares(t: CtTypeReference[?], subst: Map[String, String], fuel: Int): Boolean =
        if t == null || fuel <= 0 then false
          ...
          if decl == null then false
            ...
            val actuals = t.getActualTypeArguments.asScala.toList.map { a =>
            ...
            decl.getMethods.asScala.exists(x =>
              x.getSimpleName == n && (x ne m) &&
  ...
  private[spoon] def defineType(t: CtType[?], owner: Option[SymId] = scala.None, sourceName: Option[String] = scala.None): SymId =
    ...
    minter.define(q)(id =>
      Symbol(
        id,
        sourceName.getOrElse(t.getSimpleName),
        q,
        typeFlags(t),
        owner.getOrElse(ownerSym(t)),
        TypeRef(NoPrefix, id),
        tags = tags,
        annotations = anns,
        droppedAnnotations = annDropped,
        permits = permittedTypes(t)
      )
    )

  /** java's `permits` clause, INTERNED (ids, not names — [[balticporter.tir.Symbol.permits]]). A permitted type the parse never saw interns as an external stub no subtype set can contain, which is
    * the case the seal must widen for. SORTED by key (Spoon hands back a `Set`).
    */
  private[spoon] def permittedTypes(t: CtType[?]): List[SymId] =
    t match
      case s: spoon.reflect.declaration.CtSealable =>
        s.getPermittedTypes.asScala.toList.map(r => typeKey(r) -> r).sortBy(_._1).map((k, r) => minter.external(k, r.getSimpleName))
      case _ => Nil
  ...
  /** `selfClass` is the class a body's `this` denotes when it is NOT the member's owner — the case for an ANONYMOUS class, whose members are owned by a synthetic symbol (so their keys stay distinct)
    * while Java's implicit `this` inside them still reports the ENCLOSING class for every enclosing member it reaches. Defaulting to `owner` keeps every other caller unchanged.
    */
    ...
    withStatic(fieldFlags(f).isStatic) {
      ...
      val id                   = minter.define(memberKey(owner, f.getSimpleName))(sid =>
        Symbol(
          sid,
          f.getSimpleName,
          qualified(owner, f.getSimpleName),
          fieldFlags(f),
          owner,
          ft,
          annotations = fanns,
          droppedAnnotations = fannDropped
        )
      )
  ...
  private[spoon] def execDef(
    ...
    anonSelf:  SymId = SymId.None,
    anonQName: String = ""
  ): Tree.DefDef = withStatic(execFlags(m).isStatic) {
    ...
    tpAccessible.prepend((if execFlags(m).isStatic then Map.empty else tpAccessible.headOption.getOrElse(Map.empty)) ++ frame)
    ...
    val pvs            = ps.map { p =>
      ...
      val pid                  = minter.define(minterKeyOf(id) + "%" + p.getSimpleName)(sid =>
        Symbol(
          ...
          p.getSimpleName,
          paramFullName,
          Flags(isParam = true, isVararg = p.isVarArgs),
          id,
          pt,
          annotations = panns,
          droppedAnnotations = pannDropped
        )
      )
    ...
    minter.set(
      ...
      Symbol(
        ...
        execFlags(m).copy(isOverride = overrides),
        owner,
        sig,
        annotations = anns,
        droppedAnnotations = annDropped,
        descriptor = descriptorOf(m)
      )
    )
    ...
    Tree.DefDef(id, paramss = List(pvs), returnTpt = tt(ret, m), rhs = body, origin = originOf(m), tparams = tpDefs, leading = mlead)
  }

  /** JLS 9.6.2's DEFAULT VALUE, for the one executable that has one and no body — `None` otherwise. Emitter reads this `rhs` as the default of the constructor parameter the `@interface` element
    * becomes (`TirEmitter.classDef1`'s annotation arm).
    */
  ...
  /** Java annotations on a declaration, plus the names of any this could not carry. Element values translate on the ordinary expression path; one that throws is REPORTED (`dropped`), never silently
    * emitted without its arguments.
    */
  private[spoon] def annotationsOf(el: CtElement, bt: Option[BodyTranslator], claimed: String => Boolean = _ => true): (List[Annot], List[String]) =
    ...
    as.foreach { a =>
      ...
      if ref == null then dropped += SpoonTir.UnresolvedAnnotation
        ...
        if vals.contains(Nil) then out += Annot(TypeRef(NoPrefix, minter.external(fqn, simpleName(fqn))), Nil, originOf(a))
  ...
  /** Java's single-value shorthand for an ARRAY-typed annotation element — `@SuppressWarnings("unchecked")` means `value = {"unchecked"}`. Scala has no such shorthand and wants `Array("unchecked")`,
    * so the element's DECLARED type decides whether to wrap. Left alone when the declaration cannot be read — a wrong wrap is worse than the compile error it replaces.
    */
  private[spoon] def arrayShorthand(ref: CtTypeReference[?], key: String, e: CtExpression[?], t: Term): Term =
    if e.isInstanceOf[CtNewArray[?]] then t
      ...
      val declared =
        typeDeclarationOf(ref).flatMap(d => d.getAllMethods.asScala.find(_.getSimpleName == key).flatMap(m => Option(m.getType)))
      declared match
  ...
  /** JLS-effective access for one declaration: exactly one of the three is set, or all are clear and it is public. `implicitPrivate` is the enum-constructor case.
    */
  ...
  /** a METHOD-LOCAL class's SOURCE name (JLS 14.3, catalog JS-C30). Spoon's binary name (`1Local`) is the interning key, not a legal identifier — strip the leading digit run (JLS 3.8). Where
    * stripping leaves nothing, keep the binary name (fails loudly, not silently).
    */
  ...
  /** `about` is the node the refusal is ABOUT, where that differs from the marker's own POSITION (e.g. Spoon's unpositioned `CtCasePattern`) — KIND still comes from the no-arm node.
    */
  private[spoon] def unlowered(el: CtElement, what: String, tpe: TypeRepr, kind: Option[UnportableKind] = scala.None, about: CtElement = null): Term =
  ...
  /** Translates one method/ctor/field-initializer body into TIR terms, resolving every reference to a `SymId`. Unmodeled constructs fail loudly via `Unsupported`. `classId` is the enclosing class
    * (for `this`); `methodId` owns locals. `anonSelf`/`anonQName` set only for an ANONYMOUS class's members — `classId` stays the ENCLOSING class regardless.
    */
  final private[spoon] class BodyTranslator(
    private[spoon] val methodId:  SymId,
    private[spoon] val classId:   SymId,
    private[spoon] val anonSelf:  SymId = SymId.None,
    private[spoon] val anonQName: String = ""
  ) extends SpoonTirBodyCore
      with SpoonTirBodyStmts
      with SpoonTirBodyExprs:
