...
// Split out of SpoonTir.scala for file size: BodyTranslator's statement-side lowering.
...
private[spoon] trait SpoonTirBodyStmts:
  ...
  /** a statement list and the element it came from → a TIR `Block`, trailing comments kept in `trailing`. The ONE place a `Tree.Block` is built out of `stmts` (three callers, one derivation).
    */
  ...
  /** Translate a statement list, folding comment-statements into the statement that follows. Second half of the pair is what is LEFT (a block's `trailing`) — returned, not attached, since not every
  ...
  /** One statement, with a java LABEL on a non-loop statement turned into [[Tree.Labeled]] — `break L` leaves THAT statement, so a labelled `if`/block/`switch` needs a node. A LOOP's label is read
    * into its own node field instead (also `continue L`'s target).
    */
  ...
  /** JS-E03/E04's PREDICATE, as ONE function (not copied at its two consult sites): the target type when java's implicit narrowing applies to a compound assignment, else `None`. Narrows whenever
    * `max(rhsRank, intRank) > targetRank` (java's binary numeric promotion).
    */
  ...
  /** THE STATEMENT DISPATCH — obligation wrapper sits HERE, not per-arm, so no arm can opt out of it. `Lowering.of` maps the runtime class to its registry name once.
    */
  ...
  private[spoon] def stmtArm(s: CtStatement)(using Obligations): Statement = s match
    ...
    case a: CtOperatorAssignment[?, ?] =>
      ...
      val narrow = Obligations.consult(JS.E(3), originOf(a))(compoundNarrow(a)).map(t => tpe(t))
    ...
    case f:   CtForEach            =>
      ...
      Tree.ForEach(
        Tree.ValDef(id, tt(vt, v), None, originOf(v)),
        iterableOperand(f.getExpression),
        blockTerm(f.getBody),
        unitT,
        originOf(f),
        labelOf(f)
    ...
    case t: CtTryWithResource =>
      ...
      // `CtLocalVariable` — refused loudly rather than silently closing one resource fewer
      val res = t.getResources.asScala.toList.map {
        ...
        case other =>
          unsupported(
            other,
            "a try-with-resources resource that is not a local DECLARATION " +
    ...
    // reports the type, which would render a member reference that does not exist — ownership
    ...
    case c: CtClass[?] =>
      ...
      classDef(c, owner = Some(methodId), sourceName = Some(localName(c)), selfClass = classId, outerVars = varScope)
    ...
    // ship the port. `unitT` because a statement produces no value.
    case other => unlowered(other, s"statement ${SpoonKinds.nameOf(other.getClass)}", unitT)

  /** THE ENHANCED-FOR'S ITERABLE, at the type JAVA READ IT AT. JLS 14.14.2 iterates at `Iterable<T>` found among the supertypes, not the expression's own type — an F-BOUNDED wildcard capture fails at
    * an INFERRED type in scala (`E057`) unless ascribed here. Ordinary bounded wildcards are left alone (capture-convert unaided). Declines where the found `Iterable` argument mentions a type
    * VARIABLE.
    */
  private[spoon] def iterableOperand(e: CtExpression[?]): Term =
    val t  = expr(e)
    val et = try Option(e.getType)
  ...
  /** is this an application with a WILDCARD at a SELF-REFERENTIALLY bounded slot — the one shape scala's capture conversion cannot answer? Read off the DECLARATION's own bounds, and the unreadable
    * answer is `false`, which is the pre-rule emission: the failure path leaves the port exactly where it was rather than interposing a view on evidence nobody has.
    */
  ...
  /** `java.lang.Iterable<E>` as reached from `r`'s supertypes, and only where `E` is a type this scope can WRITE — java's own enhanced-for lookup, with an honest decline where it cannot. */
  ...
  /** Assignment to a member whose DECLARED (emitted) type is a bare TYPE PARAMETER, where Java's view of the access was ERASED (a RAW-bounded type variable). Restates the unchecked step. Guarded on
    * the parameter's name resolving here and the value not already having that type.
    */
  ...
  /** the DECLARED type of an assignment target — the field's / local's own declaration, not Spoon's (possibly raw-erased) view of the access. The field path goes through [[fieldDeclOf]]; the variable
    * path wraps `getDeclaration` for the same noClasspath reason — a local's declaration is absent when the enclosing method is external.
    */
  ...
  /** Java permits two implicit conversions Scala forbids: array covariance (`Sub[]` → a `Super[]` slot) and `null` → a type parameter. Insert an explicit `asInstanceOf` so the ported
    * assignment/initializer type-checks.
    */
  private[spoon] val primRank = Map("byte" -> 1, "short" -> 2, "char" -> 2, "int" -> 3, "long" -> 4, "float" -> 5, "double" -> 6)

  /** Java's UNCHECKED generic conversion — converts a RAW-typed value to any instantiation without a check; emits exactly the cast java performs implicitly.
    */
  /** JS-G31 — a POLY EXPRESSION (JLS 15.2), typed by the slot it fills: a cast would elaborate the literal to `scala.FunctionN` first then fail, so emit it AT THE SLOT, never a cast. */
  ...
  /** JS-G31's answer AT THE CALL — every POLY-EXPRESSION argument restored to what `expr` produced, with any cast an argument arm added removed. Answered ONCE here rather than per arm (six and
    * growing). ARITY answered PER INDEX, never by declining the whole call: a vararg-packed tail is answered INSIDE the array against the arguments it was built from.
    */
  private[spoon] def polyArgsUncast(argEs: List[CtExpression[?]], args: List[Term], at: Origin)(using Obligations): List[Term] =
    Obligations
      .consult(JS.G(31), at) {
        val poly = argEs.zipWithIndex.collect { case (e, i) if polyExpression(e) => i }.toSet
        if poly.isEmpty then scala.None
        else if args.sizeIs == argEs.size then Some(args.zipWithIndex.map((t, i) => if poly(i) then uncastAdded(t, argEs(i)) else t))
        else packedUncast(argEs, args, poly)
      }
      .getOrElse(args)

  /** …the OTHER half: a poly expression takes its type from the SLOT, and an OVERLOAD SET gives scala no single slot to type a lambda from (javac resolves by SHAPE; scalac types the literal FIRST,
    * `E134`). Ascribes, never CASTS. Fires only for a LAMBDA argument at an OVERLOADED callee whose slot names no expected type ([[overloadedSamSlot]]), NAMEABLE HERE and un-cast in java. Target is
    * the LAMBDA'S OWN type ([[samResultTpt]]), never the callee's.
    */
  private[spoon] def polyArgsAscribed(ex: CtExecutableReference[?], argEs: List[CtExpression[?]], args: List[Term]): List[Term] =
  ...
  /** the LAMBDA whose own type is the target this argument ascribes to — the argument itself, or a BRANCH of a poly CONDITIONAL (JLS 15.25 pushes the target type through both branches). Target
    * ascribed on the WHOLE conditional, not each branch. `polyExpression` deliberately NOT widened to match — different catalog population.
    */
  ...
  /** is the callee overloaded at this arity, AND does the slot at argument `i` fail to give scala an expected type — read off the declaring type's ALL methods (java's overload set spans the
    * hierarchy) by QUALIFIED NAME. Fires when alternatives DISAGREE at `i`, or agree on a TYPE VARIABLE not yet inferred. Unreadable declaration → no alternatives; `RuntimeException` only, so a deep
    * model's `StackOverflowError` is not swallowed.
    */
  private[spoon] def overloadedSamSlot(ex: CtExecutableReference[?], arity: Int, i: Int): Boolean =
    ...
    (slots.map(_.getQualifiedName).distinct.sizeIs > 1 ||
      ...
      slots.exists(s =>
        s.isInstanceOf[CtTypeParameterReference] &&
          !s.isInstanceOf[CtWildcardReference]
      ))

  /** [[polyArgsUncast]] where a VARARG PACK has changed the arity: `args` is the fixed prefix plus ONE term holding the variadic elements, built from the `argEs` tail in order.
    */
  private[spoon] def packedUncast(argEs: List[CtExpression[?]], args: List[Term], poly: Set[Int]): Option[List[Term]] =
  ...
  /** the casts an ARGUMENT ARM added, removed; the ones the JAVA SOURCE wrote, kept. Unreadable cast list DECLINES (not "java wrote none") — a term left as-is is at worst a cast too many.
    * `RuntimeException` only, so a `StackOverflowError` is not swallowed.
    */
  private[spoon] def uncastAdded(t: Term, e: CtExpression[?]): Term =
    val own = try Some(e.getTypeCasts.size)
    catch { case _: RuntimeException => scala.None }
  ...
  /** THE FORMAL OF AN INHERITED CALLEE, with the ANCESTOR's type variables replaced by what THIS class instantiated them with — `None` where nothing substitutes (`extends` says what THIS class
    * instantiated it as, `ParentSubst`). Declines a WILDCARD formal.
    */
  /** how many `[]` a type reference carries — decides whether a cast at an inherited formal is a translation or a `ClassCastException`.
    */
    ...
    if fuel <= 0 then scala.None
      ...
      TypeShape.of(tr) match
        ...
        case TypeShape.Arr(_, c) =>
          inheritedFormal(c, fuel - 1).map(e => AppliedType(TypeRef(NoPrefix, minter.external("scala.Array", "Array")), List(e)))
        ...
        case s                                    =>
          ...
          if as.isEmpty || subs.forall(_.isEmpty) then scala.None
            ...
            try Some(AppliedType(TypeRef(NoPrefix, typeSym(s.ref)), as.zip(subs).map((a, x) => x.getOrElse(tpe(a)))))
            catch { case _: Throwable => scala.None }

  private[spoon] def uncheckedGeneric(target: CtTypeReference[?], e: CtExpression[?], t: Term, rawTarget: Boolean = true, ownScope: Boolean = true): Term =
    ...
    // via the `extends` clause). A DIMENSION MISMATCH DECLINES rather than casts — a cast would
    // make an arity defect COMPILE and throw at run time instead of a loud typer error.
    ...
    if target == null || et == null || bad then t
      ...
      val ct = try
        if ownScope then tpe(target)
        else calleeInferredTarget(target, e).getOrElse(tpBoundErased(target))
      finally inOverridingMember = savedOv
      Tree.Typed(t, tt(ct, e), ct, originOf(e))

  /** The type java's UNCHECKED CONVERSION actually converts TO, where the callee's formal is its own type VARIABLE and this call INFERRED it: `setSerializer(Class<T>, Serializer<T>)` given
    * `Foo.class` converts a raw `Serializer` to `Serializer<Foo>` (JLS 5.1.9, 18), not to `T`'s bound. `None` where the parser supplies no inference or a name is not writable here — then the bound
    * erasure stands.
    */
  private[spoon] def calleeInferredTarget(target: CtTypeReference[?], e: CtExpression[?]): Option[TypeRepr] =
    Option(e.getParent(classOf[CtInvocation[?]])).flatMap { inv =>
      val fs = Option(inv.getExecutable.getExecutableDeclaration)
        .collect { case m: CtMethod[?] =>
          m.getFormalCtTypeParameters.asScala.toList
        }
        .getOrElse(Nil)
      val as = inv.getActualTypeArguments.asScala.toList
      if fs.isEmpty || fs.sizeIs != as.size then scala.None
      else
        val subst = fs.map(_.getSimpleName).zip(as).collect { case (n, a) if a != null && tpNameableHere(a) => n -> tpe(a) }.toMap
  ...
  /** JS-G13's clause, as a function of the SLOT — java's array covariance (JLS 10.10) puts a value of one array type where another is declared, and scala's `Array` is invariant. Extracted so
    * [[coerce]]/[[slotConsults]] read one predicate.
    */
  ...
  /** …the SAME question asked at the RENDERING, which a java-name test cannot see: java's own ERASURE can collapse two array types into one (e.g. an F-bounded `<E> E[] getUniverse`), so
    * [[arrayCovSlot]] finds nothing to compare while the emitted `Array[E]` still disagrees with `Array[Enum[?]]` (scala arrays are INVARIANT). `want` is HANDED IN, not re-looked-up — a second
    * `tpe(target)` moves the lowering denominators for nothing (measured, 1,675).
    */
  ...
  /** JS-G14's clause — a primitive at a reference slot is java autoboxing, and the boxing's target is the WRAPPER rather than the (often erased) formal. See [[arrayCovSlot]] for why this is a named
    * predicate rather than an inline condition.
    */
  ...
  /** JS-G09's question at a slot — java's UNCHECKED CONVERSION (JLS 5.1.9), legal at a raw type, no scala image but a cast. A SHAPE test, deliberately not [[uncheckedGeneric]]'s narrower gate list,
    * so a refused site still reports "the difference applies here".
    */
  ...
  /** THE SLOT ROWS, consulted at every arm that has a slot — JS-G09, JS-G13, JS-G14. One function, six call sites (`Differences.everySlot`, one JLS 5.2 conversion). Called from the ARM, never
    * [[coerce]] (unreached for a slot-less node, honest discharge). Reads [[castType]] bare, same as `coerce`.
    */
  private[spoon] def slotConsults(slots: List[(CtTypeReference[?], CtExpression[?])], at: Origin)(using Obligations): Unit =
  ...
  /** [[slotConsults]] reached from the DECLARATION dispatch — `fieldDef`'s one caller.
    *
    * A forwarder and not a second statement of the rule: the slot rows are decided in one function whichever dispatch reached them, and this exists only because the three predicates read `castType`,
    * which is a `BodyTranslator` member.
    */
  private[spoon] def slotConsultsAt(slots: List[(CtTypeReference[?], CtExpression[?])], at: Origin)(using Obligations): Unit = slotConsults(slots, at)

  /** the (formal, argument) pairs of a call — the slot list [[slotConsults]] wants. Empty where arities disagree (same case `coerceArgs` declines). Formals read BARE — a `catch` here would fabricate
    * "this callee takes no parameters".
    */
  private[spoon] def argSlots(ex: CtExecutableReference[?], argEs: List[CtExpression[?]]): List[(CtTypeReference[?], CtExpression[?])] =
    val formals = ex.getParameters.asScala.toList
    if formals.sizeIs == argEs.size then formals.zip(argEs).filter(_._1 != null) else Nil

  private[spoon] def coerce(target: CtTypeReference[?], e: CtExpression[?], t: Term, arrayCov: Boolean = true, tpToObject: Boolean = true, unchecked: Boolean = true): Term =
    ...
    // names the WRAPPER to convert to, not just whether to.
    val et        = castType(e)
    ...
    if et != null && !et.isPrimitive && target.isPrimitive && wrapperOf.values.toSet(et.getQualifiedName)
      && wrapperOf.get(target.getSimpleName).exists(_ != et.getQualifiedName)
    then return unbox(t, et.getQualifiedName, target.getSimpleName, e)
    ...
    if cast then
      ...
      // ([[uncheckedGeneric]]'s own fact) — else `tpe` renders a sentinel `Array[?]`. Asked ONLY
      // where a cast is really emitted, to avoid moving denominators for nothing.
      ...
      // to the target type separately) — recurses through `coerce`, not `uncheckedGeneric`
      // directly, so each branch gets whatever conversion IT needs
  ...
  /** the conditional and the `If` it produced, when `t` really is that conditional's translation. Both halves are checked: `expr` may have wrapped or replaced it, and rebuilding something that is no
    * longer an `If` would silently drop a branch.
    */
  ...
  /** the unchecked-conversion decision for a NON-conditional expression — extracted only so the branch recursion above reads as one case beside it.
    */
  ...
  /** `wrapper.<prim>Value()` — explicit unboxing to a primitive, plus the WIDENING beside it where a shortcut would name a nonexistent member. Java's unboxing is TWO conversions (JLS 5.1.8 then
    * 5.1.2); collapsed to one call for the six `Number` wrappers. `Character`/`Boolean` are NOT `Number`s — emitted as two explicit steps instead.
    * @param from
    *   the wrapper's FQN — the SOURCE, known by both callers.
    */
  ...
  /** Java VARARGS at the CALL SITE (`T...` emitted `Array[T]`) — materializes the array java would build; already-array/generic components pass through. Stops at an EXTERNAL callee's `T...` (class
    * file, read REPEATED). [[varargHoldsArray]] below: cast wins, primitive component must match exactly, bare `null` counts, array dimension decides.
    */
  /* …and every one of the three reads below is BARE, because `varargPack` — the TRANSLATION
     ...
     `getTypeCasts` failing answered *the source wrote no cast*. The `null` handling is
     unchanged — an absent `getType` is normal under `noClasspath` and is what the
  ...
  /** the callee's declared parameters, or `scala.None` where the declaration cannot be read, shared by [[varargPack]]/[[callConsults]] so they never disagree. At a `CtNewClass` the parser SYNTHESISES
    * a wrong declaration — Spoon's anonymous-subtype constructor has no real parameter list — so the SUPERCLASS's constructor is read instead (JLS 15.9.5.1), chosen by the ERASED parameter types the
    * reference carries.
    */
  private[spoon] def declParams(ex: CtExecutableReference[?]): Option[List[CtParameter[?]]] =
    anonSuperCtor(ex).orElse(execDeclOf(ex)).map(_.getParameters.asScala.toList)

  /** the SUPERCLASS constructor an anonymous-class construction really invokes — see [[declParams]]. Matches the erased signature FIRST, arity only where unambiguous (a generic constructor's names
    * don't meet under noClasspath erasure, JS-G18).
    */
  private[spoon] def anonSuperCtor(ex: CtExecutableReference[?]): Option[CtExecutable[?]] =
    ...
    def named = cands.filter(_.getParameters.asScala.toList.map(p => Option(p.getType).map(_.getQualifiedName)) == want)
  ...
  /** THE CALL ROWS, consulted at every call dispatch — JS-G18, JS-G32, JS-G37…G40, JS-G42. Called from [[coerceArgs]] (the ONE function both `invocation`/`ctorCall` reach). Predicates read off the
    * REFERENCE/DECLARATION, not by re-running [[varargPack]].
    */
  private[spoon] def callConsults(ex: CtExecutableReference[?], argEs: List[CtExpression[?]], at: Origin)(using Obligations): Unit =
    ...
    // failure would move the consult from one row to its opposite.
    ...
    val comp     = ps
      .filter(_ => variadic)
      .map(_.last.getType)
      .collect { case a: CtArrayTypeReference[?] =>
        a.getComponentType
      }
      .orNull
    ...
    Obligations.consult(JS.G(32), at)(Option.when(ps.exists(_.exists(p => Option(p.getType).exists(f => mentionsAnyTypeVar(f) && !tpResolvable(f)))))(()))
  ...
  private[spoon] def varargPack(ex: CtExecutableReference[?], argEs: List[CtExpression[?]], recvSubst: Map[String, CtTypeReference[?]]): Option[List[Term]] =
    val ps = declParams(ex)
    ps match
      case Some(l) if l.nonEmpty && l.last.isVarArgs =>
        ...
        // SPREAD at an external one (see `passThrough`). Component types must agree (java would
        // otherwise pack a primitive array mismatch instead); reference components pass
        ...
        // (preferred over argument inference — erase USES, never DECLARATIONS); else the
        // RECEIVER's instantiation for a known receiver's own type variable; else inferred from
        // the trailing arguments' own type, only when they all agree on one concrete type
        val elemRef: Option[CtTypeReference[?]] =
          if comp != null && tpConcrete(comp) then Some(comp)
          else if comp != null && !comp.isInstanceOf[CtTypeParameterReference] then Some(comp)
          else if comp != null && recvSubst.contains(comp.getSimpleName) then Some(recvSubst(comp.getSimpleName))
          else
            val ts = argEs.drop(fixed).map(e => e.getType)
            Option.when(
              ts.nonEmpty && ts.forall(t => t != null && !t.isPrimitive && tpConcrete(t)) &&
                ts.map(_.getQualifiedName).distinct.sizeIs == 1
            )(ts.head)
        ...
        if comp == null || argEs.sizeIs < fixed then None
          ...
          // THE ELEMENT TYPE, with an ANCESTOR's type variables replaced — else `tpe` renders a
          // `?H` sentinel. [[inheritedFormal]] is the SAME lookup the
          ...
          Some(fixedTerms :+ (if external then Tree.Repeated(elems, at, o)
                              else Tree.NewArray(TypeTree(ct, o), Nil, Some(elems), at, o)))
      case _ => None

  /** java already holds the array and passes it WHOLE through the `T...` slot — the MIRROR of the pack above. Callee OURS: `None`, ordinary argument list. Callee a CLASS FILE: scalac reads `T...` as
    * REPEATED and a bare array conforms as ONE element (a silent `Object`-element bug, or an uncounted compile error otherwise) — so the array is SPREAD (`arr*`), which still ALIASES `arr` as java's
    * pass-through does.
    */
  private[spoon] def passedThrough(ex: CtExecutableReference[?], argEs: List[CtExpression[?]], external: Boolean, recvSubst: Map[String, CtTypeReference[?]]): Option[List[Term]] =
  ...
  /** coerce each argument to its formal parameter type (Java autoboxing / numeric narrowing that Scala won't do implicitly). Skipped when arities differ (varargs spread etc.).
    */
  private[spoon] def coerceArgs(ex: CtExecutableReference[?], argEs: List[CtExpression[?]], at: Origin, recvSubst: Map[String, CtTypeReference[?]] = Map.empty)(using Obligations): List[Term] =
  ...
  /** the receiver's own type arguments, by the declaring class's parameter NAMES — `Graph<V>` called on `DirectedGraph<Integer>` gives `V -> Integer`. Only a fully known instantiation (same arity,
    * every argument NAMEABLE HERE — not merely CONCRETE, `tpConcrete` excludes the caller's own variable wrongly, [[tpNameableHere]] is the repair — no wildcards).
    */
  ...
  /** what a REFERENCE's instantiation says the DECLARING type's formals are — `Bag<V>` says `E := V`, by position. ONE derivation, read by [[receiverTypeArgs]] and by [[nullToSamResult]] — Spoon's
    * `TypeAdaptor` measurably does NOT answer this for a lambda target, so it is derived here rather than asked of it.
    */
  private[spoon] def typeArgSubst(rt: CtTypeReference[?]): Map[String, CtTypeReference[?]] =
    if rt == null || rt.isPrimitive || rt.isInstanceOf[CtArrayTypeReference[?]] ||
      rt.isInstanceOf[CtTypeParameterReference] || rt.isInstanceOf[CtWildcardReference]
    then Map.empty
    else
  ...
  /** @param recvSubst
    *   the RECEIVER's own type arguments ([[receiverTypeArgs]]) — THREADED from [[coerceArgs]], never re-derived. Only [[nullToTypeParam]] reads it: a callee's own type variables do not resolve at
    *   the call site, but the CLASS's do.
    */
  private[spoon] def coerceArgsFixed(ex: CtExecutableReference[?], argEs: List[CtExpression[?]], recvSubst: Map[String, CtTypeReference[?]] = Map.empty): List[Term] =
    ...
    if formals.size == argEs.size then
      argEs.zipWithIndex.map { (e, i) =>
        val base = expr(e)
        val c    = nullToTypeParam(e, declFormals(i), recvSubst, coerce(formals(i), e, base, arrayCov = external, tpToObject = false, unchecked = false))
  ...
  /** `null` passed to a callee slot whose real (un-erased) formal is a type parameter — cast it (`m(null)` → `m(null.asInstanceOf[T])`). Dominant case: a self-call in scope. Second case: the
    * RECEIVER's — the declaring CLASS's variables resolve through the receiver's type arguments, tried FIRST (exact, keyed on the DECLARING CLASS's own formals, unlike the name-based
    * `resolveTypeParam`), and only for a variable the class declares.
    */
  private[spoon] def nullToTypeParam(e: CtExpression[?], declFormal: Option[CtTypeReference[?]], recvSubst: Map[String, CtTypeReference[?]], t: Term): Term =
    val isNull = e match { case l: CtLiteral[?] => l.getValue == null; case _ => false }
    ...
    declFormal match
      case Some(tp: CtTypeParameterReference)
          if isNull &&
  ...
  /** An ARRAY argument whose emitted element type is not the declared formal's — java arrays are COVARIANT with an erased generic element, scala's are INVARIANT. Faithful port is an explicit
    * `asInstanceOf` at the USE, never a widened DECLARATION (measured catastrophic, see [[erasureOfFormal]]). Driven by the DECLARATION's formal, never the reference's erased one. Gated on
    * [[formalNameableHere]] so the cast never names an unwritable variable.
    */
  ...
  /** A type-parameter-typed value flowing into a slot whose real formal is concretely `java.lang.Object` (`Json.writeValue(String, Object, …)`): Java erases `T` to `Object`, but Scala's unbounded
    * `T <: Any` does NOT conform to `Object`. Cast (`resource.asInstanceOf[Object]`). Gated on the DECLARED formal being Object — NOT a type parameter erased to Object — so we never break our own
    * `foo(x: T)` methods (whose real Scala signature keeps the invariant `T`).
    */
  private[spoon] def typeParamToObject(e: CtExpression[?], declFormal: Option[CtTypeReference[?]], t: Term): Term =
    ...
    // it hands `Any` to an `Object` slot. Read off THIS FRONTEND's own record, not the java — the
    // widening happened only at the DECLARATION.
    val anyDeclared = t match
      case Tree.Ident(s, _, _) =>
        minter.infoOf(s) match
          case TypeRef(_, a) => minter.fullNameOf(a) == "scala.Any"
          case _             => false
      case _ => false
    declFormal match
      case Some(f)
          if !f.isInstanceOf[CtTypeParameterReference] && f.getQualifiedName == "java.lang.Object"
  ...
  /** Java switch → TIR `Match`. Empty (grouping) cases merge their labels into the next; genuine fallthrough is lowered by TAIL DUPLICATION — a non-terminated case's body is its own statements
    * followed by the next case's closure (the same faithful lowering the BIR frontend uses), so no `Unsupported`.
    */
  ...
  /** A SWITCH EXPRESSION — JLS 15.28, catalog `JS-S09`. `CtSwitchExpression` does NOT extend `CtSwitch`; `Tree.Match` renders in either position, only the arms differ. THREE JLS rules: no fall-out
    * arm (EXHAUSTIVE, 15.28.1); an arm produces a VALUE (tail `yield` peeled, others stay [[Tree.Yield]] under a boundary); `yield` not unwrapped here. Fallthrough/break/label shared with the
    * statement form via [[switchArms]].
    */
  private[spoon] def switchExpr(sw: CtSwitchExpression[?, ?])(using Obligations): Term =
    ...
    Tree.Match(expr(sw.getSelector), switchArms(cases, sw, selT, resT, isExpr = true), resT, originOf(sw), isExpr = true)

  /** the statements of one `case`, with Spoon's ARROW normalisation undone where java has no such construct. Flattens an arrow-arm's `CtBlock`; unwraps an arrow STATEMENT arm's synthetic
    * `CtYieldStatement` (JLS 14.21: `yield` is legal only in a switch EXPRESSION).
    */
  private[spoon] def caseBody(c: CtCase[?], isExpr: Boolean): List[CtStatement] =
    ...
    if isExpr then raw
      ...
      raw.map {
        case y: CtYieldStatement =>
          y.getExpression match
            case st: CtStatement => st
            case _ => y
        case other => other
      }

  /** ONE java switch's arms, at either of its two positions. The fallthrough lowering, the labelled-vs-unlabelled break distinction and the empty-arm label accumulation are the same rules for a
    * statement and an expression — java's colon form falls through in both — so they are stated once. What the caller supplies is how an arm's BODY becomes a term: a statement arm is a `Unit` block,
    * an expression arm's block result is the arm's value.
    */
  private[spoon] def switchArms(cases: List[CtCase[?]], el: CtElement, selT: TypeRepr, resT: TypeRepr, isExpr: Boolean)(using Obligations): List[Tree.CaseDef] =
    ...
    val split = cases.map { c =>
      ...
      raw.reverse.dropWhile(_.isInstanceOf[CtComment]) match
        ...
        case rest =>
          (raw,
           c.getCaseKind == CaseKind.ARROW || rest.headOption.exists {
    ...
    Obligations.consult(JS.S(7), originOf(el))(Option.when(cases.zip(split).exists((c, sp) => sp._2 && sp._1.size != caseBody(c, isExpr).size))(()))
    ...
    Obligations.consult(JS.S(4), originOf(el))(Option.when(cases.indices.exists(i => !split(i)._2 && i != cases.length - 1 && split(i)._1.nonEmpty))(()))
    ...
    // consult written in each would be one rule with two copies.
    Obligations.consult(JS.S(10), originOf(el))(Option.when(cases.exists(_.getCaseExpressions.asScala.exists(_.isInstanceOf[CtCasePattern])))(()))
  ...
  /** one case LABEL — the SPLIT `JS-S10` is about. A TYPE PATTERN (JLS 14.11.1) lowers to `Tree.TypePattern` + `CaseDef.guard`; a RECORD pattern too ([[recordPattern]]). An UNNAMED pattern stays
    * refused, since no source this parser accepts builds one. MARKER minted HERE since `CtCasePattern` carries no source POSITION — carries the SELECTOR's type. The binding is an ordinary local,
    * interning correctly even when two same-named arms shadow.
    */
  ...
  /** `case Point(int x, int y) ->` — java's RECORD PATTERN, as scala's constructor pattern. THE ONE DISTINCTION: JLS 14.30.2's UNCONDITIONAL component matches `null` ([[Tree.BindPattern]]), a
    * narrowing one ([[Tree.TypePattern]]) does not — asked of SPOON's `isSubtypeOf`. A component that is neither shape is refused IN PLACE. The RECORD must be one this run LOWERS (`JS-C43`'s
    * `unapply`), decided structurally, never by name.
    */
  private[spoon] def recordPattern(rp: CtRecordPattern, c: CtCase[?], selT: TypeRepr): Term =
    ...
    val decl = Option(rp.getRecordType).flatMap(r => Option(r.getTypeDeclaration)).collect { case r: CtRecord =>
      r
    }
    if decl.isEmpty then
      return unlowered(
        c,
        "a RECORD PATTERN over a record this run does not model (JLS 14.30.1). " +
    ...
    val comps = decl.toList.flatMap(_.getRecordComponents.asScala.toList.sortBy(posKey).map(_.getType))
    val subs  = rp.getPatternList.asScala.toList.zipWithIndex.map { (p, k) =>
      p match
        ...
        case other =>
          unlowered(
            c,
            s"a record-pattern COMPONENT — ${SpoonKinds.nameOf(other.getClass)} " +
              "(JLS 14.30.1)",
            tpe(rp.getRecordType),
            about = other
          )
    }
    Tree.RecordPattern(tt(rt, rp), subs, rt, at)

  /** is a component pattern UNCONDITIONAL — does its type already cover the component's (JLS 14.30.2)? `false` where the component's type is unknown, which is the narrowing arm and the conservative
    * side.
    */
  ...
  /** one switch-EXPRESSION arm's statements as a term whose VALUE is the arm's. The last statement is the arm's result; a tail `yield` is peeled, since that is what a scala arm already means and
    * carrying it would need a boundary nobody wants. Everything else is left exactly as translated — a `Throw`, or an `if` whose branches all jump, is `Nothing` in scala and conforms, java's own
    * definite-completion rule (JLS 15.28.1) doing the work.
    */
  ...
  /** peel a TAIL `yield` to the value it carries — through a comment wrapper, which is where the trivia harvest puts an arm's own comments.
    */
  private[spoon] def unYield(t: Term): Term = t match
    case y: Tree.Yield     => y.value
    case c: Tree.Commented =>
      c.stmt match
        case y: Tree.Yield => c.copy(stmt = y.value)
        case _ => t
    case _ => t

  /** is this an ENHANCED switch STATEMENT — one java requires EXHAUSTIVE (JLS 14.11.2), so it does NOT fall out? Asks BOTH disjuncts: the LABEL shape, and the SELECTOR'S TYPE (a QUALIFIED ENUM
    * CONSTANT betrays nothing in the label list, JEP 441). Label alone is wrong against javac. `noClasspath` unresolvable → `false`.
    */
  ...
  /** JLS 14.11.2's classic selector set, by qualified name. A selector typed as one of these is a classic switch however its labels are spelled; an ENUM is the set's sixth member and is asked
    * structurally below, because there is no name to list.
    */
  ...
  /** does the selector's type PROVABLY resolve to something outside [[ClassicSelectorTypes]]? `false` is the answer for everything this cannot see — an absent type, an unresolved name, a type
    * parameter, an annotation type: keeps the fall-out arm java's classic form has rather than fabricating a fact. The one lookup wrapped is the RESOLUTION, where an absent value is normal under
    * `noClasspath`.
    */
  private[spoon] def selectorOutsideClassicSet(selector: CtExpression[?]): Boolean =
    val ref = try Option(selector.getType)
    catch { case _: Throwable => None }
    ref.exists { r =>
