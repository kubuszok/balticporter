...
/** Populates the typed IR ([[balticporter.tir]]) directly from Spoon's resolved model: every declaration mints a stable-identity [[Symbol]], every type reference resolves to a structured
  * [[TypeRepr]], and externals are lazily interned so `usagesOf` works with no local definition. Scope: declarations, signatures, types, and method bodies. [[Xref.build]] indexes usages.
  */
object SpoonTir:

  /** the PUBLIC instance methods of `java.lang.Object`, by Spoon signature — JLS 9.8's exclusion when counting a functional interface's abstract methods. `clone`/`finalize` excluded: they are
    * `protected`, so JLS 9.8 does not exclude them.
    */
  ...
  /** Sentinel entries `annotationsOf` can put in `Symbol.droppedAnnotations` beside real annotation names, so `omissions` distinguishes the reasons a drop happened: `<unresolved>` (type would not
    * resolve), `<unreadable-annotations>` (whole set failed to read), `<annotation-arguments-failed>` (constant-expression path threw — an engine defect, not policy).
    */
  ...
  /** Build the Spoon model over a whole closure and return its top-level types. Full classpath by default; `lenient` uses noClasspath mode so unconfigured external deps still parse (unresolved types
    * degrade to unmapped references).
    */
  ...
  /** Convenience for tests / snippets: parse one in-memory source (no external classpath; JDK types resolve by qualified name) and populate the TIR from its top-level types.
    */
  def fromSource(
    code:        String,
    fileName:    String = "Snippet.java",
    subs:        Substitutions = Substitutions.none,
    catalog:     CatalogLog = CatalogLog.discarding,
    annotations: AnnotationPolicy = AnnotationPolicy.none,
    internTypes: Set[String] = Set.empty
  ...
  /** The same, over several compilation units — needed to test package-boundary rules (default access, `protected`, cross-package overrides) that one snippet cannot exercise. Each pair is
  ...
  private[spoon] enum TypeShape:
    ...
    /** the reference's own type ARGUMENTS exactly as `getActualTypeArguments` reports them, so a caller that treats several kinds alike reproduces the `case r =>` it used to fall into.
      */
  ...
  private[spoon] object TypeShape:
    ...
    /** THE derivation. The WILDCARD arm is first, and that order is the whole point (see the enum's doc): `CtWildcardReference <: CtTypeParameterReference`, so any other order silently answers "type
      * variable" for every `?` in the program.
      */
