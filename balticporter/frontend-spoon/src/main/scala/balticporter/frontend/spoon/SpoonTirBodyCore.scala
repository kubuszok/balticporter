...
private[spoon] trait SpoonTirBodyCore:
  ...
  /** locals visible from HERE, by name — handed to a nested anonymous class so the locals it CAPTURES resolve to the real symbols rather than to `?var$x` stubs (the emitted text was already right,
    * since Scala captures by name too; this keeps the xref honest).
    */
  ...
  /** a java LABEL on a loop (`outer: for (…)`), the target of `break outer` / `continue outer`. Kept so the emitter can name the corresponding `boundary` and jump to it explicitly.
    */
  private[spoon] def labelOf(s: CtStatement): Option[String] =
    try Option(s.getLabel).filter(_.nonEmpty)
    catch { case _: Throwable => scala.None }

  private[spoon] def superTerm(el: CtElement): Term = Tree.Super(classId, selfT, originOf(el))

  /** true when a `this`-access targets THIS class (not an enclosing one) — only then does it need qualifying; an outer `Outer.this.x` resolves bare in Scala.
    */
  ...
  /** A `this` used as a VALUE. Inside an anonymous class body it denotes the ANONYMOUS instance, not the enclosing one — only for a `this` Spoon EXPLICITLY types as the anonymous class, and only in
    * value position (as a member-access TARGET, the existing bare-name resolution stays in charge, matching java's own lexical resolution).
    */
  ...
  /** `Outer.this` — the enclosing instance. Only for a type that LEXICALLY ENCLOSES the access; Spoon also reports a plain `this` typed at an INHERITED member's DECLARING type, which `Outer.this`
    * syntax cannot denote and must not be qualified as.
    */
