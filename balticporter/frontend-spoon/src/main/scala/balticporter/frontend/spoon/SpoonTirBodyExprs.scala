...
// Split out of SpoonTir.scala for file size: BodyTranslator's expression-side lowering.
...
private[spoon] trait SpoonTirBodyExprs:
  ...
  // ---- expressions ----
  /** the casts the SOURCE wrote, applied innermost-first. `(T) x` is `x.asInstanceOf[T]` except a boxed-WRAPPER operand cast to a primitive, which is a CONVERSION in java (JLS 5.1.8 then 5.1.2) —
    * probed javac/scalac 3.8.4 over all 45 cells. `Object`/`Number` excluded (java performs no dispatch there, JLS 5.5); same-type unbox left to `Predef` ([[coerce]]).
    */
  private[spoon] def expr(e: CtExpression[?]): Term =
    ...
    casts
      .foldRight((core, et0)) { case (t, (acc, src)) =>
        (castOf(t, src, acc, e), t: CtTypeReference[?])
      }
      ._1

  /** one source cast, as the conversion or the assertion java performs there. See [[expr]]. */
  private[spoon] def castOf(target: CtTypeReference[?], src: CtTypeReference[?], acc: Term, e: CtExpression[?]): Term =
  ...
  /** THE TYPE AN EXPRESSION HAS WHERE IT STANDS — after the source's own casts, at the OUTERMOST one, the HEAD of `getTypeCasts` ([[expr]] folds `foldRight`, so the head is the OUTER `Tree.Typed`,
    * matching java's order). One function, six callers — the idiom was written six times taking `lastOption`, the INNERMOST cast, silently wrong. `null` where Spoon has no answer; callers decline
    * honestly.
    */
  private[spoon] def castType(e: CtExpression[?]): CtTypeReference[?] =
    e.getTypeCasts.asScala.headOption.getOrElse(e.getType)

  /** THE EXPRESSION DISPATCH — the wrapper's second entry, symmetrical with [[stmtKind]] and for the same reason. See that method for why it is here and not in the arms.
    */
  ...
  private[spoon] def exprArm(e: CtExpression[?])(using Obligations): Term = e match
    ...
    case b:   CtBinaryOperator[?]                                          =>
      ...
      if b.getKind == BinaryOperatorKind.INSTANCEOF then
        b.getRightHandOperand match
          ...
          // diverges under CAPTURE). MARKED rather than thrown: the whole `instanceof` is a
          // boolean EXPRESSION, a shape a term marker takes exactly.
          case p: CtPattern =>
            unlowered(
              b,
        ...
        stringified
          .orElse(identity)
          .getOrElse(
            opText(b.getKind).fold(unknownOp(b.getKind, b, ty(b)))(op => binApply(op, expr(b.getLeftHandOperand), expr(b.getRightHandOperand), ty(b)))
          )
    case u: CtUnaryOperator[?] =>
      ...
      Obligations
        .consult(JS.E(2), originOf(u))(incDecOf(u))
        .getOrElse(
          u.getKind match
            ...
            // origin. Default is a MARKER, `FrontendBlindSpot` (not `UnmodelledNodeKind`,
            // since the kind IS dispatched on here — what is missing is one shape of it).
            case other =>
              unlowered(
                u,
                s"unary operator kind '$other' — this arm enumerates java's eight and " +
                  "the parser produced a ninth",
                ty(u),
                Some(UnportableKind.FrontendBlindSpot)
              )
        )
    ...
    case a: CtOperatorAssignment[?, ?] =>
      ...
      val narrow = Obligations.consult(JS.E(4), originOf(a))(compoundNarrow(a)).map(t => tpe(t))
    ...
    case a: CtAssignment[?, ?] =>
      ...
      Obligations.consult(JS.E(15), originOf(a))(Some(lhs)).fold[Term](st)(v2 => Tree.Block(List(st), v2, ty(a), originOf(a)))
    case c: CtConditional[?] =>
      ...
      val ascribe = Obligations.consult(JS.E(5), originOf(c))(if ct != NoType && condTypeResolves(c) then Some(ct) else scala.None)
  ...
  /** JS-E05's NUMERIC half — JLS 15.25.2's binary numeric promotion, on the OPERAND, not a cast at the enclosing slot: java computes a conditional's PRIMITIVE type where scala takes the lub, so each
    * operand converts here. BOTH DIRECTIONS (scala 3 dropped weak conformance); never promotes on its own, never touches a REFERENCE conditional. Operand read through [[castType]]; an unresolvable
    * type is left ALONE.
    */
  ...
  /** the conditional's static type is safe to ascribe onto a null branch — a concrete type, or a type parameter that actually resolves in scope (not the `?T` unresolved stub).
    */
  ...
  private[spoon] def newArray(na: CtNewArray[?])(using Obligations): Term =
    ...
    Obligations.consult(JS.G(15), originOf(na))(
      Option.when(
        idiomCasts.collect { case a: CtArrayTypeReference[?] => a.getComponentType }.exists(c => c != null && (c.isInstanceOf[CtTypeParameterReference] || isGenericUse(c)))
      )(())
    )
  ...
  private[spoon] def lambda(l: CtLambda[?]): Term =
    ...
    // …and the SAM METHOD's result type where the class file states one. Without it the emitter
    // cannot name the nested `def` that restores java's `return`-leaves-the-LAMBDA, and what it
    // leaves instead is not a compile error but a scala NON-LOCAL RETURN from the enclosing
    // method — valid, green, and something else.
    Tree.Lambda(pvs, body, ty(l), originOf(l), resultTpt = samResultTpt(l))

  /** [[nullToTypeParam]]'s rule at the ONE expression position with no formal: an EXPRESSION- bodied lambda takes its body type from the SAM's RESULT (JLS 15.27.3), so where that result is the
    * target's own type variable, `Null` needs an explicit cast. Variable resolved through the TARGET's own instantiation ([[typeArgSubst]]).
    */
  ...
  /** What does reference `at` say the variable `tv`, DECLARED BY `owner`, is? Composed one edge at a time through the `extends` chain (`Maker<T> extends Fn<String,V>` resolves `Fn.R` via `V := T`
    * then `R := V`) — replaces Spoon's `TypeAdaptor`, which under `noClasspath` hands back the interface's own variable un-adapted. Fuel-bounded; `None` where a declaration cannot be read, caught by
    * the caller's bare-`null` fallback.
    */
  private[spoon] def actualFor(at: CtTypeReference[?], owner: String, tv: String, fuel: Int): Option[CtTypeReference[?]] =
      ...
      if f <= 0 || here == null then scala.None
        ...
        supers.iterator
          .map { s =>
            ...
            val actuals = s.getActualTypeArguments.asScala.toList.map { case tp: CtTypeParameterReference => subst.getOrElse(tp.getSimpleName, tp); case o => o }
  ...
  /** the referenced executable's `static` modifier and its declared ARITY — read HERE since neither survives to the symbol for an EXTERNAL member ([[Tree.MethodRef.referent]]). The DECLARATION
    * answers where the parse resolved one (even a class-file shadow); the REFERENCE is the fallback and its arity is exact even under a lenient parse. `isStatic` on a reference with no declaration is
    * the one value here that can be a guess.
    */
  private[spoon] def referentOf(ex: CtExecutableReference[?]): Referent =
    ...
    // too (see [[Referent]]).
    val n = decl.map(_.getParameters.asScala.size).getOrElse(ex.getParameters.asScala.size)
    if stat then Referent.Static(n) else Referent.Instance(n)

  private[spoon] def fieldAccess(ref: CtFieldReference[?], target: CtExpression[?], at: CtExpression[?])(using Obligations): Term =
    ...
    Obligations.consult(JS.C(2), originOf(at))(
      Option.when(
        staticRecv && (target match
          case ta: CtTypeAccess[?] =>
            ...
            Option(ta.getAccessedType).exists(a => declaringStaticType(a, ref.getSimpleName).exists(_.getQualifiedName != a.getQualifiedName))
          case _ => false)
      )(())
    )
    ...
    if ref.getSimpleName == "class" then
    ...
    else if ref.getSimpleName == "length" && Option(ref.getDeclaringType).exists(_.isInstanceOf[CtArrayTypeReference[?]]) then Tree.ArrayLength(expr(target), ty(at), originOf(at))
    else
      ...
      target match
        ...
        case ta: CtThisAccess[?] if !isOwnThis(ta) =>
          ...
          outerThis(ta).map(q => Tree.Select(q, fid, ty(at), originOf(at))).getOrElse(Tree.Ident(fid, ty(at), originOf(at)))
  ...
  /** A field read/written through a WILDCARD/RAW receiver: `assetDesc.type` off `AssetDescriptor[?]` yields a fresh CAPTURE that unifies with nothing downstream. Java reads such a field at the ERASED
    * type; emit that view. Returns BOTH the receiver's erased type and the field's type through it, since a `Tree.Select` carrying Spoon's un-erased field type over an erased receiver is a TIR node
    * the emitted Scala does not have.
    */
  private[spoon] def erasedFieldReceiver(ref: CtFieldReference[?], target: CtExpression[?]): Option[(TypeRepr, TypeRepr)] =
    ...
    then None
      ...
      if unknown && depends then
        ...
        Some((AppliedType(TypeRef(NoPrefix, typeSym(rt)), erasedArgs), declTpe.map(erasedFormal(_, subst)).getOrElse(objectT)))
      else None

  /** A Java static field read through a SUBCLASS (`Rotational3D.TMP_V3`, where `TMP_V3` is declared in an ancestor `DynamicsModifier`) — Scala companion objects don't inherit statics, so resolve the
    * field to its real DECLARING type and qualify by that (`DynamicsModifier.TMP_V3`). Falls back to the written qualifier when the declaring type can't be located.
    */
  ...
  /** the type that DECLARES a static field `name` (degrades to `None` on shadow/unresolved types). Walks the WHOLE inheritance closure (superclass AND superinterfaces — a java interface constant is
    * inherited through `implements` too), breadth-first with the class edge FIRST (java's own shadowing precedence).
    */
  ...
  private[spoon] def fieldSym(ref: CtFieldReference[?]): SymId =
    val ownerQ  = fieldDeclOf(ref).flatMap(fd => Option(fd.getDeclaringType)).map(_.getQualifiedName).orElse(Option(ref.getDeclaringType).map(_.getQualifiedName)).getOrElse("java.lang.Object")
    val ownerId = minter.external(ownerQ, simpleName(ownerQ))
    outer.externalMember(
      ownerId,
      ref.getSimpleName,
      ref.getSimpleName,
      info = externalFieldType(ref),
      annotations = fieldDeclOf(ref).toList.flatMap(outer.deprecatedOf)
    )

  /** the DECLARED type of an EXTERNAL field, as a class file states it — [[externalSignature]]'s fact for a field (the seam a `Select` node makes is invisible to anything keyed on `Tree.Apply`).
    * Rendered SCOPE-FREE through [[externalSlot]]. Only for a SHADOW declaration; a program-declared field gets its real type from `fieldDef`.
    */
  ...
  /** Java's WILDCARD/RAW-receiver calls: scala gives every member access a fresh CAPTURE, so a value off one receiver never conforms to another's formal. Java's own view is ERASED, performed
    * unchecked — cast the RECEIVER to its erased instantiation and each dependent argument to that formal's erasure (same erasure rules, so they agree). Gated to calls that genuinely DEPEND on the
    * receiver's type variables.
    */
        ...
        case t                                                             =>
          ...
          val declaredVar = t match
            case fa: CtFieldAccess[?] =>
              fieldDeclOf(fa.getVariable).map(_.getType).exists(_.isInstanceOf[CtTypeParameterReference])
            case _ => false
          ...
          then None
            ...
            val names   = formals.map(_.getSimpleName).toSet
            // The RESULT is deliberately NOT asked: a raw type erases its members whole (JLS 4.8),
            // but Spoon reports a CAPTURE as raw, so a result-driven view erases receivers java
            // never erased.
            val depends =
              execDeclOf(ex).map(_.getParameters.asScala.toList.map(_.getType)).exists(_.exists(p => p != null && mentionsTypeVarFilled(p, names)))
            // Only an F-BOUNDED class needs this: there the erasure has no Scala image at all, so
            // the name-directed fill is the only expressible reading. Everywhere else the erasure
            // is load-bearing.
            ...
            // F-BOUNDED classes excluded whole (arguments discharge each other's bounds).
            def writtenAt(i: Int): Option[TypeRepr] =
              if anyFBounded then scala.None
              else
                actuals.lift(i).flatMap { a =>
            ...
            if unknown && depends then
              ...
              val args    = formals.zipWithIndex.map { (f, i) =>
                writtenAt(i).getOrElse {
                  ...
                  named
                    .map { id =>
                      val r = TypeRef(NoPrefix, id); namedOf(f.getSimpleName) = r; r
                    }
                    .getOrElse {
  ...
  /** `(N) this` — the SELF-TYPE conversion at a raw call. Java accepted `this` at a raw receiver's `N` formal only because the receiver is raw — libGDX writes `(N) this` itself where it isn't.
    * Restricted to `this`: a general "cast any argument" rule measured 1 -> 11.
    */
  private[spoon] def selfTypeArgs(
    ex:    CtExecutableReference[?],
    argEs: List[CtExpression[?]],
    args:  List[Term],
      ...
      ps match
        case Some(l) if l.sizeIs == args.size && argEs.sizeIs == args.size =>
          args.zipWithIndex.map { (t, i) =>
            (l(i), argEs(i)) match
              case (tv: CtTypeParameterReference, _: CtThisAccess[?]) if nm.contains(tv.getSimpleName) && nm(tv.getSimpleName) != t.tpe =>
  ...
  /** cast each argument whose DECLARED formal mentions a receiver type variable to that formal's erasure, matching the erased receiver the call is now made through.
    */
  private[spoon] def eraseDependentArgs(
    ex:    CtExecutableReference[?],
    argEs: List[CtExpression[?]],
    args:  List[Term],
    subst: Map[String, TypeRepr],
  ...
  /** Java's UNCHECKED conversion at an ARGUMENT, when the RECEIVER's instantiation is KNOWN — the complement of [[erasedReceiverView]] (whose receiver is UNKNOWN, mutually exclusive). Narrow: only a
    * formal mentioning a receiver type variable, only an argument our raw fill wildcarded, only when the substituted formal is fully nameable here.
    */
    ...
    then args
      ...
      (known, ps) match
        case (true, Some(l)) if l.sizeIs == args.size && argEs.sizeIs == args.size =>
          ...
          val subst      = formals.map(_.getSimpleName).zip(if fieldRecv then atDeclScope(actuals.map(tpe)) else actuals.map(tpe)).toMap
          val rawElement = actuals.exists(a => isRawGenericUse(a))
          args.zipWithIndex.map { (t, i) =>
            val f = l(i)
            if f == null || !mentionsTypeVarBounded(f, subst.keySet) then t
            else
              substFormal(f, subst) match
                ...
                case Some(ct)
                    if ct != t.tpe && !ct.isInstanceOf[TypeBounds] &&
  ...
  /** A call THROUGH A TYPE VARIABLE whose bound is a RAW generic (`N extends Node`): java sees the callee's members ERASED, so it accepts arguments the un-erased Scala signature rejects. Cast each
    * argument whose DECLARED formal is a type parameter to that parameter as resolved HERE. Gated on the receiver being a type variable, so an ordinary generic call is untouched.
    */
  ...
  private[spoon] def invocation(inv: CtInvocation[?])(using Obligations): Term =
    ...
    Obligations.consult(JS.C(2), originOf(inv))(
      Option.when(
        staticRecv && (inv.getTarget match
          case ta: CtTypeAccess[?] =>
    ...
    // Read off the view this arm has just computed, never re-derived.
    Obligations.consult(JS.G(22), originOf(inv))(Option.when(erasedRecv.isDefined)(()))
    // JS-G29/G30: a variable some FORMAL mentions infers the same in both languages (JS-G29); one
    // no formal mentions resolves to its BOUND in java and `Nothing` in scala (JS-G30)
    val calleeTpNames = execDeclOf(ex).collect { case m: CtMethod[?] => m.getFormalCtTypeParameters.asScala.toList }.getOrElse(Nil).map(_.getSimpleName).toSet
    val constrained   = calleeTpNames.nonEmpty &&
      Option(ex.getExecutableDeclaration).exists(_.getParameters.asScala.exists(p => mentionsTypeVar(p.getType, calleeTpNames)))
    ...
    val args = polyArgsAscribed(ex, argEs, polyArgsUncast(argEs, typeVarReceiverArgs(inv, argEs, knownReceiverArgs(inv, argEs, args0)), o))
    val fun: Term =
      if ex.isConstructor then
        ...
        inv.getTarget match
          ...
          case ta: CtThisAccess[?] if !isOwnThis(ta) =>
            shadowedImplicitCall(inv, mid, o).orElse(outerThis(ta).map(q => Tree.Select(q, mid, NoType, o))).getOrElse(Tree.Ident(mid, NoType, o))
    ...
    val app = ascribeUnconstrainedResult(inv, Tree.Apply(pinTypeArgs(fun, inv, o), args, mid, erasedResult(args, ty(inv)), o), o)
    erasedRecvResult(inv, erasedRecv, app)

  /** The downcast an ERASED RECEIVER's result needs — java pays for calling through the erased view ([[erasedReceiverView]]) with an implicit downcast on the result, which this writes down. Gated on
    * the un-erased result being nameable HERE and actually different.
    */
  private[spoon] def erasedRecvResult(
    inv:  CtInvocation[?],
    recv: Option[(TypeRepr, Map[String, TypeRepr], Map[String, TypeRepr])],
    app:  Term
  ): Term = recv match
    case None                => app
    case Some((_, subst, _)) =>
      val declRet = execDeclOf(inv.getExecutable).collect { case m: CtMethod[?] => m.getType }
  ...
  /** The result type an ERASED ARGUMENT drags with it — recording Spoon's un-erased type would make the TIR assert what the emitted scala does not have. Only the erasure WE introduced is modelled,
    * decided from the EMITTED argument (a JDK shadow's formals are unreliable).
    */
  ...
  /** Java keeps SEPARATE namespaces for variables and methods; Scala does not. `boolean delete = …; cursor = delete(false);` is legal Java — the call still reaches the METHOD — but in Scala the local
    * hides it. Qualify such an implicit-target call with the instance whose class provides the method, exactly what Java resolved. Only for INSTANCE methods in a non-static context.
    */
  private[spoon] def shadowedImplicitCall(inv: CtInvocation[?], mid: SymId, o: Origin): Option[Term] =
    ...
    val shadowed = !inStatic && exec != null &&
      exec.getElements(new spoon.reflect.visitor.filter.TypeFilter(classOf[CtVariable[?]])).asScala.exists(v => !v.isInstanceOf[CtField[?]] && v.getSimpleName == name)
  ...
  /** Java resolves a generic call's type arguments (often by inference); Scala re-infers from the EXPECTED type, which can pick a different `T` (E007). Pin the Java-resolved arguments as an explicit
    * `m[Targs](...)` so inference matches Java and unboxing conversions apply at the result. Conservative: only when every argument is a fully concrete class type.
    */
  private[spoon] def pinTypeArgs(fun: Term, inv: CtInvocation[?], o: Origin): Term =
    if inv.getExecutable.isConstructor then return fun
    val formals = Option(inv.getExecutable.getExecutableDeclaration)
      .collect { case m: CtMethod[?] =>
        m.getFormalCtTypeParameters.size
      }
      .getOrElse(0)
  ...
  /** A method TYPE PARAMETER that appears in NO FORMAL, at a call with no target type either. Java instantiates it at its BOUND; scala instantiates it at `Nothing`, failing a selection on it.
    * [[pinTypeArgs]] declines here (no argument mentions it) — this pins the DECLARATION's answer instead. Four conditions: no formal mentions the variable; no TARGET TYPE; every variable has a REAL
    * bound; the bound mentions no type variable of its own.
    */
  private[spoon] def pinUnconstrainedTypeArgs(fun: Term, inv: CtInvocation[?], o: Origin): Term =
    Option(inv.getExecutable.getExecutableDeclaration).collect { case m: CtMethod[?] => m } match
      ...
      case Some(m)    =>
        ...
        val bounds = fs.map(f => Option(f.getSuperclass).filter(_.getQualifiedName != "java.lang.Object").filterNot(mentionsNamedTypeVar))
  ...
  /** [[pinUnconstrainedTypeArgs]]'s pin at the shape its FOURTH condition declines — an F-BOUND — by ascribing the RESULT instead of instantiating the ARGUMENT: no denotable `X` satisfies an F-bound
    * as a type ARGUMENT, but the ascription supplies the TYPE THE SELECTION READS while the argument still infers `Nothing`. Fires where the raw-fill's bound cannot be written: that pin's first three
    * conditions plus the RESULT is the method's own variable.
    */
  private[spoon] def ascribeUnconstrainedResult(inv: CtInvocation[?], app: Term, o: Origin): Term =
    Option(inv.getExecutable.getExecutableDeclaration).collect { case m: CtMethod[?] => m } match
      ...
      case Some(m)    =>
        ...
        val bound = resultVar.flatMap(tp => fs.find(_.getSimpleName == tp.getSimpleName)).flatMap(f => Option(f.getSuperclass)).filter(_.getQualifiedName != "java.lang.Object")
        // ONLY where the type-argument pin declined, so one seam has one mechanism: a bound
        // with no named variable in it is [[pinUnconstrainedTypeArgs]]'s and is already answered there.
        if bound.isEmpty || !bound.exists(mentionsNamedTypeVar) then app
          ...
          // instantiation, exactly as an argument's own type variable is: `IRichSequence<T>`'s `T`, read
          ...
          val viaRecv: String => Option[CtTypeReference[?]] = n => ownerQ.flatMap(q => if recvT == null then scala.None else actualFor(recvT, q, n, 8))
          wildcardOwnVars(bound.get, names, viaRecv) match
            case Some(t)    => Tree.Typed(app, tt(t, inv), t, o)
            case scala.None => app

  /** the bound, with the METHOD's own variables rendered `?` and every other one required to be writable here. `None` where one is not — see [[ascribeUnconstrainedResult]].
    *
    * The WILDCARD arm comes first for the same reason it always must: Spoon's `CtWildcardReference` EXTENDS `CtTypeParameterReference`, so a variable arm above it claims every `?`.
    */
  private[spoon] def wildcardOwnVars(r: CtTypeReference[?], own: Set[String], viaRecv: String => Option[CtTypeReference[?]]): Option[TypeRepr] = r match
  ...
  /** does this type mention a NAMED type variable — one an F-bound or an enclosing class declares — as opposed to a WILDCARD, which is writable anywhere? `mentionsAnyTypeVar` cannot answer it:
    * Spoon's `CtWildcardReference` EXTENDS `CtTypeParameterReference`, so its generic arm claims every `?` and the wildcard arm below it is dead. `Map<String, ?>` is the bound this pin exists for.
    */
  ...
  /** does this invocation stand as the RECEIVER of another member access — the one position that gives its result no expected type at all, and the one where scala's `Nothing` is then selected from?
    * See [[pinUnconstrainedTypeArgs]].
    */
  ...
  private[spoon] def ctorCall(cc: CtConstructorCall[?])(using Obligations): Term =
    ...
    val args = polyArgsAscribed(
      cc.getExecutable,
      argEs,
      polyArgsUncast(argEs, appliedCtorArgs(cc, argEs, rawCtorArgs(cc, argEs, coerceArgs(cc.getExecutable, argEs, originOf(cc)))), originOf(cc))
    )
    ...
    Obligations.consult(JS.C(17), originOf(cc))(
      Option.when(cc match
        case nc: CtNewClass[?] =>
    ...
    // without a body scala infers the argument from the expected type, and with one the
    // anonymous class's type is fixed, so a raw use gives `Parent[Nothing]` and
    ...
    if boxing then
      ...
      Tree.Apply(Tree.Select(Tree.Ident(wSym, TypeRef(NoPrefix, wSym), originOf(cc)), vSym, NoType, originOf(cc)), args, vSym, t, originOf(cc))
    else Tree.Apply(Tree.New(tt(t, cc), t, originOf(cc), anon), args, cid, t, originOf(cc))

  /** A RAW constructor call — `return new Values(this)` inside `ArrayMap<K,V>`. Java checks a raw `new`'s arguments against the ERASED constructor, so `this: ArrayMap<K,V>` passes unchecked. We
    * render the raw type name-FILLED (`new Values[V](…)`), re-imposing the un-erased formal — so arguments must be filled by the SAME name-directed rule, or the two halves of one raw use disagree.
    * Only for formals that mention a type variable resolving here.
    */
    ...
    if !isRawGenericUse(cc.getType) then args
      ...
      ps match
        case Some(l) if l.sizeIs == args.size && argEs.sizeIs == args.size =>
          ...
          args.zipWithIndex.map { (t, i) =>
            val f = l(i)
            if f == null || !isGenericUse(f) || !mentionsAnyTypeVar(f) || !tpAccessibleHere(f) then specialised.get(i).fold(t)(ct => Tree.Typed(t, tt(ct, argEs(i)), ct, t.origin))
            else
              val ct = tpe(f)
  ...
  /** SPECIALISE the erased arguments of a raw constructor call, rather than erasing the precise ones — java checked none of it (constructor is raw), but SOME instantiation must be chosen, recovered
    * from a precise argument's own supertype chain. Casting the OTHER way (precise argument DOWN to erased) measured worse. Narrow: one class type parameter, one binding found, only arguments AT the
    * erasure touched.
    */
  private[spoon] def rawCtorSpecialisation(
    cc:    CtConstructorCall[?],
    l:     List[CtTypeReference[?]],
    argEs: List[CtExpression[?]],
    args:  List[Term]
  ): Map[Int, TypeRepr] =
    val clsFormals = Option(cc.getType).flatMap(typeDeclarationOf).map(_.getFormalCtTypeParameters.asScala.toList).getOrElse(Nil)
        ...
        if idx < 0 then None
          ...
          def walk(t: CtTypeReference[?], depth: Int): Option[CtTypeReference[?]] =
            if t == null || depth <= 0 then None
              ...
              t.getActualTypeArguments.asScala.toList
                .lift(idx)
                .filterNot(a =>
                  a == null || a.isInstanceOf[CtWildcardReference] ||
  ...
  /** An APPLIED generic constructor call whose argument java unchecked-converted — read through the erased receiver view, scala needs the conversion java made implicitly written out. Target is the
    * declared formal with the class's own parameters substituted by the call's EXPLICIT type arguments (else `uncheckedGeneric` would render `?T`). Raw counterpart is [[rawCtorArgs]]. Gated on the
    * ARGUMENT mentioning a raw generic.
    */
  private[spoon] def appliedCtorArgs(cc: CtConstructorCall[?], argEs: List[CtExpression[?]], args: List[Term]): List[Term] =
    ...
    val formals = Option(cc.getType).flatMap(typeDeclarationOf).map(_.getFormalCtTypeParameters.asScala.toList.map(_.getSimpleName)).getOrElse(Nil)
    val ps      = execDeclOf(cc.getExecutable).map(_.getParameters.asScala.toList.map(_.getType))
        ...
        case Some(l) if l.sizeIs == args.size && argEs.sizeIs == args.size =>
          ...
          args.zipWithIndex.map { (t, i) =>
            ...
            val bad = argEs(i) match
              ...
              case e =>
                polyExpression(e) || e.isInstanceOf[CtLiteral[?]] ||
  ...
  /** SymId of a called executable — via its declaration (keyed identically to how we define our own methods) or, for unresolved externals, by its reference. Under `noClasspath`,
    * `getExecutableDeclaration` can resolve to an UNRELATED same-named method — guarded STRUCTURALLY (declaration's owner must be the receiver's type or a SUPERTYPE); disagreement falls through to
    * interning by the RECEIVER's declaring type.
    */
  ...
  /** Does the resolved declaration's declaring type agree with the reference's declaring type? The owner must be the SAME type or a SUPERTYPE of the reference's declaring type; an unrelated type
    * sharing a name does not agree. Where no declaring type is available on the REFERENCE, the declaration is ACCEPTED — state the refutation, not reject every untyped reference. Uses
    * [[typeDeclarationOf]] as [[declaringStaticType]] does.
    */
  ...
  /** Is `targetQ` a supertype of `start`? BFS over the hierarchy — superclass then superinterfaces — same shape as [[selfAndAncestors]]. Where the hierarchy is unreadable the walk stops at that node
    * and answers `false` (the safe direction: a mis-resolution falls through to the reference branch). No bare `catch` — parents read through [[typeDeclarationOf]] (the one lookup where absence is
    * normal).
    */
        ...
        if q != null && seen.add(q) then
          typeDeclarationOf(r) match
            ...
            case scala.None =>
              ...
              // The safe direction: an unresolvable hierarchy cannot confirm the declaration
              // agrees, so we decline and fall through to the reference branch.
              ()
    found
  ...
  /** the receiver of a STATIC CALL is the member's DECLARING type, not the type the source wrote (java lets a static be named through ANY subclass; scala companions inherit nothing). Read off the
    * SYMBOL'S OWNER, never the written name; re-qualifies for an IN-PROGRAM parent too.
    */
  private[spoon] def staticCallQualifier(ta: CtTypeAccess[?], mid: SymId, at: CtElement): Term =
    ...
    if declaring != SymId.None && minter.fullNameOf(declaring) != written then Tree.Ident(declaring, TypeRef(NoPrefix, declaring), originOf(at))
    else typeTerm(ta, at)
  ...
  private[spoon] def incNarrow(opnd: CtExpression[?], res: Term): Term =
    ...
    then Tree.Typed(res, tt(tpe(ot), opnd), tpe(ot), originOf(opnd))
    else res

  /** The narrowing TYPE for an increment, or `None` — the same predicate as `incNarrow`, returning the type rather than wrapping. Used by the statement arm where the narrowing is carried on
  ...
  private[spoon] def stringify(t: Term, el: CtElement): Term =
    ...
    Tree.Apply(
      Tree.Select(Tree.Ident(strSym, TypeRef(NoPrefix, strSym), originOf(el)), vSym, NoType, originOf(el)),
      List(t),
      vSym,
      TypeRef(NoPrefix, strSym),
      originOf(el)
    )

  /** Java's `==` between REFERENCE types is identity; scala's `==` is `equals` — inside an `equals` implementation that is infinite recursion (151 sites). `eq` is faithful for boxed
    * wrappers/enums/interned Strings too; skipped for `null` or PRIMITIVE operands.
    */
  /** JS-E14's PREDICATE: a NON-`String` left operand is stringified (`String.valueOf(obj)+"s"`); `scala.None` for every other operator.
    */
  ...
  /** JS-E02's PREDICATE: `++`/`--` in either position, which java evaluates to the value BEFORE the update for the postfix forms. `Tree.IncDec` carries the distinction; the emitter is what renders
    * `{ val p = x; x += 1; p }` rather than `{ x += 1; x }`.
    */
  ...
  /** the Scala spelling of a java binary operator, or `scala.None` for a kind this arm does not enumerate. An `Option`, not a defaulted string: `BinaryOperatorKind` is a java enum scalac cannot
    * check, and a fabricated name (`"?" + other`) would silently become a real method call in `binApply`. `INSTANCEOF` (java's twentieth kind) never reaches here; enumerates the other nineteen.
    */
  ...
  /** the MARKER for an operator kind [[opText]] does not enumerate — located, named, and `FrontendBlindSpot` rather than `UnmodelledNodeKind` because the node KIND is dispatched on here and what is
    * missing is one shape of it. The emission gate then refuses to ship the port until it is closed, which is the whole difference between this and a method name nobody can resolve.
    */
  private[spoon] def unknownOp(k: BinaryOperatorKind, at: CtElement, tpe: TypeRepr): Term =
    unlowered(
      at,
      s"binary operator kind '$k' — this arm enumerates java's nineteen and the " +
        "parser produced a twentieth",
      tpe,
      Some(UnportableKind.FrontendBlindSpot)
    )
