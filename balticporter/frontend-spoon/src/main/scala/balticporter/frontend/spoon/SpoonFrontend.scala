...
final class SpoonFrontend(preservedAnnotationPrefixes: List[String] = Nil) extends Frontend:
  ...
  /** Like parse, but isolates conversion failures per file (corpus/coverage runs). The model is still built once over all files.
    */
...
final private class UnitBuilder(sourcePath: String, source: String, preservedAnnotationPrefixes: List[String] = Nil):
  ...
  private def triviaOf(c: CtComment): Trivia =
    ...
    val text =
      if pos != null && pos.isValidPosition && pos.getSourceEnd >= pos.getSourceStart then source.substring(pos.getSourceStart, pos.getSourceEnd + 1)
      else c.toString
    Trivia(kind, text)

  /** every comment handed out as trivia, by identity — deepComments only scoops what no closer harvest point claimed.
    */
  private val claimed: java.util.Set[CtComment] = java.util.Collections.newSetFromMap(new java.util.IdentityHashMap[CtComment, java.lang.Boolean]())

  private def leadingOf(el: CtElement): List[Trivia] =
    el.getComments.asScala.toList.map { c => claimed.add(c); triviaOf(c) }

  /** comments Spoon attached to expression-level descendants (arg lists, fluent chains, initializer exprs). BIR carries no expression trivia, so they hoist to the nearest enclosing harvest point.
    * Call AFTER translating children so nested statements claim their own first.
    */
  private def deepComments(el: CtElement): List[Trivia] =
    el.getElements(new spoon.reflect.visitor.filter.TypeFilter[CtComment](classOf[CtComment])).asScala.toList.filter(claimed.add).map(triviaOf)
  ...
  /** Fill args for a raw type: the type param's non-Object simple upper bound where there is one, else `?`. Self-referential F-bounds (T extends IRichSequence<T>) and type-var-carrying bounds fall
    * back to `?` to avoid unbound references.
    */
  private def rawFill(r: CtTypeReference[?]): List[BType] =
    ...
    scala.util
      ...
      .toOption
      .filter(_.length == rawArity(r))
      .getOrElse(List.fill(rawArity(r))(wild))
  ...
  private def typeDecl(t: CtType[?]): BTypeDecl = t match
    ...
    case c: CtClass[?] =>
      ...
      val (staticBlocks, instanceBlocks) = c.getAnonymousExecutables.asScala.toList.sortBy(posKey).partition(_.hasModifier(ModifierKind.STATIC))
  ...
  /** Static nested types → companion members; inner (non-static) classes → class body (Scala nested classes are inner by default, capturing the outer instance).
    */
  ...
  /** returns (serialVersionUID, instance fields, static fields). Interface fields are implicitly static.
    */
  ...
  private def stmt(s: CtStatement): BStmtK = s match
    ...
    case r: CtReturn[?] =>
      ...
      val retTVar   =
        enclosing.map(_.getType).collect { case tp: CtTypeParameterReference => tp.getSimpleName }.filter(n => enclosing.exists(_.getFormalCtTypeParameters.asScala.exists(_.getSimpleName == n)))
      BStmtK.Return(
        Option(r.getReturnedExpression).map { e =>
          val base = expr(e)
          retTVar match
    ...
    case b: CtBreak =>
      ...
      if owner != null && breakLoops.contains(owner) then BStmtK.LoopBreak(label = if mixedLoops.contains(owner) then Some("break$") else None)
      else
  ...
  /** Registers the loop's break/continue mode. Mixed break+continue nests two boundaries: the outer (break target) names its Label `break$`, so translated breaks use `break()(using break$)` while
    * continues hit the inner unnamed one.
    */
  private def analyzeLoop(loop: CtElement): (Boolean, Boolean) =
    val hasB = loop.getElements(new spoon.reflect.visitor.filter.TypeFilter(classOf[CtBreak])).asScala.exists(b => nearestOwner(b, includeSwitch = true) eq loop)
    val hasC = loop.getElements(new spoon.reflect.visitor.filter.TypeFilter(classOf[CtContinue])).asScala.exists(c => nearestOwner(c, includeSwitch = false) eq loop)
  ...
  /** `for (init; cond; update) body` → `{ init; while (cond) { body; update } }`. A translated `continue` exits the boundary wrapped around body-without-update, so the update still runs — matching
    * Java's continue-jumps-to-update.
    */
  ...
  /** `for (T x : e) body` — array-backed iterables get an index loop, everything else the explicit iterator()/hasNext()/next() desugaring (works for any java.lang.Iterable without needing Scala
    * collection conversions). The iterated expression is hoisted so it evaluates once, as in Java.
    */
          ...
          List(
            ...
            BStmt(
              ...
              BStmtK.While(
                ...
                BStmt(
                  Nil,
                  BStmtK.LocalVar(x, elemT, Some(ArrayAccess(Ident(arr, RefKind.Local), Ident(i, RefKind.Local))), effectivelyFinal = !loopVarMutable)
                )
                  :: body
          ...
          List(
            ...
            BStmt(
              ...
              BStmtK.While(
                ...
                BStmt(
                  Nil,
                  BStmtK.LocalVar(x, elemT, Some(Call(Recv.On(Ident(it, RefKind.Local)), "next", Nil, None, None)), effectivelyFinal = !loopVarMutable)
                )
                  :: body
              )
            )
  ...
  /** Fallthrough-free switch statements → match. Empty colon-cases group with the next case; a non-terminated non-empty case is genuine fallthrough → Unsupported. A missing default becomes
    * `case _ => ()` (Java's silent fall-past).
    */
  private def switchToMatch(s: CtSwitch[?]): BStmtK =
    ...
    val midCaseBreaks = s.getElements(new spoon.reflect.visitor.filter.TypeFilter(classOf[CtBreak])).asScala.exists { b =>
    ...
    cases.zipWithIndex.foreach { (c, idx) =>
      ...
      if split(idx)._1.isEmpty && effStmts(c).isEmpty && !isDefault && !isLast then pendingExprs = pendingExprs ++ exprs
      else
  ...
  /** Any mutation of the named variable inside scope — plain/compound assignment OR increment/decrement (i++ is a write even though Spoon models it as a unary op).
    */
  private def writesToVar(scope: CtElement, name: String): Boolean =
    scope.getElements(new spoon.reflect.visitor.filter.TypeFilter(classOf[CtAssignment[?, ?]])).asScala.exists { a =>
      a.getAssigned match
        case w: CtVariableWrite[?] => w.getVariable.getSimpleName == name
        case _ => false
    }
      || scope.getElements(new spoon.reflect.visitor.filter.TypeFilter(classOf[CtUnaryOperator[?]])).asScala.exists { u =>
        import UnaryOperatorKind.*
  ...
  private def exprNoCasts(e: CtExpression[?]): BExpr = e match
    ...
    case inv: CtInvocation[?] =>
      ...
      val recv = inv.getTarget match
        ...
        case ta: CtTypeAccess[?]  =>
          ...
          val owner =
            if ex != null && ex.isStatic then Option(ex.getDeclaringType).map(_.getQualifiedName).getOrElse(ta.getAccessedType.getQualifiedName)
      ...
      val methodGenericReturn = scala.util
        .Try {
          Option(ex.getExecutableDeclaration).exists { d =>
            ...
            d.asInstanceOf[CtFormalTypeDeclarer].getFormalCtTypeParameters.asScala.exists(_.getSimpleName == d.getType.asInstanceOf[CtTypeParameterReference].getSimpleName)
          }
        }
        .getOrElse(false)
    ...
    case mr: CtExecutableReferenceExpression[?, ?] =>
      ...
      if ex.isConstructor then
        val formals = Option(ex.getExecutableDeclaration).map(_.getParameters.asScala.toList.map(p => btype(p.getType))).getOrElse(unsupported(mr, "constructor reference with unresolvable formals"))
      ...
      mr.getTarget match
        ...
        case ta: CtTypeAccess[?] =>
          val formals =
            Option(ex.getExecutableDeclaration).map(_.getParameters.asScala.toList.map(p => btype(p.getType))).getOrElse(unsupported(mr, "unbound method reference with unresolvable formals"))
  ...
  /** Spoon's getFieldDeclaration can return null for a constant inherited through interfaces/superclasses (BasedSequence.LS where LS is SequenceUtils'), leaving only the ACCESS type. Scala companions
    * don't inherit, so walk the access type's hierarchy — superclass then super-interfaces (sorted for determinism) — to the type that actually DECLARES the field, whose companion object holds the
    * constant.
    */
  ...
  private def fieldAccess(ref: CtFieldReference[?], target: CtExpression[?]): BExpr =
    ...
    val owner = scala.util
      .Try(Option(ref.getFieldDeclaration).flatMap(fd => Option(fd.getDeclaringType)).map(_.getQualifiedName))
      .toOption
      .flatten
    ...
    if ref.getSimpleName == "class" then
      target match
        ...
        case _ => ()
    if ref.getSimpleName == "length" && Option(ref.getDeclaringType).exists(_.isArray) then ArrayLength(expr(target))
  ...
  /** Java performs unchecked conversion when a raw-typed expression flows into a parameterized target — Scala needs the cast made explicit.
    */
  ...
  /** Argument wrapped with its resolved static type — the printer's call-site adaptations (array spread, Object[] casts) need it.
    */
  ...
  private def literal(l: CtLiteral[?]): BExpr =
    ...
    def rawSlice: Option[String] =
      if pos != null && pos.isValidPosition && pos.getSourceEnd >= pos.getSourceStart then Some(source.substring(pos.getSourceStart, pos.getSourceEnd + 1))
      else None
