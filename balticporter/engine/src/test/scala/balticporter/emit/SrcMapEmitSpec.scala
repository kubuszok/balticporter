...
/** The source map is only worth anything if its line ranges are TRUE of the text that was actually written, and only safe to add if turning it on cannot move a byte of that text. Both are asserted
  * here against a hand-built program with a nested class and two same-named overloads — the two shapes that break a naive position table.
  */
class SrcMapEmitSpec extends munit.FunSuite:
  ...
  private val barProgram = new Program(
    List(bar),
    SymbolTable(
      List(
        Symbol(BAR, "Bar", "srcmapdemo.Bar", Flags(), SymId.None, TypeRef(NoPrefix, BAR), origin = Origin("/a/b/src/srcmapdemo/Bar.java", 3, 1)),
        Symbol(INT, "Int", "scala.Int", Flags(), SymId.None, NoType),
  ...
  test("every recorded range names the lines the member was ACTUALLY written to") {
    ...
    es.foreach { e =>
      ...
      if name.nonEmpty && e.kind != "class" then assert(lines(e.start - 1).contains(name), s"line ${e.start} `${lines(e.start - 1)}` does not declare ${e.member}")
    }
  }
