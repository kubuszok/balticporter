...
/** Emits a hand-built TIR `Program` to Scala source and checks the shape. Core-only (no frontend), so it pins the backend's output independently of the populator.
  */
class TirEmitterSpec extends munit.FunSuite:
  ...
  test("a keyword MEMBER name is escaped — already true, and pinned so it stays true") {
    ...
    val get  = Tree.DefDef(
      ...
      paramss = List(Nil),
      returnTpt = tt(tInt),
      rhs = Some(Tree.Block(Nil, Tree.Return(Some(Tree.Select(Tree.This(MFOO, TypeRef(NoPrefix, MFOO), O), MVAL, tInt, O)), tInt, O), tInt, O)),
      origin = O
    )
    val cd   = Tree.ClassDef(MFOO, parents = Nil, selfType = None, body = List(Tree.ValDef(MVAL, tt(tInt), rhs = None, origin = O), get), origin = O)
    val syms = SymbolTable(
      List(
  ...
  test("a Repeated argument flattens into the argument list, and an EMPTY one disappears") {
    ...
    val body = Tree.Block(
      List(
        ...
        call(
          List(
            Tree.Literal(Constant.StringC("%s"), TS, O),
            Tree.Repeated(List(Tree.Literal(Constant.StringC("a"), TS, O), Tree.Literal(Constant.StringC("b"), TS, O)), NoType, O)
          )
        )
      ),
      Tree.Literal(Constant.UnitC, NoType, O),
      NoType,
      O
    )
  ...
  test("a Spread argument renders as the scala spread, and stays ONE argument") {
    ...
    val body = Tree.Block(
      List(
        Tree.Apply(
          Tree.Ident(EXT, NoType, O),
          List(Tree.Literal(Constant.StringC("%s"), TS, O), Tree.Spread(Tree.Ident(ARG, TS, O), TS, O)),
          EXT,
          TS,
          O
        )
    ...
    val d    = Tree.DefDef(M, paramss = List(List(Tree.ValDef(ARG, tt(TS), rhs = None, origin = O))), returnTpt = tt(NoType), rhs = Some(body), origin = O)
    val cd   = Tree.ClassDef(CLS, parents = Nil, selfType = None, body = List(d), origin = O)
    val syms = SymbolTable(
      List(
  ...
  test("an unresolved type variable renders as `?`, never as its marker name") {
    ...
    val bounded = AppliedType(TypeRef(NoPrefix, LIST), List(TypeBounds(NoType, TypeRef(NoPrefix, STUB))))
    val cd      = Tree.ClassDef(CLS, parents = Nil, selfType = None, body = List(Tree.ValDef(FLD, tt(bounded), rhs = None, origin = O)), origin = O)
    val syms    = SymbolTable(
      List(
        Symbol(CLS, "Use", "demo.Use", Flags(), SymId.None, TypeRef(NoPrefix, CLS)),
  ...
  test("a bare unresolved type variable renders `?` rather than its marker") {
    ...
    val cd   = Tree.ClassDef(CLS, parents = Nil, selfType = None, body = List(Tree.ValDef(FLD, tt(t), rhs = None, origin = O)), origin = O)
    val syms = SymbolTable(
      List(
  ...
  private def foreachBody(assignBinding: Boolean): String =
    ...
    val write: List[Statement] =
      if assignBinding then List(Tree.Assign(Tree.Ident(BND, OBJ, O), Tree.Literal(Constant.NullC, OBJ, O), NoType, O))
      else Nil
    val loop = Tree.ForEach(bind, Tree.Ident(ARR, arrT, O), Tree.Block(write, Tree.Ident(BND, OBJ, O), OBJ, O), NoType, O)
    val d    = Tree.DefDef(
      M,
      paramss = List(Nil),
      returnTpt = tt(NoType),
      rhs = Some(Tree.Block(List(loop), Tree.Literal(Constant.UnitC, NoType, O), NoType, O)),
      origin = O
    )
    val cd   = Tree.ClassDef(CLS, parents = Nil, selfType = None, body = List(d), origin = O)
    val syms = SymbolTable(
  ...
  private def k9ForEach(
    ...
    if assignBinding then bodyStmts += Tree.Assign(Tree.Ident(BND, strT, O), Tree.Literal(Constant.NullC, strT, O), NoType, O)
    if withBreak then bodyStmts += Tree.Break(None, NoType, O)
    val bodyTerm = Tree.Ident(BND, strT, O)
    val body     = Tree.Block(bodyStmts.toList, bodyTerm, strT, O)

    val iterable: Term =
      if sideEffectIterable then Tree.Apply(Tree.Select(Tree.This(CLS, TypeRef(NoPrefix, CLS), O), SIDE, listT, O), Nil, SIDE, listT, O)
      else Tree.Ident(LIST, listT, O)
  ...
  private def guardedSwitch(guard: Option[Term]): String =
    ...
    val mtch = Tree.Match(
      scr,
      List(
        Tree.CaseDef(List(Tree.Literal(Constant.IntC(1), tI, O)), guard, Tree.Literal(Constant.UnitC, NoType, O), isDefault = false),
  ...
  test("externalParenless: a listed member is called without parens") {
    ...
    val myMethod  = Tree.DefDef(MY_METHOD, paramss = List(Nil), returnTpt = TypeTree(tStr, O), rhs = Some(body), origin = O)
    val descField = Tree.ValDef(DESC_FIELD, TypeTree(tDesc, O), rhs = None, origin = O)
    val myClass   = Tree.ClassDef(MY_CLASS, parents = Nil, selfType = None, body = List(descField, myMethod), origin = O)
    ...
    assert(clue(withParens).contains("getTestClass()"), "without externalParenless, the call should have parens")

    // WITH externalParenless — should emit getTestClass (no parens)
    val withoutParens = new TirEmitter(prog, externalParenless = Set("org.junit.runner.Description#getTestClass")).emit
    assert(clue(withoutParens).contains(".getTestClass"), "with externalParenless, the call should appear")
    assert(!withoutParens.contains("getTestClass()"), "with externalParenless, the call must NOT have parens")
  }
  ...
  test("externalParenless: an UNLISTED member keeps its parens") {
    ...
    val out = new TirEmitter(prog, externalParenless = Set("org.junit.runner.Description#getTestClass")).emit
    assert(clue(out).contains("getMethodName()"), "unlisted member must keep parens")
  }
