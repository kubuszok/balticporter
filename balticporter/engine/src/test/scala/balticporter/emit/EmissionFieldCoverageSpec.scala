...
class EmissionFieldCoverageSpec extends munit.FunSuite:
  ...
  /** every class file in `balticporter.tir` — the same technique `NodeKindTotalitySpec` uses on Spoon's jar, and for the same reason: a `Class.forName` sweep over a hand-written list is the shape
    * that cannot see a node nobody thought to name. Works from a classes DIRECTORY (sbt) and from a jar (a published artifact), because those are the two ways this code is ever loaded.
    */
  private lazy val tirClasses: List[Class[?]] =
    ...
    val names: List[String] =
      if java.nio.file.Files.isDirectory(root) then
        ...
        try
          zf.entries().asScala.map(_.getName).filter(_.startsWith("balticporter/tir/")).map(n => n.substring(n.lastIndexOf('/') + 1)).toList
        finally zf.close()
    names
  ...
  /** the case's own name — cut at the LAST separator, whichever it is. `Tree$Ident` is `Ident` and the top-level `balticporter.tir.TypeTree` is `TypeTree`; reading only the `$` leaves the whole
    * package path on every `Tree` that is not nested, which is the one node kind most likely to be forgotten.
    */
  ...
  /** the case classes declared inside `object Tree` that are NOT themselves `Tree`s — the aggregates a node carries (`CaseDef`, `CatchCase`, `AnonClass`, `EnumCase`). Their fields are emitted text
    * exactly as a node's are, and perturbing the node's LIST field does not exercise them: dropping a `CaseDef` from `Match.cases` says nothing about whether `CaseDef.guard` is rendered.
    */
  ...
  private lazy val aggregates: Set[String] =
    tirClasses.filter(c => c.getName.startsWith("balticporter.tir.Tree$")).filterNot(c => classOf[Tree].isAssignableFrom(c)).map(simpleName).toSet
  ...
  /** why a field legitimately does not move the emitted text. TWO reasons and no third — a bare string would make this list a suppression list, which is what the audit point asks about.
    */
  private enum Why:
    /** not RENDERED, but a named emitter symbol READS it to select a rendering. The entry cites that symbol, so it dies with the code it points at.
      */
  ...
  final private class Probe[T <: Product](
    node:           T,
    host:           T => String,
    moved:          Seq[(String, T)],
  ...
  private def probe[T <: Product](node: T, host: T => String)(moved: (String, T)*)(notEmitted: (String, Why)*): Probe[T] =
  ...
  private val originIsMetadata =
    Metadata(
      "provenance, never rendered — it reaches `srcmap.tsv` and a finding's location, and " +
  ...
  private val probes: List[Probe[?]] = List(
    ...
    probe(
      Tree.ClassDef(
        ...
        List(tt(tOth)),
        Some(tt(tOth2)),
        List(Tree.ValDef(A, tt(tInt), Some(iLit(1)), O)),
        O,
        tparams = List(Tree.TypeDef(TP, tt(TypeBounds(NoType, NoType)), O)),
        enumCases = Nil,
        leading = List(triv("the class")),
        unitLeading = List(triv("the file"))
      ),
      hostTop
    )(
      "symbol" -> Tree.ClassDef(
        OTHER2,
        List(tt(tOth)),
        Some(tt(tOth2)),
        List(Tree.ValDef(A, tt(tInt), Some(iLit(1)), O)),
    ...
    probe(Tree.ClassDef(ENUMT, Nil, None, Nil, O, enumCases = List(Tree.EnumCase(EC1, Nil, Nil, O), Tree.EnumCase(EC2, Nil, Nil, O))), hostTop)(
    ...
    probe(
      Tree.DefDef(
        RUN,
        List(List(Tree.ValDef(P1, tt(tInt), None, O))),
        tt(tUnit),
        Some(unitLit),
        O,
        tparams = List(Tree.TypeDef(TP, tt(TypeBounds(NoType, NoType)), O)),
        leading = List(triv("doc"))
      ),
      ...
      "tparams" -> Tree.DefDef(RUN, List(List(Tree.ValDef(P1, tt(tInt), None, O))), tt(tUnit), Some(unitLit), O, tparams = Nil, leading = List(triv("doc"))),
      "leading" -> Tree.DefDef(
        RUN,
        List(List(Tree.ValDef(P1, tt(tInt), None, O))),
        tt(tUnit),
    ...
    // ---- ValDef -------------------------------------------------------------------------------
    probe(Tree.ValDef(A, tt(tInt), Some(iLit(1)), O, leading = List(triv("doc"))), (x: Tree.ValDef) => hostMember(x))(
        ...
        hostTop(
          Tree.ClassDef(
            ...
            List(
              ...
              Tree.ClassDef(NESTED, Nil, None, List(Tree.DefDef(NM, List(Nil), tt(tInt), Some(Tree.Select(x, A, tInt, O)), O)), O)
            ),
            O
          )
        )
    ...
    probe(Tree.Super(HOST, tOth, O), (x: Tree.Super) => hostTerm(Tree.Select(x, A, tInt, O)))(
      ...
      "cls" -> Indirect(
        "TirEmitter's `Tree.Super` arm renders the keyword `super`; the class is " +
    ...
    // ---- New / AnonClass ----------------------------------------------------------------------
    probe(Tree.New(tt(tOth), tOth, O, anon = Some(Tree.AnonClass(ANON, List(Tree.ValDef(A, tt(tInt), Some(iLit(1)), O)), O))), hostTerm)(
    ...
    probe(Tree.Apply(Tree.Select(Tree.Ident(OTHER, tOth, O), M1, tInt, O), List(iLit(1)), M1, tInt, O), hostTerm)(
      ...
      "method" -> Indirect(
        "the CALLEE's symbol, read by `TirEmitter.applyStr` (and by the vararg " +
    ...
    // ---- Block --------------------------------------------------------------------------------
    probe(Tree.Block(List(Tree.Assign(refA, iLit(1), tUnit, O)), iLit(7), tInt, O, trailing = List(triv("tail"))), hostTerm)(
    ...
    // ---- Lambda -------------------------------------------------------------------------------
    probe(
      Tree.Lambda(List(Tree.ValDef(P1, tt(tInt), None, O)), Tree.Return(Some(iLit(1)), tInt, O), tOth, O, resultTpt = Some(tt(tInt))),
      hostTerm
    )(
      "params" -> Tree.Lambda(List(Tree.ValDef(P2, tt(tInt), None, O)), Tree.Return(Some(iLit(1)), tInt, O), tOth, O, resultTpt = Some(tt(tInt))),
      "body" -> Tree.Lambda(List(Tree.ValDef(P1, tt(tInt), None, O)), Tree.Return(Some(iLit(2)), tInt, O), tOth, O, resultTpt = Some(tt(tInt))),
      "resultTpt" -> Tree.Lambda(List(Tree.ValDef(P1, tt(tInt), None, O)), Tree.Return(Some(iLit(1)), tInt, O), tOth, O, resultTpt = Some(tt(tStr)))
    )("tpe" -> tpeIsMetadata, "origin" -> originIsMetadata),
    ...
    // ---- Repeated -----------------------------------------------------------------------------
    probe(
      Tree.Repeated(List(iLit(1), iLit(2)), tArrI, O),
    ...
    // ---- While --------------------------------------------------------------------------------
    probe(
      Tree.While(Tree.Literal(Constant.BoolC(true), tBool, O), Tree.Assign(refA, iLit(1), tUnit, O), tUnit, O, label = Some("outer")),
      ...
      "label" -> Tree.While(
        Tree.Literal(Constant.BoolC(true), tBool, O),
        Tree.Block(List(Tree.Break(Some("outer"), tUnit, O)), unitLit, tUnit, O),
        tUnit,
        O,
        label = Some("inner")
      )
    ...
    // ---- ForEach ------------------------------------------------------------------------------
    probe(
      Tree.ForEach(Tree.ValDef(L1, tt(tInt), None, O), refArr, Tree.Assign(refA, iLit(1), tUnit, O), tUnit, O, label = Some("outer")),
      ...
      "iterable" -> Tree.ForEach(
        Tree.ValDef(L1, tt(tInt), None, O),
        Tree.Select(Tree.This(HOST, tOth, O), A, tArrI, O),
    ...
    probe(
      ...
      "init" -> Tree.For(
        List(Tree.ValDef(L2, tt(tInt), Some(iLit(0)), O)),
        Some(Tree.Literal(Constant.BoolC(true), tBool, O)),
        List(Tree.Assign(refA, iLit(1), tUnit, O)),
        Tree.Assign(refB, iLit(1), tUnit, O),
        tUnit,
      ...
      "cond" -> Tree.For(
        List(Tree.ValDef(L1, tt(tInt), Some(iLit(0)), O)),
        Some(Tree.Literal(Constant.BoolC(false), tBool, O)),
        List(Tree.Assign(refA, iLit(1), tUnit, O)),
        Tree.Assign(refB, iLit(1), tUnit, O),
        tUnit,
        O,
    ...
    probe(
      Tree.Try(
        List(Tree.ValDef(L1, tt(tOth), Some(Tree.New(tt(tOth), tOth, O)), O)),
        Tree.Assign(refA, iLit(1), tUnit, O),
        List(Tree.CatchCase(Tree.ValDef(L2, tt(tExc), None, O), Tree.Assign(refB, iLit(1), tUnit, O))),
        Some(Tree.Assign(refA, iLit(2), tUnit, O)),
        tUnit,
        O
      ),
      ...
      "origin" -> originIsMetadata,
      "id" -> Metadata(
        "the identity token `Tree.Try.id` — excluded from `equals`/`hashCode`, " +
    ...
    probe(
      Tree.CatchCase(Tree.ValDef(L2, tt(tExc), None, O), Tree.Assign(refB, iLit(1), tUnit, O)),
      (x: Tree.CatchCase) => hostTerm(Tree.Block(List(Tree.Try(Nil, Tree.Assign(refA, iLit(1), tUnit, O), List(x), None, tUnit, O)), unitLit, tUnit, O))
    )(
    ...
    probe(
      Tree.Match(
        ...
        List(
          ...
          Tree.CaseDef(Nil, None, Tree.Assign(refB, iLit(2), tUnit, O), isDefault = true)
        ),
        tUnit,
        O
      ),
      ...
      "cases" -> Tree.Match(refA, List(Tree.CaseDef(Nil, None, Tree.Assign(refB, iLit(2), tUnit, O), isDefault = true)), tUnit, O)
    )(
      "tpe" -> tpeIsMetadata,
      "origin" -> originIsMetadata,
      "id" -> Metadata(
        "the identity token `Tree.Match.id` — the same contract as `Tree.Try.id`, so " +
    ...
    probe(
      Tree.Match(
        refA,
        List(Tree.CaseDef(Nil, None, Tree.Block(List(Tree.Yield(iLit(1), tInt, O)), iLit(0), tInt, O), isDefault = true)),
        tInt,
        O,
        isExpr = true
      ),
    ...
    probe(
      Tree.CaseDef(List(iLit(1)), Some(Tree.Literal(Constant.BoolC(true), tBool, O)), Tree.Assign(refB, iLit(1), tUnit, O), isDefault = false),
      (x: Tree.CaseDef) => hostTerm(Tree.Block(List(Tree.Match(refA, List(x, Tree.CaseDef(Nil, None, unitLit, isDefault = true)), tUnit, O)), unitLit, tUnit, O))
    )(
      "labels" -> Tree.CaseDef(List(iLit(2)), Some(Tree.Literal(Constant.BoolC(true), tBool, O)), Tree.Assign(refB, iLit(1), tUnit, O), isDefault = false),
      "guard" -> Tree.CaseDef(List(iLit(1)), None, Tree.Assign(refB, iLit(1), tUnit, O), isDefault = false),
      "body" -> Tree.CaseDef(List(iLit(1)), Some(Tree.Literal(Constant.BoolC(true), tBool, O)), Tree.Assign(refB, iLit(9), tUnit, O), isDefault = false),
      "isDefault" -> Tree.CaseDef(List(iLit(1)), Some(Tree.Literal(Constant.BoolC(true), tBool, O)), Tree.Assign(refB, iLit(1), tUnit, O), isDefault = true)
    )(),
    ...
    probe(
      Tree.Yield(iLit(1), tInt, O),
      (x: Tree.Yield) =>
        hostTerm(
          Tree.Block(
            List(Tree.Match(refA, List(Tree.CaseDef(Nil, None, Tree.Block(List(x), iLit(0), tInt, O), isDefault = true)), tInt, O, isExpr = true)),
            unitLit,
            tUnit,
            O
          )
    ...
    probe(
      Tree.TypePattern(L1, tt(tStr), tStr, O),
      (x: Tree.TypePattern) =>
        hostTerm(
          Tree.Block(
            List(
              Tree.Match(
                ...
                List(
                  ...
                  Tree.CaseDef(Nil, None, unitLit, isDefault = true)
                ),
                tUnit,
                O
              )
    ...
    probe(
      Tree.RecordPattern(tt(tStr), List(Tree.BindPattern(L1, tStr, O)), tStr, O),
      (x: Tree.RecordPattern) =>
        hostTerm(
          Tree.Block(
            List(
              Tree.Match(
          ...
          Tree.Block(
            List(
              Tree.Match(
                refA,
                List(
                  Tree.CaseDef(List(Tree.RecordPattern(tt(tStr), List(x), tStr, O)), None, Tree.Assign(refB, iLit(1), tUnit, O), isDefault = false),
                  Tree.CaseDef(Nil, None, unitLit, isDefault = true)
                ),
                tUnit,
                O
    ...
    // ---- Break --------------------------------------------------------------------------------
    probe(
      Tree.Break(None, tUnit, O),
      (x: Tree.Break) =>
        hostTerm(
          Tree.Block(
            List(
              Tree.While(Tree.Literal(Constant.BoolC(true), tBool, O), Tree.Block(List(x), unitLit, tUnit, O), tUnit, O, label = Some("outer"))
            ),
            unitLit,
            tUnit,
            O
    ...
    // ---- Labeled ------------------------------------------------------------------------------
    probe(
      Tree.Labeled("here", Tree.Block(List(Tree.Break(Some("here"), tUnit, O)), unitLit, tUnit, O), tUnit, O),
      ...
      "name" -> Indirect(
        "`TirEmitter.labelNeedsBoundary` (whether a boundary is emitted at all — " +
    ...
    // ---- DoWhile ------------------------------------------------------------------------------
    probe(
      Tree.DoWhile(Tree.Assign(refA, iLit(1), tUnit, O), Tree.Literal(Constant.BoolC(true), tBool, O), tUnit, O, label = Some("outer")),
    ...
    //      act on is.
    probe(Tree.Unportable.open(iLit(1), UnportableKind.RawGenericConversion, None, "a raw generic", tInt, O2), hostTerm)(
      "kind" -> Tree.Unportable.open(iLit(1), UnportableKind.JdkBoundaryFlow, None, "a raw generic", tInt, O2),
      "diff" -> Tree.Unportable.open(
        iLit(1),
        UnportableKind.RawGenericConversion,
        Some(balticporter.catalog.DiffId(balticporter.catalog.Area.G, 2)),
        "a raw generic",
        tInt,
        O2
      ),
      "what" -> Tree.Unportable.open(iLit(1), UnportableKind.RawGenericConversion, None, "something else", tInt, O2),
      "state" -> Tree.Unportable.open(iLit(1), UnportableKind.RawGenericConversion, None, "a raw generic", tInt, O2).resolved("APhase", "rewrote it"),
      ...
      "origin" -> Tree.Unportable.open(iLit(1), UnportableKind.RawGenericConversion, None, "a raw generic", tInt, Origin("Elsewhere.java", 9, 2))
    )("tpe" -> tpeIsMetadata),
    ...
    probe(
      Tree.Unportable.open(iLit(1), UnportableKind.RawGenericConversion, None, "a raw generic", tInt, O2).resolved("APhase", "rewrote it"),
      hostTerm
    )(
      "inner" -> Tree.Unportable.open(iLit(2), UnportableKind.RawGenericConversion, None, "a raw generic", tInt, O2).resolved("APhase", "rewrote it")
    )(),
    ...
    probe(Tree.Opaque("f( " + "0" + " )", tInt, O, holes = List(iLit(1))), hostTerm)(
      ...
      "tpe" -> tpeIsMetadata,
      "origin" -> originIsMetadata,
      "companionMember" -> Indirect(
        "TirEmitterDecls.isStatic and ownStaticNames, plus " +
    ...
    // ---- Commented ----------------------------------------------------------------------------
    probe(
      Tree.Commented(List(triv("note")), Tree.Assign(refA, iLit(1), tUnit, O)),
    ...
    // ---- AnonClass ----------------------------------------------------------------------------
    probe(
      Tree.AnonClass(ANON, List(Tree.ValDef(A, tt(tInt), Some(iLit(1)), O)), O, dropped = Nil),
      ...
      "symbol" -> Indirect(
        "the anonymous type's OWNER symbol, read by `TirEmitter`'s member " +
      ...
      "dropped" -> Indirect(
        "what the FRONTEND could not translate, read by `OmissionCheck` so a " +
          "lost member is reported instead of vanishing — a residue about the input, never output"
      ),
      "sam" -> Indirect(
        "JAVA'S OWN single-abstract-method answer about the target's CLASS FILE " +
    ...
    // ---- EnumCase -----------------------------------------------------------------------------
    probe(
      Tree.EnumCase(EC1, Nil, Nil, O, leading = List(triv("the constant"))),
      ...
      "body" -> Tree.EnumCase(EC1, Nil, List(Tree.DefDef(M1, List(Nil), tt(tInt), Some(iLit(1)), O)), O, leading = List(triv("the constant"))),
  ...
  test("the class-file scan actually found the node set — a silent zero makes every assertion below vacuous") {
    ...
    assertEquals(
      aggregates,
      Set("EnumCase", "AnonClass", "CatchCase", "CaseDef"),
  ...
  test("COVERAGE: every FIELD of every node is either moved by a probe or reasoned about — never both, never neither") {
    ...
    for (owner, ps) <- byOwner do
      ...
      for p <- ps; k <- p.movedKeys ++ p.notEmitted.keySet if !p.fields.contains(k) do phantom += s"$owner.$k"
      for f <- fields do
    ...
    assertEquals(doubled.result().sorted, Nil, "a field cannot both move the emitted text and be excused from doing so")
  }
  ...
  test("every catalog row attaching to the RENDERING dispatch names a Tree kind that exists") {
    ...
    assertEquals(phantom, Nil, s"these rows attach to a `Tree` kind the IR does not have: ${phantom.mkString(", ")}")
  }
  ...
  test("every catalog row attaching to the TYPE dispatch names a `TypeRepr` case that exists") {
    ...
    assertEquals(phantom, Nil, s"these rows attach to a `TypeRepr` case the algebra does not have: ${phantom.mkString(", ")}")
  }
  ...
  test("every `notEmitted` reason is one of the two admissible kinds, and says something") {
    ...
    assertEquals(thin.result().sorted, Nil, "a one-word reason is a suppression with a label on it")
  }
