...
class ResolutionSpec extends munit.FunSuite:
  ...
  test("…and the PLAN narrows through the same predicate, so a widened remedy reaches both callers") {
    ...
    val size      = p.symbols.all.find(_.fullName == "com.demo.Widget#size").get
    assertEquals(plan.selected(size.id, SpecRemedyPhase.Lane, SpecRemedyPhase.Kind).map(_.remedy.id), Some("spec-noop"))
    assertEquals(plan.selected(size.id, SpecRemedyPhase.Lane, "second-kind").map(_.remedy.id), Some("spec-noop"))
  ...
  test("the DRAIN reads what a phase RECORDED, never what the manifest declared") {
    ...
    plan.applied(plan.selected(size.id, SpecRemedyPhase.Lane, SpecRemedyPhase.Kind).get, "com.demo.Widget#size", size.id, here, "applied")
    assert(plan.appliedAt(size.id, SpecRemedyPhase.Lane, SpecRemedyPhase.Kind, here))
  ...
  test("a selection that BOUND and was never applied is its own issue — not `never matched`") {
    ...
    val size = p.symbols.all.find(_.fullName == "com.demo.Widget#size").get
    plan.applied(
      plan.selected(size.id, SpecRemedyPhase.Lane, SpecRemedyPhase.Kind).get,
      "com.demo.Widget#size",
      size.id,
      Origin.synthetic,
      "applied"
    )
  ...
  test("…and the same key with a descriptor binds to exactly one of them") {
    ...
    assertEquals(plan.selected(target.get, SpecRemedyPhase.Lane, SpecRemedyPhase.Kind).map(_.remedy.id), Some("spec-noop"))
  }
  ...
  test("two entries binding ONE declaration on ONE lane is reported — not silently first-wins") {
    ...
    val (plan, _) = planFor(p, Map("com.demo.Widget#size" -> "spec-noop", "com.demo.Widget#size()" -> "spec-other"), vocab, vocab.byId.keySet)
    val issues    = PolicyReport.fromResolutions(plan.troubles).findings
  ...
  test("…and two selections at one member on DIFFERENT LANES are legal, and say nothing") {
    ...
    val vocab     = RemedyVocabulary.declared("spec", List(SpecRemedyPhase.Noop, other))
    val (plan, _) = planFor(p, Map("com.demo.Widget#size" -> "spec-noop", "com.demo.Widget#size()" -> "spec-other"), vocab, vocab.byId.keySet)
    assertEquals(plan.troubles.map(_.issue), List(ResolutionPlan.Issue.NeverApplied, ResolutionPlan.Issue.NeverApplied))
    val size = p.symbols.all.find(_.fullName == "com.demo.Widget#size").get
    assertEquals(plan.selected(size.id, SpecRemedyPhase.Lane, SpecRemedyPhase.Kind).map(_.remedy.id), Some("spec-noop"))
    assertEquals(plan.selected(size.id, "other-lane", SpecRemedyPhase.Kind).map(_.remedy.id), Some("spec-other"))
  }
  ...
  test("…and two on ONE lane with DISJOINT KINDS are legal too — the lane is only half the question") {
    ...
    val size = p.symbols.all.find(_.fullName == "com.demo.Widget#size").get
    assertEquals(plan.selected(size.id, SpecRemedyPhase.Lane, SpecRemedyPhase.Kind).map(_.remedy.id), Some("spec-noop"))
    assertEquals(plan.selected(size.id, SpecRemedyPhase.Lane, "other-kind").map(_.remedy.id), Some("spec-other"))
  }
  ...
  test("…while `AnyKind` and an ALSO-KIND still overlap, and are still reported") {
    ...
    val p = program
    List(
      SpecRemedyPhase.Noop.copy(id = "spec-other", kind = Remedy.AnyKind),
      SpecRemedyPhase.Noop.copy(id = "spec-other", kind = "other-kind", alsoKinds = List(SpecRemedyPhase.Kind))
    ).foreach { other =>
      val vocab     = RemedyVocabulary.declared("spec", List(SpecRemedyPhase.Noop, other))
      val (plan, _) = planFor(p, Map("com.demo.Widget#size" -> "spec-noop", "com.demo.Widget#size()" -> "spec-other"), vocab, vocab.byId.keySet)
      assertEquals(
        plan.troubles.map(_.issue),
        List(ResolutionPlan.Issue.ConflictingSelection, ResolutionPlan.Issue.ConflictingSelection),
  ...
  private val typeRemedy     = SpecRemedyPhase.Noop.copy(id = "spec-type", subject = Remedy.Subject.OwnedType, what = "a residue that is a fact about the TYPE")
  ...
  test("an EXTERNAL-subject remedy binds a callee this program references and does not declare") {
    ...
    assertEquals(plan.selected(target.get, SpecRemedyPhase.Lane, SpecRemedyPhase.Kind).map(_.remedy.id), Some("spec-external"))
  }

  test("…and the same key under an OwnedMember remedy is ExternalOnly, which is the RIGHT refusal there") {
    val p           = program
    val (_, binder) = planFor(p, Map("java.lang.StringBuilder#append" -> "spec-noop"), vocabulary, vocabulary.byId.keySet)
  ...
  private def drainRows(plan: ResolutionPlan, rows: List[Row], menu: List[Remedy] = List(SpecRemedyPhase.Noop)): List[Row] =
    plan.drain(menu, rows)(r => ResolutionPlan.Residue(r.kind, r.at, r.subject, Origin("Widget.java", r.line, 1), s"accepted ${r.subject} at ${r.line}"))
  ...
  test("draining moves the selected rows OUT of the lane and INTO the ledger, by exactly that count") {
    ...
    val rows      = List(
      Row(SpecRemedyPhase.Kind, size.id, "com.demo.Widget#size", 4),
  ...
  test("…and a SIBLING's selection on the same lane is not this caller's to drain") {
    ...
    assertEquals(plan.selected(size.id, SpecRemedyPhase.Lane, SpecRemedyPhase.Kind).map(_.remedy.id), Some("spec-yours"))
  ...
  test("an applied resolution files under `remediation` with kind `resolved`, naming its lane") {
    val a = AppliedResolution(
      Resolution("com.demo.Widget#size", Some(MemberKey.of("com.demo.Widget#size")), SymId.None, SpecRemedyPhase.Noop),
      "com.demo.Widget#size",
      SymId.None,
      Origin("Widget.java", 4, 3),
      "wrapped at the seam"
    )
  ...
  test("the pipeline hands a bound plan to the phase, and the phase's application is recorded") {
    ...
    binder.resolving(ResolutionPlan.of(Map("com.demo.Widget#size" -> "spec-noop"), vocab, vocab.byId.keySet, binder))
