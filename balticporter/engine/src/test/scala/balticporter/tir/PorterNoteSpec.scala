...
class PorterNoteSpec extends munit.FunSuite:
  ...
  test("a CONFIGURED reason names the phase and the key — the two things an agent edits") {
    val note = PorterNote.render(d(Decision.Kind.DroppedType, "p.Json", Map("key" -> "p.Json"), Reason.Configured("substitutions", "p.Json")), "")
  ...
  test("a value containing WHITESPACE is quoted, so the pair list stays parseable") {
    val note = PorterNote.render(
      d(Decision.Kind.RenamedPackage, "p.Foo", Map("from" -> "p.Foo", "to" -> "q.Foo"), Reason.Configured("package-rename", "p -> q")),
      ""
    )
  ...
  test("a kind OUTSIDE `Rendered` produces no note at all — one edit turns a family off") {
    ...
    assertEquals(PorterNote.render(d(Decision.Kind.RetypedSignature, "p.Foo#bar", Map.empty, Reason.Universal("r")), ""), "")
  }
  ...
  test("clean: a decision about an emitted subject, noted, backed, and in the file") {
    assertEquals(NoteCoverageCheck.check(List(renamed), List(printed), Set(sym), List(goodText)), Nil)
  }

  test("MISSING: the run emitted the subject, recorded the decision, and printed no note") {
    val fs = NoteCoverageCheck.check(List(renamed), printed = Nil, emitted = Set(sym), texts = List("p.Foo" -> "class Foo { var style$shadow = 0 }"))
  ...
  test("a decision about a subject this run does NOT emit is covered by decisions.tsv, not by a note") {
    assertEquals(NoteCoverageCheck.check(List(renamed), printed = Nil, emitted = Set.empty, texts = List("p.Foo" -> "class Foo")), Nil)
  }
  ...
  test("a kind outside `Rendered` is never demanded — that is a statement, not a gap") {
    ...
    assertEquals(NoteCoverageCheck.check(List(retyped), printed = Nil, emitted = Set(sym), texts = List("p.Foo" -> "class Foo")), Nil)
  }
  ...
  test("UNBACKED also catches a slug no `Decision.Kind` has — nothing could have derived it") {
    val fs = NoteCoverageCheck.check(Nil, Nil, Set.empty, List("p.Foo" -> "/* porter: invented-by-hand key=x */\nclass Foo"))
    assertEquals(fs.map(_.issue), List(NoteCoverageCheck.Issue.Unbacked))
    assertEquals(fs.head.kind, scala.None)
  }

  test("NOT WRITTEN: the emitter recorded a note the assembled file does not carry") {
    val fs = NoteCoverageCheck.check(List(renamed), List(printed), Set(sym), List("p.Foo" -> "class Foo { var style$shadow = 0 }"))
