...
class DependencyCoverageSpec extends munit.FunSuite:
  ...
  test("the whole time/locale DEPENDENCY family is on the dependency lane and nowhere else") {
    ...
    ).foreach(a => assert(deps.contains(a), s"$a is not on the lane"))
    val unportable = PortabilityCheck.rulesFor(All).map(_.api).toSet
    List("java.time.", "java.util.Locale", "java.security.MessageDigest").foreach(a => assert(!unportable.contains(a), s"$a is reported as an unportability"))
  }
  ...
  test("a verdictOverride moves a row OFF the dependency lane — the third conjunct, structurally") {
    ...
    val mine = Map(time -> Map(Platform.ScalaJs -> Verdict.Shim("our own"), Platform.ScalaNative -> Verdict.Shim("our own")))
    assert(PortabilityCheck.dependencyRulesFor(All).exists(_.at.contains(time)))
    assert(
      !PortabilityCheck.dependencyRulesFor(All, mine).exists(_.at.contains(time)),
  ...
  test("a declared dependency COVERS its requirements, matched on org and name and not on version") {
    ...
    val req = DependencyCheck.Requirement(
      PortabilityCheck.all.find(_.api == "java.time.").get,
      row,
      Map(Platform.ScalaJs -> dep, Platform.ScalaNative -> dep),
      "java.time.Instant",
      Origin.synthetic,
      UsageKind.MemberType,
      SymId.None
    )
  ...
  /** an external TYPE row, as `ExternalUsage.all` produces one: `owner` is None for a type, so the name the provides-set is matched against is `fullName` itself.
    */
  ...
  private def cellOf(
    before:    List[DependencyCheck.Requirement],
    beforeExt: List[ExternalUsage.Row],
    after:     List[DependencyCheck.Requirement],
    afterExt:  List[ExternalUsage.Row],
    dep:       ArtifactDep,
  ...
  test("…and an UNREADABLE jar is a THIRD value, never a `no`") {
    ...
    val d = cellOf(Nil, Nil, Nil, Nil, Wrapper, _ => DependencyCheck.Provides.Unverifiable("no network"))
  ...
  test("the emitted column is a SUPERSET of the old test — a finding can only turn OFF") {
    ...
    List(known(), known("anything.at.All"), (_: ArtifactDep) => DependencyCheck.Provides.Unverifiable("x")).foreach { p =>
      assertEquals(DependencyCheck.unneeded(DependencyCheck.declarations(List(Time), Nil, Nil, silentBefore, Nil, p)), Nil)
    }
  }
  ...
  test("a NESTED class matches at a SEPARATOR and never by prefix") {
    ...
    assertEquals(d.cell, DependencyCheck.Cell.Unused)
    val nested = cellOf(Nil, Nil, Nil, List(typeRow("a.b.Outer$Inner")), Wrapper, known("a.b.Outer", "a.b.Outer$Inner"))
  ...
  test("a MEMBER row is asked about its OWNER — the jar declares types, not members") {
    val member = ExternalUsage.Row(
      SymId.None,
      "org.example.wrapper.Providers#load",
      Some("org.example.wrapper.Providers"),
      "load",
      scala.None,
  ...
  private def cellWithSpliced(after: List[ExternalUsage.Row], spliced: Set[String], provides: ArtifactDep => DependencyCheck.Provides) =
  ...
  test("a `call-site-substitution` ALONE answers the emitted column — the static-utility shape") {
    ...
    assertEquals(cellWithSpliced(Nil, Set.empty, known("org.example.wrapper.Providers")).cell, DependencyCheck.Cell.Unused)
  }
  ...
  test("…and the SPLICED half never answers the ORIGINAL column — the pre-pipeline tree has no text") {
    ...
    val d = cellWithSpliced(Nil, Set("org.example.wrapper.Providers.load"), known("org.example.wrapper.Providers"))
    assertEquals(d.cell, DependencyCheck.Cell.Introduced)
  ...
  test("the dotted runs of a real substitution TEMPLATE, hole markers and all") {
    val tmpl = balticporter.transform.CallSiteSubstitutionTransform.Template.parse("org.example.wrapper.Providers.load({arg0}, {recv}.tag)").toOption.get
    val op   = tmpl.splice(
      Some(Tree.Literal(Constant.NullC, TypeRepr.NoType, Origin.synthetic)),
  ...
  test("the published rows say which coordinates a JVM COMPILE of the emitted code needs") {
    ...
    val named      = cellWithSpliced(Nil, Set("org.example.wrapper.Providers.load"), known("org.example.wrapper.Providers"))
  ...
  test("…and a declaration that covers NOTHING is a policy finding, not silence") {
    ...
    def unneeded(declared: List[ArtifactDep], reqs: List[DependencyCheck.Requirement]) =
      DependencyCheck.unneeded(DependencyCheck.declarations(declared, reqs, Nil, reqs, Nil, known())).map(_._1)
