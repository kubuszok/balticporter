...
class ContextCaptureSpec extends munit.FunSuite:
  ...
  test("the captured type carries the value as a field, applies per constructor, and no clause") {
    val phase        = new GlobalsToImplicitsTransform(holders = List(holder(Map("com.demo.Handle" -> "files.getExternalStoragePath() as externalStoragePath = null"))))
    ...
    assert(
      handle.contains("def apply(n: java.lang.String): Handle = {") && handle.contains("built.externalStoragePath = null"),
      handle
    )
  ...
  test("a WRAPPED capture: the field is the wrapper applied, callers wrap, the default is the wrapper's") {
    ...
    val phase = new GlobalsToImplicitsTransform(
      holders = List(
        holder(Map("com.demo.Handle" -> "files.getExternalStoragePath() as externalStoragePath: com.demo.Opt = com.demo.Opt.empty"))
      )
    )
