...
class ClassTagParamsTransformSpec extends munit.FunSuite:
  ...
  test("the parameter becomes a clause, the body reads the tag, a literal caller names the type argument") {
    ...
    assert(
      clue(pm).contains("def add[T <: java.lang.Object](s: java.util.function.Supplier[T])(using scala.reflect.ClassTag[T]): scala.Unit")
    )
    assert(
      pm.contains(
    ...
    assert(
      phase.policyReport.findings.exists(_.detail.contains("passes a `Class` VALUE")),
