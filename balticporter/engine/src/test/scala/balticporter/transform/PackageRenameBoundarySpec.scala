...
class PackageRenameBoundarySpec extends munit.FunSuite:
  ...
  private def run(allow: Set[String]) =
    val phase        = new PackageRenameTransform(typeRenames = Map("com.demo.Table" -> "elsewhere.util.Table"), allowPackageSplit = allow)
  ...
  test("a declared move renders the moved type's package-private members public, one widening each") {
    ...
    assertEquals(
      declared.sorted,
      List("com.demo.Table", "com.demo.Table#<init>", "com.demo.Table#size", "com.demo.Table#slot", "com.demo.Table#tableSize")
    )
    assert(
      out.linesIterator.exists(l => l.startsWith("class Table") || l.startsWith("final class Table")),
      out.linesIterator.filter(_.contains("class Table")).mkString("\n")
    )
