...
class PortMapFollowSpec extends munit.FunSuite:
  ...
  test("a form-only entry strips `()` at the dependent's call sites without any name change") {
    ...
    val map    = PortMap.Map0(
      "base",
      engine = "test",
      entries = List(
        PortMap.Entry("member", "com.base.Box#width()", "com.base.Box#width", PortMap.Disposition.Renamed, shape = "form=parenless")
      )
    )
  ...
  test("a field and a method sharing a name: the method row follows the method, the field stays") {
    ...
    val map    = PortMap.Map0(
      ...
      engine = "test",
      entries = List(
        PortMap.Entry("member", "com.base.Node#parent", "com.base.Node#parent$field", PortMap.Disposition.Renamed, shape = "name=parent$field"),
        PortMap.Entry("member", "com.base.Node#parent()", "com.base.Node#parent", PortMap.Disposition.Renamed, shape = "form=parenless")
      )
    )
  ...
  test("a renamed AND parenless entry (`isLeaf()` -> `leaf`) strips `()` at the call too") {
    ...
    val map    = PortMap.Map0(
      ...
      engine = "test",
      entries = List(
        PortMap.Entry("member", "com.base.Node#isLeaf()", "com.base.Node#leaf", PortMap.Disposition.Renamed, shape = "form=parenless")
      )
    )
    val out = new TirEmitter(new PortMapTransform(List(map)).run(before)).emit
