...
class UnusedSymbolTransformSpec extends munit.FunSuite:
  ...
  test("UnusedSymbolTransform has correct ordering constraints") {
    ...
    assert(
      clue(phase.runsBefore).contains(SuppressionPhase.Name),
  ...
  test("isSideEffectFree: literal") {
    assert(UnusedSymbolTransform.isSideEffectFreeTerm(Tree.Literal(Constant.IntC(0), TypeRepr.NoType, Origin.synthetic)))
  }

  test("isSideEffectFree: null literal") {
    assert(UnusedSymbolTransform.isSideEffectFreeTerm(Tree.Literal(Constant.NullC, TypeRepr.NoType, Origin.synthetic)))
  }

  test("isSideEffectFree: ident (variable read)") {
    assert(UnusedSymbolTransform.isSideEffectFreeTerm(Tree.Ident(SymId(1), TypeRepr.NoType, Origin.synthetic)))
  }

  test("isSideEffectFree: this.field select") {
    assert(
      UnusedSymbolTransform.isSideEffectFreeTerm(Tree.Select(Tree.This(SymId(1), TypeRepr.NoType, Origin.synthetic), SymId(2), TypeRepr.NoType, Origin.synthetic))
    )
  }

  test("isSideEffectFree: method call is NOT side-effect-free") {
    assert(
      !UnusedSymbolTransform.isSideEffectFreeTerm(
        Tree.Apply(
          Tree.Select(Tree.Ident(SymId(1), TypeRepr.NoType, Origin.synthetic), SymId(2), TypeRepr.NoType, Origin.synthetic),
          Nil,
          SymId(2),
          TypeRepr.NoType,
          Origin.synthetic
  ...
  test("isSideEffectFree: new object is NOT side-effect-free") {
    assert(
      !UnusedSymbolTransform.isSideEffectFreeTerm(Tree.New(TypeTree(TypeRepr.NoType, Origin.synthetic), TypeRepr.NoType, Origin.synthetic))
    )
  }
  ...
  test("RequiredChecks includes unused-symbol lanes") {
    assert(
      PortRun.RequiredChecks.contains(UnusedSymbolTransform.Handled),
      "unused-symbol(handled) must be in RequiredChecks — the phase is unconditional"
    )
    assert(
      PortRun.RequiredChecks.contains(UnusedSymbolTransform.Refused),
      "unused-symbol(refused) must be in RequiredChecks — the phase is unconditional"
    )
  }

  test(
    "refCollector counts Tree.MethodRef references — a private method referenced only through this::visit must not be deleted"
  ) {
    ...
    val symbols = SymbolTable(
      List(
        ...
        Symbol(visitSym, "visit", "pkg.C#visit", Flags(isPrivate = true), classSym, TypeRepr.NoType),
        Symbol(handlerSym, "myHandler", "pkg.C#myHandler", Flags(isPrivate = true), classSym, TypeRepr.NoType),
        Symbol(ctorSym, "<init>", "pkg.C#<init>", Flags(), classSym, TypeRepr.MethodType(Nil, TypeRepr.NoType, false))
      )
    )
  ...
  test("UnusedSymbolHandled is NOT in PorterNote.Rendered") {
    assert(
      !balticporter.tir.PorterNote.Rendered(Decision.Kind.UnusedSymbolHandled),
