...
/** The `retargetRewrites` parameter on [[CollectionsTransform]]: per-retarget member rewrites. Tests the construction-time assertions, fingerprinting, and `MergeablePolicy` composition.
  */
class CollectionsRetargetRewriteSpec extends munit.FunSuite {
  ...
  test("a retargetRewrites key with no matching retarget entry is refused") {
    val ex = intercept[IllegalArgumentException] {
      new CollectionsTransform(retargetRewrites = Map("com.example.Bits" -> Map(("get", 1) -> Rename("apply"))))
    }
  ...
  test("empty retargetRewrites is accepted with or without retarget") {
    ...
    val ct2 = new CollectionsTransform(retarget = Map("com.example.Foo" -> "com.example.Bar"))
  ...
  test("empty retargetRewrites does not change the fingerprint") {
    val base      = new CollectionsTransform(retarget = Map("com.example.Foo" -> "com.example.Bar"))
    val withEmpty = new CollectionsTransform(retarget = Map("com.example.Foo" -> "com.example.Bar"), retargetRewrites = Map.empty)
  ...
  test("mergeWith unions independent retargetRewrites sources") {
    ...
    val dep    = new CollectionsTransform(retarget = Map("com.b.Y" -> "scala.Y"), retargetRewrites = Map("com.b.Y" -> Map(("set", 1) -> Rename("update"))))
  ...
  test("mergeWith refuses same source with different rewrite tables") {
    val base   = new CollectionsTransform(retarget = Map("com.a.X" -> "scala.X"), retargetRewrites = Map("com.a.X" -> Map(("get", 1) -> Rename("apply"))))
    val dep    = new CollectionsTransform(retarget = Map("com.a.X" -> "scala.X"), retargetRewrites = Map("com.a.X" -> Map(("get", 1) -> Rename("head"))))
    val merged = base.mergedWith(dep)
  ...
  test("mergeWith accepts same source with identical rewrite tables") {
    ...
    val base   = new CollectionsTransform(retarget = Map("com.a.X" -> "scala.X"), retargetRewrites = Map("com.a.X" -> table))
    val dep    = new CollectionsTransform(retarget = Map("com.a.X" -> "scala.X"), retargetRewrites = Map("com.a.X" -> table))
    val merged = base.mergedWith(dep)
    assert(clue(merged).isRight)
  }
  ...
  test("Construct changes the fingerprint") {
    ...
    val withConstruct = new CollectionsTransform(
      retarget = Map("com.example.Foo" -> "com.example.Bar"),
      retargetRewrites = Map("com.example.Foo" -> Map(("<init>", 0) -> Construct("com.example.Bar", "apply")))
    )
  ...
  test("different Construct factory methods produce different fingerprints") {
    ...
    val b = new CollectionsTransform(
      retarget = Map("com.example.Foo" -> "com.example.Bar"),
      retargetRewrites = Map("com.example.Foo" -> Map(("<init>", 0) -> Construct("com.example.Bar", "from")))
    )
    assertNotEquals(a.surfaceFingerprint, b.surfaceFingerprint)
  }
  ...
  test("mergeWith unions Construct entries from independent sources") {
    ...
    val dep    = new CollectionsTransform(retarget = Map("com.b.Y" -> "scala.Y"), retargetRewrites = Map("com.b.Y" -> Map(("<init>", 0) -> Construct("scala.Y", "apply"))))
    val merged = base.mergedWith(dep)
    assert(clue(merged).isRight)
    val ct = merged.toOption.get.phase.asInstanceOf[CollectionsTransform]
    assert(ct.retargetRewrites.contains("com.a.X"))
    assert(ct.retargetRewrites.contains("com.b.Y"))
    assertEquals(ct.retargetRewrites("com.b.Y")(("<init>", 0)), Construct("scala.Y", "apply"))
  }
  ...
  test("subjects includes retarget keys but not retargetRewrites separately") {
    val ct = new CollectionsTransform(retarget = Map("com.a.X" -> "scala.X"), retargetRewrites = Map("com.a.X" -> Map(("get", 1) -> Rename("apply"))))
  ...
  test("ForEach toString is readable") {
    assertEquals(ForEach("foreachEntry", 2).toString, "ForEach(foreachEntry,2)")
  }

  test("ForEach changes the fingerprint") {
    val base   = new CollectionsTransform(retarget = Map("com.a.X" -> "scala.X"))
    val withFE = new CollectionsTransform(retarget = Map("com.a.X" -> "scala.X"), retargetRewrites = Map("com.a.X" -> Map(("entries", 0) -> ForEach("foreachEntry", 2))))
    assertNotEquals(base.surfaceFingerprint, withFE.surfaceFingerprint)
  }

  test("ForEach with same values has the same fingerprint") {
    val a = new CollectionsTransform(retarget = Map("com.a.X" -> "scala.X"), retargetRewrites = Map("com.a.X" -> Map(("entries", 0) -> ForEach("foreachEntry", 2))))
    val b = new CollectionsTransform(retarget = Map("com.a.X" -> "scala.X"), retargetRewrites = Map("com.a.X" -> Map(("entries", 0) -> ForEach("foreachEntry", 2))))
    assertEquals(a.surfaceFingerprint, b.surfaceFingerprint)
  }

  test("different ForEach target methods produce different fingerprints") {
    val a = new CollectionsTransform(retarget = Map("com.a.X" -> "scala.X"), retargetRewrites = Map("com.a.X" -> Map(("entries", 0) -> ForEach("foreachEntry", 2))))
    val b = new CollectionsTransform(retarget = Map("com.a.X" -> "scala.X"), retargetRewrites = Map("com.a.X" -> Map(("entries", 0) -> ForEach("foreachKey", 1))))
    assertNotEquals(a.surfaceFingerprint, b.surfaceFingerprint)
  }
  ...
  test("mergeWith unions ForEach entries from independent sources") {
    val base = new CollectionsTransform(
      retarget = Map("com.a.X" -> "scala.X", "com.b.Y" -> "scala.Y"),
      retargetRewrites = Map("com.a.X" -> Map(("entries", 0) -> ForEach("foreachEntry", 2)))
    )
    val dep    = new CollectionsTransform(retarget = Map("com.b.Y" -> "scala.Y"), retargetRewrites = Map("com.b.Y" -> Map(("keys", 0) -> ForEach("foreachKey", 1))))
    val merged = base.mergedWith(dep)
    assert(clue(merged).isRight)
    val ct = merged.toOption.get.phase.asInstanceOf[CollectionsTransform]
    assert(ct.retargetRewrites.contains("com.a.X"))
    assert(ct.retargetRewrites.contains("com.b.Y"))
    assertEquals(ct.retargetRewrites("com.a.X")(("entries", 0)), ForEach("foreachEntry", 2))
    assertEquals(ct.retargetRewrites("com.b.Y")(("keys", 0)), ForEach("foreachKey", 1))
  }
  ...
  test("Construct with dropTrailing=0 has the same fingerprint as Construct without it") {
    ...
    val b = new CollectionsTransform(
      retarget = Map("com.example.Foo" -> "com.example.Bar"),
      retargetRewrites = Map("com.example.Foo" -> Map(("<init>", 0) -> Construct("com.example.Bar", "apply", dropTrailing = 0)))
    )
    assertEquals(a.surfaceFingerprint, b.surfaceFingerprint)
  }
  ...
  test("Construct with dropTrailing>0 changes the fingerprint") {
    val a = new CollectionsTransform(
      retarget = Map("com.example.Foo" -> "com.example.Bar"),
      retargetRewrites = Map("com.example.Foo" -> Map(("<init>", 4) -> Construct("com.example.Bar", "apply")))
    )
    val b = new CollectionsTransform(
      retarget = Map("com.example.Foo" -> "com.example.Bar"),
      retargetRewrites = Map("com.example.Foo" -> Map(("<init>", 4) -> Construct("com.example.Bar", "apply", dropTrailing = 2)))
    )
    assertNotEquals(a.surfaceFingerprint, b.surfaceFingerprint)
  }
  ...
  test("mergeWith unions Construct entries with dropTrailing from independent sources") {
    val base = new CollectionsTransform(
      retarget = Map("com.a.X" -> "scala.X", "com.b.Y" -> "scala.Y"),
      retargetRewrites = Map("com.a.X" -> Map(("<init>", 0) -> Construct("scala.X", "apply")))
    )
    val dep = new CollectionsTransform(
      retarget = Map("com.b.Y" -> "scala.Y"),
      retargetRewrites = Map("com.b.Y" -> Map(("<init>", 4) -> Construct("scala.Y", "apply", dropTrailing = 2)))
    )
    val merged = base.mergedWith(dep)
    assert(clue(merged).isRight)
    val ct = merged.toOption.get.phase.asInstanceOf[CollectionsTransform]
    assertEquals(ct.retargetRewrites("com.b.Y")(("<init>", 4)), Construct("scala.Y", "apply", dropTrailing = 2))
  }

  // ---- Collect ----

  test("Collect toString is readable") {
    assertEquals(Collect("foreachKey", "lowlevel.util.DynamicArray").toString, "Collect(foreachKey,lowlevel.util.DynamicArray)")
  }
  ...
  test("mergeWith unions Chain entries from independent sources") {
    ...
    val b      = new CollectionsTransform(retarget = Map("com.b.Y" -> "scala.Y"), retargetRewrites = Map("com.b.Y" -> Map(("values", 0) -> Chain(List("getValues")))))
    val merged = a.mergedWith(b)
    assert(merged.isRight, s"merge refused: ${merged.left.getOrElse("")}")
    val ct = merged.toOption.get.phase.asInstanceOf[CollectionsTransform]
  ...
  test("two sources sharing a target with DIFFERENT rewrite tables are accepted") {
    ...
    val ct = new CollectionsTransform(
      retarget = Map("com.a.Array" -> "lls.DynamicArray", "com.a.Queue" -> "lls.DynamicArray"),
      retargetRewrites = Map(
        "com.a.Array" -> Map(("get", 1) -> Rename("apply"), ("removeValue", 2) -> BoolDispatch(1, "removeValueByRef", "removeValue")),
        "com.a.Queue" -> Map(("get", 1) -> Rename("apply"), ("addLast", 1) -> Rename("add"))
      )
    )
  ...
  test("two sources sharing a target with IDENTICAL rewrite tables are accepted") {
    ...
    val ct  = new CollectionsTransform(retarget = Map("com.a.X" -> "lls.DA", "com.a.Y" -> "lls.DA"), retargetRewrites = Map("com.a.X" -> tbl, "com.a.Y" -> tbl))
    assertEquals(ct.retargetRewrites.size, 2)
  }
  ...
  test("FieldWrite changes the fingerprint") {
    val a = new CollectionsTransform(retarget = Map("com.a.X" -> "scala.X"), retargetRewrites = Map("com.a.X" -> Map(("size", 0) -> FieldWrite("size", "setSize"))))
    val b = new CollectionsTransform(retarget = Map("com.a.X" -> "scala.X"))
    assertNotEquals(a.surfaceFingerprint, b.surfaceFingerprint)
  }
  ...
  test("FieldWrite with same values has the same fingerprint") {
    val a = new CollectionsTransform(retarget = Map("com.a.X" -> "scala.X"), retargetRewrites = Map("com.a.X" -> Map(("size", 0) -> FieldWrite("size", "setSize"))))
    val b = new CollectionsTransform(retarget = Map("com.a.X" -> "scala.X"), retargetRewrites = Map("com.a.X" -> Map(("size", 0) -> FieldWrite("size", "setSize"))))
    assertEquals(a.surfaceFingerprint, b.surfaceFingerprint)
  }
  ...
  test("FieldWrite can coexist with other variants at the SAME source") {
    ...
    val ct = new CollectionsTransform(
      retarget = Map("com.a.X" -> "scala.X"),
      retargetRewrites = Map("com.a.X" -> Map(("size", 0) -> FieldWrite("size", "setSize"), ("get", 1) -> Rename("apply")))
    )
  ...
  test("mergeWith unions FieldWrite entries from independent sources") {
    val a      = new CollectionsTransform(retarget = Map("com.a.X" -> "scala.X"), retargetRewrites = Map("com.a.X" -> Map(("size", 0) -> FieldWrite("size", "setSize"))))
    val b      = new CollectionsTransform(retarget = Map("com.b.Y" -> "scala.Y"), retargetRewrites = Map("com.b.Y" -> Map(("size", 0) -> FieldWrite("size", "clear"))))
    val merged = a.mergedWith(b)
    assert(merged.isRight, s"merge refused: ${merged.left.getOrElse("")}")
    val ct = merged.toOption.get.phase.asInstanceOf[CollectionsTransform]
  ...
  test("IndexedField changes the fingerprint") {
    val a = new CollectionsTransform(retarget = Map("com.a.X" -> "scala.X"), retargetRewrites = Map("com.a.X" -> Map(("items", 0) -> IndexedField("items"))))
    val b = new CollectionsTransform(retarget = Map("com.a.X" -> "scala.X"))
    assertNotEquals(a.surfaceFingerprint, b.surfaceFingerprint)
  }
  ...
  test("IndexedField with same values has the same fingerprint") {
    val a = new CollectionsTransform(retarget = Map("com.a.X" -> "scala.X"), retargetRewrites = Map("com.a.X" -> Map(("items", 0) -> IndexedField("items"))))
    val b = new CollectionsTransform(retarget = Map("com.a.X" -> "scala.X"), retargetRewrites = Map("com.a.X" -> Map(("items", 0) -> IndexedField("items"))))
    assertEquals(a.surfaceFingerprint, b.surfaceFingerprint)
  }
  ...
  test("IndexedField can coexist with Rename and FieldWrite at the same source") {
    val ct = new CollectionsTransform(
      retarget = Map("com.a.X" -> "scala.X"),
      retargetRewrites = Map(
        "com.a.X" -> Map(("items", 0) -> IndexedField("items"), ("size", 0) -> FieldWrite("size", "setSize"), ("get", 1) -> Rename("apply"))
      )
    )
  ...
  test("mergeWith unions IndexedField entries from independent sources") {
    val a      = new CollectionsTransform(retarget = Map("com.a.X" -> "scala.X"), retargetRewrites = Map("com.a.X" -> Map(("items", 0) -> IndexedField("items"))))
    val b      = new CollectionsTransform(retarget = Map("com.b.Y" -> "scala.Y"), retargetRewrites = Map("com.b.Y" -> Map(("data", 0) -> IndexedField("data"))))
    val merged = a.mergedWith(b)
    assert(merged.isRight, s"merge refused: ${merged.left.getOrElse("")}")
    val ct = merged.toOption.get.phase.asInstanceOf[CollectionsTransform]
  ...
  test("Construct with dropTrailing > 0 fingerprints differently from dropTrailing = 0") {
    val with0 = new CollectionsTransform(
      retarget = Map("com.a.X" -> "scala.X"),
      retargetRewrites = Map("com.a.X" -> Map(("<init>", 3) -> Construct("scala.X", "apply", dropTrailing = 0)))
    )
    val with1 = new CollectionsTransform(
      retarget = Map("com.a.X" -> "scala.X"),
      retargetRewrites = Map("com.a.X" -> Map(("<init>", 3) -> Construct("scala.X", "apply", dropTrailing = 1)))
    )
  ...
  test("Template with $T0 at an <init> entry is accepted") {
    val ct = new CollectionsTransform(
      retarget = Map("com.a.X" -> "scala.X"),
      retargetRewrites = Map("com.a.X" -> Map(("<init>", 1) -> Template("$Target.from[$T0]($0)")))
    )
  ...
  test("Template fingerprint changes with the expression") {
    ...
    val b = new CollectionsTransform(
      retarget = Map("com.a.X" -> "scala.X"),
      retargetRewrites = Map("com.a.X" -> Map(("<init>", 1) -> Template("$Target.apply[$T0]($0)")))
    )
    assertNotEquals(a.surfaceFingerprint, b.surfaceFingerprint)
  }
  ...
  test("Chain at arity 0 is accepted alongside Rename and Template") {
    val ct = new CollectionsTransform(
      retarget = Map("com.a.Bits" -> "scala.collection.mutable.BitSet"),
      retargetRewrites = Map(
        "com.a.Bits" -> Map(
          ("get", 1) -> Rename("apply"),
  ...
  test("Chain at arity 0 changes the fingerprint") {
    val a = new CollectionsTransform(
      retarget = Map("com.a.Bits" -> "scala.BitSet"),
  ...
  test("Template at arity 0 changes the fingerprint") {
    val a = new CollectionsTransform(
      retarget = Map("com.a.Bits" -> "scala.BitSet"),
      retargetRewrites = Map("com.a.Bits" -> Map(("length", 0) -> Template("(if ($recv.isEmpty) 0 else $recv.last + 1)")))
    )
    val b = new CollectionsTransform(retarget = Map("com.a.Bits" -> "scala.BitSet"))
    assertNotEquals(a.surfaceFingerprint, b.surfaceFingerprint)
  }
  ...
  test("mergeWith unions Chain entries from independent retarget sources") {
    ...
    val b = new CollectionsTransform(
      retarget = Map("com.b.Flags" -> "scala.Flags"),
  ...
  test("mergeWith unions Template entries from independent retarget sources") {
    ...
    val b = new CollectionsTransform(
      retarget = Map("com.b.Flags" -> "scala.Flags"),
      retargetRewrites = Map("com.b.Flags" -> Map(("numBits", 0) -> Template("$recv.size")))
    )
    val merged = a.mergedWith(b)
    assert(merged.isRight, s"merge refused: ${merged.left.getOrElse("")}")
    val ct = merged.toOption.get.phase.asInstanceOf[CollectionsTransform]
    assertEquals(ct.retargetRewrites("com.a.Bits")(("length", 0)), Template("(if ($recv.isEmpty) 0 else $recv.last + 1)"))
    assertEquals(ct.retargetRewrites("com.b.Flags")(("numBits", 0)), Template("$recv.size"))
  }
}
