...
class OpaqueExternalCalleeSpec extends munit.FunSuite:
  ...
  test("an opaque argument at an external callee is unwrapped and the seam counted") {
    val phase = new PrimitiveToOpaqueTransform(
      OpaqueSpec(
        fqn = "com.demo.Seconds",
        hints = Set("com.demo.Timer#delta"),
  ...
  test("a formal in a unit this run does not emit is a seam, never a hub the flow crosses") {
    ...
    phase.bindPolicy(new PolicyBinder(before, before.members, RunScope.of(emitted = before.units.map(_.symbol).toSet -- theirs, own = Map.empty)))
  ...
  test("a constant variable is never a seed: the call wraps the read, the `inline val` stays") {
    ...
    val phase  = new PrimitiveToOpaqueTransform(
      OpaqueSpec(
        fqn = "com.demo.Cap",
        hints = Set("com.demo.GL#enable#cap"),
  ...
  test("an enum case's constructor arguments are coerced against the retyped constructor") {
    ...
    val phase  = new PrimitiveToOpaqueTransform(
      OpaqueSpec(
        fqn = "com.demo.Mode",
        hints = Set("com.demo.GL#draw#mode"),
        underlying = OpaqueSpec.Primitive.Int,
        scope = RuleScope.Everywhere(Set.empty)
      )
    )
