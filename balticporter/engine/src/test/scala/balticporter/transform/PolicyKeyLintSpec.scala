...
class PolicyKeyLintSpec extends munit.FunSuite:
  ...
  /** the engine's own `src/main/scala`, written into the test resources by build.sbt so the check does not depend on where the suite is run from (the same device `RuntimeArtifactSpec` uses).
    */
  ...
  final private case class Shape(name: String, hits: String => Boolean, instead: String)

  /** a `$`-interpolation IMMEDIATELY BESIDE a `#` — `s"${owner}#$name"`, `s"$owner#$name"`, `s"$owner#${name}"`. Adjacency is the whole test: prose that merely mentions `owner#member` beside an
    * interpolated value is not building a key, and a `#` used as some other separator (`PortMapTransform`'s fingerprint) is allow-listed by name rather than by weakening this.
    */
  ...
  private val Forbidden: List[Shape] = List(
    ...
    Shape(
      "fullName.startsWith",
      _.contains("fullName.startsWith"),
  ...
  private def sources: List[(String, String)] =
    ...
    Files.list(transformDir).iterator().asScala.toList.filter(_.getFileName.toString.endsWith(".scala")).sortBy(_.getFileName.toString).map(p => p.getFileName.toString -> Files.readString(p))
  ...
  private def codeLines(src: String): List[(Int, String)] =
    src.linesIterator.zipWithIndex.map((l, i) => (i + 1, l)).toList.filterNot { (_, l) =>
