...
class CollectionsFamiliesSpec extends munit.FunSuite {
  ...
  test("a family key that collides with a JDK typeMap entry is refused at construction") {
    val ex = intercept[IllegalArgumentException] {
      new CollectionsTransform(families = Map("java.util.List" -> ("scala.collection.mutable.Buffer", Kind.Seq)))
    }
  ...
  test("a non-empty families parameter adds a families= segment to the fingerprint") {
    val base         = new CollectionsTransform()
    val withFamilies = new CollectionsTransform(families = Map("com.lib.Array" -> ("scala.collection.mutable.ArrayBuffer", Kind.Seq)))
  ...
  test("two instances with the same families have the same fingerprint") {
    val a = new CollectionsTransform(families = Map("com.lib.Array" -> ("scala.collection.mutable.ArrayBuffer", Kind.Seq)))
    val b = new CollectionsTransform(families = Map("com.lib.Array" -> ("scala.collection.mutable.ArrayBuffer", Kind.Seq)))
    assertEquals(a.surfaceFingerprint, b.surfaceFingerprint)
  }

  test("different families produce different fingerprints") {
    val a = new CollectionsTransform(families = Map("com.lib.Array" -> ("scala.collection.mutable.ArrayBuffer", Kind.Seq)))
    val b = new CollectionsTransform(families = Map("com.lib.Array" -> ("scala.collection.mutable.HashMap", Kind.Map)))
  ...
  test("subjects includes retarget source FQNs") {
    val ct = new CollectionsTransform(retarget = Map("com.lib.Foo" -> "com.lib.Bar"))
  ...
  test("mergedWith unions disjoint families") {
    val base   = new CollectionsTransform(families = Map("com.lib.A" -> ("scala.collection.mutable.ArrayBuffer", Kind.Seq)))
    val dep    = new CollectionsTransform(families = Map("com.lib.B" -> ("scala.collection.mutable.HashMap", Kind.Map)))
  ...
  test("mergedWith unions two `Only` scopes — a dependent widens its base's scope onto its own entry") {
  ...
  test("mergedWith refuses same source with different target") {
    val base   = new CollectionsTransform(families = Map("com.lib.A" -> ("scala.collection.mutable.ArrayBuffer", Kind.Seq)))
    val dep    = new CollectionsTransform(families = Map("com.lib.A" -> ("scala.collection.mutable.HashMap", Kind.Map)))
    val result = base.mergedWith(dep)
  ...
  test("mergedWith accepts same source with same target") {
    val base   = new CollectionsTransform(families = Map("com.lib.A" -> ("scala.collection.mutable.ArrayBuffer", Kind.Seq)))
    val dep    = new CollectionsTransform(families = Map("com.lib.A" -> ("scala.collection.mutable.ArrayBuffer", Kind.Seq)))
    val result = base.mergedWith(dep)
    assert(result.isRight, s"merge failed: ${result.left.getOrElse("")}")
  ...
  test("mergedWith refuses a cross-clash between families and retarget") {
    val base   = new CollectionsTransform(families = Map("com.lib.A" -> ("scala.collection.mutable.ArrayBuffer", Kind.Seq)))
    val dep    = new CollectionsTransform(retarget = Map("com.lib.A" -> "scala.collection.mutable.HashMap"))
    val result = base.mergedWith(dep)
  ...
  test("mergedWith unions disjoint retargets") {
    val base   = new CollectionsTransform(retarget = Map("com.lib.A" -> "com.target.A"))
    val dep    = new CollectionsTransform(retarget = Map("com.lib.B" -> "com.target.B"))
    val result = base.mergedWith(dep)
    assert(result.isRight, s"merge failed: ${result.left.getOrElse("")}")
  }
