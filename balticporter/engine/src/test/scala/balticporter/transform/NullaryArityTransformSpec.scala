...
/** [[NullaryArityTransform]] — dropping `()` from a getter-like nullary method, and the four guards that decline.
  */
class NullaryArityTransformSpec extends munit.FunSuite:

  // ---- harness ------------------------------------------------------------------------------

  private case class Ran(before: Program, after: Program, phase: NullaryArityTransform, log: DecisionLog, idioms: IdiomLog):
  ...
  test("idiomKinds is empty at the no-op and NullaryArity once scoped") {
    ...
    assertEquals(new NullaryArityTransform(RuleScope.Only(Set("com.foo"))).idiomKinds, Set(IdiomKind.NullaryArity))
  }

  test("subjects is the declared entries — what SurfaceIntrusion screens") {
    assertEquals(new NullaryArityTransform(RuleScope.Only(Set("com.foo", "com.bar"))).subjects, Set("com.foo", "com.bar"))
  ...
  /** The denominator claim: nothing owned, nilary and value-returning leaves the scan without a verdict — converted or refused, one row each, no third outcome.
    */
  test("every owned nilary value-returning declaration takes exactly one lane row") {
    ...
    assertEquals(clue(rows.map(_.subject).sorted), List("Mix#a", "Mix#bump", "Mix#mode", "Mix#toString"))
  ...
  /** `next()` mutates and returns. Parenless, `c.next` reads as a value access and every reader of the emitted surface would be wrong about what the call DOES — java's own `()` is the only thing
    * distinguishing "read a value" from "do something and answer".
    */
  ...
  test("the refusal names the guard AND says why — every finding's classification obligation") {
    ...
    assert(clue(why).exists(_.contains("assignments")), "the sentence must say what the guard saw, not merely that it declined")
  }
  ...
  /** `java.lang.Object` is above every type whether or not the parse lists it, so a `toString()` override is anchored by a declaration no port may re-arity. Dropped, `o.toString` would still compile
    * — scala's own `toString` is parenless — while the emitted OVERRIDE would no longer match what it overrides.
    */
  ...
  /** An ABSTRACT member has no body to inspect, so it is never getter-like — and its arity is part of its contract (a SAM ascription reads it). The concrete implementor is therefore in a component
    * that cannot move whole, and moving HALF a component breaks the override edge: `def size` does not implement `def size()`.
    */
  ...
  /** …and the same component with BOTH halves concrete and getter-like moves WHOLE. That is what makes the guard above a component test rather than an abstractness test.
    */
  ...
  /** `toArray()` beside `toArray(Class)`: dropping `()` from the nilary one makes `a.toArray(classOf[X])` resolve to the parenless `toArray` APPLIED to the argument, rather than calling the 1-arg
    * overload. The guard fires at the DECLARATION.
    */
  ...
  /** The ABSENCE of a parameterful sibling means no ambiguity: a nilary method on its own is safe to convert. This test verifies the guard does NOT fire when no overload exists.
    */
  ...
  test("mergedWith lets the no-op side defer — an empty dependent inherits the base's scope") {
    ...
    assertEquals(base.mergedWith(dep).toOption.get.phase.asInstanceOf[NullaryArityTransform].arityScope, everywhere)
    assertEquals(dep.mergedWith(base).toOption.get.phase.asInstanceOf[NullaryArityTransform].arityScope, everywhere)
  }
  ...
  test("candidates on a substituted owner type are skipped") {
    ...
    val conv = idioms.all.filter(c => c.kind == IdiomKind.NullaryArity && c.verdict == IdiomVerdict.Converted)
    assertEquals(clue(conv.size), 0, "a substituted owner's members must not be converted")
