...
class CollapseDivergenceSpec extends munit.FunSuite:
  ...
  private def base(module: String, types: List[String], members: List[(String, String)]): (String, PortMap.Map0) =
    val rows =
      types.map(t => PortMap.Entry("type", t, t, PortMap.Disposition.Ported)) ++
        members.map((k, form) => PortMap.Entry("member", k, k, PortMap.Disposition.Ported, shape = Surface.render(Surface.MemberShape(form = form))))
    module -> PortMap.Map0(module, "eng", rows)

  private def log(rows: (String, IdiomVerdict)*): IdiomLog =
    val l = new IdiomLog
    rows.foreach((k, v) => l.record(IdiomCandidate(IdiomKind.BeanCollapse, v, k, s"property via `getW/setW`", Origin.synthetic)))
    l

  /** the GAPS half. Every assertion below is about what disagreed; the DENOMINATOR — how many verdicts were compared at all — has its own cell at the bottom, because `0 gaps` because everything
    * agreed and `0 gaps` because nothing was compared are the same line otherwise.
    */
  private def gapsOf(idioms: IdiomLog, bases: List[(String, PortMap.Map0)], pairs: Map[String, String], t: String => Target): List[Surface.Gap] =
  ...
  test("a base that COLLAPSED and a dependent that REFUSES is FATAL, and the gap names BOTH answers") {
    val gaps = gapsOf(log("p.Base#w" -> IdiomVerdict.Refused("OverriddenBelow", "a subclass overrides it")), List(collapsed), pairs, asVar)
  ...
  test("…and the other direction too — a dependent that COLLAPSES what the base did not") {
    ...
    val gaps  = gapsOf(log("p.Base#w" -> IdiomVerdict.Converted), List(plain), pairs, asVar)
    assertEquals(clue(gaps).size, 1)
  ...
  test("AGREEMENT is silent — in both shapes") {
    assertEquals(gapsOf(log("p.Base#w" -> IdiomVerdict.Converted), List(collapsed), pairs, asVar), Nil)
    val plain = base("base-mod", List("p.Base"), List("p.Base#getW" -> ""))
  ...
  test("a base that emits the TYPE and publishes no member row is UNKNOWN, never 'not collapsed'") {
    ...
    val gaps   = gapsOf(log("p.Base#w" -> IdiomVerdict.Converted), List(silent), pairs, asVar)
    assertEquals(clue(gaps).size, 1)
    ...
    assertEquals(gapsOf(log("p.Base#w" -> IdiomVerdict.Refused("NotRequested", "mine")), List(silent), pairs, asVar), Nil)
  }

  test("a BASE port — no bases, or no pairs — asks nothing at all") {
    assertEquals(gapsOf(log("p.Base#w" -> IdiomVerdict.Converted), Nil, pairs, asVar), Nil)
  ...
  test("…and the SHAPE comes from the port's own `targetOf`, so `val` and `var` are not one answer") {
    ...
    assertEquals(
      gapsOf(log("p.Base#w" -> IdiomVerdict.Converted), List(collapsed), pairs, asVal).size,
      1,
  ...
  test("the DENOMINATOR is published beside the gaps — §3, read at this check") {
    ...
    val agreed = PortRun.collapseDivergence(log("p.Base#w" -> IdiomVerdict.Converted), List(collapsed), pairs, asVar)
    ...
    assertEquals(PortRun.collapseDivergence(log("p.Base#w" -> IdiomVerdict.Converted), Nil, pairs, asVar).checked, 0, "no base")
    assertEquals(
      PortRun.collapseDivergence(log("p.Base#w" -> IdiomVerdict.Converted), List(collapsed), Map.empty, asVar).checked,
      0,
      "no pairs"
    )
    val elsewhere = base("base-mod", List("p.Other"), List("p.Other#x" -> "var"))
    assertEquals(
      PortRun.collapseDivergence(log("p.Base#w" -> IdiomVerdict.Converted), List(elsewhere), pairs, asVar).checked,
      0,
  ...
  test("D15 bug 2: lookup by ACCESSOR name, not property key — a key mismatch silently reads as 'no row'") {
    ...
    val accessorBase  = base("base-mod", List("p.Base"), List("p.Base#getW" -> "var")) // upstream key is the ACCESSOR name
    val accessorPairs = Map("p.Base#w" -> "getW/setW")
    // The idiom log records by PROPERTY key
    val gaps = gapsOf(log("p.Base#w" -> IdiomVerdict.Converted), List(accessorBase), accessorPairs, asVar)
    ...
    val accessorBasePlain = base("base-mod", List("p.Base"), List("p.Base#getW" -> "")) // base did NOT collapse
    val disagree          = gapsOf(log("p.Base#w" -> IdiomVerdict.Converted), List(accessorBasePlain), accessorPairs, asVar)
