...
class PortRunSpec extends munit.FunSuite:
  ...
  test("a test source set lands in src_managed/test/scala with the same mechanics") {
    val (root, src) = fixture()
    val r           = PortRun("demo", root.resolve("port"), SourceSet.Test, FrontendConfig(src, List("com/demo/Widget.java"), Nil), Nil).execute()
  ...
  test("a resolution root is RESOLVED against, never emitted") {
    ...
    val r = PortRun(
      ...
      root.resolve("port"),
      SourceSet.Main,
      FrontendConfig(other, List("com/demo2/Uses.java"), Nil, resolutionRoots = List(src)),
      Nil,
      manifest = Some(PortManifest("base").extendedBy(PortManifest("dependent")))
    ).execute()
  ...
  test("…AND WHEN THE SOURCE ROOT CONTAINS THE RESOLUTION ROOT, which the prefix test got wrong") {
    ...
    val r = PortRun(
      ...
      FrontendConfig(root, List("java2/com/demo2/Uses.java"), Nil, resolutionRoots = List(src)),
      Nil,
      manifest = Some(PortManifest("base").extendedBy(PortManifest("dependent")))
    ).execute()
  ...
  test("a per-TYPE rename moves ONE type, composes with the package rename, and lands in the FILE") {
    val (root, src) = fixture()
    val r           = run(root, src)(_.copy(packageRenames = Map("com.demo" -> "org.port"), typeRenames = Map("com.demo.Widget" -> "Gizmo")))
  ...
  test("dropped-types.tsv carries a per-TYPE rename in BOTH namespaces (§4.56)") {
    ...
    assertEquals(
      Correlate.parseDropped(rep.resolve("run-latest/dropped-types.tsv")),
  ...
  private def mergedRedirects(): PortManifest =
    val base = PortManifest(
      "base",
      surface = List(
        new balticporter.transform.TypeRedirectTransform(Map("com.demo.Widget" -> "com.dep.W2"), Map("com.demo.Widget" -> Map("theirs<T>" -> "y")))
      )
    )
    base.extendedBy(
      PortManifest(
        "dep",
        surface = List(
          new balticporter.transform.TypeRedirectTransform(Map("com.demo.Gadget" -> "com.dep.G2"), Map("com.demo.Gadget" -> Map("mine<T>" -> "x")))
        )
      )
    )
  ...
  test("a typo'd DEPENDENT key on a merged phase surfaces its `policy` finding") {
    ...
    assertEquals(mine.map(f => (f.key, f.issue)), List(("com.demo.Gadget#mine<T>", PolicyIssue.Malformed)))
  ...
  test("…and the BASE's key is withheld from the dependent's report — it is not fixable here") {
    ...
    val r           = run(root, src)(_.copy(manifest = Some(mergedRedirects())))
    assert(
      !clue(r.report.policy.render).contains("theirs<T>"),
  ...
  private def refusedPair(): PortManifest =
    PortManifest(
      "base",
      surface = List(new balticporter.transform.ClassTableTransform(Map("com.demo.Widget#of" -> "com.demo.Widget#classFor")))
    ).extendedBy(
      PortManifest(
        "dep",
        surface = List(new balticporter.transform.ClassTableTransform(Map("com.demo.Widget#of" -> "com.demo.Gadget#classFor")))
      )
    )
  ...
  test("NEGATIVE: two EQUAL instances COLLAPSE TO ONE, and the run is green") {
    ...
    val m           =
      PortManifest("base", surface = List(new balticporter.transform.ClassTableTransform(table))).extendedBy(PortManifest("dep", surface = List(new balticporter.transform.ClassTableTransform(table))))
  ...
  private def claimingRun(root: Path, src: Path, other: Path, phases: List[Phase]) =
    PortRun(
      ...
      root.resolve("port"),
      SourceSet.Main,
      FrontendConfig(other, List("com/demo/WidgetTest.java"), Nil, resolutionRoots = List(src)),
      Nil,
      manifest = Some(PortManifest("basemod", governs = Set("com.demo")).extendedBy(PortManifest("dependent", surface = phases)))
    )
  ...
  test("a dependent's key at an FQN the base's published map does NOT emit is ADMITTED") {
    ...
    val r = withReport(rep)(
      claimingRun(
        ...
        src,
        other,
        List(new balticporter.transform.TypeRedirectTransform(Map("com.demo.WidgetTest" -> "com.dep.WidgetTest")))
      ).execute()
    )
  ...
  test("…and one at an FQN it DOES emit is refused, before any phase runs") {
    ...
    val err = intercept[RuntimeException] {
      withReport(rep)(
        claimingRun(root, src, other, List(new balticporter.transform.TypeRedirectTransform(Map("com.demo.Widget" -> "com.dep.Widget")))).execute()
      )
    }
  ...
  test("a signature change re-keys the dependent unit, a body change does not (early cutoff)") {
    ...
    val program     = PortRun(
      ...
      root.resolve("port"),
      SourceSet.Main,
      FrontendConfig(src, List("com/demo/Widget.java", "com/demo/Gadget.java"), Nil),
      Nil
    ).execute().program
  ...
  /** Turn the artifact layer on, into a directory of this test's own. `CheckReport` is gated on process-global flags, so this is set and restored around one run.
    */
  ...
  test("every RequiredCheck reaches findings.tsv — a number on stdout that is not persisted fails the run") {
    ...
    val counts = Files.readAllLines(rep.resolve("run-latest/counts.tsv")).toArray(Array.empty[String]).toList.filterNot(_.startsWith("#")).flatMap(_.split('\t').headOption).toSet
  ...
  test("\u2026and a run that asks NOTHING still names the check \u2014 a zero is not an absence") {
    ...
    val counts = Files.readAllLines(rep.resolve("run-latest/counts.tsv")).toArray(Array.empty[String]).toList.filterNot(_.startsWith("#")).map(_.split('\t').toList)
  ...
  test("correlation runs IN-PROCESS against this run's own report directory") {
    ...
    val result      = withReport(rep) {
      ...
      PortRun("demo", root.resolve("port"), SourceSet.Main, FrontendConfig(src, Nil, Nil), Nil).correlate(scalac = Some(log))
  ...
  test("with the artifact layer OFF, correlation creates NO directory — it is a write like any other") {
    ...
    val out = PortRun("demo", root.resolve("port"), SourceSet.Main, FrontendConfig(src, Nil, Nil), Nil).correlate(scalac = Some(log))
  ...
  test("omissions are checked over EMITTED units only — a dropped type's findings are not this port's") {
    ...
    val r = run(root, src, files = List("com/demo/Par.java", "com/demo/Kept.java", "com/demo/Gone.java")) { p =>
  ...
  test("a DROPPED NILARY CONSTRUCTOR carries a porter note in the body it is missing from (C11)") {
    ...
    assertEquals(r.report.omissions.filter(_.what == "nilary constructor dropped").map(_.owner), List("com.demo.Font"))
  }
  ...
  final private class MintUnit(fqn: String) extends Phase:
    ...
    override def run(program: Program): Program =
      ...
      val sym = Symbol(id, fqn.substring(fqn.lastIndexOf('.') + 1), fqn, Flags(isModule = true), SymId.None, TypeRepr.NoType)
      program.rebuilt(program.units :+ Tree.ClassDef(id, Nil, scala.None, Nil, Origin.synthetic), SymbolTable(program.symbols.all.toList :+ sym))

  /** publish a base's map claiming one EMITTED type, where `PortMap.discover` will find it. */
  private def publishBase(reportRoot: Path, module: String, emits: List[String], dropped: List[String] = Nil): Unit =
    val entries =
    ...
    PortMap.write(reportRoot.resolve(module).resolve("run-latest"), PortMap.Map0(module, EngineInfo.fingerprint, entries))
  ...
  test("NEGATIVE: a synthesised unit the base does NOT emit is written, and the run is green") {
    ...
    val r = withReport(rep)(dependentRun(root, src, other, "basemod", List(new MintUnit("com.demo.Handle"))).execute())
  ...
  test("the refusal is a pure function of what this run would write and what its bases published") {
    ...
    assertEquals(
      claims(List(PortMap.Entry("type", mintedFqn, mintedFqn, PortMap.Disposition.Ported))).map(c => c.fqn -> c.base),
      List(mintedFqn -> "basemod")
    )
  ...
  test("…and a selection nothing applies is a classified POLICY finding, never silence") {
    ...
    val (root, src) = fixture()
    val inert       = new balticporter.tir.SpecRemedyPhase(remedy = balticporter.tir.SpecRemedyPhase.Noop.copy(lane = "a-lane-nobody-asks-about"))
    val r           = PortRun(
      label = "demo",
      portRoot = root.resolve("port"),
      sourceSet = SourceSet.Main,
