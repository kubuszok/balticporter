...
class ManifestAgreementSpec extends munit.FunSuite:
  ...
  ) = PortMap.of(module, "eng", emitted, SrcMap.Recording(Nil), dropTypes, dropMethods, injected, Set.empty, renames)
  ...
  /** The static layer's `InheritedKeyNeverFired` is dropped throughout: it is a property of the `Substitutions` tally, which nothing populates when the check is driven by hand, and every one of these
    * tests is about the DYNAMIC layer. Keeping it would make each assertion carry a constant that says nothing about what is being tested.
    */
  private def run(shared: List[SharedType], ports: List[BasePort]) =
    ManifestAgreement.check(Some(dependent), shared, foreignRoots = true, ports).filterNot(_.kind == Kind.InheritedKeyNeverFired)
  ...
  test("with a fresh map the shared surface agrees, and nothing is reported") {
    ...
    val fs = run(
      List(SharedType("up.Kept", "up.Kept", substituted = false), SharedType("up.Gone", "up.Gone", substituted = true)),
  ...
  test("a SUBSTITUTED type and a DROPPED type both oblige the dependent to tag it") {
    ...
    val fs = run(
      List(SharedType("up.Gone", "up.Gone", substituted = false), SharedType("up.Swapped", "up.Swapped", substituted = false)),
      List(BasePort(base, Some(m), "run-latest"))
    )
  ...
  test("the emitted NAME is compared against what the base wrote, not against its rename map") {
    ...
    assertEquals(kinds(ManifestAgreement.check(Some(dep), sh, true, List(BasePort(renaming)))), List(Kind.BaseMapMissing))
  ...
  test("an EMPTY base manifest is not asked for a map — that is how a non-ported root is declared") {
    ...
    val fs       = ManifestAgreement.check(Some(dep), List(SharedType("third.party.T", "third.party.T", substituted = false)), true, List(BasePort(vendored)))
    assertEquals(clue(fs).map(_.render), Nil)
  }
  ...
  private def statics(m: PortManifest) =
    ManifestAgreement.check(Some(m), Nil, foreignRoots = true, Nil).filterNot(_.kind == Kind.InheritedKeyNeverFired)
  ...
  test("a dependent that moves a type INSIDE the base's namespace the base leaves alone is caught") {
    val dep = PortManifest(name = "dep", subPackages = Map("up.Extra" -> "internal")).mirroring(PortManifest(name = "base", governs = Set("up")))
    val fs  = statics(dep).filter(_.kind == Kind.TypeRenameDivergence)
  ...
  test("a DECLARED boundary split is half of the rename: inheriting one and not the other is caught") {
    val split = PortManifest(name = "base", governs = Set("up"), typeRenames = Map("up.A" -> "other.A"), allowPackageSplit = Set("up.A"))
  ...
  test("the name a base gives a shared type includes its per-TYPE moves, not only its packages") {
    val b   = PortManifest(name = "base", governs = Set("up"), packageRenames = Map("up" -> "sge"), typeRenames = Map("up.Map" -> "MapFilter"))
  ...
  private def targeted(baseT: Set[balticporter.catalog.Platform], depT: Set[balticporter.catalog.Platform]) =
  ...
  test("a dependent may target FEWER platforms than its base, and equally many") {
    ...
    assertEquals(ManifestAgreement.check(Some(plain), Nil, true, Nil).filter(_.kind == Kind.TargetWidening), Nil)
  }
  ...
  test("a TYPE-RENAMED type is found by EMITTED name when the upstream lookup misses (D16)") {
    ...
    assertEquals(
      clue(fs).filter(_.kind == Kind.BaseSurfaceAbsent),
      Nil,
