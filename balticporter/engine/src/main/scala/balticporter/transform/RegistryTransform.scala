...
final class RegistryTransform(
  ...
  /** members of an injected facade that THROWS on every reflective path: counted, never rewritten, so the port's largest silent run-time refusal has a number. Empty = 0 rows.
    */
  ...
  def bindPolicy(binder: PolicyBinder): Unit =
    ...
    boundCallees = entries.zipWithIndex.map { (e, i) =>
      i -> binder.bindCallee(Name, s"Registry(${e.callee}).callee", e.callee, Ownership.Either).toOption.flatMap(_.sym).toSet
    }.toMap
    boundSeeds = entries.zipWithIndex.map { (e, i) =>
      i -> e.seeds.flatMap(s => binder.bindType(Name, s"Registry(${e.callee}).seeds", s, Ownership.Either).toOption)
    }.toMap
    boundHandles = entries.zipWithIndex.map { (e, i) =>
      i -> e.handles.toList.sorted.flatMap(h => binder.bindType(Name, s"Registry(${e.callee}).handles", h, Ownership.Either).toOption).toSet
    }.toMap
    entries.foreach {
    ...
    facadeCallees = facadeMembers.toList.sorted.flatMap(m => binder.bindMembers(Name, "RegistryTransform(facadeMembers)", m, Ownership.Either).toOption.getOrElse(Nil).flatMap(_.sym)).toSet
  ...
  /** Independent callees UNION; the same callee, or the same placement SLOT, with a different entry REFUSES — two registries at one declaration is a conflict only a human can resolve.
    */
    ...
    case o: RegistryTransform =>
      ...
      val conflicts = o.entries.sortBy(_.callee).flatMap { e =>
        mine.get(e.callee).filter(_ != e).map(_ => s"${e.callee}: a different registry entry").toList ++
          slots.get(Placement.slot(e.placement)).filter(_ != e).map(_ => s"${Placement.slot(e.placement)}: already carries a registry").toList
      }
      if conflicts.nonEmpty then Left(conflicts.distinct.mkString("; "))
      else
        Right(
          MergeablePolicy.Merged(
            new RegistryTransform(entries ++ o.entries.filterNot(e => mine.contains(e.callee)), facadeMembers ++ o.facadeMembers),
  ...
  def subjects: Set[String] =
    (entries.map(_.callee) ++ entries.map(e => Placement.owner(e.placement)) ++ facadeMembers.toList).map(MergeablePolicy.subjectOf).toSet
  ...
  /** call ORIGINS this run classified as rewritable — the site key, since a rebuilt tree is a new node and a call's `(path, line, col)` is what stays put.
    */
  ...
  override def run(program: Program): Program =
        ...
        StandardTraversal.scanClassDef(u, ()) {
          case (_, t: Tree.Apply) if facadeCallees(t.method) =>
            found += RegistryCheck.Finding(
              RegistryCheck.Issue.Facade,
              owner,
              program.symbolOf(t.method).map(_.fullName).getOrElse("<facade member>") +
                " throws on every reflective path; no registry replaces it",
              t.origin,
              u.symbol
            )
          case (_, _) => ()
    ...
    here.map(s => s.entryIx -> s.entry).distinct.filter((_, e) => e.miss.isInstanceOf[Miss.JvmReflect]).sortBy(_._1).foreach { (i, e) =>
      offJvm.foreach(t =>
        found += RegistryCheck.Finding(
          RegistryCheck.Issue.JvmOnlyMiss,
          slotOf(i),
          s"`${e.callee}` resolves through the JVM reflective miss arm; on $t it answers the " +
            "miss value for every type no `seeds` entry and no consumer registered",
          Origin.synthetic
        )
      )
    }
    ...
    rewritable.map(s => s.entryIx -> s.entry).distinct.sortBy(_._1).foreach { (i, e) =>
      ...
      val ownerSym =
        if isObject then mint(ownerFqn.substring(ownerFqn.lastIndexOf('.') + 1), ownerFqn, SymId.None, Flags(isModule = true))
      ...
      if ownerSym != SymId.None then
        ...
        val calleeInfo = boundCallees(i).toList.sortBy(_.raw).flatMap(program.symbolOf).map(_.info).headOption.getOrElse(NoType)
  ...
  private def admits(program: Program, s: Site, record: Boolean): Boolean =
    ...
    def refuse(i: RegistryCheck.Issue, why: String): Boolean =
      ...
      false
    if !e.scope.includes(subject) then refuse(RegistryCheck.Issue.OutOfScope, s"`${e.callee}` is called from `$subject`, which this entry's scope does not name")
    else
      s.call.args match
        case List(arg) =>
          if namedAt(program, arg, "forName") then
            refuse(
              RegistryCheck.Issue.ByName,
          ...
          else if !isClassValue(program, arg.tpe) then refuse(RegistryCheck.Issue.NonClassArg, "the argument's type is not `java.lang.Class[…]`, so there is no key to look up")
          else true
        case other =>
          refuse(
            RegistryCheck.Issue.NonClassArg,
  ...
  /** `callee(c)` → `<registry>.create(c)`, same argument and same result type, so every phase after this one and every check read the call exactly as they read the java one.
    */
  ...
  /** A `try` whose ONLY reason to exist was the callee this phase retired — every `catch` names a type the entry DECLARES as that callee's failure — is replaced by its body: the thrower is gone, and
    * the entry's `miss` is what the port says a failed creation answers. Every other `try` around a rewritten call is left as java wrote it and COUNTED.
    */
  ...
  /** the ONE rewritten `create` call this term contains — its entry and its site — if that is all it holds.
    */
  private def holdsRewrittenCall(t: Term)(using Program): Option[(Int, Origin)] =
    if created.isEmpty then scala.None
    else
      StandardTraversal
        .scanTerm(t, List.empty[(Int, Origin)]) {
  ...
  private def memberSources(e: Registry, sp: Spelling, isObject: Boolean, seeds: List[SymId]): List[Statement] =
    ...
    val seeded = seeds.map { s =>
      ...
      Tree.Opaque(
        s"locally { ${sp.register}(${Tree.Opaque.hole(0)}, () => ${Tree.Opaque.hole(1)}) }",
        NoType,
        o,
        List(Tree.Literal(Constant.ClassOfC(ref), NoType, o), Tree.New(TypeTree(ref, o), ref, o))
      )
    }
    List(Tree.Opaque(table, NoType, o), Tree.Opaque(register, NoType, o), Tree.Opaque(create, NoType, o)) ++ seeded
...
object RegistryTransform:
  ...
  final private[transform] case class Site(entryIx: Int, entry: Registry, call: Tree.Apply, unit: SymId)

  /** what a bound callee rewrites to. */
  final private[transform] case class Rewritten(owner: SymId, create: SymId, entry: Int)
