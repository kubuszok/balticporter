...
object CollectionClosureCheck:
  ...
  /** one referenced JDK collection type the mapping does not cover, at one usage site.
    *
    * `coveredBy` is the NEAREST mapped supertype, not merely some mapped ancestor: it is what a reader needs to pick the target, since the faithful target for a subtype is a scala subtype of whatever
    * the supertype already maps to.
    */
  ...
  /** every JDK type this table knows about at all — keys and targets alike. The targets matter: `java.util.Dictionary` is named only as `Hashtable`'s parent and is still a type the family contains.
    * Used by [[CollectionBoundaryCheck]] to decide which side of a slot is JDK.
    */
  val jdkFamily: Set[String] = jdkSupertypes.keySet ++ jdkSupertypes.values.flatten

  /** transitive supertypes, NEAREST FIRST (breadth-first over the declared edges). Order is load-bearing: the finding names the nearest mapped ancestor, which is what tells a reader which target
    * preserves the relation — `LinkedHashSet`'s answer is `HashSet`, not `Iterable`.
    */
    ...
    program.symbols.all.toList.flatMap { sym =>
      if mapped.contains(sym.fullName) then Nil
        ...
        supertypesOf(sym.fullName).find(mapped.contains) match
          case None    => Nil
          case Some(s) =>
            program.usages(sym.id).filter(u => owned(u.enclosing)).map(u => Finding(sym.fullName, s, target(s), u.site.origin, u.kind, u.enclosing))
    }

  /** every symbol whose top-level owner is one of `units`. Fuel-bounded, and a symbol that cannot be rooted counts as NOT owned — attributing another module's finding is the defect this exists to
    * prevent, so the failure direction is to say nothing.
    */
  ...
  /** grouped one-line summary, most-referenced first — the shape `PortabilityCheck.summary` has, for the same reason: a per-site dump is what `findings.tsv` is for.
    */
  def summary(fs: List[Finding]): String =
    if fs.isEmpty then "  none"
    else
      fs.groupBy(f => (f.tpe, f.coveredBy, f.mapsTo))
        .toList
        .sortBy((k, v) => (-v.size, k._1))
