    ...
    case d: DiscriminatedUnionTransform =>
      (scope, d.scope) match
        case (RuleScope.Only(a), RuleScope.Only(b)) =>
          val merged = a ++ b
          Right(
            MergeablePolicy.Merged(DiscriminatedUnionTransform(RuleScope.Only(merged), mutableFields || d.mutableFields, discriminatorField), merged)
          )
  ...
  override def run(program: Program): Program =
    ...
    for (aliasSymId, branches) <- candidates do
      ...
      val discriminatorSym = Symbol(
        ...
        flags = Flags(isAbstract = true),
        owner = traitId,
        info = TypeRepr.TypeRef(TypeRepr.NoPrefix, SymId.None),
        origin = origin
      )
      syms = syms.updated(discriminatorSym)

      val traitDiscriminator = ValDef(discriminatorId, TypeTree(TypeRepr.TypeRef(TypeRepr.NoPrefix, SymId.None), origin), None, origin)
      ...
      val traitDef = ClassDef(traitId, Nil, None, traitDiscriminator :: caseClasses, origin)
      newUnits += traitDef
    ...
    program.rebuilt(units = rewrittenUnits ++ newUnits.toList, symbols = syms)

  private case class UnionBranch(discriminatorValue: String, fields: List[(String, TypeRepr)])
