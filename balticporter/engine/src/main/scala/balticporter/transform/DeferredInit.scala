...
final class DeferredInit(
  ...
  /** `scala.<op>#unary_!` is the convention the frontend's own operator lowering uses for the emitter to render a call as an operator.
    */
  ...
  private def deferField(v: Tree.ValDef, d: ContextNeed.Deferral)(using Program): List[Statement] =
    ...
    List(
      ...
      Tree.DefDef(
        v.symbol,
        List(List(mint.usingParam(v.symbol, holder.context.fqn, ctxRef, at))),
        TypeTree(v.tpt.tpe, at),
        Some(body),
        at,
        leading = v.leading
      )
  ...
  /** The class initialiser keeps everything the deferral did not move; dropped rather than emitted empty if nothing remains.
    */
  ...
  /** The static flags are reused verbatim (the symbol stays a static, landing in the companion where java put it), which is what keeps every read in the program naming the same thing.
    */
