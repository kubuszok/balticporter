...
final class MethodBodyTransform(val bodies: Map[String, String] = Map.empty) extends Phase, PolicySource, SurfacePolicy, MergeablePolicy, PolicyBound:
  ...
  def bindPolicy(binder: PolicyBinder): Unit =
    bound = bodies.keys.toList.sorted.map(k => k -> binder.bindMembers(name, "MethodBodyTransform", k)).toMap
    records = binder.recordsFor(name)
    val (bare, precise) = bound.toList.sortBy(_._1).partition((k, _) => MemberKey.parse(k).toOption.exists(_.isBare))
    bySym = (bare ++ precise).flatMap((k, b) => b.toOption.getOrElse(Nil).flatMap(_.sym).map(_ -> k)).toMap

  /** A body is not a signature, so two modules replacing different bodies don't disagree about the shared surface — but keys are fingerprinted anyway, including body text, since a base and a
    * dependent supplying different Scala for one member is a policy mistake worth surfacing.
    */
  def surfaceFingerprint: String =
    bodies.toList.sorted.map((k, v) => s"$k=${v.hashCode.toHexString}").mkString(",")

  /** Independent keys union; same key with a different body refuses (a conflict only a human can resolve). Same key, same body is accepted silently — the same decision stated twice, which
    * `base.extendedBy(…)` can do legitimately. Lets a dependent's own instance merge with the base's via `surfaceFold` instead of a fatal `SurfaceDivergence`.
    */
  ...
  /** Declared keys that matched nothing, plus the two shapes this phase refuses — both known from the binding, complete before [[run]].
    */
