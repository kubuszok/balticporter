...
object BeanCollapse:
  ...
  /** WHY a configured pair was not collapsed. A closed enum, because [[IdiomVerdict.Refused]]'s `guard` is the string a reader classifies a row by and an open one would let two sites describe one
    * refusal two ways.
    */
  enum Guard:
    ...
    if acc.exists(a => graph.closureOf(a).isAnchored) then Verdict.Refuse(Guard.AnchoredClosure)
      ...
      trivialField(p, g) match
        ...
        case Some(f)    =>
          ...
          else if target == Target.Var && prop.setter.isEmpty then Verdict.Refuse(Guard.VarWithoutSetter)
          else if target == Target.Val && prop.setter.isDefined then Verdict.Refuse(Guard.ValWithSetter)
          else if target == Target.Val && !immutableHere(p, f, written) then Verdict.Refuse(Guard.MutableStorage)
  ...
  /** may this field really be emitted as a `val`? See [[Guard.MutableStorage]]. Neither conjunct is java's `final` keyword — a non-`final` field that is never reassigned is common, and refusing on
    * the modifier would decline most of the get-only population for a syntax fact rather than a program one. Structural instead: the initialiser is on the node, and nothing writes it (the
  ...
  /** the field a TRIVIAL getter reads — `return this.f`, and nothing else. `None` for every other body, which is a refusal and never a widening. Read through [[soleStatement]]: a method body is a
    * `Tree.Block` whose STATEMENTS hold the `return`, and matching on the return alone would report every accessor as computed.
    */
  def trivialField(p: Program, getter: SymId): Option[SymId] =
    p.definitionOf(getter).collect { case d: Tree.DefDef => d }.flatMap(_.rhs).flatMap(soleStatement).flatMap {
  ...
  def trivialSetterField(p: Program, setter: SymId): Option[SymId] =
    p.definitionOf(setter).collect { case d: Tree.DefDef => d }.flatMap(_.rhs).flatMap(soleStatement).flatMap {
  ...
  /** does any OTHER declaration of this member's component have a BODY? See [[Guard.ConcreteRelative]] — the half of the override question `overriddenBelow` does not answer, deciding whether the
    * emitted `var` is an IMPLEMENTATION or an (illegal) OVERRIDE.
    */
  ...
  /** does a SUBCLASS of the declaring class declare this member too? See [[Guard.OverriddenBelow]] — an ancestor is fine and a descendant is not. Uses `OverrideGraph.overriders`'s transitive answer,
    * not a one-hop direct-parent test: a re-declaration two levels down would otherwise read as "nothing below" and the collapse would emit a `var` it cannot legally be.
    */
