...
/** Re-points a runtime class lookup by name (`Class.forName`-shaped) at an explicit name→class table the port supplies, since Scala.js/Native have no runtime class registry. Rewrites
  * `Wrapper.forName(s)` → `Table.classFor(s)`, same arguments/result type. Keys/values are `owner#member`; a key naming no program member is a no-op, reported by [[policyReport]].
  */
final class ClassTableTransform private (private[transform] val entries: List[ClassTableTransform.Entry]) extends Phase, PolicySource, SurfacePolicy, MergeablePolicy, PolicyBound:
  ...
  def mergedWith(later: Phase): Either[String, MergeablePolicy.Merged] = later match
    case o: ClassTableTransform =>
      ...
      if conflicts.nonEmpty then Left(conflicts.distinct.mkString("; "))
        ...
        Right(MergeablePolicy.Merged(new ClassTableTransform(merged), o.subjects -- subjects))
  ...
  /** Declared redirects that matched no member of this program, plus any whose value is not the `owner#member` shape the rewrite needs, plus scope entries no call site was under.
    */
