...
final class PrimitiveToOpaqueTransform(val spec: OpaqueSpec) extends Phase, Rewrite, PolicySource, MergeablePolicy, PolicyBound:
    ...
    case o: PrimitiveToOpaqueTransform =>
      ...
      refusals match
        case Nil =>
          ...
          val merged      = new PrimitiveToOpaqueTransform(
            OpaqueSpec(
              fqn = spec.fqn,
  ...
  def boundary(units: List[Tree.ClassDef]): List[OpaqueBoundaryCheck.Finding] =
    ...
    boundaryIssues.toList.filter(f => emitted(f.unit)).sortBy(f => (f.origin.javaPath, f.origin.line, f.subject, f.issue.toString))
  ...
  override def run(program: Program): Program =
    ...
    narrowerPrimSyms = {
      ...
      program.symbols.all
        .filter { s =>
          val i = WideningOrder.indexOf(s.fullName); i >= 0 && i < mine
        }
        .map(_.id)
        .toSet
    }
    ...
    val named = program.symbols.all.filter(s =>
      spec.hints(s.fullName) || spec.extraHints(s.fullName) ||
    ...
    def admissible(id: SymId): Boolean = program
      .symbolOf(id)
      .exists(s =>
    ...
    val (derivedIds, grownFrom) =
      hints.partition(id => program.symbolOf(id).exists(s => (derivedIdSet(id) || DerivedPolicy.keysOf(program, s).exists(derivedHints)) && !spec.hints(s.fullName) && !spec.extraHints(s.fullName)))
    ...
    val synthUnit: Option[Tree.ClassDef] = spec.target match
      case OpaqueSpec.Target.Mint =>
        ...
        // array wrap/unwrap
        val arrayMembers = if arraySym == SymId.None then Nil
        else
          val vaw = mint("v", "v", Flags(isParam = true), info = primArrayRef)
          val vau = mint("v", "v", Flags(isParam = true), info = opaqueArrayRef)
          wrapArraySym = mint("wrapArray", s"${spec.fqn}.wrapArray", Flags(), objSym, TypeRepr.MethodType(List("v" -> primArrayRef), opaqueArrayRef))
          unwrapArraySym = mint("unwrapArray", s"${spec.fqn}.unwrapArray", Flags(), objSym, TypeRepr.MethodType(List("v" -> opaqueArrayRef), primArrayRef))
          List(
            Tree.DefDef(
              wrapArraySym,
              List(List(Tree.ValDef(vaw, TypeTree(primArrayRef, Origin.synthetic), None, Origin.synthetic))),
              TypeTree(opaqueArrayRef, Origin.synthetic),
              Some(Tree.Ident(vaw, primArrayRef, Origin.synthetic)),
              Origin.synthetic
            ),
            Tree.DefDef(
              unwrapArraySym,
              List(List(Tree.ValDef(vau, TypeTree(opaqueArrayRef, Origin.synthetic), None, Origin.synthetic))),
              TypeTree(primArrayRef, Origin.synthetic),
              Some(Tree.Ident(vau, opaqueArrayRef, Origin.synthetic)),
              Origin.synthetic
            )
          )
      ...
      case OpaqueSpec.Target.Existing(typeFqn, wrapName, unwrapName) =>
        ...
        applySym = mint(wrapName, s"$companionFqn.$wrapName", Flags(), objSym, TypeRepr.MethodType(List("v" -> primRef), opaqueRef))
        unwrapSym = mint(unwrapName, s"$companionFqn.$unwrapName", Flags(), objSym, TypeRepr.MethodType(List("v" -> opaqueRef), primRef))

        // array coercions for the Existing form too — the same erasure identity applies.
        if arraySym != SymId.None then
          wrapArraySym = mint("wrapArray", s"$companionFqn.wrapArray", Flags(), objSym, TypeRepr.MethodType(List("v" -> primArrayRef), opaqueArrayRef))
          unwrapArraySym = mint("unwrapArray", s"$companionFqn.unwrapArray", Flags(), objSym, TypeRepr.MethodType(List("v" -> opaqueArrayRef), primArrayRef))
    ...
    /** one method's signature: every seeded parameter slot, plus the result when the method itself is a seed. An array parameter/result is retyped too.
      */
  ...
  /** the top-level unit a symbol belongs to. Fuel-bounded so a corrupt owner chain cannot hang the phase.
    */
  ...
  /** the type a declaration's value has — a method's result, anything else's own info. Matches [[taggablePrim]]'s domain: a hint naming a parameter is a different mistake with a policy exit (name the
    * parameter directly).
    */
  ...
  /** Does the value this term yields belong to the opaque family? Read through the declaration and through compound expressions that CARRY a value (`if`, block, `match`) rather than the node's own
    * `tpe`, which is blind to `x == null ? 0 : x.getHandle()`. Carriers are enumerated; an unenumerated one is a missed coercion — a loud compile error, not a spurious unwrap.
    */
  ...
  /** coerce into the opaque type. A leaf that already carries it is left alone, so a mixed carrier wraps only the branches that are still plain.
    */
  ...
  /** Emits the wrap: `OpaqueCompanion(value)`. Also handles the boxed form of the primitive (Java's auto-unbox is implicit in the TIR) — an opaque type IS the primitive at the JVM level, so
    * `Align(integerValue)` auto-unboxes the same way java's `int x = integerValue` does.
    */
  private def wrapCall(e: Term): Term =
    if isPrim(e.tpe) || isBoxedPrim(e.tpe) || widensToPrim(e.tpe) then Tree.Apply(Tree.Ident(objSym, TypeRepr.NoType, e.origin), List(e), applySym, opaqueRef, e.origin)
    else e

  private def unwrapCall(e: Term): Term =
    Tree.Apply(
      Tree.Select(Tree.Ident(objSym, TypeRepr.NoType, e.origin), unwrapSym, TypeRepr.NoType, e.origin),
      List(e),
      unwrapSym,
      primRef,
      e.origin
  ...
  /** coerce out of it — a no-op unless the value really is a seed's, safe to ask at every boundary. An array-of-opaque value is unwrapped with `unwrapArray`.
    */
  ...
  /** java widens a narrower primitive at an assignment or argument (JLS 5.2, 5.3): `lastTime = 0` at a `long` seed is the literal `0`, and `Nanos(0)` widens the same way.
    */
  ...
  /** is this the boxed form of the spec's primitive (`java.lang.Integer` for `Int`)? Auto-unboxes to the primitive, so `Align(integerValue)` is valid.
    */
  ...
  /** an EXISTING target declares no array helpers (sge's `Pixels` has none); an array of an opaque over a primitive IS that primitive's array at run time, so the move is a cast.
    */
  private def wrapArrayCall(e: Term): Term =
    if !spec.isMint then Tree.Typed(e, TypeTree(opaqueArrayRef, e.origin), opaqueArrayRef, e.origin)
      ...
      Tree.Apply(
        Tree.Select(Tree.Ident(objSym, TypeRepr.NoType, e.origin), wrapArraySym, TypeRepr.NoType, e.origin),
        List(e),
        wrapArraySym,
        opaqueArrayRef,
        e.origin
  ...
  private def unwrapArrayCall(e: Term): Term =
    if !spec.isMint then Tree.Typed(e, TypeTree(primArrayRef, e.origin), primArrayRef, e.origin)
      ...
      Tree.Apply(
        Tree.Select(Tree.Ident(objSym, TypeRepr.NoType, e.origin), unwrapArraySym, TypeRepr.NoType, e.origin),
        List(e),
        unwrapArraySym,
        primArrayRef,
        e.origin
  ...
  /** is this type `Array[Opaque]`? Needed because the retyped symbol table reads `Array[Opaque]` where the original had `Array[Prim]`, and `wrapFor` at an Assign LHS reads it after retyping.
    */
  ...
  /** is this a taggable type — the primitive itself or `Array[Prim]`? A method's result decides; a parameter is not independently taggable.
    */
