...
private[transform] trait CollectionsBoundary:
  ...
  def boundary(program: Program, units: List[Tree.ClassDef]): List[CollectionBoundaryCheck.Finding] =
    CollectionBoundaryCheck.check(program, units, mappedTypes, retypedTargets, scopedOut, classFileOverrides) ++
      ...
      opaqueEgressSites.toList
        ...
        .map((m, o) =>
          CollectionBoundaryCheck.Finding(
            ...
            "a value this port may have retyped",
            o,
            m
          )
        )
  ...
  def internal(program: Program, units: List[Tree.ClassDef]): List[CollectionInternalCheck.Finding] =
    CollectionInternalCheck.check(program, units, mappedTypes, targetOf, CollectionsTransform.standaloneTargets)
  ...
  /** the java symbols this run's mapping sends to a target that CANNOT BE A PARENT — see [[restoreUninheritableParents]]. EMPTY unless the program actually names one, which makes the pass a no-op by
    * arithmetic on every port that does not.
    */
  private[transform] var uninheritableSyms: Set[SymId] = Set.empty

  /** …and the classes among them whose value at the TARGET's own slot really is a detached one, with the target FQN it may be projected to — see [[detachedEntriesIn]]. EMPTY unless the program
    * declares such a class AND the class already refuses the write, so the projection declines by arithmetic on every port that does neither.
    */
  private[transform] var detachedEntries: Map[SymId, String] = Map.empty

  /** `java.lang.UnsupportedOperationException`, as this run's own type — see [[CollectionsTransform.UnsupportedOnTarget]].
    */
  private[transform] var unsupportedOpTpe: TypeRepr = TypeRepr.NoType
  private[transform] var unsupportedOpSym: SymId    = SymId.None

  /** every external seam this run could NOT close, in the order it met them. Reported through [[boundary]], because it is the same residue `CollectionBoundaryCheck` counts and a reader looking for
    * "what did the retyping leave open" must find all of it in one place.
    */
  private[transform] val externalSeams = collection.mutable.ListBuffer[CollectionBoundaryCheck.Finding]()

  /** retarget refusals recorded during the transform — shapes `retargetConstructRef` and `collectAndDrainNestedLabels` could not handle. Reported through [[retargetBoundary]].
    */
  ...
  private[transform] def refuseOnTarget(d: Tree.DefDef, owner: Tree.ClassDef, broke: String)(using p: Program): Tree.DefDef =
    ...
    val exn = Tree.Apply(
      Tree.New(TypeTree(unsupportedOpTpe, o), unsupportedOpTpe, o),
    ...
    seam(
      s"member (implements) $nm",
      TirPrinter.tpe(d.returnTpt.tpe, TirPrinter.Style.canonical),
      CollectionsTransform.UnsupportedOperationFqn,
      o,
      d.symbol,
  ...
  /** Fills [[retainedOverrides]]: members overriding a class-file declaration whose signature this phase would move (isOverride, mentionsMapped, no program-declared ancestor, an uncovered external
    * ancestor could declare it). Holds the member and all overriders below.
    */
  private[transform] def applyClassFileOverrides(p: Program): Unit =
    ...
    given Program = p
    val owned     = p.owned
    val seeds     = p.symbols.all.iterator
      .filter { s =>
    ...
    seeds.foreach { m =>
      if graph.overridden(m).isEmpty then
        ...
        val declarers = graph.externalAncestorsOf(graph.ownerOf(m)).filter(fqn => sig.exists(ExternalSurface.default.mayDeclare(fqn, _)))
        ...
        if kept.nonEmpty then
          ...
          (m :: graph.overriders(m)).filter(owned).filter(spliceable(graph)).foreach { x => held += x; anchor += (x -> label) }
    }
  ...
  /** True when [[restoreExcluded]] can reach this member (owner has a `ClassDef` definition). Anonymous classes hang off `Tree.New` inside a term and are not on the splice spine.
    */
    ...
    before.all.iterator
      .filter { s =>
        ...
        graph.signatureOf(s.id).exists { sig =>
          ...
          !graph.externalAncestorsOf(s.owner).filterNot(tabulatedTarget).exists(ExternalSurface.default.mayDeclare(_, sig)) &&
  ...
  /** One decision row per member for [[strippedOverrides]]. `Reason.Universal`, since there is no key to point a reader at — the engine chose the target, not the port. `parent=` answers the reader's
    * next question (`overrides nothing` names a type the java file never mentions).
    */
  private[transform] def recordStrippedOverrides(stripped: Set[SymId], before: SymbolTable)(using p: Program): Unit =
    stripped.toList.flatMap(id => before.all.find(_.id == id)).sortBy(_.fullName).foreach { s =>
      val parents = parentClash.get(s.owner).toList.flatMap(mp => mp.targets.toList ++ mp.shims.toList).distinct.sorted
      record(
        Decision(
  ...
  /** one bridge this run will build: the class, the kind it was minted at, the parent's java type arguments, the row, and the java member the body delegates to. `rename` is `CapturedByTarget`'s
    * answer, carried so the rename pass and body builder cannot disagree.
    */
  final private[transform] case class Bridge(cls: SymId, kind: Kind, args: List[TypeRepr], row: CollectionsTransform.Bridged, java: SymId, rename: Boolean)

  private[transform] var bridges: List[Bridge] = Nil

  /** the java types the mapping sends to a given target — the inverse of `typeMap`. `subsumed` is keyed on the target FQN; an override anchor is spelled with the java type, so the two are joined
    * through this table rather than by a name that looks alike.
    */
  ...
  private[transform] def planBridges(p: Program): List[Bridge] =
    ...
    /** the delegate, on THIS class and no ancestor of it — so one bridge lands on the component rather than once per subclass, and a java interface (which declares none of these) declines
      * structurally.
      */
    def ownMember(cls: SymId, want: ExternalSurface.Member): Option[SymId] =
      ...
      then scala.None
        ...
        def packs(m: SymId): Boolean = sigOf(m)
          .flatMap(_.descriptor)
          .exists(_.params.lastOption match
        ...
        if rows.isEmpty || found.isEmpty then Nil
          ...
          rows.flatMap { row =>
            if row.from.isEmpty then Some(Bridge(cls, k, args, row, SymId.None, false))
              ...
              row.from.iterator.flatMap(ownMember(cls, _)).nextOption() match
                case Some(j) =>
                  val captured = CollectionsTransform.CapturedByTarget.getOrElse(k.toString, Set.empty).exists(m => sigOf(j).exists(m.matches))
  ...
  private[transform] def refuseBridge(p: Program, cls: SymId, k: Kind, guard: String, why: String): Unit =
    seam(
      s"minted-parent surface [$guard]",
      k.toString,
      why,
      Decision.originOf(p, cls),
      cls,
  ...
  private[transform] def renameBridgeDelegates(p: Program): SymbolTable =
    ...
    val reParented: Set[String] = typeMap.collect {
      case (fqn, (tgt, _))
          if !CollectionsTransform.standaloneTargets(tgt) &&
            !CollectionsTransform.UninheritableTargets(tgt) =>
        fqn
    }.toSet
    ...
    val (renamed, refusals) = MemberRenamer.rename(p, graph, requests, MemberRenamer.OnCollision.SuffixUntilFree, decisions)
    if refusals.nonEmpty then
      val lost = refusals.flatMap(r => owners.get(r.request.member)).toSet
      refusals.foreach { r =>
        refuseBridge(p, owners.getOrElse(r.request.member, SymId.None), Kind.Map, "rename-refused", r.why)
      }
  ...
  /** One bridged member, as a tree. Every body is a delegation: java's own behaviour, unchanged, reached under a new name, plus one of four shape conversions the parent asked for (`Option(x)`,
    * `{ x; this }`, `.asScala`, `{ x; () }`). The three `Kind.Seq` rows with no delegate are documented at their bodies (`JavaCollections.buffer*`). `None` for a row this builder has no arm for — the
    * honest arm rather than a crash.
    */
  private[transform] def buildBridge(b: Bridge, cd: Tree.ClassDef, symbols: SymbolTable, mint: (String, String, SymId, TypeRepr, Boolean) => SymId)(using p: Program): Option[Tree.DefDef] =
    val o        = cd.origin
    val cls      = cd.symbol
    ...
    val javaRes  = javaInfo.collect { case TypeRepr.MethodType(_, r, _) => r }.getOrElse(TypeRepr.NoType)
    ...
    /** …and the ITERATOR half, whose discriminator is the RENAME rather than a type. A delegate this pass renamed is the one java's `Iterable` declares, so its result is a `JavaIterator`-shaped value
      * and needs the view; a delegate it did NOT rename is `entrySet()`, whose result the mapping already retyped to a `scala.collection` and whose `.iterator` is java's own idiom for the same
      * traversal.
      */
    ...
    def defd(nm: String, ps: List[Tree.ValDef], res: TypeRepr, body: Term, tps: List[Tree.TypeDef] = Nil): Option[Tree.DefDef] =
      val info = TypeRepr.MethodType(ps.map(v => p.symbolOf(v.symbol).map(_.name).getOrElse("x") -> v.tpt.tpe), res)
      val sym  = mint(nm, MemberKey(ownerFqn, nm).render, cls, info, true)
      recordBridge(b, sym, MemberKey(ownerFqn, nm).render, if b.java == SymId.None then "-" else MemberKey(ownerFqn, javaName).render, o)
      Some(Tree.DefDef(sym, if ps.isEmpty then Nil else List(ps), TypeTree(res, o), Some(body), o, tparams = tps))
    ...
    (b.kind, b.row.scalaName, b.row.arity) match
      case (Kind.Map, nm, ar) =>
        ...
        (nm, ar) match
          ...
          case ("addOne", 1) =>
            ...
            defd("addOne", List(pe), selfT, stat(List(Tree.Select(ref(pe), key1Sym, k, o), Tree.Select(ref(pe), value2Sym, v, o)), v, self, selfT))
          case ("subtractOne", 1) =>
            val pk = param("key", k)
      ...
      case (Kind.Seq, nm, ar) =>
        ...
        (nm, ar) match
          ...
          case ("prepend", 1) =>
            val pe = param("elem", a)
            defd("prepend", List(pe), selfT, stat(List(Tree.Literal(Constant.IntC(0), tpe(intSym), o), ref(pe)), unitTpe, self, selfT))
          case ("addOne", 1) =>
            val pe = param("elem", a)
            defd("addOne", List(pe), selfT, stat(List(ref(pe)), tpe(boolSym), self, selfT))
          case ("remove", 1) =>
          ...
          case ("remove", 2) =>
            ...
            defd("remove", List(pi, pc), unitTpe, helper("bufferRemoveRange", List(ref(pi), ref(pc)), unitTpe))
          case ("insertAll", 2) =>
            val pi = param("idx", tpe(intSym)); val pe = param("elems", tpe(iterableOnceSym, a))
            defd("insertAll", List(pi, pe), unitTpe, helper("bufferInsertAll", List(ref(pi), ref(pe)), unitTpe))
          case ("patchInPlace", 3) =>
            ...
            defd(
              ...
              List(pf, pp, pr),
              selfT,
              Tree.Block(List(helper("bufferPatchInPlace", List(ref(pf), ref(pp), ref(pr)), unitTpe)), self, selfT, o)
            )
          case _ => scala.None
      case _ => scala.None
  ...
  private[transform] def recordSuperDefaults(using p: Program): Unit =
    ...
    superDefaults.toList.groupBy(r => (r._2, r._3)).foreach { case ((callee, member), _) =>
      Decision.declarationsUsing(p, callee).filter((encl, _) => p.symbolOf(encl).exists(s => classes.contains(s.owner))).foreach { (encl, o) =>
        record(
          Decision(
  ...
  /** Record a `Decision` for each declaration whose `info` moved between before/after symbol tables. `Reason.Universal` for the default scope; `Reason.Configured` under `RuleScope.Only`.
    */
    ...
    before.all.foreach { s =>
      after.get(s.id).foreach { now =>
        if now.info != s.info && Decision.isDeclaration(p, s) then
          ...
          if mentionsOrderedShim(now.info) then cite(balticporter.catalog.JS.C(42), s.fullName)
      }
    }
  ...
  private[transform] def externalFieldProducer(sel: Tree.Select)(using p: Program): Term =
    if fromJavaSym == SymId.None || !externalCallee(sel.sym) then sel
      ...
      declaredFieldHead(sel.sym) match
        case scala.None => sel
        case Some(_)    =>
          headSym(sel.tpe).filter(liveWrappableSyms.contains) match
            case scala.None                          => sel
            case Some(_) if mentionsRetyped(sel.tpe) =>
              seam(
                "external field (nested element)",
                "a one-level wrap",
                TirPrinter.tpe(sel.tpe, TirPrinter.Style.canonical),
                sel.origin,
                sel.sym
              )
              sel
  ...
  private[transform] def externalProducer(t: Tree.Apply)(using p: Program): Term =
    if fromJavaSym == SymId.None || !externalCallee(t.method) || instantiation(t) then t
      ...
      headSym(t.tpe).filter(s => kindOf.contains(s) || shimSyms.contains(s)) match
        case scala.None => t
        ...
        case Some(_) if passesThrough(t) =>
          ...
          if !signatureReadable(t) then
            seam(
              "external result (unverified pass-through, no signature)",
              "a live scala view, IF the value was ever java's",
              TirPrinter.tpe(t.tpe, TirPrinter.Style.canonical),
              t.origin,
              t.method
            )
          t
        case Some(s) if !liveWrappableSyms.contains(s) =>
          seam("external result", "a live scala view", TirPrinter.tpe(t.tpe, TirPrinter.Style.canonical), t.origin, t.method)
          t
        case Some(_) if mentionsRetyped(t.tpe) =>
          seam(
            "external result (nested element)",
            "a one-level wrap",
            TirPrinter.tpe(t.tpe, TirPrinter.Style.canonical),
            t.origin,
            t.method
  ...
  /** True if this application is a `new` (constructor), not an external call. Constructors hand back the object this program built, not a java value to wrap.
    */
  ...
  /** True when the result type occurs on the input side (a generic pass-through). Class file checked first; structural guess used only when no signature is readable.
    */
  private[transform] def passesThrough(t: Tree.Apply)(using p: Program): Boolean =
    !declaredResultIsMapped(t) && {
      ...
      want != TypeRepr.NoType && (t.args.exists(a => occursIn(want, a.tpe)) || (t.fun match
  ...
  /** could the callee's class file be read for a signature at all? The two answers a suppression has to be told apart by: a refusal the CLASS FILE licensed, and one resting on a GUESS.
    */
  ...
  /** Count external-callee argument seams where no signature is readable (cannot-verify). Where a signature IS readable, [[CollectionBoundaryCheck]] classifies it instead.
    */
    ...
    if externalCallee(t.method) && p.symbolOf(t.method).forall(_.info == TypeRepr.NoType) then
      t.args.foreach { a =>
        headSym(a.tpe).filter(s => kindOf.contains(s) || shimSyms.contains(s)).foreach { _ =>
          seam(
            "argument (external callee, no signature)",
            "unknown — the callee is a class file",
            TirPrinter.tpe(a.tpe, TirPrinter.Style.canonical),
            a.origin,
            t.method
          )
        }
  ...
  private[transform] def seam(
    ...
    origin:    Origin,
    enclosing: SymId,
    issue:     CollectionBoundaryCheck.Issue = CollectionBoundaryCheck.Issue.ExternalCallee
  ): Unit =
  ...
  private[transform] def retargetSeam(
    what:      String,
    produced:  String,
    slot:      String,
    origin:    Origin,
    enclosing: SymId,
    issue:     RetargetBoundaryCheck.Issue = RetargetBoundaryCheck.Issue.ExternalProducer
  ): Unit =
  ...
  private[transform] def instantiatedFormals(t: Tree.Apply, formals: List[TypeRepr])(using p: Program): List[TypeRepr] =
    if formals.sizeIs != t.args.size then formals
      ...
      val ctorBound: Map[SymId, TypeRepr] =
        if !instantiation(t) then Map.empty
          ...
          (classTparams(owner), t.tpe) match
            case (ps, TypeRepr.AppliedType(tc, as)) if ps.nonEmpty && ps.sizeIs == as.size && headSym(tc).contains(owner) =>
              ps.zip(as).toMap
            case _ => Map.empty
      ...
      def bind(f: TypeRepr, a: TypeRepr): Unit = (f, a) match
        ...
        case (TypeRepr.AppliedType(ftc, fs), TypeRepr.AppliedType(atc, as)) if headSym(ftc) == headSym(atc) && headSym(ftc).isDefined && fs.sizeIs == as.size =>
  ...
  private[transform] def coerce(expected: TypeRepr, actual: Term, expectedScoped: Boolean = false, expectedExternal: Boolean = false, expectedSink: Boolean = false)(using p: Program): Term =
    ...
    val factory           = from match
      ...
      case Some(Kind.Seq | Kind.Stack | Kind.Set | Kind.Map) if (wantsJava || wantsUniversal || wantsStream) && mentionsRetyped(actualT) => SymId.None
      ...
      case _
          if entryToPairSym != SymId.None &&
            got.flatMap(detachedEntries.get).exists(tgt => wants.flatMap(p.symbolOf).exists(_.fullName == tgt)) =>
        entryToPairSym
      ...
      case _
          if wantsIs(javaIteratorSym) && iteratorFromSym != SymId.None &&
            got.flatMap(p.symbolOf).exists(_.fullName == "scala.collection.Iterator") =>
        iteratorFromSym
      case _ => SymId.None
  ...
  /** Per-class `MintedParents`, read from original units (before traversal moves parents), transitive over program-declared parents; scoped-out/uninheritable/unmapped parents excluded, standalone
    * targets recorded in `shims`.
    */
  private[transform] def declaredParentKinds(p: Program): Map[SymId, MintedParents] =
    ...
    /** (what this type extends, what type parameters it declares) — for both class bodies and anonymous classes (one parent, written at the `new`, no type parameters).
      */
            ...
            case Some((parents, tparams)) =>
              ...
              val subsumed = shimParents.flatMap { (sh, shTpe) =>
                kindParents.collectFirst {
                  case (tgt, k, kTpe)
                      if CollectionsTransform.SubsumesShim.get(k.toString).exists(_(sh)) &&
                        carriesElement(k, kTpe, shTpe) =>
                    sh -> tgt
                }
              }.toMap
              val shims = shimParents.map(_._1).toSet -- subsumed.keySet
              MintedParents(
                mapped.map(_._1).toSet ++ above.flatMap(_.kinds),
    ...
    shapeOf.keys.map(id => id -> resolve(id, Set.empty)).filter((_, mp) => mp.kinds.nonEmpty || mp.shims.nonEmpty || mp.subsumed.nonEmpty).toMap
  ...
  /** Does the kind parent really iterate what the shim parent says this class iterates? `SubsumesShim` claims a `scala.collection` target answers for `JavaIterable`'s one member, but only where the
    * two clauses agree on the element. Asked in java's own types, since [[declaredParentKinds]] reads the original units. Declines on a raw clause or an unhandled arity, leaving the duplicate parent
    * and scalac's own `E164`.
    */
  private[transform] def carriesElement(k: Kind, kindParent: TypeRepr, shimParent: TypeRepr)(using p: Program): Boolean =
    def entryOf(t: TypeRepr): Option[(TypeRepr, TypeRepr)] = t match
      case TypeRepr.AppliedType(_, a :: b :: Nil) =>
        headSym(t).flatMap(p.symbolOf).map(_.fullName).filter(fqn => typeMap.get(fqn).exists((_, ek) => ek == Kind.Entry)).map(_ => (a, b))
      case _ => scala.None
    ...
    if uninheritableSyms.isEmpty then Map.empty
      ...
      def targetOf(id: SymId, seen: Set[SymId]): Option[String] =
        if seen(id) then scala.None
          ...
          byId.get(id).flatMap { cd =>
            parentsOf(cd).iterator
              .flatMap { h =>
                p.symbolOf(h).flatMap(s => typeMap.get(s.fullName)).map(_._1).filter(CollectionsTransform.UninheritableTargets).orElse(targetOf(h, seen + id))
              }
              .nextOption()
          }
              ...
              .collectFirst {
                case d: Tree.DefDef
                    if d.rhs.nonEmpty && d.paramss.map(_.size).sum == sig.arity &&
                      p.symbolOf(d.symbol).exists(_.name == sig.name) =>
                  d
              }
      ...
      classes.flatMap { cd =>
        targetOf(cd.symbol, Set.empty)
          .filter { tgt =>
            val sigs = CollectionsTransform.UnsupportedOnTarget.getOrElse(tgt, Set.empty)
            sigs.nonEmpty && sigs.forall(sig => nearest(cd.symbol, sig, Set.empty).flatMap(_.rhs).exists(throwsFirst))
          }
          .map(cd.symbol -> _)
      }.toMap

  /** does this body THROW before it does anything else? The capability test [[detachedEntriesIn]] rests on, and it is asked of the first statement rather than of the whole body, because that is
    * exactly the property that makes a write impossible — anything after an unconditional throw is unreachable. A conditional throw answers `false`: java's own `setValue` may refuse for one receiver
    * state and write for another, and that class writes through.
    */
