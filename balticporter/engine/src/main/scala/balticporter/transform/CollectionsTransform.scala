...
/** `java.util` collections to `scala.collection.mutable`. Retypes every collection occurrence via the symbol table, rewrites call shapes kind-aware, coerces at JDK/external seams. [[RuleScope]]
  * controls which declarations are reached; unclosable scope seams are counted by [[CollectionBoundaryCheck]].
  */
final class CollectionsTransform(
  val scope: RuleScope = RuleScope.Everywhere(),
  /** Java FQN to scala FQN retarget: retyped at every occurrence, NO kind-aware rewrites. The scala target must extend/implement the java source (no coercion needed). A key colliding with `typeMap`
    * is refused. Empty = no-op.
    */
  val retarget: Map[String, String] = Map.empty,
  /** Per-retarget member call-site rewrites, keyed `(memberName, arity)` to [[RetargetRewrite]]. Descriptor-keyed [[retargetRewritesByDesc]] wins over arity-keyed at the same member. Empty = no-op.
    * `MergeablePolicy` unions independent keys.
    */
  val retargetRewrites: Map[String, Map[(String, Int), CollectionsTransform.RetargetRewrite]] = Map.empty,
  /** Descriptor-keyed retarget rewrites — `(name, Descriptor)` to [[RetargetRewrite]]. Wins over arity-keyed [[retargetRewrites]] at the same member. For members overloaded at the same arity. Empty =
    * no-op.
    */
  val retargetRewritesByDesc: Map[String, Map[(String, Descriptor), CollectionsTransform.RetargetRewrite]] = Map.empty,
  /** Type argument mapping for arity-changing retargets — source FQN to target arg template. Each element: `SourceArg(i)` or `FixedType(fqn)`. List length must equal target arity. Orphan keys (no
    * `retarget` entry) refused at construction. Empty = no-op.
    */
  ...
  /** Retarget coercions — `(actualHeadFQN, expectedHeadFQN)` to template string. `$0` = actual value. Rendered as `Tree.Opaque.spliced` at type boundaries. Empty = no-op. `MergeablePolicy` unions;
    * same pair with different template refuses.
    */
  ...
  /** Per-class info about minted collection parents. `kinds` = set (a class may implement several). `probes` = first type arg of each mapped parent. `shims` = standalone targets (no member clash).
    * `targets` = scala FQNs. `declared` = this class's own mapped clauses (non-transitive).
    */
  final private[transform] case class MintedParents(
    kinds:    Set[Kind],
    probes:   List[TypeRepr],
    tparams:  List[SymId],
    shims:    Set[String],
  ...
  /** java 8 `Collection.forEach(Consumer)` — scala's is `foreach`, differing only in case, which makes the failure read like a typo rather than a missing mapping.
    */
  ...
  /** `JavaCollection` + its `from` factory — the same seam, one type up. `unmodifiableFromSym` is the read-only sibling (a `Map.values()` view); `unmodifiableSym` is
    * `Collections.unmodifiableCollection`.
    */
  private[transform] var javaCollectionSym, collectionFromSym: SymId = SymId.None

  /** the `Kind.Set` source's factory into a `JavaCollection` slot — a DISTINCT NAME rather than an overload of `from`, for the reason `JavaCollection.unmodifiableFrom` gives: an overload resolves on
    * the static type, and every candidate here is a `scala.collection.Iterable`.
    */
  ...
  /** each HASHED scala collection symbol → its companion's `defaultLoadFactor`, for [[capacityConstructor]]. Keyed on the phase's OWN targets, exactly as `fromSyms` is.
    */
  ...
  /** scala's own `toBuffer` — how a `Kind.Set` or `Kind.Map` stream SOURCE reaches the `Buffer` every collapsed operation is declared over. See `streamSource`.
    */
  ...
  /** java's three `Object`-keyed map members, and the two `Object`-keyed collection ones, for a receiver or an ARGUMENT at which the element type cannot be named — see [[objectProbe]].
    */
  ...
  /** `java.lang.Object` as THIS run's symbol — the top of java's reference hierarchy, and the one type an argument can carry that conforms to no scala element type at all ([[objectProbe]]).
    */
  ...
  /** every symbol THIS PHASE minted in [[run]] — the rewrites' own targets. They are owned by nothing and named by no class file, so the external-seam arms would otherwise read each of them as a
    * third party's method.
    */
  private[transform] var mintedSyms: Set[SymId] = Set.empty

  /** every symbol this run's [[scope]] held OUT of the rewrite. EMPTY for the default scope — and for any scope whose entries matched nothing — which is what makes the no-op a no-op.
    */
  ...
  /** every member this run held back because it OVERRIDES A CLASS FILE — see [[classFileOverridesIn]]. EMPTY for a program that extends no unconverted java type, which is what makes this a no-op by
    * arithmetic exactly as an unrestricted scope is.
    */
  private[transform] var retainedOverrides: Set[SymId] = Set.empty

  /** …plus the PARAMETER symbols of those members, because a tree that says `java.util.Collection` over a symbol that says `JavaCollection` is the lie [[mapSignatures]] already refuses to write for
    * the scope. Held apart from [[classFileOverrides]] so the boundary check keys on MEMBERS and nothing else.
    */
  ...
  /** the union — every declaration whose type this run reads LITERALLY, whichever refusal held it. `isEmpty` is the pre-refusal code path by arithmetic, which both halves need.
    */
  ...
  override def run(program: Program): Program            =
    ...
    classDefsBySym = program.units.flatMap(StandardTraversal.allClassDefs(_)(using program)).map(cd => cd.symbol -> cd).toMap
    ...
    remap = program.symbols.all.flatMap { s =>
      ...
      effectiveRetarget
        ...
        .map { sc =>
          ...
          else byScala.getOrElseUpdate(sc, mint(sc.substring(sc.lastIndexOf('.') + 1), sc))
          s.id -> sym
        }
        .orElse(
          typeMap.get(s.fullName).map(_._1).map { sc =>
    ...
    staticSyms = CollectionsTransform.StaticHelpers.map(n => n -> mint(n, s"$JavaCollectionsFqn.$n")).toMap
    ...
    retargetEntryTargets = program.symbols.all.flatMap { s =>
      effectiveRetarget.get(s.fullName).filter(CollectionsTransform.UninheritableTargets.contains).flatMap(_ => remap.get(s.id))
    }.toSet
    ...
    val rwIdxFields = retargetRewrites.flatMap { (srcFqn, tbl) =>
      tbl.collect { case ((fieldName, 0), idx: CollectionsTransform.RetargetRewrite.IndexedField) =>
        program.symbols.all.filter(s => s.fullName == srcFqn).flatMap { ownerSym =>
          program.symbols.all.filter(m => m.owner == ownerSym.id && m.name == fieldName).map(m => m.id -> (srcFqn, idx))
        }
      }.flatten
    }
    ...
    def collectFqns(arg: CollectionsTransform.RetargetArg): Set[String] = arg match
      ...
      case _ => Set.empty
    retargetFixedTypeSyms = retargetTypeArgs.values.flatten
      .flatMap(collectFqns)
      .toSet
      .map { fqn =>
        val sym = byScala.getOrElseUpdate(
          fqn,
          // check program symbols before minting — the frontend may already have this FQN
          program.symbols.all.find(_.fullName == fqn).map(_.id).getOrElse(mint(fqn.substring(fqn.lastIndexOf('.') + 1), fqn))
        )
        fqn -> sym
      }
      .toMap
    ...
    retargetRewriteSyms = retargetRewrites.flatMap { (src, tbl) =>
      tbl.values.flatMap {
        ...
        case CollectionsTransform.RetargetRewrite.IndexedField(_, v, vw) =>
          ...
          List((src, v) -> mint(v, s"$src#retargetRewrite:$v"), (src, vw) -> mint(vw, s"$src#retargetRewrite:$vw"))
      ...
      tbl.values.flatMap { idx =>
        List(
          (src, idx.via) -> mint(idx.via, s"$src#retargetRewrite:${idx.via}"),
    ...
    enumSetSyms = List("noneOf", "allOf", "of", "copyOf", "range", "complementOf").map(n => n -> mint(n, s"${CollectionsTransform.JavaEnumSetFqn}.$n")).toMap
  ...
  /** the rest of [[run]], over the symbol table the bridge renames produced — a separate method because a second `given Program` in one scope after the rename would be an ambiguity, not a shadow.
    */
  private[transform] def finishRun(program: Program, symbols: SymbolTable): Program =
          ...
          if p.owns(s.id) then
            infoResultHead(s.info).foreach { headId =>
              p.symbolOf(headId).foreach { hs =>
                if effectiveRetarget.contains(hs.fullName) then buf(s.id) = hs.fullName
              }
            }
        }
        buf.toMap
    ...
    var units: List[Tree.ClassDef] = program.units.map(u => dropSubsumedParents(restoreUninheritableParents(u, restoreExcluded(u, StandardTraversal.mapClassDef(this, u)))))
    ...
    if scopedFamilyIds.nonEmpty then
      ...
      scopedGroups.toList.sortBy(_._1.fingerprint).foreach { (sc, srcIds) =>
        ...
        units = units.map { u =>
          if inFamilyScope(sc, scopedP, u.symbol) then dropSubsumedParents(restoreUninheritableParents(u, StandardTraversal.mapClassDef(this, u)))
          else u
        }
      }
    ...
    if retargetRewrites.values.exists(_.values.exists(_.isInstanceOf[CollectionsTransform.RetargetRewrite.Collect])) ||
      retargetRewritesByDesc.values.exists(_.values.exists(_.isInstanceOf[CollectionsTransform.RetargetRewrite.Collect]))
    then
    ...
    val symbols3 =
      if stripped.isEmpty then symbols2
        ...
        symbols2.all.foldLeft(symbols2) { (t, s) =>
        ...
        case Some(sym) =>
          remapByFullName.get(sym.fullName) match
            case Some(newSym) =>
              ...
              retargetArgsBySource
                .get(s)
                .orElse(
                  remap.collectFirst {
                    case (srcId, `newSym`) if retargetArgsBySource.contains(srcId) =>
  ...
  /** A raw-type occurrence's substituted bound still carries its OWN unresolved wildcards (a self-bounded generic's erasure); stripping an upper bound to such a bound mis-narrows an invariant slot,
    * so upper-bound stripping only fires where this is `false` (subplan item 3).
    */
  ...
  /** `asInstanceOf[T]` needs a REIFIABLE `T`; a DECLARATION may keep `?` (valid Scala, the right image of java's wildcard) but a cast target may not, so the upper-bound strip [[hasNestedBound]]
  ...
  /** A `val`'s declared type is an expected type exactly as a formal parameter is, so it routes to the same [[coerce]] every other position does.
    */
  ...
  override def transformTerm(t: Term)(using Program): Term = t match
    ...
    case io: Tree.InstanceOf                      =>
      wildcardReifiedTest(io) match
  ...
  /** Entry copy-construction fold: `val e2 = Tuple2(default, default); e2._1 = X; e2._2 = Y` folds into `val e2 = Tuple2(X, Y)` — java's `Entry` has mutable public fields, `Tuple2` has `val _1/_2`,
    * so the copy-construct pattern is faithfully rewritten to construct with the right values from the start. Guards: type head is `retargetEntryTargets`, assigns target `_1`/`_2` contiguously right
    * after the ValDef, variable not reassigned elsewhere.
    */
      ...
      case Left(tt) if !mr.referent.isInstanceOf[Referent.Static] =>
        kindOf.get(headSym(tt.tpe).getOrElse(SymId.None)) match
          ...
          case Some(k) =>
            ...
            val callT = Tree.Apply(Tree.Select(self, mr.method, TypeRepr.NoType, o), Nil, mr.method, TypeRepr.NoType, o)
          ...
          case Some(k) =>
            ...
            if arity < 0 || arity > argParamSyms.size then mr
              ...
              val (self, stats) = recv match
                ...
                case _ =>
                  (Tree.Ident(recvBindSym, recv.tpe, o), List(Tree.ValDef(recvBindSym, TypeTree(recv.tpe, o), Some(recv), o)))
              val ps    = argParamSyms.take(arity).toList
              val args  = ps.map(s => Tree.Ident(s, TypeRepr.NoType, o))
              val callT = Tree.Apply(Tree.Select(self, mr.method, TypeRepr.NoType, o), args, mr.method, TypeRepr.NoType, o)
              rewrite(k, self, mr.method, o, callT) match
                case None       => mr
                case Some(body) =>
  ...
  private[transform] def retargetConstructRef(mr: Tree.MethodRef, tt: TypeTree)(using p: Program): Option[Term] =
    ...
    if arity > argParamSyms.size then
      retargetSeam(
        "constructor reference arity > argParamSyms pool",
        s"C::new with arity $arity",
        "Construct not applied — pool has ${argParamSyms.size}",
        mr.origin,
        SymId.None
      )
      return scala.None
  ...
  override def transformApply(t: Tree.Apply)(using Program): Term =
    ...
    val out = tokenConstructor(t2).orElse(copyConstructor(t2)).orElse(capacityConstructor(t2)).orElse(retargetConstruct(t2)).orElse(staticRewrite(t2)).getOrElse {
  ...
  /** Selects rewritten by the Chain/Template handler in `retargetSelectRewrite`, tracked by identity of the RESULT term, so the Apply handler can strip an outer `()` a chain-rewritten parenless
    * target should not have.
    */
  private[transform] val selectChainRewritten = java.util.Collections.newSetFromMap(new java.util.IdentityHashMap[Term, java.lang.Boolean]())

  /** [[inheritedKind]] with no receiver to read. The scope suppression `inheritedKind` applies is about the RECEIVER's declaration, and there is no receiver here; the enclosing class's own [[kindAt]]
    * — which does go through `actualOf` — is what stands in for it at the claim.
    */
          ...
          if !implicitPending.remove(x.origin) then x
            ...
            x.fun match
              case Tree.Ident(m, _, so) =>
                implicitInheritedKind(m).flatMap(k => rewrite(k, Tree.This(a.symbol, t.tpe, x.origin), m, so, x)).getOrElse(x)
              case _ => x
  ...
  /** A `return` is a shim-typed slot exactly as a formal or `val` is — the declared return type is the expected type of every `return` in the body. DELIBERATELY BOUNDED: a `return` inside a
    * lambda/anon/local class returns from THAT, so an unhandled kind MISSES a coercion (a loud compile error) rather than wrongly coercing. Tail expression is not a return value here — every java
    * method exits through `Tree.Return`.
    */
...
object CollectionsTransform:
  ...
  private[transform] def spine(orig: List[Statement], mapped: List[Statement], of: SymId): List[(Statement, Statement)] =
  ...
  /** The declared type of a scoped-out declaration's symbol, or `None` if not scoped out. Reads `Symbol.info` instead of the node's `tpe` (which `transformType` already remapped).
    */
  ...
  /** A call-site rewrite for a retarget entry's member. `retarget` swaps the type; `retargetRewrites` rewrites call sites on that retarget's target, keyed on the retarget's source FQN, each entry
    * mapping `(memberName, arity)` to one of these.
    */
  ...
  object RetargetRewrite:
    ...
    /** Boolean-dispatched: inspect `args(flagIndex)` — literal `true` calls `onTrue(remaining)`, literal `false` calls `onFalse(remaining)`, non-literal is refused and counted.
      */
    case class BoolDispatch(flagIndex: Int, onTrue: String, onFalse: String) extends RetargetRewrite

    /** Construction rewrite: `new Source(args)` to `companionFqn.factoryMethod(args)`. `dropTrailing` strips trailing args; `fillTypeArgs` generates null placeholders for 0-arg case.
      */
    case class Construct(
      companionFqn:  String,
      factoryMethod: String,
      dropTrailing:  Int = 0,
      fillTypeArgs:  Boolean = false,
      /** a `given` clause (`Type = expr`, `$T0` = the element type), in scope when the element is a type variable */
      typeVarEvidence: Option[String] = None
    ) extends RetargetRewrite

    /** For-each structural rewrite: `for (E e : recv.sourceMethod())` over a retarget target lowers to `recv.targetMethod(e => body)` (or a 2-arg lambda for entry iteration). `break`/ `continue`
      * become `boundary` around the call/lambda body; `return` has no faithful image (no explicit iterator) and is refused and counted on `collection-retarget`, as is any usage of `sourceMethod()`
      * outside a for-each header. `arity` is the lambda parameter count.
      */
    case class ForEach(targetMethod: String, arity: Int) extends RetargetRewrite

    /** Standalone collection: `recv.sourceMethod()` outside a for-each header collects eagerly into a `DynamicArray` via `into`'s `apply()`, walked by `via` (the forEach method name); inside a
      * for-each header it lowers like `ForEach(via, 1)`. Deltas (eager copy vs java's live view; no `ConcurrentModificationException`) are counted on `collection-retarget`.
      */
    ...
    /** Field write rewrite: `recv.field = value` -> `recv.method(value)`, for a java public field whose target exposes only a getter method. Keyed at `(field, 0)`, the same slot a `Rename`/`Chain`
      * occupies for the read side — the two fire on different node kinds (`Assign` vs `Select`/`Apply`) and coexist at one key.
      */
  ...
  /** How to construct a retarget target's type arguments from the source type's — one element per target type parameter. E.g. `IntMap<V>` (1 param) -> `ObjectMap[K,V]` (2 params) needs
  ...
  object RetargetArg:
    ...
    /** An applied type at this position (e.g. `Tuple2[K,V]` for a nested Entry type in the source), whose own type arguments are each a `RetargetArg`.
      */
    case class Applied(fqn: String, args: List[RetargetArg]) extends RetargetArg

  /** the shape of a collection, which decides the call rewrite (a `Seq` `get` is `apply`, a `Map` `get` is `getOrElse`).
    */
  enum Kind:
    case Seq, Map, Set
    ...
    /** a `java.util.Stack`: a [[Seq]] plus five LIFO members, one of which (`peek`) is the opposite end from `Deque.peek` — java's `Stack.peek()` throws on empty, `Deque.peek()` returns null, so one
      * arm cannot answer both. Falls back to [[Seq]] once the five decline.
      */
    case Stack

    /** a `java.util.Optional{Int,Long,Double}`, mapped to an `Option[…]` alias — not a collection, but its member names differ the same way (`getAsInt`/`isPresent`/`orElse`).
      */
    case Opt
  ...
  /** `java.util.Stack`'s target — a `mutable.ArrayBuffer` carrying java's own LIFO five. See the [[typeMap]] entry for why the stdlib type is the wrong answer and why this is not a rewrite.
    */
  val JavaStackFqn = s"${RuntimeArtifact.Package}.JavaStack"

  /** `java.util.EnumMap`/`EnumSet`'s targets — a `mutable.Map`/`Set` that iterates in ORDINAL order, which is the GUARANTEE no stdlib type carries (catalog `JS-C42`).
    */
  val JavaEnumMapFqn = s"${RuntimeArtifact.Package}.JavaEnumMap"
  val JavaEnumSetFqn = s"${RuntimeArtifact.Package}.JavaEnumSet"

  /** `java.util.Optional{Int,Long,Double}`'s targets — type ALIASES for `Option[…]`, because the retype is arity-changing and the head swap is not. See the alias's own doc.
    */
  ...
  /** `java.util.Collections`' statics — a receiver-less utility class, which is why they need their own home rather than a rewrite keyed on a receiver's collection kind.
    */
  ...
  /** Java FQN to (scala FQN, Kind). In companion so checks can read it without a phase instance. Must stay BELOW the `*Fqn` vals (declaration-order initialisation).
    */
  ...
  private[transform] lazy val unmappedSupertypes: Set[String] =
    ...
    val all = typeMap.keys.flatMap { k =>
      try closure(Class.forName(k))
      catch { case _: Throwable => Set.empty[String] }
    }.toSet
    all -- typeMap.keySet - "java.lang.Object"

  /** every `JavaCollections` member the transform may emit. One list, so a new JDK utility is one line here, one arm in `staticRewrite` and one method in the runtime object — and a typo is a
    * `SymId.None` that declines the rewrite rather than a dangling name in emitted code.
    */
  /** the three members java declares over `Object`, so a `Map<?, ?>` receiver supports them and scala's `Map[K, V]` does not — see `wildcardMapCall`.
    */
  ...
  /** Mapping TARGETS that cannot be a PARENT, however right they are as a use.
    *
    * `scala.Tuple2` is the one: final, no `setValue`, and a constructor taking the two components. A class that IMPLEMENTS `java.util.Map.Entry` therefore cannot be emitted at that target at all —
    * see `restoreUninheritableParents`, which keeps java's parent and counts the refusal.
    */
  ...
  final private[balticporter] case class MemberSig(name: String, arity: Int)

  /** Members a minted parent declares at its type parameter that shadow java's `Object`-keyed members. Keyed by Kind. Used by [[pinnedByObject]] to resolve ambiguous overloads.
    */
  private[balticporter] val ShadowedByTarget: Map[String, Set[MemberSig]] = Map(
    // `mutable.Map`: `get`/`remove`/`contains`/`apply` all take `K`.
    Kind.Map.toString -> Set(MemberSig("get", 1), MemberSig("remove", 1), MemberSig("contains", 1), MemberSig("apply", 1)),
    // `mutable.Set`: `remove`/`contains`/`apply` all take `A`.
    Kind.Set.toString -> Set(MemberSig("remove", 1), MemberSig("contains", 1), MemberSig("apply", 1)),
  ...
  private[balticporter] val OverridesTarget: Map[String, Set[ExternalSurface.Member]] = Map(
    ...
    Kind.Map.toString -> Set(
      ...
      ExternalSurface.Member("isEmpty", 0),
      ExternalSurface.Member("clear", 0),
      ExternalSurface.Member("keySet", 0),
      ExternalSurface.Member("keys", 0),
      ExternalSurface.Member("values", 0),
      ExternalSurface.Member("iterator", 0),
      ExternalSurface.Member("put", 2)
    ),
    // mutable.Set
  ...
  private[balticporter] val OverridesShim: Map[String, Set[ExternalSurface.Member]] = Map(
    ...
    JavaIteratorFqn -> Set(ExternalSurface.Member("hasNext", 0), ExternalSurface.Member("next", 0), ExternalSurface.Member("remove", 0)),
    JavaListIteratorFqn -> Set(
      ExternalSurface.Member("hasNext", 0),
      ExternalSurface.Member("next", 0),
      ExternalSurface.Member("remove", 0),
      ExternalSurface.Member("hasPrevious", 0),
      ExternalSurface.Member("previous", 0),
      ExternalSurface.Member("nextIndex", 0),
      ExternalSurface.Member("previousIndex", 0),
      ExternalSurface.Member("set", 1),
      ExternalSurface.Member("add", 1)
    ),
    JavaCollectionFqn -> Set(
      ExternalSurface.Member("iterator", 0),
      ExternalSurface.Member("size", 0),
      ExternalSurface.Member("isEmpty", 0),
      ExternalSurface.Member("contains", 1),
      ExternalSurface.Member("add", 1),
      ExternalSurface.Member("remove", 1),
      ExternalSurface.Member("clear", 0),
      ExternalSurface.Member("containsAll", 1),
      ExternalSurface.Member("addAll", 1),
      ExternalSurface.Member("removeAll", 1),
      ExternalSurface.Member("retainAll", 1),
      ExternalSurface.Member("removeIf", 1),
      ExternalSurface.Member("toArray", 0),
      ExternalSurface.Member("toArray", 1)
    )
  )
  ...
  /** One bridged member: scala name/arity, with `from` = java member preference list to delegate to. `from = Nil` means unconditional (no java counterpart; body from `JavaCollections`).
    */
  final private[balticporter] case class Bridged(scalaName: String, arity: Int, from: List[ExternalSurface.Member], required: Boolean = true)

  private val ObjectArg = Some(Descriptor(List(Param.Named("Object"))))
  private val IntArg    = Some(Descriptor(List(Param.Prim("int"))))

  /** keyed by [[Kind]] for [[OverridesTarget]]'s reason: several map kinds share one `MapOps` trait surface. `Kind.Stack`'s target has nothing abstract; `Kind.Entry`/`Kind.Opt` are never a parent
    * ([[UninheritableTargets]]) — all three are a no-op here by arithmetic.
    */
  private[balticporter] val BridgedTarget: Map[String, List[Bridged]] = Map(
    Kind.Map.toString -> List(
      ...
      Bridged("iterator", 0, List(ExternalSurface.Member("iterator", 0), ExternalSurface.Member("entrySet", 0))),
  ...
  /** Which delegates the emitted parent would CAPTURE — the set that decides the rename. Where the parent declares the same (name, arity), a bridge body's call binds to the PARENT's own member,
    * making the bridge an infinite recursion; a captured delegate is renamed out of the way. Deliberately absent: every delegate the target does not declare (`entrySet()`, a `Buffer`'s indexed
    * members) since renaming those would move surface for no hazard.
    */
  private[balticporter] val CapturedByTarget: Map[String, Set[ExternalSurface.Member]] = Map(
    ...
    Kind.Map.toString -> Set(
      ExternalSurface.Member("put", 2),
      ExternalSurface.Member("get", 1, ObjectArg),
    ...
    Kind.Set.toString -> Set(
      ExternalSurface.Member("contains", 1, ObjectArg),
      ExternalSurface.Member("add", 1),
      ExternalSurface.Member("remove", 1, ObjectArg),
      ExternalSurface.Member("iterator", 0)
    ),
  ...
  /** every `owner#name` a [[staticRewrite]] arm matches, including the two collector keys the `collect` arms read out of a guard — a table that omitted them would report the port's own translation as
    * its wall.
    */
  ...
  /** java's RAW static CONSTANTS → the typed factory java itself says they are, read by `staticFieldRewrite`. Three entries, and the list is closed by JAVA rather than by this table: these are the
    * only members of `java.util.Collections` that are fields at all.
    */
  ...
  /** collection KIND → the instance member names [[rewrite]] handles for it, with `JdkSurfaceCheck.AnyKind` for an arm whose kind pattern is `_`. Read by eye and deliberately not asserted: assigning
    * a name to more kinds than its arm covers can only make the check kinder, never miss a real hole.
    */
  val handledInstance: Map[String, Set[String]] = Map(
    ...
    Kind.Map.toString -> Set("get", "put", "remove", "containsKey", "entrySet", "values", "putIfAbsent", "computeIfAbsent", "containsValue"),
    ...
    Kind.Opt.toString -> Set("getAsInt", "getAsLong", "getAsDouble", "orElseThrow", "isPresent", "orElse", "ifPresent"),
  ...
  /** This phase's record, in the shape [[balticporter.tir.JdkSurfaceCheck]] reads. `ran` is the caller's to supply: absent, an unhandled member on a mapped type is an offer (`mappable`, report-only);
    * present, the same member is a hole the phase MADE, and is a finding.
    */
  ...
  /** Runtime shim types this retyping requires (e.g. `JavaIterator` = scala `Iterator` + `remove`). Live in `balticporter-runtime`.
    */
  val runtimeTypes: Set[String] =
    Set(
      ...
      JavaOptionalIntFqn,
      JavaOptionalLongFqn,
      JavaOptionalDoubleFqn
    )

  /** What [[runtimeSources]] brings, for a consumer that must reason about the injected supertypes it cannot parse — `JavaIterator.remove` is concrete, so a class extending both it and a superclass
    * also defining `remove` is a scala linearisation conflict `TirEmitter` needs to know about. Prefer `RuntimePlan.of(phases).concreteMembers`, which derives this.
    */
  ...
  /** The support sources, as text, for a port that vendors them instead of depending on the artifact ([[balticporter.core.RuntimeMode.Vendored]]). Not the source of truth — the build-time copy of
    * `runtime/src/main/scala`. Prefer `RuntimePlan.of(phases, mode).writeSources(dir)`.
    */
