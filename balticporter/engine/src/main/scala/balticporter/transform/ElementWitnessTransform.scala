...
final class ElementWitnessTransform(
  ...
  /** upstream class FQN -> the 0-based TYPE-PARAMETER INDEXES whose arrays move onto the witness. Empty map is the no-op: no clause, no rewrite, no bound moved, no `policy=` segment.
    */
  val subjectTypes: Map[String, List[Int]] = Map.empty,
  /** the declarations that LOSE java's implicit `<: java.lang.Object` bound — a SUBJECT at its element positions, any other named type at all of them, and every method they declare. Apart from the
    * subject set on purpose: a table that reads `null` as SLOT EMPTY keeps the bound (`ElementWitnessCheck.Issue.OccupancySentinel`). A bound java WROTE is never dropped.
    */
  ...
  def surfaceFingerprint: String =
    if isNoOp then ""
      ...
      val ds   = if defaultSuppliers.isEmpty then "" else "|suppliers=" + defaultSuppliers.toList.sorted.map((k, v) => s"$k->$v").mkString(",")
  ...
  /** Independent subjects UNION; the same subject with DIFFERENT indexes REFUSES — two answers for which parameter an array is keyed on is a choice, not a composition.
    */
    ...
    case o: ElementWitnessTransform =>
      val witnessClash =
        if witness.nonEmpty && o.witness.nonEmpty && witness != o.witness then
          List(
            s"""both modules name a witness type, "$witness" and "${o.witness}" — the emitted """ +
      ...
      val memberClash =
        if members != o.members && witness.nonEmpty && o.witness.nonEmpty then
          List(
            s"""both modules name the witness's members, "${members.fingerprint}" and """ +
        ...
        case Nil =>
          Right(
            MergeablePolicy.Merged(
              ...
              (o.subjectTypes.keySet ++ o.dropBound -- subjectTypes.keySet -- dropBound).map(MergeablePolicy.subjectOf)
            )
          )
        case whys =>
          Left(
  ...
  def subjects: Set[String] =
    (subjectTypes.keySet ++ dropBound ++ defaultSuppliers.keySet ++ scope.entries).map(MergeablePolicy.subjectOf)
  ...
  def bindPolicy(binder: PolicyBinder): Unit =
    ...
    if witness.isEmpty && subjectTypes.nonEmpty then
      bad += PolicyFinding(
        name,
        "ElementWitnessTransform.witness",
        "",
    ...
    subjectTypes.toList.sortBy(_._1).foreach { (fqn, idxs) =>
      if idxs.isEmpty then
        bad += PolicyFinding(
          name,
          "ElementWitnessTransform.subjectTypes",
          fqn,
          PolicyIssue.Malformed,
      ...
      binder.bindType(name, "ElementWitnessTransform.subjectTypes", fqn).toOption.foreach(s => boundSubjects = boundSubjects.updated(s, idxs.distinct))
    }
    dropBound.toList.sorted.foreach { fqn =>
      binder.bindType(name, "ElementWitnessTransform.dropBound", fqn).toOption.foreach(s => boundUnbound = boundUnbound + s)
    }
    defaultSuppliers.toList.sortBy(_._1).foreach { (key, tmpl) =>
      if !tmpl.contains("{elem}") then
        bad += PolicyFinding(
          name,
          "ElementWitnessTransform.defaultSuppliers",
          key,
          PolicyIssue.Malformed,
      ...
      binder
        .bindMembers(name, "ElementWitnessTransform.defaultSuppliers", key, Ownership.Either)
        .toOption
        .toList
        .flatten
        .flatMap(_.sym)
  ...
  private def refuse(issue: ElementWitnessCheck.Issue, subject: String, detail: String, origin: Origin, unit: SymId): Unit =
  ...
  override def run(program0: Program): Program =
        ...
        applications.foreach { (tc, args) =>
          tparamsOfClass.get(tc).foreach { tps =>
            args.zip(tps).foreach {
              case (TypeRepr.TypeRef(_, p), tp) if dropped(p) && objectBounded(tp) && !dropped(tp) =>
    ...
    /** Which type ARGUMENT positions of a subject class are element positions — the question a method's own type parameter is asked to decide whether it inherits the class's answers.
      */
    ...
    /** Does this method's BODY construct a subject class at its own type parameter `tp`? That is the shape of a generic FACTORY (`Array.of`, `ObjectSet.with`), which the constructor threading cannot
      * see: the clause belongs on the METHOD, not on any class. A RAW `new` — java wrote no type argument — owes the witness just as much, and is recognised where the declaration leaves only one
      * answer for which parameter it means.
      */
    ...
    val allDefs: List[Tree.DefDef] =
      program0.units.flatMap(StandardTraversal.allClassDefs(_)).flatMap(_.body.collect { case d: Tree.DefDef => d })

    /** THE METHODS THAT TAKE THE CLAUSE, to a fixpoint: a generic factory constructing a subject, and then any single-parameter method that CALLS one — threading stops at a call as surely as at a
      * `new`, and the caller has no other way to supply what the callee now asks for.
      */
      ...
      while changed do
        ...
        allDefs.foreach { d =>
          if !acc.contains(d.symbol) && d.tparams.sizeIs == 1 then
            val calls = d.rhs.exists(body =>
              StandardTraversal.scanTerm(body, false) { (a, t) =>
    ...
    /** `scala.Predef.summon[<witness>[<elem>]]`, as TEXT: the clause is ANONYMOUS (a named context parameter shadows an emitted root package, `TirEmitterMembers.givenParam`), so the only way to reach
      * it is resolution. The element's name is the emitted type parameter's own.
      */
    ...
    def elemOf(t: TypeRepr, elems: Set[SymId]): Option[(SymId, TypeRepr)] = t match
      case at @ TypeRepr.AppliedType(TypeRepr.TypeRef(_, arr), List(TypeRepr.TypeRef(_, e))) if elems(e) && program0.symbolOf(arr).exists(_.fullName == "scala.Array") => Some(e -> at)
      case _                                                                                                                                                           => None

    /** one level of the engine's own `asInstanceOf[Array[…]]` peeled off an argument, so the witness's `Array[T]` formal is met by a cast to `Array[T]` and not by one to `Array[Object]`.
      */
    ...
    /** Does this class's CONSTRUCTORS already take a clause of the witness type? Asked of the tree because another phase may have put it there (`GlobalsToImplicitsTransform.requiredGivens`), and a
      * second, minted given would make every construction inside ambiguous.
      */
    val carriesClause: SymId => Boolean =
      val hits = ownedClasses
        .filter { cd =>
          cd.body.exists {
            case d: Tree.DefDef =>
              d.paramss.flatten.exists(v =>
    ...
    final class BodyRewrite(elems: Map[SymId, TypeRepr], owner: String, unit: SymId, dropped: Set[SymId], home: SymId, ownTparams: Set[SymId], threadedHere: Boolean) extends Phase:
      ...
      override def transformTerm(t: Term)(using Program): Term = t match
        ...
        case a @ Tree.Assign(Tree.ArrayAccess(arr, idx, _, _), rhs, _, o, _) if isNullLiteral(rhs) =>
          elemOf(arr.tpe, elemSet) match
            case Some((e, arrTpe)) =>
              Tree.Opaque.spliced(List(s"${summonOf(e)}.${members.nullOut}(", ", ", ")"), List(recast(arr, arrTpe, o), idx), a.tpe, o)
            case None => a
        ...
        case ap: Tree.Apply if ap.args.exists(isSupplierCall) =>
          ...
          ap.copy(
            args = ap.args.zipWithIndex.map { (a, i) =>
              if !isSupplierCall(a) then a
                ...
                slotElement(formals.lift(i)) match
                  case Some(e) =>
                    Tree.Opaque(tmpl.replace("{elem}", program0.symbolOf(e).map(_.name).getOrElse("?")), a.tpe, a.origin)
                  case scala.None =>
                    refuse(
                      ElementWitnessCheck.Issue.UnhandledCreation,
                      owner,
          ...
          head.flatMap(h => elemOf(h.tpe, elemSet).orElse(elemOf(unwrapArrayCast(h).tpe, elemSet)).map(h -> _)) match
            case Some((arr, (e, arrTpe))) if args.sizeIs == 2 && isNullLiteral(args(1)) =>
              ...
              if !isStablePath(arr) then
                ...
                Tree.Opaque(s"${summonOf(e)}.${members.nullOutRange}($h, 0, $h.length)", ap.tpe, o, List(recast(arr, arrTpe, o)))
            case Some((arr, (e, arrTpe))) if args.sizeIs == 4 && isNullLiteral(args(3)) =>
              Tree.Opaque.spliced(List(s"${summonOf(e)}.${members.nullOutRange}(", ", ", ", ", ")"), List(recast(arr, arrTpe, o), args(1), args(2)), ap.tpe, o)
            case _ => ap
        ...
        case Tree.Apply(Tree.Select(l, op, st, so), List(r), m, tp, o) if isRefIdentity(program0, m) && (needsRef(l) || needsRef(r)) =>
      ...
      private def isObjectArray(t: TypeRepr): Boolean = t match
        case TypeRepr.AppliedType(TypeRepr.TypeRef(_, arr), List(el)) if program0.symbolOf(arr).exists(_.fullName == "scala.Array") =>
      ...
      /** Every `new C[…]` of a threaded subject, with a WILDCARD argument filled in: a wildcard is java's raw type, the emitter renders no argument for it, and a constructor that now takes a witness
        * has no way to say which element type it is for. The element types are RECORDED so the enclosing declaration can be given the witness it must supply.
        */
      ...
      /** the ELEMENT type a supplier slot names — `F[Array[E]]` or `F[E]`, `E` one of the element parameters in scope HERE. Anything else is refused rather than guessed.
        */
      ...
      /** A RAW `new C(...)` of a threaded subject, completed with the type arguments the CAST around it already states — java wrote none, and a constructor that now takes a witness has no other way
        * to say which element type it is for. A wildcard argument becomes `java.lang.Object`, which is what java's raw type meant. Then the element type is RECORDED, so the enclosing declaration can
        * be given the witness it must supply.
        */
      ...
      /** the element positions this construction needs a witness at, recorded against the class that must supply it. Nothing is recorded where the declaration is already threaded.
        */
      private def noteWitness(c: SymId, args: List[TypeRepr], o: Origin): Unit =
        if threadedHere || home == SymId.None || carriesClause(home) then ()
        else
          val wanted = elementIndexes(c).flatMap(i => args.lift(i)).flatMap(elementText(_, ownTparams))
      ...
      private def erasedArrayCast(inner: Term, target: TypeRepr, o: Origin): Unit =
        ...
        val fromDropped = inner.tpe match
          case TypeRepr.AppliedType(TypeRepr.TypeRef(_, arr), List(TypeRepr.TypeRef(_, e))) if program0.symbolOf(arr).exists(_.fullName == "scala.Array") => dropped(e)
          case _                                                                                                                                          => false
      ...
      private def creation(inner: Term, e: SymId, arrTpe: TypeRepr, o: Origin): Option[Term] =
        Tree.uncomment(inner) match
          ...
          case na: Tree.NewArray =>
            refuse(
              ...
              s"an array with ${na.dims.size} dimension(s)" +
                ...
                " — the witness allocates one dimension and fills nothing",
              o,
              unit
            )
            None
          case Tree.Apply(_, args, m, _, _) if isArraysMember(program0, m, "copyOf") && args.sizeIs == 2 =>
            Some(
              Tree.Opaque.spliced(List(s"${summonOf(e)}.${members.copyOf}(", ", ", ")"), List(recast(args.head, arrTpe, o), args(1)), arrTpe, o)
            )
          case Tree.Apply(_, args, m, _, _) if isArraysMember(program0, m, "copyOfRange") && args.sizeIs == 3 =>
            Some(
              Tree.Opaque.spliced(
                List(s"${summonOf(e)}.${members.copyOfRange}(", ", ", ", ", ")"),
                List(recast(args.head, arrTpe, o), args(1), args(2)),
                arrTpe,
                o
              )
            )
          case Tree.Apply(_, _, m, _, _) if isReflectiveNewArray(program0, m) =>
            refuse(
              ...
              "the array type is REFLECTED out of a `Class` argument java's own signature carries; " +
                "a witness would have to arrive on a clause this method does not declare",
              o,
              unit
            )
            None
    ...
    def rewriteClass(cd: Tree.ClassDef, unit: SymId): Tree.ClassDef =
      ...
      val mine: Map[SymId, TypeRepr] =
        ...
        else if !program0.symbolOf(cd.symbol).exists(s => scopeAdmits(program0, s)) then Map.empty
        else elementIndexes(cd.symbol).flatMap(i => cd.tparams.lift(i)).map(td => td.symbol -> TypeRepr.TypeRef(TypeRepr.NoPrefix, td.symbol)).toMap
      ...
      if cd.tparams.exists(td => unboundClassTparams(td.symbol)) then
        record(
          Decision(
            ...
            subjectFqn = fqn,
            detail = Map(
              "elements" -> cd.tparams.filter(td => unboundClassTparams(td.symbol)).flatMap(td => program0.symbolOf(td.symbol)).map(_.name).mkString(","),
      ...
      val givens = needsBoxed.get(cd.symbol).toList.flatMap(_.toList).zipWithIndex.map { (elem, i) =>
        ...
        Tree.Opaque(
          s"private given bpMk$i: $witness[$elem] = " +
    ...
    def rewriteStat(s: Statement, classElems: Map[SymId, TypeRepr], owner: String, unit: SymId, inUnbound: Boolean, home: SymId, ownTps: Set[SymId], threadedHere: Boolean): Statement = s match
      ...
      case d: Tree.DefDef   =>
        ...
        val unboundHere = d.tparams.filter(td =>
          inUnbound ||
            feedsSubject(signatureTypes(d), td.symbol, unboundOnly = true) ||
            constructsSubjectAt(d, td.symbol, unboundOnly = true)
        )
        val elems   = classElems ++ threaded.map(td => td.symbol -> TypeRepr.TypeRef(TypeRepr.NoPrefix, td.symbol)).toMap
        val tparams = d.tparams.map(td => if unboundHere.exists(_.symbol == td.symbol) then dropUpperBound(td) else td)
        val body    = d.rhs.map(r =>
          StandardTraversal.mapTerm(
            new BodyRewrite(elems, owner, unit, unboundClassTparams ++ unboundHere.map(_.symbol), home, ownTps, threadedHere || threaded.nonEmpty),
            r
          )(using program0)
        )
        val paramss =
      ...
      case v: Tree.ValDef =>
        v.copy(
          rhs = v.rhs.map(r => StandardTraversal.mapTerm(new BodyRewrite(classElems, owner, unit, unboundClassTparams, home, ownTps, threadedHere), r)(using program0))
        )
      case t: Term =>
        StandardTraversal.mapTerm(new BodyRewrite(classElems, owner, unit, unboundClassTparams, home, ownTps, threadedHere), t)(using program0)
      case other => other
    ...
    val symbols2 = StandardTraversal.mapSymbols(wildcardFill, SymbolTable(symbols1b.all ++ mint.minted))(using program0)
    ...
    val filledClasses: Set[SymId] = tparamsOfClass.collect {
      case (c, tps) if tps.exists(unboundClassTparams) => c
    }.toSet
    ...
    def rawConvert(a: Term, formal: TypeRepr, o: Origin, owner: SymId, unit: SymId): Term =
      (a.tpe, formal) match
        case (TypeRepr.AppliedType(TypeRepr.TypeRef(_, c1), aa), TypeRepr.AppliedType(TypeRepr.TypeRef(_, c2), fa))
          ...
          refuse(
            ElementWitnessCheck.Issue.RawConversion,
            program0.symbolOf(owner).map(_.fullName).getOrElse("?"),
            "java's raw formal, filled with `java.lang.Object` where the java wrote no type argument, takes an argument " +
              "whose type argument differs — the unchecked conversion is written out",
            o,
            unit
          )
    ...
    /** the (index, element) a raw construction's array-supplier argument states for a slot the fill set to `Object`: the argument's LAMBDA BODY (`n => new E[n]`, read through a cast) or its type
      * (`F[Array[E]]`, `F[Int, Array[E]]`), `E` neither `Object` nor a wildcard.
      */
      ...
      def arrayElem(t: TypeRepr): Option[TypeRepr] = t match
        case TypeRepr.AppliedType(TypeRepr.TypeRef(_, arr), List(e))
            if program0.symbolOf(arr).exists(_.fullName == "scala.Array") &&
              !isFilledObject(e) && !e.isInstanceOf[TypeRepr.TypeBounds] =>
          Some(e)
        case _ => scala.None
      ...
      StandardTraversal.allClassDefs(u).foreach { cd =>
        ...
        if elems.nonEmpty then
          ...
          def elemArray(t: TypeRepr): Option[SymId] = t match
            case TypeRepr.AppliedType(TypeRepr.TypeRef(_, arr), List(TypeRepr.TypeRef(_, e))) if elems(e) && program.symbolOf(arr).exists(_.fullName == "scala.Array") => Some(e)
            case _                                                                                                                                                     => scala.None
            ...
            t match
              ...
              case Tree.Apply(Tree.Select(l, _, _, _), List(r), m, _, o)
                ...
                once(ElementWitnessCheck.Issue.OccupancySentinel, o) {
                  refuse(
                    ElementWitnessCheck.Issue.OccupancySentinel,
                    fqn,
                    s"`null` at a `${nameOf(e)}` slot answers IS THIS SLOT EMPTY, so the element " +
                      "type keeps java's `<: java.lang.Object` bound and stays boxed",
                    o,
                    u.symbol
                  )
                }
              ...
              case Tree.Typed(inner, tpt, _, o) if !subject =>
                val creates = Tree.uncomment(inner) match
                  ...
                  case _ => false
                if creates then
                  elemArray(tpt.tpe).foreach { e =>
                    once(ElementWitnessCheck.Issue.NonSubject, o) {
                      refuse(
                        ElementWitnessCheck.Issue.NonSubject,
                        fqn,
  ...
  final private class WitnessMinter(program: Program):
    ...
    /** the clause — ANONYMOUS, for `TirEmitterMembers.givenParam`'s reason: a named context parameter shadows a fully-qualified reference and nothing reads the name. One per (owner, element), so a
      * two-element factory takes two.
      */
    def usingParam(owner: SymId, witnessFqn: String, elem: SymId, at: Origin): Tree.ValDef =
      val w   = tpe(witnessFqn.split('.').last, witnessFqn)
      val tpt = TypeRepr.AppliedType(TypeRepr.TypeRef(TypeRepr.NoPrefix, w), List(TypeRepr.TypeRef(TypeRepr.NoPrefix, elem)))
      val id  = usings.getOrElseUpdate(
        (owner, elem), {
          val s = fresh()
          buf += Symbol(s, "", MemberKey(witnessFqn, "<using>").render, Flags(isParam = true, isGiven = true), owner, tpt)
          s
        }
      )
...
object ElementWitnessTransform:
  ...
  /** `java.lang.reflect.Array.newInstance` — java's only way to allocate an array of a type known at run time, and the one a `Class`-taking factory reaches. Matched on the JDK member; a library's own
    * wrapper is recognised by the member it forwards to at its own call site.
    */
  def isReflectiveNewArray(program: Program, m: SymId): Boolean =
    program
      ...
      .exists(s =>
        s.name == "newInstance" &&
          program.symbolOf(s.owner).exists(o => o.fullName == "java.lang.reflect.Array" || o.name == "ArrayReflection")
      )
  ...
  /** Can this term be READ TWICE without changing what the program does — an identifier, `this`, or a selection through one of those? Anything else is an effect java evaluated once.
    */
