...
object SamLambda:
  ...
  /** WHY a site was declined. A closed enum, because [[IdiomVerdict.Refused]]'s `guard` is the string a reader classifies a row by and an open one would let two sites describe one refusal two ways.
    * Each case's [[why]] says whether the refusal is permanent.
    */
  enum Guard:
  ...
  /** the ANSWER for one site. `Convert` carries everything the rewrite needs, so the transformer never re-reads the tree to find it.
    */
  enum Verdict:
    case Convert(method: Tree.DefDef, iface: String, javaClassName: String)
    case Refuse(guard: Guard, iface: String)

  /** The decision for one `new T(){…}` in one enclosing declaration. Guard order decides which guard a multiply-refused site is attributed to — cheapest fact first.
    */
  ...
  /** `java.lang.Object` is an IMPLICIT parent the tree never writes down, so an ancestry walk over declared parents cannot reach it — kept as a name instead.
    */
  private val JavaLangObject = "java.lang.Object"

  /** The anon's strict ancestry — the SAM interface it named plus every parent this program DECLARES, transitively. Seeded from `nw.tpt` (the anon symbol carries no parent list). An external
    * super-interface is not reached — its members are unknowable, covered by `Guard.Unreadable` instead of guessed. Cycle-safe and fuel-bounded.
    */
  ...
  final private[transform] case class Converted(iface: String, javaClassName: String, method: String, origin: Origin)

  /** does the body CAPTURE an enclosing binding — instance identity, guard 5. Three structural shapes count: `this` naming an ENCLOSING class, a reference to a local/parameter owned by an outer
    * executable, or a reference to a non-static instance member of an enclosing instance. A local declared inside the body is not a capture. "provably evaluated once" is NOT implemented — such sites
    * are counted under [[Guard.NonCapturing]] rather than guessed.
    */
  ...
  /** is `s` an instance member of an ENCLOSING instance ([[captures]]'s third shape)? `static` is read off the referenced symbol, not its owner, so a companion constant is not a capture.
    */
  ...
  /** the name java gave the anonymous class — `Outer$1`. Recorded on the decision: the one thing the lambda cannot carry (`getClass()` differs).
    */
...
final class SamLambdaTransform extends Phase, IdiomPhase:
  ...
  final private class Converter(owner: SamLambdaTransform) extends Phase:
    ...
    /** The conversion happens at the `Apply`, not the `Tree.New`: the frontend models `new I(){…}` as `Apply(New(…, anon), args, <init>)`, and converting at `New` alone would leave the `Apply`
      * standing and emit a lambda applied to the constructor args. A non-empty `args` here is counted (`Guard.ConstructorArgs`) rather than silently dropped.
      */
      ...
      case nw @ Tree.New(_, _, _, Some(anon)) =>
        SamLambda.decide(nw, anon) match
          ...
          case SamLambda.Verdict.Convert(d, iface, javaName) =>
            pending += SamLambda.Converted(iface, javaName, p.symbolOf(d.symbol).map(_.name).getOrElse("?"), nw.origin)
    ...
    /** everything considered since the last member-level definition hook belongs to THIS declaration — conversions and refusals alike; `idiom(refused)` is read per-declaration.
      */
    private def claim(sym: SymId, at: Origin)(using p: Program): Unit =
      ...
      if refused.nonEmpty && isMember(sym) then
        ...
        rs.foreach { (g, iface, o) =>
          owner.consider(IdiomCandidate(IdiomKind.SamLambda, IdiomVerdict.Refused(g.toString, g.why), fqn, s"anonymous `$iface`", o))
        }
      if pending.nonEmpty && isMember(sym) then
        val cs = pending.toList
        pending.clear()
        cs.foreach(c => owner.consider(IdiomCandidate(IdiomKind.SamLambda, IdiomVerdict.Converted, fqn, s"anonymous `${c.iface}` -> lambda", c.origin)))
        owner.record(
          Decision(
            Decision.Kind.SamLambda,
            sym,
            fqn,
            Map(
