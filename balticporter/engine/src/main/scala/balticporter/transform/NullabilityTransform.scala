...
final class NullabilityTransform(
  ...
  /** the lane that counts every site this phase refused, every wrapper seam it could not close, and every retype whose transparency the LANGUAGE does not grant (`Rewrite`).
    */
  def accountedBy: Set[String] = Set(NullabilityBoundaryCheck.Name)

  /** After the collection retypes (they must land first — see the class doc) and before the namespace rename (the annotation FQNs are upstream). Both are declared by NAME and `Pipeline.order` ignores
    * an edge naming a phase this pipeline does not have, so a port that runs neither is unaffected.
    */
  ...
  /** `nullableMembers` entries that actually matched at least one symbol during [[run]], tracked for never-matched reporting. Populated at run time, not at bind time, because earlier phases
  ...
  /** what the RUN knows about itself — which units it EMITS, and which of this (possibly MERGED) instance's keys THIS manifest contributed. Both are needed by [[intrudesOnBase]] and neither is
    * derivable from the `Program`; see [[RunScope]]. The default is the base-port answer.
    */
  private var runScope: RunScope = RunScope.whole

  /** the subjects THIS module contributed to this phase's policy — `None` where this module declares no instance of the phase at all, which is the no-screen answer.
    */
      ...
      (mergedTarget.left.toOption.toList ++ scopeMerged.left.toOption.toList) match
        case Nil =>
          for {
            t <- mergedTarget
            s <- scopeMerged
          } yield MergeablePolicy.Merged(
            new NullabilityTransform(annotations ++ o.annotations, t, s, nullableMembers ++ o.nullableMembers, deriveMembers || o.deriveMembers),
            o.subjects -- subjects
          )
  ...
  /** the NEW type of an annotated occurrence: a value's or parameter's declared type, or a METHOD's RESULT. One map, because a method symbol never also denotes a value.
    */
  private var newTypes: Map[SymId, TypeRepr] = Map.empty

  /** members the reference gives a `@targetName`: their erasure carries that JVM name, so two wrapped overloads told apart by it are no clash (`add` beside `add` -> `addLabel`).
    */
  ...
  /** Type-variable symbols by NAME (a type parameter's `info` is `TypeBounds` — the same test [[mentionsTypeParam]] makes). Captured for the walk, which asks *can this type be written here* without
    * threading a `Program`; the name is kept because a refusal names `T`, not an id.
    */
  ...
  /** the primitive symbol IDs — cached at RUN time so [[coerceTo]] can decide `.get` vs `.orNull` without threading a `Program` through a function passed as a value.
    */
  ...
  private def retypable(p: Program, id: SymId, fuel: Int = 16): Boolean =
    p.owns(id) && {
      def substituted(x: SymId, f: Int): Boolean =
        f > 0 && x != SymId.None && p.symbolOf(x).exists(sym => sym.tags.exists(_.isInstanceOf[balticporter.core.Substituted]) || substituted(sym.owner, f - 1))
      !substituted(id, fuel)
    }
  ...
  override def run(program: Program): Program =
    ...
    program.symbols.all.toList.sortBy(_.id.raw).foreach { s =>
      ...
      if (hits.nonEmpty || memberHit.isDefined) && (retypable(program, s.id) || derivedFact) then
        ...
        // direction matters: an entry EXCLUDES under Everywhere and INCLUDES under Only; quote it either way.
          ...
          if !scope.includes(program, s) then scopedOut(program, s, entry.getOrElse(scope.fingerprint))
          else
            slotOf(program, s) match
              ...
              case Some((slot, was)) =>
                if alreadyNullable(was) then () // idempotent: nothing to do
                  ...
                  if target == Target.Union && mentionsTypeParam(program, was) then refuse(program, s, key, Issue.AbstractTypeParameter)
    ...
    if target != Target.Union then
                ...
                program.symbolOf(param.symbol).foreach { paramSym =>
                  ...
                  if !claimed.contains(paramSym.id) && retypable(program, paramSym.id) && !referenceKeepsPlain then
                    slotOf(program, paramSym) match
                      case Some((Slot.Param, was))
                          if !alreadyNullable(was) &&
      ...
      plan.toList.foreach { x =>
        ...
        val overridable = program.symbolOf(member).exists(m => m.name != "<init>" && !m.flags.isStatic && !m.flags.isFinal)
          ...
          graph.overriders(member).foreach { d =>
            ...
            tgt.foreach { s =>
              if !claimed.contains(s.id) && retypable(program, s.id) && scope.includes(program, s) then
                slotOf(program, s) match
                  case Some((slot, was))
                      if slot == x.slot && !alreadyNullable(was) &&
                        !isPrimitive(program, was) &&
    ...
    /** Annotated parameters by owning method and BY POSITION, never by name: a `MethodType`'s parameter list and the `DefDef`'s are parallel by construction, but names are not — a
      * reassigned-parameter transform can rename a slot without touching `info`.
      */
    val paramsByOwner: Map[SymId, Map[Int, TypeRepr]] =
      plan.iterator
        .filter(_.slot == Slot.Param)
        .toList
        .groupBy(_.sym.owner)
        .flatMap { (owner, ps) =>
    ...
    def methodType(id: SymId, mt: TypeRepr.MethodType): TypeRepr.MethodType =
      ...
      TypeRepr.MethodType(mt.params.zipWithIndex.map((nt, i) => nt._1 -> ps.getOrElse(i, nt._2)), newTypes.getOrElse(id, mt.result), mt.isImplicit)
  ...
  /** Which occurrence of a declaration's type the annotation names, and what it is today. Java's nullability annotations are DECLARATION-position, not `TYPE_USE`: result on a method, declared type on
    * a field/parameter. A TYPE declaration has no such occurrence (`None`, counted refusal).
    */
  ...
  /** The overload sets a wrapper would collapse — refused, at both members, at the positions that carry java's distinction (a wrapper's erasure drops type arguments, so `W[Font]`/ `W[BitmapFont]`
    * collapse to one descriptor, `E120`). Refuses the minimum: planned parameters differing pre-retype, on both members. Head-symbol comparison under-approximates deliberately — a false negative is a
    * loud compile error, a false positive would silently over-refuse.
    */
  private def refuseErasureClashes(p: Program, plan: List[Planned]): List[Planned] =
    ...
    /** the erasure-ish key of a parameter's CURRENT type — its head symbol, or the type itself where there is no head to read.
      */
    ...
    methods.groupBy((m, ps) => (m.owner, m.name + derivedTargetNames.getOrElse(m.id, ""), ps.size)).valuesIterator.foreach { group =>
      ...
      do
        ...
        if collapses then
          differ.foreach { k =>
            List(as(k)._1, bs(k)._1).foreach(id =>
              plannedParam.get(id).foreach { pl =>
  ...
  /** Does this type mention an abstract type parameter anywhere? Decided structurally (a type parameter's `info` is a `TypeBounds`), never from a name. `Foo[T]` counts as much as bare `T`.
    */
  ...
  /** Scala's own primitives — `scala.Int` and friends — cannot be null. Engine identity, not per-library policy: these are the names the frontend interns for java's primitives.
    */
  private def isPrimitive(p: Program, t: TypeRepr): Boolean =
    headSym(t).flatMap(p.symbolOf).exists(s => PrimitiveNames(s.fullName))

  /** Wrapper mode changes a member's SIGNATURE, so it cannot move one end of an override pair alone (union mode can — both narrowing and widening overrides compile without `-Yexplicit-nulls`).
    * Conservative test: a member that overrides, or is overridden, refuses both ends; over-approximates across unrelated hierarchies (safe direction — counted, never silently under-refused).
    */
  ...
  private def overriding(p: Program): Set[(String, Option[Descriptor])] =
    if !overridingRead then
      overridingCache = p.symbols.all.iterator.filter(s => s.flags.isOverride && p.owns(s.id)).map(s => s.name -> s.descriptor).toSet
      overridingRead = true
    overridingCache
  ...
  /** every declared scope entry this run OBSERVED deciding something — the input to [[RuleScope.neverFired]], and the only honest one.
    */
  private val observedEntries = collection.mutable.Set.empty[String]

  /** did the plan loop run? `policyReport` is read whether or not the phase ever ran (the whole point of deriving the never-fired half from the BINDING), and "no entry fired" means nothing before the
    * walk that would have fired them.
    */
  ...
  private def deadScopeFindings: List[PolicyFinding] =
    if !planned then Nil
      ...
      scope.neverFired(observedEntries.toSet).intersect(bound).toList.sorted.map { e =>
        PolicyFinding(
          name,
          s"NullabilityTransform(scope) ${scope.productPrefix} entry",
          e,
  ...
  private def deadMemberFindings: List[PolicyFinding] =
    if !planned || nullableMembers.isEmpty then Nil
      ...
      (nullableMembers -- matchedMembers).toList.sorted.map { e =>
        PolicyFinding(
          name,
          "NullabilityTransform(nullableMembers) entry",
          e,
          PolicyIssue.NeverMatched,
  ...
  private def baseIntrusionFindings: List[PolicyFinding] =
    intrusions.toList.sortBy(_._1).map { (key, subjects) =>
      ...
      PolicyFinding(
        name,
        s"NullabilityTransform(annotations) `$key`",
        key,
  ...
  private def refuse(p: Program, s: Symbol, key: String, issue: Issue): Unit =
    issues += Finding(issue, s.fullName, s"`$key` on ${describe(p, s)}", Decision.originOf(p, s.id), unitOf(p, s.id), declarationOf(p, s))

  /** The declaration a site belongs to (not the same question as [[unitOf]]). A parameter or local climbs ONE level to its enclosing executable (java nests no declaration inside a parameter); the
    * climb stops there rather than falling back to the unit, or one selection could drain every row in a file. Where nothing above is a declaration either, the answer is `SymId.None`.
    */
  ...
  private def describe(p: Program, s: Symbol): String =
    if s.flags.isParam then s"parameter `${s.name}` of ${p.symbolOf(s.owner).map(_.fullName).getOrElse("?")}"
    else s"`${s.fullName}`"

  /** The complement of a retype: the declaration kept its upstream type while the code around it moved, so `decisions.tsv` needs its own row for it. Always `Reason.Configured`. A parameter's FINDING
    * is filed at the parameter (closes the lane's arithmetic) but its DECISION is recorded one level out, at [[declarationOf]] — a parameter carries no note position of its own.
    */
  ...
  private def recordDecisions(before: Program, plan: List[Planned], after: SymbolTable)(using Program): Unit =
    plan.groupBy(p => (declarationOf(p.sym), p.key)).toList.sortBy((k, _) => (k._1.raw, k._2)).foreach { case ((decl, key), ps) =>
  ...
  /** Union mode's one body rewrite: the frontend renders `return null` at a type-parameter return as `null.asInstanceOf[T]` (`T = null` does not type-check at an abstract `T`); once the return is
    * `T | Null` the cast is a placeholder and goes. Narrow on purpose: only a null literal cast to EXACTLY the former return type.
    */
    ...
    if !isWrapper then t
      ...
      val result = nullTest(t).getOrElse {
        val recvFixed = t.fun match
          case f @ Tree.Select(recv, m, _, _)
              if isWrapped(recv) && !isWrapperMember(m)
      ...
      result match
        ...
        case a: Tree.Apply if !newTypes.contains(a.method) && a.tpe != TypeRepr.NoType =>
          ...
          if retWrapped && !isWrapperType(a.tpe) then a.copy(tpe = TypeRepr.AppliedType(TypeRepr.TypeRef(TypeRepr.NoPrefix, wrapperSym), List(a.tpe)))
          else result
        case _ => result
  ...
  /** `.orNull` at every LEAF of a value-producing expression — [[coerceTo]]'s unwrap half where no target type is available to name (lambda bodies going to external SAM results, which java accepts
    * null by default). A primitive CAST target is the exception — unboxing null NPEs.
    */
  ...
  /** the slot an assignment writes: a PROPERTY's setter parameter where one exists (a getter may return the wrapper while its setter takes the plain value, `gl30`/`gl30_=`), else the lhs type.
    */
  private def assignedSlot(lhs: Term)(using p: Program): TypeRepr = lhs match
    case Tree.Select(_, m, tpe, _) =>
      ...
      p.symbolOf(m)
        .filter(_.info match { case TypeRepr.MethodType(Nil, _, _) => true; case _ => false })
        .flatMap(ms =>
          p.symbols.all.find(s => s.owner == ms.owner && s.name == ms.name + "_=").map(_.info).collect { case TypeRepr.MethodType(List((_, pt)), _, _) =>
            pt
          }
        )
        .getOrElse(tpe)
  ...
  /** `W.apply(e)`, or `W.empty`/`None` for a bare `null` — the two directions of the wrap half.
    *
    * For `OptionTarget`, `None` is a top-level object, not a member of `Option` — so it is emitted as `Ident(noneSym)` rather than `Select(Ident(wrapperSym), emptySym)`.
    */
  ...
  /** `e.get` — the unchecked unwrap. NPE on empty IS Java's semantics at a DEREFERENCE (member access, method call, array element/length), and the contract asks for this member for exactly that case.
    * See [[unwrapOrNull]] for the slot-coercion counterpart.
    */
  private def unwrap(e: Term): Term =
    Tree.Select(e, getSym, elementOf(e.tpe), e.origin)

  /** `e.orNull` — the null-preserving unwrap. Java's value flows through unchanged: an empty wrapper becomes `null`, which is what the java slot received. This is the faithful spelling at a SLOT that
    * ACCEPTS null — an unannotated java field, local, parameter, result, or `Object` formal — which is the default in java. See [[slotUnwrap]] for the decision rule and [[unwrap]] for the dereference
    * counterpart.
    */
  ...
  /** Is the target slot a primitive (about the slot's FORMAL, not the annotated declaration — planning already refused annotated primitives). Checked against cached [[primSyms]] so no `Program` is
    * needed, keeping [[coerceTo]] a plain function passable to [[mapReturns]].
    */
  private def isPrimitiveSlot(t: TypeRepr): Boolean =
    headSym(t).exists(primSyms.contains)

  /** the wrapper's own ABSENT value, however this target spells it — `W.empty` for a `Named` wrapper, `None` for `Option`.
    */
    ...
    if isWrapperType(want) && isNullLiteral(e) then wrap(want, e)
      ...
      if isEmptyOfWrapper(e) then e
        ...
        typeVarsIn(want) match
          ...
          case vs  =>
            issues += Finding(
              Issue.UnwritableFormal,
              vs.distinct.mkString(", "),
            ...
            e
    else if !isWrapperType(want) && want != TypeRepr.NoType then
      e match
  ...
  /** Receiver type dropped+injected -- its surface's nullability is not java's `@Null`, so a formal wrapped at the (inherited) callee's OWN declaration must not coerce here. Item 2.
    */
  ...
  private def coerceArgs(t: Tree.Apply)(using p: Program): Term =
    ...
    if retargetted then
      t.args.filter(isWrapped).foreach { a =>
        issues += Finding(
          Issue.UncoercibleSeam,
          p.symbolOf(t.method).map(_.fullName).getOrElse("?"),
  ...
  /** `x == null` / `x != null` on a wrapped value → `x.isEmpty` / `!x.isEmpty`. Not an ergonomic nicety: equality against `null` on an opaque wrapper is a COMPILE ERROR (`E172`, no `CanEqual`), so
    * every java null test on a wrapped value must be rewritten or the port does not build. The rewrite is the portable mechanism (a `CanEqual` given need not exist).
    */
  ...
  private def negate(e: Term, o: Origin): Term =
    Tree.Apply(Tree.Select(e, notSym, TypeRepr.NoType, o), Nil, notSym, TypeRepr.TypeRef(TypeRepr.NoPrefix, boolSym), o)
  ...
  /** is this the ENGINE's own synthetic operator namespace, minted by the frontend under exactly that owner? Engine identity, never a policy key.
    */
  ...
  /** mint-or-reuse: the engine's OWN externals — `scala.Null`, the operator namespace, the configured wrapper — are symbols this program never declares, so there is nothing to bind and the FQN is the
    * only identity there is. Exactly the question `PrimitiveToOpaqueTransform` asks about its underlying primitive.
    */
  private def externalNamed(p: Program, fqn: String): Option[SymId] =
    p.symbols.all.iterator.find(s => s.fullName == fqn).map(_.id)

  final private case class Planned(sym: Symbol, key: String, slot: Slot, was: TypeRepr, hits: List[Annot]):
