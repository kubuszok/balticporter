...
object IdiomCensus:
  ...
  /** Splits `return this` methods by what a `this.type` narrowing would buy: nothing at a self-typed return (precision only on a subclass call) vs. a removed downcast at every chained call on an
    * ancestor-typed one. Published so the go/no-go is a maintainer's, taken on a number.
    */
  enum ReturnShape:
  ...
  /** The population is deliberately NOT "every method" — only one with at least one `return this`; a constructor, a `static`, a body-less declaration, or a non-program return type is outside it and
    * produces no row.
    */
...
final class ReturnThisCensus extends Phase, IdiomPhase:
  ...
  /** the narrowing moves a declaration's return type, so this must run before every retyping phase — the type it compares against must be java's own.
    */
  ...
  private def row(d: Tree.DefDef, shape: IdiomCensus.ReturnShape)(using p: Program): IdiomCandidate =
    ...
    IdiomCandidate(IdiomKind.NarrowedReturn, IdiomVerdict.Refused(shape.toString, shape.why), fqn, "a method that answers `this`", d.origin)

  /** `None` for a declaration outside the population (no row, not a fourth bucket). */
  private def shapeOf(clsSym: SymId, d: Tree.DefDef)(using p: Program): Option[IdiomCensus.ReturnShape] =
