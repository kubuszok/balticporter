...
/** Ships a listed member PUBLIC where java declared it narrower — the hand port's own widening (`FileHandle(File, FileType)` is protected in java, public in the reference). A SIGNATURE fact on the
  ...
  */
final class VisibilityTransform(val widen: Set[String] = Set.empty, val derive: Boolean = false) extends Phase, PolicySource, SurfacePolicy, MergeablePolicy, PolicyBound, Rewrite:
  ...
  def mergedWith(later: Phase): Either[String, MergeablePolicy.Merged] = later match
    case o: VisibilityTransform =>
      Right(
        MergeablePolicy.Merged(new VisibilityTransform(widen ++ o.widen, derive || o.derive), (o.widen -- widen).map(MergeablePolicy.subjectOf))
      )
