...
private[transform] trait CollectionsPolicy:
  ...
  /** Check lanes that count this retyping's residue: [[CollectionClosureCheck]] (unmapped types), [[CollectionBoundaryCheck]] (JDK-side slot seams), [[RetargetBoundaryCheck]] (producer-side).
    */
  ...
  def bindPolicy(binder: PolicyBinder): Unit =
    ...
    boundRetarget = retarget.keys.toList.sorted.map(k => k -> binder.bindType(name, RetargetSetting, k, Ownership.Either)).toMap
    boundCarriers = reifiedCarriers.toList.sorted.map(k => k -> binder.bindType(name, CarrierSetting, k, Ownership.Either)).toMap
    boundSinks = reflectiveSinks.toList.sorted.map(k => k -> binder.bindType(name, SinkSetting, k, Ownership.Either)).toMap
    boundFamilies = families.keys.toList.sorted.map(k => k -> binder.bindType(name, FamilySetting, k, Ownership.Either)).toMap
    substitutedOwners = binder.run.baseSubstitutedOwners ++ binder.run.ownSubstitutedOwners

  /** Fingerprint covering scope, mapping table, retargets, carriers and families. Segments omitted when their parameter is empty, so no baseline moves.
    */
  def surfaceFingerprint: String =
    val parts = List(
      // mapping table digest — rendered unconditionally
      Some("mapping=" + CollectionsTransform.mappingDigest),
      scala.Option.when(retarget.nonEmpty)("retarget=" + retarget.toList.sorted.map((k, v) => s"$k->$v").mkString(",")),
      scala.Option.when(retargetRewrites.nonEmpty || retargetRewritesByDesc.nonEmpty)("retargetRewrites=" + retargetRewritesDigest),
      scala.Option.when(retargetTypeArgs.nonEmpty)("retargetTypeArgs=" + retargetTypeArgsDigest),
      scala.Option.when(reifiedCarriers.nonEmpty)("carriers=" + reifiedCarriers.toList.sorted.mkString(",")),
      scala.Option.when(families.nonEmpty)("families=" + familiesDigest),
      scala.Option.when(retargetIndexedFields.nonEmpty)("retargetIndexedFields=" + retargetIndexedFieldsDigest)
    ).flatten
  ...
  /** Merge `later` into this. A dependent ADDS families and retargets; same source with a different target refuses with the phase's sentence. Scopes on the SAME family source must agree.
    */
  def mergedWith(later: Phase): Either[String, MergeablePolicy.Merged] = later match
    case o: CollectionsTransform =>
      ...
      val scopeClash = (families.keySet & o.families.keySet).toList.sorted.filter(k => familyScopeOf(k) != o.familyScopeOf(k))
      // --- scope clashes on retargets both sides declare ---
      val retargetScopeClash = (retarget.keySet & o.retarget.keySet).toList.sorted.filter(k =>
        scope.entries.intersect(Set(retarget(k))).nonEmpty !=
          o.scope.entries.intersect(Set(o.retarget(k))).nonEmpty
      ) // rough — retargets do not have per-entry scopes
      // --- retargetRewrites clashes: same source FQN, different rewrite table ---
      val rewriteClash     = (retargetRewrites.keySet & o.retargetRewrites.keySet).filter(k => retargetRewrites(k) != o.retargetRewrites(k))
      val descRewriteClash = (retargetRewritesByDesc.keySet & o.retargetRewritesByDesc.keySet).filter(k => retargetRewritesByDesc(k) != o.retargetRewritesByDesc(k))
      // --- retargetTypeArgs clashes: same source FQN, different arg mapping ---
      val typeArgsClash = (retargetTypeArgs.keySet & o.retargetTypeArgs.keySet).filter(k => retargetTypeArgs(k) != o.retargetTypeArgs(k))
      // --- retargetIndexedFields clashes ---
      val idxFieldClash = (retargetIndexedFields.keySet & o.retargetIndexedFields.keySet).filter(k => retargetIndexedFields(k) != o.retargetIndexedFields(k))
      ...
      if retargetClash.nonEmpty || familyClash.nonEmpty || crossClash.nonEmpty ||
        ...
        typeArgsClash.nonEmpty || idxFieldClash.nonEmpty
      then
        Left(
          (retargetClash.toList.sorted.map(k => s"""both modules retarget "$k", to "${retarget(k)}" and "${o.retarget(k)}"""") ++
            ...
            crossClash.toList.sorted.map(k => s""""$k" appears in families on one side and retarget on the other — two answers for one type""") ++
            scopeClash.map(k =>
            ...
            rewriteClash.toList.sorted.map(k => s"""both modules declare retarget rewrites for "$k" and disagree""") ++
            descRewriteClash.toList.sorted.map(k => s"""both modules declare descriptor-keyed retarget rewrites for "$k" and disagree""") ++
            typeArgsClash.toList.sorted.map(k => s"""both modules declare retarget type args for "$k" and disagree""") ++
            idxFieldClash.toList.sorted.map(k => s"""both modules declare retarget indexed fields for "$k" and disagree""")).mkString("; ") +
  ...
  /** the RuleScope a FAMILY ENTRY declares — `Everywhere(Set.empty)` (the pre-scope code path) when the source FQN has no explicit scope. JDK entries are not scoped through this map; they use the
    * phase-level `scope` as they always have.
    */
  def familyScopeOf(from: String): RuleScope = familyScopes.getOrElse(from, RuleScope.everywhere)

  /** digest of the `families` table — sorted by source FQN, carrying the target and kind. The scope is part of the fingerprint too (a scope difference is a surface difference), so it joins the sorted
    * string. Used only when `families.nonEmpty`.
    */
  private[transform] def familiesDigest: String =
    balticporter.tir.TirPrinter.sha256(families.toList.map((k, v) => s"$k->${v._1}:${v._2};scope=${familyScopeOf(k).fingerprint}").sorted.mkString(",")).take(16)

  /** digest of the `retargetRewrites` + `retargetRewritesByDesc` tables — sorted by source FQN then by key rendering. Arity-keyed entries render as `src#name/arity->Rw`, descriptor-keyed as
    * `src#name/(desc)->Rw`. The two maps are combined so one digest covers both.
    */
  private[transform] def retargetRewritesDigest: String =
    def renderRw(rw: CollectionsTransform.RetargetRewrite): String = rw match
      ...
      case CollectionsTransform.RetargetRewrite.IndexedField(f, v, vw) =>
        val extra = List(scala.Option.when(v != "apply")(s"via=$v"), scala.Option.when(vw != "update")(s"viaWrite=$vw")).flatten
  ...
  /** digest of the `retargetTypeArgs` table — sorted by source FQN, each arg rendered as `arg(i)`, `fixed(fqn)`, or `applied(fqn,args)`. Used only when `retargetTypeArgs.nonEmpty`.
    */
  private[transform] def retargetTypeArgsDigest: String =
    ...
    balticporter.tir.TirPrinter
      .sha256(
        retargetTypeArgs.toList
          .sortBy(_._1)
          .map { (src, args) =>
  ...
  /** Digest of [[retargetIndexedFields]]. Sorted by (source FQN, field name); renders each entry the same way as [[retargetRewritesDigest]] renders an `IndexedField`.
    */
    ...
    val entries = retargetIndexedFields.toList.sortBy(_._1).flatMap { (src, tbl) =>
      tbl.toList.sortBy(_._1).map { case (fld, idx) =>
        val extra = List(
          ...
          scala.Option.when(idx.viaWrite != "update")(s"viaWrite=${idx.viaWrite}")
        ).flatten
  ...
  private[transform] def applyScope(p: Program): Unit =
    ...
    val externalOnly = p.symbols.all.iterator.filterNot(s => owned(s.id)).flatMap(s => RuleScope.longestPrefix(s.fullName, scope.entries)).toSet -- fired
    ...
    report = PolicyReport(
      scope.neverFired(fired).toList.sorted.map { k =>
        PolicyFinding(
          name,
          s"CollectionsTransform(scope) ${scope.productPrefix} entry",
          k,
  ...
  /** could this phase retype what this symbol DECLARES? A value's own type, or a method's RESULT — the two positions a pure-move flow moves a collection through.
    */
  ...
  private[transform] def isMapped(p: Program, t: TypeRepr): Boolean =
    headSym(t).flatMap(p.symbolOf).exists(s => typeMap.contains(s.fullName) || effectiveRetarget.contains(s.fullName))
  ...
  /** the nearest ancestor-or-self that is a MEMBER of a type rather than of a method — the unit a body is rewritten in. A propagated parameter or local pulls this in with it, because half a rewritten
    * body is not a translation.
    */
  ...
  /** Restores scoped-out members by splicing originals back into the mapped unit. Empty `excluded` returns the unit untouched; an excluded class restores only its own positions (parents, tparams)
    * without short-circuiting its body.
    */
  ...
  /** `StandardTraversal.mapSymbols`, minus the symbols the scope held back, so an excluded declaration's signature stays exactly as the frontend read it — matching the restored tree.
    */
