...
/** A method's `Class<T>` parameter, `T` its own type parameter, becomes a `ClassTag[T]` context clause: the body reads the class off the tag, an owned call passing `X.class` names `[X]`. Whole
  ...
  */
final class ClassTagParamsTransform(val members: Set[String] = Set.empty, val derive: Boolean = false) extends Phase, PolicySource, SurfacePolicy, MergeablePolicy, PolicyBound, Rewrite:
  ...
  def mergedWith(later: Phase): Either[String, MergeablePolicy.Merged] = later match
    case o: ClassTagParamsTransform =>
      Right(
        MergeablePolicy.Merged(new ClassTagParamsTransform(members ++ o.members, derive || o.derive), (o.members -- members).map(MergeablePolicy.subjectOf))
      )
  ...
  def bindPolicy(binder: PolicyBinder): Unit =
    bound = members.toList.sorted.flatMap(k => binder.bindMembers(name, "ClassTagParamsTransform(members)", k).toOption.getOrElse(Nil).flatMap(_.sym)).toSet
  ...
  override def run(program: Program): Program =
    ...
    /** the class parameters of every candidate component: a call passing one of them (a converted method delegating to another) is determined by that parameter's own type parameter, whose tag is in
      * scope there. Shrinks as components refuse (the fixpoint below).
      */
    ...
    while !stable do
      ...
      targets.toList.sortBy(_.raw).foreach { t =>
        if !seen(t) then
          ...
          if closure.isAnchored then refusedComps += (t -> closure.anchorReason(program).getOrElse("the override component reaches a declaration this program cannot move"))
        ...
        plans.get(d.symbol).fold(d) { plan =>
          ...
          val clause  = plan.params.map { (_, tp) =>
            Tree.ValDef(
              mint.member(
                "",
                MemberKey(p.symbolOf(d.symbol).map(_.fullName).getOrElse("?"), "<using-classtag>").render + "/" + tp.raw,
                d.symbol,
                ctOf(tp),
                Flags(isParam = true, isGiven = true)
              ),
              TypeTree(ctOf(tp), at),
              scala.None,
              at
          ...
          val binds = plan.params.map { (v, tp) =>
            ...
            Tree.ValDef(
              v,
              TypeTree(TypeRepr.AppliedType(TypeRepr.TypeRef(TypeRepr.NoPrefix, cls), List(TypeRepr.TypeRef(TypeRepr.NoPrefix, tp))), at),
              Some(classOfTag(tp, at)),
              at
            )
          }
          val rhs = d.rhs.map {
