...
final class TypeRedirectTransform(
  ...
  /** The scope of one redirect entry — absent means `RuleScope.everywhere`, the phase's pre-scope behaviour.
    */
  ...
  /** The member renames, parsed and bound `Owned` (unlike the type above: a rename rewrites a declaration, and an interned-external-only key has nothing to rewrite).
    */
  private var boundRenames: List[TypeRedirectTransform.Rename] = Nil

  /** This phase's own findings — a malformed member segment, a `memberRenames` block for a type that is not redirected, and every counted refusal the run makes.
    */
  private var ownFindings: List[PolicyFinding] = Nil

  def bindPolicy(binder: PolicyBinder): Unit =
    bound = redirects.keys.toList.sorted.map(k => k -> binder.bindType(name, "TypeRedirectTransform(redirects) source", k, Ownership.Either)).toMap
    ...
    boundRenames = memberRenames.toList.sortBy(_._1).flatMap { (from, renames) =>
      redirects.get(from) match
        ...
        case scala.None =>
          bad += PolicyFinding(
            name,
            "TypeRedirectTransform(memberRenames)",
            from,
        ...
        case Some(to) =>
          renames.toList.sortBy(_._1).flatMap { (member, newName) =>
            MemberKey.parseIn(from, member) match
              case Left(m) =>
                bad += PolicyFinding(name, s"TypeRedirectTransform(memberRenames) of `$from`", MemberKey.spell(from, member), PolicyIssue.Malformed, m.what)
                Nil
              case Right(_) if newName.isEmpty =>
                bad += PolicyFinding(
                  name,
                  s"TypeRedirectTransform(memberRenames) of `$from`",
                  MemberKey.spell(from, member),
                  PolicyIssue.Malformed,
              ...
              case Right(mk) =>
                ...
                val hits = binder.bindMembers(name, s"TypeRedirectTransform(memberRenames) of `$from`", entry).toOption.getOrElse(Nil).flatMap(_.sym)
  ...
  /** Re-pointing a type changes emitted signatures, so it is shared surface; a member rename moves the signature too and is rendered here, while an entry with no renames spells exactly what it always
    * did so old configs still compare equal.
    */
  def surfaceFingerprint: String = redirects.toList.sorted
    .map { (f, t) =>
  ...
  def mergedWith(later: Phase): Either[String, MergeablePolicy.Merged] = later match
    case o: TypeRedirectTransform =>
      ...
      val scopeClash = (redirects.keySet & o.redirects.keySet).toList.sorted.filter(k => scopeOf(k) != o.scopeOf(k))
      if typeClash.nonEmpty || memberClash.nonEmpty || scopeClash.nonEmpty then
        Left(
          (scopeClash.map(k =>
            ...
            typeClash.toList.sorted.map(k => s"""both modules redirect "$k", to "${redirects(k)}" and "${o.redirects(k)}"""") ++
  ...
  /** Declared sources that occur nowhere, plus this phase's own malformed entries and counted rename refusals.
    */
  def policyReport: PolicyReport =
    PolicyReport.fromBindings(records) ++ PolicyReport(ownFindings ++ runFindings) ++
      PolicyReport(
        scopes.toList.sortBy(_._1).flatMap { (from, sc) =>
          sc.neverFired(scopeFired.getOrElse(from, Set.empty))
            .toList
            .sorted
            .map(k =>
              PolicyFinding(
                name,
                s"""TypeRedirectTransform(scope) of "$from", ${sc.productPrefix} entry""",
                k,
  ...
  /** Per redirect source, the scope entries that placed at least one unit this run — the complement of `RuleScope.neverFired`.
    */
  private var scopeFired: Map[String, Set[String]] = Map.empty

  /** Is this declaration one the given scope admits, asked through the owner chain? An unresolvable symbol takes each direction's conservative arm — IN for `Everywhere`, OUT for `Only`.
    */
  ...
  override def run(program0: Program): Program =
    ...
    /** The scope governing each mapped source symbol — the entry's own, so a base's whole-program redirect and a dependent's package-scoped one can sit in the merged instance together.
      */
    ...
    redirects.toList.sorted.foreach { (from, to) =>
      bound.get(from).flatMap(_.toOption).foreach { fromId =>
        val sites = (fromId :: membersOf.getOrElse(fromId, Nil))
          .flatMap(Decision.declarationsUsing(program, _))
  ...
  /** Rename every declaration of each named member's whole override component to what the target calls it, then hand the rewritten program to the redirect. Three counted refusals: the target must
    * declare the new name where known; the component must be movable (`MemberRenamer`'s own screen); and the new name must be free (`OnCollision.Refuse` — the requested name IS the target's, so a
    * `$`-suffixed landing spot would defeat the first screen).
    */
  ...
  /** Does the redirect target declare the name this entry asks for? `None` when it does, or when nothing is known about the target (the target compiler stays the gate). The signature checked is the
    * member's own with the new name substituted, so an arity mismatch is caught too.
    */
  ...
  private def refuseRename(r: TypeRedirectTransform.Rename, why: String): Unit =
    runFindings = runFindings :+ PolicyFinding(
      name,
      s"TypeRedirectTransform(memberRenames) of `${r.source}`",
      r.key,
  ...
  /** A type also occurs as a TERM: a static access's receiver is an `Ident` of the type's own symbol, rendered by the emitter from that symbol rather than the node's `TypeRepr`, so `transformType`
    * alone would leave a partial redirect at every static reference. Only type symbols are in `mapping`, so a same-named term cannot be caught by accident.
    */
  ...
  /** The member half: for a PARSED redirected type the emitter re-derives a static access from the member symbol's owner, undoing a qualifier-only rewrite — so this points at the twin instead,
...
object TypeRedirectTransform:
  ...
  /** One declared member rename, parsed and bound.
    * @param source
    *   the redirected type @param target what it redirects TO @param entry the bound member key (`owner#name`) @param newName the target's name for it @param hits every declaration the key named —
