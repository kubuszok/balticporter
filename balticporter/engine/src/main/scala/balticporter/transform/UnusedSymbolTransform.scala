  ...
  override def run(program: Program): Program =
    ...
    def classifyPrivateMember(v: Tree.ValDef, anon: Boolean = false): Unit =
      program.symbolOf(v.symbol).foreach { s =>
        ...
        else if isSubstitutionReferenced(s) then refused += ((s.fullName, "substituted-body-reference", v.origin))
        else if s.name == "serialVersionUID" then toSuppress(v.symbol) = "msg=unused"
        ...
        else if isWriteOnly(v.symbol) && s.flags.isMutable then toSuppress(v.symbol) = "msg=not read"
      }
    ...
    def classifyPrivateDef(d: Tree.DefDef): Unit =
      program.symbolOf(d.symbol).foreach { s =>
        if s.flags.isPrivate && !s.flags.isParam && isUnreferenced(d.symbol) &&
          ...
          !Set("equals", "hashCode", "toString", "clone", "finalize").contains(s.name)
        then
          if isSubstitutionReferenced(s) then refused += ((s.fullName, "substituted-body-reference", d.origin))
    ...
    def classifyNonPrivateDef(d: Tree.DefDef): Unit =
      program.symbolOf(d.symbol).foreach { s =>
        if !s.flags.isPrivate && !s.flags.isParam && s.name != "<init>" &&
          !s.flags.isOverride && isUnreferenced(d.symbol)
        then refused += ((s.fullName, "non-private member", d.origin))
      }
            ...
            case d: Tree.DefDef =>
              ...
              program.symbolOf(d.symbol).foreach { s =>
                if !s.flags.isPrivate && !s.flags.isOverride && !s.flags.isParam && s.name != "<init>" &&
                  isUnreferenced(d.symbol)
                then toSuppress(d.symbol) = "msg=unused"
              }
            case _ => ()
          }
        }
    ...
    toSuppress.toList.sortBy(_._1.raw).foreach { (id, msg) =>
      program.symbolOf(id).foreach { s =>
        ...
        val d = Decision(
          ...
          subject = id,
          subjectFqn = s.fullName,
          detail = Map("action" -> "suppressed", "symbol-kind" -> symbolKind, "annotation" -> s"""@nowarn("$msg")""", "why" -> why),
          reason = Reason.Universal("unused-symbol"),
          origin = Decision.originOf(program, id)
        )
        record(d)
    ...
    def nowarnAnnot(msg: String): Annot = Annot(
      ...
      args = List("value" -> Tree.Literal(Constant.StringC(msg), TypeRepr.TypeRef(TypeRepr.NoPrefix, SymId.None), Origin.synthetic)),
      origin = Origin.synthetic
    )

    val alreadyAnnotated = program.symbols.all
      .filter { s =>
        s.annotations.exists(a =>
          program
            .symbolOf(a.tpe match {
    ...
    if toAnnotate.isEmpty then return program.rebuilt(units, program.symbols)
    ...
    else updated ++ List(Symbol(nowarnSym, "nowarn", "scala.annotation.nowarn", Flags(), SymId.None, TypeRepr.NoType))
...
object UnusedSymbolTransform:
  ...
  def symbolKindOf(s: Symbol): String =
    if s.flags.isPrivate then s"unused private ${if s.flags.isMutable then "var" else if s.descriptor.isDefined then "def" else "val"}"
    else s"unused local ${if s.flags.isMutable then "var" else "val"}"
  ...
  def isSideEffectFreeTerm(t: Term, opName: SymId => String = _ => ""): Boolean = t match
    ...
    case Tree.Apply(Tree.Select(l, op, _, _), args, _, _, _) if args.sizeIs <= 1 && pureOperators(opName(op)) => (l :: args).forall(isSideEffectFreeTerm(_, opName))
    case _                                                                                                    => false

  private val pureOperators = Set("+", "-", "*", "<<", ">>", ">>>", "&", "|", "^", "<", ">", "<=", ">=", "==", "!=", "&&", "||", "unary_-", "unary_+", "unary_!", "unary_~")
