...
final class AddMembersTransform(val members: Map[String, List[AddMembersTransform.MemberSpec]] = Map.empty, val fromReference: Map[String, List[String]] = Map.empty)
  ...
  def bindPolicy(binder: PolicyBinder): Unit                      =
    runScope = binder.run
    boundRef = fromReference.keys.toList.sorted.flatMap(o => binder.bindType(name, "AddMembersTransform(fromReference)", o).toOption.map(o -> _)).toMap
    records = binder.recordsFor(name)

  /** Fingerprint: owner -> sorted member names; a reference-read name is marked `@ref`. Empty map = empty string = omitted segment.
    */
  def surfaceFingerprint: String =
    val own = members.toList.sortBy(_._1).map((o, ms) => s"$o=${ms.map(m => s"${m.name}/${m.arity}${if m.static then "!" else ""}").sorted.mkString(",")}")
    val ref = fromReference.toList.sortBy(_._1).map((o, ns) => s"$o=${ns.sorted.map(_ + "@ref").mkString(",")}")
    (own ++ ref).mkString(";")

  /** Independent owners UNION; same owner+name REFUSES — two different members at the same declaration is a conflict only a human can resolve.
    */
        ...
        if !ownerFound.contains(owner) then
          List(
            PolicyFinding(
              name,
              "AddMembersTransform",
              MemberKey(owner, s.name).render,
              PolicyIssue.NeverMatched,
              s"no type '$owner' in this program"
            )
          )
        else Nil
      }
  ...
  /** `fromReference` resolved against the reference tree: owner FQN -> the specs it yields; a name the reference does not declare, or a run with no reference, is a counted finding.
    */
  private def referenceSpecs(program: Program): Map[String, List[MemberSpec]] =
    ...
    val specs = fromReference.toList
      .sortBy(_._1)
      .flatMap { (owner, names) =>
        ...
        (sym, runScope.referenceSource) match
          ...
          case (_, scala.None) =>
            names.foreach(n =>
              out += PolicyFinding(
                name,
                "AddMembersTransform(fromReference)",
                MemberKey(owner, n).render,
                PolicyIssue.Unverifiable,
                "this run has no reference port (`parity` undeclared), so there is no tree to read the member from"
              )
            )
            Nil
          case (Some(s), Some(lookup)) =>
            ...
            names
              .filterNot(n => found.exists(_.name == n))
              .foreach(n =>
                out += PolicyFinding(
                  name,
                  "AddMembersTransform(fromReference)",
                  MemberKey(owner, n).render,
                  PolicyIssue.NeverMatched,
                  s"the reference declares no `$n` on the type standing for `$owner`"
                )
              )
            val ms = found.map { m =>
              MemberSpec(
                m.name,
                0,
                (m.imports :+ m.source).mkString("\n"),
                Reason.Configured("add-members", MemberKey(owner, m.name).render),
                Some(s"the reference port's own ${m.kind} `${m.name}`, spliced verbatim from its tree"),
                m.static
              )
            }
      ...
      val appended = byOwner.get(cd.symbol) match
        case Some(specs) =>
          specs.map { s =>
            ...
            Tree.Opaque(s.source, TypeRepr.NoType, Origin.synthetic, Nil, Option.when(s.static)(s.name))
          }
        case _ => Nil
