...
final class ClassToTraitTransform(
  ...
  def mergedWith(later: Phase): Either[String, MergeablePolicy.Merged] = later match
    case o: ClassToTraitTransform =>
      ...
      yield fqn
      if clashes.nonEmpty then Left(clashes.sorted.mkString("both modules declare `class-to-trait` for ", ", ", " with DIFFERENT param mappings"))
      else
        Right(
          MergeablePolicy.Merged(new ClassToTraitTransform(specs ++ o.specs), (o.specs.keySet -- specs.keySet).map(MergeablePolicy.subjectOf))
        )
  ...
  override def run(program: Program): Program =
    ...
    yield
      ...
      val widestCtor = parentCd.toList.flatMap { cd =>
        cd.body.collect { case d: Tree.DefDef if program.symbolOf(d.symbol).exists(_.name == "<init>") => d }.maxByOption(c => CtorFunnel.valueParams(program, c).size)
      }.headOption
      ...
      val defaults    = parentCd.toList.flatMap { cd =>
        ...
        ctors.find(c => CtorFunnel.valueParams(program, c).isEmpty).toList.flatMap { nc =>
          CtorFunnel.stmtsOf(nc).headOption.collect { case t: Term => Tree.uncomment(t) }.toList.flatMap {
            case Tree.Apply(Tree.Select(_, m, _, _), args, _, _, _) if program.symbolOf(m).exists(_.name == "<init>") => args
            case _                                                                                                    => Nil
          }
        }
      }
      val ownedFields = program.symbols.all
        .filter { s =>
        ...
        allCtors.flatMap { ctor =>
          ...
          if arity >= widestArity then None // the widest itself needs no expansion
            ...
            CtorFunnel.stmtsOf(ctor).headOption.collect { case t: Term => Tree.uncomment(t) }.flatMap {
              case Tree.Apply(Tree.Select(r, m, _, _), delegArgs, _, _, _) if program.symbolOf(m).exists(_.name == "<init>") && !r.isInstanceOf[Tree.Super] && delegArgs.nonEmpty =>
    ...
    val allSyms        = finalSyms.all.map { s =>
      if resolved.contains(s.id) then s.copy(flags = s.flags.copy(isTrait = true, isAbstract = false))
    ...
    val abstractVals = spec.mappings.flatMap { m =>
      if m.index < spec.formalTypes.size then
        ...
        newSyms += Symbol(
          defId,
          m.valName,
          s"${program.symbolOf(cd.symbol).map(_.fullName).getOrElse("")}#${m.valName}",
          Flags(isAbstract = true, isProtected = true),
          cd.symbol,
          tpe,
          origin
        )
  ...
  private def rewriteNamedSubclass(cd: Tree.ClassDef, spec: ResolvedSpec)(using program: Program): Tree.ClassDef =
    ...
    val ctorsWithSuperArgs = ctors.map(c => c -> CtorFunnel.superArgsOf(program, c)).filter(_._2.nonEmpty)
    ...
    val rewrittenBody: List[Statement] = if multipleRootsCallSuper then
      ...
      cd.body.map {
        case d: Tree.DefDef
            if program.symbolOf(d.symbol).exists(_.name == "<init>") &&
    ...
    val widestWithArgs2 = ctors2.map(c => c -> CtorFunnel.superArgsOf(program, c)).filter(_._2.nonEmpty).maxByOption(_._2.size)
    ...
    val overrideVals = widestWithArgs2 match
      case Some((_, superArgs)) =>
        spec.mappings.flatMap { m =>
          ...
          else if m.index < spec.defaults.size && m.index < spec.formalTypes.size then Some(mkVal(m, spec.formalTypes(m.index), Some(spec.defaults(m.index)), origin, cd.symbol, cdSym))
          else None
        }
      case None =>
        if spec.defaults.size < spec.mappings.size then return cd
        spec.mappings.flatMap { m =>
          if m.index < spec.defaults.size && m.index < spec.formalTypes.size then Some(mkVal(m, spec.formalTypes(m.index), Some(spec.defaults(m.index)), origin, cd.symbol, cdSym))
          else None
        }
              ...
              case Some(anon) =>
                ...
                val overrideVals = if app.args.nonEmpty then
                  spec.mappings.flatMap { m =>
                    ...
                    else if m.index < spec.defaults.size && m.index < spec.formalTypes.size then Some(mkVal(m, spec.formalTypes(m.index), Some(spec.defaults(m.index)), origin, anon.symbol, anonSym))
                    else None
                  }
                else if spec.defaults.size >= spec.mappings.size then
                  spec.mappings.flatMap { m =>
                    if m.index < spec.defaults.size && m.index < spec.formalTypes.size then Some(mkVal(m, spec.formalTypes(m.index), Some(spec.defaults(m.index)), origin, anon.symbol, anonSym))
                    else None
                  }
                else Nil
  ...
  /** Fix a RAW `new Pool[?](a,b) { ... }.asInstanceOf[Pool[T]]`: the `New` node's `tpt` carries wildcards, making the anonymous class `Pool[Nothing]` and every abstract member unreachable, while the
    * `Typed` wrapper carries the real type argument. Replaces the wildcard args with the `Typed` target's concrete ones, narrows `Object`-returning overrides, drops the wrapper.
    */
              ...
              case Some(_) =>
                ...
                val targetArgs = castTpt.tpe match
                  case TypeRepr.AppliedType(tc, args)
                      if headSymOf(tc) == headSym &&
                        !args.exists(_.isInstanceOf[TypeRepr.TypeBounds]) =>
                    Some(args)
                  case _ => None
                        ...
                        val fixedBody = anon.body.map {
                          case d: Tree.DefDef if !program.symbolOf(d.symbol).exists(_.name == "<init>") =>
                            val isObject = d.returnTpt.tpe match
                              case TypeRepr.TypeRef(_, s) => program.symbolOf(s).exists(n => n.fullName == "java.lang.Object" || n.name == "Object")
                              case _                      => false
                            if isObject then d.copy(returnTpt = d.returnTpt.copy(tpe = actualTpe))
                            else d
                          case other => other
                        }
  ...
  /** Rewrite a narrower root's `super(args)` into `this(expanded_args)` targeting the widest constructor of THIS class (`targetCtor`), using the parent's own delegation chain.
    */
  ...
  private def rewriteSuperCallToThis(d: Tree.DefDef, newArgs: List[Term], targetCtor: SymId)(using program: Program): Tree.DefDef =
    ...
    stmts.headOption.collect { case t: Term => t } match
      ...
      case Some(head) =>
        ...
        unwrapped match
          case Tree.Apply(Tree.Select(sup: Tree.Super, m, sel, selOrigin), _, method, tpe, appOrigin) if program.symbolOf(m).exists(_.name == "<init>") =>
            ...
            val newBody: Term = newStmts match
              ...
              case _                                      =>
                ...
                Tree.Block(newStmts.init.collect { case s: Statement => s }, last, d.rhs.map(_.tpe).getOrElse(TypeRepr.NoType), d.origin, trailing)
  ...
  /** ResolvedSpecs for classes extending one REWRITTEN by the main pass (step 2): no mappings (no override vals), only the delegation chain data the multi-root rewrite needs.
    */
  private def buildTransitiveSpecs(p: Program): Map[SymId, ResolvedSpec] =
    ...
    p.units.flatMap(u => StandardTraversal.allClassDefs(u)(using p)).foreach { cd =>
      val parentSym = cd.parents.iterator
        .flatMap {
      ...
      parentSym match
        case Some(ps) if !rewrittenSyms.contains(ps) && !resolved.contains(ps) =>
          ...
          val parentCd          = p.units.flatMap(u => StandardTraversal.allClassDefs(u)(using p)).find(_.symbol == ps)
          val parentIsRewritten = parentCd.exists { pcd =>
            pcd.parents.iterator
              .flatMap {
                case tt: TypeTree => ClassToTraitTransform.headSymOf(tt.tpe)
                case t:  Term     => ClassToTraitTransform.headSymOf(t.tpe)
              }
            ...
            parentCd.foreach { pcd =>
              ...
              if widestArity > 0 then
                ...
                val defaults = ctors.find(c => CtorFunnel.valueParams(p, c).isEmpty).toList.flatMap { nc =>
                  CtorFunnel.stmtsOf(nc).headOption.collect { case t: Term => Tree.uncomment(t) }.toList.flatMap {
                    case Tree.Apply(Tree.Select(_, m, _, _), args, _, _, _) if p.symbolOf(m).exists(_.name == "<init>") => args
                    case _                                                                                              => Nil
                  }
                }
                val ctorDelegations: Map[Int, (List[SymId], List[Term])] = ctors.flatMap { ctor =>
                  ...
                  if arity >= widestArity then None
                    ...
                    CtorFunnel.stmtsOf(ctor).headOption.collect { case t: Term => Tree.uncomment(t) }.flatMap {
                      case Tree.Apply(Tree.Select(r, m, _, _), delegArgs, _, _, _) if p.symbolOf(m).exists(_.name == "<init>") && !r.isInstanceOf[Tree.Super] && delegArgs.nonEmpty =>
  ...
  private def mkVal(m: ParamMapping, tpe: TypeRepr, rhs: Option[Term], origin: Origin, owner: SymId, ownerSym: Symbol): Tree.ValDef =
    val valId = fresh()
    newSyms += Symbol(valId, m.valName, s"${ownerSym.fullName}#${m.valName}", Flags(isOverride = true, isProtected = true), owner, tpe, origin)
  ...
  private def stripSuperArgs(d: Tree.DefDef)(using program: Program): Tree.DefDef =
    ...
    CtorFunnel.headStmt(d) match
      case Some(Tree.Apply(Tree.Select(_: Tree.Super, m, _, _), args, _, _, _)) if program.symbolOf(m).exists(_.name == "<init>") && args.nonEmpty =>
        val newHead: Term = stmts.head match
          case t: Term =>
            stripTopSuperArgs(Tree.uncomment(t))(using program) match
        ...
        val origExpr: Option[Term] = d.rhs
          .map {
        ...
        val newBody: Term = origExpr match
          case Some(e) =>
            Tree.Block(newStmts.collect { case s: Statement => s }, e, d.rhs.map(_.tpe).getOrElse(TypeRepr.NoType), d.origin, trailing)
          case None =>
  ...
  private def stripTopSuperArgs(t: Term)(using program: Program): Option[Term] = t match
    case Tree.Apply(sel @ Tree.Select(_: Tree.Super, m, _, _), args, method, tpe, origin) if program.symbolOf(m).exists(_.name == "<init>") && args.nonEmpty =>
...
object ClassToTraitTransform:
  ...
  final private[transform] case class ResolvedSpec(
    fqn:         String,
    ...
    parentSymId: SymId,
    /** For each non-widest parent constructor: its arity -> the expanded argument list to the widest constructor, with the parent's own parameter SYMBOLS still in place — a subclass calling
      * `super(args)` at that arity substitutes its actual args for those symbols to build the `this(expanded)` delegation. Built from the parent's own `this(...)` delegation chain.
      */
