...
/** Counts java `public` fields a reflective framework cannot see once emitted as private scala members — a java public field is read reflectively by third parties and must stay visible to bean
  * readers. [[Issue.NameTaken]]: seam from this phase's own scope. [[Issue.Unexposed]]: review list of java-public-field types not yet scoped. [[Issue.NameUnreachable]]: name unreachable via
