...
final class InjectedSurfaceFollowTransform(
  ...
  droppedEmitted: Map[String, String]
) extends Phase:
  ...
  private def lowerAcronym(s: String) =
    ...
    else if run.length > 1 then run.toLowerCase
    else decap(s)
  ...
  override def run(program: Program): Program =
    ...
    /** the EMITTED owner of a member of a dropped type — the dropped top-level type's emitted name plus the nesting below it (`Pixmap$Format` -> `sge.graphics.Pixmap.Format`); None outside
      */
    /** an interned external member (`@<owner>#name()`, the frontend's spelling for a member it could not resolve scope-free — `Enum#name` per enum type) carries its owner in the NAME
      */
    ...
    def emittedOwnerOf(s: Symbol): Option[String] =
      val chain = Iterator.iterate(ownerOf(s))(id => program.symbolOf(id).map(_.owner).getOrElse(SymId.None)).takeWhile(_ != SymId.None).take(8).flatMap(program.symbolOf).toList
    ...
    val renamed = program.symbols.all.toList.sortBy(_.id.raw).flatMap { s =>
      if s.flags.isParam || s.name == "<init>" || s.name.contains('$') then Nil
        ...
        emittedOwnerOf(s).toList.flatMap { emittedOwner =>
          val n = arityOf(s)
          if surface.lookup(emittedOwner, s.name, n).isDefined then Nil
            ...
            candidates
              .find(c =>
                surface.lookup(emittedOwner, c, n).isDefined || (n == 0 && surface.memberHasParens(emittedOwner, c).isDefined)
    ...
    parenless = renamed.collect {
      case (s, to)
          if arityOf(s) == 0 &&
            emittedOwnerOf(s).exists(o => surface.memberHasParens(o, to).contains(false)) =>
        s.id
    }.toSet
  ...
  /** `recv.getWidth()` -> `recv.width`: the call's parens go where the injected member has none; `javaEnum.name()` on a dropped enum replaced by a scala 3 `enum` -> `toString` (Enum's `name` is
    * java.lang.Enum's, no symbol of the dropped type's own to rename).
    */
  override def transformApply(t: Tree.Apply)(using p: Program): Term = t match
    ...
    case Tree.Apply(Tree.Select(q, _, _, _), Nil, m, tpe, o)
        if p
          .symbolOf(m)
          .exists(ms =>
            ms.name.stripSuffix("()") == "name" && arityOf(ms) == 0
              && p.symbolOf(ms.owner).exists(_.fullName == "java.lang.Enum")
          ) && receiverDropped(q) =>
    ...
    case Tree.Apply(fun, args, m, tpe, o) if p.symbolOf(m).exists(_.name == "<init>") && emittedOwnerOfType(tpe).exists(owner => surface.lookup(owner, "apply", args.size).isDefined) =>
  ...
  private def emittedOwnerOfType(tpe: TypeRepr)(using p: Program): Option[String] =
    ...
    head(tpe).flatMap(p.symbolOf).flatMap { cls =>
      val chain = cls :: Iterator.iterate(cls.owner)(id => p.symbolOf(id).map(_.owner).getOrElse(SymId.None)).takeWhile(_ != SymId.None).take(8).flatMap(p.symbolOf).toList
      chain.indexWhere(o => droppedEmitted.contains(o.fullName)) match
        case -1 => None
        case i  => Some((droppedEmitted(chain(i).fullName) :: chain.take(i).reverse.map(_.name)).mkString("."))
    }
  ...
  private def receiverDropped(q: Term)(using p: Program): Boolean =
    ...
    head(q.tpe).exists { sym =>
      Iterator.iterate(sym)(id => p.symbolOf(id).map(_.owner).getOrElse(SymId.None)).takeWhile(_ != SymId.None).take(8).flatMap(p.symbolOf).exists(x => droppedEmitted.contains(x.fullName))
    }
