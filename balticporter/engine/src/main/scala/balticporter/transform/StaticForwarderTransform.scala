...
/** Inlines a static forwarder — `Wrapper.m(x, rest…)` -> `x.m(rest…)` — removing a dependency on a platform-swapping wrapper the port does not need. Which class forwards which members to which
  * receiver is per-library policy (`Forwarder`); empty is a no-op. Mints proper `<receiver>#<member>` symbols so xref/`PortabilityCheck` see each call for what it now is. `policyReport` surfaces two
  * failure modes: name matches nothing, or matches an overload set.
  */
final class StaticForwarderTransform(forwarders: List[StaticForwarderTransform.Forwarder]) extends Phase, PolicySource, SurfacePolicy, PolicyBound:
  ...
  /** The members to inline, with the forwarder that named them — bound in a deterministic order, since the symbols this phase mints are numbered in it (unlike `program.symbols.all`'s hash order).
    */
  ...
  def bindPolicy(binder: PolicyBinder): Unit =
    ...
    val hits: List[(StaticForwarderTransform.Forwarder, String, List[Symbol])] =
      forwarders.flatMap { f =>
        f.members.toList.sorted.map { m =>
          val ss = boundMembers.get(MemberKey(f.wrapper, m).render).toList.flatMap(_.toOption.getOrElse(Nil)).flatMap(_.sym).flatMap(binder.program.symbolOf)
          (f, m, ss)
        }
      }
  ...
  /** Declared wrappers/members that matched nothing, plus matched members whose receiver-first shape the engine could not verify. On a name-only overload match this diagnoses rather than guards — the
    * receiver-first assumption cannot be checked honestly (the first parameter is legitimately a supertype in some wrappers, unresolved in others), so a wrong guard would silently refuse correct
    * rewrites. Only a provably nullary member is excluded outright.
    */
  ...
  override def run(program: Program): Program =
    ...
    if targets.isEmpty then program
      ...
      def ownerOf(fqn: String): SymId = owners.getOrElseUpdate(
        fqn,
...
object StaticForwarderTransform:
  /** One library's forwarding fact: the statics of `wrapper` named `members` are plain members of `receiver`, reached through the call's FIRST argument. Stated by the library's manifest — the engine
    * has no business knowing any particular wrapper's name.
    */
