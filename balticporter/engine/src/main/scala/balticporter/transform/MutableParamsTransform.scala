...
/** Java lets a method reassign its parameters, and its EXCEPTION parameters (JLS 14.20 — only a multi-catch's is implicitly final); Scala's are `val` and a pattern binding. For each one written to in
  * its body, renames it to `name$arg` and prepends a mutable local `var name: T = name$arg`, so every body reference binds to the `var`. KNOWN LIMIT: a LAMBDA's own reassigned parameter is not
  * reached — degrades loudly as a compile error.
  */
final class MutableParamsTransform extends Phase, SurfacePolicy, MergeablePolicy:
  ...
  override def run(program: Program): Program =
    ...
    argOf.keys.toList.flatMap(p => program.symbolOf(p).map(s => s.owner -> s.name)).groupBy(_._1).foreach { (owner, ps) =>
    ...
    catchArgOf.keys.toList.flatMap(p => program.symbolOf(p).map(s => s.owner -> s.name)).groupBy(_._1).foreach { (owner, ps) =>
      program.symbolOf(owner).foreach { m =>
        record(
          Decision(
            kind = Decision.Kind.RetypedSignature,
  ...
  private def rewriteDef(d: Tree.DefDef)(using Program): Tree.DefDef =
    ...
    val prelude: List[Statement] = shadows.map(v => Tree.ValDef(v.symbol, v.tpt, Some(Tree.Ident(argOf(v.symbol), v.tpt.tpe, o)), o))
    ...
    val body   = d.rhs match
      ...
      case Some(b @ Tree.Block(stats, _, _, _, _)) =>
        ...
        stats match
          case (deleg @ Tree.Apply(Tree.Select(_, m, _, _), _, _, _, _)) :: rest if isCtor && summon[Program].symbolOf(m).exists(_.name == "<init>") =>
  ...
  /** The same move at a CATCH: the pattern binds `name$arg` and the handler opens with `var name: T = name$arg`, so every reference in the body still reads `name`. The clause is rewritten where it
    * stands — a catch has no parameter list to rename (JLS 14.20).
    */
      ...
      t.copy(
        catches = t.catches.map { c =>
          catchArgOf.get(c.param.symbol) match
            ...
            case Some(a)    =>
              val o    = c.body.origin
              val decl = Tree.ValDef(c.param.symbol, c.param.tpt, Some(Tree.Ident(a, c.param.tpt.tpe, o)), o)
  ...
  /** A constructor's leading `super(…)`/`this(…)` reads the PARAMETER SLOTS, never the `var`s — the `var` is prepended AFTER the delegation (JLS 8.8.7), so it does not exist yet there. Left naming
    * the repurposed parameter symbol, a promoted constructor's funnel hoists the delegation into the `extends` clause, evaluated before the class body — `Not found: byteOffset`.
    */
