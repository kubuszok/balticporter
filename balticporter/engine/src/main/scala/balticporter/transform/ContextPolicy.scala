...
final case class ContextHolder(
  ...
  /** this holder with a dependent's per-declaration entries folded in; clashing keys have already been refused by the caller, so this only composes.
    */
  def extendedBy(e: ContextHolderExtension): ContextHolder =
    copy(
      sites = sites ++ e.sites,
      selfSupplied = selfSupplied ++ e.selfSupplied,
      retain = retain ++ e.retain,
      cache = cache ++ e.cache,
      through = through ++ e.through,
      capture = capture ++ e.capture
    )

object ContextHolder:

  /** the PER-DECLARATION half of a holder or of an extension, rendered by ONE body so a fingerprint cannot depend on which side of a merge stated an entry. `selfSupplied` is DIGESTED (arbitrary
    * Scala); `retain`/`cache` are spelled out (one identifier, emitted surface).
    */
  private[transform] def perDeclaration(
    sites:        Map[String, ContextSite],
...
/** WHERE the context type comes from: `Injected` (the port wrote the Scala by hand — member paths are unvalidated, so a bad path is a compile error at that line) or `Minted` (the engine synthesises
  * one mutable member per mapped field — member paths ARE validated; a two-hop path is refused at bind time, since there is no intermediate type to hang the second hop off).
  */
...
/** a PER-SITE override of [[ContextBoundary]]. [[LazyInit]] changes semantics and is never a default: java runs a class initialiser at first ACTIVE USE, the rewritten form at first READ of the field
  * — coinciding only when nothing else in the class is touched first, which the mechanism cannot know. So it is per-site opt-in, a `DeferredInit` decision, a porter note and a counted seam.
  */
