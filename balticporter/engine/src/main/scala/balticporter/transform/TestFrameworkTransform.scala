...
final class TestFrameworkTransform(
  ...
  /** Instance field FQNs (`Owner#name`) excluded from `bpFreshState`: a field the port DROPS via `dropMethods` must not have its initialiser hoisted into the per-test reset, since the initialiser may
    * reference platform-unavailable types. Empty = no-op.
    */
  ...
  /** MUnit declares assertions on both the `Assertions` TRAIT `FunSuite` mixes in and the `Assertions` OBJECT; emitting through the object also resolves inside a companion (a java `static` test
    * helper), which does not extend the suite.
    */
  private val MunitAssertions = "munit.Assertions"

  /** MUnit's own members this phase emits; `MinArity` is the matching `org.junit.Assert` list — a junit name absent from it (`assertThat`) is reported, never guessed at.
    */
  ...
  /** The one hamcrest member this phase names: `matches(Object)` is the `Matcher` CONTRACT, so asserting it over the intercepted value is universal rather than per-matcher-class.
    */
  ...
  /** members the `ExpectedException` translation emits: hamcrest's own `matches`, and the two `java.lang.Throwable`/`java.lang.String` members junit's `expectMessage(String)` means.
    */
  ...
  /** …and the members the RULE's own state machine is spelled with: junit accumulates matchers in a list and requires all of them (`allOf`), so the emitted accumulator is a list and the check is a
    * `forall` over it. `Nil`/`:+`/`forall`/`isEmpty`/`nonEmpty`/`apply` are scala's; `&&` and `+` are the two operators the message predicate and the failure clue need.
    */
  ...
  /** JS-E07's citation state — set when [[promote]] widens an operand, read by [[transformDefDef]] after the body it belongs to. Per-declaration, since the assertion rewrite (`transformApply`) sees a
    * call and never the enclosing member; cleared per declaration.
    */
  private var promotedHere = false

  /** A TEST-CLASS HIERARCHY. [[suiteAnchors]]: the suite parent goes at the ROOT of the program-declared chain above the `@Test` declarer (scalac rejects a second class parent). [[virtualTests]]: a
    * `@Test` in an override relation stays a `def`, registering ONCE at the top declarer as a call (a statement would duplicate and lose `super.m()`). Both computed once per [[run]] over the whole
    * program.
    */
  ...
  /** For Parameterized suites with constructor injection: constructor parameter symbol -> suite-level `bpParam_i` field symbol. Set by [[convertParameterized]], read by [[freshState]] to substitute
    * parameter references in the replayed constructor body.
    */
  private var paramSubst: Map[SymId, SymId] = Map.empty
  ...
  /** class → the NEAREST ANCESTOR that also declares one, i.e. `super.bpFreshState()`'s target. Not necessarily the direct parent: a class the lowering could not reach is skipped over.
    */
  private var freshSuper: Map[SymId, SymId] = Map.empty

  /** class → the member a `test(…)` registration EMITTED IN IT may call: its own, or the nearest inherited one. Distinct from [[freshSym]] since a virtual test registers at the TOP declarer, which
    * may hold no state of its own while its subclasses do.
    */
  private var freshCall: Map[SymId, SymId] = Map.empty

  /** java `final` instance fields the reconstruction ASSIGNS, hence made `var`s — narrowed to the fields the lowering really writes; every other field is emitted exactly as it was.
    */
  ...
  /** every program-declared class STRICTLY above `cd`, nearest first. `seen` is a cycle guard: a malformed hierarchy must not hang the pipeline, and answering the prefix it did see is the
    * conservative arm.
    */
  ...
  /** the `@Test` methods a class DECLARES, keyed the way an override is decided here: by NAME and ARITY. Deliberately looser than a descriptor and exact for this question — java cannot overload a
    * zero-argument test method, and every JUnit 4 `@Test` is zero-argument.
    */
  ...
  private def planHierarchy(program: Program)(using p: Program): Unit =
    ...
    val vs          = for
      ...
      if virtualKeys(k) && (above(s).contains(k) || classDefs.keys.exists(d => keysOf(d).contains(k) && above(d).contains(k) && classAncestry(classDefs(d)).exists(_.symbol == s)))
    yield (s, k, sym)
  ...
  private def planFreshState(program: Program)(using p: Program): Unit =
    ...
    borrowed.filter(s => scope.emitsSymbol(program, s)).foreach { s =>
      val cd = classDefs(s)
      found += Finding(
        "fresh-state(base-ancestor)",
        cd.origin,
        Fix.EngineRule,
        at = s,
        advice = "this suite's java superclass is a test class ANOTHER MODULE emits, so the per-test " +
      ...
      ctorToReplay(classDefs(s)) match
        case Left(why) =>
          ...
          found += Finding(
            ...
            Fix.EngineRule,
            at = s,
            advice = s"JUnit constructs a fresh instance before every `@Test`, so this class's field " +
    ...
    val inSet = chains.filter(c => c.exists(cd => !blocked(cd.symbol) && holdsState(cd))).flatten.map(_.symbol).distinct.filterNot(blocked).toSet
    ...
    freshSym = inSet.iterator.map { s =>
      ...
      val fqn = p.symbolOf(s).map(o => MemberKey(o.fullName, FreshStateMember).render).getOrElse(FreshStateMember)
      s -> mint(FreshStateMember, fqn, Flags(isOverride = supers.contains(s)), TypeRepr.MethodType(Nil, unitT), owner = s)
    }.toMap
  ...
  /** does java rebuild anything here — an instance field, an instance initialiser block, or a constructor body with a statement in it?
    */
  ...
  /** THE ONE CONSTRUCTOR THIS LOWERING MAY REPLAY, or the sentence the refusal reports. Matches JUnit's own precondition (`validateOnlyOneConstructor` + `validateZeroArgConstructor`), so the guard
    * costs nothing on a class JUnit would run. For a Parameterized suite, a constructor with parameters IS replayable — the Parameterized runner supplies them.
    */
  private def ctorToReplay(cd: Tree.ClassDef)(using p: Program): Either[String, Option[Tree.DefDef]] =
    cd.body.collect { case d: Tree.DefDef if p.symbolOf(d.symbol).exists(_.name == "<init>") => d } match
      ...
      case one :: Nil =>
        if one.paramss.flatten.nonEmpty && !paramSuites.contains(cd.symbol) then Left("takes constructor parameters")
        else
  ...
  private def ctorStatements(d: Tree.DefDef): List[Statement] = d.rhs match
    case scala.None                             => Nil
    case Some(Tree.Block(stats, expr, _, _, _)) =>
      stats ++ (expr match
        case Tree.Literal(Constant.UnitC, _, _) => Nil
        case t                                  => List(t))
    case Some(t) => List(t)

  /** the constructor body MINUS the delegation java writes at its head — the parent's construction is carried by the emitted `extends` clause and by the chain, never by a replayed statement.
    */
  ...
  /** THE REWRITE: the class's own initialisation moved out of its body and into [[freshSym]]'s member. Returns the class unchanged where the lowering does not reach it.
    */
  private def freshState(cd: Tree.ClassDef)(using p: Program): Tree.ClassDef =
    ...
    freshSym.get(cd.symbol) match
      ...
      case Some(member) =>
        ...
        cd.body.foreach {
          ...
          case v: Tree.ValDef if instanceField(v) =>
            ...
            defaultTerm(v.tpt.tpe, v.origin) match
              ...
              case scala.None =>
                found += Finding(
                  "fresh-state(no-default)",
                  v.origin,
                  Fix.EngineRule,
                  at = v.symbol,
                  advice = "JUnit's fresh instance leaves this field at the JVM default before every test, " +
          ...
          case d: Tree.DefDef if ctor.exists(_.symbol == d.symbol) && replayedStatements(d).nonEmpty =>
            // for Parameterized suites, constructor parameter references must be substituted with suite-level bpParam fields
            val replayed        = replayedStatements(d)
            val substStatements =
              if paramSubst.isEmpty then replayed
              else replayed.map(s => substParams(s))
            inits ++= substStatements
            kept += d.copy(rhs = Some(Tree.Block(ctorStatements(d).filter(isSuperCall), unitLit(o), unitT, o)))
          case other => kept += other
        }
        val sup = freshSuper.get(cd.symbol).map { a =>
          Tree.Apply(Tree.Select(Tree.Super(cd.symbol, TypeRepr.NoType, o), a, TypeRepr.NoType, o), Nil, a, unitT, o)
        }
        ...
        if escapes > 0 then
          found += Finding(
            "fresh-state(instance-escape)",
            cd.origin,
            Fix.EngineRule,
            at = cd.symbol,
            advice = s"$escapes use(s) of this suite's own instance AS A VALUE (a `this` that is not the " +
  ...
  /** Substitute constructor parameter symbol references with `bpParam*` field references in a statement. Used by `freshState` to replay a Parameterized constructor body with suite-level fields. */
  private def substParams(s: Statement)(using p: Program): Statement =
    val subst = new Phase:
      def name:                                                  String = "param-subst"
      override def transformIdent(t: Tree.Ident)(using Program): Term   =
        paramSubst.get(t.sym).map(ns => Tree.Ident(ns, t.tpe, t.origin)).getOrElse(t)
    s match
      case t: Term => StandardTraversal.mapTerm(subst, t)
      case other => other

  /** the instance used as a VALUE — a `this` that is the QUALIFIER of a selection is a field/method access and not an escape, so this is the difference of two standard walks.
    */
  ...
  private def planParameterized(program: Program)(using p: Program): Unit =
    ...
    /** Read `@Parameter(value)` index from carried annotations, or `None` when the argument is lost. */
    def paramIndex(sym: SymId): Option[Int] =
      p.symbolOf(sym).flatMap { s =>
        s.annotations.filter(a => ParameterAnns(nameOf(a.tpe))).flatMap(_.args.collect { case ("value", Tree.Literal(Constant.IntC(n), _, _)) => n }).headOption
      }
        ...
        dataMethod match
          ...
          case Some(dm)   =>
            val namePattern = p
              ...
              .getOrElse("{index}")
            // constructor injection: the constructor PARAMETERS (not fields) and the constructor body
            val ctorOpt = cd.body.collectFirst {
              case d: Tree.DefDef if p.symbolOf(d.symbol).exists(_.name == "<init>") && d.paramss.flatten.nonEmpty => d
            }
            // field injection: @Parameter-annotated fields with their index
            val paramFieldDefs = cd.body.collect {
              case v: Tree.ValDef if hasParamAnn(v.symbol) => v
            }
            val injection: Injection | Null =
              if ctorOpt.isDefined then
                val d      = ctorOpt.get
                val params = d.paramss.flatten.flatMap(v => p.symbolOf(v.symbol).map(s => (v.symbol, v.tpt.tpe)))
                Injection.Constructor(params, d)
              else if paramFieldDefs.nonEmpty then
                // read @Parameter(value) index for each field; refuse if ANY field (beyond a single one at default 0) has no readable index
                val indexed = paramFieldDefs.map { v =>
                  val idx = paramIndex(v.symbol)
                  (v.symbol, v.tpt.tpe, idx)
                }
                if paramFieldDefs.sizeIs == 1 then
                  // single @Parameter defaults to index 0
                  Injection.Fields(indexed.map { case (s, t, i) => (s, t, i.getOrElse(0)) })
                else if indexed.exists(_._3.isEmpty) then
                  found += Finding(
                    "parameterized(unread-parameter-index)",
                    cd.origin,
                    Fix.EngineRule,
                    at = sym,
                    advice = "this Parameterized suite uses field injection with multiple @Parameter fields, " +
                      "and the index argument of at least one @Parameter annotation could not be read. " +
                      "The row element order cannot be determined and the suite converts as a plain suite."
                  )
                  null
                else Injection.Fields(indexed.map { case (s, t, i) => (s, t, i.get) })
              else
                found += Finding(
  ...
  override def run(program: Program): Program =
    ...
    def prim(fqn: String): TypeRepr =
      TypeRepr.TypeRef(
        TypeRepr.NoPrefix,
        byName.get(fqn).flatMap(_.headOption).map(_.id).getOrElse(mint(fqn.substring(fqn.lastIndexOf('.') + 1), fqn))
      )
    ...
    predListType = TypeRepr.AppliedType(TypeRepr.TypeRef(TypeRepr.NoPrefix, mint("List", "scala.collection.immutable.List")), List(predType))
  ...
  /** `owner` is `SymId.None` for every mint EXCEPT the per-test reconstruction's: `Program.owns` decides membership, and an unowned member publishes at the PACKAGE rather than its class.
    */
  private def mint(nm: String, full: String, flags: Flags = Flags(), info: TypeRepr = TypeRepr.NoType, owner: SymId = SymId.None): SymId =
  ...
  private def report(): Unit =
    ...
    if suitesRebuilt > 0 then
      println(
        s"  fresh instance: $suitesRebuilt class(es) rebuild their instance state before " +
    ...
    if rulesConverted > 0 then
      println(
        s"  ExpectedException @Rule: $rulesConverted `thrown.expect(…)` site(s) MODELLED — " +
      ...
      cd.parents.foreach {
        case tt: TypeTree if nameOf(tt.tpe) == "junit.framework.TestCase" =>
          found += Finding(
            ...
            Fix.EngineRule,
            at = cd.symbol,
            advice = "a JUnit 3 suite declares its tests by NAMING them `testXxx` on a `TestCase` subclass; " +
          ...
          program
            ...
            .foreach(u =>
              found += Finding(
                ...
                Fix.EngineRule,
                at = u.enclosing,
                advice = "Hamcrest is a second assertion vocabulary (`assertThat(x, is(equalTo(y)))`); this " +
  ...
  private def adviceFor(fqn: String): (Fix, String) = fqn match
    case "org.junit.Rule" | "org.junit.ClassRule" =>
      (Fix.EngineRule,
    ...
    case f if f.startsWith("org.junit.jupiter.") =>
      (Fix.EngineRule,
    ...
    case f if f.startsWith("org.testng.") =>
      (Fix.EngineRule,
  ...
  /** `org.junit.Assert.assertX(…)` → the MUnit assertion meaning the same thing. Java's `(expected, actual)` with an optional leading `String message` becomes MUnit's `(obtained, expected, clue)`;
    * the overload is read from the ARGUMENTS' static types, not the callee's signature. An unmapped member (`assertThat`) is left alone and reported. `SpoonTir.fromSource` builds with `noClasspath`,
    * so a static-import snippet never fires this hook.
    */
  ...
  /** java's `(message?, expected, actual, delta?)` → MUnit's `(obtained, expected, delta?, clue?)`. `hasMsg` is decided STRUCTURALLY: a leading `String` is junit's message exactly when the call
    ...
    MinArity.get(nm).toRight("").flatMap { min =>
      ...
      (nm, rest) match
        ...
        case ("assertArrayEquals", List(e, a)) =>
          ...
          Right(call("assertEquals", select(guarded(a), toSeqSym, o) :: select(guarded(e), toSeqSym, o) :: clue, o))
        ...
        // has no clue slot).
        case ("assertThrows", List(Tree.Literal(Constant.ClassOfC(ex), _, _), lam: Tree.Lambda)) if clue.isEmpty && lam.params.isEmpty =>
          Right(intercept(munitSyms("intercept"), ex, lam.body, o))
        case ("assertThrows", List(Tree.Literal(Constant.ClassOfC(_), _, _), _: Tree.Lambda)) if clue.nonEmpty =>
          Left(
            "MUnit's `intercept[T](body)` has no clue slot, so junit's leading `String message` " +
        ...
        case ("assertThrows", List(Tree.Literal(Constant.ClassOfC(_), _, _), _)) =>
          Left(
            "the runnable must be a NO-ARGUMENT LAMBDA. `intercept[E] { r }` EVALUATES a " +
  ...
  /** `intercept[E] { body }` — MUnit's assertion that the body throws. `sym` is a parameter since there are two spellings: [[testCase]] uses the inherited one where `suite` is a parent, and
    * [[munitCall]] the `munit.Assertions` OBJECT one, for scopes that don't extend the suite ([[MunitAssertions]]).
    */
  private def intercept(sym: SymId, ex: TypeRepr, body: Term, o: Origin): Term =
    val fn = Tree.TypeApply(Tree.Ident(sym, TypeRepr.NoType, o), List(TypeTree(ex, o)), TypeRepr.NoType, o)
    Tree.Apply(fn, List(body), sym, TypeRepr.NoType, o)

  /** MUnit splits java's one `assertEquals(…, delta)` by WIDTH, and its `delta` parameter is not generic — so a `Double` operand anywhere forces the double form, exactly as java's own overload
    * resolution did.
    */
  private def deltaMember(operands: List[Term])(using p: Program): String =
    ...
    then "assertEqualsFloat"
    else "assertEqualsDouble"

  /** JAVA'S BINARY NUMERIC PROMOTION, re-applied. `assertEquals(int, long)` is legal java (the call promoted `int` first); MUnit infers each operand independently, so nothing drives scala's widening.
    * Widening the NARROWER operand is the only safe direction; `Char`/`Short` share a rank (neither widens to the other), so equal-rank promotes both to `Int`.
    */
  ...
  /** …and the CLASS BOUNDARY is the backstop: a class-body term neither hook above owns belongs to its own type, and this fires only for what they do not cover.
    */
  ...
  /** A type's full structural name, type ARGUMENTS included — [[nameOf]] answers only the type CONSTRUCTOR, which would make the pair [[widened]] exists for compare equal.
    */
  ...
  private def callAt(member: String, targ: TypeRepr, args: List[Term], o: Origin): Term =
    ...
    val fn = Tree.TypeApply(Tree.Ident(s, TypeRepr.NoType, o), List(TypeTree(targ, o), TypeTree(targ, o)), TypeRepr.NoType, o)
    Tree.Apply(fn, args, s, TypeRepr.NoType, o)

  /** Parenthesize a receiver that would otherwise re-associate. `a * b` is a bare `Apply` in the TIR but renders INFIX, so `.toLong` on it would attach to `b` — and `x >> 2.toLong` is not
  ...
  private def guarded(t: Term)(using p: Program): Term = t match
    case Tree.Apply(Tree.Select(_, m, _, _), _, _, _, _) if p.symbolOf(m).exists(_.fullName.startsWith("scala.<op>#")) =>
      Tree.Block(Nil, t, t.tpe, t.origin)
    case _: Tree.If | _: Tree.Match | _: Tree.Lambda => Tree.Block(Nil, t, t.tpe, t.origin)
    case _                                           => t

  /** The ONE junit assertion with no MUnit counterpart: elementwise comparison with a tolerance. Emitted as the loop it means. Both arrays are bound to locals FIRST — the operands are arbitrary
    * expressions, and naming each once is the difference between one evaluation and one per element. The length check comes first, as in junit.
    */
  private def arrayWithDelta(e: Term, a: Term, delta: Term, clue: List[Term], o: Origin)(using p: Program): Option[Term] =
    if a.tpe == TypeRepr.NoType || e.tpe == TypeRepr.NoType then scala.None
      ...
      val lengths                    = call("assertEquals", Tree.ArrayLength(obtained, int, o) :: Tree.ArrayLength(expected, int, o) :: clue, o)
      val body                       = call(deltaMember(List(elemProbe(e, o), elemProbe(a, o), delta)), at(obtained, a.tpe) :: at(expected, e.tpe) :: delta :: clue, o)
      val loop                       = Tree.ForEach(Tree.ValDef(iS, TypeTree(int, o), None, o), select(obtained, indicesSym, o), body, unit, o)
      Some(
        Tree.Block(List(Tree.ValDef(oS, TypeTree(a.tpe, o), Some(a), o), Tree.ValDef(eS, TypeTree(e.tpe, o), Some(e), o), lengths), loop, unit, o)
      )

  /** a stand-in term carrying the array's ELEMENT type, so [[deltaMember]] picks the member from the element width rather than from `Array`.
    */
  ...
  /** WHICH of [[AssertClasses]] a call's receiver names, or `None` for every other receiver. The name is returned rather than a `Boolean` because the refusal path reports it.
    */
  ...
  /** A class is a SUITE when it declares at least one `@Test` member. Nested classes are converted too, so the walk is explicit rather than top-level only. Gates the CONVERSION alone (parent,
    * registrations, lifecycle inlining) — the assertion rewrite already ran over the whole unit ([[run]]), since a `@Test`-less class is a HELPER, not a non-test.
    */
  private def convert(cd: Tree.ClassDef)(using p: Program): Tree.ClassDef =
    ...
    // for Parameterized constructor injection, mint bpParam fields and set up substitution BEFORE freshState replays the constructor body
    val paramFieldSyms = paramSuites.get(cd.symbol) match
      case Some(ParamInfo(_, _, Injection.Constructor(params, _))) =>
        val n    = nextTmp; nextTmp += 1
        val syms = params.zipWithIndex.map { case ((paramSym, tpe), i) =>
          val fs = mint(s"bpParam${n}_$i", s"bpParam${n}_$i", Flags(isMutable = true), tpe, owner = cd.symbol)
          paramSym -> fs
        }
        paramSubst = syms.toMap
        syms.map { case (_, fs) => (fs, params.find(p => paramSubst.get(p._1).contains(fs)).get._2) }
      case _ => paramSubst = Map.empty; Nil
    ...
    if !nested.exists(isAnnotated(_, TestAnn)) then withSuite(cd2)
      ...
      val ruleFields0 = cd2.body.collect {
        case v: Tree.ValDef if hasAnn(v.symbol, RuleAnn) && nameOf(v.tpt.tpe) == ExpectedExceptionCls => v.symbol
      }.toSet
      ...
      val ruleFields =
        if ruleFields0.isEmpty then ruleFields0
          ...
          val mine = cd1.body.collect { case d: Tree.DefDef if isAnnotated(d, TestAnn) => d.rhs }.flatten.map(b => StandardTraversal.scanTerm(b, 0)(refs)).sum
          if all <= mine then ruleFields0
          else
            found += Finding(
              s"$ExpectedExceptionCls(arming-outside-test)",
              cd.origin,
              Fix.EngineRule,
              at = cd.symbol,
              advice = s"${all - mine} reference(s) to this suite's `ExpectedException` rule field stand " +
      ...
      cd2.body.foreach {
        case v: Tree.ValDef if hasAnn(v.symbol, ClassRuleAnn) && nameOf(v.tpt.tpe) == ExpectedExceptionCls =>
          found += Finding(
            s"$ExpectedExceptionCls(class-rule)",
            v.origin,
            Fix.EngineRule,
            at = v.symbol,
            advice = "an `ExpectedException` declared as a `@ClassRule` wraps the WHOLE CLASS RUN, not each " +
  ...
  /** A Parameterized suite: all `@Test` methods stay as `def`s, and one registration loop iterates the `@Parameters` data method's result, calling each test from the body per row.
    *
    * JUnit's exact sequence per (row, test): construct a new instance with the row, run @Before, run the test. Here: assign bpParam fields from the row (constructor injection) or @Parameter fields
    * from the row (field injection), call bpFreshState (which replays the constructor body with parameter references substituted by bpParam fields, and resets all non-parameter instance state), call
    * @Before,
  ...
  private def convertParameterized(
    ...
    // the row element accessor: bpRow.asInstanceOf[Array[Object]](idx)
    val n      = nextTmp; nextTmp += 1
    ...
    def rowElemAt(idx: Int): Term =
      Tree.ArrayAccess(
        Tree.TypeApply(Tree.Select(rowRef, asInstanceOfSym, TypeRepr.NoType, o), List(TypeTree(arrayT, o)), objType, o),
        Tree.Literal(Constant.IntC(idx), intT, o),
        objType,
        o
      )
    def castTo(elem: Term, tpe: TypeRepr): Term =
      Tree.TypeApply(Tree.Select(elem, asInstanceOfSym, TypeRepr.NoType, o), List(TypeTree(tpe, o)), tpe, o)
    // the row assignment: for constructor injection, assign bpParam fields; for field injection, assign @Parameter fields
    val (assigns, arity) = pi.injection match
      case Injection.Constructor(params, _) =>
        val asgn = params.zipWithIndex.map { case ((_, tpe), i) =>
          val bpSym = paramSubst.values.toList(i)
          Tree.Assign(Tree.Ident(bpSym, tpe, o), castTo(rowElemAt(i), tpe), unitT, o): Statement
        }
        (asgn, params.size)
      case Injection.Fields(fields) =>
        val asgn = fields.map { case (sym, tpe, rowIdx) =>
          madeMutable += sym
          Tree.Assign(Tree.Ident(sym, tpe, o), castTo(rowElemAt(rowIdx), tpe), unitT, o): Statement
        }
        (asgn, fields.size)
    // call data method
    val dataCall = call(pi.dataMethod, o)
    // JUnit's sequence: row assignment -> bpFreshState -> @Before -> test
    ...
    val regs     = tests.map { d =>
      ...
      testsConverted += 1
      val nameTerm = paramTestName(nm, pi.namePattern, iRef, rowRef, arity, o)
      ...
      record(
        Decision(
          ...
          detail = Map(
            ...
            "injection" -> (pi.injection match { case Injection.Constructor(_, _) => "constructor"; case Injection.Fields(_) => "field" }),
    ...
    // bpParam fields added to the body for constructor injection
    val bpParamDefs: List[Statement] = pi.injection match
      case Injection.Constructor(params, _) =>
        params.zipWithIndex.map { case ((_, tpe), i) =>
          val bpSym   = paramSubst.values.toList(i)
          val defTerm = defaultTerm(tpe, o).getOrElse(nul(o))
          Tree.ValDef(bpSym, TypeTree(tpe, o), Some(defTerm), o): Statement
        }
      case _ => Nil
    // assemble: original body with @Test stripped to defs, bpParam fields, registration block
    val body = cd.body.map {
      case d: Tree.DefDef if isAnnotated(d, TestAnn) => d
      case other => other
    } ++ bpParamDefs :+ regBlock
    suitesConverted += 1
    withSuite(cd).copy(
  ...
  /** the calls as one term — a `Block` when there is more than one, since `Tree` has no statement-sequence node and a bare `List` would have to be flattened by the emitter.
    */
  ...
  private def hasAnn(s: SymId, fqn: String)(using p: Program): Boolean =
    p.symbolOf(s).exists(sy => sy.annotations.exists(a => nameOf(a.tpe) == fqn) || sy.droppedAnnotations.contains(fqn))
  ...
  /** WHICH of junit's four `expect`/`expectMessage` overloads a call resolved to, or the guard that declines it. ONE function, because the rewrite recomputes what the guard pass validated — over the
    * same node, with the same program, so the two answers are the same answer.
    */
  private def expectAt(nm: String, a: Tree.Apply)(using p: Program): Either[String, Either[Expect, ExpectMsg]] =
    if nm == "expect" then expectKind(a).map(Left(_)) else expectMsgKind(a).map(Right(_))

  /** the sentence each overload guard declines with — one table, so the refusal a run REPORTS and the guard the code took cannot drift apart.
    */
  ...
  /** the conversion — the armings rewritten in place, and the rule's own `try`/`catch` as a WRAPPER the caller applies outside the lifecycle nesting (difference 9 above).
    */
  private def model(d: Tree.DefDef, body: Term, rules: Set[SymId], calls: List[(String, Tree.Apply)], kinds: List[Either[Expect, ExpectMsg]])(using p: Program): (Term, Term => Term, String) =
    val o           = d.origin
    val unitT       = primTypes("scala.Unit")
    ...
    val matched = Tree.Apply(Tree.Select(expectedRef, forallSym, TypeRepr.NoType, o), List(predicateTest(caughtRef, o)), forallSym, boolT, o)
    val checked = Tree.If(
      infix(caughtRef, neSym, nul(o), o),
      Tree.If(
        Tree.Select(expectedRef, isEmptySym, boolT, o),
        ...
        call(
          ...
          List(
            matched,
            infix(
              constTerm(Constant.StringC("the exception thrown does not satisfy the expectation the java test armed: "), "java.lang.String", o),
              plusSym,
              caughtRef,
              o
            )
      ...
      Tree.If(
        Tree.Select(expectedRef, nonEmptySym, boolT, o),
        call(
          "fail",
          List(
            constTerm(
        ...
        Tree.Literal(Constant.UnitC, unitT, o),
        unitT,
        o
      ),
      unitT,
    ...
    def wrap(inner: Term): Term =
      Tree.Block(
        List(
          Tree.ValDef(expected, TypeTree(predListType, o), Some(Tree.Ident(nilSym, predListType, o)), o),
  ...
  private def predicateTest(caught: Term, o: Origin): Term =
    ...
    Tree.Lambda(
      List(Tree.ValDef(p, TypeTree(predType, o), scala.None, o)),
      invoke(Tree.Ident(p, predType, o), applySym, List(caught), o),
      TypeRepr.NoType,
      o
    )

  /** ONE `thrown.expect(…)` / `thrown.expectMessage(…)` call, as junit's own append. The operand is bound to a local FIRST (java evaluated it at the call), so a closure appended on a loop's second
    * iteration captures that iteration's matcher. A LITERAL needs no binding.
    */
  private def arming(nm: String, a: Tree.Apply, expected: SymId, k: Int)(using p: Program): Term =
    ...
    val pred: Option[Term] = expectAt(nm, a).toOption.map {
      ...
      case Right(ExpectMsg.Contains(t)) =>
        ...
        infix(infix(message(exRef, o), neSym, nul(o), o), andSym, invoke(message(exRef, o), containsSym, List(bound(s"bpMessage$k", t)), o), o)
    ...
    pred match
      case scala.None => a
      case Some(cond) =>
        val lam = Tree.Lambda(List(Tree.ValDef(ex, TypeTree(throwableType, o), scala.None, o)), cond, predType, o)
        val ref = Tree.Ident(expected, predListType, o)
        val app = Tree.Assign(
          ref,
          Tree.Apply(Tree.Select(ref, appendSym, predListType, o), List(lam), appendSym, predListType, o),
          primTypes("scala.Unit"),
          o
        )
        pre.result() match
          case Nil   => app
  ...
  /** a REFERENCE to one of this class's `ExpectedException` rule fields — `thrown` or `this.thrown`, and never the `expect` selection ON one, whose member symbol is junit's.
    */
  ...
  private def ruleCallIn(t: Term, rules: Set[SymId])(using p: Program): Option[(String, Tree.Apply)] = t match
    case a @ Tree.Apply(Tree.Select(rcv, m, _, _), _, _, _, _) if isRuleRef(rcv, rules) =>
      Some(p.symbolOf(m).map(_.name).getOrElse("") -> a)
    case _ => scala.None

  /** WHICH `expect` overload java resolved, read from the CALLEE's own formal. The class form additionally needs a LITERAL `classOf` — `intercept` takes a type ARGUMENT and a `Class` value is not
    * one, so a `Class`-typed variable is declined.
    */
  ...
  /** the callee's ONE declared parameter type, where the frontend could read the class file. An external member with no signature answers `None`, which every caller here declines on.
    */
  private def formalOf(a: Tree.Apply)(using p: Program): Option[TypeRepr] =
    p.symbolOf(a.method).map(_.info).collect { case TypeRepr.MethodType(List((_, f)), _, _) =>
      f
    }

  /** `@Test def m(): Unit = { … }` → `test("m") { … }`, a statement in the class body. An `expected = classOf[E]` argument becomes `intercept[E] { … }` — never dropped, since running the body bare
    * would PASS while checking nothing. `@Ignore` becomes `test(munit.TestOptions("m").ignore) { … }`, never enabled, since MUnit does not evaluate an ignored body.
    */
  private def testCase(d: Tree.DefDef, owner: SymId, setups: List[SymId], teardowns: List[SymId], allIgnored: Boolean, ruleFields: Set[SymId], viaCall: Boolean = false)(using p: Program): Statement =
    val nm = p.symbolOf(d.symbol).map(_.name).getOrElse("test")
    val expectsThrow: Option[TypeRepr] = p
      .symbolOf(d.symbol)
      .flatMap(
        _.annotations.filter(a => nameOf(a.tpe) == TestAnn).flatMap(_.args.collect { case ("expected", Tree.Literal(Constant.ClassOfC(t), _, _)) => t }).headOption
      )
    if d.rhs.isEmpty then d
      ...
      val nameTerm =
        if !ignored then lit
          ...
          val opts = Tree.Apply(Tree.Ident(testOptionsSym, TypeRepr.NoType, d.origin), List(lit), testOptionsSym, TypeRepr.NoType, d.origin)
          Tree.Select(opts, ignoreSym, TypeRepr.NoType, d.origin)
      val head = Tree.Apply(Tree.Ident(testSym, TypeRepr.NoType, d.origin), List(nameTerm), testSym, TypeRepr.NoType, d.origin)
      ...
      val (ruleBody, ruleWrap, ruleNote) =
        if !viaCall then expectedException(d, d.rhs.get, ruleFields)
          ...
          if ruleFields.nonEmpty && StandardTraversal.scanTerm(d.rhs.get, 0)((n, t) => if isRuleRef(t, ruleFields) then n + 1 else n) > 0 then
            found += Finding(
              s"$ExpectedExceptionCls(rule-in-overridden-test)",
              d.origin,
              Fix.EngineRule,
              at = d.symbol,
              advice = "this `@Test` arms an `ExpectedException` rule AND takes part in java's own override " +
      ...
      val rhs0 =
        ...
        else Tree.Try(Nil, setUp, Nil, Some(seq(teardowns.map(call(_, d.origin)), TypeRepr.NoType, d.origin)), setUp.tpe, d.origin)
...
object TestFrameworkTransform:
  ...
  enum Injection:
    /** Constructor injection: the runner calls `new Suite(row[0], row[1], ...)`. `params` lists the constructor parameters (symbol, type) in declaration order; `ctor` is the constructor DefDef whose
      * body must be replayed (with parameter references substituted by suite-level fields) as part of `bpFreshState`.
      */
    case Constructor(params: List[(SymId, TypeRepr)], ctor: Tree.DefDef)

    /** Field injection: the runner calls `new Suite()` then assigns `@Parameter(n)` fields from the row. Each entry is (field symbol, type, index in the row). */
    case Fields(fields: List[(SymId, TypeRepr, Int)])
  ...
  /** the member each converted class declares to rebuild its own instance state before every test — JUnit's `createTest()`, which MUnit has no counterpart for. `bp`-prefixed so it cannot collide with
    * a java member a suite declares.
    */
  val FreshStateMember = "bpFreshState"

  /** the frontend's name for a java INSTANCE INITIALISER BLOCK (`SpoonTir.classDef`), half of JLS 12.5 step 4.
    */
  ...
  /** WHICH of junit's two `ExpectedException.expect` overloads a call resolved to — read from the callee's own formal, never from the argument's shape.
    */
  enum Expect:
  ...
  def summary(fs: Seq[Finding]): String =
    if fs.isEmpty then "  (none)"
      ...
      fs.groupBy(_.construct)
        .toList
        .sortBy(g => (-g._2.size, g._1))
        .map { (c, gs) =>
          s"  $c × ${gs.size} — (${gs.head.fix.label}) ${gs.head.where.javaPath}:${gs.head.where.line}"
        }
        .mkString("\n")

  /** Widening rank for java's BINARY NUMERIC PROMOTION: a value of rank r converts, without loss, to any numeric type of higher rank. `Char` and `Short` share a rank since neither widens to the
    * other. Deliberately NOT the emitter's copy — that one disambiguates an OVERLOAD at emission, this one rewrites a TREE.
    */
  val NumericRank: Map[String, Int] = Map("scala.Byte" -> 1, "scala.Short" -> 2, "scala.Char" -> 2, "scala.Int" -> 3, "scala.Long" -> 4, "scala.Float" -> 5, "scala.Double" -> 6)

  /** The types every other type conforms to — MUnit's `Compare` resolves whatever the other operand is, so its constraint is ALREADY VACUOUS there (see [[TestFrameworkTransform.widened]]). Scala
    * roots are here because a port's own retyping can put one on an operand.
    */
  val Roots: Set[String] =
    Set("java.lang.Object", "scala.Any", "scala.AnyRef", "scala.Matchable")

  /** How many arguments each `org.junit.Assert` member takes WITHOUT java's optional leading `String message` — everything above this count with a leading `String` is that message.
    */
