...
final class SuppressionPhase extends Phase:
  ...
  override def run(program: Program): Program =
    ...
    /** Scan a class body for both `.orNull` references and exhaustive enum matches. Runs after TestFrameworkTransform, so a converted test body is already a class-body statement.
      */
    ...
    val toAddDeprecated      = deprecatedMembers.toSet.filter(id => program.symbolOf(id).exists(s => !hasNowarnMsg(s, "msg=deprecated")))
    val toAddUnreachableCase = unreachableCaseMembers.toSet.filter(id => program.symbolOf(id).exists(s => !hasNowarnMsg(s, "msg=Unreachable case")))
...
object SuppressionPhase:
  ...
  /** A `Tree.Match` whose scrutinee type is an enum, whose non-default/non-null case labels cover every enum constant, and which has a `case _ =>` default arm — JLS 14.11 does not require
    * exhaustiveness for a classic switch, so scalac's proof is a fact java lacks.
    */
