...
final class RemediationTransform(
  ...
  /** The selections are surface, fingerprinted in the manifest; this adds only [[classTables]], the one value that is a phase parameter. Targets are NOT here — they are the run's
  ...
  /** a table entry no selection reaches is dead policy — computed in `run` rather than at bind time because the question is "did any selection ask for this table", not "does this key exist".
    */
  def policyReport: PolicyReport =
    PolicyReport(
      unusedTables.sorted.map(k =>
        PolicyFinding(
          name,
          "RemediationTransform(classTables) key",
          k,
      ...
      val dropped = chosen.flatMap { (unit, r) =>
        ...
        fixes.find(f => f.mechanism == RemediationTransform.Drop.id && f.subject == name) match
          case Some(f) if f.confidence == Remediator.Confidence.High =>
            ...
            plan.applied(r, name, unit.symbol, unit.origin, s"dropped this type outright — ${f.observed}", drained = sites)
            Some(unit.symbol)
          case Some(f) =>
            plan.refuse(
              r,
  ...
  private def applyClassTables(program: Program, asked: collection.mutable.Set[String]): Program =
    val chosen = program.symbols.all.toList.sortBy(_.id.raw).filter(s => scope.emitsSymbol(program, s.id)).flatMap(s => plan.selected(s.id, RemediationTransform.ClassTable).map(s -> _))
    if chosen.isEmpty then program
    else
...
object RemediationTransform:
  ...
  /** the lane every remedy here drains — `portability(emitted)`, read from the check that mints the rows rather than spelled here, so a renamed lane is a compile error.
    */
  private val Lane: String = PortabilityCheck.EmittedLane

  /** `Remediator`'s own `mechanism` strings — the id a port writes in `resolutions` is the id printed beside the snippet in the `remediation` lane.
    */
  val Drop: Remedy = Remedy(
    id = "substitutions-drop",
    lane = Lane,
    kind = Remedy.AnyKind,
