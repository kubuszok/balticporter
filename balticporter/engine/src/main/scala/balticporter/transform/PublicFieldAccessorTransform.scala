...
final class PublicFieldAccessorTransform(
  /** Which declarations are read reflectively. `Only(Set.empty)` (default) admits nothing; `Everywhere(Set.empty)` is the whole port. Entries are FQNs cut at a `Symbol.fullName` separator, so an
    * enclosing type also reaches its nested and anonymous classes.
    */
  ...
  /** The getter's bridge, declared only where the scope can admit something so an unconfigured port acquires no unused runtime dependency.
    */
  ...
  /** Every field this phase could not expose, and every type it was not asked about that has one (`exposure`'s two rows).
    */
  ...
  override def run(program: Program): Program =
    ...
    objectSym =
      program.symbols.all.find(_.fullName == PublicFieldAccessorTransform.ObjectFqn).map(_.id).getOrElse(mint("Object", PublicFieldAccessorTransform.ObjectFqn, Flags(), SymId.None, TypeRepr.NoType))
    if scope.isUnrestricted || scope.entries.nonEmpty then toJavaValueSym = mint("toJavaValue", PublicFieldAccessorTransform.ToJavaValueFqn, Flags(), SymId.None, TypeRepr.NoType)
    val unitSym =
      program.symbols.all.find(_.fullName == PublicFieldAccessorTransform.UnitFqn).map(_.id).getOrElse(mint("Unit", PublicFieldAccessorTransform.UnitFqn, Flags(), SymId.None, TypeRepr.NoType))
  ...
  /** An anonymous class, reached through its `New` and never through `transformClassDef` — the shape this phase exists for (`new Inspectable() { public Date a = …; }`).
    */
    ...
    if owner.isEmpty then body
      ...
      if fields.nonEmpty then
        refusals += BeanExposureCheck.Finding(
          BeanExposureCheck.Issue.Unexposed,
          owner.get.fullName,
          fields.flatMap(v => p.symbolOf(v.symbol)).map(_.name).sorted.mkString(","),
          origin
        )
      body
    else
      ...
      val extra    = publicFields(body).flatMap { v =>
        ...
        if !PublicFieldAccessorTransform.invertible(fs.name) then
          ...
          refusals += BeanExposureCheck.Finding(
            BeanExposureCheck.Issue.NameUnreachable,
            MemberKey(owner.get.fullName, fs.name).render,
          ...
          refusals += BeanExposureCheck.Finding(
            BeanExposureCheck.Issue.NameTaken,
            MemberKey(owner.get.fullName, fs.name).render,
            want.filter(existing.contains).mkString(","),
            v.origin
          )
          Nil
        else
  ...
  /** The fields a java `public` modifier put on the class file's surface — instance fields only (a `static` field is not a bean property).
    */
  ...
  private def accessors(cls: SymId, v: Tree.ValDef, fs: Symbol, bean: String)(using p: Program): List[Statement] =
    ...
    val getSym   = mint("get" + bean, MemberKey(ownerFqn, "get" + bean).render, Flags(), cls, TypeRepr.MethodType(Nil, getTpe))
    ...
    val setter   =
      if !fs.flags.isMutable then Nil
        ...
        val sSym = mint("set" + bean, MemberKey(ownerFqn, "set" + bean).render, Flags(), cls, TypeRepr.MethodType(List("v" -> tpe), unitTpe))
        List(
          Tree.DefDef(
            sSym,
            List(List(Tree.ValDef(pSym, TypeTree(tpe, o), scala.None, o))),
            TypeTree(unitTpe, o),
            Some(Tree.Assign(Tree.Select(self, fs.id, tpe, o), Tree.Ident(pSym, tpe, o), TypeRepr.NoType, o)),
            o
          )
        )
    record(
  ...
  def exposure(units: List[Tree.ClassDef]): List[BeanExposureCheck.Finding] =
    ...
    refusals.toList.filter(f => paths.contains(f.origin.javaPath)).sortBy(f => (f.issue.toString, f.origin.javaPath, f.origin.line, f.subject))
...
object PublicFieldAccessorTransform:
  ...
  /** The JavaBeans capitalisation, not `capitalize`: a name whose first two characters are both upper case keeps its spelling (`URL` -> `getURL`), matching what `Introspector.decapitalize` inverts.
    */
  ...
  /** `java.beans.Introspector.decapitalize`, spelled out — reproduced rather than called since it is the half `beanSuffix` is checked against. Not its inverse everywhere: they compose to the identity
    * except a `lowerUpper` name (see `invertible`).
    */
  ...
  /** Does the round trip `decapitalize(beanSuffix(name)) == name` hold? A `lowerUpper` field like `eMail` fails it (`getEMail` decapitalises to `EMail`), registering under a name nobody asks for —
    * refused rather than emitted as false coverage.
    */
