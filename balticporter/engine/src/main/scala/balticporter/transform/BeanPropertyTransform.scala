...
final class BeanPropertyTransform(
  ...
  /** Idiom kinds this run reports: `BeanCollapse` always, plus `BeanDetect` only when the auto-detection scope is active — an inactive kind must not report a misleading zero.
    */
  ...
  def mergedWith(later: Phase): Either[String, MergeablePolicy.Merged] = later match
    case b: BeanPropertyTransform =>
      ...
      if conflicts.nonEmpty then
        Left(
          s"`bean-properties` has ${conflicts.size} key(s) with different accessor spellings: " +
            conflicts.toList.sorted.take(3).map(k => s"`$k` (${myPairs(k)} vs ${theirPairs(k)})").mkString("; ")
        )
      else
        ...
        if targetConflicts.nonEmpty then
          Left(
            s"`bean-properties` has ${targetConflicts.size} key(s) with different targets: " +
              targetConflicts.toList.sorted.take(3).map(k => s"`$k` (${targetOf(k).config} vs ${b.targetOf(k).config})").mkString("; ")
          )
        else
          ...
          val mergedScope: Either[String, RuleScope] = (detectScope, b.detectScope) match
            ...
            case _ =>
              Left(
                s"`bean-properties` scope disagrees: one is `Only` and the other is " +
  ...
  def bindPolicy(binder: PolicyBinder): Unit =
    ...
    val derivedEntries =
      if !derive then Nil
        ...
        binder.run.derived.propertyPairs.flatMap { case (owner, prop, g, s) =>
          val key = MemberKey(owner, prop).render
          Option.when(!configured.exists(_.key == key))(BeanPropertyTransform.Entry(key, g + s.fold("")("/" + _), owner, prop, g, s))
        }
  ...
  /** The never-fired half (from the binding) plus this phase's own malformed entries and counted refusals.
    */
  def policyReport: PolicyReport = PolicyReport.fromBindings(records) ++ PolicyReport(ownFindings)

  // ---- the run ------------------------------------------------------------------------------

  /** Every declaration of the getter's whole override COMPONENT -> the property, and likewise for the setter — an interface `getX()` and an implementor's kept `def x()` would otherwise not override
    * each other.
    */
  ...
  /** setter declaration -> the SAME-owner getter declaration whose name its assignment's LHS must carry, so the emitted `this.x = v` names a member of the class it is written in.
    */
  ...
  override def run(program: Program): Program =
    ...
    def refuse(e: BeanPropertyTransform.Entry, why: String): scala.None.type =
      refusals += PolicyFinding(name, s"BeanPropertyTransform(pairs) `${e.key}`", e.key, PolicyIssue.Unverifiable, why)
      record(
        Decision(
    ...
    val detectedProperties =
      if !scopeActive then Nil
        ...
        val detected = BeanPropertyTransform.detect(
          program,
          graph,
          scope,
          configuredAccessors,
          ...
          substitutedOwners,
          emitted = s => runScope.emitsSymbol(program, s),
          isVoid = (p, t) => isVoid(p, t),
          headOf = t => headOf(t),
          callsAreRewritable = (comp, ar) =>
            comp.forall(s =>
              program.usages(s).forall {
    ...
    val requests = properties.flatMap { p =>
      ...
      MemberRenamer.Request(p.getter, p.property, reason, p.key, p.key) ::
        p.setter.map(s => MemberRenamer.Request(s, p.property + "_=", reason, p.key, p.key)).toList
    }
    val (renamed, renameRefusals) = MemberRenamer.rename(program, graph, requests, MemberRenamer.OnCollision.DeferToEmitter, decisions)
      ...
      detectedProperties.find(_.key == k).foreach { p =>
        consider(
          IdiomCandidate(
            ...
            IdiomVerdict.Refused("RenameRefused", why),
            p.key,
            s"auto-detected pair `${p.property}`",
            Decision.originOf(program, p.getter)
          )
        )
      }
    }
    ...
    verdicts.foreach { (e, v) =>
      val at      = applied.find(_.key == e.key).map(p => Decision.originOf(program, p.getter)).getOrElse(Origin.synthetic)
    ...
    collapsed = verdicts.collect { case (e, BeanCollapse.Verdict.Collapse(f)) =>
      ...
      BeanPropertyTransform.Collapsed(
        e.key,
        e.value,
        p.property,
        p.getter,
  ...
  /** `o.getX()` -> `o.x`, `o.setX(v)` -> `o.x = v`. Bottom-up, so `o.setX(o.getX() + 1)` needs no special case. The assignment's LHS names the GETTER symbol — scalac desugars `recv.x = v` to `x_=` —
    * which is also why a set-only entry has nothing to put on an LHS.
    */
  ...
  /** The call's receiver: `Some(None)` bare, `Some(Some(q))` qualified, `None` a shape `validate` has already refused the pair for.
    */
  ...
  private def applyCollapse(p: Program): Program =
    ...
    val out     =
      ...
      retyped.rebuilt(units = retyped.units.map(u => StandardTraversal.mapClassDef(new BeanPropertyTransform.Collapser(collapsed), u)))
  ...
  private def finality(c: BeanPropertyTransform.Collapsed): String =
    if c.target != BeanPropertyTransform.Target.Val then ""
      ...
      "; and this one is a `val`, so its BACKING FIELD is `final` on the JVM where java's was " +
        "not — a reflective writer (`setAccessible` + `Field.set`) that worked against the java " +
        "field does not work against this one"

  private def residue(p: Program, c: BeanPropertyTransform.Collapsed, sym: SymId, what: String, keep: Usage => Boolean = _ => true): Unit =
    p.usages(sym).filter(keep).map(_.enclosing).distinct.sortBy(_.raw).foreach { encl =>
      consider(
        IdiomCandidate(
          IdiomKind.BeanCollapse,
          IdiomVerdict.Residue(what),
          Decision.fqnOf(p, encl, c.key),
          s"property `${c.property}`",
          Decision.originOf(p, encl)
        )
      )
    }
  ...
  private def validate(p: Program, graph: OverrideGraph, e: BeanPropertyTransform.Entry, refuse: (BeanPropertyTransform.Entry, String) => scala.None.type): Option[BeanPropertyTransform.Property] =
    ...
    /** Every recorded call of every declaration in the whole override COMPONENT is an ordinary `Apply` of the right arity through a movable receiver — a `MethodRef`/bare `Select` is a method value,
    ...
    e.setterName match
      case scala.None if e.accessors.sizeIs != 1 =>
        refuse(e,
               s"`${e.value}` names ${e.accessors.size} accessors; the value is `getter` or " +
                 "`getter/setter`"
        )
      case _ =>
        nilary(hits(e.getterName)) match
          case Nil =>
            ...
            if hits(e.getterName).isEmpty then scala.None
              ...
              refuse(
                e,
                s"`${e.getterName}` has no NILARY overload, so there is no getter to " +
                  "convert (an overload with parameters is left exactly as it is)"
              )
          case g :: rest if rest.nonEmpty =>
            refuse(e,
                   s"`${e.getterName}` has ${rest.size + 1} nilary declarations in this program " +
          ...
          case g :: Nil =>
            ...
            if isStatic(g) then
              refuse(
                e,
                s"`${e.getterName}` is STATIC; a companion property is out of scope for " +
                  "this phase (v1), so the accessor is left as it is"
              )
            else if isVoid(p, gd.returnTpt.tpe) then refuse(e, s"`${e.getterName}` returns void, so it is not a getter")
            else if !callsAreRewritable(gComp, 0) then
              refuse(
                e,
                s"`${e.getterName}` is referenced in VALUE position (a method reference or " +
                  ...
                  cands match
                    case Nil =>
                      if hits(sn).isEmpty then scala.None
                        ...
                        refuse(e,
                               s"`$sn` has no single-parameter overload taking " +
                    ...
                    case s :: Nil =>
                      ...
                      if isStatic(s) then refuse(e, s"`$sn` is STATIC; a companion property is out of scope (v1)")
                        ...
                        refuse(
                          e,
                          s"`$sn` is FLUENT and a call USES its result (a chain): `o.x = v` is " +
                            "Unit, so that call has no assignment rendering"
                        )
                      else if !callsAreRewritable(graph.closureOf(s).members, 1) then
                        refuse(e,
                               s"`$sn` is referenced in VALUE position; an eta-expanded `x_=` is " +
                                 "not the SAM java saw"
                        )
                      else {
                        ...
                        if setterOnly.nonEmpty then
                          refuse(
                            e,
                            s"the setter's override component reaches " +
                              s"${setterOnly.mkString(", ")} which declares the setter without a " +
                              "getter — `x.prop = v` needs `prop` in scope on the receiver"
                          )
                        else Some(BeanPropertyTransform.Property(e.key, e.property, g, Some(s), gComp, sComp, fluent))
                      }
  ...
  private def paramHead(p: Program, m: SymId): Option[SymId] =
    p.definitionOf(m).collect { case d: Tree.DefDef => d }.flatMap(_.paramss.flatten.headOption).flatMap(v => headOf(v.tpt.tpe))
  ...
  /** Java `void`, as the frontend writes it — not `scala.Unit`, the engine's own rendering. See `PolicyKeyLintSpec`'s allow-list.
    */
...
object BeanPropertyTransform:
  ...
  /** One collapsed property: the policy key, the accessors as spelled, the emitted name, the surviving symbol, the setter that goes, the storage field, the target shape, and the deleted declarations'
    * comments.
    */
  ...
  final private[transform] class Collapser(cs: List[Collapsed]) extends Phase:
    ...
    /** Every read and write of the backing field now names the property — sound because the trivial-body guard already established `this.f` and `this.x` are the same storage.
      */
  ...
  /** One applied property: the policy key, the emitted name, and every declaration of each accessor's whole override component, which is what the arity edit and call-site rewrite range over.
    */
  ...
  final case class Entry(key: String, value: String, owner: String, property: String, getterName: String, setterName: Option[String]):
    def accessors: List[String] = getterName :: setterName.toList

  /** Parse the declared map; anything outside the grammar is a `Malformed` finding rather than a best-effort reading.
    */
  def parse(pairs: Map[String, String]): (List[Entry], List[PolicyFinding]) =
    ...
    def bad(k: String, what: String): scala.None.type =
      findings += PolicyFinding("bean-properties", s"BeanPropertyTransform(pairs) `$k`", k, PolicyIssue.Malformed, what)
      scala.None
    val entries = pairs.toList.sortBy(_._1).flatMap { (k, v) =>
      MemberKey.parse(k) match
        ...
        case Right(mk) if mk.descriptor.isDefined =>
          bad(
            k,
            "the KEY names the emitted PROPERTY, which has no parameter list — write " +
        ...
        case Right(mk) =>
          v.split("/", -1).toList.map(_.trim) match
            ...
            case List(g, _) if g.isEmpty =>
              bad(
                k,
                "a SET-ONLY entry (`/setX`) is refused: scala's `o.x = v` sugar needs `x` in " +
            ...
            case _ =>
              bad(
                k,
                s"`$v` is not `getter` or `getter/setter` — a value with more than one `/` " +
  ...
  /** is this setter's body ONE assignment of its parameter to a field (`this.f = p;`) — the shape `x = v` faithfully stands for; an abstract setter (no body) counts as plain.
    */
  ...
  def propertyNameOf(methodName: String): Option[String] =
    if methodName.startsWith("get") && methodName.length > 3 && methodName.charAt(3).isUpper then Some(decapitalize(methodName.substring(3)))
    else if methodName.startsWith("is") && methodName.length > 2 && methodName.charAt(2).isUpper then Some(decapitalize(methodName.substring(2)))
    else scala.None

  /** The standard `java.beans.Introspector.decapitalize` rule: lowercase the first character UNLESS the first TWO characters are uppercase, in which case leave the string as-is.
    */
  def decapitalize(s: String): String =
    if s.length > 1 && s.charAt(0).isUpper && s.charAt(1).isUpper then s
    else if s.nonEmpty then s.updated(0, s.charAt(0).toLower)
    else s
      ...
      if !substitutedOwners.contains(ownerFqn) && emitted(ownerSym) && ownerSymObj.exists(o => scope.includes(program, o)) then
        ...
        getterCandidates.foreach { (getterSym, propName, getterDef) =>
          ...
          if !configuredAccessors.contains(getterKey) && !configuredPropertyKeys.contains(key) &&
            ...
            !ambiguousProps.contains(propName)
          then
            ...
            if BeanPropertyTransform.ReservedName(propName) then
              phase.consider(
                IdiomCandidate(
                  IdiomKind.BeanDetect,
                  IdiomVerdict.Refused("ReservedName", s"`$propName` is a Scala keyword: a backticked property is no API, java's accessor is kept"),
                  key,
                  s"auto-detected `$propName`",
              ...
              phase.consider(
                IdiomCandidate(
                  IdiomKind.BeanDetect,
                  IdiomVerdict.Refused("ValuePosition", "the getter is referenced in value position"),
                  key,
                  s"auto-detected `$propName`",
                  Decision.originOf(program, getterSym.id)
                )
                ...
                case List(s) =>
                  ...
                  if headOf(sd.returnTpt.tpe).exists(h => ownerTypeOf(s.id).contains(h)) then
                    phase.consider(
                      IdiomCandidate(
                        IdiomKind.BeanDetect,
                        IdiomVerdict.Refused("FluentSetter", "the setter is fluent — returns its own type for chaining"),
                        key,
                        s"auto-detected `$propName`",
                        Decision.originOf(program, getterSym.id)
                      )
                    ...
                    phase.consider(
                      IdiomCandidate(
                        IdiomKind.BeanDetect,
                        IdiomVerdict.Refused("NonVoidSetter", "the setter returns a value, and an assignment discards it"),
                        key,
                        s"auto-detected `$propName`",
                        Decision.originOf(program, getterSym.id)
                      )
                    ...
                    phase.consider(
                      IdiomCandidate(
                        IdiomKind.BeanDetect,
                        IdiomVerdict.Refused("ValuePosition", "the setter is referenced in value position"),
                        key,
                        s"auto-detected `$propName`",
                        Decision.originOf(program, getterSym.id)
                      )
              ...
              if !setterRefused then
                ...
                val setterOnlyOwners = setterOpt.toList.flatMap { s =>
                  ...
                  sComp
                    ...
                    .flatMap(o => program.symbolOf(o).map(_.fullName))
                    .toList
                    .distinct
                    .sorted
                }
                if setterOnlyOwners.nonEmpty then
                  ...
                  phase.consider(
                    IdiomCandidate(
                      ...
                      key,
                      s"auto-detected pair `$propName` via `$accessorsStr`",
                      Decision.originOf(program, getterSym.id)
                    )
                  )
                  phase.record(
