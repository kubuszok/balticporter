...
final class PackageRenameTransform(
  ...
  /** upstream TYPE FQNs whose boundary move the port declares deliberate. An entry that refuses nothing is itself reported.
    */
  ...
  /** declared per-type keys that named nothing, named only an external, or could never have been carried out. Complete before the pipeline runs.
    */
  ...
  def bindPolicy(binder: PolicyBinder): Unit =
    ...
    val requested: List[Request] =
      typeRenames.toList.sorted.filterNot((k, _) => flatAndType(k)).map((k, v) => Request(k, TypeSetting, Some(v))) ++
        ...
        flattenNestedTypes.toList.sorted.map(k => Request(k, FlatSetting, subPackages.get(k).filter(_ => flatAndSub(k)), typeRenames.get(k).filter(_ => flatAndType(k))))
    ...
    val kept: List[Move] = free.flatMap { mv =>
      ...
      val declared: List[Widening] =
        if !allowPackageSplit(mv.key) || packageOf(mv.emitted) == packageOf(renamed(mv.key, renames)) then Nil
          ...
          under(program, mv.sid).toList
            .flatMap(program.symbolOf)
            .filter(m => program.owns(m.id) && m.flags.isPackagePrivate) // the TYPE itself included
    ...
    widenings ++= stayed.flatMap { (k, t) =>
      (t :: under(program, t.id).toList.flatMap(program.symbolOf))
        .filter(m => program.owns(m.id) && m.flags.isPackagePrivate)
  ...
  /** The upstream-namespace destination a request names, or why it is not one this phase can carry out. Shares `PortManifest.TypeMove`'s string half; adds the one judgement a manifest cannot make
    * since it needs a symbol: only a STATIC nested type can be promoted (an inner class carries an implicit reference to its enclosing instance).
    */
      ...
      case _           =>
        TypeMove.flatten(r.key).flatMap { t =>
          if !sym.exists(_.flags.isStatic) then
            Left(
              "only a STATIC nested type can be promoted: a Java inner class carries an " +
  ...
  /** Which declarations a move puts on the wrong side of an access boundary Java gave them. A per-type rename (or flatten) can split two types that shared a package or top-level enclosure (JLS 6.6.1)
    * that a whole-package rename never could. Reports one row per broken declaration, for a refusal or a declared move's record. Java's package-private is unrepresented in this TIR, so [[restricted]]
    * sees only the `protected` half. Each entry judged alone.
    */
        ...
        if !program.owns(m.id) || !restricted(m.flags, cause) then Nil
          ...
          program.usages(m.id).map(_.enclosing).filter(_ != SymId.None).distinct.flatMap { user =>
            if declaredInside == inside(user) || name(user).isEmpty then Nil
              ...
              val (was, is) =
                if cause == Cause.PackageSplit then (reaches(before(m.id), before(user)), reaches(now(m.id), now(user)))
                else (typeHeadOf(before(m.id)) == typeHeadOf(before(user)), typeHeadOf(now(m.id)) == typeHeadOf(now(user)))
              if !(was && !is) then Nil
              else List(Widening(mv.key, cause, m.id, name(m.id), Decision.fqnOf(program, user, ""), Decision.originOf(program, m.id)))
          }
      }
      rows.distinctBy(w => (w.subject, w.readerFqn)).sortBy(w => (w.subjectFqn, w.readerFqn))

  /** Can a declaration emitted at `reader` still see a package-scoped member emitted at `decl`? Not string equality: Scala's `private[p]` covers `p` and its subpackages, while Java's package boundary
    * is exact — so nesting under `p.internal` keeps `p`'s restricted members reachable from it, and only the other direction is lost. Cut at a separator like every other prefix question here.
    */
  private def reaches(decl: String, reader: String): Boolean =
    val (d, r) = (packageOf(decl), packageOf(reader))
    d == r || covers(r, d)

  /** which visibilities a given boundary move can strip. `private` is exact within a top-level enclosure; `protected` carries Java's package half. Package-private is not representable
    * ([[boundaryOf]]).
    */
  ...
  private def hoist(program: Program): Program =
    ...
    val symbols = ids.foldLeft(program.symbols)((t, id) => program.symbolOf(id).fold(t)(s => t.updated(s.copy(owner = SymId.None))))
    program.rebuilt(units = units ++ out, symbols = symbols)

  /** One decision row per TYPE that moved, not per symbol — a member's namespace moves because its type's did. `RenamedPackage`/`RenamedType` are separated by which map matched, never by comparing
    * the two strings.
    */
  ...
  private def whyOf(key: String): String =
    if flattenNestedTypes.contains(key) then "promoted out of its enclosing type: a consumer reads a nested type as a path-dependent one"
...
object PackageRenameTransform:
  ...
  /** what a move takes away: a package boundary (`protected[pkg]`/`private[pkg]`) or a top-level enclosure (`private[TopLevel]`).
    */
  ...
  final private case class Request(key: String, setting: String, value: Option[String], rename: Option[String] = scala.None)

  /** an accepted request, with both namespaces of its destination: `upstreamTarget` is what the policy author wrote, `emitted` is that name after the package renames.
    */
  final private case class Move(request: Request, sid: SymId, upstreamTarget: String, emitted: String):
  ...
  /** the package of a fully-qualified name, cut at `.` after the `$`/`#` tail is gone, so a nested type answers with its package and not its enclosure.
    */
  ...
  /** the top-level type of a fully-qualified name — the part a Java `private` reaches throughout (JLS 6.6.1), which is the boundary flattening moves.
    */
  private[transform] def typeHeadOf(fqn: String): String =
    val i = fqn.indexWhere(c => c == '$' || c == '#')
    if i < 0 then fqn else fqn.substring(0, i)

  /** does `fullName` sit under `prefix`, cut at a separator? (`com.foo` covers `com.foo.Bar`, never `com.foobar`.)
    */
  ...
  /** longest covering prefix, so a more specific entry wins; also orders a per-type key (always longer) ahead of any package prefix covering it.
    */
  ...
  /** every class the program declares, nested ones included — via `StandardTraversal.allClassDefs`, not a `cd.body` recursion, which misses a method-local class (JLS 14.3).
    */
  ...
  def check(program: Program, renames: Map[String, String]): Report =
    ...
    val counts = program.symbols.all.toList.collect { case s if owned(s.id) => longestMatch(s.fullName, renames.keySet) }.flatten.groupBy(identity).view.mapValues(_.size).toMap
