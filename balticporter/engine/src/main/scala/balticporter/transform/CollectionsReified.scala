...
private[transform] trait CollectionsReified:
  ...
  /** the mapping targets `JavaCollections.fromJava` can actually PRODUCE — see [[CollectionsTransform.liveWrappable]], read as symbols so [[externalProducer]] asks a membership question about what
    * this run minted rather than a question about a name. EMPTY when the program names none of them, which makes the wrap arm decline by arithmetic.
    */
  ...
  /** every symbol THIS PROGRAM names that is an unmapped SUPERTYPE of a type this phase retypes — see [[unmappedReified]]. EMPTY where the program names none of them, which makes the refusal decline
    * by arithmetic exactly as the two maps above do.
    */
  ...
  /** the carriers that actually RUN — what the port declared plus the one java guarantees. A `val`, so [[preservesTypeArgsOf]], the recorder and the fingerprint cannot disagree.
    */
  private[transform] val effectiveCarriers: Set[String] = reifiedCarriers ++ CollectionsTransform.UniversalCarriers

  /** …resolved to THIS program's symbols, once per run. Read off `program.symbols` and not off the mapping, for the reason [[unmappedSupertypeSyms]] states: a carrier is a type this phase leaves
    * alone, so its symbol keeps the id it arrived with. EMPTY where the program names none of them, which is what makes every arm below a no-op by arithmetic.
    */
  private[transform] var carrierSyms: Set[SymId] = Set.empty

  /** the REFLECTIVE SINKS this program actually names, as this program's own symbols. EMPTY where the port declares none, which is what makes the egress bridge a no-op with no code path.
    */
  ...
  /** java's `(T) x` where this phase retyped `T` and cannot vouch for what `x` produces. The cast is KEPT and the coercion goes inside it — exact rather than tidy, since java's cast to a generic type
    * is unchecked in its type arguments (JLS 5.5), which the surviving `asInstanceOf` expresses, while the coercion answers only the erased class java checked.
    */
  private[transform] def reifiedCast(t: Tree.Typed)(using p: Program): Term =
    ...
    if !isNullLiteral(t.expr) && reifiedTarget(t.tpt.tpe).isEmpty then unmappedReified("reified cast", t.tpt.tpe, t.origin)
    if vouched(t.expr) || isNullLiteral(t.expr) then t
    else
      reifiedTarget(t.tpt.tpe) match
        case scala.None => t
        case Some(tgt)  =>
          reifiedAsSyms.get(tgt) match
            case Some(f) =>
              reifiedHere = true
              t.copy(expr = Tree.Apply(Tree.Ident(f, TypeRepr.NoType, t.origin), List(t.expr), f, t.tpt.tpe, t.origin))
            case scala.None =>
  ...
  /** `null` is an instance of nothing and a cast of it checks nothing — no runtime object for a reified question. Left as-is, keeping `null.asInstanceOf[T]` recognisable downstream.
    */
  ...
  private[transform] def reifiedSeam(slot: String, target: TypeRepr, origin: Origin)(using Program): Unit =
    seam(
      slot,
      "a representation-agnostic test or coercion",
      TirPrinter.tpe(target, TirPrinter.Style.canonical),
      origin,
      SymId.None,
  ...
  private[transform] def unmappedReified(slot: String, target: TypeRepr, origin: Origin)(using Program): Unit =
    if headSym(target).exists(unmappedSupertypeSyms) then
      seam(
        slot,
        "no coercion exists: the target is OUTSIDE the mapping, and a retyped value is not one",
        TirPrinter.tpe(target, TirPrinter.Style.canonical),
        origin,
        SymId.None,
        CollectionBoundaryCheck.Issue.ReifiedOccurrence
  ...
  /** Rewrites `Collections.EMPTY_LIST/SET/MAP` to the typed `emptyList()`/etc. helper. Consulted ahead of [[externalFieldProducer]] to avoid wrapping a raw type.
    */
  ...
  /** The same bridge where there is no formal to read — a generic external method has no readable `MethodType`, so [[bridgeJavaFormals]]'s arity test declines. Honest and counted for an ordinary
    * external callee; for a DECLARED SINK it is not (the port already stated the fact a signature would have carried) — the argument decides instead, a retyped value or `java.lang.Object`.
    */
  private[transform] def bridgeSinkArgs(t: Tree.Apply)(using p: Program): Tree.Apply =
    if formalsOf(t).sizeIs == t.args.size || !externalCallee(t.method) then t
      ...
      sinkOf(t.method) match
        case scala.None => t
        case Some(fqn)  =>
          val as = t.args.map(a =>
            if !mayBeRetypedValue(a) || toJavaValueSym == SymId.None then a
            else Tree.Apply(Tree.Ident(toJavaValueSym, TypeRepr.NoType, a.origin), List(a), toJavaValueSym, objectTpe(a), a.origin)
          )
          if as == t.args then t
          else
  ...
  /** does this type mention a wildcard at any depth? Not a nameability test — a wildcard-applied type IS nameable (`Class[? <: N]`) — but a narrower question for [[wildcardMapCall]]: could scala's
    * invariance bite at this key. Complete over `TypeRepr`, never a partial walk.
    */
  ...
  /** Route `Object`-keyed map members through helpers when the key/value has a bare capture or the probe's type disagrees with the key's (invariance).
    */
  ...
  /** True when the argument type is `Object` and the expected element type is not — routes through a helper that widens the probe position at erasure.
    */
    ...
    (headSym(arg.tpe), want.flatMap(headSym)) match
      case (Some(a), Some(e)) if a != SymId.None && e != SymId.None && a != e =>
        ...
        def reaches(id: SymId, fuel: Int): Boolean =
          fuel > 0 && classDefsBySym.get(id).exists(c => parentsOf(c).exists(s => s == a || reaches(s, fuel - 1)))
        reaches(e, 64)
      case _ => false
