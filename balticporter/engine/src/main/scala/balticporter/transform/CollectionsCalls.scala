...
private[transform] trait CollectionsCalls:
  ...
  /** scala nullary accessors that take NO parens (`def size: Int`) — a Java `size()` emitted as `size()` would be an illegal application. Strip the `Apply`.
    */
  private[transform] val parenless = Set("size", "isEmpty", "iterator", "keySet", "values", "nonEmpty", "hasNext", "next")

  /** Java's collection copy constructor — `new ArrayList<>(c)`, `new HashSet<>(c)`, etc. A capacity hint (`new ArrayList<>(10)`) maps correctly by accident; a COPY needs `<Companion>.from(c)`
    * instead, gated on the argument being a collection.
    */

  /** Java's class-token constructor — `new EnumMap<K, V>(K.class)` — routed to a named factory since the shim orders by `ordinal` and the token has nothing to size. Ordered before
  ...
  private[transform] def tokenConstructor(t: Tree.Apply)(using Program): Option[Term] = t.fun match
    case n: Tree.New if enumMapOfTypeSym != SymId.None =>
      ...
      yield Tree.Apply(Tree.Ident(enumMapOfTypeSym, TypeRepr.NoType, t.origin), t.args, enumMapOfTypeSym, n.tpe, t.origin)
    case _ => scala.None
  ...
  /** `java.util.Collections`' static utilities — receiver-less, so `rewrite` never sees them and the call is emitted verbatim against the real JDK class unless mapped here. Keyed on `owner#name`
    * (`PortabilityCheck.exactMember`'s identification). Deliberately small: an unmapped static fails to COMPILE rather than silently approximating (a read-only `Buffer` view for `unmodifiableList`
  ...
  private[transform] def staticRewrite(t: Tree.Apply)(using p: Program): Option[Term] =
    ...
    (member, t.args) match
      ...
      case (Some("java.util.Arrays#asList"), args) =>
        asListArgs(args) match
          ...
          case AsList.Elements(as) =>
            Some(
              elementArg(t).fold(factory(sym("asList"), as))(a =>
                Tree.Apply(
                  Tree.TypeApply(Tree.Ident(sym("asList"), TypeRepr.NoType, t.origin), List(a), TypeRepr.NoType, t.origin),
                  as,
                  sym("asList"),
                  t.tpe,
                  t.origin
      ...
      case (Some("java.util.stream.IntStream#range"), List(a, b)) =>
        Some(Tree.Apply(Tree.Ident(sym("intRange"), TypeRepr.NoType, t.origin), List(a, b), sym("intRange"), asBuffer(t.tpe), t.origin))
      ...
      case (Some("java.util.stream.Stream#mapToDouble"), List(f)) if collapsed(recv) =>
        recv.map(r => Tree.Apply(Tree.Ident(sym("mapToDouble"), TypeRepr.NoType, t.origin), List(r, f), sym("mapToDouble"), asBuffer(r.tpe), t.origin))
      case (Some("java.util.stream.DoubleStream#sum" | "java.util.stream.IntStream#sum" | "java.util.stream.LongStream#sum"), Nil) if collapsed(recv) =>
        recv.map(r => Tree.Select(r, sumSym, t.tpe, t.origin))
      case (Some("java.util.stream.Stream#sorted"), List(cmp)) if collapsed(recv) =>
        recv.map(r => Tree.Apply(Tree.Ident(sym("sortedWith"), TypeRepr.NoType, t.origin), List(r, cmp), sym("sortedWith"), r.tpe, t.origin))
      case (Some("java.util.stream.Stream#collect"), List(collector)) if collapsed(recv) && qualified(collectorOf(collector)).contains("java.util.stream.Collectors#toCollection") =>
      ...
      case (Some("java.util.stream.Stream#filter"), List(pred)) if filteredSym != SymId.None && collapsed(recv) =>
        recv.map(r => Tree.Apply(Tree.Ident(filteredSym, TypeRepr.NoType, t.origin), List(r, pred), filteredSym, r.tpe, t.origin))
      ...
      case (Some("java.util.stream.Stream#anyMatch"), List(pred)) if existsSym != SymId.None && collapsed(recv) =>
        recv.map(r => Tree.Apply(Tree.Select(r, existsSym, TypeRepr.NoType, t.origin), List(pred), existsSym, t.tpe, t.origin))
      case (Some("java.util.stream.Stream#allMatch"), List(pred)) if forallSym != SymId.None && collapsed(recv) =>
        recv.map(r => Tree.Apply(Tree.Select(r, forallSym, TypeRepr.NoType, t.origin), List(pred), forallSym, t.tpe, t.origin))
      ...
      case (Some("java.util.stream.Stream#collect"), List(collector)) if collapsed(recv) && qualified(collectorOf(collector)).contains("java.util.stream.Collectors#toList") =>
        recv
      // `collect(toSet)` / `collect(toMap)` — need helpers, cannot guess target type.
      case (Some("java.util.stream.Stream#collect"), List(collector)) if collapsed(recv) && qualified(collectorOf(collector)).contains("java.util.stream.Collectors#toSet") =>
        recv.map(r => factory(sym("toSet"), List(r)))
      case (Some("java.util.stream.Stream#collect"), List(collector)) if collapsed(recv) && qualified(collectorOf(collector)).contains("java.util.stream.Collectors#toMap") =>
  ...
  /** The single type argument of an applied type, or `None` — the one shape [[asListViewArg]] compares. Not a general "element type of": `Buffer[A]`/`Array[A]` both have exactly one.
    */
  ...
  /** Which of `Arrays.asList`'s two java shapes a call site is — see [[asListArgs]]. Not an `Option`: the aliasing form is a DIFFERENT helper, not "no arguments to pass".
    */
  private[transform] enum AsList:
    case Elements(args: List[Term])
    case Aliased(array: Term)

  /** A `JavaCollections` static by name. Minted eagerly in `run`, before the traversal consults it. An unlisted name yields `SymId.None`, treated as "not available", never a dangling ref.
    */
  ...
  /** Convert a `stream()` receiver to the scala sequence the collapse consumes. Shims use `asScalaBuffer`; `Set`/`Map` sources use `.toBuffer`.
    */
  private[transform] def streamSource(r: Term, m: SymId)(using p: Program): Term =
    val effective = headSym(r.tpe).filter(s => kindOf.contains(s) || shimSyms.contains(s)).orElse(p.symbolOf(m).flatMap(x => p.symbolOf(x.owner)).flatMap(o => remap.get(o.id)))
    effective match
  ...
  /** The other direction from [[coerce]]: a shim reaching a slot that wants a scala collection — `new ArrayList<>(c)` routed through `ArrayBuffer.from`, which needs an `IterableOnce`. Both directions
    * exist because the two families are deliberately unrelated.
    */
  ...
  /** True when the receiver is already collapsed from a `Stream` to a scala collection. Shims excluded (collapse consumes them, not produces). Keyed on `kindOf`.
    */
  ...
  /** The same type with `Buffer` as its head — what `asScalaBuffer` on a `JavaCollection[E]` returns. Falls back to a bare `Buffer` when the input has no head (`NoType`, common on an external call's
    * node) — the head is the only part any caller reads.
    */
  ...
  private[transform] def mayBeRetypedValue(a: Term)(using p: Program): Boolean =
    headSym(a.tpe).exists(s =>
      kindOf.contains(s) || shimSyms.contains(s) ||
        p.symbolOf(s).exists(_.fullName == CollectionsTransform.ObjectFqn)
    )

  /** `java.lang.Object` as this program spells it — the bridge's result type. Falls back to the argument's own type where the program never names `Object`.
    */
  private[transform] def objectTpe(a: Term)(using p: Program): TypeRepr =
    p.symbols.all.find(_.fullName == CollectionsTransform.ObjectFqn).map(s => TypeRepr.TypeRef(TypeRepr.NoPrefix, s.id)).getOrElse(a.tpe)
  ...
  /** the shims as FQNs, so a `typeMap` target can be recognised as one — [[shimSyms]] answers only for a program that names the shim's java original, interned on first reference.
    */
  private[transform] def shimFqns: Set[String] = CollectionsTransform.ShimFqns

  /** True when the type is a shim or inherits from one (transitively, fuel-bounded). Suppresses arity rewrites on receivers that carry java's member shape.
    */
  ...
  /** kind-aware call rewrite; `None` = leave the call as-is (same-named method binds to the scala API against the retyped receiver at compile time).
    */
  private[transform] def rewrite(k: Kind, recv: Term, m: SymId, so: Origin, t: Tree.Apply)(using Program): Option[Term] =
    ...
    /** is the receiver one of the runtime SHIMS rather than a scala collection? Java's arity and member names mean the scala-shaped rewrites below must leave them alone — a blanket refusal (`case _
      * if onShim`), asked of the ANCESTRY via [[shimShaped]], not the head symbol.
      */
    val onShim = shimShaped(recv.tpe)

    /** is the receiver `super`? Scala allows `super` only as a Select qualifier; several rewrites below would otherwise place it elsewhere (E040 syntax error). Checked structurally after the rewrite
      * is built — see [[superPlaced]] — so a new arm cannot reintroduce it by omission.
      */
    ...
    val out     = (name, t.args, k) match
      ...
      case ("spliterator", Nil, k @ (Kind.Seq | Kind.Stack | Kind.Set)) if orderedSpliteratorSym != SymId.None =>
      ...
      case ("putIfAbsent", List(key, v), Kind.Map) if sym("putIfAbsent") != SymId.None =>
        Some(
          Tree.Apply(Tree.Ident(sym("putIfAbsent"), TypeRepr.NoType, so), List(recv, keyArg(key, recv), v), sym("putIfAbsent"), t.tpe, t.origin)
        )
      ...
      case ("sort", List(c), Kind.Seq) if sym("sort") != SymId.None =>
        Some(Tree.Apply(Tree.Ident(sym("sort"), TypeRepr.NoType, so), List(recv, c), sym("sort"), t.tpe, t.origin))
      case ("computeIfAbsent", List(key, f), Kind.Map) if sym("computeIfAbsent") != SymId.None =>
        Some(
          Tree.Apply(
            Tree.Ident(sym("computeIfAbsent"), TypeRepr.NoType, so),
            List(recv, keyArg(key, recv), f),
            sym("computeIfAbsent"),
            t.tpe,
            t.origin
          )
      ...
      case ("removeIf", List(p), Kind.Seq) if sym("removeIf") != SymId.None =>
        Some(Tree.Apply(Tree.Ident(sym("removeIf"), TypeRepr.NoType, so), List(recv, p), sym("removeIf"), t.tpe, t.origin))
      case ("removeIf", List(p), Kind.Set) if sym("removeIfSet") != SymId.None =>
        Some(Tree.Apply(Tree.Ident(sym("removeIfSet"), TypeRepr.NoType, so), List(recv, p), sym("removeIfSet"), t.tpe, t.origin))
      case ("containsValue", List(v), Kind.Map) if sym("containsValue") != SymId.None =>
        Some(Tree.Apply(Tree.Ident(sym("containsValue"), TypeRepr.NoType, so), List(recv, v), sym("containsValue"), t.tpe, t.origin))
      case ("containsAll", List(c), Kind.Seq | Kind.Set) if sym("containsAll") != SymId.None =>
        Some(Tree.Apply(Tree.Ident(sym("containsAll"), TypeRepr.NoType, so), List(recv, c), sym("containsAll"), t.tpe, t.origin))
      ...
      case ("removeAll", List(c), Kind.Seq | Kind.Set) if sym("removeAll") != SymId.None =>
        Some(Tree.Apply(Tree.Ident(sym("removeAll"), TypeRepr.NoType, so), List(recv, c), sym("removeAll"), t.tpe, t.origin))
      case ("retainAll", List(c), Kind.Seq | Kind.Set) if sym("retainAll") != SymId.None =>
        Some(Tree.Apply(Tree.Ident(sym("retainAll"), TypeRepr.NoType, so), List(recv, c), sym("retainAll"), t.tpe, t.origin))
      case ("ensureCapacity", List(n), Kind.Seq) if sym("ensureCapacity") != SymId.None =>
        Some(Tree.Apply(Tree.Ident(sym("ensureCapacity"), TypeRepr.NoType, so), List(recv, n), sym("ensureCapacity"), t.tpe, t.origin))
      ...
      case ("addAll", List(c), _)
          if (wildcardElement(c.tpe) || standaloneSource(c.tpe)) &&
    ...
    if !onSuper then out
      ...
      out
        .filter(superPlaced)
        .orElse(if superIsThis(recv, name) then rewrite(k, thisOf(recv), m, so, t).filter(superPlaced)
        else scala.None)

  /** May a rewrite that cannot stand on `super` stand on `this` instead — do `super.m` and `this.m` name the same member for every value this expression can have? True iff neither the class nor any
    * subclass IN THIS PROGRAM declares `m` (the port cannot answer beyond its own scope; the alternative is a refused rewrite that does not compile). Both walks read class definitions, not the symbol
    * table; the subclass walk is transitive.
    */
  private[transform] def superIsThis(recv: Term, member: String)(using p: Program): Boolean = recv match
    case Tree.Super(cls, _, _) if cls != SymId.None =>
      ...
      /** does `c` reach `cls` through its parents? Fuel-bounded; an exhausted walk counts as reaching, the conservative answer since the caller refuses on `true`.
        */
  ...
  /** True when re-parenting removed the JDK default this `super.<member>` targeted and no program-declared ancestor declares the member.
    */
  ...
  /** the `this` standing where `recv`'s `super` stood — same class, same origin, whose type every rewrite here reads the receiver's kind from.
    */
  ...
  /** replace every legal `super.member` with a marker the placement scan does not object to (`Tree.This`, discarded after the scan), so the scan sees only misplaced occurrences.
    */
  ...
  /** did java resolve `Collection.remove(Object)` (by value, returning `boolean`) rather than `List.remove(int)` (by index)? A call whose result type the frontend could not record answers `false` and
    * falls back to scala's index removal.
    */
  ...
  /** `null` — the faithful default for a Java `Map.get` miss (Java map values are always reference types, so `null` always type-checks). Ascribed to `V` by [[dflt]].
    */
  private[transform] def nullOf(so: Origin): Term = Tree.Literal(Constant.NullC, TypeRepr.NoType, so)

  /** Ascribe a `getOrElse` default to the map's value type `V` (`default.asInstanceOf[V]`), so inference gives `getOrElse` result type `V` instead of widening to `V | Default` (which breaks e.g.
    * `m.getOrElse(k, 0) + 1` when `V = java.lang.Integer`). Falls back to the bare default when the receiver's `Map[K, V]` isn't fully applied.
    */
  ...
  /** the receiver's (already-retyped, bottom-up) head type, if it is one of our scala collections → its [[Kind]].
    */
  private[transform] def kindAt(recv: Term)(using Program): Option[Kind] = headSym(actualOf(recv)._1).flatMap(kindOf.get)

  /** Kind of a call via an inherited JDK collection method (resolved method's owner in `typeMap`). Covers `extends HashMap` etc. where `kindAt` returns `None`. Suppressed for scoped-out receivers.
    */
  ...
  /** the type a term really has — [[CollectionsTransform.scopedType]] against this run's [[excluded]] set, with a flag for whether the answer came from a scope hold-back. `excluded.isEmpty` always
    * answers `(t.tpe, false)`, the pre-scope code path by arithmetic.
    */
