...
/** Checks whether this module's policy agrees with its declared bases.
  *
  * Two layers: '''static''' (manifest vs manifest, before parsing) and '''dynamic''' (the base's published [[PortMap]] or re-derived policy vs what this run modelled of the shared surface). Prefers
  * the published map; falls back to re-derivation when it is stale or absent.
  */
object ManifestAgreement:
  ...
  /** A base module's manifest and its published map (if usable).
    * @param map
    *   published [[PortMap]], if fresh. Stale maps are refused (`stale` carries why).
    * @param jdk
    *   `(published, running)` on JDK mismatch -- fatal, fallback cannot help.
    */
  ...
  enum Kind(val fatal: Boolean, val classification: String):
    ...
    /** a TYPE both modules see is moved — renamed, sub-packaged or flattened — differently. Separate from [[RenameDivergence]]: a package divergence moves a whole namespace, visible in every
      * `import`; a per-TYPE one moves ONE class, invisible until the dependent names it. Both are fatal — the two ports each compile alone and cannot compile together.
      */
    ...
    /** two instances of one phase in one effective pipeline that the fold could neither MERGE nor prove equal — configured differently, or configured in a way the engine cannot read.
      */
    ...
    /** one location, two remedy SELECTIONS, in one policy chain.
      *
      * A sibling of [[SurfaceDivergence]] and not an instance of it: that one is about two INSTANCES of a phase whose policies could not be composed, and this is about one key with two values, which
      * composes perfectly (nearest wins) and composes WRONG.
      */
    ...
    /** a base selects a remedy at a location this module answers NOTHING at. `MissingDrop` read at a member key, reachable only on the `mirroring` path: an INHERITING module's `effectiveResolutions`
      * already contains its base's (vacuous there); a module stating its policy IN FULL has no such guarantee, and [[ResolutionDivergence]] cannot see the omission since its `policyChain` is
      * `List(this)` here.
      */
    ...
    /** a base this module depends on claims NO namespace, which switches off the intrusion screen that protects its surface from this module. Not fatal: nothing has gone wrong YET, and the base's own
      * author is the one who must act. Loud, because the alternative is a screen that reports nothing whether it passed or was never asked.
      */
    ...
    /** the base's map was found and is PROVEN out of date. Not fatal: the check falls back to re-derivation, which is weaker but still valid. Loud, because the fallback is silent.
      */
    case BaseMapStale
        extends Kind(
          false,
    ...
    /** the base's map was published by a JVM of a DIFFERENT JDK specification than this run's. FATAL, the only member of this family that is: `Stale`/`Missing` fall back to re-deriving the base's
      * decisions from its manifest (honest, since re-derivation would agree with a re-run base). A JDK mismatch breaks that — the base's EMITTED SCALA was produced from a different set of class
      * files, and no re-derivation on THIS JVM reproduces it.
      */
    case BaseMapJdk
        extends Kind(
          true,
    ...
    manifest match
      case None =>
        if foreignRoots then
          List(
            Finding(Kind.NoBaseDeclared, "-", "(this run)", "resolution roots outside the source root, and no `PortManifest` at all")
          )
        else Nil
      case Some(m) =>
        val noBase =
          if foreignRoots && m.baseChain.isEmpty then
            List(
              Finding(Kind.NoBaseDeclared, "-", m.name, "resolution roots outside the source root, and the manifest declares no `bases`")
            )
          else Nil
  ...
  private def mapHealth(ports: List[BasePort]): List[Finding] =
    ports.sortBy(_.name).flatMap { p =>
      p.jdk.map((published, running) => Finding(Kind.BaseMapJdk, p.name, s"${p.name} port map", s"published by a JVM on JDK $published; this run is on JDK $running")).toList ++
  ...
  private def statik(m: PortManifest, fired: Set[String], ports: List[BasePort]): List[Finding] =
    ...
    val perBase = m.baseChain.flatMap { b =>
      ...
      val missingTypes   = (bDropTypes -- mine).toList.sorted.map(k => Finding(Kind.MissingDrop, b.name, k, "declared `dropTypes` in the base, absent here"))
      ...
      val extraTypes = (mine -- bDropTypes)
        .filter(k => b.claims(k) && baseHas(k))
        .toList
        .sorted
        .map(k => Finding(Kind.ExtraDrop, b.name, k, "dropped here, inside the base's declared namespace, and not by the base"))
      val extraMethods = (myMethods -- bDropMethods)
        .filter { k =>
          val owner = k.takeWhile(_ != '#'); b.claims(owner) && baseHas(owner)
        }
        .toList
        .sorted
        .map(k => Finding(Kind.ExtraDrop, b.name, k, "method dropped here, inside the base's declared namespace, and not by the base"))
      ...
      def subsumes(bp: balticporter.tir.Phase): Boolean = bp match
        case mergeable: MergeablePolicy =>
          m.effectiveSurface.filter(_.name == bp.name).exists(mine => mergeable.mergedWith(mine).exists(r => PortManifest.fingerprint(r.phase) == PortManifest.fingerprint(mine)))
        case _ => false
      val surfaceGap = b.effectiveSurface.distinct
        .filterNot(p =>
          mySurface.contains(PortManifest.fingerprint(p)) ||
            m.surfaceFold.absorbed.contains(PortManifest.fingerprint(p)) || subsumes(p)
        )
        .map(PortManifest.fingerprint)
        .distinct
        .map(f => Finding(Kind.SurfaceMissing, b.name, f, "signature-affecting phase present in the base's surface, absent from this module's"))
  ...
  /** Surface half of the static layer: what the fold could not compose + intrusion screen. Shared with [[surfaceGate]] so the gate and the report use one derivation.
    */
  private def surfacePairs(m: PortManifest, ports: List[BasePort]): List[Finding] =
    ...
    val intrusions      =
      m.surfaceFold.intrusions.filterNot(i => divergentPhases.contains(i.phase)).filter(i => standsAt(ports, i.base, i.subject)).groupBy(_.phase).toList.sortBy(_._1).map { (phase, is) =>
        val first = is.head
        Finding(
          Kind.SurfaceIntrusion,
          m.name,
          phase,
    ...
    val chainConflicts = m.resolutionConflicts.map { (key, claims) =>
      Finding(
        ...
        claims.map(_._1).distinct.mkString("+"),
        key,
        claims.map((who, k, id) => s"""`$who` selects "$id"""" + (if k == key then "" else s" at `$k`")).mkString(", ")
      )
    }
    ...
    val mirrored = if m.inherit then Nil
    else
      m.baseChain.flatMap { b =>
        b.effectiveResolutions.toList.sorted.flatMap { (key, id) =>
          myRes.find((k, _) => MemberKey.mayNameSame(k, key)) match
            ...
            case None => List(Finding(Kind.MissingResolution, b.name, key, s"""base `${b.name}` selects "$id" here; this module selects nothing"""))
        }
      }
  ...
  /** Does anything stand at `subject` in `base`'s output? Uses the published map when usable; falls back to `true` (strictly more refusing) when no map is available.
    */
  ...
  private def evidence(ports: List[BasePort], base: String, subject: String): String =
    ports.find(_.name == base).flatMap(_.map) match
      case Some(m0) =>
        m0.byUpstream("type").get(subject).map(e => s""" — its published map emits it as "${e.emitted}"""").getOrElse("")
      case scala.None =>
  ...
  private def dynamic(m: PortManifest, shared: List[SharedType], ports: List[BasePort]): List[Finding] =
    if shared.isEmpty then Nil
        ...
        emittedByBase.get(t.upstreamFqn).orElse(emittedByBaseName.get(t.emittedFqn)) match
          ...
          case Some((who, e)) =>
            ...
            if notTranslated == t.substituted then Nil
              ...
              List(
                Finding(Kind.TagUnexpected, who, t.upstreamFqn, s"""this run tagged it `Substituted`; the base emitted it as "${e.emitted}"""")
              )
          case scala.None =>
            claimedBy(t.upstreamFqn) match
              ...
              case scala.None =>
                ...
                if expectedDrop == t.substituted then Nil
                  ...
                  List(
                    Finding(Kind.TagMissing, baseName, t.upstreamFqn, "the base declares it dropped; this run modelled it as an ordinary type")
                  )
                else List(Finding(Kind.TagUnexpected, baseName, t.upstreamFqn, "this run tagged it `Substituted`; the base emits it mechanically"))
      }
      ...
      val names = sorted
        ...
        .groupBy(t => PortManifest.longestPrefix(t.upstreamFqn, prefixes).getOrElse(t.upstreamFqn.take(t.upstreamFqn.lastIndexOf('.').max(0))))
        .toList
        .sortBy(_._1)
        .map { (prefix, ts) =>
          val e   = ts.head
