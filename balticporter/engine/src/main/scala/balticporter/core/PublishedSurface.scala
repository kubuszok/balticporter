...
final class PublishedSurface(
  ...
  /** each base's published contract, module name → map. Empty is the ordinary single-module case and makes every non-owned question `Unknown`, which is correct and, for a run with no base, never
    * consumed.
    */
  ...
  /** Every base's member rows, grouped by `owner#name` (the overload set). Keyed on bare name because `Symbol.fullName` carries no parameter spelling. `Dropped` rows are excluded: they have no
    * emitted shape.
    */
  private lazy val memberRows: Map[String, List[(String, PortMap.Entry)]] =
    bases
      .flatMap { (mod, m) =>
        m.members.filter(e => e.emitted.nonEmpty && e.disposition != PortMap.Disposition.Dropped).map(e => bareName(e.emitted) -> (mod, e))
      }
      .groupMap(_._1)(_._2)
        ...
        case scala.None =>
          Surface.Answer.Unknown(
            if bases.isEmpty then s"this run declares no base port, so nothing publishes a contract for $fqn"
              ...
              s"no declared base publishes a contract row for $fqn " +
                s"(searched ${bases.map(_._1).sorted.mkString(", ")})"
            ,
            bases.map(_._1).headOption.filter(_ => bases.sizeIs == 1)
          )

  /** Is this symbol an executable? Read from the definition, not `Symbol.descriptor` (which is also `None` for unresolved externals).
    */
  ...
  /** Look up a member's shape by choosing rows matching the symbol's kind (field vs executable), then requiring agreement across overloads. `Unknown` when overloads disagree.
    */
  def memberShape(s: SymId): Surface.Answer[Surface.MemberShape] =
    if owns(s) then Surface.Answer.Own
      ...
      mine match
        ...
        case Nil =>
          Surface.Answer.Unknown(
            s"no declared base publishes a contract row for the member $fqn",
            bases.map(_._1).headOption.filter(_ => bases.sizeIs == 1)
          )
