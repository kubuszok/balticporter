...
object PortManifestConfig:
  ...
  /** Extract (dropTypes, dropMethods, packageRenames) from a base's published map. `Dropped`/`Substituted` types both become drops; renames are recovered as longest common prefixes.
    */
  def declarativeFrom(map: PortMap.Map0): (Set[String], Set[String], Map[String, String]) =
    ...
    val renames = map.types
      .collect {
        case e if e.disposition == PortMap.Disposition.Renamed && e.upstream.nonEmpty && e.emitted.nonEmpty =>
          ...
          while i < e.upstream.length && i < e.emitted.length &&
            e.upstream.charAt(e.upstream.length - 1 - i) == e.emitted.charAt(e.emitted.length - 1 - i)
          do i += 1
      ...
      .filter((f, t) => f.nonEmpty && t.nonEmpty)
      .distinct
      .toMap
    (drops, dropMethods, renames)

  /** Build a manifest whose declarative half comes from a base's published map. `bases` is left empty; callers with the base value should use `extendedBy` instead.
    */
  def fromPortMap(
    name:    String,
    map:     PortMap.Map0,
    ...
    PortManifest(name = name, governs = governs, dropTypes = dt, dropMethods = dm, packageRenames = pr, surface = surface, inject = inject)

  private val Header = "#setting\tvalue\tto"

  /** Render the declarative half as TSV. The `surface` is NOT rendered (it is code); its count is stated instead.
    */
