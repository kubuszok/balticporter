...
/** What a module's port actually did to its upstream surface, published for its dependents. A projection of SrcMap, Substitutions, PackageRenameTransform, RuntimePlan and MethodBodyTransform.
...
object PortMap:
  ...
  enum Disposition:
    ...
    /** not translated; replaced at the same FQN by injected Scala. A caller sees the same name and a different implementation.
      */
    case Substituted

    /** not emitted and NOT replaced. Every reference must have been rewritten away — a dependent that still calls it will not compile, which is the point of recording it.
      */
    case Dropped

    /** present in the port and absent upstream: an injected type, a runtime shim, or a member a library-specific rule introduced.
      */
    case Added

  /** @param upstream
    *   Java-side name (`owner#name(P1,P2)` for a member)
    * @param emitted
    *   emitted Scala name; empty when [[Disposition.Dropped]]
    * @param body
  ...
  /** @param sources
    *   source fingerprint at publication time (see [[sourcesDigest]])
    * @param files
    *   count of distinct Java files the fingerprint covers
    * @param policy
    *   sorted `SurfacePolicy` fingerprint (see [[policyDigest]])
    * @param jdk
    *   `java.specification.version` of the publishing JVM
    * @param schema
    *   schema the map was read at
    */
  ...
  /** Reverse a package rename: emitted name -> upstream name. Longest matching value prefix wins; ambiguous targets decline.
    */
  /** Strip generic arguments from a member key's parameter list: `f(Class<T>)` -> `f(Class)`. Reconciles emitted (generic) keys to the erased manifest form for lookup.
    */
  ...
  def of(
    ...
    val droppedEntries = dropTypes.toList.sorted.map { fqn =>
      ...
      if injectedFqns(at) then Entry("type", fqn, emitted, Disposition.Substituted, shape = typeShapes.getOrElse(at, ""))
      else Entry("type", fqn, emitted, Disposition.Dropped, shape = typeShapes.getOrElse(at, "form=class"))
    }

    // Genuine additions: files that replace no drop (runtime support, port helpers).
    val replacements = dropTypes.map(emittedAt)
    val added        = (injectedFqns -- replacements).toList.sorted.map(fqn => Entry("type", "", fqn, Disposition.Added))

    val memberEntries = srcMap.entries.filter(_.kind != "class").sortBy(e => (e.unit, e.member)).map { e =>
    ...
    val refusedEntries = refusedMembers.toList.sortBy(_._1).map { (emitted, shape) =>
      ...
      Entry("member", erase(upstreamOf(emitted, originOf.getOrElse(unit, ""), renames)), emitted, Disposition.Dropped, shape = shape)
    }

    val bare = Map0(module, engine, typeEntries ++ droppedEntries ++ added ++ memberEntries ++ droppedMembers ++ refusedEntries, policy = policy, jdk = jdk)
    sourceRoot match
  ...
  /** Compare a published map against the current engine, sources on disk, policy and JDK. Empty `policy`/`jdk` skips the respective comparison.
    */
  def freshness(m: Map0, engine: String, roots: List[Path], policy: String = "", jdk: String = ""): Freshness =
    if m.engine.nonEmpty && m.engine != engine then Freshness.Stale(s"published by engine ${m.engine}; this run is $engine — re-run the base port")
    // JDK check before policy/sources: this mismatch hides behind matching source digests.
    else if jdk.nonEmpty && m.jdk.nonEmpty && m.jdk != jdk then Freshness.JdkMismatch(m.jdk, jdk)
    ...
    else if m.sources.isEmpty then Freshness.Unverified("the map carries no source fingerprint (published by an older engine)")
    else
    ...
    if !Files.exists(p) then Left(s"no port map at $p")
      ...
      schema match
        case Some(s) if s > Schema =>
          Left(
            s"port map at $p declares schema $s; this engine reads $Schema — it was published by a " +
  ...
  /** Discover all port maps under `reportRoot`, keyed by module name from the file header. `run-latest` wins over `baseline`. `exclude` prevents a module from reading its own map.
    */
  def discover(reportRoot: Path, exclude: Set[String] = Set.empty, configured: List[Path] = Nil): List[Published] =
    discoverIn(reportRoot :: searchPath(configured), exclude)

  /** Extra report roots: `configured` if non-empty, else the `baseReports` debug flag as fallback. Not merged: an extra root can only add a base.
    */
