...
object EngineFingerprint:
  ...
  lazy val value: String =
    ...
    ).map(_.getProtectionDomain.getCodeSource).flatMap(cs => Option(cs).map(_.getLocation.toURI)).distinct
...
/** Action-cache keys for TIR emission. Three-part key: engine fingerprint, canonical TIR digest, dependencies' interface hashes (early cutoff). ADVISORY: whole-program decisions (clash renames, ctor
  * funnel, diamond overrides) can invalidate units with no reference edge.
  */
object TirCacheKey:
  ...
  /** `unit symbol -> action key` for every unit given. Includes the decision log digest because porter notes are in the emitted text but not in the tree.
    */
  def forUnits(program: Program, units: List[Tree.ClassDef], decisions: List[Decision] = Nil): Map[SymId, String] =
    ...
    val notes     = Digest.string(decisions.filter(d => PorterNote.Rendered(d.kind)).map(_.tsv).sorted.mkString("\n"))
    units.map { u =>
  ...
  private def javaDigest(path: String): String =
    if path.isEmpty then ""
      ...
      javaDigests.getOrElseUpdate(
        path, {
          val p = java.nio.file.Path.of(path)
          if java.nio.file.Files.isRegularFile(p) then
            try Digest.string(java.nio.file.Files.readString(p))
