...
object ExternalUsage:
  ...
  /** Every referenced symbol (owned or not), in `program.referenced` order. Callers wanting only externals use [[external]].
    */
  def all(program: Program): List[Row] =
    program.referenced.toList.flatMap { id =>
      program.symbolOf(id).map { sym =>
        Row(id, sym.fullName, program.symbolOf(sym.owner).map(_.fullName), sym.name, sym.descriptor, program.usages(id))
      }
    }
  ...
  /** Write `external-surface.tsv` with both `emitted` and `all` lanes. Gated on the artifact layer by the caller.
    */
