...
/** Which third-party ARTIFACTS the port's declared backends need, and which its build does not name — a BUILD-GRAPH question, not a symbol-reference one. A finding is *a usage FIRED* ∧ *no dependency
  * covers it* ∧ *no declared alternative*; [[unneeded]] is the opposite (on `policy`). Coverage compares ORIGINAL and EMITTED programs (a redirect can remove the JDK usage a coordinate answers,
  * `ENGINE-LIMITS.md` P8). [[Provides]] unfetchable = `Unverifiable` (§4.6).
  */
object DependencyCheck:

  /** the RESIDUE lane: requirements that survived BOTH filters — this module's own emitted code (D2), and no declared dependency covering them. The number a reader acts on.
    */
  val Name = "dependency-coverage"

  /** the ENUMERATION behind it — every requirement the walk found, before either filter (`portability(all|emitted)`'s reason one check over: a residue of zero and an empty walk are the same row, and
    * a dependent port is where they come apart). NOT spelled `(emitted)`: the residue passes TWO filters (belonging to the base, or covered by a declared dependency).
    */
  val All = "dependency-coverage(all)"

  /** the THIRD artifact of the family, counting DECLARATIONS rather than usage sites: `policy = 0` is a bar a port can hold by DECLARING NOTHING, invisible to the usage-enumerating lanes above
    * (`ENGINE-LIMITS.md` P8). One row per declared coordinate, naming its [[Cell]]; the `policy` residue is the subtraction.
    */
  ...
  final case class Requirement(
    ...
    row:  ApiRow,
    /** the declared targets that need it, and the artifact each names — two platforms can want DIFFERENT artifacts for one API, so this is a map and not one coordinate.
      */
  ...
  /** filters requirements already covered by a declared artifact, matched on org+name — never the revision: a port pinning a different version has already answered the question.
    */
  ...
  /** what an artifact says it provides, read from the jar and nothing else. THREE-valued: [[Unverifiable]] is a jar this run could not read (no network, dropped snapshot, no `cs`) — not a coordinate
    * that provides nothing, since collapsing it either way fabricates an answer (§4.6). [[Known]] holds CLASS names as `Symbol.fullName` spells them, every enclosing prefix of a nested entry
    * included, so a match is equality and never `startsWith` (§4.56).
    */
  enum Provides:
    case Known(classes: Set[String])
    case Unverifiable(why: String)

  /** WHICH of [[uses]]' three evidences answered — a value, never re-derived from the `why` prose. Decides whether a JVM compile of the emitted code needs the coordinate on its classpath: [[Catalog]]
    * answers a JDK API the JVM already has (a JS-only need), the other two mean the emitted code NAMES the artifact directly.
    */
  enum Evidence:
    case Catalog, Classes, Spliced

  /** the answer ONE program gives about ONE artifact — three-valued for [[Provides]]'s reason, and carrying its own evidence for the reader to check.
    */
  ...
  /** one cell of the 2×2 — see the object's own doc for the table.
    *
    * @param keep
    *   whether the coordinate must STAY. The emitted column alone decides it; the original column decides which sentence the reader gets.
    */
  enum Cell(val label: String, val keep: Boolean, val advice: String):
    case Covered extends Cell("covered", true, "the API this coordinate answers is called in this module's own emitted code — nothing to do")
    case Introduced
        extends Cell(
    ...
    /** [[Introduced]] with the ORIGINAL column UNKNOWN: the emitted column answered `Yes` from the CATALOG half (no jar needed) while the original column's jar could not be read, so its own sentence
      * would assert what the run does not know (§4.6). KEEP is unchanged either way.
      */
    case IntroducedOriginalUnknown
        extends Cell(
          "introduced by translation (original unknown)",
          true,
  ...
  /** every dotted name a phase SPLICED into this program as literal text — the third evidence, and the one no symbol table holds. A `Tree.Opaque` is ready-made Scala the engine deliberately does not
    * parse, so a symbol-keyed check reads past it. Walked with `StandardTraversal` over the PROGRAM (§1 — `Rewrite.accountedBy`). Returns the MAXIMAL dotted run, uncut — [[namesClass]] cuts.
    */
  ...
  /** the maximal `ident(.ident)+` runs of a piece of ready-made Scala — a CANDIDATE list, each tested against a jar's own class listing. The hole marker (`Tree.Opaque.Mark`, NUL) is an
    * IDENTIFIER-IGNORABLE control character (JLS 3.8), so it would otherwise glue onto the literal name in front of it — exclude the whole ignorable CLASS, not just the marker.
    */
  ...
  /** does `name` — or any prefix of it cut at a `.` — name a class this artifact declares? Equality at every cut, never `startsWith` (§4.56): `classes` already holds each enclosing prefix, and a
  ...
  /** does THIS program use THIS artifact — the derivation both columns of the coverage table read. The union of THREE evidences, catalog half first: a `Depend` row, a reference to a class the
    * artifact declares, or a name a phase SPLICED in as literal text. `external` is this module's own external usage rows (D2, the same list `jdk-surface` reads); `spliced` is [[splicedNames]],
    * `Set.empty` for the pre-pipeline program.
    */
  def uses(dep: ArtifactDep, reqs: List[Requirement], external: List[ExternalUsage.Row], provides: ArtifactDep => Provides, spliced: Set[String] = Set.empty): Answer =
    ...
    if byCatalog.nonEmpty then
      ...
      Answer.Yes(
        s"${byCatalog.size} site(s) at ${apis.take(3).mkString(", ")}" +
      ...
      provides(dep) match
        ...
        case Provides.Known(classes)    =>
          ...
          if hits.nonEmpty then
            ...
            Answer.Yes(
              s"${hits.map(_.sites).sum} reference(s) to ${names.take(3).mkString(", ")}" +
            ...
            if text.isEmpty then Answer.No
              ...
              Answer.Yes(
                s"${text.size} spliced name(s) — ${text.take(3).mkString(", ")}" +
                  (if text.sizeIs > 3 then s" (+${text.size - 3} more)" else "") +
                  " — written as literal Scala by a surface phase, which interns no symbol",
                Evidence.Spliced
              )

  /** the 2×2 itself: one [[Declaration]] per declared coordinate. PRE-pipeline halves are BY NAME and forced exactly once, here — never per declaration, or a by-name parameter would re-walk the
    * program per coordinate. @param before/beforeExternal this module's own PRE-pipeline requirements/usage rows @param after the same over the EMITTED program @param splicedAfter [[splicedNames]]
    * over the EMITTED program (the pre-pipeline one holds no spliced text).
    */
  def declarations(
    declared:       List[ArtifactDep],
    before:         => List[Requirement],
    beforeExternal: => List[ExternalUsage.Row],
    after:          List[Requirement],
    afterExternal:  List[ExternalUsage.Row],
    provides:       ArtifactDep => Provides,
    splicedAfter:   Set[String] = Set.empty
  ): List[Declaration] =
    if declared.isEmpty then Nil
    else
  ...
  /** the same filter read backwards: a declared artifact no requirement in this module's own emitted code names. Reported as `NeverApplied` (not `NeverMatched`): well formed, real artifact, but no
    * requirement fired. The SUBTRACTION of [[declarations]], not a second derivation: rows whose [[Cell]] does not [[Cell.keep]], carrying that cell's sentence.
    */
  ...
  /** violations in code this run actually EMITS — the same D2 filter every other check carries: an artifact a base's declaration needs is the BASE's to add.
    */
  ...
  /** @param lane
    *   which of the two counts these rows are recorded as — [[All]] or [[Name]] — so a hard-coded name does not merge both counts into one bucket in `findings.tsv`.
    */
  def report(reqs: List[Requirement], declared: List[ArtifactDep], lane: String = Name)(using program: Program): List[CheckReport.Finding] =
    reqs.map { r =>
      ...
      CheckReport.Finding(
        ...
        CheckReport.relativise(r.origin.javaPath),
        r.origin.line,
  ...
  /** the [[Declared]] lane — one row per declared coordinate, whatever its cell, reported apart from the residue (trivia family's reason).
    *
    * There is no `Origin` for a build coordinate and inventing one would be a fabricated fact, so the `path` column carries the manifest FIELD and the line is 0 (`PolicyReport`'s own convention).
    */
  def reportDeclared(decls: List[Declaration]): List[CheckReport.Finding] =
    decls.map { d =>
      CheckReport.Finding(
        ...
        d.cell.label,
        d.dep.toString,
        balticporter.core.PolicyReport.DependencySetting,
        0,
        s"original: ${d.original.why} | emitted: ${d.emitted.why} — ${d.cell.advice}"
      )
    }

  /** the same rows as an ARTIFACT a BUILD can read (`run-latest/dependencies.tsv`) — one value, one spelling, so a measure lane derives its classpath from the run (CLAUDE.md §1.5). `coordinate` is
    * the EXPLICIT jvm form, never a `::` shorthand that can differ per checkout. `onClasspath` is DERIVED here since only the run has the evidence — Unknown takes the INCLUDING arm (an unneeded jar
    * costs a resolution, a missing one a wall of errors).
    */
  ...
  def declaredTsv(decls: List[Declaration], coordinateOf: ArtifactDep => String): List[String] =
    decls.map { d =>
      ...
      List(
        ...
        d.dep.name,
        d.dep.rev,
        d.dep.cross.toString.toLowerCase,
        d.dep.resolver.getOrElse(""),
        coordinateOf(d.dep),
        if onCp then "yes" else "no",
        d.cell.label,
        d.emitted.why
      ).mkString("\t")
    }

  /** grouped one-line summary, most-referenced first — `PortabilityCheck.summary`'s shape, because an operator reads the two lines together.
    */
  def summary(reqs: List[Requirement]): String =
    if reqs.isEmpty then "  none"
    else
      reqs
        .groupBy(r => (r.api, r.deps))
        .toList
        .sortBy(-_._2.size)
