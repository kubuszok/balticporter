...
/** Pre- and post-rewrite signature verification.
  *
  * [[impact]]: every usage site of the given symbols (BEFORE a rewrite). [[check]]: every usage that disagrees with its declaration's current signature (AFTER). Only reports on symbols this program
  * defines; external symbols are skipped.
  */
object RewriteTrace:
  ...
  final case class Mismatch(what: String, sym: SymId, name: String, expected: Int, found: Int, origin: Origin):
    ...
    def report: CheckReport.Finding =
      CheckReport.Finding("signature", what, name, CheckReport.relativise(origin.javaPath), origin.line, s"expects $expected, found $found")
          ...
          case Some(n) if n != args.size =>
            List(
              Mismatch(
                "type-argument arity",
                headSym(tc).get,
                program.symbolOf(headSym(tc).get).map(_.fullName).getOrElse("?"),
                n,
                args.size,
                origin
              )
