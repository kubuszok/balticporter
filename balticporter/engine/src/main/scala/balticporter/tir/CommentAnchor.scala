...
/** Which declaration a comment belongs to, decided from the java text. Shared by the recovery backstop and [[TriviaCheck]] (one derivation, two consumers).
  */
object CommentAnchor:

  /** One declaration of a java file, by the line it starts on.
    * @param emitted
    *   false for a member the port drops (its javadoc is classified as deliberate).
    */
  ...
  def membersOf(program: Program): Map[String, List[Member]] =
    ...
    def take(o: Origin, emitted: Boolean): Unit =
      if o.javaPath.nonEmpty && o.line > 0 then acc += (keys.getOrElseUpdate(o.javaPath, key(o.javaPath)) -> Member(o.line, emitted))
    val scan = new Phase:
  ...
  /** The declaration a comment documents: the next one if only blanks/annotations separate them (javadoc), otherwise the previous one (body comment). `None` when there is no member nearby.
    * @param lines
    *   the java file's lines, 0-based.
    */
