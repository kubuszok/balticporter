...
object Remediator:
  ...
  final case class Suggestion(
    ...
    /** Machine-readable values of the same computation `snippet` renders. Empty for the observation fallback.
      */
  ...
  /** Template 1: `Substitutions(dropTypes = Set(W))`. `High` when nothing else references `W`; `Medium` when other units do (needs injection).
    */
        ...
        val proportion       =
          if total > 0 && touched > 0 && touched <= total then
            s"; $touched of its $total member(s) touch the API" +
              (if touched * 2 < total then " — `Substitutions.dropMethods` on just those members is the smaller seam when nothing calls them"
               else "")
          else ""
        (Suggestion(
           "substitutions-drop",
  ...
  /** Template 2: `StaticForwarderTransform.Forwarder(wrapper, receiver, members)`. Precondition checked: a static `W.m(x: X, ...)` forwarding to `X#m`. Excludes members that are themselves unportable
    * (forwarding would relocate the dependency).
    */
  ...
  /** Template 3: `ClassTableTransform(Map(callee -> table))`. Scoped to `Class.forName`. `Medium` because the destination table must be hand-written. Key must be an OWNED member; a direct
    * `Class.forName` call has no selectable key and gets an observation only.
    */
  private def classTableSuggestions(
    ...
    if lookups.isEmpty then Nil
      ...
      val wrappers = lookups
        .flatMap { (v, _) =>
          program.symbolOf(v.enclosing).filter(s => s.flags.isStatic && declares(program, s.owner)).map(s => s"${fullName(program, s.owner)}#${s.name}")
        }
        .distinct
        .sorted
      wrappers match
        ...
        case ws =>
          val shape =
            if ws.isEmpty then s"${lookups.size} direct site(s) of `$ClassForName`"
              ...
              s"${lookups.size} site(s) reached through ${ws.size} of this port's own static " +
          ...
          List(
            (Suggestion(
               ...
               scala.None,
               Some(
                 "no snippet, deliberately: a pasted `java.lang.Class#forName` key is `ExternalOnly` " +
  ...
  private def externalReferrers(program: Program, w: SymId): List[String] =
    program.usages(w).flatMap(u => PortabilityCheck.owningType(program, u.enclosing)).filterNot(_ == w).distinct.map(fullName(program, _)).sorted
