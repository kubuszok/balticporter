...
object OverloadRiskCheck extends RemedySource:
  ...
  enum Issue:
    ...
    /** two applicable candidates differ by a PRIMITIVE against its wrapper or a universal type at one position — java's phase 1/2 boundary.
      */
  ...
  val AscribeJavacChoice: Remedy = Remedy(
    ...
    lane = Name,
    kind = Issue.VarargPhaseSpan.toString,
  ...
  /** the other answer, a STATEMENT and not an act: the operator read this call and accepts that scala may bind another. Not a suppression — the row moves into `remediation(resolved)` with a porter
    * note beside the emitted call, so an unexamined risk becomes an examined one. NOT emission-affecting: changes no tree, so two modules may disagree about it.
    */
  ...
  /** one call at risk. `alternatives` names the candidates so a reader can dismiss the row without re-deriving the candidate set. @param declaration the MEMBER this call is written in — never
    * reported, the key a `resolutions` entry names ([[Resolution]]'s granularity); absent from the row's own text since a finding says WHAT is at risk and WHERE, a different question.
    */
  final case class Finding(issue: Issue, owner: String, member: String, alternatives: List[String], origin: Origin, declaration: SymId = SymId.None):
    def detail: String =
  ...
  /** the findings AND the denominator they came out of. An over-approximation whose false-positive rate nobody can see is a lane that gets ignored. `calls` is every call examined, `overloaded` those
    * with more than one applicable candidate, `findings` the subset spanning a phase — `overloaded - findings.size` is exactly what the narrowing declined to report.
    */
  ...
  final class Overloads(program: Program):
    ...
    /** the type's program-declared ancestors, nearest first. External parents end the walk: their members are in a class file the frontend interns only on reference, so an inherited overload from one
      * is invisible here — stated in the class doc rather than counted as a zero.
      */
    ...
    /** every method named `name` declared by `owner` or by a program-declared ancestor of it. Memoised, since every rendered call asks it and the answer is a fact about the program — without it the
      * emitter walks the owner's member list per call site, a real cost on this corpus's largest port.
      */
  ...
  /** the OTHER side of JLS 5.1.7's boxing conversion — java's phase 1/phase 2 boundary. The wrapper keeps its JAVA name: a class the port references and does not declare.
    */
  ...
  /** a slot that admits a BOXED value and nothing narrower. Deliberately just these three and not java.lang.Number/Comparable, whose wider reference set would turn a phase-boundary count into an
  ...
  /** the head symbol of a type, shared by the receiver root below and [[Overloads]]'s parent walk so a ThisType reaching only one of them cannot stop an implicit receiver resolving.
    */
  ...
  /** WHERE JAVA LOOKED — the receiver's STATIC TYPE (JLS 15.12.1's candidate set), not the resolved callee's OWNER (differs when the winner is inherited). Rooting at the receiver strictly WIDENS the
    * old set. Three shapes: a SELECT's qualifier head; a bare IDENT resolves against the ENCLOSING class; `super.f(x)` is NOT widened (java resolves over the SUPERCLASS). The guard: a root is used
    * only where its candidate set CONTAINS the member javac bound.
    */
  private def rootOf(a: Tree.Apply, callee: SymId, owner: SymId, name: String, ov: Overloads, enclosing: SymId)(using Program): SymId =
  ...
  /** JLS 15.12.2's own arity test, the only part of applicability this check performs. TYPES are deliberately not checked — that is the resolver this is not; reported as the denominator.
    */
  ...
  /** java's phase 1/2 boundary: two FIXED-arity candidates of the same length whose parameter at some position is a primitive on one side and its wrapper (or a universal slot) on the other. Vararg
    * excluded — its own span is the row above, and reporting both would double-count.
    */
  ...
  /** every call in this subtree, paired with the class it is WRITTEN IN (innermost `ClassDef`, for [[rootOf]]'s no-receiver shape) and the MEMBER it is written in (`Decision.isKeyable`, an
    * independent question). PRODUCT-REFLECTION, not `StandardTraversal`: a bottom-up derivation once misattributed a sibling class's call. TOTAL by construction; an ANONYMOUS class is not a boundary
    * (its symbol is not in the overload index).
    */
  private def callsIn(t: Any, enclosing: SymId, decl: SymId, f: (Tree.Apply, SymId, SymId) => Unit)(using p: Program): Unit = t match
  ...
  /** Over the units the run EMITS — the same ownership filter every other per-site report carries.
    ...
    */
  def check(program: Program, units: List[Tree.ClassDef], ov: Overloads, resolutions: ResolutionPlan = ResolutionPlan.empty): Report =
    given Program = program
      ...
      callsIn(
        ...
        { (a, enclosing, decl) =>
          ...
          analyse(a, ov, enclosing, decl).foreach { (_, fs) =>
            overld += 1
            out ++= fs.filterNot(f => resolutions.appliedAt(f.declaration, Name, f.issue.toString, f.origin))
          }
        }
      )
    }
    Report(out.toList.sortBy(f => (f.issue.toString, f.origin.javaPath, f.origin.line, f.member)), calls, overld)
  ...
  def ascription(a: Tree.Apply)(using p: Program): Either[Decline, TypeRepr.MethodType] =
    ...
    val shape = fun match
      case Tree.Select(_: Tree.Super, _, _, _) =>
        no(
          "super-receiver",
          "scala admits `super` only as a member selection's qualifier, so there " +
      ...
      case _                              =>
        no(
          "not-a-selection",
          "the callee is not named by a selection or an identifier, so this is " +
    ...
    shape.flatMap { _ =>
      p.symbolOf(a.method) match
        case scala.None =>
          no(
            "callee-unresolved",
            "the frontend interned no symbol for this callee, so there is no " +
        ...
        case Some(s) if s.flags.isStatic =>
          no(
            "static-callee",
            "java lets a static be called through an INSTANCE and the emitter has " +
        ...
        case Some(s) if s.fullName.startsWith("scala.<op>#") =>
          no("operator",
             "the emitter renders this call infix, and an ascription would have to " +
        ...
        case Some(_) =>
          p.definitionOf(a.method).collect { case x: Tree.DefDef => x } match
            case scala.None =>
              no(
                "callee-external",
                "this program does not DECLARE the callee, so its parameter list " +
            ...
            case Some(d) =>
              ...
              if d.tparams.nonEmpty then
                no(
                  "generic-callee",
                  "a polymorphic method value has no plain function type, so the " +
                ...
                no(
                  "vararg-callee",
                  "`JS-G37` emits the pack as `Array[T]`, so the emitted arity is " +
                ...
                no("bare-wildcard",
                   "a parameter or the result is a bare wildcard, and `(?) => R` " +
                     "names nothing"
                )
              else Right(TypeRepr.MethodType(ps.map(v => p.symbolOf(v.symbol).map(_.name).getOrElse("_") -> v.tpt.tpe), d.returnTpt.tpe))
    }

  /** WHY THE ASCRIPTION COULD NOT BE WRITTEN HERE — the guard the refusal population groups by. Produced by [[ascription]] itself and nothing else, so emission and refusal cannot drift.
    */
  ...
  final class Apply extends Phase, PolicyBound:
    ...
    /** the candidate index, built once per program — the same value the check and the emitter read, so all three answer one question about one program.
      */
    ...
    override def run(program: Program): Program =
      if plan.isEmpty then program
        ...
        try super.run(program)
        finally index = scala.None

    /** at a declaration a `resolutions` key can NAME (`Decision.isKeyable`, same as the check). An anonymous class's method is skipped HERE, covered by the enclosing member's own walk.
      */
    ...
    /** bind the declaration, then rewrite the calls in its body. `None` where there is nothing to do at all, so an untouched declaration is returned as itself rather than rebuilt.
      */
    private def rewritten(decl: SymId, body: Term)(using p: Program): Option[Term] =
      index.collect { case (prog, ov) if (prog eq p) && scope.emitsSymbol(p, decl) => ov }.map { ov =>
    ...
    private def act(a: Tree.Apply, decl: SymId, subject: String, enclosing: SymId, ov: Overloads)(using Program): Term =
      ...
      val fs = risks(a, ov, enclosing).filterNot(f => plan.all.exists(x => x.origin == f.origin && x.remedy.lane == Name))
      ...
      chosen.headOption match
        ...
        case Some((_, r)) if r.remedy.id != AscribeJavacChoice.id =>
          chosen.foreach((f, sel) => record(sel, subject, decl, f, s"risk at `${f.member}` accepted by this port"))
          a
        case Some((f0, r0)) =>
          ascription(a) match
            ...
            case Left(d) =>
              ...
              a
            case Right(mt) =>
              chosen.foreach((f, sel) => record(sel, subject, decl, f, s"call to `${f.member}` pinned to the alternative javac bound"))
              a.copy(fun = Tree.Typed(a.fun, TypeTree(mt, a.origin), mt, a.origin))

    /** one ledger row per DRAINED ROW, never one per call: a call may file two JLS 15.12.2 boundaries and one act answers both. The SITE's line rides in the text since a selection BROADCASTS and the
      * porter note sits at the DECLARATION — without the line two answered calls read as one duplicated note.
      */
