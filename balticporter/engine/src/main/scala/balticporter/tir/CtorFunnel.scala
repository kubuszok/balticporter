...
object CtorFunnel:
  ...
  /** the plan for one class: which Java constructor is Scala's primary, the super arguments to lift into the `extends` clause, and the leftover statements that become class body.
    */
  final case class Plan(
    ...
    /** Synthesised primary slots: parent constructor's formals as `(name, type)` pairs. Non-empty when no java constructor can be the primary but every root reaches the same parent constructor.
      * Admissible only for a uniform target; differing overloads refuse.
      */
    ...
    /** Full delegation argument list per root (super slots then field slots). Marker `null` appended by the emitter.
      */
    ...
    /** Parent secondary's post-delegation body, guarded by a null check on the post-body parameter. Rendered in the synthesised primary's class body (not in each secondary). Already retyped through
      * the child's `extends` clause.
      */
    primaryPostBody: List[Statement] = Nil,
    /** True when the promoted root passes its own parameters straight through to super, enabling positional delegation for all siblings.
      */
  ...
  object Plan:
    /** No primary nominated; the class keeps scala's implicit nilary primary. May still carry [[Plan.givens]] for classes whose constructors have a context clause.
      */
    val none: Plan = Plan(scala.None, Nil, Nil)

  /** One hoisted field: the primary takes its value as a parameter. `mutable` decides `val` vs `var` and is separate from slot-eligibility.
    */
  ...
  /** How one secondary's `super(args)` reaches the parent. Per-ROOT: the emitter renders it and [[OmissionCheck]] counts it through the same function.
    */
  enum SuperCall:
  ...
  final class Plans(program: Program, surfaceView: Option[Surface] = scala.None):
    ...
    /** Promote the nilary constructor when no constructor carries `super(args)`, to avoid `def this()` clashing with scala's implicit nilary primary.
      */
    private def nilaryPlan(cd: Tree.ClassDef): Option[Plan] =
      ...
      if ctors.exists(c => superArgsOf(program, c).nonEmpty) then scala.None
        ...
        for
          ...
          head <- headStmt(nil)
          (m, as) <- head match
            case Tree.Apply(Tree.Select(r, mm, _, _), aas, _, _, _) if isInitName(program, mm) && !r.isInstanceOf[Tree.Super] && aas.nonEmpty => Some((mm, aas))
            case _                                                                                                                            => scala.None
          eff <- effects(m, as, 0)
    ...
    private def reachableArgumentFree(s: SymId, p: Plan): Boolean =
      val hasNilary = classes.find(_.symbol == s).exists(cd => ctorsOf(program, cd.body).exists(valueParams(program, _).isEmpty))
      ...
      // parent was converted to a trait (structural signature: `isTrait && !isAbstract`).
        ...
        val hasTraitConvertedParent = classes.find(_.symbol == s).exists { cd =>
          parentSyms(cd).exists(ps => program.symbolOf(ps).exists(sym => sym.flags.isTrait && !sym.flags.isAbstract))
        }
    ...
    /** Reconcile non-owned classes against the base's published `primary=` row. Non-wall classes are cross-checked (disagreement is fatal); wall classes adopt the base's demotion.
      */
    private def reconciled(acc: Map[SymId, Plan]): Map[SymId, Plan] =
      ...
      val nonOwned = classes.filterNot(cd => surface.owns(cd.symbol) || program.symbolOf(cd.symbol).exists(_.flags.isEnum))
    ...
    private lazy val decided: Map[SymId, Plan] =
      ...
      plans.map { (s, p) =>
        if p.fieldSlots.isEmpty then s -> p
          ...
          s -> p.copy(
            fieldSlots = p.fieldSlots.map { fs =>
    ...
    def shape(cd: Tree.ClassDef): String =
      ...
      if p.isSynthesised then if jdkThrowableParent(program, cd) then "padded-throwable-synthesis" else "synthesised-primary"
      else
        p.primary match
    ...
    /** the constructors on whose path java did not run the promoted body — BEFORE the prefix strip below has a chance to make the duplication disappear.
      */
    private def escapingRoots(cd: Tree.ClassDef): List[Tree.DefDef] =
      val p = plans.getOrElse(cd.symbol, Plan.none)
      escapingRootsOf(program, cd, p.primary, p.primaryBody)

    /** The escaping root's body after stripping the promoted body as a prefix. `None` when not escaping or when the prefix does not match. Compared via canonical rendering.
      */
    ...
    /** Post-body statements for the synthesised primary's class body (guarded by a null check). Non-empty only when `resolvedThroughParent` produced a post-body carried through a parameter. Rendered
      * once in the class body, not per-secondary.
      */
            ...
            if parentRoots.sizeIs != 1 then scala.None
              ...
              if target == parentRootSym || args.isEmpty then scala.None
                ...
                inlineDelegation(program, target, args, parentRootSym, depth = 0) match
                  ...
                  case scala.None =>
                    Some(
                      "parent-delegation resolution refused: the parent constructor's " +
    ...
    /** Per-root delegation: synthesised = positional, promoted = type-matched. Every argument must find a home or the result is [[SuperCall.Dropped]].
      */
    def superCall(cd: Tree.ClassDef, args: List[Term]): SuperCall =
      ...
      if plan.isSynthesised then if args.sizeIs == plan.superSlots then SuperCall.Positional(args) else SuperCall.Dropped
      else
        ...
        if ps.isEmpty || plan.superArgs.size != ps.size || args.sizeIs > ps.size then positional
          ...
          if slots.exists(_.isEmpty) || used.size != args.size then positional
            ...
            if isJdkCauseCall(cd, args, ss) && simple(args.head) then SuperCall.Matched(ss.map { case _: Slot.NullAt => Slot.CauseMessage(args.head); case s => s })
    ...
    /** True when a subclass replay cannot reach this member: private, package-private, or protected accessed through a prefix (not `this`).
      */
    ...
    private def superApply(d: Tree.DefDef): Option[(SymId, List[Term])] = headStmt(d) match
      case Some(Tree.Apply(Tree.Select(_: Tree.Super, m, _, _), args, _, _, _)) if args.nonEmpty && isInitName(program, m) => Some((m, args))
      case _                                                                                                               => scala.None
      ...
      if depth > 8 then scala.None
        ...
        classOfSym(cls) match
          ...
          case Some(cd)   =>
            ...
            if p.isSynthesised then ctorsOf(program, cd.body).find(valueParams(program, _).isEmpty).flatMap(n => effects(n.symbol, Nil, 0)).map(_.stats)
    ...
    /** Over-estimate: every field any path may write. `None` if the statement is not a field write, branch, block or no-op. Used on the PROLOGUE side of [[supersedes]].
      */
    ...
    /** Ascribe a `null` argument to the formal's type (scala's `null` is `Null`, which has no members). Skipped when the formal type mentions a constructor-scoped type parameter.
      */
    private def nullAtFormal(ctor: SymId, p: Tree.ValDef, a: Term): Term = a match
      case lit @ Tree.Literal(Constant.NullC, _, o) if p.tpt.tpe != TypeRepr.NoType && !ctorScoped(ctor, p.tpt.tpe) && !statesNull(p.tpt.tpe) =>
    ...
    final private case class Effects(stats: List[Statement], deferredTo: Option[SymId])
        ...
        surface.memberShape(s) match
          ...
          case Surface.Answer.Unknown(why, module) =>
            report(
              Surface.Gap(
                who,
                why + " — the constructor replay that reaches it is refused, " +
      ...
      program.definitionOf(target).collect { case d: Tree.DefDef => d }.toList.flatMap { d =>
        d.tparams.map { tp =>
          val bounds = program
            .symbolOf(tp.symbol)
            .map(_.info)
            .collect { case TypeRepr.TypeBounds(lo, hi) =>
              TypeRepr.TypeBounds(ParentSubst.subst(lo, classSubst), ParentSubst.subst(hi, classSubst))
            }
  ...
  /** Classify which of the JDK's four throwable constructors was reached. Read off the target's formals, not the argument types. `None` if outside the four.
    */
  ...
  private def throwableCtor(program: Program, target: SymId, args: List[Term]): Option[ThrowableCtor] =
    ...
    if args.isEmpty then Some(ThrowableCtor.Nilary)
      ...
      formalsOf(program, target) match
      ...
      def pad(r: Tree.DefDef, args: List[Term], k: ThrowableCtor): Option[List[Term]] =
        ...
        k match
          ...
          case ThrowableCtor.Cause   =>
            Option.when(simple(args.head))(List(Tree.Opaque.spliced(List("java.util.Objects.toString(", ", null)"), List(args.head), formals.head, o), args.head))
  ...
  /** Constructor body as a statement list, seen through `Tree.Commented` wrappers. A comment on the body's first statement is re-attached, never discarded.
    */
  def stmtsOf(d: Tree.DefDef): List[Statement] =
    ...
    d.rhs match
      case None                    => Nil
      case Some(c: Tree.Commented) =>
        stats(Tree.uncomment(c)) match
  ...
  def enumPrimaryCtor(program: Program, cd: Tree.ClassDef): Option[Tree.DefDef] =
    ...
    if ctors.sizeIs <= 1 then ctors.headOption
      ...
      ctors.filterNot(delegatesToThis(program, _)) match
  ...
  def enumSupersededBy(program: Program, cd: Tree.ClassDef): Map[SymId, SymId] =
    ...
    if params.isEmpty then Map.empty
      ...
      cd.body.collect {
        case v: Tree.ValDef if nameOf(v.symbol).flatMap(byName.get).exists((_, t) => t == v.tpt.tpe) =>
          nameOf(v.symbol).flatMap(byName.get).get._1 -> v.symbol
      }.toMap

  /** Resolve a constant's arguments to the root constructor, following delegations. `None` when the overload is ambiguous or the delegation does more than delegate.
    */
  def enumConstantArgs(program: Program, cd: Tree.ClassDef, args: List[Term], fuel: Int = 8): Option[List[Term]] =
    val ctors = ctorsOf(program, cd.body)
    if ctors.sizeIs <= 1 then Some(args)
    else if fuel <= 0 then scala.None
    else
      ctors.filter(c => valueParams(program, c).sizeIs == args.size) match
          ...
          stmtsOf(c) match
            case (t: Term) :: Nil =>
              Tree.uncomment(t) match
                case Tree.Apply(Tree.Select(_, m, _, _), das, _, _, _) if isInitName(program, m) && !das.exists(mentions(_, ps)) =>
  ...
  /** `Some(args)` when a nilary constructor's body is only delegation(s); `None` if paramful or if the body does anything else. Nilary = value params only (ignores `using` clauses).
    */
  ...
  def givenClauses(program: Program, d: Tree.DefDef): List[List[Tree.ValDef]] =
    d.paramss.reverse.takeWhile(ps => ps.nonEmpty && ps.forall(v => program.symbolOf(v.symbol).exists(_.flags.isGiven))).reverse
  ...
  def reachesCtor(program: Program, d: Tree.DefDef, target: SymId, depth: Int = 0): Boolean =
    ...
    d.symbol == target || (depth <= 8 && (headStmt(d) match
      case Some(Tree.Apply(Tree.Select(r, m, _, _), _, _, _, _)) if !r.isInstanceOf[Tree.Super] && isInitName(program, m) =>
        program.definitionOf(m).collect { case x: Tree.DefDef => x }.exists(reachesCtor(program, _, target, depth + 1))
      case _ => false))

  /** the constructors on whose path java did not run `primary`'s body — before the prefix strip. */
  def escapingRootsOf(program: Program, cd: Tree.ClassDef, primary: Option[Tree.DefDef], primaryBody: List[Statement]): List[Tree.DefDef] =
    primary match
  ...
  def residualBodyOf(program: Program, cd: Tree.ClassDef, primary: Option[Tree.DefDef], primaryBody: List[Statement], d: Tree.DefDef): Option[List[Statement]] =
  ...
  /** prepend `ts` to whatever the first of `rest` already carries; `Nil` when nothing survives to carry it (a constructor whose ONLY statement was the lifted `super(args)`).
    */
  ...
  def plan0(program: Program, cd: Tree.ClassDef, synthesis: Boolean = true, parentPlanOf: SymId => Option[Plan] = _ => scala.None): Plan =
    ...
    if s.exists(x => x.flags.isModule || x.flags.isTrait || x.flags.isEnum) then Plan.none
      ...
      chosen match
        ...
        case other if !throwableParent && synthesis =>
          syntheticPrimary(program, cd, roots, parentPlanOf = parentPlanOf).getOrElse {
            other match
              case None    => Plan.none
  ...
  private def syntheticPrimary(program: Program, cd: Tree.ClassDef, roots: List[Tree.DefDef], throwablePad: Boolean = false, parentPlanOf: SymId => Option[Plan] = _ => scala.None): Option[Plan] =
    ...
    val slotArgs:       Option[(SymId, Map[SymId, List[Term]])] =
      Option
        .when(targets.sizeIs == 1 && arities.sizeIs == 1)(targets.head -> roots.map(r => r.symbol -> superArgsOf(program, r)).toMap)
        // resolve through parent delegation chain when roots call different parent secondaries
        .orElse(
          Option.when(targets.sizeIs > 1 && !throwablePad)(resolvedThroughParent(program, cd, roots, calls, parentPlanOf)).flatten.map { rr =>
    ...
    if roots.sizeIs < 2 || roots.exists(_.tparams.nonEmpty) then scala.None
      ...
      def ancestorOf(anc: SymId, s: SymId, fuel: Int): Boolean =
        s != SymId.None && fuel > 0 && (anc == s ||
          program.definitionOf(s).collect { case c: Tree.ClassDef => c }.exists(c => parentSyms(c).exists(ancestorOf(anc, _, fuel - 1))))
      ...
      val (pbSlots, pbPostBody) = resolvedResult match
        case Some(rr) if rr.postBodyParams.nonEmpty =>
          ...
          val needsBoolGuard = typedSlots.headOption.exists((_, t) =>
            javaDefault(program, t, cd.origin).exists {
          ...
          val paramSubst = rr.postBodyParams
            .zip(typedSlots)
            .map { case ((pv, _), (slotName, slotTpe)) =>
      ...
      val delegations = roots.map { r =>
        ...
        val pbVals    =
          if pbHasBoolGuard then
            ...
            val boolVal: Term = Tree.Literal(Constant.BoolC(contributes), pbSlots.head._2, cd.origin)
            boolVal :: rawPbVals
          else rawPbVals
        ...
        val sv = if !hasDhContrib then superValues(r.symbol)
          ...
          superValues(r.symbol).zipWithIndex.map { (ea, i) =>
            if dhSuperExprs.contains(i) then Tree.Literal(Constant.NullC, sup.lift(i).map(_._2).getOrElse(TypeRepr.NoType), cd.origin): Term
            else ea
          }
      ...
      def synthesise(mark: Option[String]): Option[Plan] =
        ...
        val superArgsTerms = sup.zipWithIndex.map { case ((n, ft), k) =>
          if dhSuperExprs.contains(k) then
            ...
            val dhSlotName = dhSlotList.headOption
              .map { (pv, _) =>
            ...
            Tree.Opaque.spliced(List(s"(if (", s" != null) ", s" else ", s")"), List(dhRef, dhExpr, supRef), ft, o)
        ...
        Some(
          Plan(
            ...
            consumed = consumedRuns,
            notSlot = refusedFields,
            postBodySlots = pbSlots,
            primaryPostBody = pbPostBody,
      ...
      if slotArity.sizeIs != 1 || !slotArity.contains(formals.size) ||
        formals.contains(TypeRepr.NoType)
      then scala.None
  ...
  private def buildRootArgs(program: Program, cd: Tree.ClassDef, roots: List[Tree.DefDef], delegations: Map[SymId, List[Term]]): Map[SymId, List[Term]] =
    ...
    def derive(c: Tree.DefDef, depth: Int): Option[List[Term]] =
      if depth > 6 then scala.None
        ...
        headStmt(c) match
          case Some(Tree.Apply(Tree.Select(r, m, _, _), as, _, _, _)) if isInitName(program, m) && !r.isInstanceOf[Tree.Super] =>
            rootPart
              .get(m)
              .orElse(
  ...
  private def collapseTo(program: Program, cd: Tree.ClassDef, candidates: List[Tree.DefDef]): Option[Plan] =
    candidates
      .sortBy(c => -valueParams(program, c).size)
      .headOption
      .map { c =>
        val (sa, rest) = split(program, c); promoted(program, c, sa, rest).copy(collapse = true)
      }
  ...
  /** Derive field slots: (slots, values per root, consumed count per root, refused with reason). Gates: one leading assignment per root, order-blind values, no body dependency on the field. `mutable`
    * is left true here; [[Plans]] decides it.
    */
  private def fieldSlotsOf(program: Program, cd: Tree.ClassDef, roots: List[Tree.DefDef]): (List[FieldSlot], Map[SymId, List[Term]], Map[SymId, Int], List[(String, String)]) =
    ...
    def leadingRun(d: Tree.DefDef): List[(SymId, Term)] =
      bodyAfterDelegation(program, d).map(st => assignment(st).filter((f, _) => byId.contains(f))).takeWhile(_.isDefined).flatten
    ...
    candidates.toList.foreach { f =>
      ...
      val fill = v.rhs.map(r => Option.when(orderBlind(program, Set.empty, r))(r)).getOrElse(javaDefault(program, v.tpt.tpe, v.origin))
    ...
    val slots = candidates.map { f =>
      val v = byId(f)
      FieldSlot(f, s"f$$${nameOf(f)}", v.tpt.tpe, mutable = true, writes = roots.count(d => runs(d.symbol).exists(_._1 == f)))
    }
  ...
  private def superTarget(program: Program, d: Tree.DefDef): SymId = headStmt(d) match
    case Some(Tree.Apply(Tree.Select(_: Tree.Super, m, _, _), args, _, _, _)) if args.nonEmpty && isInitName(program, m) => m
    case _                                                                                                               => SymId.None

  /** Result of [[resolvedThroughParent]]: the parent root, per-root effective args, per-root post-body param values, and the shared post-body params and statements.
    */
  final case class ResolvedResult(
    parentRoot: SymId,
    argsMap:    Map[SymId, List[Term]],
    /** Per-root values for the post-body params (in the order of [[postBodyParams]]). Roots that target the parent root directly have empty lists.
      */
    postBodyValues: Map[SymId, List[Term]],
    /** The parent secondary's params that the post-body uses — `(paramDef, callerArg)`. Source of the synthesised parameter names and types.
      */
    ...
    /** Per super-arg index, the effective-arg expression that references delegation-head slot Opaques (e.g., `region$$dh != null ? Array.with(region$$dh) : null`). Only positions where a
      * delegation-head slot was introduced; other positions use the normal super slot. The Opaque names must be substituted to the slot names before use.
      */
  ...
  private def resolvedThroughParent(
    ...
    val resolved = roots.zip(calls).map { case (dependentRoot, (target, superArgs)) =>
      ...
      if target == parentRootSym then Some((dependentRoot.symbol, InlineResult(superArgs, Nil, Nil)))
      else
        ...
        inlineDelegation(program, target, superArgs, parentRootSym, depth = 0).map(result => (dependentRoot.symbol, result))
    }
    if resolved.exists(_.isEmpty) then scala.None
      ...
      if effectiveArities.sizeIs != 1 then scala.None
        ...
        val pbValues       = flat.map { (sym, r) =>
          if needsBoolGuard then
            val v: Term = Tree.Literal(
              if r.postBody.nonEmpty then Constant.BoolC(true) else Constant.BoolC(false),
              TypeRepr.TypeRef(TypeRepr.NoPrefix, boolSym),
              cd.origin
            )
            sym -> List(v)
          else
            ...
            sym -> uniquePbParams.map { (pv, _) =>
              contributed.getOrElse(
                pv.symbol,
                javaDefault(program, pv.tpt.tpe, cd.origin).getOrElse(Tree.Literal(Constant.NullC, TypeRepr.NoType, cd.origin): Term)
              )
            }
        }.toMap
        ...
        Some(
          ResolvedResult(
            ...
            needsBoolGuard,
            withPb,
            delegHeadSlots = uniqueDhSlots,
            delegHeadValues = dhValues,
      ...
      pp.rootArgs.get(target).flatMap { parentDelegArgs =>
        ...
        program.definitionOf(target).collect { case d: Tree.DefDef => d }.flatMap { td =>
          ...
          if targetPs.length != superArgs.length then scala.None
            ...
            val effectiveArgs = superDelegArgs.map(a => StandardTraversal.mapStat(substPh, a).asInstanceOf[Term])
    ...
    if depth > 6 then scala.None
      ...
      program.definitionOf(currentCtor).collect { case d: Tree.DefDef => d }.flatMap { d =>
        ...
        val postBody = headStmt(d) match
          case Some(Tree.Apply(Tree.Select(r, m, _, _), _, _, _, _)) if isInitName(program, m) && !r.isInstanceOf[Tree.Super] => stms.tail
          case _                                                                                                              => Nil
        ...
        val needsSlot = ps.zip(callerArgs).filter((p, a) => !simple(a) && delegCounts.getOrElse(p.symbol, 0) > 1)
        ...
        val mixedHead = needsSlot.exists { (p, _) =>
          headArgs.exists { a =>
            val m = mentions(a); m(p.symbol) && (m - p.symbol).exists(paramSyms)
          }
        }
          ...
          head match
            case Some(Tree.Apply(Tree.Select(_, m, _, _), as, _, _, _)) if isInitName(program, m) =>
              ...
              if m == targetRoot then Some(InlineResult(substArgs, postBody, pbParams, dhSlots))
                ...
                inlineDelegation(program, m, substArgs, targetRoot, depth + 1)
                  .map(inner => InlineResult(inner.effectiveArgs, inner.postBody ++ postBody, inner.postBodyParams ++ pbParams, inner.delegHeadSlots ++ dhSlots))
            case _ => scala.None
      }
