...
object PortResources:
  ...
  /** Undeclared files under declared roots -- candidates for [[Kind.NamedUnshipped]]. Walked to ASK, never to SHIP (a scan would include upstream build files).
    */
  ...
  /** One row per shipped resource plus each residue.
    * @param named
    *   whether the emitted program holds a string literal naming this classpath path (checked in both `/path` and bare forms; scoped to this module's own units).
    */
  def findings(shipped: List[Res], candidates: List[Res], trees: List[ResourceTree], named: String => Boolean): List[CheckReport.Finding] =
