...
object PortabilityCheck extends RemedySource:
  ...
  /** THE `CheckReport` LANE THIS CHECK'S EMITTED-CODE RESIDUE IS COUNTED IN — one spelling, since a lane had THREE literals to agree by inspection: `PortRun.PortabilityEmitted`, [[AcceptJvmOnly]]'s
    * own, and `RemediationTransform.Lane`'s. Lives on the CHECK, which mints the rows; `PortRun` and `RemediationTransform` both read it.
    */
  val EmittedLane: String = "portability(emitted)"

  /** THE ONE REMEDY THIS CHECK CAN CARRY OUT — see [[AcceptJvmOnly]]. Declared here since this lane's producer IS a check, not a phase.
    */
  def remedies: List[Remedy] = List(AcceptJvmOnly)

  /** WHAT IS NOT ON THIS MENU: drop/inline/redirect are on the PHASE's menu (`RemediationTransform`), which changes the tree before this check runs; suppressing the row is REFUSED (narrow
    * `PortManifest.targets` instead); a per-API acceptance keyed on the FQN is REFUSED (`verdictOverrides` already does this, read by `rulesFor`). [[Remedy.AnyKind]] rather than `alsoKinds`: this
    * lane's kind column is the offending API's FQN, an open set.
    */
  ...
  val AcceptJvmOnly: Remedy = Remedy(
    ...
    subject = Remedy.Subject.OwnedMember,
  ...
  /** @param api
    *   a prefix (`java.nio.file.`) or an exact `owner#member` @param why one sentence
    * @param exactMember
    *   which of the two `api` is @param on THE PLATFORMS THIS RULE IS A REFUSAL FOR (unconditional would tell a JVM+Native port to remove APIs that work on both) @param at the catalog row holding the
    *   availability FACT, `None` only for a non-JDK dependency.
    */
  ...
  object Rule:
    /** both non-JVM backends — the answer for a rule whose two platforms agree, which is most of them, and the value this list carried implicitly when it had no per-platform half.
      */
  ...
  /** APIs a non-JVM backend cannot provide as they stand, each with the backends it is about.
    *
    * Read [[rulesFor]] rather than this list: a port's rule set is this filtered by its declared targets, and the SIZE of that filtered set is what a run states.
    */
  val all: List[Rule] = List(
    Rule("java.lang.reflect.", "runtime reflection does not exist on Scala.js / Native", at = p(24)),
    ...
    Rule(
      "java.net.IDN",
      "the class exists on both backends and is unusable on either without " +
    ...
    Rule(
      ...
      "verified absent from the Native javalib, which has " +
    ...
    Rule(
      ...
      "the same split as SocketChannel: Native's " +
    ...
    Rule(
      "java.nio.channels.",
      "the java.nio.channels package does not exist in the Scala.js " +
    ...
    Rule(
      "java.nio.file.",
      "the java.nio.file filesystem API does not exist in the Scala.js " +
    ...
    Rule(
      ...
      "Scala.js is single-threaded and ships no ExecutorService " +
    ...
    Rule(
      "java.lang.Thread",
      "Scala.js's Thread is a private-constructor singleton — getId is " +
    ...
    Rule(
      ...
      "a browser cannot spawn a process; Scala Native has " +
    ...
    Rule(
      "org.hamcrest.",
      "Hamcrest is JVM-only, and arrives TRANSITIVELY with junit; MUnit has no " +
        "matcher algebra, so each `assertThat(x, is(y))` has to become the assertion it means"
    ),
    Rule(
      "java.lang.System#getProperty",
      "Scala.js answers getProperty from a LINK-TIME-configured " +
    ...
    Rule(
      ...
      "Scala.js returns an empty map from getenv() and null from " +
    ...
    Rule(
      "java.util.zip.",
      "no zlib in a browser without a JS dependency; Scala Native implements " +
    ...
    Rule(
      ...
      "JS's WeakMap requires object keys and cannot enumerate, so no " +
    ...
    Rule(
      "java.util.ServiceLoader",
      "Scala Native has it FOR REAL, resolved at LINK time — but its " +
    ...
    Rule(
      "java.text.Collator",
      "locale-aware collation has no cross-platform Scala answer today; " +
        "both backends leave you lexicographic-by-codepoint ordering and nothing else",
      at = l(69)
    ),
    Rule(
      "java.text.BreakIterator",
      "no implementation in any surveyed source tree — the same " +
        "answer as Collator, for the same reason",
      at = l(70)
    ),
    Rule(
      "java.util.Calendar",
      "no cross-platform implementation exists; java.time is the " +
        "replacement and it is a DEPENDENCY rather than a rewrite of this class",
      at = l(72)
    ),
    Rule(
      "java.util.GregorianCalendar",
      "the same answer as its superclass — no cross-platform " +
        "implementation exists",
      at = l(72)
    ),
    Rule(
      "java.util.TimeZone",
      "absent from both core javalibs AND from the locales artifact, whose " +
    ...
    Rule(
      "java.time.ZoneId#of",
      "zone lookup is BY STRING, so the whole IANA database ships unless " +
    ...
    Rule(
      "java.time.",
      "Scala.js's working java.time classes live only in an unpublished " +
    ...
    Rule(
      "java.util.Locale",
      "the same unpublished-stub story on Scala.js (its default is ROOT) and " +
    ...
    Rule(
      ...
      "an ext-dummies placeholder with almost no surface on Scala.js " +
        "and absent on Native; the artifact covers the family, setRoundingMode included",
      at = l(66)
    ),
    Rule("java.text.NumberFormat", "the same artifact as DecimalFormat, which it is the supertype of", at = l(66)),
    Rule("java.text.DecimalFormatSymbols", "the same artifact as DecimalFormat", at = l(66)),
    Rule("java.text.SimpleDateFormat", "an ext-dummies placeholder on Scala.js and absent on Native", at = l(67)),
    ...
    Rule(
      "java.util.Currency",
      "supplied by the locales artifact's java/util tree, which holds this " +
    ...
    Rule(
      ...
      "absent from BOTH core javalibs — the exception types " +
    ...
    Rule(
      ...
      "absent from Scala.js's core javalib. UNSTATED for Native — " +
    ...
    Rule(
      ...
      "the same artifact as WeakReference, which it is useless " +
        "without",
      on = Rule.JsOnly,
      at = p(31)
    ),
    Rule(
      ...
      "expected to be the same opt-in artifact as WeakReference. " +
  ...
  /** the rules whose answer is an ARTIFACT rather than a removal — feeds `DependencyCheck`, never `portability(*)`, since a `Verdict.Depend` finding is unanswerable as an unportability (the reader
    * would be told to remove a call one `libraryDependencies` line makes correct).
    */
  ...
  /** Does this rule still ask a question of any declared target, AFTER the port's own overrides? Without an override this is exactly [[Rule.asks]]. WITH one, a `verdictOverrides` entry saying `Keep`
    * is the port accepting the JDK type on that backend, so the rule leaves BOTH lanes. A rule with no cited row always asks — nothing to override.
    */
  ...
  /** Is every platform this rule still asks about answered by an ARTIFACT? ALL rather than ANY: `DependencyCoverageSpec` asserts no rule in the list is MIXED. A rule with no cited row is never a
    * dependency. Read THROUGH the overrides, so a port shipping its own shim changes the verdict here rather than in a second, potentially disagreeing filter.
    */
  ...
  /** Every violation the PROGRAM references. Recorded as `portability(all)`, separately from `inEmittedCode` below — the two answer different questions (referenced vs. SHIPPED).
    */
  ...
  /** The RULE FILTER, over [[ExternalUsage.all]] (every referenced symbol's `owner#name` and usages), which answers more questions than [[all]] asks. Order-preserving on purpose: `ExternalUsage.all`
    * iterates in the same order this loop did, so `portability(all)`'s baseline stays byte-identical rather than merely equal in count.
    */
  ...
  /** the top-level type a definition belongs to — used to attribute a violation to the unit that will (or will not) be emitted.
    */
  ...
  /** Violations occurring in code that is actually EMITTED — a violation inside a substituted (dropped) type would overstate the problem. @param isExcluded a type this run does NOT ship — either
    * DROPPED (`Substitutions.dropTypes`) or merely RESOLVED against (`resolutionRoots`, a foreign unit). The second was once missing and the misattribution was total (Ashley: 67 sites, none its own).
    */
  ...
  /** INJECTED replacements are copied verbatim, so the symbol table cannot see them; scan their text for the same rules instead. Coarse, but closes the hole: a hand-written shim quietly reintroducing
    * the API the substitution was meant to remove.
    */
  def inInjectedSource(fileName: String, source: String, rules: List[Rule] = all): List[InjectedViolation] =
    ...
    val code = source.linesIterator.map(_.trim).filterNot(l => l.startsWith("//") || l.startsWith("*") || l.startsWith("/*")).toList
    rules.flatMap { r =>
  ...
  /** One injected file's use of an API its substitution was meant to remove. The COUNT is carried in the `line` column and left out of the finding id, so a second use is not a new finding.
    */
  ...
  def summary(violations: List[Violation], fixes: List[Remediator.Suggestion] = Nil): String =
    if violations.isEmpty then "  none"
      ...
      val head = violations.groupBy(_.api).toList.sortBy(-_._2.size).map((api, vs) => s"  $api: ${vs.size} site(s) — ${vs.head.why}").mkString("\n")
