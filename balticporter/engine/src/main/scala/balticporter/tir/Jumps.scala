...
/** Java's `break`/`continue` binding rules as predicates over a subtree.
  *
  * Shared by `TirEmitter` (boundary placement) and `BreakCatchCheck` (crossing detection). Uses product reflection to walk children (stops at re-binding constructs, so `StandardTraversal` cannot
  * serve here).
  */
object Jumps:

  /** True if this subtree contains an unlabelled `break` belonging to THIS construct. Stops at nested loops/switches (they re-bind unlabelled `break`).
    */
  def breaksOut(t: Any): Boolean = t match
    case Tree.Break(scala.None, _, _)                                                    => true
    case _: Tree.While | _: Tree.DoWhile | _: Tree.Match | _: Tree.For | _: Tree.ForEach => false // binds to the inner one
    case xs: Iterable[?] => xs.exists(breaksOut)
    case Some(x) => breaksOut(x)
    case p: Product => p.productIterator.exists(breaksOut)
    case _ => false

  /** True if this subtree contains an unlabelled `continue` for THIS loop. Does NOT stop at `match` (java's `continue` inside a switch continues the enclosing loop).
    */
  ...
  /** True if this subtree contains a non-tail `yield` for THIS switch expression (JLS 14.21). Stops at nested switch EXPRESSIONS only (not statements); also stops at lambdas, defs and class bodies.
    */
  ...
  /** The caught types a `scala.util.boundary.Break` is an instance of. Matched by name (these are JDK types no phase retypes or renames).
    */
