...
import balticporter.catalog.{ Attaches, CatalogLog, DiffId, Difference, Differences, Status }

/** Five coverage lanes over a [[CatalogLog]]: `consulted`, `unreached` (mechanised rows only), `unmechanised` (registry-derived), `undischarged` (one finding per row), and `uncited`
...
object CatalogCheck:
  ...
  def consulted(log: CatalogLog): List[CheckReport.Finding] =
    Differences.all.filter(d => log.consulted(d.id) > 0 || log.declarations(d.id) > 0).map { d =>
      val (path, line) = at(d.id, log)
      val where        = Differences
        .leaves(d.attaches)
        .map {
  ...
  def unreached(log: CatalogLog): List[CheckReport.Finding] =
    Differences.mechanised
      ...
      .map(d => CheckReport.Finding(Unreached, kindOf(d), d.id.toString, "-", 0, s"never reached by this port — ${d.evidence}: ${d.title}"))
  ...
  /** Rows with no Scala-side normative citation. Registry-derived, same on every port. Never a failing assertion — a spec failing here incentivises inventing citations.
    */
  def uncited: List[CheckReport.Finding] =
    Differences.all.filter(_.scala.startsWith("UNCITED")).map { d =>
      CheckReport.Finding(Uncited, kindOf(d), d.id.toString, "-", 0, s"${d.scala} (JLS side: ${d.jls}): ${d.title}")
    }
  ...
  def undischarged(log: CatalogLog): List[CheckReport.Finding] =
    log.undischarged.map { h =>
      ...
      CheckReport.Finding(
        ...
        known,
        h.id.toString,
        CheckReport.relativise(h.at.javaPath),
        h.at.line,
