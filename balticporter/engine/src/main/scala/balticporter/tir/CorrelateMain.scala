...
/** Correlation step as a request — runnable in-process ([[PortRun.correlate]]) or from CLI. All paths are resolved against [[DebugFlags.root]] before use; a missing declared input is fatal.
  */
object CorrelateRun:

  /** @param srcmaps
    *   `scope -> srcmap.tsv`; scope is `main` or `test`.
    * @param out
    *   output directory for errors.tsv / tests.tsv / correlate.txt.
    * @param baseline
    *   baseline directory to diff against.
    * @param markers
    *   `unit<TAB>member` approximate regions (Stage 2).
    */
  final case class Request(
    ...
    def absolute: Request =
      ...
      copy(
        srcmaps = srcmaps.map((s, p) => s -> abs(p)),
        scalac = scalac.map(abs),
        tests = tests.map(abs),
        out = abs(out),
        baseline = baseline.map(abs),
        markers = markers.map(abs)
      )
  ...
  def run(req0: Request): Result =
    ...
    val markerSet = req.markers
      .filter(Files.isRegularFile(_))
      .map { p =>
        Files.readAllLines(p).toArray(Array.empty[String]).toList.filterNot(l => l.startsWith("#") || l.isBlank).map(_.trim).toSet
      }
      .getOrElse(Set.empty)
    ...
    val dropped = (req.srcmaps.map(_._2.getParent) ++ portDirs.map(_.resolve("run-latest")) :+ outDir).distinct.flatMap(d => Correlate.parseDropped(d.resolve("dropped-types.tsv"))).toSet
    ...
    if baseMembers.nonEmpty then
      ...
      Files.writeString(outDir.resolve("members-changed.tsv"), ("#unit\tmember" :: changed.toList.sorted).mkString("", "\n", "\n"))
...
object CorrelateMain:
  ...
  def main(args: Array[String]): Unit =
    ...
    val result =
      try
        CorrelateRun.run(
          CorrelateRun.Request(srcmaps = srcmaps, scalac = scalac, tests = tests, out = out.getOrElse(usage()), baseline = baseline, markers = markers)
        )
      catch
