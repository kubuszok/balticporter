...
object JdkSurfaceCheck extends RemedySource:
  ...
  enum Disposition:
    ...
    /** a refusal the tables now contradict. `toCollection` was refused in a comment for a release after the `into` arm started handling it.
      */
  ...
  /** A deliberate non-translation, with its reason and its citation. Check data, not a decision: `decisions.tsv` records what changed an emitted DECLARATION, and a kept JDK call changes no
    * declaration — this is a fact about the SURFACE. `cite` is required because this table's value is entirely in being followed: a `why` with no pointer is the doc comment one step removed.
    */
  ...
  /** one sentence, three owners — java re-declares `spliterator()` down its own hierarchy and a refusal keyed on `owner#name` has to be spelled at each, so the REASON is stated once.
    */
  ...
  final case class Mapping(
    ...
    /** the owners the STATIC table names — derived, so `Collections#rotate` is seen as a hole in a family the phase does rewrite rather than as an untouched utility class.
      */
  ...
  /** THE EMITTED CALL IS RIGHT AS IT STANDS — the port read this JDK member and states so. An `Unhandled` row claims the phase has no entry, NOT that the emission is broken (coverage may be by
    * COINCIDENCE) — a reading only the port can settle. Keyed at the EXTERNAL CALLEE (`Remedy.Subject.ExternalMember`). NOT emission-affecting. `kept-iterable`/`stale-refusal` are deliberately NOT on
    * this menu (would silence a real defect or the guard's own signal).
    */
  val AcceptJdkMember: Remedy = Remedy(
    ...
    id = "accept-jdk-member",
    lane = Name,
    kind = Disposition.Unhandled("").label,
  ...
  /** Classify every external JDK member `rows` holds, plus K9's ForEach demand over `units`.
    ...
    * @param m
    *   what the retyping phase did, or [[noMapping]]
    */
  ...
  /** every classified row, findings and non-findings alike — what the SUMMARY counts. Only `isFinding` rows reach `findings.tsv`.
    */
      ...
      case scala.None =>
        m.types.get(owner) match
          case Some((target, kind)) =>
            ...
            else if m.handles(owner, r.name, kind) then if m.ran then Disposition.Mapped(target) else Disposition.Mappable(target)
          ...
          case scala.None =>
            ...
            if m.staticOwners.contains(owner) then
              if m.statics.contains(key) then if m.ran then Disposition.Mapped(owner) else Disposition.Mappable(owner)
