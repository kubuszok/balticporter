...
object Correlate:
  ...
  /** Parse a dotty/scala-cli compile log. Both the coded and bare header forms are recognised — counting only coded errors silently undercounts.
    */
      ...
      lines(i) match
        case ErrHeader(code, kind, path, ln, col) =>
          ...
          out += ScalacError(
            ...
            Option(ln).flatMap(_.toIntOption).getOrElse(0),
            Option(col).flatMap(_.toIntOption).getOrElse(0),
            msg
          )
        case _ => ()
      i += 1
  ...
  /** the same marker with NO duration — how MUnit prints an IGNORED test. Without this the line is skipped, the test vanishes from `tests.tsv`, and the diff reports it as "did not run" — loud, but
    * wrong about why. An ignored test is a DECISION; a missing one is a defect.
    */
  ...
  /** `X` failed, `s` never ran, anything else (`i`/`I`) was ignored on purpose. `skipped` is kept apart from `ignored`: an ignored test is a DECISION, a skipped one is PREVENTION.
    */
  private def marked(suite: String, mark: String, full: String, detail: String): Outcome =
    ...
    val status = mark match
      ...
      case "s" => "skipped"
      case _   => "ignored"
    Outcome(if suite.nonEmpty then suite else full.stripSuffix("." + name), name, status, detail.trim, Nil)

  /** Parse an MUnit console log into per-test outcomes. Passes carry no frames; a failure carries every stack frame printed under it, in order, which is what the anchor is chosen from.
    */
      ...
      l match
        ...
        case FrameLine(qual, file, ln) =>
          pending.foreach { o =>
            ...
            val f   = Frame(
              if cut > 0 then qual.substring(0, cut) else qual,
  ...
  /** @param derived
    *   true when computed from `Substitutions.dropTypes` rather than the hand-written escape hatch — a declared entry is a CLAIM, a derived one a consequence.
    * @param frame
    *   the PORTED CLASS this failure is expected AT (normally main-frame). See [[anchorHolds]].
    */
  final case class Expected(suite: String, test: String, reason: String, derived: Boolean = false, frame: Option[String] = scala.None):
    def matches(o: Outcome): Boolean = o.suite == suite && (test == "*" || test == o.name)
    def source:              String  = if derived then "derived" else "declared"

    /** DOES THE REASON STILL HOLD? — the half `(suite, test)` cannot answer. A declared entry is keyed on the test's NAME, so a NEW failure with a different cause in the same test is absorbed
      * silently. A row may carry the ANCHOR the correlator already computes; where it stops matching, the failure counts UNEXPECTED. OPTIONAL for compatibility with rows predating the column.
      */
        ...
        if l.startsWith("#") || l.isBlank then scala.None
          ...
          l.split('\t').toList match
            case s :: t :: rest =>
              ...
              Some(
                Expected(s.trim, t.trim, prose.mkString(" ").trim, frame = tagged.headOption.map(_.drop(FrameTag.length).trim).filter(_.nonEmpty))
              )
            case _ => scala.None
      }
  ...
  /** One `Substitutions.dropTypes` entry, IN BOTH NAMESPACES: a port's policy is written UPSTREAM and its package rename runs LAST, so recording only one namespace left this rule dead on every
    ...
    * @param emitted
    *   the FQN the port emits it under — what a stack frame and a compiler report.
    */
  ...
  /** The FQNs a port declares in `Substitutions.dropTypes`, as `PortRun` wrote them: `upstream` TAB `emitted`. A single-column line means the two namespaces coincide.
    */
  ...
  /** `.` separates packages and the top-level type, `$` precedes a nested type or a companion, `#` a member — the three separators `Symbol.fullName` and a JVM class name are cut at.
    */
  ...
  /** Is this failure explained BY CONSTRUCTION — does its stack reach a type the port deliberately does not have? The whole stack is consulted (a drop shows up below the anchor's member), and the
    * FIRST dropped type encountered is reported. Matched against RAW frames, never source-mapped ones: a dropped type is the one type the port does NOT emit, so it has no source-map entry — its
    * replacement is INJECTED Scala the emitter never saw.
    */
  def derivedExpectation(t: Outcome, dropped: Set[Dropped]): Option[Expected] =
    if dropped.isEmpty || t.status != "fail" then scala.None
    else
      t.frames.iterator.flatMap(f => dropped.find(d => covers(d.emitted, f.cls))).nextOption().map { d =>
        Expected(
          t.suite,
          t.name,
          s"Substitutions.dropTypes ${d.render} — the port deliberately does not translate this " +
            "type, and this failure's stack reaches it",
          derived = true
        )
      }
  ...
  enum Lane:
    ...
    /** in a file the source map does not cover: injected Scala, a runtime shim, a dependency. Not an engine gap and not a marked region — reported separately rather than silently counted as either.
      */
    case Unmapped

    /** the engine DECLARED this one: a `scala.compiletime.error` the emitter wrote itself under `PortRun(preview = true)`. Its own lane, classified BEFORE the source-map lookup, since it is the
      * opposite of a finding — counted with engine gaps it would drown them.
      */
    case Declared
  ...
  /** `markers` is the set of `unit\tmember` keys the engine marked approximate. Empty input is the Stage-1 state and must stay a legal, meaningful input — an engine that needed markers to report
    * anything would be useless on the first day of a new library.
    */
  ...
  final case class LocatedTest(
    ...
    /** a DECLARED entry matching this test by name whose ANCHOR no longer does. Reported apart, NOT put in [[expected]] — the failure counts unexpected. Always empty for a derived expectation, which
      * has no claim to go stale.
      */
    staleExpectation: Option[Expected] = scala.None,
    /** how many members of the ANCHORED UNIT changed since the baseline — the unit count catches a class-initialiser cycle triggered by a sibling member the per-member flag misses.
      */
    unitChanged: Int = 0
  ):
    def tsv: String =
      ...
      s"${outcome.suite}\t${outcome.name}\t${outcome.status}\t$anchor\t${e.map(_.unit).getOrElse("")}\t" +
        ...
        s"${expected.map(x => s"expected#${x.source}").orElse(staleExpectation.map(_ => "unexpected#stale-declaration")).getOrElse("unexpected")}\t${digestChanged}\t$unitChanged\t${outcome.detail}"
  ...
  /** Anchor a failing test. Frames are walked TOP-DOWN, so the deepest ported frame wins. A `main`-scope frame is preferred over `test`-scope even when deeper — a stack reaching the library at all
    * makes the library the subject.
    */
  def locateTests(
    outs:           List[Outcome],
    idx:            SrcMap.Index,
    ...
    outs.map { o =>
      ...
      val (anchor, entry) =
        if o.status != "fail" then ("none", scala.None)
          ...
          main
            .map(x => ("main-frame", Some(x._2)))
            .orElse(ported.headOption.map(x => ("test-frame", Some(x._2))))
            .orElse(
              AssertAt.findFirstMatchIn(o.detail).flatMap(m => idx.resolveFile(m.group(1), m.group(2).toIntOption.getOrElse(0)).map(x => ("assert-site", Some(x))))
            )
  ...
  final case class TestDiff(
    ...
    /** a DECLARED expectation whose ANCHOR no longer holds. Reported apart from [[expectedButPassing]] since the next action differs (delete the row vs. read the new failure). Not its own gate: the
      * failure is already in [[newlyFailing]]/[[stillFailing]].
      */
    staleExpectations: List[LocatedTest] = Nil
  ):
    /** the gate: an UNEXPECTED newly-failing test, or a test that stopped RUNNING — a SKIP (still counted, no assertion produced) or a DISAPPEARANCE (a conversion regression that stops emitting a
      * suite removes its tests from both sides, reporting success on a smaller suite). A deliberate deletion is ACKNOWLEDGED by re-accepting the baseline.
      */
  ...
  private def laneNote(l: Lane): String = l match
    ...
    case Lane.Declared  =>
      "DECLARED by the port under `preview = true`: the engine had no faithful " +
  ...
  def renderTests(all: List[LocatedTest], d: TestDiff, limit: Int = 25): String =
    ...
    sb.append(
      s"tests: ${all.size}  passing=$pass  failing=$fail  " +
    ...
    if !d.hasBaseline then sb.append("NO TEST BASELINE — these are the current results, nothing to compare against.\n")
    def block(title: String, xs: List[LocatedTest], note: String = ""): Unit =
      if xs.nonEmpty then
        ...
        xs.take(limit).foreach { t =>
          ...
          t.portedFrames.take(6).foreach((f, e) => sb.append(s"          · ${e.scope}  ${e.member}  [${e.javaAt}]  (${f.file}:${f.line})\n"))
        }
