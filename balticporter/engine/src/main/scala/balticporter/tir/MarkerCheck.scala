...
/** Enforces marker conservation: an `Open` marker may be DISCHARGED by a phase, never erased. Compares minted markers (before pipeline) against surviving ones (after), keyed on
  * [[Tree.Unportable.markerKey]]. A marker whose owning declaration was removed is legitimate; one whose declaration survives but whose marker is gone is an engine defect.
  */
object MarkerCheck:
  ...
  /** Every marker in `units`, sited on the innermost declaration that holds it. Uses a claimed-identity set keyed on `markerKey` (not node identity, since traversal rebuilds nodes). Cross-class dedup
    * only; duplicates within one member are preserved.
    */
  def inventory(program: Program, units: List[Tree.ClassDef]): List[Sited] =
    ...
    units.flatMap { cd =>
      ...
      def inMember(owner: SymId, ts: List[Tree]): List[Sited] =
        ts.flatMap {
          case t: Term =>
            StandardTraversal.scanTerm(t, List.empty[Sited]) {
  ...
  /** Compare markers minted in `before` against survivors in `after`, scoped to `units` (ownership filter). */
  def check(before: Program, after: Program, units: List[Tree.ClassDef]): List[Finding] =
    ...
    val survivorIds = units.flatMap(u => StandardTraversal.allClassDefs(u)(using after)).flatMap(ownMembers).map(_._1).toSet

    val erased = minted
      .filterNot(s => survivingKeys(s.marker.markerKey))
  ...
  /** Sentinel symbols: unresolvable type variables (`?T`) and variable references (`?var$name`) interned by the frontend. Counted on the `markers` lane but NOT gated (unlike open markers). Matched by
    * exact equality against the mint site's construction, not by `?` prefix.
    */
  def sentinels(program: Program, units: List[Tree.ClassDef]): List[Finding] =
    ...
    program.symbols.all.toList.filter(sym => isTypeVarSentinel(sym) || isVarSentinel(sym)).sortBy(_.fullName).flatMap { sym =>
      ...
      here.headOption.map { u =>
        Finding(
          "sentinel",
          program.symbolOf(u.enclosing).map(_.fullName).getOrElse("?"),
  ...
  /** The open markers alone -- the emission gate's input. */
