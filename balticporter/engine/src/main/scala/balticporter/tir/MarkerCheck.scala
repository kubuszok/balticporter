...
/** Enforces marker conservation: an `Open` marker may be DISCHARGED by a phase, never erased. Compares minted markers (before pipeline) against surviving ones (after), keyed on
  * [[Tree.Unportable.markerKey]]. A marker whose owning declaration was removed is legitimate; one whose declaration survives but whose marker is gone is an engine defect. // DESIGN.md §6.2, §6.5
  */
object MarkerCheck:
  ...
  /** Sentinel symbols: unresolvable type variables (`?T`) and variable references (`?var$name`) interned by the frontend. Counted on the `markers` lane but NOT gated (unlike open markers). Matched by
    * exact equality against the mint site's construction, not by `?` prefix. // ENGINE-LIMITS M6
    */
