...
object NoteCoverageCheck:
  ...
  /** @param decisions
    *   the run's complete log, after the ownership filter
    * @param printed
    *   what the emitter recorded printing, one entry per note
    * @param emitted
    *   every symbol this run wrote a declaration for
    * @param texts
    *   `(unit fqn, emitted text)` for every file the run wrote from the TIR
    */
  def check(
    ...
    val fromText = texts.flatMap { (unit, text) =>
      ...
      val unbacked = found.map(_.kind).distinct.filterNot(_.exists(kinds)).map { k =>
        Finding(
          Issue.Unbacked,
          k,
          k.map(PorterNote.slug).getOrElse(found.find(_.kind.isEmpty).map(_.slug).getOrElse("?")),
          unit,
    ...
    (missing ++ fromText).sortBy(f => (f.issue.toString, f.kind.map(_.toString).getOrElse(""), f.subject, f.where))
