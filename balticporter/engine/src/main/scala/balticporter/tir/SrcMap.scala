...
object SrcMap:
  ...
  /** The port's Java source root, derived by stripping the unit's package path from `javaPath`. `None` when the origin does not end in the package path (renamed package, synthetic origin).
    */
  ...
  /** Relativise a Java path against a root derived from the port ([[sourceRootOf]]), falling back to [[CheckReport.relativise]] and then to the path as recorded.
    */
  ...
  /** One emitter's source map — a value one emitter owns, written by `PortRun`. `missed` entries are holes where a rendered member could not be located in the finished unit.
    */
  ...
  def write(out: Path, rec: Recording): Unit =
    ...
    Files.writeString(
      out.resolve("members.tsv"),
      (MembersHeader :: all.sortBy(e => (e.unit, e.member, e.kind)).map(_.memberTsv)).mkString("", "\n", "\n")
    )
    println(
      s"[balticporter] srcmap: ${all.size} members over ${all.map(_.unit).distinct.size} units -> $out" +
  ...
  final class Index(val entries: List[Entry]):
    ...
    def unitEntry(unit: String): Option[Entry] =
      byUnit.get(unit).flatMap(_.find(e => e.member == unit && e.kind == "class")).orElse(byUnit.get(unit).flatMap(_.minByOption(_.start)))
