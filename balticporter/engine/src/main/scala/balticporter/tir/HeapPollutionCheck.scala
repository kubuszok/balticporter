...
/** Counts every declaration whose vararg parameter has a non-reifiable component type (java's heap pollution, catalog `JS-G41`). Reports `Acknowledged` (java had `@SafeVarargs`) and `Unacknowledged`
  * (javac warned, author did not annotate). Nothing to translate -- the port reproduces java's unsoundness exactly; this makes it a number. // JLS 9.6.4.7, JLS 4.7, JLS 4.12.2
  */
object HeapPollutionCheck extends RemedySource:
  ...
  val Acknowledge: Remedy = Remedy(
    id = "acknowledge",
    lane = Name,
    kind = Issue.Unacknowledged.toString,
  ...
  /** Not on the menu: emitting a scala-side marker (inert), proving the body safe (an analysis, not a port choice), or narrowing the signature (different API).
    */
  def remedies: List[Remedy] = List(Acknowledge)

  /** @param param
    *   the vararg parameter's name
    * @param component
    *   the emitted COMPONENT type — the thing that is not reifiable
    */
  ...
  def uncheckedVararg(d: Tree.DefDef)(using p: Program): Option[Finding] =
    ...
    d.paramss.flatten.collectFirst {
      case v if p.symbolOf(v.symbol).exists(_.flags.isVararg) && !reifiable(componentOf(v.tpt.tpe)) =>
        Finding(
          if ack then Issue.Acknowledged else Issue.Unacknowledged,
          owner,
          p.symbolOf(v.symbol).map(_.name).getOrElse("?"),
          render(componentOf(v.tpt.tpe)),
          d.origin
        )
    }
  ...
  def check(program: Program, units: List[Tree.ClassDef], resolutions: ResolutionPlan = ResolutionPlan.empty): List[Finding] =
    ...
    val scan      = new Phase:
      ...
      override def transformDefDef(d: Tree.DefDef)(using Program): Tree.DefDef =
        uncheckedVararg(d).filterNot(f => resolutions.appliedAt(d.symbol, Name, f.issue.toString, f.origin)).foreach(out += _)
        d
