...
object ServiceProviders:
  ...
  /** Plan every declared descriptor. Pure — no filesystem writes. Caller must verify source existence.
    * @param sources
    *   upstream descriptor files.
    * @param emittedName
    *   upstream→emitted FQN translation (covers all rename rules).
    */
  def plan(sources: List[Path], emittedName: String => String): List[Descriptor] =
    sources.map { src =>
      ...
      Descriptor(
        ...
        lines = stripBom(Files.readAllLines(src, java.nio.charset.StandardCharsets.UTF_8).toArray(Array.empty[String]).toList).map { raw =>
    ...
    ds.flatMap { d =>
      ...
      val perLine = d.providers.zipWithIndex.flatMap { case (l, i) =>
        ...
        if isDropped(u) then
          ...
          row(Kind.Shipped, u, i + 1, s"provider shipped as `$e` in `${d.target}`") ::
