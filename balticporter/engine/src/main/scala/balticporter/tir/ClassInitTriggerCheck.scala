...
object ClassInitTriggerCheck:
  ...
  /** @param owner
    *   the type carrying, or inheriting, the class initialiser
    * @param declarer
    *   where the class initialiser is declared — the same as `owner` for [[Issue.Unforced]] and an ancestor for [[Issue.SubclassInitUnforced]]
    */
  ...
  /** @param forced
    *   emitter-attached (type, trigger) pairs
    * @param shapeOf
    *   emitter-decided form and companion presence for an emitted FQN
    */
  def check(program: Program, units: List[Tree.ClassDef], forced: Set[(SymId, String)], shapeOf: String => Option[Surface.TypeShape]): List[Finding] =
    ...
    val unforced        = clinitBearers.toList.collect {
      case (s, cd) if mine(s) && !forced(s -> Instantiation) && !formOf(fqn(s)).exists(notInstantiable) =>
        ...
        Finding(
          if cyclic.isDefined then Issue.ReentrantRefused else Issue.Unforced,
          fqn(s),
          cyclic.map(fqn).getOrElse(fqn(s)),
          formOf(fqn(s)).getOrElse("?"),
          cd.origin
        )
    }
    ...
    val subclass = all.collect {
      case cd if shapeOf(fqn(cd.symbol)).exists(_.companion) && !forced(cd.symbol -> SubclassInit) =>
        nearest(List(cd.symbol), Set(cd.symbol)).map(a => Finding(Issue.SubclassInitUnforced, fqn(cd.symbol), fqn(a), formOf(fqn(cd.symbol)).getOrElse("?"), cd.origin))
    }.flatten
  ...
  /** Parent edges JLS 12.4.1 item 7 traverses: superclasses plus superinterfaces with default methods. Interfaces without defaults are excluded (java does not initialise them).
    */
