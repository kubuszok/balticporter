...
/** Cross-unit constructor knowledge for the no-arg-primary + effect-replay funnel. Tracks super-chain resolution, field widening, and replayability guards.
  */
...
final class CtorRegistry(units: List[BUnit]):
  ...
  def resolveFqcn(pkg: String, name: String): Option[String] =
    ...
    if byFqcn.contains(plain) then Some(plain)
      ...
      byFqcn.keysIterator.filter(k => k.startsWith(prefix) && k.endsWith("$" + name)).toList match
        case k :: Nil => Some(k)
        case _        => None

  /** Field names up the superclass chain (within closure); promoted ctor params sharing these must be renamed to avoid shadowing.
    */
  ...
  /** True when a supertype declares a nilary method `name`; the accessor cannot be dropped because a `val` cannot satisfy a `def m(): T`.
    */
  ...
  /** Redundant accessors: field `f` + nilary method `f()` whose body is `return f`. The method is dropped; calls rewrite to field reads. Keys: (fqcn, name). Not collapsed when the method overrides an
    * inherited nilary or is `@Override`.
    */
  ...
  /** Same-name field/method clashes where the method is `@Deprecated` and not a pure accessor: keep the field, drop the deprecated method.
    */
  ...
  /** Private non-final parent fields assigned by subclass super-chain ctor effects; widened to `protected var` so the effect replay can reach them.
    */
      ...
      byFqcn.get(fqcn).foreach { case (pd, pi) =>
        pi.resolve(args.length, target).foreach { c =>
          if fqcn != rootFqcn then
            c.body.foreach { st =>
              st.k match
                case BStmtK.Assign(Ident(f, RefKind.OwnField), _, _) if pd.fields.exists(fd => fd.name == f && fd.mods.vis == Vis.Private && !fd.mods.isFinal) =>
                  out += ((fqcn, f))
                case _ => ()
            }
          c.thisArgs match
  ...
  private def replayableFrom(declaringFqcn: String, body: List[BStmt]): Boolean =
    def findField(fqcn: String, name: String): Option[(String, Mods)] =
      byFqcn.get(fqcn) match
        case None               => None
        case Some((decl, info)) =>
          decl.fields.find(_.name == name).map(f => (fqcn, f.mods)).orElse(info.superFqcn.flatMap(findField(_, name)))
  ...
  /** Follow `this()`-delegation chain to the terminal ctor, substituting args through. Returns the args reaching the terminal. None when no matching ctor exists.
    */
  ...
  /** Raw transitive effects of constructing `parentFqcn(args)`, no replayability guard. None when structurally impossible (depth cap, outside the translated set).
    */
      ...
      case Some((_, info)) =>
        info.resolve(args.length, target) match
          ...
          case Some(c) =>
            ...
            val ownBody = c.body
              .filterNot(_.k == BStmtK.Empty)
              .map(BirTransform.mapStmt(_) {
                case Ident(n, RefKind.Param(_)) if subst.contains(n) => subst(n)
                case x                                               => x
              })
  ...
  /** Returns the DELTA beyond the parent's no-arg path (which the synthetic primary already runs via super()). Guards replayability in `forFqcn`.
    */
  def inlineSuperEffects(parentFqcn: String, args: List[BExpr], depth: Int = 0, forFqcn: String = "", target: Option[List[BType]] = None): Option[List[BStmt]] =
    rawEffects(parentFqcn, args, depth, target) match
      ...
      case Some(full) =>
        ...
        if delta.nonEmpty && parentFqcn != forFqcn && !replayableFrom(forFqcn, delta) then miss(s"$parentFqcn: replay delta touches private/final members — not replayable in $forFqcn")
        else Some(delta)
