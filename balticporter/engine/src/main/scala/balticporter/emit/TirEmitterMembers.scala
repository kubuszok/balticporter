...
/** Type-parameter/parent alignment, statement dispatch, member visibility/sealing, def/val/param rendering and control-flow boundary handling split out of TirEmitter to keep it within its file-size
  * limit.
  */
private[emit] trait TirEmitterMembers:
  ...
  /** The parent's promoted-constructor formal at position `n`, with the parent's OWN type parameters replaced by whatever the `extends` clause supplies for them — `MapIterator<V>`'s `IntMap<V>` seen
    * from `Entries[V] extends MapIterator[V]` is `IntMap[V]`, `V` now being `Entries`'. `None` when the parent is external, has no promoted constructor, or is applied at a different arity than it
    * declares.
    */
  ...
  /** An argument lifted into the `extends` clause whose declared type kept a wildcard fill (`map$p: IntMap[?]`) but the parent's constructor formal needs a real type there. Take the parent's formal
    * under its actual instantiation when available (matches a named type parameter exactly); fall back to the isolated wildcard elimination only when the parent cannot be seen into.
    */
  private[emit] def superArg(parent: TypeRepr, a: Term, n: Int, i: Int): String =
    if !hasWildcardArg(a.tpe) then term(a, i)
    else
      val target = superFormal(parent, n).filterNot(hasWildcardArg).map(tpe).getOrElse(deWildcarded(a.tpe, named = false))
  ...
  /** a parent type in an `extends` clause: a wildcard type argument is ILLEGAL here, so each `?` is replaced with its upper bound (or `AnyRef`). Only the HEAD is a `namedInner` position — a type
  ...
  private[emit] def declBounds(tycon: SymId): List[(SymId, TypeRepr)] =
    program
      .definitionOf(tycon)
      .collect { case c: Tree.ClassDef =>
        c.tparams.map(tp =>
          tp.symbol -> (tp.rhs.tpe match
  ...
  /** ONE substitution function for the whole engine — [[ParentSubst.subst]]. */
  private[emit] def substTp(t: TypeRepr, m: Map[SymId, TypeRepr]): TypeRepr = ParentSubst.subst(t, m)

  /** Render a type with every WILDCARD argument eliminated — illegal in an `extends` clause and as a cast target. A wildcard becomes its own written bound, else the type parameter's declared upper
    * bound (never a blanket `AnyRef`), resolved LEFT TO RIGHT. `named` is a Boolean rather than a by-name combinator (the mutable flag would not survive a strict function parameter).
    */

  /** The de-wildcarding CHOICE, as types rather than text — the same decision [[deWildcarded]] renders, exposed so a parent's elimination and its overrides derive from ONE answer. `None` where the
    * slot stays a wildcard (F-bounded, or nothing to fill from).
    */
  private[emit] def deWildcardedArgs(tc: TypeRepr, args: List[TypeRepr]): List[Option[TypeRepr]] =
    ...
    args.zipWithIndex
      .foldLeft((List.empty[Option[TypeRepr]], Map.empty[SymId, TypeRepr])) { case ((acc, m), (a, i)) =>
  ...
  private[emit] def deWildcarded(t: TypeRepr, named: Boolean): String =
    ...
    t match
      case TypeRepr.AppliedType(tc, args) =>
        val bounds  = headSymOf(tc).map(declBounds).getOrElse(Nil)
        val (as, _) = args.zipWithIndex.foldLeft((List.empty[String], Map.empty[SymId, TypeRepr])) { case ((acc, m), (a, i)) =>
  ...
  /** a statement rendered on ONE LINE, with any comment stripped — for the two positions where a newline is illegal (a `for` header's init and update clauses).
    */
  ...
  /** THE STATEMENT RENDERING DISPATCH — the emitter's statement-level surface, one of two. The obligation wrapper is here, at the dispatch, so no arm can escape it.
    */
  private[emit] def stat(s: Statement, i: Int): String =
    Rendering.of(TirKinds.of(s), s.origin, s)(statArm(s, i))

  /** JS-C47 / C48 / C49 / C50 — java's four access levels, consulted once here (where the three rendering arms converge) rather than once per declaration kind. JS-C47/C50 fire together: java's
    * package-private default vs scala's public one.
    */
  private[emit] def declVisibility(s: Symbol, at: Origin)(using Obligations): Unit =
    ...
    Obligations.consult(JS.C(49), at)(Option.when(v == Visibility.Vis.Private && s.owner != SymId.None && !topLevelSyms(s.owner))(()))
  ...
  /** the top-level type a symbol is emitted INSIDE — which is the emitted FILE, since a unit is a file. `SymId.None` for anything this program does not own, and that answer is load-bearing: a
    * permitted subtype the port does not declare is one no file of ours contains.
    */
  ...
  /** JS-C44 — java's `sealed`/`permits` (naming subclasses anywhere) against scala's file-scoped `sealed` (containing them). `sealed` is emitted only where the program-declared subtype set ACCOUNTS
    * FOR every permitted type ([[balticporter.tir.Symbol.permits]]), never from parsed survivors alone. Otherwise the type ships OPEN, recorded as a residue, since the untranslated open form is also
    * valid Scala and the compiler will not flag it.
    */
  private[emit] def sealOf(cd: Tree.ClassDef, s: Symbol, i: Int): (String, String) =
    if !s.flags.isSealed then ("", "")
      ...
      // interned ids, never names — the permits list and the emitted FQN are two namespaces.
  ...
  /** JS-C43 — the members javac DERIVES from a record header (JLS 8.10.3), on a plain `final class` not a `case class` (loses java's format/NaN/-0.0 semantics, adds surface java never had).
    * `equals`/`hashCode`/`toString` read FIELDS; the extractor reads ACCESSORS. Skipped where the record already declares the member, by SIGNATURE. @return CLASS members, COMPANION members, porter
    * note.
    */
  private[emit] def recordMembers(cd: Tree.ClassDef, s: Symbol, i: Int): (List[String], List[String], String) =
    if !s.flags.isRecord then (Nil, Nil, "")
      ...
      val declared: Set[(String, Int)] = cd.body.collect { case d: Tree.DefDef =>
        (sym(d.symbol).name, d.paramss.headOption.getOrElse(Nil).size)
      }.toSet
      ...
      /** does the record declare JAVA'S `equals` — the ONE-argument one taking `java.lang.Object` (JLS 8.10.3, 8.4.9)? By SIGNATURE, not (name, arity): java resolves `equals(String)` and
        * `equals(Object)` separately, so suppressing the wrong one silently downgrades to REFERENCE equality (`AnyRef.equals` stays concrete, no abstract-class error, no finding).
        */
      ...
      /** the emitted VALUE-CLASS name of a component, when it is a java primitive — the whole of "does this compare, hash and print by value". Read through `TirEmitter.ScalaValueClasses`, which is
        * the one place that set is spelled.
        */
      def primOf(c: RecordComponent): Option[String] =
        headSymOf(fieldTpe.getOrElse(c.field, sym(c.field).info)).map(x => sym(x).fullName).filter(TirEmitter.ScalaValueClasses.contains)
      ...
      val eqM =
        ...
        else if comps.isEmpty then List(s"${ind(i + 1)}override def equals(o$$rec: scala.Any): scala.Boolean = o$$rec.isInstanceOf[$self$tpWild]")
        else
          val cmp = comps.map(c => eqOf(c, mine(c), theirs(c))).mkString(" && ")
          List(
            s"${ind(i + 1)}override def equals(o$$rec: scala.Any): scala.Boolean = o$$rec match {",
      ...
      val hashM =
        if declared(("hashCode", 0)) then Nil
          ...
          List(s"${ind(i + 1)}override def hashCode(): scala.Int = {", s"${ind(i + 2)}var hash$$rec: scala.Int = 0") ++
      ...
      val synthesised = List("equals" -> eqM, "hashCode" -> hashM, "toString" -> strM, "unapply" -> unap).collect { case (n, ms) if ms.nonEmpty => n }
      val kept        = List("equals" -> eqM, "hashCode" -> hashM, "toString" -> strM, "unapply" -> unap).collect { case (n, ms) if ms.isEmpty => n }
      val d           = Decision(
  ...
  /** THE AREA-C ROWS A TYPE DECLARATION OWES — consulted at the dispatch, above every arm, since `classDef` forks into `enumDef`/`classDef1` and a row consulted inside only one would be a HOLE at the
    * other shape. Predicates are read off the tree and symbol table rather than by re-running the emitter, so a `fired` count means "this class has the shape", not "the repair emitted text".
    */
  private[emit] def classConsults(cd: Tree.ClassDef)(using Obligations): Unit =
    ...
    /** a member java runs in a class-initialisation step — a field WITH an initialiser, or a block. The same predicate at both steps: JLS 12.4.2 step 9 for the static pair and 12.5 step 4 for the
      * instance one, which is exactly why the two rows below share it.
      */
    ...
    Obligations.consult(JS.C(7), at)(Option.when(hasClinit(statics) || nearestClinitAncestor(cd.symbol).isDefined)(()))
    ...
    Obligations.consult(JS.C(18), at)(
      Option.when(
        inst.exists { case v: Tree.ValDef => v.rhs.isDefined; case _ => false } &&
    ...
    Obligations.consult(JS.C(38), at)(
      Option.when(
        isEnum &&
  ...
  /** does any of these type parameters mention ITSELF in its own bound — java's F-bound, which scala checks at every use and javac does not (JS-G35)?
    */
  ...
  private[emit] def statArm(s: Statement, i: Int)(using Obligations): String = s match
    ...
    case d: Tree.DefDef =>
      ...
      Obligations.consult(JS.G(41), d.origin)(HeapPollutionCheck.uncheckedVararg(d)(using program).map(_ => ()))
      declVisibility(sym(d.symbol), d.origin)
      if isInitBlock(d) then d.rhs.map(r => s"${declNotes(d.symbol, i)}${ind(i)}locally ${term(r, i)}").getOrElse("")
      else defDef(d, i)
  ...
  /** Disambiguate a member that arrives CONCRETE from both the superclass and a mixin. Java has single inheritance, never ambiguous there; scala linearises and refuses. Forward to the SUPERCLASS
    * (parents-list head) — the member java would have run. A `final` superclass member takes NO forwarder: minting one would override a `final` member, which scala forbids.
    */
  private[emit] def diamondOverrides(cd: Tree.ClassDef, i: Int): List[String] =
    ...
    if parentTs.sizeIs < 2 then Nil
      ...
      /** What an EXTERNAL parent makes concrete: a support type this run injected (`externalConcrete`) or an interface whose JLS 9.4.3 `default` methods the frontend read off the class file
        * (`Program.internedDefaults`). Both are MEASURED sets — an external parent absent from both contributes nothing.
        */
      def externalOf(t: TypeRepr): Set[(String, List[Int])] =
        headOf(t).map(x => sym(x).fullName).toSet[String].flatMap(fqn => externalConcrete.getOrElse(fqn, Set.empty) ++ program.internedDefaults.getOrElse(fqn, Set.empty))
        ...
        classOf_(t) match
          case Some(c) if !seen(c.symbol) =>
            val own = c.body.collect {
              case d: Tree.DefDef
                  if d.rhs.isDefined && sym(d.symbol).name != "<init>" &&
                    !sym(d.symbol).flags.isStatic =>
                (sym(d.symbol).name, d.paramss.map(_.size)) -> d
            }.toMap
            c.parents.map { case tt: TypeTree => tt.tpe; case x: Term => x.tpe }.foldLeft(own)((acc, pt) => concrete(pt, seen + c.symbol) ++ acc)
          case _ => Map.empty
      val sup     = concrete(parentTs.head)
      val mixins  = parentTs.tail.flatMap(t => concrete(t).keySet ++ externalOf(t)).toSet
      val ownKeys = cd.body.collect { case d: Tree.DefDef =>
        (sym(d.symbol).name, d.paramss.map(_.size))
      }.toSet
      ...
      // constructor replay run.
      val psub = ParentSubst.of(cd)(using program)
      supName.toList.flatMap { sn =>
        sup.toList.filter((k, d) => mixins(k) && !ownKeys(k) && !sym(d.symbol).flags.isFinal).sortBy((k, _) => k._1).map { (_, d) =>
          ...
          else d.tparams.map(td => typeParam(td.copy(rhs = td.rhs.copy(tpe = ParentSubst.subst(td.rhs.tpe, psub))))).mkString("[", ", ", "]")
          ...
          val pss = d.paramss.map(ps => paramClause(ps.map(v => v.copy(tpt = v.tpt.copy(tpe = ParentSubst.subst(v.tpt.tpe, psub)))))).mkString
  ...
  /** JS-S25 — java REJECTS unreachable code, Scala allows it, composed with `break`. A body ending in java's `while(true){ … return … }` never falls through, but Scala types `while(true)` as `Unit`,
    * so a non-Unit method needs a tail java did not have. One function since the dispatch consults it and `defDef` renders from it — a rule needed at two sites must be one shared function.
    */
  ...
  private[emit] def defDef(d: Tree.DefDef, i: Int)(using Obligations): String =
    ...
    if isCtor && d.tparams.nonEmpty then
      tparamSubst = savedSubst ++ d.tparams
        .map(tp =>
          tp.symbol -> (tp.rhs.tpe match
            case TypeRepr.TypeBounds(_, hi) if hi != TypeRepr.NoType => hi
            case _                                                   => TypeRepr.NoType)
    ...
    // trivia first, porter note last, member next — the note must not displace the licence.
  ...
  private[emit] def ctorRest(plan: CtorFunnel.Plan, cdef: Tree.DefDef, stats: List[Statement], after: List[Statement], eaten: Int): List[Statement] =
    if plan.delegations.contains(cdef.symbol) then after.drop(eaten)
      ...
      CtorFunnel.headStmt(cdef) match
        case Some(Tree.Apply(Tree.Select(_, m, _, _), _, _, _, _)) if sym(m).name == "<init>" => stats.tail
        case _                                                                                => stats

  /** Every statement `ctorBody` renders for a secondary — delegation arguments, replayed parent statements, own residual body — the one list the `@nowarn` decision reads.
    */
  private[emit] def ctorRendered(cd: Tree.ClassDef, cdef: Tree.DefDef): List[Statement] =
    ...
    val delegTerms: List[Term] = plan.delegations
      .get(cdef.symbol)
      .getOrElse(CtorFunnel.headStmt(cdef) match
        case Some(Tree.Apply(_, args, _, _, _)) => args
        case _                                  => Nil)
    plans.replayFor(cd, cdef).getOrElse(Nil) ++ body ++ delegTerms

  /** Loop-jump scope, as scala `boundary` nesting. `break`/`continue` need boundaries in DIFFERENT places (loop / body), so when a loop needs both the body boundary is innermost and the outer one
    * must be NAMED. `None` = no enclosing loop boundary; `Some("")` = unnamed, innermost; `Some(name)` = named because another boundary sits inside it. Re-pointed by `match` at the CASE's own
    * boundary (`contTarget` is not — `continue` inside a switch still continues the loop).
    */
  ...
  /** names the `def` that carries a lambda body containing `return`. SCOPED TO ONE DECLARATION by [[inDeclaration]], never to the program, or the name would renumber under an unrelated edit and
    * rename many members.
    */
  private[emit] var lambdaSeq = 0

  /** counter for lvalue-binding temporaries (`$lv1`, `$lv2`, …), scoped identically to `lambdaSeq` — a compound-assignment temporary in one member must not consume another's.
    */
  private[emit] var lvSeq = 0

  /** run `f` with the synthetic-name counters SAVED, reset, and restored — a nested declaration (an anonymous class's method inside a lambda) is its own naming scope and must not consume the
    * enclosing member's numbers.
    */
  private[emit] def inDeclaration[A](f: => A): A =
    ...
    lvSeq = 0
    try f
    finally { lambdaSeq = saved; lvSeq = savedLv }
  private[emit] def inLoop[A](brk: Option[String], cont: Option[String])(f: => A): A =
    ...
    try f
    finally { breakTarget = sb; contTarget = sc }
  private[emit] def inSwitch[A](brk: Option[String])(f: => A): A =
    val sb = breakTarget
    breakTarget = brk
    try f
    finally breakTarget = sb

  /** the value-carrying `Label` a non-tail `yield` must name — a switch EXPRESSION's arm boundary. Kept apart from [[breakTarget]]: a `break` carries `Unit` and a `yield` the switch's own type, so
    * one boundary cannot serve both (and JLS 15.28 keeps them from ever coexisting). ALWAYS named, so nothing nearer can steal `break(v)(using n)`.
    */
  ...
  private[emit] def inYield[A](y: Option[String])(f: => A): A              =
    ...
    yieldTarget = y
    try f
    finally yieldTarget = sy

  /** java LABEL -> the scala boundary name a `break`/`continue` naming it must target. A labelled jump can sit at any depth, so unlike the unlabelled ones these are looked up, not scoped.
    */
  ...
  /** A java enhanced-for BINDING is a declaration with its own type; scala's `for (x <- xs)` binds at the ITERABLE's element type, and java lets them differ (`for (Object e : collection)` over a
    * raw/wildcarded `Collection`). Returns the declared type to re-bind at, or `None` when scala's binding is already exact. Conservative: an unreadable element type agrees rather than inventing a
    * cast on no evidence.
    */
  ...
  /** is the enhanced-for BINDING written to inside the loop body? Java's binding is an ordinary local and may be assigned; scala's generator binds a `val`. Scanned with `StandardTraversal`, counting
    * `IncDec` beside `Assign`; over-approximating costs only an unneeded `var`.
    */
  private[emit] def reassignsBinding(body: Tree, binding: SymId): Boolean =
    given Program = program
    body match
      case t: Term =>
        StandardTraversal.scanTerm(t, false) { (found, x) =>
          x match
  ...
  /** the element type of something java could put in an enhanced-for: an applied generic's single argument, or an array's element. `None` = not readable, which callers must treat as no evidence
  ...
  /** Is the iterable's POST-PIPELINE type a JDK `Iterable` the pipeline left in the java namespace? Such a type has no scala `foreach`, so `for (x <- xs)` does not compile. Decided from the NODE:
    * external (not program-owned) AND in `java.*`/`javax.*`. A retyped type is no longer in that namespace by the time the emitter runs, and arrays are excluded by construction (`headSymOf` returns
    * `None` for them).
    */
  /** Does an enhanced-for over this type need java's own iterator protocol? Yes for a JDK iterable the pipeline KEPT, and for a PROGRAM type that reaches `java.lang.Iterable` through parents this
    * program declares while neither it nor any of them declares a `foreach` (libGDX's own collections, 225 sites). A retyped or runtime type has `foreach`.
    */
      ...
      program.definitionOf(id) match
        case Some(cd: Tree.ClassDef) =>
          ...
          if declaresForeach then false
            ...
            val reachesJavaIterable = parentIds.exists(pid => program.symbolOf(pid).exists(_.fullName == "java.lang.Iterable"))
            reachesJavaIterable ||
            parentIds.exists(pid => program.owns(pid) && ownedJavaIterableWithoutForeach(pid, seen + id))
        case _ => false

  private[emit] def loopWithJumps(body: Tree, label: Option[String], render: (=> String) => String, bodyStr: => String)(using Obligations): String =
    ...
    if !hasB && !hasC then render(bodyStr)
      ...
      val inner =
        try
          inLoop(if hasB then Some(bName) else scala.None, if hasC then Some(cName) else scala.None) {
  ...
  /** does this loop body contain a construct the emitter renders with a `boundary` of ITS OWN? `boundary.break(())` with no `using` resolves the innermost `Label`, so an interposed boundary silently
    * retargets an un-annotated jump — a [[Tree.Labeled]] actually broken to, or a switch case's mid-case `break`. Deliberately OVER-approximates; stops at a nested loop, lambda, `def` or anonymous
    * class.
    */
  private[emit] def interposes(t: Any): Boolean = t match
    ...
    case m: Tree.Match   =>
      ...
      m.cases.exists(c =>
        caseNeedsBoundary(c.body) || (m.isExpr && caseYieldsOut(c.body)) ||
          interposes(c.body)
      )
  ...
  /** a labelled statement earns a boundary only when something actually breaks to its label — java lets a label sit on a statement nobody jumps to, and an empty boundary would be noise that also has
    * to be shielded against.
    */
  private[emit] def labelNeedsBoundary(l: Tree.Labeled): Boolean = jumpsTo(l.stmt, l.name, brk = true)

  /** an unlabelled `break` in a switch case that is NOT the case terminator. The frontend strips a trailing unlabelled `break` (it is what ends the case, which scala's `match` does anyway) and lowers
    * real fallthrough by duplicating the next case's tail — so a `break` still standing in a case body means "stop HERE and leave the switch", over statements that follow it.
    */
  private[emit] def caseNeedsBoundary(body: Term): Boolean = breaksOut(body)

  /** a non-tail `yield` in a switch EXPRESSION's arm — the value-carrying twin of the predicate above. The frontend peels the TAIL yield into the arm's value, so anything reaching this is a `yield`
    * that leaves the arm from inside an `if`, a nested block or ahead of another statement, and scala has no expression-level jump to render it with.
    */
  ...
  // same trees: two copies would be two answers, and the one
  ...
  /** does this subtree `return` from the construct that OWNS it? Stops at a nested `Lambda`, `DefDef` or anonymous-class body, for `breaksOut`'s reason. Product reflection rather than a case per node
    * — a hand-rolled walk stopping short is how two of this project's silent defects survived.
    */
  ...
  /** the result type to give the `def` that carries a lambda body containing `return`. TWO SOURCES, tried in order: the node's own `resultTpt` (a converted SAM method's fact), then the body (every
    * `return` VALUELESS is a `void` lambda). `None` means DO NOT REWRITE, never "use `Any`" — a guessed type compiles and means something else, and an untranslatable construct is refused and counted
  ...
  /** A Scala secondary constructor must delegate to `this(...)` first — never `super(...)`. Keep a Java `this(args)` delegation. A leading `super(args)` becomes `this()` followed by the parent
    * constructor's own statements, when `CtorFunnel` has established that the two together run exactly what `super(args)` ran; where it has not, the arguments are still lost and `OmissionCheck` still
    * counts them.
    */
  private[emit] def ctorBody(cdef: Tree.DefDef, i: Int): String =
    ...
    // post-bodies now in the primary's class body, not per-secondary.
    ...
    val deleg = plan.delegations.get(cdef.symbol) match
      ...
      case None =>
        CtorFunnel.headStmt(cdef) match
          ...
          case _ => "this()"
    // a CONSUMED `this.f = e` does not disappear from the file, so its comment rides THIS
    ...
    // PREFIX STRIP: where this constructor ESCAPES the promotion and its
  ...
  /** A secondary constructor's `super(args)` — which scala cannot write — expressed as a delegation to the PRIMARY, whose own `extends Parent(…)` makes the call. The DECISION is
    * `CtorFunnel.Plans.superCall`; this only renders it, so `OmissionCheck` can count a `Dropped` super call independently of what this method lowers it to.
    */

  /** the DISAMBIGUATOR's argument, when the class's primary takes one. ASCRIBED, never a bare `null` — an unascribed `this(null)` against an overload `C(String)` is `E051 Ambiguous overload`;
    * `(null: C.Funnel)` has exactly one candidate.
    */
  private[emit] def markerArg(cd: Tree.ClassDef, name: String): String =
    s"(null: ${typeValue(cd.symbol)}.${esc(name)})"

  /** the same ascription at a slot argument: a synthesised primary's delegation is an argument list JAVA NEVER WROTE, so a root not assigning a hoisted field contributes that field's own (often
    * `null`) java initialiser — ambiguous the same way [[markerArg]] is. Declines on a delegation JAVA WROTE and at an ABSTRACT type slot (`Null` does not conform).
    */
  private[emit] def slotArg(a: Term, slot: Option[TypeRepr], i: Int): String = (a, slot) match
    // an OPAQUE slot over a primitive has no `null`: the cast erases to the primitive's zero.
    ...
    // an `if` with a block body in a synthesised delegation misparsed by Scala 3's
    // `this(...)` grammar -- parenthesise to delimit the expression.
    case (_: Tree.If, _) => s"(${term(a, i)})"
    case _               => term(a, i)

  /** does this slot's type name a TYPE PARAMETER this program declares? `Null` does not conform to one, which is the same fact `CtorFunnel.javaDefault` refuses to mint a `null` for.
    */
  ...
  /** A `using` parameter with NO NAME renders ANONYMOUSLY — `(using T)` — not cosmetic: a named context parameter named after an emitted root package SHADOWS it and breaks every fully-qualified
    * reference in scope (this backend emits fully-qualified names, no imports). Nothing reads the name (`using` resolution and `summon` never do); an empty name cannot capture a real one since the
    * frontend gives every parameter java's own name.
    */
  ...
  private[emit] def param(v: Tree.ValDef): String =
    ...
    // DIFFERENT signature than java's.
    val t = injectedOverrideTypes.getOrElse(v.symbol, overrideAlign.getOrElse(v.symbol, v.tpt.tpe))
    // a DEFAULT a phase minted rides as verbatim source (a captured value's `= null`)
    val df = v.rhs match
  ...
  private[emit] def valDef(v: Tree.ValDef, i: Int)(using Obligations): String =
    ...
    Obligations.consult(JS.C(36), v.origin)(Option.when(vs.flags.isStatic && ownerCd.exists(c => sym(c.symbol).flags.isTrait))(()))
    ...
    Obligations.consult(JS.C(53), v.origin)(
      Option.when(
    ...
    // @nowarn — without it a phase-attached annotation is silently dropped: unused locals and
    // private members are errors under the reference build's warning flags.
  ...
  private[emit] def valDef0(v: Tree.ValDef, i: Int): String =
    ...
    if s.flags.isGiven then
      // An EMPTY NAME renders an ANONYMOUS given: a minted name could shadow
    ...
    v.rhs match
      ...
      case Some(r) =>
        ...
        // write set is BeanCollapse.writtenSymbols) emits val instead of var. NON-PRIVATE
        // fields stay var even when unwritten HERE — a dependent port may write them.
        val isField = program.definitionOf(s.owner).exists(_.isInstanceOf[Tree.ClassDef])
        val kw      =
          if s.flags.isMutable && (isWritten(v) || (isField && !s.flags.isPrivate))
          then "var"
          else "val"
        val q = privateQualifier(s.owner)
        val m = if kw == "var" then mods(s, q).replace("final ", "") else mods(s, q)
  ...
  /** the literal rendered AT the field's declared type. `inline val` takes its constant type from the literal (a type ascription is rejected), so `static final float degFull = 360` must emit `360.0f`
    * and not `360`, or the field becomes an Int constant and divisions using it round.
    */
  private[emit] def constAt(r: Term, t: TypeRepr): String = (r, t) match
    case (Tree.Literal(c, _, _), TypeRepr.TypeRef(_, sy)) =>
      def num: Option[BigDecimal] = c match
        case Constant.ByteC(v)   => Some(BigDecimal(v.toInt));
        case Constant.ShortC(v)  => Some(BigDecimal(v.toInt))
        case Constant.IntC(v)    => Some(BigDecimal(v));
        case Constant.LongC(v)   => Some(BigDecimal(v))
        case Constant.FloatC(v)  => Some(BigDecimal(v.toDouble));
        case Constant.DoubleC(v) => Some(BigDecimal(v))
        case Constant.CharC(v)   => Some(BigDecimal(v.toInt));
        case _                   => scala.None
  ...
  /** a java CONSTANT VARIABLE: `static final`, primitive or `String`, literal initialiser. Delegates to `ClassInitTriggerCheck.constantVariable` — the same predicate the class-init census uses, so
    * the two can never disagree about which fields are step-9 content.
    */
  ...
  /** A declaration's Java annotations, rendered ahead of it.
    *
    * FULLY QUALIFIED like every other reference this phase emits, so `@Test` becomes `@org.junit.Test` and needs no import. Losing these is a silent correctness defect: a JUnit suite whose `@Test`
    * did not survive runs ZERO tests and reports SUCCESS.
    */
  ...
  /** The top-level type a symbol lives in, when it is NOT that type itself — the qualifier a nested class's `private` member needs. Java scopes `private` to the enclosing TOP-LEVEL class; scala's
    * bare `private` is class-only, so `private[TopLevel]` is faithful. Applied ONLY to a NESTED class's members (a top-level class's own `private` needs none — regressed once).
    */
  ...
  /** The ACCESS modifier alone — [[Visibility]] decided the level, this supplies the qualifier. The two package-scoped levels take [[currentPkgTail]], the package being written right now; only a
    * cross-package override carries its own (enclosing) package.
    */
