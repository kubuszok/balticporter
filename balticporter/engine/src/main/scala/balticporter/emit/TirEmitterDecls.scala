...
/** Class/enum declaration and constructor/class-initialiser ordering emission split out of TirEmitter to keep it within its file-size limit. */
private[emit] trait TirEmitterDecls:
  self: TirEmitter =>

  // ---- definitions ----
  /** the classes currently being rendered, outermost first. Lets a `Tree.This` naming an ENCLOSING class render Java's qualified `Outer.this` rather than a bare `this` (which names the inner one).
    */
  ...
  private[emit] def classDef(cd: Tree.ClassDef, i: Int): String =
    ...
    try classDef0(cd, i)
    finally { classStack.removeLast(); currentClass = outer }

  /** Parameter symbol to type for raw-parent override alignment. Uses `deWildcardedArgs` substitution so parent and override agree by construction.
    */
  private[emit] def rawParentAlignment: Map[SymId, TypeRepr] =
    ...
    def inheritedThrough(od: Tree.DefDef, pcd: Tree.ClassDef): Option[Tree.DefDef] =
      val chain = chainOf.getOrElseUpdate(pcd.symbol, (pcd.symbol :: graph.ancestorsOf(pcd.symbol)).toSet)
      graph.overridden(od.symbol).iterator.filter(m => chain(graph.ownerOf(m))).flatMap(m => program.definitionOf(m).collect { case d: Tree.DefDef => d }).nextOption()
        ...
        do
          ...
          val subst = pt match
            case TypeRepr.AppliedType(tc, args)
              ...
              pcd.tparams.map(_.symbol).zip(deWildcardedArgs(tc, args)).collect { case (s, Some(t)) => s -> t }.toMap
  ...
  /** Override alignment against injected parents' wildcard bounds. */
    ...
    if injectedSurface.isEmpty then Map.empty
      ...
      do
        ...
        for
          ...
          injSig <- injectedSurface.lookup(pSym.fullName, sym(od.symbol).name, od.paramss.flatten.size)
  ...
  private[emit] def alignToInjected(tirType: TypeRepr, injected: String, tparamSubst: Map[String, TypeRepr]): TypeRepr =
  ...
  /** An ARGUMENT reaching a parameter [[rawParentAlignment]] re-rendered. Java accepted the call because the callee's formal was RAW there (`ParticleEffect.save(AssetManager, ResourceData)` taking a
    * `ResourceData<ParticleEffect>`); once the formal reads `ResourceData[Object]` the conversion java made silently has to be written. Only fires where the argument disagrees.
    */
  ...
  /** A cast onto a parameter that [[rawParentAlignment]] re-rendered must land on the type the parameter now HAS — the frontend built these casts against the raw fill's `ResourceData[?]`, and once
    * the declaration reads `ResourceData[Object]` the same cast asserts nothing new. Only wildcarded targets on an aligned symbol are touched.
    */

  /** a POLY EXPRESSION (JLS 15.2) — a lambda or method reference with no type of its own, taking one from the slot it fills. The emitter's copy of `SpoonTir.polyExpression`'s question, asked of the
    * TIR since this is the one place a cast on such a term can still be reached (a cast the java SOURCE wrote is kept by design). `uncomment`: trivia wraps a term without changing it.
    */
  ...
  /** `this` in Scala always names the INNERMOST class, where Java's `Outer.this` names an enclosing one. Qualify by simple name only when the enclosing class is actually being rendered around this
    * point. A SUPERTYPE is never qualified even when it also encloses: constructor replay moves the base's `this` into the subclass body, where bare `this` is right.
    */
  ...
  private[emit] def inheritsFrom(child: SymId, anc: SymId): Boolean =
    ...
    def go(c: SymId): Boolean =
      c == anc || (seen.add(c) && program
        .definitionOf(c)
        .collect { case cd: Tree.ClassDef =>
  ...
  private[emit] def classDef1(cd: Tree.ClassDef, i: Int): String =
    ...
    // whole-program question) — bare `protected`, matching the reference
    // ports. A DISAMBIGUATED synthesis adds a marker-typed parameter to change ARITY.
    ...
    // `summon` in the body goes unresolved. Empty for a port threading
    // nothing, so no emitted byte moves.
    val givenClause = plan.givens.map(paramClause).mkString
    // A PROMOTED java constructor is still a java DECLARATION, so the visibility mapping governs it as it
    ...
    // Decided by SYMBOL over the LOWERED body — every write here is a `Tree.Assign`.
    ...
    val prim =
      if plan.isSynthesised then s" protected (${(plan.synthetic.map((n, t) => s"$n: ${tpe(t)}") ++ markerParam).mkString(", ")})$givenClause"
    ...
    // value parameter is exactly the shape a context clause's rendering measured. A `trait` reaches this with no clause on
    ...
    if kw == "class" && parents.isEmpty && cd.body.nonEmpty && !hasInstanceState && pparams.isEmpty &&
      !extendedTypes(cd.symbol) && !instantiatedTypes(cd.symbol) && !typeNamedElsewhere(cd.symbol)
    then
      ...
      // its kind as `class` (the cross-module face of the all-static collapse rule). Recorded here rather than
      ...
      // nowhere to go here — counted rather than silently dropped.
    ...
    // an `@interface`'s ELEMENTS (JLS 9.6.1) become the emitted class's CONSTRUCTOR
    ...
    // slots (`!! UNLOCATABLE`, and a relocated javadoc — the trivia recovery lane, not two defects).
    ...
    // the CLASS-INITIALISATION trigger, ahead of every other statement since that is where
    ...
    // and never where forcing would RE-ENTER an in-progress initialisation.
    val force =
      if hasClinit(statics) && kw == "class" && !s.flags.isAnnotation &&
    ...
    // parent secondary's post-body, guarded.
    ...
    // the visibility mapping renders every nested type QUALIFIED (`private[TopLevel]`/`private[pkg]`), so the
    ...
    // …and its NOTES go the same way: a PROMOTED constructor has no `def` for an
    // `AtDeclaration` note to sit above, so a decision subjected at it produced NO NOTE at all
    // (measured 1 on libGDX base). The class is where scala documents a
    // primary constructor, matching the javadoc above.
    val ctorNote = plan.primary.toList.map(c => declNotes(c.symbol, i)).mkString
    // JS-C44 — note placed after cnote in the porter-note ordering.
    val (seal, sealNote) = sealOf(cd, s, i)
    // `@interface` elements become the class's parameter list (a `val`, keeping java's
    ...
    val cls       =
      if s.flags.isAnnotation then s"${leading(cd.leading ++ annotLead, i)}$cnote${annots(s, i)}${ind(i)}class ${esc(s.name)}$tps$annotPrim extends scala.annotation.StaticAnnotation"
      else
        s"${leading(cd.leading ++ ctorLead, i)}$cnote$ctorNote$sealNote$recNote${annots(s, i)}$promotedNowarn${ind(i)}${mods(s, privateQualifier(s.owner))}$seal$abs$kw ${esc(s.name)}$tps$prim$ext$open"
    ...
    delivered.flatMap(_.keys).distinct.foreach { n =>
      ...
      if at.sizeIs > 1 then
        ...
        // than guess.
        val winner = at.find(j => owners.forall(o => o == delivered(j)(n) || ancestorsOf(delivered(j)(n)).contains(o))).getOrElse(at.head)
    ...
    // engine's runtime artifact for a purely local encoding. A class that needs
    ...
    recordTypeShape(cd, form = if s.flags.isAnnotation then "annotation" else kw, plan = plan, companion = hasCompanion, statics = ownStaticNames)
    if !hasCompanion then cls
    else
      // the SECOND class-initialisation trigger — JLS 12.4.1 item 7: initialising a class initialises its SUPERCLASS
      ...
      // `hasCompanion` rather than re-deriving "does anything read a static of this type".
      val superForce =
        nearestClinitAncestor(cd.symbol).filterNot(reentrantBearers.contains).map(a => forceCompanion(cd, a, balticporter.tir.ClassInitTriggerCheck.SubclassInit, i + 1)).toList.filter(_.nonEmpty)
  ...
  /** Java enum → `sealed abstract class Name <parents-minus-Enum> { members }` plus a companion `object` holding each constant as a `case object` and a `values` array.
    */

  /** A java ENUM takes ONE of two shapes, a fact about java's own declaration — [[balticporter.tir.EnumShape]], read here and by `OmissionCheck.enumShapeRefusals` so the two can never disagree. Scala
    * 3 `enum` IS a `java.lang.Enum[X]`, answering every bound/`EnumSet`/`EnumMap` a ported enum needs. A class BODY on a constant, or a member colliding with `java.lang.Enum`'s finals, keeps the
    * sealed shape instead — a REFUSAL, counted, never silently chosen.
    */
  ...
  /** The parts BOTH enum shapes are made of, derived ONCE. The two arms differ only in the header and in what `java.lang.Enum` supplies — never in the primary constructor, its promoted params,
    * superseded fields or surviving statements. Two derivations would drift.
    */
  final private[emit] case class EnumParts(ctorParams: List[Tree.ValDef], paramNames: Set[String], instance: List[Statement], ctorStats: List[Statement], statics: List[Statement], eprimary: String)
  ...
  private[emit] def enumParts(cd: Tree.ClassDef): EnumParts =
    ...
    // a named field, so it is dropped and COUNTED. THE ROOT, never
    ...
    // exactly the one the parameter was renamed out of the way of.
    ...
    def enumParam(v: Tree.ValDef): String =
      ...
      standsFor.get(v.symbol) match
        case Some(f) =>
          val fs = sym(f)
          val kw =
            if fs.flags.isMutable || writtenAfterPromotion(f) || writtenAfterPromotion(v.symbol)
            then "var"
            else "val"
  ...
  /** THE PRE-EXISTING SHAPE — a `sealed abstract class` plus one `case object` per constant. What a java enum the scala 3 `enum` cannot express is emitted as; does NOT extend `java.lang.Enum[X]`, so
    * `name()`/`ordinal()`/`values()`/`valueOf` are all supplied by hand. `EnumShape.refusal` decides which enums arrive here; `OmissionCheck` counts them.
    */
  private[emit] def sealedEnumDef(cd: Tree.ClassDef, i: Int): String =
    ...
    // it. A promoted ctor parameter counts as a declared `name` too.
    ...
    // the visibility mapping governs the enum TYPE; its constructor is left public — java's implicit `private`
  ...
  /** `enum X(…) extends java.lang.Enum[X] with …` — the shape used where a caller depends on the `java.lang.Enum` supertype itself (a bound, `EnumSet`/`EnumMap`, `Comparable`, `isEnum`).
    * `name()`/`ordinal()` are FINAL there and `values`/`valueOf` come from the `enum` desugaring, so `EnumShape.Reserved` refuses an enum whose own declaration needs one of those names — a promoted
    * enum constructor parameter becoming a member can collide with `Enum.name()` or a declared accessor just as a body field can. `values` renders parenless here; `applyStr0` matches.
    */
  ...
  /** Replace the constructor `CtorFunnel` promoted to Scala's PRIMARY by its own body statements, which run at construction. `super(args)` is already in the `extends` clause and its params in the
    * class's parameter list; every other constructor stays a secondary `def this(...)`.
    */
  ...
  /** the local `def` a PROMOTED constructor body carrying a `return` is wrapped in. Named per class, not per program, so it cannot renumber under an unrelated edit and rename many members.
    */
  private[emit] val CtorBodyName = "ctorBody$"

  /** JS-C51 — java `return` in a constructor leaves the constructor (JLS 14.17); promoted into the class body it becomes `E091`. Wrapped in a local `def` (a valid `return` target, unlike
    * `boundary.Break`) inside a block so it is not a class member. Only `plan.primaryBody` moves inside; [[returnsIn]] stops at a nested `Lambda`/`DefDef`/`AnonClass`. A value-carrying `return` is
    * refused (javac itself rejects it in a constructor).
    */
  ...
  /** does this member list carry java CLASS INITIALISER content — JLS 12.4.2 step 9, never the instance initialiser? Delegates to `ClassInitTriggerCheck.stepNine`, the one definition the repair and
    * its watchdog share — keyed on the `static { }` block alone this missed a registration written as a static field initialiser (same `<clinit>` sequence). The constant variable stays outside it:
    * javac inlines it (JS-C08).
    */
  ...
  // a java class initialiser (JLS 12.4.2 step 9) runs at class init; a scala companion
  ...
  // REFLECTION cannot be reproduced and is stated, not counted.

  /** The note and statement that force `target`'s companion, recorded as one [[Decision]] about `cd` so the note is DERIVED and `NoteCoverageCheck` sees the pair. `target` is `cd` itself for the
    * instantiation trigger, an ANCESTOR for the subclass one (JLS 12.4.1 item 7).
    * @param trigger
    *   which of JLS 12.4.1's actions this statement stands for.
    */
  ...
  /** every type this program declares whose class initialiser does anything — [[hasClinit]]'s question (JLS 12.4.2 step 9), asked of the DECLARED body.
    */
  ...
  /** …and the ones whose force would be RE-ENTRANT, which the repair declines and `class-init-trigger` counts. The check's own function, so the refusal and the count cannot disagree about which types
    * it names.
    */
  ...
  /** the ancestor edges JLS 12.4.1 item 7 traverses — the SUPERCLASS chain plus a superinterface declaring a default method, and never `parentsBySym`, whose edges are every parent there is.
    */
  private[emit] lazy val item7ParentsBySym: Map[SymId, List[SymId]] =
    balticporter.tir.ClassInitTriggerCheck.item7Parents(program)

  /** the nearest ancestor of `s` carrying a class initialiser — the ONE this type's companion has to force. Java initialises the whole superclass chain, and forcing only the nearest reproduces that
    * because THAT type's companion carries the same line for ITS own nearest, recursively. Breadth-first, so "nearest" means nearest and not "first found down one branch".
    */
  ...
  /** every (type, trigger) pair this emitter forced — the input to `class-init-trigger`, which takes the CENSUS of `static { }` blocks from the trees itself. An empty set therefore reproduces the
    * un-repaired engine on the same trees, exactly as `switch-null` does. Keyed by the TRIGGER as well as the type because the two are answered at different call sites and a type covered for one is
    * not covered for the other.
    */
  ...
  private[emit] def isStatic(s: Statement): Boolean = s match
    ...
    // (`AddMembersTransform` static specs).
    case o: Tree.Opaque => o.companionMember.isDefined
    case _ => false

  /** Scala secondaries must delegate to a PRECEDING constructor, so order fields first, then constructors in DELEGATION-TOPOLOGICAL order (`this(...)` edges, not arity), then everything else. `owner`
    * decides WHICH `ValDef`s hoist: java's STEP-4 members hoist ahead of the body (field initialisers AND instance init blocks, textual order); a PROMOTED CONSTRUCTOR LOCAL stays in place. Told apart
  ...
  private[emit] def orderBody(body: List[Statement], owner: SymId, paramfulPrimary: Boolean = false): List[Statement] =
    ...
    def delegateTarget(d: Tree.DefDef): Option[SymId] = CtorFunnel.headStmt(d) match
      case Some(Tree.Apply(Tree.Select(r, m, _, _), args, _, _, _)) if sym(m).name == "<init>" && args.nonEmpty && !r.isInstanceOf[Tree.Super] => Some(m)
      case _                                                                                                                                   => None
    ...
    // a FIELD of `owner` — not merely a `ValDef`. See the doc above for why the difference is
