...
/** Emission backend: transformed TIR to Scala 3 source.
  ...
  *   concrete instance members of injected supertypes, keyed `(FQN, param counts)` — required for [[diamondOverrides]] to see injected parents
  * @param provenance
  *   upstream attribution header stamped on every emitted unit; `None` = no header
  * @param notes
  *   the run's decision log (read-only) — decisions render as porter notes beside code.
  */
final class TirEmitter(
  ...
  /** Member surface of injected Scala files (param types and arity), so emitted overrides adopt the injected signature. Populated by `PortRun`; empty for specs.
    */
  ...
  private[emit] val prepared =
    TirEmitter.resolveCapturedLocalClashes(
      TirEmitter.funnelParamRenames(
        TirEmitter.resolveFieldShadowing(TirEmitter.resolveMemberClashes(source, own, surface, catalog), own, surface, catalog),
        own,
        surface
      ),
      own
  ...
  private[emit] val (orNullCtors, orNullClasses): (Set[SymId], Set[SymId]) = {
    ...
    def record(s: SymId, what: String, slug: String): Unit =
      TirEmitter.note(
        own,
        Decision.Kind.SuppressedWarning,
        program,
        s,
        Map(
      ...
      StandardTraversal.allClassDefs(u).foreach { cd =>
        ...
        CtorFunnel.ctorsOf(program, cd.body).foreach { d =>
          ...
          if ds.name == "<init>" && !hasDeprecatedNowarn(ds) && DeprecatedUseScan.count(ctorRendered(cd, d)) > 0 then
            ...
            record(
              d.symbol,
              "this constructor's rendered statements (delegation arguments, replayed parent statements, own body)",
  ...
  /** the top-level type's symbol, and the class whose body is being rendered right now. A java `private` needs a qualifier only when the two DIFFER, i.e. the member lives in a NESTED class.
    */
      ...
      if !bestEffort then ""
        ...
        if mine.isEmpty then ""
          ...
          "// ############################################################################\n" +
            ...
            mine
              .map(m =>
                s"//   ${m.kind.label}${m.diff.fold("")(d => s" [$d]")} at " +
  ...
  private[emit] lazy val renamedMembers: Map[SymId, String] =
    own.iterator.collect {
      case d
          if d.kind == Decision.Kind.RenamedMember && d.subject != SymId.None &&
  ...
  /** Access level as actually rendered: `private`, `private[Outer]`, or `public`. Java `protected` is emitted as public (loosening can only remove errors).
    */
  ...
  private[emit] def recordTypeShape(cd: Tree.ClassDef, form: String, plan: CtorFunnel.Plan, companion: Boolean, statics: List[String]): Unit =
    val s  = sym(cd.symbol)
  ...
  private[emit] lazy val noteIndex: Map[SymId, List[Decision]] =
    notes.all
      .filter(d => PorterNote.Rendered(d.kind) && d.subject != SymId.None)
      .groupBy(_.subject)
      // Stable sort within a subject for deterministic output.
      .view
      .mapValues(_.sortBy(d => (d.kind.toString, d.reason.className, d.reason.detail, d.tsv)))
      .toMap
  ...
  private[emit] lazy val noteIndexByFqn: Map[String, List[Decision]] =
    notes.all.filter(d => PorterNote.Rendered(d.kind) && d.subject == SymId.None).groupBy(_.subjectFqn).view.mapValues(_.sortBy(_.tsv)).toMap
  ...
  private[emit] def checkClause(cd: Tree.ClassDef, rendered: Boolean, form: String): Unit =
    if !rendered && CtorFunnel.ctorsCarryGivens(program, cd) then clauseLost(cd.symbol) = TirEmitter.ClauseLoss(cd.symbol, sym(cd.symbol).fullName, form, cd.origin)
...
object TirEmitter:
  ...
  /** the emitted types a JAVA PRIMITIVE renders as — "this value cannot be null". Spelled ONCE and read by both the emitter and `SwitchNullCheck`, in EMITTED names (a java int arrives here as
  ...
  /** the WRAPPER whose static `hashCode` javac uses for a primitive record component (JS-C43), keyed on [[ScalaValueClasses]]'s own spellings (minus `scala.Unit`, which no component has).
    */
  ...
  /** Scala's reserved words, plus the soft keywords a bare occurrence can still steer the parser into. Backticking one that did not need it costs two characters and can never change meaning, so the
    * set over-approximates deliberately.
    */
  ...
  /** The default `javaSource`: the upstream file, read once per path. Memoised per JVM (a pure function of the path, asked once per top-level type). An unreadable path is `None`, not an exception — a
    * source tree that moved after parsing must not fail the emitter; `TriviaCheck` reports the same absence as an uncompared file.
    */
  ...
  /** the last segment of a dotted package name — the only form a Scala access qualifier has, since the language has no dotted qualifier at all (`private[a.b]` does not parse).
    */
  ...
  /** THE BASE-SURFACE CONTRACT, as one emitter recorded it (DESIGN.md §8.3).
    ...
    * @param members
    *   emitted member key (the source map's spelling) → the same, per member
    */
    ...
    if all.isEmpty then p
      ...
      p.symbols.all.foreach { s =>
        if all(s.id) && narrower(s.flags) then
          ...
          note(
            out,
            Decision.Kind.WidenedVisibility,
            src,
            s.id,
            Map(
  ...
  /** Promoting a constructor to Scala's primary widens the SCOPE of everything it declares: its params and top-level locals become class members visible to the whole body, which risks a double
    * definition against an own member or a silent capture of an inherited one. Suffixing `$p` removes both — the rename is invisible everywhere it matters.
    */
  def funnelParamRenames(p: Program, out: collection.mutable.Buffer[Decision] = collection.mutable.ListBuffer.empty, surface: Surface = null): Program =
    ...
    /** everything this class's promoted constructor contributes to the class BODY — its params and its top-level locals. Neither is in `cd.body`, and both become members, so both are names a SUBCLASS
      * must avoid: `DepthShader extends DefaultShader` promotes the same two constructor locals and landed on `attributes$p` twice.
      */
  ...
  def resolveFieldShadowing(
    ...
    /** JS-C04's citation surface: a whole-program pass CITES the row once per declaration decided about (Decision's own granularity), discarding by default so the determinism twin does not double the
      * count.
      */
    ...
    /** EFFECTIVE names — a renamed ancestor field contributes its NEW name, so a descendant asking "is this taken?" sees what will actually be emitted. Requires parents-first scanning.
      */
    def eff(id: SymId):                     String      = renames.getOrElse(id, nm(id))
    ...
    def inherited(cd: Tree.ClassDef, seen: Set[SymId] = Set.empty): Set[String] =
      cd.parents
        .flatMap { case tt: TypeTree => headSym(tt.tpe); case t: Term => headSym(t.tpe) }
        .filterNot(seen)
        .flatMap(declOf.get)
    ...
    /** does an UNPARSED ancestor declare a member this field's name would collide with? A field is arity 0, so the far side can only be a PARAMETERLESS method. javaLangObjectDeclares is asked first:
      * that type is above every java type whether or not the parent list shows it.
      */
    ...
    def scan(cd: Tree.ClassDef): Unit =
      ...
      cd.parents.flatMap { case tt: TypeTree => headSym(tt.tpe); case t: Term => headSym(t.tpe) }.flatMap(declOf.get).foreach(scan)
      ...
      cd.body.foreach {
        case v: Tree.ValDef
            if (shadowed(nm(v.symbol)) || externallyShadowed(cd, nm(v.symbol))) &&
    ...
    renames.keys.foreach(id => catalog.cite(JS.C(4), p.symbolOf(id).map(_.fullName).getOrElse(id.toString)))
    if renames.isEmpty then p
    else
      ...
      val syms = p.symbols.all.map(s => renames.get(s.id).map(n => s.copy(name = n, flags = s.flags.copy(isPrivate = false, isProtected = false))).getOrElse(s))
      p.rebuilt(symbols = SymbolTable(syms))

  /** WHAT THE BASE SAYS ABOUT A FIELD THIS RUN WOULD RENAME — three answers (see [[baseName]]). */
  private[emit] enum BaseName:
    /** this run EMITS the field, or no base publishes a row for it: the local derivation stands, recorded as a gap first in the second case.
      */
    case Derive
    ...
    /** the base published a row and KEPT java's name. This run may not move the field — the clash it saw is ITS OWN, made by declarations only this run has.
      */
    case Kept
    ...
    if view.owns(field) then BaseName.Derive
      ...
      view.memberShape(field) match
        ...
        case Surface.Answer.Unknown(why, module) =>
          view.gap(
            Surface.Gap(
              p.symbolOf(field).map(_.fullName).getOrElse("?"),
  ...
  def resolveCapturedLocalClashes(p: Program, out: collection.mutable.Buffer[Decision] = collection.mutable.ListBuffer.empty): Program =
    ...
    /** every nested BODY, its own type symbol and its parents. A StandardTraversal, not a walk over cd.body: an anonymous class's body lives inside a TERM.
      */
    val bodies  = collection.mutable.ListBuffer[(SymId, List[Statement], List[SymId])]()
    val methods = collection.mutable.Set[SymId]()
    val declOf  = collection.mutable.Map[SymId, Tree.ClassDef]()

    /** body symbol -> the METHODS it is lexically inside. Each method re-walks its OWN body once and claims what it finds (the traversal is post-order, so a push/pop stack cannot see it).
      */
    ...
    def discoverScope(t: Term)(using p0: Program): Unit =
      ...
      if localOwners.nonEmpty && innerBodies.nonEmpty then
        ...
        innerBodies.foreach(b => localOwners.foreach(m => enclosedBy(b) = enclosedBy.getOrElse(b, Set.empty) + m))
    val collector = new Phase:
      ...
      override def transformClassDef(t: Tree.ClassDef)(using Program): Tree.ClassDef =
        declOf(t.symbol) = t
        bodies += ((t.symbol, t.body, t.parents.flatMap { case tt: TypeTree => headSym(tt.tpe); case tm: Term => headSym(tm.tpe) }))
        t
    ...
    /** POST-PASS: bodies with no enclosedBy entry — anonymous classes inside a scope that is neither DefDef nor Lambda. Deliberately a post-pass rather than a transformBlock hook, which would re-walk
      * every block in the program instead of only the ones that need it.
      */
    ...
    /** the definitions and the references a body holds, at any depth. Both come from one walk of the SAME traversal, so "declared inside" and "referenced inside" can never disagree about what
      * "inside" means.
      */
    ...
    /** the member SYMBOLS a body INHERITS — symbols, not names, because the ambiguity question needs to ask whether the body referenced the INHERITED member (a self-declared member is never
      * ambiguous). Falls back to the SYMBOL TABLE for a RESOLUTION-ROOT parent with no indexed ClassDef — a level-1 read, unable to walk further ancestors, but covering every rename a surface phase
      * made on the parent's own declarations.
      */
        ...
        .flatMap { pid =>
          declOf.get(pid) match
            case Some(cd) =>
              ...
              } ++ visibleMembers(cd.parents.flatMap { case tt: TypeTree => headSym(tt.tpe); case tm: Term => headSym(tm.tpe) }, seen + cd.symbol)
            case None =>
    ...
    bodies.foreach { (bodySym, stats, parents) =>
      ...
      if shadowing.nonEmpty then
        ...
        def move(s: SymId, clash: String, why: String): Unit =
          ...
          note(out, Decision.Kind.RenamedMember, p, owner, Map("from" -> n, "to" -> fresh, "clash" -> clash, "why" -> why), MemberRenameRule)
        ...
        (refs -- defs).toList.sortBy(_.raw).foreach { s =>
          if shadowing(renames.getOrElse(s, nm(s))) && p.symbolOf(s).exists(sy => methods(sy.owner)) then
            move(
              s,
              "captured-local-vs-nested-member",
        ...
        if ambiguous.nonEmpty then
          enclosedBy.getOrElse(bodySym, Set.empty).toList.sortBy(_.raw).flatMap(m => scopeOf.getOrElse(m, Nil)).sortBy(_.id.raw).foreach { s =>
            if ambiguous(renames.getOrElse(s.id, s.name)) && !defs(s.id) then
              move(
                s.id,
                "captured-local-vs-inherited-member",
  ...
  /** Rename any field whose simple name collides with a method in the same EMITTED SCOPE (legal in java, illegal in scala) by suffixing `$field`. "Same emitted scope" is PLACEMENT, not name — a java
    * `static` member leaves for the companion, so an instance field of the same name cannot collide with it. INSTANCE scope is inherited (a descendant's method still clashes); STATIC scope is not. A
    * `module` symbol has one body for both, collapsing the partition.
    */
  def resolveMemberClashes(
    p:       Program,
    out:     collection.mutable.Buffer[Decision] = collection.mutable.ListBuffer.empty,
    surface: Surface = null,
    ...
    /** METHOD renames, kept apart from [[renames]]: the field map also drives the visibility strip below (a renamed field needs it), and widening a renamed method's visibility would move emitted
      * surface for no cause.
      */
    ...
    lazy val graph = OverrideGraph.build(p, baseUnits = p.units.map(_.symbol).toSet -- view.ownedUnits.map(_.symbol).toSet)
    ...
    def scan(cd: Tree.ClassDef): Unit =
      ...
      def moveOwnMethods(v: Tree.ValDef): Unit =
        val n        = nm(v.symbol)
        val mine     = selfOrDescClasses(cd.symbol).flatMap(c => instMethodSyms.getOrElse(c, Nil)).filter(m => nm(m) == n && view.owns(m))
          ...
          if !methodRenames.contains(m) then
            ...
            c.anchorReason(p) match
              case Some(why) =>
                view.gap(
                  Surface.Gap(
                    p.symbolOf(m).map(_.fullName).getOrElse("?"),
    ...
    (renames.keys ++ methodRenames.keys).foreach(id => catalog.cite(JS.C(46), p.symbolOf(id).map(_.fullName).getOrElse(id.toString)))
    if renames.isEmpty && methodRenames.isEmpty then p
      ...
      val syms = p.symbols.all.map { s =>
        renames
          .get(s.id)
          .map(n => s.copy(name = n, flags = s.flags.copy(isPrivate = false, isProtected = false)))
