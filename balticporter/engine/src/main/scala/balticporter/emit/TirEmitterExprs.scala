...
private[emit] trait TirEmitterExprs:
  ...
  // ---- terms ----
  /** true when an `Ident`'s symbol is actually a TYPE used as a value (a static-access receiver like `Float.compare`) — those must render as the (qualified) type name.
    */
  ...
  /** a type used as a VALUE (static-access receiver) — dotted path, never a `#` projection (which is type-position-only syntax).
    */
  ...
  /** Does a bare reference to this INSTANCE member collide with a static of the same name the enclosing companion carries or re-exports? Java has one namespace for statics and instance fields (the
    * inherited instance field simply wins); scala reports the bare name as ambiguous. `this.` says what java meant. Decided from the TIR symbol, not the frontend (which cannot see it under
    * noClasspath).
    */
  ...
  private[emit] def termArm(t: Term, i: Int)(using Obligations): String = t match
    ...
    case a @ Tree.Apply(fun, args, _, _, _) =>
      ...
      locally {
        ...
        val risks = balticporter.tir.OverloadRiskCheck.risks(a, overloads, classStack.lastOption.getOrElse(balticporter.tir.SymId.None))(using program)
        Obligations.consult(JS.C(22), a.origin)(Option.when(risks.exists(_.issue != balticporter.tir.OverloadRiskCheck.Issue.GenericTieBreak))(()))
        Obligations.consult(JS.C(23), a.origin)(Option.when(risks.exists(_.issue == balticporter.tir.OverloadRiskCheck.Issue.GenericTieBreak))(()))
      }
      ...
      Obligations.consult(JS.G(39), a.origin)(Option.when(args.exists(_.isInstanceOf[Tree.Repeated]))(()))
      applyStr(fun, args, i)
    ...
    case ty @ Tree.Typed(e, tpt, _, _) =>
      ...
      Obligations.consult(JS.E(6), ty.origin)(CastConversionCheck.crossTypeUnbox(ty)(using program).map(_ => ()))
    ...
    case Tree.ForEach(b, it, body, _, _, lbl) =>
      ...
      (widenedBinding(b, it), mutable, keptJdk, hasReturn) match
        ...
        case (widened, _, _, _) =>
          ...
          loopWithJumps(body, lbl, bd => s"for ($fresh <- ${term(it, i)}) { $kw $name: $decl = $rhs; $bd }", term(body, i))
    case Tree.For(init, cond, upd, body, _, _, lbl) =>
      ...
      val emptyBody = Tree.uncomment(body) match
        ...
        case _                                                            => false
      loopWithJumps(body, lbl, bd => if emptyBody then s"{ $is; while ($c) { $u } }" else s"{ $is; while ($c) { $bd; $u } }", term(body, i))
    ...
    case mr @ Tree.MethodRef(q, s, mrT, _, referent) =>
      ...
      Obligations.consult(JS.G(17), mr.origin)(
        Option.when(
          isCtor && (q match
            case Left(tt) =>
              tt.tpe match
      ...
      q match
        ...
        case Left(tt) if isCtor =>
          tt.tpe match
            ...
            case _ =>
              ...
              samAscribed(s"((${ps.mkString(", ")}) => new ${ctorTpe(tt.tpe)}(${ps.mkString(", ")}))", mrT, tt.tpe)
        ...
        case Left(tt) if isStaticRef =>
          ...
          val ps      = (0 until arity).map(k => if named then s"a$k$$: ${tpe(formals(k))}" else s"a$k$$").mkString(", ")
        ...
        case Left(tt) =>
          ...
          val extra = (0 until arity).toList.map(k => if named then s"a$k$$: ${tpe(formals(k))}" else s"a$k$$")
    ...
    case b @ Tree.Break(scala.None, _, _) =>
      unrenderable(
        "break",
        "no enclosing loop or switch, and the frontend did not recognise it as " +
          "a switch-case terminator",
        "give the enclosing construct a `boundary`, or teach the " +
          "frontend this jump's shape",
        b.origin,
        "/* break: no enclosing loop or switch */ ()"
      )
    ...
    case y @ Tree.Yield(v, _, _) =>
      unrenderable(
        "yield",
        "no enclosing switch EXPRESSION arm — a `yield` outside one is not a " +
        ...
        y.origin,
        s"/* yield: no enclosing switch expression */ ${term(v, i)}"
      )
    ...
    case Tree.IncDec(tgt, op, post, _, _) =>
      ...
      else if post then s"{ val ${'$'}prev = ${term(tgt, i)}; ${term(tgt, i)} $op= 1; ${'$'}prev }"
  ...
  /** Render a marker. `Open` has two answers: best-effort (the approximation inside deterministic comment fences, since a comment cannot change program shape) or, by default,
    * `scala.compiletime.error` — the loudest available answer, deliberately opposite `unrenderable`'s default, since here the engine has nothing to say at all. Either way the refusal is recorded as
    * `Decision.Kind.Unrenderable`.
    */
    ...
    m.state match
      ...
      case MarkerState.Open           =>
        ...
        if bestEffort then
          ...
          val msg = PorterNote.safe(
            s"balticporter: ${m.kind.label}: ${m.what}; " +
  ...
  /** A Java constructor reference (`Foo::new`) is typed by the TARGET functional interface java resolved, not by `Foo`. Emitted bare, `() => new Foo()` is a `Function0`, which SAM-converts to ANY
  ...
  /** conservatively: can evaluating this term have an effect? Only shapes that provably cannot are treated as free, because being wrong in the other direction DROPS an effect.
    */
  ...
  /** Bind the non-trivial subexpressions of an assignable lvalue to temporaries, returning (list-of-val-bindings, bound-lvalue-string).
    *
    * For `arr(f())`: `(List("val $lv1 = f()"), "arr($lv1)")` For `g().field`: `(List("val $lv1 = g()"), "$lv1.field")`
    */
  ...
  /** A `Tree.Repeated` in an ARGUMENT position is the argument list's TAIL, not one argument — decisive at ZERO elements, where a node rendering `""` would leave `f(a, )` instead of `f(a)` (java's
    * f(a) against f(A, B...), e.g. Paths.get(".")). Flattened HERE, a fact about the position: the same node elsewhere still stands for a sequence of its own.
    */
  ...
  /** does this qualifier NAME a type this emitter renders as a scala 3 `enum`? A TYPE and never a value: a `Select` is admitted beside an `Ident` for a NESTED enum's `Outer.Inner`, and the answer
    * comes from `EnumShape`, so a value of enum type (`l.values()`) cannot be mistaken — its symbol is a local, not a class.
    */
  ...
  /** widening rank — a value of rank r converts implicitly to any numeric type of higher rank. `Char` and `Short` share a rank because neither widens to the other.
    */
  ...
  /** what the RECEIVER's static type says an ancestor's type parameters are — [[ParentSubst.of]] composed with the receiver's OWN application, collapsing `Bar.T` to `Int` for `Foo[Int] <: Bar[X]` in
    * one map. Empty for a receiver this program does not declare, or a raw one — the caller's own nameability test then declines.
    */
  ...
  /** A Java anonymous class's body → Scala's anonymous-class expression `new Base(args) { … }`. The symbol is pushed on `classStack` while members render, so `thisRef` qualifies an enclosing
    * reference as `Outer.this.m`. Captured locals need no lowering — scala closes over them where javac synthesised ctor parameters. `Some(Nil)` still renders braces: `new Base()` and `new Base(){}`
    * are DIFFERENT types.
    */
  private[emit] def anonBody(anon: Option[Tree.AnonClass], i: Int): String = anon match
    ...
    case Some(a) =>
      ...
      val members = try a.body.map(stat(_, i + 1)).filter(_.trim.nonEmpty)
      finally classStack.removeLast()
  ...
  /** the same question for a term spliced into a [[Tree.Opaque]] HOLE, which is harder: a hole's neighbours are whatever a policy entry wrote around `{recv}`, so this over-approximates by three more
    * node kinds than [[operand]] — a redundant parens pair costs two characters. NOT folded into `operand`, which is on every emitted expression's path in every port.
    */
  ...
  /** join block statements, terminating one with `;` when the NEXT begins with `{` — otherwise Scala greedily reads `new T(a)\n{ … }` as an anonymous-class body rather than two statements.
    */
  ...
  /** The first character a PARSER would see in `s` — comments skipped, exactly as scala's scanner skips them. A bare `l.trim.startsWith("{")` test misses a preceding comment, which is whitespace to
    * the parser (measured: 2 "anonymous class cannot extend final class" errors).
    */
  ...
  /** A java `try`, plus the arm that keeps a translated JUMP out of its handlers. Java's break/continue is not an exception; scala's translation IS one (`boundary.Break extends RuntimeException`), so
    * a broad catch would silently swallow it — always the exception form (dotty's `DropBreaks.prepareForTry`). Repair: a re-throw arm ahead of the java arms, only where a jump really CROSSES the
    * catch (`crossesCatch`). `finally` is untouched.
    */
  ...
  /** JLS 14.20.3.1's lowering of a try-with-resources, emitted INLINE — not `Using`/a lambda (this emitter's `return`/`break`/`continue` bind to labels outside the try). Reproduces java's contract:
    * reverse declaration order, every `close()` attempted after an earlier throws, suppression on the body's own exception, closed on ANY completion including a jump (a jump takes the `Break` arm
    * ahead of the recorder). Numbered per nesting level.
    */
  ...
  /** one counter for every resource block this emitter opens — see [[resourceStr]] for why the primary/thrown/suppressed binders may not repeat across a nesting.
    */
  private[emit] var resourceSeq = 0

  /** every `try` whose RESOURCES this emitter lowered — input to `try-resource`, which finds the resource-carrying trys independently and reports the ones nothing closed. Keyed by [[Tree.Try.id]]: an
    * Origin is not unique across trys, nor is object identity after a rebuild.
    */
  ...
  /** does a jump in this try BODY leave the try — i.e. is its `boundary` outside it? Read off the emitter's own boundary state: a jump rendering as `boundary.break` has its target in scope HERE,
    * opened by a construct enclosing this try. A label bound INSIDE the body is not in these maps yet, so the labelled lanes need no extra test to exclude it.
    */
  ...
  /** every `try` this emitter put a [[TirEmitter.BreakGuard]] arm on — input to `break-catch`, which finds the crossings independently and reports the ones nothing guarded. Keyed by the try's own
    * TOKEN, never by Origin (two trys can share path/line/column, e.g. every phase-synthesised one) and never by object identity (StandardTraversal rebuilds every node; Tree.Try.id survives a rebuild
    * because copy carries it).
    */
  ...
  /** A java `switch`, with a boundary around any case body that still contains an unlabelled `break` (mid-case fallthrough — the frontend strips only the CASE-TERMINATING one). Also emits
    * `case null => throw` ahead of the java arms for a REFERENCE selector java NPEs on implicitly (JLS 14.11.2), unless the switch already declares its own `case null` (SE21).
    */
  ...
  /** does java's implicit null check apply to this switch, and has nothing already written one? Both needed: the selector's type is a REFERENCE type (a primitive cannot be null; decided from the
    * emitted type's head against scala's value classes), and no case label is already `null` (SE21's pattern switch may deliberately handle it, JLS 14.11.1).
    */
  ...
  /** every switch this emitter gave a `case null` arm — input to `switch-null`. Keyed by [[Tree.Match.id]], the reason `breakGuarded` is keyed by `Tree.Try.id`.
    */
  ...
  // ---- types ----
  /** a type in `new` position: `new Foo[?]` is illegal (you can't instantiate a wildcard), so when a raw generic type carries wildcard args, drop them and let Scala infer the arguments from the
    * expected type (`new Foo(...)`).
    */
  ...
  /** THE TYPE DISPATCH — the emitter half of the catalog's FOURTH obligation surface. At the dispatch and never in an arm ([[stat]]'s reason): a TypeRepr is not a Tree, so [[Rendering]] could never
    * enter one. The subject is the TypeRepr; the origin is the enclosing scope's (`CatalogLog.currentOrigin`), since a type has no position of its own.
    */
  private[emit] def tpe(t: TypeRepr): String =
    Typing.ofRepr(TirKinds.ofType(t), t)(tpeArm(t))

  /** JS-C29 and JS-G12 — the two questions a NAME is asked at the type dispatch's TypeRef arm, read off the SYMBOL rather than by re-running typeSym's cascade.
    */
  ...
  /** JS-E20 — a PRIMITIVE class literal. `int.class` is statically `Class<Integer>` (JLS 15.8.2) while scala's `classOf[Int]` is `Class[Int]`, which no `Class[T <: Object]` slot accepts. Both facts
    * kept: the runtime object stays the primitive class, the STATIC type becomes java's. `None` at every other constant, which is where the consult does not apply.
    */
  private[emit] def primitiveClassLiteral(c: Constant): Option[String] = c match
    case Constant.ClassOfC(t @ TypeRepr.TypeRef(_, s)) =>
      Descriptor.ValueClassBoxes.get(sym(s).fullName).map(b => s"classOf[${tpe(t)}].asInstanceOf[java.lang.Class[$b]]")
    case _ => scala.None

  /** A PREFIX operator and its operand, with the two kept as two tokens. Scala's lexer takes a maximal run of operator characters as ONE identifier, so a prefix `-` against an operand already
    * rendering with a leading `-` produces `--`, a different token (e.g. java negating a literal whose value is already negative — measured 48 errors in one method). Parenthesising the operand is the
    * only fix that cannot mis-lex; a separating space would misparse as infix.
    */
  ...
  /** the ASCII half of Scala's `opchar` (SLS 1.1); the Unicode `Sm`/`So` half cannot begin any rendering this emitter produces.
    */
  private[emit] def isOpChar(c: Char): Boolean = "!#%&*+-/:<=>?@\\^|~".indexOf(c.toInt) >= 0

  /** Render a string or char literal's VALUE as Scala source that denotes the same value. Every character needs escaping that would otherwise: end the literal (raw `\n`), be an "illegal character" (a
    * raw control char), or silently change on UTF-8 write-out (a lone surrogate). Everything else, including ordinary non-ASCII text, is emitted verbatim. `\uXXXX` is a scala 3 escape sequence
    * expanded inside the literal only, so an emitted `\\u` cannot leak into one.
    */
...
/** THE `Tree` KIND, as the rendering dispatch names it. `Tree` is sealed and every case is a case class, so the name is the compiler's own `productPrefix` — a function, not a table, since a new node
  * kind needs no listing to be covered.
  */
