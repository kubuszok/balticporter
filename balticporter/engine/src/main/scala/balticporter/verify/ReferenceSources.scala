...
final class ReferenceSources(roots: List[Path], typeRenames: Map[String, String], flattenNestedTypes: Set[String]) extends RunScope.ReferenceSourceLookup:
  ...
  val index:       Map[String, List[RunScope.ReferenceMember]] =
    ...
    val files = roots.flatMap { root =>
      ...
      else Files.walk(root).iterator().asScala.filter(p => p.toString.endsWith(".scala") && Files.isRegularFile(p)).toList.sorted
    }
    files.foreach { f =>
      ...
      dialects.Scala3(Input.VirtualFile(f.toString, text)).parse[Source] match
        case Parsed.Success(tree) =>
          ...
          val outer = packageChain(tree).dropRight(1).scanLeft(List.empty[String])(_ :+ _).drop(1).map(segs => s"import ${segs.mkString(".")}.*")
