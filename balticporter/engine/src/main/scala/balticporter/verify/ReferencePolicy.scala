...
object ReferencePolicy:
  ...
  private val PrimitiveFqns = Set("scala.Int", "scala.Float", "scala.Long", "scala.Double", "scala.Short", "scala.Byte", "scala.Boolean", "scala.Char")
  ...
  /** @param emitted
    ...
    *   upstream nested FQNs emitted top-level
    * @param opaqueTargets
    *   target type FQNs of the deriving `OpaqueSpec`s (`OpaqueSpec.typeFqn`)
    */
  /** every reference path a java class may sit at: a static nested type lands in the hand port's class OR its companion (`/Outer/Inner`, `/Outer$/Inner`), so each enclosing segment is tried both
    * ways; a java STATIC member is read under the innermost companion (`/Outer$`).
    */
  ...
  def derive(
    ...
    flattenNestedTypes: Set[String],
    opaqueTargets:      Set[String],
    ...
    def emittedPkg(javaFqn: String): String =
      ...
      packageRenames.toList.sortBy(-_._1.length).find((from, _) => pkg == from || pkg.startsWith(from + ".")).map((from, to) => to + pkg.drop(from.length)).getOrElse(pkg)

    /** candidates at a key, the reference's PACKAGE agreeing where any candidate's does — two `ParticleEffect`s (g2d, g3d.particles) share one path and must not answer for each other.
      */
    ...
    /** a java accessor pair is ONE property in the hand port (the bean step's own convention): a getter `getX()`/`isX()` is also read at `x` (a `def` or a `val`/`var`), a setter `setX(v)` at `x_=` or
      * the `var x` — the property's type is the slot's.
      */
    ...
    /** `GL30Available` -> `gl30Available`, `URLPath` -> `urlPath`: a leading acronym lowered whole, keeping its last letter where the next word begins with one (the hand port's casing).
      */
    ...
    def lookupMethod(paths: List[String], name: String, arity: Int, pkg: String): Option[List[SurfaceDecl]] =
      lookup(paths, "def", name, arity, pkg).orElse(
        propertyRaw(name).flatMap(raw => List(decap(raw), lowerAcronym(raw)).distinct.iterator.map(prop => lookupProp(paths, name, arity, pkg, prop)).collectFirst { case Some(x) => x })
      )
    def lookupProp(paths: List[String], name: String, arity: Int, pkg: String, prop: String): Option[List[SurfaceDecl]] =
      Some(prop).flatMap { prop =>
        if arity == 0 && !name.startsWith("set") then lookup(paths, "def", prop, 0, pkg).orElse(lookup(paths, "prop", prop, 0, pkg))
        else if arity == 1 && name.startsWith("set") then lookup(paths, "def", prop + "_=", 1, pkg).orElse(lookup(paths, "prop", prop, 0, pkg)).orElse(lookup(paths, "def", prop, 1, pkg))
    ...
    /** among several reference candidates at one key, those whose parameter types match java's best (`BitmapFont(data, Array<TextureRegion>, boolean)` against the primary taking a `DynamicArray` and
      ...
      */
    /** is this java parameter typed by a TYPE PARAMETER (of the method or its class)? The parity parser canonicalises those to `$0`, `$1`… on the reference side.
      */
    ...
    /** the key a row is written under: descriptor-qualified where the java NAME is overloaded in its class, so one overload's spelling never reaches another (`DerivedPolicy.keysOf`).
      */
    ...
    program.units.filter(u => emitted(u.symbol)).foreach { unit =>
      StandardTraversal.allClassDefs(unit)(using program).foreach { cd =>
        program.symbolOf(cd.symbol).foreach { cls =>
          ...
          if !declaredHere then findings += finding(LaneUnmatched, "NoReferenceType", cls, s"no reference declaration at `${paths.head}`")
          else
            // which java NAMES are overloaded here: their rows are keyed by descriptor
            val nameCounts = cd.body.collect { case d: Tree.DefDef => program.symbolOf(d.symbol).map(_.name) }.flatten.groupBy(identity).view.mapValues(_.size).toMap
            cd.body.foreach {
              case d: Tree.DefDef =>
                program.symbolOf(d.symbol).filter(s => !s.name.contains('$') && s.name != "<init>").foreach { ms =>
                  ...
                  propertyName(ms.name).foreach { prop =>
                    ...
                    if keptName && !twin then rows += DerivedPolicy.Row(DerivedPolicy.Family.KeepName, rowKey(ms, overloaded), s"def ${ms.name}")
                  }
                  ...
                  propertyName(ms.name).foreach { prop =>
                    ...
                    if asProp && n == 0 && !ms.name.startsWith("set") then rows += DerivedPolicy.Row(DerivedPolicy.Family.Property, rowKey(ms, overloaded), s"var $prop", prop)
                    ...
                    else if asProp && n == 1 && ms.name.startsWith("set") then rows += DerivedPolicy.Row(DerivedPolicy.Family.PropertySetter, rowKey(ms, overloaded), s"var $prop", prop)
                  ...
                  lookupMethod(mp, ms.name, n, pkg).orElse(renamedTo.flatMap(lookupMethod(mp, _, n, pkg))).orElse(asExtension(ms.name)).orElse(renamedTo.flatMap(asExtension)) match
                    ...
                    case None =>
                      ...
                      val classParams = d.paramss.flatten.count(p =>
                        p.tpt.tpe match
                      ...
                      if classParams > 0 then
                        val tagged = (ms.name :: renamedTo.toList).flatMap(nm => lookup(mp, "def", nm, n - classParams, pkg).toList.flatten).filter(rd => rd.typeParams.contains("ClassTag"))
                        if tagged.nonEmpty then rows += DerivedPolicy.Row(DerivedPolicy.Family.ClassTagParam, rowKey(ms, overloaded), s"def ${tagged.head.name}${tagged.head.typeParams}")
                }
                    ...
                    case None        =>
                      ...
                      lookup(fmp, "prop", "_" + fs.name, 0, pkg).foreach { cands =>
                        ...
                        agree(
                          ...
                          cands,
                          r =>
                            fieldRows(v, fs, r).map(_.copy(upstream = fieldKey)) :+
    ...
    val rowFindings = all.map(r => CheckReport.Finding(LaneRows, r.family.toString, r.upstream, "", 0, if r.target.isEmpty then r.reference else s"${r.reference} -> ${r.target}"))
