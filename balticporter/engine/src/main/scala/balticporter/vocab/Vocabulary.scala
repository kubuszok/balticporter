...
/** Diffable data tables mapping Java symbols onto Scala counterparts.
  *
  * Entry kinds: `type A -> B` (rewrites all occurrences), `method O#m -> n` (renames calls matched against the original owner), `getter O#m -> n` (nilary call to parenless member).
  */
final case class Vocabulary(
  ...
  def digestInput: String =
    (typeMap.toList.sorted.map((a, b) => s"type $a -> $b") ++
      ...
      getterMap.toList.sortBy(_._1).map { case ((o, m), n) => s"getter $o#$m -> $n" }).mkString("\n")
