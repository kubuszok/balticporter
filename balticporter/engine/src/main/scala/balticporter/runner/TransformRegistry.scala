...
final class TransformRegistry(val factories: List[TransformFactory]):
  ...
  /** every REMEDY this classpath declares, whether or not a port enables the declaring phase — see [[TransformFactory.remedies]]. Assembled with the same refusal `byName` above uses: a remedy id is
    * published API, so two remedies claiming one id is a run that cannot say which mechanism a port asked for.
    */
  lazy val remedies: balticporter.tir.RemedyVocabulary =
    factories.foldLeft(balticporter.tir.RemedyVocabulary.empty)((acc, f) => acc ++ balticporter.tir.RemedyVocabulary.declared(f.getClass.getName, f.remedies))
