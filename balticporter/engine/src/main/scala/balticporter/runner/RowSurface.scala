...
/** The surface code outside a type can reach, read off EMITTED Scala: every declaration that is not plain `private`, keyed by where it is declared (`T#m` in the class, `T.m` in the companion), rendered
  * without parameter names or bodies. Two translations of one type compile against the same shared code only where these agree; package-private members count, because shared code in the package
  * reaches them.
  */
object RowSurface:
  ...
  private def collect(tree: Tree): Map[String, List[String]] =
    ...
    def template(kind: String, mods: List[Mod], name: String, tparams: String, ctor: Option[Ctor.Primary], templ: Template, owner: String, sep: String): Unit =
      if !hidden(mods) then
        ...
        add(self, s"${modsOf(mods)}$kind $name$tparams$params" + (if parents.isEmpty then "" else parents.mkString(" extends ", " with ", "")))
        ctor.foreach(_.paramClauses.flatMap(_.values).foreach { p =>
    ...
    def walk(t: Tree, owner: String, sep: String): Unit = t match
      ...
      case d: Defn.Def              =>
        if !hidden(d.mods) then add(join(owner, sep, d.name.value), s"${modsOf(d.mods)}def ${d.name.value}${groups(d.paramClauseGroups)}: ${d.decltpe.fold("?")(_.syntax)}")
      case d: Decl.Def =>
        if !hidden(d.mods) then add(join(owner, sep, d.name.value), s"${modsOf(d.mods)}def ${d.name.value}${groups(d.paramClauseGroups)}: ${d.decltpe.syntax}")
