...
object ArtifactIndex:
  ...
  /** Full `cs` invocation; also the cache fingerprint. `--intransitive` must come AFTER `-r` flags (`cs` reads everything after it as a module).
    */
  ...
  /** Classes named by a jar entry as `Symbol.fullName`, plus enclosing prefixes (cut at `$`). Skips `META-INF/`, `module-info`, `package-info`. Pure function for testability.
    */
  ...
  def provides(d: ArtifactDep, cacheDir: Option[Path], scalaBinary: String = "3", resolve: List[String] => Either[String, List[Path]] = fetch): DependencyCheck.Provides =
      ...
      case scala.None =>
        resolve(cmd) match
          ...
          case Right(jars) =>
            ...
            catch case NonFatal(e) => DependencyCheck.Provides.Unverifiable(s"resolved ${jars.size} jar(s) and could not read them: ${e.getClass.getSimpleName}: ${e.getMessage}")
  ...
  /** Run `cs fetch --intransitive`, filter to existing `.jar` paths. Lines are paths (may contain spaces); progress lines filtered by existence check.
    */
  def fetch(cmd: List[String]): Either[String, List[Path]] =
    try
      ...
      if proc.waitFor() != 0 || jars.isEmpty then
        Left(
          s"could not resolve `${cmd.mkString(" ")}` (is `cs` installed, and is this machine " +
  ...
  /** Memoised supplier for `DependencyCheck.declarations`. Keyed on full command string (includes cross kind and resolver) so two different coordinates never share a result.
    */
  def supplier(cacheDir: Option[Path], scalaBinary: String = "3", resolve: List[String] => Either[String, List[Path]] = fetch): ArtifactDep => DependencyCheck.Provides =
    val memo = scala.collection.mutable.Map.empty[String, DependencyCheck.Provides]
    d => memo.getOrElseUpdate(command(d, scalaBinary).mkString(" "), provides(d, cacheDir, scalaBinary, resolve))
