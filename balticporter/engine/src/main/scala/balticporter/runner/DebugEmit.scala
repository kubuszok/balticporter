...
/** Model a Java source tree once and print one type as TIR / emitted Scala, optionally around named phases. Does NOT take a `.conf` — it is pipeline inspection, not port assembly.
  *
  * Flags: `--root` (required), `--fqn`, `--phases a,b`, `--dump-before`/`--dump-after`, `--scala`, `--canonical`, `--include`, `--fast`, `--classpath`, `--lenient`.
  */
object DebugEmit:
          ...
          case scala.None =>
            try Right(done :+ registry.phase(n, empty, s"--phases $n"))
              ...
              case e: ConfigError if registry.get(n).isDefined =>
                Left(
                  s"[debug-emit] '$n' takes POLICY and this tool reads no port configuration — " +
  ...
  private def opts(args: List[String]): Map[String, List[String]] =
    args
      .foldLeft((Map.empty[String, List[String]], Option.empty[String])) {
  ...
  private def run(
    ...
    println(
      s"[debug-emit] modelling ${files.size} file(s) under $root" +
    ...
    println(
      s"[debug-emit] TIR: ${program.units.size} units, ${program.symbols.all.size} symbols" +
