...
/** [[ConfigView]] over HOCON. Every read is recorded; [[unread]] reports keys nobody accessed (catches typos that HOCON silently tolerates). Absent key = `None`; wrong shape = throws. Containment is
  * a tree walk, not a dot-split (keys may contain dots).
  */
final class HoconView private (val path: String, obj: ConfigObject) extends ConfigView:
  ...
  /** Every key in this subtree that no accessor asked for, deepest paths included, sorted. A key in `readKeys` that this object does not have is harmless and not reported.
    */
...
object HoconView:
  ...
  def parse(file: java.nio.file.Path): Config =
    ...
    com.typesafe.config.ConfigFactory.parseFile(file.toFile).resolveWith(com.typesafe.config.ConfigFactory.systemProperties)
