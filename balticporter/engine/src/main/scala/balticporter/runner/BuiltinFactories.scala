...
object BuiltinFactories:
  ...
  def all: List[TransformFactory] = List(
    new CollectionsFactory,
    new MutableParamsFactory,
    new PanamaFfiFactory,
    new TestFrameworkFactory,
    new StaticForwarderFactory,
    new ClassTableFactory,
    new TypeRedirectFactory,
    new MemberRenameFactory,
    new MethodBodyFactory,
    new AddMembersFactory,
    new CallSiteSubstitutionFactory,
    new PortMapMigrationFactory,
    new PrimitiveToOpaqueFactory,
    new GlobalsToImplicitsFactory,
    new BeanPropertyFactory,
    new NullabilityFactory,
    new PublicFieldAccessorFactory,
    new RemediationFactory,
  ...
  def fromConfig(config: ConfigView): Phase =
    ...
    def parseRewrite(tbl: ConfigView, memberKey: String, mName: String): RetargetRewrite =
      if tbl.isObject(memberKey) then
        ...
        if c.int("boolDispatch").isDefined then RetargetRewrite.BoolDispatch(c.int("boolDispatch").get, c.requireString("onTrue"), c.requireString("onFalse"))
        ...
        else if c.string("forEach").isDefined then RetargetRewrite.ForEach(c.requireString("forEach"), c.int("arity").getOrElse(1))
        else if c.string("collect").isDefined then RetargetRewrite.Collect(c.requireString("collect"), c.string("into").getOrElse("lowlevel.util.DynamicArray"))
        else if c.strings("chain").isDefined then
          val members = c.strings("chain").get
          RetargetRewrite.Chain(members, parens = c.strings("parens").getOrElse(Nil).toSet, dropArgs = c.bool("dropArgs").getOrElse(false))
        else if c.string("fieldWrite").isDefined then RetargetRewrite.FieldWrite(mName, c.requireString("fieldWrite"))
        else if c.string("indexedField").isDefined then RetargetRewrite.IndexedField(c.requireString("indexedField"))
        else if c.string("template").isDefined then RetargetRewrite.Template(c.requireString("template"))
        else
          throw ConfigError(
            tbl.at(memberKey),
            "object entry must have 'boolDispatch', 'companion', 'forEach', 'collect', 'chain', 'fieldWrite', 'indexedField', or 'template'"
          )
      else RetargetRewrite.Rename(tbl.requireString(memberKey))
    ...
    config.child("retargetRewrites").foreach { rr =>
      rr.keys.foreach { srcFqn =>
        ...
        tbl.keys.foreach { memberKey =>
          ...
          if parts.length != 2 then throw ConfigError(rr.at(srcFqn), s"retargetRewrites key '$memberKey' must be 'memberName/arity' or 'memberName/(descriptor)'")
          val mName       = parts(0)
          ...
          if arityOrDesc.startsWith("(") && arityOrDesc.endsWith(")") then
            ...
            val arity = try arityOrDesc.toInt
            catch case _: NumberFormatException => throw ConfigError(rr.at(srcFqn), s"retargetRewrites key '$memberKey': arity part must be an integer or (descriptor)")
    ...
    val typeArgs = config
      .child("retargetTypeArgs")
      .map { ta =>
        import CollectionsTransform.RetargetArg
        ta.keys.map { srcFqn =>
          val argList = ta.strings(srcFqn).getOrElse(throw ConfigError(ta.at(srcFqn), "expected a list of arg mappings, e.g. [\"arg(0)\", \"scala.Int\"]"))
          srcFqn -> argList.map { v =>
            if v.startsWith("arg(") && v.endsWith(")") then RetargetArg.SourceArg(v.drop(4).dropRight(1).toInt)
            else RetargetArg.FixedType(v)
          }
        }.toMap
      }
      .getOrElse(Map.empty)
...
final class PublicFieldAccessorFactory extends TransformFactory:
  ...
  def fromConfig(config: ConfigView): Phase =
    new PublicFieldAccessorTransform(TransformFactory.scopeOf(config, default = RuleScope.Only(Set.empty)))
          ...
          StaticForwarderTransform.Forwarder(
            ...
            members = f
              ...
              .getOrElse(
                throw ConfigError(
                  f.at("members"),
                  "required, and absent — a forwarder with no members " +
                    "forwards nothing, which is a policy entry that can only ever be a mistake"
                )
              )
              .toSet
          )
...
/** `{ transform = "remediation", classTables { "a.B#forName" = "c.D#classFor" } }`
  *
  * Portability menu. Location list is `manifest.resolutions`; `classTables` is the destination table for `class-table`. `remedies` restates the menu for load-time validation.
  */
      ...
      .flatMap(rs =>
        rs.keys.map { k =>
          if !rs.isObject(k) then (k, rs.requireString(k), Map.empty[String, String], RuleScope.everywhere)
            ...
            (k, e.requireString("to"), e.stringMap("memberRenames").getOrElse(Map.empty), TransformFactory.scopeOf(e))
        }
      )
...
/** `.conf` shape for `bean-properties`: `pairs` maps a member key to either an accessor-pair string or an object adding `target` (def-pair | var | val). Empty `pairs` is a no-op.
  */
      ...
      .flatMap(ps =>
        ps.keys.map { k =>
          if !ps.isObject(k) then (k, ps.requireString(k), Target.DefPair)
            ...
            (k, e.requireString("accessors"), e.enumerated("target", Target.byConfigName).getOrElse(Target.DefPair))
        }
      )
...
final class AddMembersFactory extends TransformFactory:
  ...
  def fromConfig(config: ConfigView): Phase =
    ...
    val fromRef = config.stringMap("fromReference").getOrElse(Map.empty).map((o, v) => o -> v.split(',').map(_.trim).filter(_.nonEmpty).toList).filter(_._2.nonEmpty)
    new AddMembersTransform(entries.toMap, fromRef)

/** `{ transform = "class-tag-params", members = ["a.B#m", "a.B#n(Class,int)"], derive = true }`
  *
  * A method's `Class<T>` parameter becomes a `ClassTag[T]` context clause; `derive` takes the reference port's `[T: ClassTag]` members.
  */
...
/** `.conf` shape for `nullability`: `annotations`, `target` (union|named|option), `wrapper` (required iff `target = "named"`, refused otherwise), `scope`. Empty `annotations` = no-op.
  */
final class NullabilityFactory extends TransformFactory:
  ...
  def fromConfig(config: ConfigView): Phase =
    ...
    val targetKey = config.enumerated("target", Map("union" -> "union", "named" -> "named", "wrapper" -> "named", "option" -> "option")).getOrElse("union")
...
/** `.conf` shape for `globals-to-implicits`: `holders` (each an entry with `holder`, `context` — `inject`/`mint` — `members`, `attach`/`reader`/`boundary`, `sites`, `selfSupplied`, `retain`, `cache`,
  * `promoteToClass`, `scope`). No `context` block = an extension (dependent's per-declaration keys only). `holders` is required; absent is refused.
  */
final class GlobalsToImplicitsFactory extends TransformFactory:
  ...
  private def extension(c: ConfigView): ContextHolderExtension =
    ContextHolderExtension(
      ...
      capture = c.stringMap("capture").getOrElse(Map.empty)
    )

  private def sites(c: ConfigView): Map[String, ContextSite] =
    c.stringMap("sites")
      .getOrElse(Map.empty)
      .map((k, v) => k -> ContextSite.fromToken(v).getOrElse(throw ConfigError(c.at("sites"), s"'$v' is not one of ${ContextSite.values.map(_.token).sorted.mkString(", ")}")))
  ...
  private def holder(c: ConfigView): ContextHolder =
    ...
    ContextHolder(
      ...
      attach = c.enumerated("attach", ContextAttach.values.map(v => v.token -> v).toMap).getOrElse(ContextAttach.Method),
      reader = c.enumerated("reader", ContextReader.values.map(v => v.token -> v).toMap).getOrElse(ContextReader.Summon),
      boundary = c.enumerated("boundary", ContextBoundary.values.map(v => v.token -> v).toMap).getOrElse(ContextBoundary.Refuse),
      sites = sites(c),
      selfSupplied = c.stringMap("selfSupplied").getOrElse(Map.empty),
      retain = c.stringMap("retain").getOrElse(Map.empty),
      cache = c.stringMap("cache").getOrElse(Map.empty),
...
final class RegistryFactory extends TransformFactory:
  ...
  def fromConfig(config: ConfigView): Phase =
    val entries = config.children("entries").getOrElse(Nil).map { e =>
      ...
      val sp        = RegistryTransform.Spelling(table = p.requireString("table"), register = p.requireString("register"), create = p.requireString("create"))
