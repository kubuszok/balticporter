...
/** Run a port from a `.conf` — the whole engine, from a command line, with no Scala to write:
  ...
  * separable from argument parsing, so a program wanting its own `main` calls the loader directly.
  * `CheckReport.dir` derives from the MAIN CLASS's simple name, so every port run through this
  * entry point reports into `port-report/PortConfigMain` — correct for one port and wrong for two,
  * which is why the corpus keeps a per-port `main`. */
