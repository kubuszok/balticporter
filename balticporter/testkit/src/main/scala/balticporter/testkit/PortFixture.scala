...
/** @param sources
  *   the java the fixture parsed, `fileName -> code`. Handed to the emitter as its `javaSource`, since an in-memory snippet's `Origin.javaPath` names a file that does not exist.
  * @param decisions
  *   everything the phases RECORDED while they ran (`Pipeline.runTraced`). Carried rather than re-derived, since a decision is a value one run owns.
  */
final case class Ported(
  before:      Program,
  after:       Program,
  phases:      List[Phase],
  ...
  /** what each phase MOVED, observed by the pipeline (`balticporter.tir.Rewrite`) -- taken while the pipeline holds the symbol table on BOTH sides of a phase, so it cannot be re-derived from
    * `before`/`after` alone once more than one phase ran.
    */
  idioms: IdiomLog = IdiomLog.discarding,
  /** what every `IdiomPhase` CONSIDERED, drained by the pipeline as it went (`balticporter.tir.IdiomLog`) -- the only surface a spec has for an idiom transform's REFUSALS, its whole safety argument.
    */
  binder: PolicyBinder = new PolicyBinder(new Program(Nil, SymbolTable(Nil), Xref.build(Nil), MemberIndex.empty), MemberIndex.empty)
):

  /** what the phases that ran require of `balticporter-runtime`. Derived, not passed: the fixture is a miniature of the orchestrator, so a test exercises the same derivation a real port does rather
    * than a hand-assembled approximation of it.
    */
  lazy val plan: RuntimePlan = RuntimePlan.of(phases, runtimeMode)

  private def emitterWith(preview: Boolean = false, bestEffort: Boolean = false, log: CatalogLog = CatalogLog.discarding): TirEmitter =
    new TirEmitter(
      after,
  ...
  /** the SHIPPED emitter, and the only one holding the fixture's obligation log — the preview and best-effort twins re-render the same tree, and a shared log would count every consult twice (the same
    * reason they do not share a source map).
    */
...
object PortFixture:
  ...
  /** parse `java`, run `phases` over it, and hand back both programs plus the emitted Scala. With no phases this is the emitter's own identity fixture — useful, and the honest baseline for "did my
    * phase change anything".
    */
  def port(java: String, phases: Phase*): Ported =
    portIn(RuntimeMode.Dependency, java, phases*)

  /** ...with the port having SELECTED a remedy at one or more locations (`balticporter.core.PortManifest.resolutions`). The vocabulary is derived from the phases handed in, so a fixture cannot select
    * a remedy nothing offers. A spec wanting the two to DIFFER builds the `ResolutionPlan` directly.
    */
  ...
  private def portIn(mode: RuntimeMode, java: String, resolutions: Map[String, String], phases: List[Phase]): Ported =
    ...
    Ported(before, after, phases, Map("Snippet.java" -> java), log.all, mode, catalog, rewrites, idioms, binder)

  /** the same over SEVERAL compilation units, each `fileName -> code`. A Java file declares exactly one package, so every rule about a PACKAGE BOUNDARY — default access, `protected`, an override that
    * crosses one — is untestable from a single snippet.
    */
  ...
  /** …several units WITH selections, which is the shape a menu's own precondition needs.
    *
    * A `Remediator` chokepoint grade is decided by whether ANOTHER UNIT references the type, so the difference between its two answers cannot be written in one snippet at all — the same reason
    * [[portAll]] exists for a package boundary.
    */
  def portAllResolving(sources: List[(String, String)], resolutions: Map[String, String], phases: Phase*): Ported =
  ...
  private def portAllIn(mode: RuntimeMode, sources: List[(String, String)], resolutions: Map[String, String], phases: List[Phase]): Ported =
    val catalog    = new CatalogLog(fatal = true)
    val rewrites   = new RewriteLog
    val idioms     = new IdiomLog
    ...
    binder.resolving(ResolutionPlan.of(resolutions, vocabulary, vocabulary.byId.keySet, binder))
    val (after, log) = Pipeline.runTraced(before, phases, binder, catalog, rewrites, idioms)
    Ported(before, after, phases, sources.toMap, log.all, mode, catalog, rewrites, idioms, binder)

  /** parse only -- for tests about the FRONTEND rather than about a phase. `fatal = true`, exactly as [[portIn]]/[[portAllIn]], and here one step stronger: an undischarged obligation is a LOWERING
    * ARM that returned without consulting an attached difference, so a frontend-only spec is the closest witness to it.
    */
  def parse(java: String): Program = parseWith(java)._1

  /** …and the LOG beside it, so "is this path fatal" is a question a spec can ask. The three entry points differ by one constructor argument and nothing reports a disagreement between them; this is
...
abstract class PortSuite extends munit.FunSuite:
  ...
  /** the emitted Scala contains `snippet` — with the WHOLE output in the failure message, because "expected substring not found" without the text is the single most expensive failure mode when the
    * emitter is what changed.
    */
  def assertEmits(p: Ported, snippet: String)(using munit.Location): Unit =
    if !p.out.contains(snippet) then fail(s"emitted Scala does not contain:\n  $snippet\n--- emitted ---\n${p.out}\n---------------")

  def assertNotEmits(p: Ported, snippet: String)(using munit.Location): Unit =
    if p.out.contains(snippet) then fail(s"emitted Scala still contains:\n  $snippet\n--- emitted ---\n${p.out}\n---------------")

  def assertEmitsMatch(p: Ported, regex: String)(using munit.Location): Unit =
    if !p.out.matches(s"(?s).*$regex.*") then fail(s"emitted Scala does not match:\n  $regex\n--- emitted ---\n${p.out}\n---------------")

  /** a type that the phase was supposed to retype AWAY is no longer referenced anywhere.
    *
    * The xref form of the assertion, not the textual one: a name can vanish from the output because the emitter stopped printing it, which is a different (and worse) fact than the symbol having no
    * usages left.
    */
  ...
  /** a decision of `kind` was RECORDED by the phases that ran. `about` is matched as a substring of `subjectFqn` -- the name the subject had AT DECISION TIME, the only form that survives a later
    * rename. Empty matches any subject. Asserts only that the engine recorded the decision, never that a porter note reached the code (`NoteCoverageCheck` holds both).
    */
  def assertDecides(p: Ported, kind: Decision.Kind, about: String = "")(using munit.Location): Unit =
    if !p.decisions.exists(d => d.kind == kind && d.subjectFqn.contains(about)) then
      fail(
        s"no $kind decision${if about.isEmpty then "" else s" about a subject containing '$about'"}" +
          s" was recorded\n--- ${p.decisions.size} decision(s) ---\n${renderDecisions(p)}\n---------------"
      )

  /** …and the other direction, which is the one a regression needs: an act the engine performed SILENTLY is a decision missing from the log, and no text assertion can see it.
    */
  def assertNotDecides(p: Ported, kind: Decision.Kind, about: String = "")(using munit.Location): Unit =
    ...
    if hits.nonEmpty then fail(s"$kind was recorded and should not have been:\n${hits.map("  " + render(_)).mkString("\n")}")
  ...
  def assertIdiomConverts(p: Ported, kind: IdiomKind, about: String = "")(using munit.Location): Unit =
    if !p.idioms.all.exists(c =>
        c.kind == kind && c.verdict == IdiomVerdict.Converted &&
          c.subject.contains(about)
      )
    then fail(s"no $kind CONVERSION${label(about)} was filed\n${renderIdioms(p)}")

  /** …and DECLINED one, naming the guard. `guard` is matched exactly, because the guard IS the classification a reader acts on: a substring match would let a spec pass on a different refusal that
    * happens to share a prefix.
    */
  def assertIdiomRefuses(p: Ported, kind: IdiomKind, guard: String, about: String = "")(using munit.Location): Unit =
    if !p.idioms.all.exists(c =>
        c.kind == kind && c.subject.contains(about) && (c.verdict match
          case IdiomVerdict.Refused(g, _) => g == guard
          case _                          => false)
      )
    then fail(s"no $kind REFUSAL under guard `$guard`${label(about)} was filed\n${renderIdioms(p)}")

  /** the site was CONSIDERED at all — the denominator half, and the one a spec needs to prove a census is not silently skipping a shape. A site nothing filed is invisible to both assertions above,
    * and "no conversion" and "never looked" are the two answers those cannot tell apart.
    */
  def assertIdiomConsiders(p: Ported, kind: IdiomKind, about: String = "")(using munit.Location): Unit =
    if !p.idioms.all.exists(c => c.kind == kind && c.subject.contains(about)) then fail(s"no $kind candidate${label(about)} was filed at all\n${renderIdioms(p)}")
  ...
  def assertIdiomIgnores(p: Ported, kind: IdiomKind, about: String)(using munit.Location): Unit =
    ...
    if hits.nonEmpty then
      fail(
        s"$kind candidates were filed and should not have been:\n" +
  ...
  private def renderIdioms(p: Ported): String =
    if p.idioms.isEmpty then "--- 0 idiom candidate(s) ---"
      ...
      s"--- ${p.idioms.size} idiom candidate(s) ---\n" +
  ...
  def assertFinds(findings: Seq[CheckReport.Finding], kind: String, detail: String = "")(using munit.Location): Unit =
    if !findings.exists(f => f.kind == kind && f.detail.contains(detail)) then
      fail(
        s"no finding of kind '$kind'${if detail.isEmpty then "" else s" whose detail contains '$detail'"}" +
  ...
  def assertConsults(p: Ported, id: DiffId, fired: Boolean = false)(using munit.Location): Unit =
    ...
    if n == 0 then
      fail(
        s"$id was never consulted while lowering this fixture" +
          s"\n--- rows reached ---\n${renderReached(p)}\n---------------"
      )
    if fired && p.catalog.fired(id) == 0 then
      fail(
        s"$id was consulted $n time(s) and never APPLIED — the branch is live and the " +
  ...
  /** …and the direction a regression needs: the engine did NOT consider this difference here.
    *
    * Every assertion in this file is tested in both directions, and this is the one that makes the positive form mean something — a fixture where every row reads as consulted is a fixture that would
    * pass with the wrapper wired to a constant.
    */
  def assertNotConsults(p: Ported, id: DiffId)(using munit.Location): Unit =
    val n = reached(p).consulted(id)
    if n != 0 then fail(s"$id was consulted $n time(s) and should not have been at this construct")

  /** THE LOG WITH BOTH SURFACES IN IT -- forcing the emission first, then reading. Only the frontend's consults have run by the time a fixture is constructed; the EMITTER's consults run lazily on
    * `out`, so reading the log directly would report every `Attaches.Rendered` row as never consulted.
    */
  ...
  /** a catalog row was CITED by a PHASE, at a declaration whose name contains `about`.
    *
    * The phase surface's assertion, and deliberately a different one: a citation is weaker than an obligation — nothing can assert that a phase *should have* considered a difference at a declaration
    * it never visited — so a suite must not be able to spell the two the same way.
    */
  def assertCites(p: Ported, id: DiffId, about: String = "")(using munit.Location): Unit =
    ...
    if !at.exists(_.contains(about)) then
      fail(
        s"$id was not cited by any phase${if about.isEmpty then "" else s" at a declaration containing '$about'"}" +
  ...
  def assertPreviewEmits(p: Ported, snippet: String)(using munit.Location): Unit =
    if !p.previewOut.contains(snippet) then fail(s"preview emission does not contain:\n  $snippet\n--- preview ---\n${p.previewOut}\n---------------")

  def assertPreviewNotEmits(p: Ported, snippet: String)(using munit.Location): Unit =
    if p.previewOut.contains(snippet) then fail(s"preview emission still contains:\n  $snippet\n--- preview ---\n${p.previewOut}\n---------------")
