...
/** `java.util.Collection` / `java.util.AbstractCollection`, as Scala — java's interface, not scala's collection.
  */

/** The ABSTRACT/CONCRETE split is `java.util.AbstractCollection`'s OWN, member for member: only `iterator()` and `size()` are abstract there, and everything else has a default written over those two.
  */
trait JavaCollection[A] extends JavaIterable[A] {
  ...
  /** `AbstractCollection.add` THROWS — it is not a no-op and not abstract. A java subclass that does not override it really does reject `add`, so reproducing that is faithful; making it abstract
    * instead would demand an implementation java never required.
    */
  def add(e: A): Boolean = throw new UnsupportedOperationException("add")

  /** removes THROUGH the iterator, as java does, so the first matching element goes and the rest of the collection is untouched.
    */
  ...
  /** java removes THROUGH the iterator here, so the receiver is modified in place and the traversal stays valid — a copy-then-clear would lose an alias the caller may hold.
    */
  def retainAll(c: JavaCollection[?]): Boolean = {
    var changed = false
    val it      = iterator()
    while it.hasNext() do if !c.contains(it.next().asInstanceOf[java.lang.Object]) then { it.remove(); changed = true }
    changed
  }
  ...
  def removeIf(filter: java.util.function.Predicate[? >: A]): Boolean = {
    var changed = false
    val it      = iterator()
    while it.hasNext() do if filter.test(it.next()) then { it.remove(); changed = true }
    changed
  }
...
object JavaCollection {
  ...
  /** Adapt a scala collection to the java-shaped one — the counterpart of [[JavaIterable.from]], for a call site where a `Collection`-typed parameter meets a collection the port itself mapped to
    * scala.
    */
  ...
  /** the OTHER direction — a `java.util.Collection` a third party HANDED BACK, at a slot the retyping made this shim.
    */
  ...
  /** The same seam for a `Kind.Set` source — `java.util.Set` IS a `java.util.Collection`, so a ported method taking a `Collection` must still accept the port's `mutable.Set`.
    */
  ...
  /** Adapt a scala collection that the port may NOT mutate through — a DISTINCT NAME rather than an overload of [[from]], deliberately.
    */
  ...
  def filtered[A](xs: scala.collection.mutable.Buffer[A], p: java.util.function.Predicate[? >: A]): scala.collection.mutable.Buffer[A] = xs.filter(p.test(_))

  extension [A](self: JavaCollection[A]) {

    /** a scala view — `map`, `filter`, `foreach` and the rest. Inherited `foreach` from [[JavaIterable]] already covers `for (x <- xs)`.
      */
