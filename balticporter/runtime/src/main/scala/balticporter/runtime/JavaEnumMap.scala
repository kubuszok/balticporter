...
final class JavaEnumMap[K <: java.lang.Enum[K], V] extends scala.collection.mutable.AbstractMap[K, V] {
  ...
  /** …and java's `isValidKey` as the READERS use it: not a key this map can hold, so it is absent. Named apart from [[requireKey]] because the two are the same question with two answers, and a single
    * helper would have to pick one.
    */
...
object JavaEnumMap {
  ...
  /** `new EnumMap<>(K.class)` — the class token java needs for its ordinal array and this implementation does not. Taken and IGNORED rather than dropped at the call site, so the emitted code still
    * reads like the java it came from.
    */
