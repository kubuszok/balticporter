...
object JavaIterable {
  ...
  /** Adapt a plain scala collection to the java-shaped one. Inserted by the engine at call sites where a shim-typed parameter meets a collection the port ITSELF mapped to scala
    * (`CharArray.appendAll(list)`). `remove()` stays at [[JavaIterator]]'s default — `UnsupportedOperationException` — because a scala iterator genuinely cannot remove, which is what java reports for
    * a non-removable iterator too.
    */
  ...
  /** Adapt an iterator-producing function into a JavaIterable view. Each call to `iterator()` invokes the function, so the result is re-traversable.
    */
  ...
  extension [A](self: JavaIterable[A]) {
    ...
    /** so `for (x <- xs)` and `xs.foreach(f)` work directly on the java shape. Written as the loop rather than delegating to the iterator: `JavaIterator` deliberately has no `foreach` extension (see
      * there), so there is nothing to delegate to.
      */
