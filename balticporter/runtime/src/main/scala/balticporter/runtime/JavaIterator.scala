...
object JavaIterator {
  ...
  /** Adapt a `scala.collection.Iterator` to the java-shaped one. `remove()` keeps the default above, which is the truth: there is nothing to remove through.
    */
  def from[A](it: scala.collection.Iterator[A]): JavaIterator[A] = it match {
    case ji: JavaIterator[A @unchecked] => ji
    case _ =>
      new JavaIterator[A] with Wrapping {
        def wrapped:   Any     = it
  ...
  /** A removing iterator for a `scala.collection.mutable.Buffer` (which includes `ArrayDeque`).
    *
    * Delegates `size`, `apply` and `remove` to the buffer's own methods. Portable: `mutable.Buffer` is in the Scala stdlib on all platforms.
    */
  ...
  extension [A](self: JavaIterator[A]) {
    ...
    /** A scala view of this java iterator. `remove()` is not expressible there and is simply not offered — the view is for traversal.
      */
    ...
    /** Deliberately NO `foreach` here, though the loop is trivial: a class that is both a java `Iterable` and a java `Iterator` — which is most of libGDX's — would then have two applicable extensions
      * and scala reports the `for` as ambiguous rather than picking one. `foreach` lives on [[JavaIterable]], which is what java's own for-each requires anyway; traverse an iterator with `asScala`.
      */
  }
}
