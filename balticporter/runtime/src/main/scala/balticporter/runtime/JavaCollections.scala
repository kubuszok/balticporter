...
/** `java.util.Collections`' statics, and the `java.util.Map.Entry` statics that follow from mapping `Map.Entry` to `Tuple2` — as Scala, with java's own semantics.
  */
object JavaCollections {
  ...
  /** `java.util.Collections.sort(list)` — the natural-ordering overload, which java resolves through `Comparable`. Scala needs an `Ordering`, and there is none for an arbitrary `A`; taking the
  ...
  /** `java.util.Arrays.asList(a, b, c)` — the ELEMENT form only. Java's `asList` has two shapes with different semantics, and only one of them may ever reach this method:
    */
  def asList[A](xs: A*): scala.collection.mutable.Buffer[A] =
    scala.collection.mutable.ArrayBuffer.from(xs)

  /** `java.util.Arrays.asList(T[] a)` — the OTHER shape behind that syntax, and the one [[asList]] above must never receive: a LIVE, FIXED-SIZE view of the caller's array.
    */
  ...
  /** `java.util.stream.Stream.noneMatch(Predicate)` — the one short-circuiting terminal with no scala namesake.
    */
  ...
  def addAll[E](
    dst: scala.collection.mutable.Iterable[E] & scala.collection.mutable.Growable[E] & scala.collection.mutable.Shrinkable[E],
    src: scala.collection.IterableOnce[?] | JavaIterable[?]
  ): Boolean = {
    val before = dst.size
  ...
  /** `java.util.List.addAll(int index, Collection c)` — the POSITIONAL sibling of [[addAll]], which inserts rather than appends and which scala spells `insertAll`.
    */
  def insertAll[E](dst: scala.collection.mutable.Buffer[E], index: Int, src: scala.collection.IterableOnce[?] | JavaIterable[?]): Boolean = {
    val before = dst.size
  ...
  /** `java.util.Collections.singletonList(x)` — one element, immutable.
    *
    * FRESH per call, and that is java's behaviour too (`new SingletonList<>(o)`), so the identity argument [[emptyList]] makes does not apply here.
    */
  ...
  def subList[A](xs: scala.collection.mutable.Buffer[A], from: Int, to: Int): scala.collection.mutable.Buffer[A] = {
    if from < 0 || to > xs.length || from > to then throw new IndexOutOfBoundsException(s"subList($from, $to) on a list of size ${xs.length}")
  ...
  def putIfAbsent[K, V](m: scala.collection.mutable.Map[K, V], k: K, v: V): V = {
    ...
    if cur == null then { m.put(k, v); null.asInstanceOf[V] }
    else cur
  }

  /** `java.util.Map.computeIfAbsent(k, f)` — java's own default implementation, which is NOT `getOrElseUpdate`, and the two differ TWICE with no compile error either time.
    */
  def computeIfAbsent[K, V](m: scala.collection.mutable.Map[K, V], k: K, f: java.util.function.Function[? >: K, ? <: V]): V = {
    val cur = m.get(k) match {
      case Some(x) => x
      case None    => null.asInstanceOf[V]
    }
    if cur != null then cur
      ...
      if v != null then { m.put(k, v); v }
      else null.asInstanceOf[V]
    }
  }

  /** `java.util.Collection.removeIf(p)` on a LIST — removes every element the predicate accepts, and returns whether any went.
    */
  def removeIf[A](xs: scala.collection.mutable.Buffer[A], p: java.util.function.Predicate[? >: A]): Boolean = {
    val q       = p.asInstanceOf[java.util.function.Predicate[A]]
    var i       = 0
    var removed = false
    while i < xs.length do
      if q.test(xs(i)) then { xs.remove(i); removed = true }
      else i += 1
    removed
  }

  /** `java.util.Collection.removeIf(p)` on a SET — the same member at the other kind. */
  def removeIfSet[A](xs: scala.collection.mutable.Set[A], p: java.util.function.Predicate[? >: A]): Boolean = {
    val q      = p.asInstanceOf[java.util.function.Predicate[A]]
  ...
  /** `java.util.Collection.containsAll(c)` — java's own default, `for (Object e : c) if (!contains(e)) return false`.
    */
  def containsAll[A](xs: scala.collection.Iterable[A], c: scala.collection.IterableOnce[?] | JavaIterable[?]): Boolean =
    elementsOf(c).forall(o => xs.exists(e => if o == null then e == null else o.equals(e)))

  /** the ARGUMENT of one of java's four bulk `Collection` operations, read once, from whichever side of the retyping it arrived on.
    */
  ...
  /** `java.util.Collection.removeAll(c)` — java's own default, `while (it.hasNext()) if (c.contains (it.next())) it.remove()`, answering whether anything went.
    */
  def removeAll[A](
    xs: scala.collection.mutable.Iterable[A] & scala.collection.mutable.Growable[A] & scala.collection.mutable.Shrinkable[A],
    c:  scala.collection.IterableOnce[?] | JavaIterable[?]
  ): Boolean = {
    val probes = elementsOf(c).toList
    dropWhere(xs, e => probes.exists(o => if e == null then o == null else e.equals(o)))
  }

  /** `java.util.Collection.retainAll(c)` — the same loop with the test negated: every element of the receiver NOT in the argument goes.
    */
  def retainAll[A](
    xs: scala.collection.mutable.Iterable[A] & scala.collection.mutable.Growable[A] & scala.collection.mutable.Shrinkable[A],
    c:  scala.collection.IterableOnce[?] | JavaIterable[?]
  ...
  /** remove every element the predicate dooms and say whether any did — the shared half of [[removeAll]] and [[retainAll]], which differ only in that predicate.
    */
  private def dropWhere[A](xs: scala.collection.mutable.Iterable[A] & scala.collection.mutable.Growable[A] & scala.collection.mutable.Shrinkable[A], doomed: A => Boolean): Boolean = xs match {
    case b: scala.collection.mutable.Buffer[A @unchecked] =>
      var i       = 0
      var removed = false
      while i < b.length do
        if doomed(b(i)) then { b.remove(i); removed = true }
        else i += 1
      removed
    case _ =>
  ...
  /** `java.util.Optional.orElse(other)` — STRICT, which is the whole reason it needs a member here rather than a rename.
    */
  ...
  /** …a `java.util.Iterator`, which the port retypes to the REMOVAL-CAPABLE shim rather than to scala's `Iterator`. The wrapper's `remove()` throws, which is java's own default for an iterator that
    * offers no removal — and `asScala` does not carry java's `remove` across, so this is the one member of the family that loses something. It loses it LOUDLY.
    */
  ...
  /** java's `Set.remove(Object)` — which RETURNS whether the set held it.
    *
    * `-=` is the rewrite everywhere else and answers the RECEIVER, so java's `boolean` is lost; this member is the probe seam and java's own result, in one.
    */
  ...
  /** java's `Map.keySet()` — the LIVE, WRITE-THROUGH view, at the type the retyping declares.
    *
    * Removal reaches the MAP, which is java's contract and is exactly the capability `scala.collection.Set` cannot express; `add` throws what java's own view throws.
    */
  ...
  /** a class the port DECLARES that kept java's `Map.Entry` as its parent, at the `Tuple2` slot the mapping gave every USE of that interface.
    */
  ...
  /** …a `Map`. Note java's `Map` is neither a `Collection` nor an `Iterable`, so this overload serves exactly the `java.util.Map` formals and no others — which is the same asymmetry `coerce`'s
    * refusal table records for the shim direction.
    */
  ...
  /** `mutable.Buffer.remove(idx, count)` — java's `List` has no such member.
    *
    * `AbstractList.removeRange`'s own body: remove at the SAME index `count` times, which is what makes it correct on a list that shifts left after each removal.
    */
  ...
  /** `mutable.Buffer.insertAll(idx, elems)` — java's nearest is `addAll(int, Collection)`, whose argument is a `java.util.Collection` and not an `IterableOnce`.
    */
  def bufferInsertAll[A](self: scala.collection.mutable.Buffer[A], idx: Int, elems: scala.collection.IterableOnce[A]): Unit = {
    var at = idx
  ...
  /** `mutable.Buffer.patchInPlace(from, patch, replaced)` — java has no counterpart at all.
    *
    * Scala's own contract, over the two above: `replaced` is CLAMPED to what is actually there, which is what `Buffer`'s own implementations do and what keeps a `replaced` past the end from throwing
    * where scala's does not.
    */
  def bufferPatchInPlace[A](self: scala.collection.mutable.Buffer[A], from: Int, patch: scala.collection.IterableOnce[A], replaced: Int): Unit = {
  ...
  /** `java.util.Map.Entry.comparingByKey(cmp)` over the `Tuple2` a `Map.Entry` becomes.
    *
    * The engine maps `Map.Entry` to `Tuple2` (a key/value pair has no identity of its own), which means `Entry`'s own statics have to come along: `Tuple2` has no `comparingByKey`, and the call
  ...
  object Reified {
    ...
    /** WHAT A SHIM IS DELEGATING TO, and the reason every predicate below asks twice.
      */
    ...
    /** java's `x instanceof java.util.Collection` — the shim target, so the mapped subtypes' targets are named beside it (see the block comment).
      */
    ...
    /** java's `x instanceof java.lang.Iterable` — every `Collection` representation plus the `Iterable` shim itself. A java `Map` is not an `Iterable`, so no map is named here.
      */
    ...
    /** the value AS JAVA WOULD HAVE HELD IT, decided from the object rather than from its type.
      *
      * Identity for everything this engine did not put there — a `String`, a boxed number, a `java.util.*` the port never touched, a class the port emits — so it is safe to insert wherever a value
      * crosses out and the phase cannot prove the value is harmless.
      */
    ...
    /** java and scala agree about arrays, so the SPINE is already right and only the elements can be wrong. Converted eagerly because an array has no view, and returned AS IT ARRIVED whenever nothing
      * moved — an array of `String` crossing out is the same object it was.
      */
    ...
    /** the one entry shape these views need. Not `java.util.AbstractMap.SimpleImmutableEntry`, whose availability is a platform question this module answers for itself; `equals` and `hashCode` are
      * java's own documented contract for `Map.Entry`.
      */
    final private class Entry(k: java.lang.Object, v: java.lang.Object) extends java.util.Map.Entry[Any, Any] {
      def getKey():                Any     = k
      def getValue():              Any     = v
    ...
    final private class MapView(m: scala.collection.Map[Any, Any]) extends java.util.AbstractMap[Any, Any] {
    ...
    final private class ListView(q: scala.collection.Seq[Any]) extends java.util.AbstractList[Any] {
      def get(i: Int):     Any = toJavaValue(q(i))
      override def size(): Int = q.size
    }

    final private class SetView(s: scala.collection.Set[Any]) extends java.util.AbstractSet[Any] {
      def iterator(): java.util.Iterator[Any] = new IteratorView(s.iterator)
      def size():     Int                     = s.size
    }

    /** a scala `Iterable` that is neither a `Seq`, a `Set` nor a `Map` — java's nearest honest shape for it is a `Collection` whose `add` throws, which is what `AbstractCollection` is.
      */
    final private class IterableView(i: scala.collection.Iterable[Any]) extends java.util.AbstractCollection[Any] {
      def iterator(): java.util.Iterator[Any] = new IteratorView(i.iterator)
      def size():     Int                     = i.size
    }

    final private class IteratorView(it: scala.collection.Iterator[Any]) extends java.util.Iterator[Any] {
      def hasNext(): Boolean = it.hasNext
      def next():    Any     = toJavaValue(it.next())
    }

    final private class ShimCollectionView(c: JavaCollection[Any]) extends java.util.AbstractCollection[Any] {
      def iterator(): java.util.Iterator[Any] = new ShimIteratorView(c.iterator())
      def size():     Int                     = c.size()
    }

    final private class ShimIterableView(i: JavaIterable[Any]) extends java.util.AbstractCollection[Any] {
    ...
    final private class ShimIteratorView(it: JavaIterator[Any]) extends java.util.Iterator[Any] {
      def hasNext(): Boolean = it.hasNext()
      def next():    Any     = toJavaValue(it.next())
    }
  }
  ...
  def sortedWith[A](xs: scala.collection.mutable.Buffer[A], cmp: java.util.Comparator[? >: A]): scala.collection.mutable.Buffer[A] =
  ...
  /** `IntStream.range(a, b)` — a stream SOURCE that is not a collection, so nothing else in the chain-collapse can produce it. Half-open, as java's is.
    */
  ...
  def toMap[A, K, V](
    ...
    key:   java.util.function.Function[? >: A, ? <: K],
    value: java.util.function.Function[? >: A, ? <: V],
    merge: java.util.function.BinaryOperator[V]
  ): scala.collection.mutable.Map[K, V] = {
    val m = scala.collection.mutable.HashMap.empty[K, V]
    xs.foreach { x =>
      val k = key.apply(x)
  ...
  /** `Collectors.toCollection(Factory::new)` — build the factory's collection and fill it.
    *
    * `Growable` is the exact bound: it is what "a collection you can add to" is in scala, and it is what every `Collectors.toCollection` target satisfies.
    */
  def into[A, C <: scala.collection.mutable.Growable[A]](xs: scala.collection.mutable.Buffer[A], factory: () => C): C = {
    val c = factory()
    c ++= xs
    c
  }
  ...
  final private class FrozenBuffer[A](under: scala.collection.Seq[A]) extends scala.collection.mutable.AbstractBuffer[A] {
  ...
  /** `java.util.Arrays.asList(T[])`'s live, fixed-size view — see [[asListView]] for the contract.
    *
    * Every size-preserving operation reads or writes the ARRAY; every size-CHANGING one throws, as `java.util.Arrays$ArrayList` does. `refuseFixedSize` rather than [[refuse]] because the reason is a
    * different one and the message a reader gets at run time is the whole value of throwing.
    */
  final private class ArrayViewBuffer[A](arr: Array[A]) extends scala.collection.mutable.AbstractBuffer[A] {
  ...
  /** `java.util.List.subList`'s view — see [[subList]] for the contract and for what is deliberately not reproduced. `until` is a `var` because java's view resizes when you insert or remove THROUGH
    * it.
    */
  final private class SubBuffer[A](under: scala.collection.mutable.Buffer[A], from: Int, private var until: Int) extends scala.collection.mutable.AbstractBuffer[A] {
  ...
  /** [[FrozenBuffer]]'s `Set`. */
  final private class FrozenSet[A](under: scala.collection.Set[A]) extends scala.collection.mutable.AbstractSet[A] {
  ...
  /** [[FrozenBuffer]]'s `Map`. */
  final private class FrozenMap[K, V](under: scala.collection.Map[K, V]) extends scala.collection.mutable.AbstractMap[K, V] {
  ...
  /** java's own answer at every one of those members — `UnsupportedOperationException`, with the message naming what the value IS, since the alternative a reader will guess is an engine bug.
    */
