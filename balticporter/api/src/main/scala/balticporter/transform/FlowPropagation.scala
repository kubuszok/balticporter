...
object FlowPropagation:
  ...
  /** Every pure-move flow edge in the program. `(a, b)` means a value moves between `a` and `b` without arithmetic, so they must share a type. Exposed separately from [[grow]] so a spec can pin the
    * edge set itself — the thing a phase's behaviour is actually a function of.
    */
  def edges(program: Program): List[(SymId, SymId)] =
    ...
    /** references returnable from a method body's TAIL (a bare-expression body, or the last expression of a block), which flow to the method's own symbol exactly as a `return` does.
      */
