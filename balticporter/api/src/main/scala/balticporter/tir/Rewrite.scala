...
/** WHAT A PHASE MOVED, and WHICH LANE COUNTS THE SEAMS IT COULD NOT CLOSE. Closes CLAUDE.md §1's "every seam is counted" obligation. A phase declares only [[Rewrite.accountedBy]] (the lane);
  * `Pipeline.runTraced` DERIVES what it retyped by comparing BOTH type records (a symbol's `info` AND its tree's `tpt`) across the phase, so it cannot under-report. Not a second boundary count (the
  * checks keep their own numbers) and not a pre-phase site count (measured wrong — K5.10).
  */
...
/** ONE PHASE'S RECORD for one translation: what its rewrite moved, and what it claims counts the residue. `retyped` is OBSERVED by the pipeline (owned symbols whose `info` changed); `accountedBy` is
  * DECLARED by the phase — see [[Rewrite]] for why. An external's signature is a class-file fact no phase may move (§4.56); `StandardTraversal.mapSymbols` already refuses it.
  */
...
/** The patches one translation produced — a value THAT RUN owns.
  *
  * Never a process-global table, for the reason `TirEmitter.srcMap`, `DecisionLog` and `CatalogLog` are values one run owns (`CLAUDE.md` §5.1): `Determinism.Full` translates twice, and two
