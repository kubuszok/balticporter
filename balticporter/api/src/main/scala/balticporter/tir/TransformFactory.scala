...
trait TransformFactory:
  ...
  /** The REMEDIES the phase this factory builds can carry out — declared HERE too, and the duplication is the point: a `resolutions` typo is validated at LOAD before a pipeline exists, while a real
    * id whose phase isn't enabled needs a different message, and only a no-construction declaration can tell them apart. Must state the SAME values as the phase's own [[RemedySource]] — a copy that
    * drifts refuses loudly. `Nil` is the default and today's honest answer.
    */
  def remedies: List[Remedy] = Nil

object TransformFactory:

  /** THE shared grammar for a [[RuleScope]] in config: `scope { except = [...] }` is `Everywhere(except)`, `scope { only = [...] }` is `Only(include)`, absent is `Everywhere()` (no-op). Both at once
    * REFUSES (no meaning to hold); an empty `only = []` is honoured as written ("only these, and there are none" — see [[RuleScope.Only]]).
    */
      ...
      case Some(s)    =>
        (s.strings("except"), s.strings("only")) match
          ...
          case (scala.None, scala.None) =>
            throw ConfigError(s.path,
                              "a scope object declares `except` or `only`; " +
                                "omit the object entirely for the unrestricted default"
            )

/** A read-only view over one object of a port's configuration. Minimal on purpose (see [[TransformFactory]]). Returns `None` for an ABSENT key, THROWS [[ConfigError]] for the wrong shape — different
  * mistakes. An implementation must RECORD every key read, so the loader can fail on a key nobody read: HOCON tolerates junk silently, and this is the config path's `PolicyReport` equivalent.
  */
trait ConfigView:
  ...
  /** a list of strings. A single string is NOT silently widened to a one-element list: a config that means a list says so, and quiet coercion is how a scalar typo becomes a valid document.
    */
  ...
  /** is the value at `key` an OBJECT? — the one shape question a reader may ask before reading. Exists for COMPATIBLE EXTENSION of a settled key (`redirects { "a.B" = { to = "c.D", … } }` beside the
    * plain-string form) — catching `string`/`child`'s `ConfigError` would turn a genuine shape error into a silent fallback. A PROBE, not a read: does not mark the key read. Absent key is `false`.
    */
  ...
  /** a value from a CLOSED set of spellings, named in the error — the loud door of `OpaqueSpec.Primitive.fromScalaName`, generalised.
    */
  final def enumerated[A](key: String, alternatives: Map[String, A]): Option[A] =
    string(key).map(v => alternatives.getOrElse(v, throw ConfigError(at(key), s"'$v' is not one of ${alternatives.keys.toList.sorted.mkString(", ")}")))

/** Anything wrong with a port's configuration, located at the key an agent has to edit. One exception type, not a findings list: a configuration that does not parse produces no port at all, so there
  * is nothing to report findings ABOUT.
  */
final case class ConfigError(where: String, why: String) extends RuntimeException(s"port config: $where: $why")
