...
/** WHICH `try` — see [[Tree.Try.id]] for why neither an `Origin` nor object identity can be that.
  *
  * A counter and nothing else: it is never printed, never emitted, never written to an artifact and never compared across runs, so no output depends on the order tokens are handed out in.
  */
...
/** Stable symbol identity — interned, NEVER a string (the analog of `reflect.Symbol` as a handle). Ergonomic queries hang off it as `(using Program)` extensions, mirroring Quotes' `(using Quotes)`
  * symbol methods.
  */
...
final case class Flags(
  ...
  /** True iff java declared this a `record` (JLS 8.10) AND every component in [[Symbol.components]] survived (a `dropMethods`'d accessor clears it) — licenses synthesising the accessor, canonical
    * constructor, `equals`/`hashCode`/`toString`. Not derivable from `extends java.lang.Record` (scalac emits no JVM record and accepts that clause on non-records too).
    */
...
/** A Java annotation on a declaration — `@Test`, `@Override`, `@Deprecated`, `@Null`. `args` are its element values, by name (a single-element `@A(x)` is named `value`), carried as real `Term`s
  * rather than text — a dropped argument is the same silent-omission defect as a dropped annotation (e.g. a `@Test` that fails to survive runs zero tests and reports success).
  */
final case class Annot(tpe: TypeRepr, args: List[(String, Term)], origin: Origin)

/** A declaration's symbol record (the analog of `reflect.Symbol`'s backing data). `info` is its type: a value/field type, a `MethodType`/`PolyType` for methods, a `TypeBounds` for type
  * params/abstract types, or the class `TypeRef` for classes. Cross-references (declarations, members, usages) are answered by `Program`, so a plain record stays serializable while the graph lives in
  * the tables.
  */
final case class Symbol(
  ...
  /** annotations the frontend could NOT carry, by name — reported by `OmissionCheck` rather than discarded, so a gap here is a number on every run.
    */
  droppedAnnotations: List[String] = Nil,
  /** The member's parameter spelling — the half of member identity [[fullName]] does not carry; see [[Descriptor]] for the grammar. `None` means: not an executable, OR an unresolvable external member
  ...
  /** Java's record components (JLS 8.10.1), in declaration order — required because `equals`/ `hashCode`/`toString`/deconstruction all read them positionally, so this cannot be a `Set` (sorted by
    * source position at harvest). Interned rather than name-keyed: field, accessor and component share one java name, and emission renames the field (`x` -> `x$field`).
    */
  components: List[RecordComponent] = Nil
)

/** One java record component (JLS 8.10.1). `field` and `accessor` are carried separately because java reads different ones: `equals`/`hashCode`/`toString` read the FIELD, a record pattern
  * deconstructs through the ACCESSOR (JLS 14.30.1) — an overridden accessor can make them diverge.
  * @param name
  *   the component's java name, before any clash resolution.
  */
...
object TypeRepr:
  ...
  final case class MethodType(params: List[(String, TypeRepr)], result: TypeRepr, isImplicit: Boolean = false) extends TypeRepr
...
/** a syntactic type occurrence (`reflect.TypeTree`) — carries its resolved `TypeRepr` and an `Origin`, bridging tree positions and the type algebra.
  */
final case class TypeTree(tpe: TypeRepr, origin: Origin) extends Tree

object Tree:
  // ---- definitions ----
  /** class / trait / object / enum. `parents` are the (typed) super constructors/mixins; `selfType` carries `self: S =>`; `tparams` are the class's own type params as `TypeDef`s whose `rhs` is a
    * `TypeBounds`, so an F-bound like `class C[T <: IRich[T]]` is walkable.
    */
  final case class ClassDef(
    ...
    /** Comments belonging to the FILE rather than this type — everything above `package` plus anything on the imports (where a licence notice lives). Kept separate from `leading` because emitting it
      * there would land below the `package` clause. Non-empty only on a top-level unit; a two-type java file becomes two Scala files, each carrying the notice.
      */
    unitLeading: List[Trivia] = Nil
  ) extends Definition

  /** one Java enum constant — `NAME(ctorArgs) { body }`. Lowered to a Scala `case object` extending the enum's sealed class. `body` carries per-constant method overrides.
    */
  ...
  /** type alias / abstract type member / type parameter (`type T = …` / `type T <: U`). For a type parameter, `rhs` carries a `TypeBounds`.
    */
  ...
  /** `super` (receiver of `super.m(...)` / `super(...)`) — distinct from `this` so the backend can emit `super`-dispatch and constructor delegation correctly.
    */
  final case class Super(cls: SymId, tpe: TypeRepr, origin: Origin) extends Term

  /** The body of a java anonymous class — `new Base(args) { members }`. `symbol` is a synthetic type that owns the members; `body` carries fields/methods/init blocks; `dropped` names any member the
    * frontend could not translate, for [[OmissionCheck]]. An empty `body` is meaningful (`new Base(){}`); `New.anon = None` means "not anonymous at all".
    */
  final case class AnonClass(
    symbol:  SymId,
    body:    List[Statement],
    origin:  Origin,
    dropped: List[String] = Nil,
    /** What the class file says about the type this anonymous class named — see [[Sam]] for why the frontend answers this rather than the acting phase. Defaults to [[Sam.Answer.unknown]], the
      * conservative arm: an unasked tree refuses loudly in `idiom(refused)`.
      */
    sam: Sam.Answer = Sam.Answer.unknown
  )

  /** `new T` / `new T { … }`. `anon` is present exactly when Java wrote an anonymous-class body (a `CtNewClass`); its members are Scala's anonymous-class-expression members. Dropping it is the
    * failure this node's shape exists to prevent — a listener that compiles and does nothing.
    */
  ...
  /** @param compound
    *   `Some((op, narrowOpt))` for a compound assignment (`lhs op= rhs`): `op` is the operator, `narrowOpt` the implicit narrowing type when the result casts back to the lvalue's type (JLS 15.26.2,
    *   e.g. `byte += int`). `rhs` is the right operand only; `lhs` appears once in the node. `None` for a plain assignment.
    */
  final case class Assign(lhs: Term, rhs: Term, tpe: TypeRepr, origin: Origin, compound: Option[(String, Option[TypeRepr])] = None) extends Term
  ...
  /** `try (resources) body catch cases finally fin`. `resources` are try-with-resources bindings, auto-closed by the backend.
    * @param id
    *   a token identifying THIS `try`, minted at construction, carried through `copy` — neither `Origin` nor object identity survives synthesis/rebuild reliably, so checks can ask "did you guard this
    *   one".
    */
  final case class Try(resources: List[ValDef], body: Term, catches: List[CatchCase], finalizer: Option[Term], tpe: TypeRepr, origin: Origin, id: TryId = TryId.fresh()) extends Term:
    ...
    override def equals(o: Any): Boolean = o match
      case t: Try =>
        t.resources == resources && t.body == body && t.catches == catches &&
  ...
  /** `scrutinee match { cases }` (from a java switch). Carries an identity token for the same reason [[Try]] does — the `switch-null` check asks "did you guard THIS one", and neither `Origin` nor
    * object identity survives synthesis/rebuild reliably.
    */
  final case class Match(
    ...
    tpe:       TypeRepr,
    origin:    Origin,
    /** Was this a switch EXPRESSION (JLS 15.28) or STATEMENT (14.11) in java? One `Tree.Match` renders both, but they bind `yield` differently (JLS 14.21 targets the innermost switch EXPRESSION,
      * skipping a nested statement switch). `tpe` cannot stand in — a statement switch's `Unit` type is coincidental, not this fact.
      */
    isExpr: Boolean = false,
    id:     MatchId = MatchId.fresh()
  ) extends Term:
    ...
    override def equals(o: Any): Boolean = o match
      case m: Match =>
        m.scrutinee == scrutinee && m.cases == cases && m.tpe == tpe &&
  ...
  /** `yield v` from a switch expression's arm, at a NON-tail position (JLS 14.21) — a tail `yield` is peeled into the arm's result term instead. Models an abrupt completion from arbitrary depth,
    * emitted as a value-carrying `scala.util.boundary` around the arm; `tpe` is `Nothing`. Never in a switch STATEMENT's arm — Spoon's `CtYieldStatement` is unwrapped there.
    */
  final case class Yield(value: Term, tpe: TypeRepr, origin: Origin) extends Term

  /** `case String s ->` — java's type pattern as a case label (JLS 14.11.1, 14.30.1). A `Term` because [[CaseDef.labels]] is a list of them; BINDS a variable rather than ascribing one that exists.
    * `bind` is interned against the enclosing `CtCase` (java allows two arms to reuse a name). See [[RecordPattern]], [[BindPattern]] for the other pattern nodes.
    */
  ...
  /** `case Point(x, y)`'s `x` — a component binding with NO type test (JLS 14.30.2's unconditional pattern). Its own node rather than a trivial [[TypePattern]] because the two differ at a `null`
    * component (`case One(s)` binds it, `case One(s: String)` does not match). Valid only inside a [[RecordPattern]].
    */
  final case class BindPattern(bind: SymId, tpe: TypeRepr, origin: Origin) extends Term

  /** `name: stmt` — a java label on a NON-loop statement, the target of `break name`. A wrapper rather than an `Option` field, since the backend emits a real construct around the statement. Loops
    * carry their label in their own `label` field instead, since `continue L` needs a boundary elsewhere (around the body, not the loop). `tpe` is `Unit`.
    */
  ...
  /** A construct with no faithful Scala image, recorded IN the tree per-site rather than failing the whole unit. `Open` is never shipped in deliverable mode; best-effort renders [[inner]] in comment
    ...
    */
  final case class Unportable(inner: Term, kind: UnportableKind, state: MarkerState, diff: Option[balticporter.catalog.DiffId], what: String, tpe: TypeRepr, origin: Origin) extends Term:

    /** discharged BY A NAMED PHASE, with what it did. The replacement term is the caller's to build; this records that the marker was ANSWERED rather than deleted.
      */
    def resolved(byPhase: String, how: String): Unportable =
      copy(state = MarkerState.Resolved(byPhase, how))

    /** The identity a conservation check compares two programs on — origin plus the mint site's own words, since object identity and `SymId` don't survive a phase rebuild. Distinguishing only because
      * [[Unportable.open]] refuses a synthetic origin (which would collapse every marker onto one key).
      */
  ...
  object Unportable:
    ...
    def open(inner: Term, kind: UnportableKind, diff: Option[balticporter.catalog.DiffId], what: String, tpe: TypeRepr, origin: Origin): Unportable =
      require(
        origin != Origin.synthetic && origin.javaPath.nonEmpty,
  ...
  /** An as-yet-unmodeled TERM, kept typed so the tree stays whole (types are never opaque, only terms). `raw` is closed Scala text; a call-site substitution may carry NUMBERED HOLES ([[Opaque.hole]])
    * with `holes` supplying one term per index, so later phases and xref still reach the spliced arguments ([[spliced]]). `holes = Nil` is the closed default.
    */
  final case class Opaque(
    ...
    /** `raw` with each hole replaced by `render` of the term it names.
      *
      * A hole index with no term is left as the marker rather than dropped or defaulted: it can only arise from a malformed construction, and a marker that survives into the output is a compile error
      * naming the file, where a silent drop would be a wrong program that compiles.
      */
  ...
  object Opaque:
    /** The hole delimiter: NUL, which cannot occur in Scala source and therefore needs no escape grammar. A printable delimiter would need one, and an escape grammar over ready-made Scala is a second
      * parser for text the engine deliberately does not parse.
      */
    ...
    /** `parts` interleaved with holes `0 … parts.size - 2`. `parts` always has one more element than `holes`; a mismatch is a construction bug and is refused here rather than emitted.
      */
    def spliced(parts: List[String], holes: List[Term], tpe: TypeRepr, origin: Origin): Opaque =
      require(
        parts.size == holes.size + 1,
  ...
  /** See THROUGH any number of [[Commented]] wrappers — the extractor form, for a pattern match on a statement's shape: `case Tree.Uncommented(Tree.Apply(fun, args, _, _, _)) => …`. Matches every
    * term, wrapped or not, so it is a drop-in around an existing pattern.
    */
  object Uncommented:
  ...
  /** put `original`'s comments back on a `rewritten` replacement for it. A rewrite that builds a fresh statement (rather than `copy`-ing) drops the wrapper, and the comment goes with it.
    */
...
/** The POSITION a symbol is used in — so a query can ask "used as a mixin / type argument / member type / bound / …", not merely "used". This is what makes the sge/ssg rewrites tractable: java→scala
  * collections retype every `MemberType` and `TypeArg` site; Int→opaque retypes `TypeArg`/`MemberType` flows; a mixin swap edits `Mixin`/`Extends`. Type positions are first-class, not just call
  * sites.
  */
enum UsageKind:
...
/** One recorded use of a symbol: WHERE (`kind`), at which tree node (`site`), and inside which enclosing definition (`enclosing` — the nearest containing method/field/class symbol). `enclosing` is
  * what makes `callersOf` a real call-graph edge.
  */
final case class Usage(kind: UsageKind, site: Tree, enclosing: SymId = SymId.None)

/** Whole-program cross-reference index: symbol → its definition, and symbol → every usage site, kinded by position (term refs and every type position). Built by [[Xref.build]] over the tree and
  * re-derived after each phase rewrite — the pipeline rebuilds it between phases.
  */
...
/** The transform substrate: all units, the symbol table, the xref index. Project- owned transformers receive this, query it (Quotes-familiar), and rewrite.
  */
final class Program(
  val units:   List[Tree.ClassDef],
  val symbols: SymbolTable,
  val xref:    XrefIndex,
  /** What the frontend saw, dropped members included — see [[MemberIndex]]. A required parameter, not defaulted: `Pipeline.runTraced` rebuilds a fresh `Program` after every phase, so a default would
  ...
  export xref.{ definitionOf, referenced, usages, usagesOf }

  def symbolOf(id: SymId): Option[Symbol] = symbols.get(id)

  /** Rebuild after a rewrite, CARRYING EVERYTHING THE CALLER DID NOT CHANGE.
    *
    * `new Program` is for the frontend and the pipeline; a phase uses this. The difference is not brevity: a phase that constructs a `Program` positionally has to remember every whole-program value
    * that exists today, and the one it forgets is dropped in silence.
    */
  def rebuilt(
  ...
  /** methods that call `method` (a call-graph edge) — walked by globals→implicits. Uses each call usage's recorded `enclosing` definition, climbing to the nearest method.
    */
  def callersOf(method: SymId): List[SymId] =
    xref.usages(method).collect { case Usage(UsageKind.Call, _, enc) if enc != SymId.None => enc }.flatMap(enclosingMethod).distinct
