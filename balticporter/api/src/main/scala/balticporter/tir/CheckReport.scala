...
/** PERSISTENCE and DIFF for the engine's checks — `before->after`, mechanically. Every check records its FULL result here; this writes a machine-readable file and diffs against a committed baseline.
  * The ORCHESTRATOR records (checks stay pure functions of a `Program`); `record` registers even an empty result, so `counts.tsv` distinguishes "0" from "never invoked". `findings.tsv` id = sha-256
  * excluding the LINE NUMBER, so a whitespace edit orphans nothing.
  */
object CheckReport:

  /** one finding, flattened to the columns every check has in common. `check` is the gate that produced it, `kind` its own classification of the finding.
    */
  final case class Finding(check: String, kind: String, owner: String, path: String, line: Int, detail: String, seq: Int = 0):
    /** identity WITHOUT the line number, so an upstream whitespace edit does not orphan a baseline entry.
      */
    def baseId: String = TirPrinter.sha256(s"$check|$kind|$owner|$path|$detail").take(12)

    /** …and a disambiguator for the case that follows from excluding the line: one member with three identical findings at three different lines has one base id and three findings. Without the suffix
      * the diff cannot see two of the three being fixed. Assigned at write time by [[assignSeq]] in line order.
      */
  ...
  /** number the findings that share a base id, in the order given. The first keeps the bare id, so a member whose findings do not repeat is unaffected.
    */
  ...
  /** Make a Java source path relative and machine-independent. `balticporter.reportPathRoot` is the source root a port was built from; without it, paths fall back to relative-to-root, which is still
    * deterministic but only within one checkout layout. An ABSOLUTE path in the artifact would make every diff machine-specific, so it is the one thing never emitted.
    */
  ...
  /** Record a check's COMPLETE result. Registers the check name even for `Nil`. No-op when disabled. Idempotent per check name within a run: a second call replaces the first, so a migration that
    * filters and re-records (portability does) reports the filtered number.
    */
  ...
  /** the shutdown-hook path. Nothing recorded ⇒ nothing written: a run that never invoked a check must not leave an artifact claiming every check found zero.
    */
  ...
  def write(out: Path): Unit =
    ...
    Files.writeString(
      out.resolve("counts.tsv"),
      ("#check\tcount" :: findings.map((c, fs) => s"$c\t${fs.size}").sorted ++ upstreamRows).mkString("", "\n", "\n")
    )
    ...
    val d = diff(
      parseAll(baselineDir.resolve("findings.tsv")),
      all,
      baselineChecks(baselineDir),
      findings.map(_._1).toSet,
  ...
  private def baselineChecks(b: Path): Set[String] =
    ...
    else Files.readAllLines(c).toArray(Array.empty[String]).toList.filterNot(_.startsWith("#")).flatMap(_.split('\t').headOption).filter(k => k.nonEmpty && k != UpstreamKey).toSet

  /** The operator document. NOT diffed and NOT committed as a baseline — it may contain anything a human finds useful, including the path root the run used, which the tsv deliberately omits.
    */
  private def reportMd(findings: List[(String, List[Finding])]): String =
    ...
    sb.append(
      s"jvm: ${balticporter.core.JvmInfo.version} (${balticporter.core.JvmInfo.vendor}), " +
    ...
    Diff(
      ...
      checks.map { c =>
        ...
        CheckDelta(
          ...
          ran = latestChecks(c),
          inBaseline = baselineChecks(c) || b.nonEmpty
        )
      }
    )
  ...
  /** How many appeared/disappeared findings to spell out per check. The FULL lists are in `findings.tsv`; this is the terminal render, and a diff that scrolls off the screen is not read.
    */
  val RenderLimit = 12
