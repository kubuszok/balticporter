...
/** ONE LOCATION'S SELECTION — the port picked [[remedy]] at [[key]], and the run bound that key to a declaration it owns. Granularity is PER DECLARATION (matching `Decision.declarationsUsing`), so a
...
object Resolution:
  ...
  /** The check an applied resolution files under, and the KIND it files as. `remediation` is REUSED (already a `PortRun.RequiredChecks` member) rather than given its own lane, so a run that stops
    * recording resolutions fails the same way one that stops recording remediations does. Lives here since `balticporter.api` cannot see the runner that builds the row.
    */
  ...
  /** the phase name every resolutions binding and finding is recorded under — one string, so the binder's records and the policy report cannot disagree about which seam asked.
    */
...
final case class AppliedResolution(
  ...
  /** the declaration the remedy was applied AT, fully qualified — `owner#member`, the grammar `IdiomCandidate.subject` and `decisions.tsv` already speak.
    */
  subjectFqn: String,
  subject:    SymId,
  origin:     Origin,
  ...
  /** …and the `decisions.tsv` row, which carries a PORTER NOTE to the emitted line. `Reason.Configured(Resolution.Seam, declaredKey)`, never the remedy's own [[FixKind]] — the reader's first question
    * is which repository the fix lives in, and for a selection that is always this port's manifest. Which repo owns the remedy's CODE travels in the detail.
    */
  def decision: Decision =
    Decision(
      Decision.Kind.SelectedRemedy,
      subject,
      subjectFqn,
      Map("remedy" -> remedy.id, "drains" -> remedy.target, "owner" -> remedy.fix.section, "why" -> what),
      Reason.Configured(Resolution.Seam, resolution.declaredKey),
      origin
    )

  def render: String = s"${remedy.id} at $subjectFqn: $what  (${origin.javaPath}:${origin.line})"

/** A RESOLUTION THAT WAS CONSULTED AND DECLINED — see [[Resolution.RefusedKind]]. Carries no [[Decision]] on purpose: a refusal changed nothing, so a porter note above the member would falsely say
  * something happened there. Names the GUARD rather than just "no", since every guard has a different next step.
  */
...
final class ResolutionPlan(val entries: List[ResolutionPlan.Entry]):
  ...
  /** entries by the declaration they bound to — the lookup a phase makes per site. Built once; a key that did not bind has no target and is absent by construction.
    */
  ...
  /** TWO ENTRIES BINDING ONE DECLARATION ON ONE LANE — the shape a flat `Map` cannot refuse (two different keys, e.g. `Foo#bar` vs `Foo#bar(int)`, can name one member) and only the BINDING can see.
    * Grouped by declaration then narrowed by [[Remedy.overlaps]] — two lanes, or disjoint kinds on one lane, may legitimately both hold selections. What remains a finding is a pair answering the SAME
    * row: `selected` takes the first match, the second reports `NeverApplied`.
    */
  ...
  /** WHAT DID THE PORT CHOOSE HERE? `target` is the DECLARATION the site sits in, never the site's own symbol — that is [[Resolution]]'s granularity. `lane`/`kind` narrow to the one caller that
    * declared the entry (ids are globally unique). Matched via [[Remedy.answers]], never two equality tests here, since a lane may split one site into several kinds.
    */
  def selected(target: SymId, lane: String, kind: String): Option[Resolution] =
    byTarget.getOrElse(target, Nil).iterator.flatMap(_.resolution).find(_.remedy.answers(lane, kind))

  /** …and the record that it fired. Separate from [[selected]] because a caller may legitimately ask and then refuse for a reason of its own, and a plan that counted the question as the answer would
  ...
  /** the narrowing [[drain]] performs: a selection at this declaration whose remedy is one of MINE and which answers this row's kind. Private, because "one of mine" is only meaningful to the declarer
    * — every other caller wants [[selected]] with a specific `Remedy`.
    */
  private def selectedAmong(target: SymId, ids: Set[String], kind: String): Option[Resolution] =
    byTarget.getOrElse(target, Nil).iterator.flatMap(_.resolution).find(r => ids(r.remedy.id) && r.remedy.answers(r.remedy.lane, kind))

  /** DID THE PORT PICK **THIS** REMEDY HERE? — narrower than `selected(target, lane, kind)`: a phase holding a specific `Remedy` wants to know if this declaration chose IT, and `(lane, kind)` would
    * match a SIBLING where a lane's remedies all declare `Remedy.AnyKind`. An id is globally unique, so this cannot.
    */
  ...
  /** …and the DECLINE, which counts as having FIRED for exactly the reason `record` does: the selection was consulted and answered, so reporting it as `NeverApplied` beside its own refusal row would
    * give one entry two contradictory diagnoses.
    */
  ...
  /** everything recorded, READ and never flushed — `PortRun.appliedRemedies`, [[decisions]] and [[appliedAt]] all read this. No flushing variant: the buffer belongs to ONE `ResolutionPlan` belonging
    * to ONE translation, so it never outlives what it records.
    */
  def all: List[AppliedResolution] = log.toList

  /** DID A REMEDY ALREADY ANSWER THIS ROW? — asked by the check that mints the lane, about what the phase RECORDED (never what the manifest declared) so a refused selection stays in the lane. Matched
    * at the SITE, not the declaration: a remedy may refuse at one site of a member and apply at another, so a per-declaration test would let the lane fall by more than `resolved` gained. `origin` is
    * the same value the applier recorded — not two spellings of a position.
    */
  ...
  /** DECLARED BESIDE APPLIED — every entry that did not do what its author asked, classified.
    *
    * A `def` and not a `val`, because the last of the three answers is only knowable once the run is over: an entry that bound and was never selected is inert, and nothing before the pipeline
    * finishes can say so.
    */
  def troubles: List[ResolutionPlan.Trouble] =
    entries.flatMap { e =>
      if e.unknown then
        Some(
          ResolutionPlan.Trouble(
            e.declared,
            e.id,
            ResolutionPlan.Issue.UnknownRemedy,
        ...
        Some(
          ResolutionPlan.Trouble(
            e.declared,
            e.id,
            ResolutionPlan.Issue.SourceAbsent,
        ...
        Some(
          ResolutionPlan.Trouble(
            e.declared,
            e.id,
            ResolutionPlan.Issue.NeverApplied,
...
object ResolutionPlan:
  ...
  /** WHAT A RESIDUE ROW MUST BE ABLE TO SAY for a remedy to drain it — [[ResolutionPlan.drain]]'s one argument. A record, not curried functions (a positional lambda list swaps fields silently):
    * `kind` is the finding kind, `at` the DECLARATION the selection keys on (never the site's own symbol), `subject`/`origin` feed the resolved/decisions rows, `what` is the note.
    */
  ...
  /** ONE declared entry, with everything the run learned about it before any phase ran.
    * @param declared
    *   the manifest key verbatim @param id the chosen remedy id @param resolution the bound selection, present iff the id names a live remedy AND the key bound to a declaration
    * @param target
    *   the declaration the key bound to, if it bound @param unknown the id names no remedy shipped @param sourceAbsent the id's declarer is not in THIS run @param declaredBy who declares it
    */
  ...
  /** Why a declared selection did nothing. Four answers because there are four different next actions, which is `PolicyBinder`'s own argument for its five: collapsed into one, "you typed the id
    * wrong", "you did not enable the phase", "the finding stopped occurring" and "another key of yours already answered this" read identically and mean entirely different things.
    */
  enum Issue:
    case UnknownRemedy, SourceAbsent, NeverApplied

    /** two entries bound ONE declaration and answer ONE lane — see [[ResolutionPlan.conflicting]]. Kept apart from [[NeverApplied]] because the loser of such a pair would take that one, whose
  ...
  def of(
    ...
    new ResolutionPlan(
      declared.toList.sortBy(_._1).map { (key, id) =>
        ...
        val bound   =
          if subject.isType then binder.bindType(Resolution.Seam, Resolution.Setting, key, subject.ownership).map(s => (scala.None: Option[MemberKey]) -> Some(s))
          else binder.bindMember(Resolution.Seam, Resolution.Setting, key, subject.ownership).map(h => Some(h.key) -> h.sym)
