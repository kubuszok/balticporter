...
enum UnportableKind(val slug: String, val summary: String):
  ...
  case ContextDependentRawFill
      extends UnportableKind(
        "context-dependent-raw-fill",
        "the type argument a raw generic needs differs per USE, so no single rendering is faithful"
      )

  /** a value crossing the seam between a retyped declaration and a JDK signature no phase can move, where no coercion exists in the direction required.
    */
  ...
  /** an API that exists on the JVM and not on a platform this port targets. The one kind whose FIRST remedy is (b): which platforms a port is for is the port's statement, not the engine's.
    */
  case PlatformHostileApi extends UnportableKind("platform-hostile-api", "the API is JVM-only and this port is not")

  /** a reflective lookup whose target this engine cannot resolve to a symbol, so no explicit table can replace it.
    */
  case ReflectiveLookup extends UnportableKind("reflective-lookup", "a reflective lookup whose target cannot be resolved to a declaration")

  /** javac and scalac would choose DIFFERENT overloads for the same argument list. */
  case OverloadDivergence extends UnportableKind("overload-divergence", "javac and scalac resolve this call to different alternatives")

  /** the frontend HAS an arm for the construct and the arm could not complete — a resolution the parser could not give it, a declaration absent from the model. Distinct from [[UnmodelledNodeKind]],
    * which is the case where no arm exists at all.
    */
  ...
  /** an annotation whose ARGUMENTS would not translate, so emitting it bare would emit a DIFFERENT annotation.
    */
  case AnnotationResidue extends UnportableKind("annotation-residue", "the annotation's arguments do not translate, and a bare `@A` is a different annotation")
  ...
  def remedies: List[RemedyHint] = this match
    case RawGenericConversion =>
      List(
        RemedyHint(
          FixKind.Universal,
          "render the raw reference `[?]` at every position, parents and " +
            "overrides included — the answer measured against the reference port"
        ),
        RemedyHint(
          FixKind.Parameterised,
          "if one fill is right for this library, state it as a type " +
            "redirect rather than letting the frontend guess"
        )
      )
    case ContextDependentRawFill =>
      List(
        RemedyHint(FixKind.Universal,
                   "bind the value to a NAMED local of the de-wildcarded type, so the " +
                     "two uses see one capture"
        ),
        RemedyHint(
          FixKind.LibraryRule,
          "where the shape recurs in one library only, a rule may rewrite " +
    ...
    case JdkBoundaryFlow =>
      List(
        RemedyHint(
          FixKind.Universal,
          "insert the coercion at the seam, in the direction the DECLARATION " +
            "requires — never the reference node's type, which a position-blind retyping already moved"
        ),
        RemedyHint(FixKind.Parameterised,
                   "scope the retyping phase OUT of the declaration that must " +
                     "keep the JDK shape"
        )
      )
    case ConstructorTopology =>
      List(
        RemedyHint(
          FixKind.Universal,
          "promote the widest super call to the PRIMARY and delegate; where " +
    ...
    case PlatformHostileApi =>
      List(
        RemedyHint(
          FixKind.Parameterised,
          "drop the type and inject a replacement, or re-point the call " +
    ...
    case ReflectiveLookup =>
      List(
        RemedyHint(
          FixKind.Parameterised,
          "re-point the lookup at an EXPLICIT table, so the answer is a " +
            "declaration rather than a name"
        ),
        RemedyHint(
          FixKind.LibraryRule,
          "where the lookup's targets are knowable only for one library, a " +
    ...
    case OverloadDivergence =>
      List(
        RemedyHint(
          FixKind.Universal,
          "make the chosen alternative EXPLICIT — an ascription at the " +
            "argument javac's phase would have taken"
        ),
        RemedyHint(
          FixKind.Universal,
          "where the divergence cannot be decided, COUNT it: a resolver for " +
    ...
    case FrontendBlindSpot =>
      List(
        RemedyHint(
          FixKind.Universal,
          "narrow the lookup that absented — a broad `catch` around a " +
            "resolution turns an unknown into a DEFAULT, and the default is what ships"
        )
      )
    case UnmodelledNodeKind(_) =>
      List(
        RemedyHint(
          FixKind.Universal,
          "add the lowering arm, with its edge-case suite; the kind registry " +
            "is what says this one was never claimed"
        ),
        RemedyHint(
          FixKind.Parameterised,
          "until then, drop the declaration that uses it and inject a " +
    ...
    case AnnotationResidue =>
      List(
        RemedyHint(
          FixKind.Universal,
          "translate the annotation's arguments; emitting `@A` where Java " +
