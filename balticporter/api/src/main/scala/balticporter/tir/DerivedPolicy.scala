...
final case class DerivedPolicy(
  ...
  /** upstream key -> the symbol it named when the rows were RESOLVED, before any phase ran: a phase that renames the member (bean properties) cannot lose the row, since a `SymId` survives every
    * rename. Empty until [[resolved]].
    */
  ...
  /** upstream symbol fullNames (parameters `C#m#p`, methods and fields `C#m`) the reference spells at the opaque type `targetFqn` (an `OpaqueSpec.Target.Existing` FQN, or a Mint's `fqn`).
    */
  ...
  /** content digest: the fingerprint segment a deriving phase contributes — a reference edit that changes a derived row changes the emitted surface, so it must move the port map.
    */
...
object DerivedPolicy:
  ...
  enum Family:
    case OpaqueSlot, NullableMember, Parenless,
      /** a java accessor the reference keeps under its OWN name (`setInputProcessor` beside `inputProcessor`): the bean step leaves the pair alone
        */
      KeepName,
      /** a java getter / setter the reference spells as a `var`/`val` PROPERTY (`target` is its name): the bean step folds the pair as if configured — fluent setters included, under its own guards
        */
      Property, PropertySetter,
      /** a nullary java method the reference keeps WITH its `()` (`def size(): Int`): the arity step leaves it alone even where its detector would drop the parens
        */
      KeepParens,
      /** a method whose `Class<T>` parameter the reference replaces with a `[T: ClassTag]` bound: the class-tag step drops the parameter and adds the clause
        */
      ClassTagParam,
      /** a member java declares narrower than public that the reference ships public: the visibility step widens it
        */
      Public,
      /** a member the reference gives a `@targetName` (`add` beside `add`, one JVM name `addLabel`): `target` is that JVM name; the rename step annotates, the nullability step's erasure check reads
        * the two as distinct
        */
      TargetName,
      /** a java FIELD the reference declares under an underscore name (`var _fillX` for `fillX`, whose name the property took): `target` is that name; the rename step moves the field ahead of the
        * emitter's own `x$field` clash repair
        */
      FieldName,
  ...
  /** every key a symbol answers to: its bare `fullName`, and — for a method with a descriptor, or a parameter of one — the descriptor-qualified spelling (`C#m(long)`, `C#m(long)#p`), which is how a
    * row tells two OVERLOADS apart (`Attributes#get(long)` is nullable, `get(DynamicArray,long)` is not). A phase matches a policy key against this set, never against `fullName` alone.
    */
