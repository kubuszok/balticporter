...
object MemberRenamer:
  ...
  /** A request that was NOT applied, and why — the counted skip. `anchors` is empty for a refusal that is not about the world (a collision, a conflicting request).
    */
  ...
  /** Expand, screen, then rewrite ONCE. @return the rewritten program (identical, `eq`-wise, when nothing applied) and one [[Refusal]] per request that did not.
    */
  def rename(p: Program, graph: OverrideGraph, requests: List[Request], onCollision: OnCollision, out: DecisionLog): (Program, List[Refusal]) =
    ...
    requests.foreach { r =>
      if r.newName.isEmpty then refuse(r, "the requested name is empty, which names nothing")
      else if !p.owns(r.member) then
        refuse(r,
               "the request names a symbol this program REFERENCES and does not DECLARE, so " +
                 "there is no declaration to rename"
        )
      else
        val c0 = graph.closureOf(r.member)
        val c  = if r.detachedParents.isEmpty then c0
        else c0.copy(externalAnchors = c0.externalAnchors.filterNot((t, _) => r.detachedParents(t)))
    ...
    def sweepGroups(): Unit =
      ...
      while again do
        ...
        newly.foreach { r =>
          closures.remove(r)
          refuse(r,
                 s"another request in group `${r.group}` was refused, and a group is applied " +
                   "whole or not at all"
          )
        }
    ...
    closures.foreach((r, c) => c.members.foreach(m => claimants.getOrElseUpdate(m, collection.mutable.LinkedHashSet.empty) += r))
    claimants.foreach { (m, rs) =>
      ...
      if names.sizeIs > 1 then
        ...
        rs.foreach(r =>
          refuse(
            r,
            s"`$fqn` is claimed by ${rs.size} requests asking for different " +
    ...
    /** the collision pass over the SURVIVORS, from an empty table. Rebuilding rather than patching: `SuffixUntilFree` writes its answer per request, and patching would strand a survivor's component
      * on its RAW requested name — undetected, since nothing counts it. Re-running is what makes it honest: `collidersOf` reads PENDING assignments, so fewer of them can change who a member collides
      * with. The caller loops until no new refusal appears.
      */
    ...
    def collisionPass(): Unit =
      ...
      live.foreach { (r, c) =>
        ...
        def collidersOf(nm: String): List[SymId] =
          visible.flatMap(t => graph.membersOf(t)).distinct.filterNot(m => c.members.contains(m) || siblings.contains(m)).filter(m => eff(m) == nm && clashes(m))
        onCollision match
          case OnCollision.SuffixUntilFree =>
            var fresh = r.newName
            var fuel  = 64
            while collidersOf(fresh).nonEmpty && fuel > 0 do { fresh += "$"; fuel -= 1 }
            if collidersOf(fresh).nonEmpty then refuse(r, s"no free name near `${r.newName}` after 64 suffixes")
  ...
  /** A name composed entirely of Scala's operator characters — `+`, `*`, `<=`, `+=`, etc.
    *
    * SLS 1.1: an operator character is a Unicode Sm/So character, or one of the seven ASCII symbols `!#%&*+-/<=>?@\^|~` that are not letters, digits, underscores, whitespace, or delimiters.
    */
  ...
  /** Is this character a Scala operator character?
    *
    * Deliberately a WHITELIST matching SLS 1.1 rather than a blacklist of what is NOT an operator: a blacklist is what `isPlain` was, and the failure mode of getting it wrong is a name that reaches
    * the symbol table and emits text the parser cannot read.
    */
  ...
  /** Is this a valid Scala MEMBER NAME — either an alphanumeric identifier (letter/digit/`_`/`$`) or a symbolic one, or one of the `unary_` prefix names?
    *
    * Used by [[MemberRenameTransform]] to validate the rename target. This replaces the old `isPlain` and admits both families.
    */
