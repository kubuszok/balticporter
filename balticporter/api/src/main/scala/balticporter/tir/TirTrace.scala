...
object TirTrace:
  ...
  /** node kinds to trace, e.g. `balticporter.traceNode=Typed,Apply`. `*` traces every wrapped site. Matched against the node's simple class name (`Typed`, `Apply`, `ClassDef`).
    */
  def traces(kind: String): Boolean =
    DebugFlags.traceNode.contains("*") || DebugFlags.traceNode.contains(kind)

  /** How many stack frames of the constructing code to show. Two is enough to distinguish sixteen sites in two files; more is noise.
    */
  val Frames = 3
  ...
  /** the same, for a value that is not a `Tree` (a `TypeRepr`, a `Symbol`) — kind must be given since there is no common supertype to read a name from.
    */
