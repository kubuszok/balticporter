...
trait RunScope:
  ...
  /** Does this run EMIT the declarations of this top-level unit? `false` for a unit the run merely resolved against — a base module's. Asked with the unit symbol, which every phase can reach by
    * climbing a symbol's owner chain.
    */
  def emits(unit: SymId): Boolean

  /** The shared-surface SUBJECTS the folded manifest itself contributed to this phase's effective policy — `SurfaceFold.ownKeys`, or, for a phase no base has a counterpart for, all of its own
    * subjects. `None` means "no instance of this phase here": every key is a base's, and every rewrite it makes is one the base's own run made identically.
    */
  ...
  /** …the SAME question asked of a MEMBER, which is what a phase actually holds. [[emits]] takes a top-level unit (the run's classification granularity); every caller climbs the owner chain to it.
    * Written once here since the climb is fuel-bounded and a caller's own would be free to choose a different bound or stop short of a unit.
    */
...
object RunScope:
  ...
  /** the TOP-LEVEL unit a symbol belongs to. Fuel-bounded for [[Program.owned]]'s reason: a cycle in an owner chain must not hang a run, and a truncated climb answers about a symbol that is not a
    * unit — which [[RunScope.whole]] accepts and a real scope refuses, the direction that does not silently widen a rewrite.
    */
  ...
  /** WHAT THIS MODULE'S MANIFEST SAYS ABOUT PLATFORMS — see [[RunScope.platform]]. A record, not two members, since both are asked TOGETHER through `PortabilityCheck.rulesFor(targets, overrides)`. @param
    * targets backends this module is ported for @param verdictOverrides the port's own answers where it disagrees with the catalog's RECOMMENDATION, never its availability.
    */
  final case class PlatformPolicy(
    targets:          Set[balticporter.catalog.Platform],
    verdictOverrides: Map[balticporter.catalog.DiffId, Map[balticporter.catalog.Platform, balticporter.catalog.Verdict]] = Map.empty
  )
  ...
  /** a member of the REFERENCE port read VERBATIM for splicing (`AddMembersTransform.fromReference`): its name, declaration kind, whether it sits in the companion, its source, and the reference
  ...
  def of(
    ...
    referenceSource: Option[ReferenceSourceLookup] = scala.None
  ): RunScope =
    val p  = platform
    val s  = substituted
