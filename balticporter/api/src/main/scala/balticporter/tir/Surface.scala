...
trait Surface:
  ...
  /** the units this run converts — the roots [[owns]] climbs to, and the domain every whole-program index must be built over.
    */
  ...
  /** Every question this run could NOT answer, in the order they were asked.
    *
    * An `Unknown` is not automatically a failure — most of them are about a base type this module merely mentions. It becomes one when its answer SHAPED EMITTED TEXT, and only the consumer knows
    * that, which is why [[Gap.fatal]] is set by the asker and not here.
    */
...
object Surface:
  ...
  object Answer:
    extension [A](a: Answer[A])
      /** the published value, if there is one. `None` for [[Answer.Own]] too — a caller that wants a value must say what it does about an owned symbol, and "derive locally" is not `getOrElse`.
        */
  ...
  final case class TypeShape(
    ...
    /** WHY that one — `unique-root` | `widest-root` | `no-arg-root` | `promoted-nilary` | `synthesised-primary` | `not-funnelled` | `no-constructor`. It tells a dependent what would change the
      * answer, which is the reader's next question after `primary`.
      */
    primaryKind: String = "",
    /** `public` | `protected` | `private`. A synthesised primary is `protected` — narrow enough that no client calls a constructor Java never exposed, wide enough that a subclass in ANY module
      * reaches it from its `extends` clause.
      */
    primaryVis: String = "",
    ...
    /** the emitted type-parameter list with bounds — what an override must COPY for a base parent.
      */
    tparams: String = "",
    /** emitted parents, in emitted order. `diamondOverrides` picks the superclass; a dependent must pick the same one.
      */
    ...
    /** Was the primary SYNTHESISED — a constructor no Java declared? The one question a consumer must not answer by reading `disambiguator` or an empty parameter list: a class disambiguated by the
      * marker ALONE has an empty slot list and is still synthesised.
      */
  ...
  /** Render a type shape. Sorted throughout, and every DEFAULT is omitted: the payload is sparse by design, so a row that says nothing costs one empty column rather than twelve `k=default` pairs.
    */
  ...
  /** A descriptor from its own rendering. Not `Descriptor.total`: a slot the publisher could not spell is `Param.Unresolved` here too, and refusing the whole descriptor for it would turn one
