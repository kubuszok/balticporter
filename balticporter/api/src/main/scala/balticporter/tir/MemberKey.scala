...
final case class MemberKey(owner: String, name: String, descriptor: Option[Descriptor]):
  ...
  /** the string a policy author writes — `owner#name` or `owner#name(P1,P2)`. Round-trips through [[MemberKey.parse]].
    */
  def render: String = owner + "#" + name + descriptor.fold("")(d => "(" + d.render + ")")

  override def toString: String = render

  /** a key with no parameter list. Legal, and it means different things at different seams — see [[MemberKey]]'s companion for the one asymmetry this project decided on purpose.
    */
...
object MemberKey:
  ...
  /** Why a declared key is not a key at all. Distinct from "named nothing": a malformed key COULD never match, and reporting it as a typo (`NeverMatched`) sends its author looking for a member that
    * is right there.
    */
  final case class Malformed(key: String, what: String)

  /** Parse a declared policy key. Small grammar, REFUSED rather than best-effort'd — a lenient parse binds to the wrong member silently. `owner#name` (bare, every overload), `owner#name(P1,P2)` (one
    * overload), `owner#name()` (nilary, distinct from bare), `Outer$Inner#<init>(int)`, `Enum#CONSTANT#member` (last `#` separates). A type ARGUMENT (`Class<T>`) is refused as MALFORMED at the `<`,
    * since the grammar is the erased spelling.
    */
  def parse(key: String): Either[Malformed, MemberKey] =
    def bad(what: String) = Left(Malformed(key, what))

    /** The type-argument refusal, scoped to ONE region of the key. Deliberately not a test over the whole string: `<init>` is a member NAME with angle brackets in it, and a whole-string test refused
      * every constructor key in every manifest — the spec that caught it is the reason this is written per region rather than once at the top.
      */
    def noTypeArgs(region: String, at: Int, what: String): Option[Malformed] =
      ...
      if lt >= 0 then
        Some(
          Malformed(
            key,
            s"a type ARGUMENT at index ${at + lt} (`${region.substring(lt)}`) in the $what: " +
        ...
        Some(
          Malformed(key,
                    s"a `>` at index ${at + gt} in the $what, with no `<`: the key grammar has no " +
                      "type arguments"
          )
        )
      else scala.None

    /** a member NAME is an identifier, or one of the angle-delimited special names Java itself uses (`<init>`, and the engine's `<clinit>` / `<initblock>` beside it).
      */
    ...
    if hash < 0 then bad("no `#`: a member key is `owner#name`, or `owner#name(P1,P2)` for one overload")
      ...
      if owner.isEmpty then bad("the owner is empty: a member key names its declaring type in full")
        ...
        noTypeArgs(owner, 0, "owner").map(Left(_)).getOrElse {
          ...
          if open < 0 then
          ...
          else if !rest.endsWith(")") then bad("an unclosed parameter list: `(` with no matching `)` at the end of the key")
            ...
            if nm.isEmpty then bad("nothing between the `#` and the `(`: a member key names a member")
              ...
              badName(nm).map(Left(_)).getOrElse {
                noTypeArgs(inner, hash + 1 + open + 1, "parameter list").map(Left(_)).getOrElse {
                  ...
                  if parts.exists(_.isEmpty) then bad("an empty parameter between commas — a stray or trailing comma")
                  else
  ...
  /** Parse a MEMBER SEGMENT against an owner named separately — `dispose`, or `dispose()` for the nilary overload alone. A policy nested UNDER its owner has the halves already apart; splicing them at
    * the reader would build member identity from a string (forbidden by `PolicyKeyLintSpec`), so the splice lives here instead.
    */
  def parseIn(owner: String, member: String): Either[Malformed, MemberKey] =
    if owner.isEmpty then Left(Malformed(member, "the owner is empty: a member key names its declaring type in full"))
      ...
      Left(
        Malformed(
          member,
          "a `#` in a member segment whose owner is already named — write the " +
            "member alone (`dispose`), or one overload of it (`dispose()`)"
        )
      )
    else parse(spell(owner, member)).left.map(m => m.copy(key = member))

  /** The `owner#member` SPELLING of a segment key that [[parseIn]] could not parse. A malformed segment has no parsed key to [[render]], so reporting it bare would give `MergeablePolicy.subjectOf`
    * the wrong leading FQN and silently drop the finding under the run's own-keys filter.
    */
  ...
  /** the OVERLOAD SET a declared key belongs to — `owner#name`, or the key itself where it is not a member key at all. What a comparison groups by before asking [[mayNameSame]] pairwise: overlap is
    * not transitive (`bar(int)` and `bar(String)` meet only through a bare `bar`), so the group is the widest set that can possibly contain a disagreement and the pairwise test is what finds one.
    */
...
/** ONE parameter's SOURCE-LEVEL spelling. Not a JVM erasure, and that is sound: java forbids two methods with the same erasure, so source spelling is already injective within an overload set (`void
  * m(T)` vs `void m(String)` differ; `<X> void m(X)` vs `void m(Object)` is illegal). The descriptor needs only to be CONSISTENTLY DERIVED, keeping a key the string a policy author writes.
  */
enum Param:
  ...
  /** an array of something, rendered `<of>[]`. A Java vararg `T…` is an array reference, so it is `Arr(Named("T"))` and spells `T[]` — the same as a declared `T[]`, which is what Java's own overload
    * rules say it is.
    */
  case Arr(of: Param)

  /** one parameter the frontend could not name. POISONS the whole descriptor — see [[Descriptor.total]]: a key with one parameter guessed is a key that matches the wrong overload.
    */
  case Unresolved
...
/** A member's parameter spelling — the half of member identity `Symbol.fullName` does not carry. Two latent divergences closed here: an ARRAY parameter (Spoon spells `int[]`, the TIR's tycon name
  * spelled `Array` — both sides now spell it `int[]`, java's own form), and `equals(Object)` (the frontend retypes its parameter to `scala.Any`; the descriptor is read BEFORE that retyping, so it
  * stays `Object` — [[ofInfo]] is the one place the old `Any` answer survives on purpose).
  */
final case class Descriptor(params: List[Param]):
  ...
  /** DOES A DECLARED DESCRIPTOR NAME THIS ONE? — compared through [[Param.simple]], never `==`: a policy author copies the QUALIFIED spelling out of a report, while this grammar is SIMPLE. Arity
    * checked first (cheap, never ambiguous). Two overloads whose simple names collide across packages both match one key; the binder's `Ambiguous` refusal then names both, qualified.
    */
  ...
  /** does every parameter have a name? A descriptor with an [[Param.Unresolved]] in it is not a key and must never be stored on a `Symbol` — see [[Descriptor.total]].
    */
...
object Descriptor:
  ...
  /** ALL of them, or none. A descriptor with a single guessed parameter is a key that matches the WRONG overload, which is strictly worse than no key at all — the rule `PortMapTransform` already
    * stated for its own precise key, kept here so there is one place it lives.
    */
  def total(params: List[Param]): Option[Descriptor] =
    val d = Descriptor(params)
    Option.when(d.isTotal)(d)

  /** Java's primitives, by their own spelling. The classification is by NAME on both derivation paths — the parser's `isPrimitive` would be more direct but would let the two paths disagree, and a
    * spec pins that they never do.
    */
  ...
  /** …and the WRAPPER java names for each (JLS 5.1.7, plus `void`/`Void`, which only a class literal reaches — JLS 15.8.2). Keyed the same way as [[ValueClassPrimitives]] so the two cannot drift;
    * `void` has no boxing conversion and is in this map for the class-literal reading alone (`JS-E20`).
    */
  ...
  /** one written parameter → a [[Param]]. `int[][]` nests; an empty or `?` spelling is [[Param.Unresolved]].
    */
  ...
  def paramOfType(program: Program, t: TypeRepr): Param =
    def nameOf(s: SymId): Param =
      program.symbolOf(s).map(y => ValueClassPrimitives.getOrElse(y.fullName, y.name)).fold(Param.Unresolved)(paramOf)
    t match
      ...
      case TypeRepr.AppliedType(TypeRepr.TypeRef(_, s), List(arg)) if program.symbolOf(s).exists(_.fullName == "scala.Array") =>
  ...
  /** …for a symbol. `None` for anything that is not an executable — which for a FIELD is the COMPLETE answer and not a gap: `owner#name` is the total identity of a field, and a binder that reported
    * it as unresolved would produce a finding for every field in the program.
    */
