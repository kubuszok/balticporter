...
final case class Remedy(
  /** the globally-unique, kebab-case slug a manifest writes. Published API — see [[Remedy.badId]] for the grammar and why anything outside it is refused rather than normalised.
    */
  id: String,
  /** the `CheckReport` check whose residue this remedy drains — `IdiomCheck.Refused`, not a string literal, so a renamed lane is a compile error rather than a silently unwired claim.
    */
  lane: String,
  ...
  /** WHAT KIND OF THING THE SELECTION KEY NAMES — see [[Remedy.Subject]]. Defaulted to `OwnedMember`, which is what every remedy whose residue sits at a declaration this run emits wants, and which is
    * therefore the answer nothing has to state.
    */
  ...
  /** every kind this remedy names, [[alsoKinds]] included. `AnyKind` is IN it and is not a kind — [[overlaps]] is the only reader and treats it as the whole lane.
    */
  def kinds: Set[String] = (kind :: alsoKinds).toSet

  /** COULD THIS REMEDY AND THAT ONE ANSWER THE SAME ROW? — not "are they on the same lane". [[ResolutionPlan.selected]] dispatches by `(lane, kind)`, so two selections with DISJOINT kinds on one lane
    * are both live (e.g. `heap-pollution`'s `Acknowledged`/`Unacknowledged`); lane equality alone would report `ConflictingSelection` with no way to comply. `AnyKind` overlaps everything on its lane
    * by construction.
    */
...
object Remedy:
  ...
  /** the [[Remedy.kind]] of a remedy that answers EVERY kind in its lane — [[alsoKinds]] at a cardinality enumeration cannot reach: `portability(emitted)` files under an offending API's FQN, an OPEN
    * set with hundreds of values, so a per-location remedy there is about the LOCATION and cannot enumerate. Read only in [[answers]], so [[drain]]/[[appliedAt]] honour it uniformly.
    */
  val AnyKind: String = "*"
  ...
  enum Subject(val ownership: Ownership, val isType: Boolean):
    ...
    /** a bare type FQN, for a residue that is a fact about the TYPE and not about one of its members. Bound through `PolicyBinder.bindType`, so a `#` in such a key is `Malformed` with that seam's own
      * sentence rather than this one's.
      */
    case OwnedType extends Subject(Ownership.Owned, true)

    /** `owner#member` naming a member this program REFERENCES and does not declare — the one shape `Ownership.External` exists for, and the one where `ExternalOnly` is the WRONG refusal.
      */
    case ExternalMember extends Subject(Ownership.External, false)

  /** Why an id is not an id. Lower-case kebab, exactly as [[TransformFactory.name]] is, and for the same reason: it is what a `.conf` writes and what an error message lists, so it must survive a
    * round trip through a config file and a TSV column without quoting. Refused rather than normalised — a vocabulary that accepted `Wrap_Checked` and answered to `wrap-checked` would publish two
    * spellings of one published name and a port could not tell which one it wrote.
    */
  def badId(id: String): Option[String] =
    if id.isEmpty then Some("an empty remedy id names nothing")
    else if !id.forall(c => (c >= 'a' && c <= 'z') || (c >= '0' && c <= '9') || c == '-') then Some(s"'$id' is not lower-case kebab: a remedy id is [a-z0-9] and `-`, and it is published API")
    else if id.startsWith("-") || id.endsWith("-") || id.contains("--") then Some(s"'$id' has a leading, trailing or doubled `-`")
    else if !id.head.isLetter then Some(s"'$id' does not start with a letter")
    else scala.None

/** A PHASE OR CHECK THAT OFFERS REMEDIES. A trait beside [[Phase]] rather than a field on it, for [[IdiomPhase]]'s reason: a CHECK is not a phase at all, and half the residue lanes a remedy could
  * drain are plain objects the orchestrator calls. The vocabulary a RUN can act on is assembled from the sources the run actually holds.
  */
trait RemedySource:
  /** every remedy this source can carry out. Empty is the honest answer for a source that offers none, and is the default nothing has to state.
    */
  def remedies: List[Remedy]

/** EVERY REMEDY VISIBLE AT ONE MOMENT, by id — assembled, never enumerated in a table. Two sets: KNOWN (every remedy the engine/classpath ship — a `.conf` naming an id outside it is a [[ConfigError]]
  * at load) and ACTIVE (this run's sources — a known-but-inactive id is a policy finding, never silence). Refuses a duplicate id rather than picking one, like `TransformRegistry`.
  */
final class RemedyVocabulary private (
  val byId: Map[String, Remedy],
  /** id → the class name of the source that declared it, for the duplicate error and for the "your phase is not in this pipeline" finding.
    */
  ...
  /** union, with the same duplicate refusal — how a load-time KNOWN set is built from the engine's own catalogue plus whatever the classpath's factories declare.
    */
...
object RemedyVocabulary:
  ...
  /** …for a declarer that is not an instance — a `TransformFactory` speaking for the phase it would build, which is how the KNOWN set can be complete without constructing anything.
    */
  ...
  private def build(pairs: List[(Remedy, String)]): RemedyVocabulary =
    pairs.foreach { (r, who) =>
      Remedy.badId(r.id).foreach(why => throw ConfigError(s"remedy declared by $who", why))
    }
