...
/** The transform pipeline — shaped by Scala 3's COMPILER PLUGIN model (`Phase` ~ `PluginPhase`/`MiniPhase`: named, `runsAfter`/`runsBefore`, `transformX` hooks the framework traverses bottom-up).
  * Every hook runs with the whole-program `Program` in scope (`using`), so a transform can ask `usagesOf`/`callersOf`/`symbolOf` while rewriting. `transformType` applies at every type occurrence AND
  * every symbol's `info`.
  */
trait Phase:
  def name:       String
  ...
  /** What CATALOG ROWS this phase decided, and at which declarations, for the run in progress.
    *
    * Owned and drained exactly as [[decisions]] is, and for the same reason: a phase instance reused across two translations must not report the first run's citations as the second's.
    */
  ...
  /** Full-control entry point. Default applies the hooks below via the standard bottom-up traversal (MiniPhase-style), then rewrites symbol infos with `transformType` so signatures stay consistent
    * with the rewritten trees. Override for whole-program passes. The pipeline rebuilds the xref afterwards.
    */
  ...
  /** rewrite a type occurrence (e.g. java.util.List → scala List). Applied by the traversal at every constituent, bottom-up.
    */
...
/** Orders phases by `runsAfter`/`runsBefore` and runs them over the `Program`, rebuilding the xref index between phases (immutable-rebuild; optimize later).
  */
object Pipeline:
  ...
  def order(phases: List[Phase]): List[Phase] =
    ...
    while ready.nonEmpty do
      ...
      ready.remove(n)
      out += n
      edges(n).toList.sortBy(m => (instances(m).name, m)).foreach { m => indeg(m) -= 1; if indeg(m) == 0 then ready.add(m) }
  ...
  /** …and with the run's IDIOM LOG, which collects what every [[IdiomPhase]] CONSIDERED. Drained here for [[DecisionLog]]'s reason: a reused phase instance would otherwise double-report a
    * translation's candidates across `Determinism.Full`'s two runs. Unconditional over the phase list, so a census phase and its eventual transformer replacement are indistinguishable to it.
    */
  def runTraced(program: Program, phases: List[Phase], binder: PolicyBinder, catalog: balticporter.catalog.CatalogLog, rewrites: RewriteLog, idioms: IdiomLog): (Program, DecisionLog) =
    ...
    if unknown.nonEmpty then
      println(
        s"[balticporter] WARNING: skipPhases names no such phase: ${unknown.toList.sorted.mkString(", ")}" +
    ...
    val out = ordered.foldLeft(program) { (prog, phase) =>
      ...
      if DebugFlags.skips(phase.name) then
        ...
        if DebugFlags.tracePhases then
          println(
            s"[balticporter] phase '${phase.name}': ${next.units.size} units, ${next.symbols.all.size} symbols" +
  ...
  private def recordPatch(rewrites: RewriteLog, phase: Phase, before: Program, after: Program): Unit =
    ...
    if moved.nonEmpty then rewrites.record(Patch(phase.name, moved, phase match { case r: Rewrite => r.accountedBy; case _ => Set.empty }))

  /** the DECLARED types a definition WRITES DOWN — the record the emitter prints, as opposed to the symbol's `info`. `None` for a definition kind that declares no type of its own (a `ClassDef` has
    * parents rather than a declared type, and a `TypeDef`'s rhs is its bounds).
    */
  ...
  /** …compared across the phase, for one owned symbol. Present-on-BOTH-sides only, for `recordPatch`'s own reason: a declaration that is gone was dropped, not retyped.
    */
  ...
  /** print the TIR at a phase boundary. `balticporter.dumpOnly=<fqn>` narrows it to one unit — without that a whole-library dump is megabytes and nobody reads it.
    */
  private def dump(when: Set[String], phaseName: String, label: String, prog: Program): Unit =
    if when.contains(phaseName) || when.contains("*") then
      ...
      val body      = DebugFlags.dumpOnly match
        case Some(fqn) =>
          TirPrinter.unit(fqn, TirPrinter.Style.debug).getOrElse(s"<no unit named '$fqn' in this program (${prog.units.size} units)>")
...
/** The standard bottom-up traversal that fuses a phase's hooks over a tree. Children are transformed first, then the node's specific hook, then the `transformTerm` catch-all. Every type occurrence
  * (in a `TypeTree` or a term/definition `tpe`/`info`) is routed through `transformType`, applied bottom-up over the `TypeRepr`.
  */
object StandardTraversal:
  ...
  def mapType(ph: Phase, t: TypeRepr)(using Program): TypeRepr =
    val mapped: TypeRepr = t match
      ...
      case TypeRepr.AppliedType(tc, as) =>
        TypeRepr.AppliedType(mapType(ph, tc), if ph.preservesTypeArgsOf(tc) then as else as.map(mapType(ph, _)))
  ...
  def mapClassDef(ph: Phase, t: Tree.ClassDef)(using Program): Tree.ClassDef =
    ...
    ph.transformClassDef(
      t.copy(
        ...
        enumCases = t.enumCases.map(ec => ec.copy(ctorArgs = ec.ctorArgs.map(mapTerm(ph, _)), body = ec.body.map(mapStat(ph, _))))
      )
    )
  ...
  def mapTerm(ph: Phase, t: Term)(using Program): Term =
    val rebuilt: Term = t match
      ...
      case x: Tree.New =>
        ph.transformNew(x.copy(tpt = mapTpt(ph, x.tpt), tpe = mapType(ph, x.tpe), anon = x.anon.map(a => a.copy(body = a.body.map(mapStat(ph, _))))))
      case x: Tree.Lambda =>
        ph.transformLambda(
          x.copy(
            params = x.params.map(mapValDef(ph, _)),
            body = mapTerm(ph, x.body),
            tpe = mapType(ph, x.tpe),
            resultTpt = x.resultTpt.map(mapTpt(ph, _))
          )
        )
      case x: Tree.Block =>
      ...
      case x: Tree.Assign =>
        x.copy(
          lhs = mapTerm(ph, x.lhs),
          rhs = mapTerm(ph, x.rhs),
          tpe = mapType(ph, x.tpe),
      ...
      case x: Tree.NewArray    =>
        x.copy(elem = mapTpt(ph, x.elem), dims = x.dims.map(mapTerm(ph, _)), init = x.init.map(_.map(mapTerm(ph, _))), tpe = mapType(ph, x.tpe))
      case x: Tree.ForEach =>
        x.copy(binding = mapValDef(ph, x.binding), iterable = mapTerm(ph, x.iterable), body = mapTerm(ph, x.body), tpe = mapType(ph, x.tpe))
      case x: Tree.For =>
        x.copy(
          init = x.init.map(mapStat(ph, _)),
          cond = x.cond.map(mapTerm(ph, _)),
          update = x.update.map(mapStat(ph, _)),
          body = mapTerm(ph, x.body),
          tpe = mapType(ph, x.tpe)
        )
      case x: Tree.Try =>
        x.copy(
          resources = x.resources.map(mapValDef(ph, _)),
          body = mapTerm(ph, x.body),
          catches = x.catches.map(c => Tree.CatchCase(mapValDef(ph, c.param), mapTerm(ph, c.body))),
          finalizer = x.finalizer.map(mapTerm(ph, _)),
          tpe = mapType(ph, x.tpe)
        )
      case x: Tree.Match =>
        x.copy(
          scrutinee = mapTerm(ph, x.scrutinee),
      ...
      case x: Tree.MethodRef =>
        x.copy(qualifier = x.qualifier match { case Left(t) => Left(mapTpt(ph, t)); case Right(e) => Right(mapTerm(ph, e)) }, tpe = mapType(ph, x.tpe))
