...
final class OverrideGraph private (
  ...
  /** every TYPE this graph has a declaration for — owned classes, anonymous-class bodies and enum-constant bodies alike.
    */
  def types: Set[SymId] = nodes.keySet

  /** `t`'s parent type symbols, in declaration order (superclass first, as the tree carries them). Includes parents this program does NOT declare — [[externalParentsOf]] is the subset that matters to
    * an anchor decision.
    */
  ...
  /** `t`'s parents that this program has no declaration for — a JDK type, a classpath jar. Named by FQN, because that is the only handle there is for a type with no `ClassDef`.
    */
  def externalParentsOf(t: SymId): List[String] =
    nodes.get(t).map(_.parents.filterNot(nodes.contains).flatMap(p => program.symbolOf(p).map(_.fullName)).filter(_.nonEmpty)).getOrElse(Nil)

  /** the member declarations `t` itself declares — methods and fields both, since a rename must be held against a FIELD of the same name just as much as against a method.
    */
  def membersOf(t: SymId): List[SymId] = nodes.get(t).map(_.members).getOrElse(Nil)

  /** every strict ancestor of `t` this program DECLARES, transitively. Cycle-safe and memoised — this is the walk the emitter re-derives six times over, published once.
    */
  ...
  def descendantsOf(t: SymId): List[SymId] = descendantsCache.getOrElseUpdate(
    t, {
  ...
  /** the types a change to a member of `t` is VISIBLE in: `t` itself, everything above it and everything below it. What a collision test has to range over, and what a threading pass has to reach.
    */
  def relativesOf(t: SymId): List[SymId] = (t :: ancestorsOf(t) ++ descendantsOf(t)).distinct

  /** the type that DECLARES `m`, as this graph indexes it. `SymId.None` for a symbol that is not a member of any declared type (a local, a parameter, an external).
    */
  def ownerOf(m: SymId): SymId = memberOwner.getOrElse(m, SymId.None)

  /** `m`'s identity as an edge is keyed on — its name plus its parameter spelling. `None` for anything that is not an executable: a Java FIELD does not override, it SHADOWS, and the two are different
    * facts (`TirEmitter.resolveFieldShadowing` is the one that handles the other).
    */
  def signatureOf(m: SymId): Option[Signature] = sigCache.get(m).flatten

  /** the parent-type declarations `m` overrides or implements, transitively — interfaces included, and every one of them, since a diamond really does have two.
    */
  ...
  /** the declarations BELOW `m` that override or implement it — every subclass, every anonymous body, transitively.
    */
  ...
  /** THE CLOSURE — every declaration that must change together with `m`, plus the reasons this program may not be allowed to change them. The walk is the connected COMPONENT (not one hop up/down),
    * cycle-safe and order-independent. A non-executable member (e.g. a field) is its own closure with no external anchors.
    */
        ...
        while work.nonEmpty do
          ...
          if seen.add(x) then
            signatureOf(x).foreach { sig =>
              ...
              if ExternalSurface.javaLangObjectDeclares(sig) then anchors += (ExternalSurface.JavaLangObject -> sig.name)
  ...
  /** one derivation per symbol, memoised: `closureOf` asks for the same signature repeatedly and `Descriptor.ofInfo` walks a type every time it is asked.
    */
  ...
  private def matchingUp(a: SymId, sig: Signature, from: SymId): List[SymId] =
    membersOf(a).filter(inherited).filter(x => signatureOf(x).exists(s => s.matches(sig) || asSeenFrom(s, a, from).matches(sig)))

  /** IS THIS MEMBER IN AN OVERRIDE RELATION AT ALL? — JLS 8.2/8.4.8.1. Only `private` answers `no` outright (NOT INHERITED, 8.2): it is its own component, no ancestor can declare what it overrides.
    * `static` HIDES rather than overrides and still participates (a hiding pair must move together). A wrong answer here over-approximates the closure, which is the honest anchor — measured freezing
    * five components on `java.lang.Enum#getBundle`.
    */
  private def inherited(m: SymId): Boolean =
    !program.symbolOf(m).exists(_.flags.isPrivate)

  /** the members of DESCENDANT `d` that `sig` names, `sig` being declared in ancestor `a`. The mirror of [[matchingUp]]: it is the ANCESTOR's signature that carries the type parameters, so this
    * substitutes `sig` rather than the candidate.
    */
  private def matchingDown(d: SymId, sig: Signature, a: SymId): List[SymId] =
    val seen = asSeenFrom(sig, a, d)
    membersOf(d).filter(inherited).filter(x => signatureOf(x).exists(s => s.matches(sig) || s.matches(seen)))
  ...
  private def tparamSpelling(ancestor: SymId, from: SymId): Map[String, Param] =
    spellingCache.getOrElseUpdate(
      (ancestor, from), {
        val tps = program.definitionOf(ancestor).collect { case c: Tree.ClassDef => c.tparams.map(_.symbol) }.getOrElse(Nil)
        if tps.isEmpty then Map.empty
        else
          val subst = substOf(from)
          tps
            .flatMap(tp =>
              (subst.get(tp), program.symbolOf(tp).map(_.name)) match
  ...
  private def substOf(t: SymId): Map[SymId, TypeRepr] =
    substCache.getOrElseUpdate(t, ParentSubst.ofParents(nodes.get(t).map(_.parentTypes).getOrElse(Nil))(using program))

  private val ancestryCache = collection.mutable.Map.empty[SymId, (List[SymId], List[String])]

  /** every strict ancestor of `t`: the owned ones by symbol, the unparsed ones by FQN. Cycle-safe — a corrupt parent edge must not hang a phase.
    */
  private def ancestry(t: SymId): (List[SymId], List[String]) = ancestryCache.getOrElseUpdate(
    t, {
...
object OverrideGraph:
  ...
  /** one declared TYPE: classes, anonymous bodies and enum-constant bodies alike. `parentTypes` carries the same edges as `parents` WITH their type arguments, for reading an override edge across a
    * generic parent ([[OverrideGraph.asSeenFrom]]). Two lists since other consumers want the head symbol without re-deriving it.
    */
  ...
  final case class Signature(name: String, descriptor: Option[Descriptor], arity: Int, approximate: Boolean):
    /** the same member, across two types. Descriptors decide when both sides have one; otherwise arity does, and the side that lacked one is already flagged approximate.
      */
  ...
  final case class Closure(
    ...
    /** one sentence naming what froze it — the string a refusal reports, so an agent does not have to re-derive it from two sets. `None` when nothing did.
      */
    def anchorReason(program: Program): Option[String] =
      ...
      (ext, base) match
        ...
        case (e, Nil)   =>
          Some(
            s"the component reaches ${e.size} declaration(s) this program does not " +
              s"parse and cannot move: ${e.mkString(", ")}"
          )
        case (Nil, b) =>
          Some(
            s"the component reaches ${b.size} declaration(s) owned by a resolution " +
              s"root rather than by this module: ${b.mkString(", ")}"
          )
        case (e, b) =>
          Some(
            s"the component reaches ${e.size} unparsed declaration(s) " +
              s"(${e.mkString(", ")}) and ${b.size} owned by a resolution root (${b.mkString(", ")})"
          )

  /** Build the graph for one program.
    * @param external
    *   what is known about types this program did NOT parse — default knows only `java.lang.Object`, so every other unparsed parent anchors
    * @param baseUnits
    *   top-level units this run RESOLVES AGAINST but does not convert (`PortRun`'s foreign partition) — empty (default) means no closure is base-anchored.
    */
  def build(p: Program, external: ExternalSurface = ExternalSurface.default, baseUnits: Set[SymId] = Set.empty): OverrideGraph =
    given Program = p
  ...
  final private class Collector extends Phase:
    ...
    override def transformClassDef(t: Tree.ClassDef)(using Program): Tree.ClassDef =
      ...
      t.enumCases.foreach(ec => nodes += Node(ec.symbol, List(t.symbol), membersIn(ec.body), List(TypeRepr.TypeRef(TypeRepr.NoPrefix, t.symbol))))
      t
...
object ExternalSurface:
  ...
  /** one member of an unparsed type. `descriptor` absent means "this arity, whatever the parameter types" — which is what the engine's own arity-keyed channels carry, and which over-matches on
    * purpose.
    */
  ...
  /** `java.lang.Object`'s member set, which is fixed by the language. Above EVERY Java type whether or not the tree says so.
    */
  ...
  /** does the implicit root declare this? Asked by every closure, whatever the tree shows. Matched on the NAME alone, for `equals`'s sake: the frontend retypes a 1-argument `equals`'s parameter to
    * `scala.Any` before building the `MethodType` ([[Descriptor.ofInfo]]'s own note), so a descriptor comparison there would answer no and un-anchor the one member most likely to be overridden. The
    * direction of the error is refusal, which is the safe one.
    */
  ...
  /** …from the arity-keyed channel the emitter already threads (`RuntimePlan.concreteMembers`, `TirEmitter.externalConcrete`): FQN → `(name, params per clause)`. Arity-only, so it over-matches —
    * which for an anchor decision is the conservative direction.
    */
