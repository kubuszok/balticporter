...
final class MemberIndex(
  /** a LIST per key, not one entry. Two members can share one identity in this grammar — a class with two `static { }` blocks has two `<clinit>()`s — and a map would silently keep one of them, which
    * is the shape of every defect this index exists to prevent.
    */
  private val byKey: Map[MemberKey, List[MemberFacts]],
  /** every TYPE the frontend walked, by qualified name. A separate field, not `byKey.map(_.owner)` — a type with no executables would otherwise answer "not seen", which is wrong for `PolicyBinder`'s
    * structural test (frontend walked the owner, did not record the member).
    */
  ...
  /** every overload of `owner#name`, in a stable order — what a BARE key names, and what an ambiguity report has to list.
    */
  def overloads(owner: String, name: String): List[(MemberKey, MemberFacts)] =
    byKey.iterator.filter((k, _) => k.owner == owner && k.name == name).flatMap((k, fs) => fs.map(k -> _)).toList.sortBy(_._1.render)

  /** …for a key, precise or bare. A precise key names one identity; a bare key names the set. Matched through `Descriptor.matches` over the overload set, not a `Map` lookup on the whole `MemberKey`,
    * so a QUALIFIED parameter (as every report shows one) still names the obvious member.
    */
  def matching(k: MemberKey): List[(MemberKey, MemberFacts)] =
    if k.isBare then overloads(k.owner, k.name)
    else
      exact(k).map(f => k -> f) match
        case Nil  => overloads(k.owner, k.name).filter((k2, _) => k2.descriptor.exists(d => k.descriptor.exists(_.matches(d))))
        case hits => hits
...
/** What the frontend knew about one executable at the moment it walked it.
  * @param sym
  *   its interned symbol — `None` for a DROPPED member, the whole reason this exists rather than a `Set[MemberKey]` @param dropped did policy remove it? A dropped member is still an ANSWER —
  *   reporting it never-matched is the failure this index prevents.
  */
