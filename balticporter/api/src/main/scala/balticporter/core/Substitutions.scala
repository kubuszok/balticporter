...
/** Principled, typed directives for replacing constructs the port must NOT translate mechanically with ready-made Scala. [[dropTypes]] excludes a whole type (a Scala def at the same FQCN expected
  * from [[inject]] or a dependency); [[dropMethods]] drops one `DefDef`; [[inject]] copies ready-made Scala verbatim, surviving re-emit. Keys are fully-qualified. A key matching NOTHING is a silent
  * no-op — [[policyReport]]/[[matched]] exist to catch it.
  */
final case class Substitutions(
  ...
  /** `owner#name` drops EVERY overload; `owner#name(P1,P2)` (erased simple param names) drops one — overload precision is what makes constructors droppable (they all share `<init>`). PURE: "did this
    * key fire" is now `PolicyBinder`'s question, answered from the program and `MemberIndex`, not a mutable tally two runs might share.
    */
...
object Substitutions:
  ...
  /** every `.scala` file under `dir`, sorted; nothing when the directory does not exist. One body, here rather than in a check, since three layers ask the same question of an injection root (the run,
    * `SubstitutionCheck`, `SurfaceFold`) and the last lives below the engine.
    */
...
/** Marks a symbol the port must NOT emit, so INTERMEDIATE layers can still see it. Kept in the model with references resolved (dropping from the parse set would degrade the code around it); tagged so
  * a phase can rewrite a use into its replacement rather than leaving it as an ordinary reference. Nothing at that FQN is emitted by the declaration itself — only what the substitution supplies
  * (injected Scala, a rewrite, or nothing if every use was rewritten away).
  */
