...
final case class PolicyFinding(
  ...
  issue:  PolicyIssue,
  detail: String,
  /** WHOSE question this answers — see [[PolicyFinding.About]]. Defaults to the pre-existing behaviour, so every construction that predates the field says exactly what it always did.
    */
...
object PolicyFinding:
  ...
  enum About:
    ...
    /** the entry is fine and THIS RUN refused what it authorised, on evidence from this run's own program. Never filtered by key ownership: the module whose declarations caused it is the only module
      * that can act, and it is usually not the module that wrote the key.
      */
    case ThisRun
...
enum PolicyIssue(val label: String):
  ...
  /** the key matched, but the engine cannot prove the rewrite it authorises is the one intended. Not an error: a warning that the policy is relying on something unchecked.
    */
...
object PolicyReport:
  ...
  /** The never-fired report, derived from what the RUN BOUND — one row per key that did not. `PolicyBinder` lives in `balticporter.tir` and cannot produce a `PolicyReport` (`core` depends on `tir`,
    * not vice versa) — so the binder owns the ANSWERS and this owns the classification.
    */
  def fromBindings(records: Iterable[balticporter.tir.PolicyBinder.Record]): PolicyReport =
    import balticporter.tir.Binding
    PolicyReport(
      records.iterator.collect {
  ...
  /** The per-location REMEDY SELECTIONS that did not do what their author asked ([[balticporter.tir.ResolutionPlan.troubles]]), classified. Here for [[fromBindings]]'s reason: `ResolutionPlan` lives
    * in `tir` and cannot produce a `PolicyReport`.
    */
  ...
  /** The ARTIFACT declarations that answered nothing — `PortManifest.dependencies` entries no requirement in this module's own emitted code names (`DependencyCheck.unneeded`). Filed under the
  ...
  def fromDependencies(unneeded: Iterable[(balticporter.catalog.ArtifactDep, String)]): PolicyReport =
    PolicyReport(
      unneeded.iterator.map { (d, why) =>
        PolicyFinding(DependencySeam, DependencySetting, d.toString, PolicyIssue.NeverApplied, why)
      }.toList
    )

  /** the two strings [[fromDependencies]] files under, named so a reader of `findings.tsv` and a reader of the manifest are looking at the same words.
    */
  val DependencySeam    = "dependencies"
  val DependencySetting = "PortManifest.dependencies"

  /** [[balticporter.tir.NotBound]] → [[PolicyIssue]]. Adds NO case: the binder's five reasons are five answers to ONE question (did the key name anything) with only three reader actions; finer
    * distinctions survive in the DETAIL. `ExternalOnly` → `NeverMatched` (did nothing); `SyntheticTarget` → `Malformed` (could never have matched — not a typo).
    */
