...
object CommentScanner:
  ...
  /** one comment, with the offset it starts at. `end` is derived rather than stored: the text is the verbatim slice, so a second field could disagree with it.
    */
    ...
    while i < n do
      source.charAt(i) match
        ...
        case '/' if i + 1 < n && source.charAt(i + 1) == '*' =>
          ...
          val kind  =
            if i + 2 < n && source.charAt(i + 2) == '*' && !(i + 3 < n && source.charAt(i + 3) == '/') then TriviaKind.Javadoc
  ...
  /** The offset of the first character a JAVA COMPILER would read as code — whitespace and comments skipped, exactly as a scanner skips them. The whole definition of "file-leading": a comment is the
    * file's own iff no code precedes it. Not "before `package`" — a comment containing the word `package` would defeat a token search.
    */
