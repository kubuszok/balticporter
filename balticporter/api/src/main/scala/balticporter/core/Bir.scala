...
/** One Java enum constant → `case Name extends E(args)`. Constant-specific class bodies are not yet mechanized.
  */
...
enum RefKind:
  ...
  /** field on an enclosing instance from inside an ANONYMOUS/local class, which has no name for `Outer.this` — emitted bare, resolved by Scala's lexical scoping.
    */
  case EnclosingField
...
object BExpr:
  ...
  /** anonBody: Java anonymous subclass — `Some(empty)` for the super-type-token pattern `new X(...) {}`, `Some(members)` for bodies with fields/methods. formals: resolved ctor signature (drives
  ...
  /** init: instance-initializer blocks (the double-brace idiom) — plain statements in the Scala anonymous body.
    */
  ...
  /** Java lambda → Scala lambda. Left = block body (statements), Right = expression body. paramTypes carries the resolved SAM parameter types when available, so the printer can annotate them where
    * Scala can't infer against an overloaded target.
    */
  final case class Lambda(params: List[String], body: Either[List[BStmt], BExpr], paramTypes: List[BType] = Nil) extends BExpr

  /** Java method reference → eta-expandable Scala selection: `Owner.m` (static) or `recv.m` (bound instance). SAM conversion supplies the target type.
    */
  ...
  /** Unbound instance method reference `Type::m` → explicit lambda `(r: Type, p1: A1, ...) => r.m(p1, ...)` — receiver + formal types from the resolved executable.
    */
...
object BStmtK:
  ...
  /** Java `assert cond : msg` → Scala assert (NOTE: Java asserts are off by default at runtime; Scala's are on — a documented, deliberate divergence).
    */
  ...
  /** `scala.util.boundary { ... }` — target for translated break/continue. label: names the Label context param, letting an inner boundary's break target this one explicitly (the mixed break+continue
    * encoding).
    */
  final case class Boundary(body: List[BStmt], label: Option[String] = None) extends BStmtK

  /** `scala.util.boundary.break()` — exits the nearest enclosing Boundary, or the named one when `label` is set.
    */
...
final case class BCtor(
  ...
  body:      List[BStmt],
  /** the resolved param types of the super()/this() target ctor — disambiguates overloads of the same arity (ContentNode has three arity-1 ctors); None when there is no explicit super/this call.
    */
...
final case class BTypeDecl(
  ...
  /** non-static nested classes — emitted in the class body (Scala nested classes are inner by default, capturing the outer instance).
    */
