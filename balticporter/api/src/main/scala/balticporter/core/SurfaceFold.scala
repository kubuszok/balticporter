...
trait MergeablePolicy extends SurfacePolicy:
  ...
  /** Merge `later` — a NEARER manifest's instance of this same phase — into this one. `Left(why)` REFUSES (same key, different value); a refused pair stays in the pipeline, reported. @return the
    * merged phase, and the shared-surface SUBJECTS `later` contributed that this instance did not already hold — what the `governs` screen reads.
    */
  def mergedWith(later: Phase): Either[String, MergeablePolicy.Merged]

  /** Every shared-surface SUBJECT this instance's policy is keyed on — each key's leading FQN, via [[MergeablePolicy.subjectOf]]. Makes the `governs` screen reachable without a MERGE: a dependent
...
object MergeablePolicy:
  ...
  /** @param phase
    *   the merged instance — a NEW value; neither input is mutated.
    * @param added
    *   the subjects the later instance ADDS, as FQNs. Empty is a statement: "nothing this instance contributes is keyed on a name of the shared surface".
    */
  final case class Merged(phase: Phase, added: Set[String])

  /** The shared-surface SUBJECT a policy key names: its leading FQN, cut at `#`. One body, since `ManifestAgreement` already reads a `dropMethods` key the same way ([[PortManifest.covers]]'s
...
/** What folding a manifest's policy chain into ONE pipeline decided — derived, never configuration.
  * @param phases
  *   the effective pipeline (a merged phase sits at the BASE's position) @param absorbed fingerprints consumed by a merge @param refusals same-name pairs that could NOT merge, with why @param ownKeys
  *   phase → subjects the folded manifest contributed @param intrusions subjects added inside a base's claim — CANDIDATES screened against its published map.
  */
...
object SurfaceFold:
  ...
  enum Cause:
    ...
    /** the phase declares no [[MergeablePolicy]] and is not even a [[SurfacePolicy]], so the two policies cannot be COMPARED, let alone composed. See [[of]].
      */
    case Unverifiable
  ...
  /** A subject a nearer manifest ADDS inside `base`'s `governs` claim, which `base`'s own manifest does not account for — a CANDIDATE for `SurfaceIntrusion`, screened against `base`'s published map
    * by the layer that holds one.
    *
    * @param why
    *   the sentence the finding carries, minus the evidence the map supplies.
    */
  final case class Intrusion(phase: String, base: String, subject: String, why: String)

  /** THE fold. `chain` is furthest base first, `owner` is the manifest being folded (the last element, whose contributions [[SurfaceFold.ownKeys]] records). Identity dedup kept from the pre-merge
    * code path: inheriting one manifest through two paths runs its phases once.
    */
          ...
          case i =>
            ...
            outcome match
              case Outcome.Merged(merged, added) =>
                ...
                if m eq owner then ownKeys = ownKeys.updated(p.name, ownKeys.getOrElse(p.name, Set.empty) ++ added)
