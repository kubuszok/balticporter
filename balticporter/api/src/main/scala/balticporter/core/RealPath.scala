...
object RealPath:
  ...
  /** `p` with symlinks resolved, falling back to `toAbsolutePath.normalize` when it does not exist. `toAbsolutePath` matters: a RELATIVE fallback makes [[relativize]] throw. Catches `Exception`, not
    * `IOException`: `toRealPath` also throws `SecurityException`/`InvalidPathException`, and a narrower catch would let either kill a whole port's emission.
    */
  def of(p: Path): Path =
    try p.toRealPath()
  ...
  /** `p` relative to `root`, both realpathed. Throws for two paths with no common root, as `Path.relativize` does — a caller that can survive that must say so.
    */
