...
/** Discovered `TirFrontend` implementations, keyed by [[TirFrontend.name]].
  * `.conf` writes `input.frontend = "<name>"` and this registry resolves it.
  * Follows the same `ServiceLoader` pattern as `TransformRegistry`. */
