...
object Differences:
  ...
  val classes: List[Difference] = List(
    ...
    Difference(cId(22), "Java's THREE-PHASE applicability (strict, then loose, then varargs) has no Scala counterpart",
      ...
      Silent, Open, Predicted, Universal,
      "no symbol; two narrow FACES are closed — Visibility.decide restores javac's candidate set, TirEmitter.numericOverloadAscription closes exact-match-vs-widening", Unmechanised("no surface exists to owe a consult: modelling scala's own overload resolution well enough to predict a divergence is a compiler-sized project, and the affordable step is a conservative RISK COUNTER over the emitted candidate set — a check, not a dispatch")),
    Difference(cId(23), "most-specific tie-break: Scala prefers a NON-GENERIC alternative and Java does not",
      ...
      Silent, Open, Predicted, Universal, "no symbol — a sub-case of JS-C22 that needs its own test", Unmechanised("a sub-case of JS-C22, waiting on the same risk counter and on a test of its own")),
    ...
    Difference(cId(37), "`name()`, `values()` and `valueOf` are SYNTHESISED on every Java enum",
      ...
      Loud, Handled, el("T11"), Universal, "TirEmitter.enumDef's nameM, values and valueOf", Rendered("ClassDef")),
    Difference(cId(38), "a promoted enum constructor parameter IS a member, and `name` collides with `Enum.name()`",
      ...
      Loud, Handled, el("T11"), Universal, "TirEmitter.enumDef's hasName", Rendered("ClassDef")),
    Difference(cId(39), "`ordinal()` is part of every Java enum's surface, mentioned or not",
      ...
      Loud, Handled, el("T13"), Universal, "TirEmitter.enumDef's hasOrdinal", Rendered("ClassDef")),
