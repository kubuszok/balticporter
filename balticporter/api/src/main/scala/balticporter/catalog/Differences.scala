...
object Differences:
  ...
  private val notYetC = Unmechanised(
    "area C obligations are not declared: most JS-C rows are decided by PHASES, and a per-phase " +
      "citation is only meaningful once each phase names the declarations it decided about")
  ...
  val expressions: List[Difference] = List(
    ...
    Difference(eId(5), "the conditional operator's type is computed, not the syntactic lub of its branches",
      ...
      Mixed, Partial("Tree.If carries the JLS-computed type and the emitter discards it; only the null-branch shape is handled"),
      Predicted, Universal, "Tir.Tree.If's `tpe` field; TirEmitter's `case Tree.If(c, th, el, _, _)` drops it", Lowered("CtConditional", Dispatch.Expression)),
  ...
  val classes: List[Difference] = List(
    Difference(cId(1), "a Java `static` is INHERITED by every subclass; a Scala companion inherits nothing",
      ...
      Loud, Handled, el("T14"), Universal, "SpoonTir.staticCallQualifier, reading the minter's owner rather than the written name", notYetC),
    Difference(cId(2), "interface constants are inherited through `implements`",
      ...
      Loud, Handled, el("T14"), Universal, "SpoonTir.declaringStaticType's BFS over the whole inheritance closure", notYetC),
    Difference(cId(3), "static HIDING is resolved by the STATIC type of the qualifier",
      ...
      Silent, Handled, el("T14"), Universal, "SpoonTir.declaringStaticType (nearest declarer wins); TirEmitter.staticOwnersOf excludes the class's own static names", notYetC),
    Difference(cId(4), "a subclass field SHADOWS a superclass field — two storage cells, not one",
      ...
      "TirEmitter.resolveFieldShadowing — the trigger is NAME membership in the inherited instance members, so a same-type field fires too", notYetC),
    Difference(cId(5), "a static nested constant reached through a SUBCLASS's name, and `import static`",
      ...
      NoImpact, Handled, el("T14"), Universal, "SpoonTir.nestedPath and SpoonTir.staticFieldAccess", notYetC),
    ...
    Difference(cId(12), "a FORWARD REFERENCE to a later field is a Java compile error and a silent default read in Scala",
      ...
      Silent, Open, Predicted, Universal,
      "nothing detects it; order preservation is the only mitigation, and TirEmitter.orderBody's `forward reference` note is about promoted LOCALS", notYetC),
    Difference(cId(13), "JLS 12.5's instance-creation sequence, whole",
      ...
      el("C12"), Universal, "CtorFunnel; SpoonTir.classDef's step-4 sort; TirEmitter.orderBody", notYetC),
    Difference(cId(14), "the superclass constructor runs before this class's field initialisers",
      ...
      NoImpact, Handled, el("C1"), Universal, "CtorFunnel's primary promotion; TirEmitter.lowerCtors splicing plan.primaryBody", notYetC),
    Difference(cId(15), "a virtual call from a constructor sees UNINITIALISED subclass state",
      ...
      NoImpact, NonDiff("an identical hazard on both sides"), NoTwin, NoFix, "no symbol — a JVM fact", notYetC),
    Difference(cId(16), "instance initialiser blocks",
      "JLS 8.6", "UNCITED — a statement in the class body is the image",
      Silent, Handled, NoTwin, Universal, "SpoonTir.classDef's CtAnonymousExecutable harvest; TirEmitter.isInitBlock", notYetC),
    Difference(cId(17), "double-brace initialisation is an anonymous subclass plus an instance initialiser",
      ...
      el("T1"), Universal, "SpoonTir.anonClass's non-static CtAnonymousExecutable arm", notYetC),
    Difference(cId(18), "field initialisers and instance-init blocks are ONE step-4 sequence in textual order, not two buckets",
      ...
      Silent, Handled, el("C12"), Universal,
      "SpoonTir.classDef's `(fields ++ initBlocks).sortBy(posKey)`; TirEmitter.orderBody's isStep4", notYetC),
    Difference(cId(19), "`this(...)` / `super(...)` in a secondary constructor — Scala's secondary constructors cannot call `super`",
      ...
      Loud, Handled, el("C3"), Universal, "CtorFunnel's promotion; TirEmitter.orderBody's delegateTarget post-order; OmissionCheck counts the residue", notYetC),
    Difference(cId(20), "the implicit `super()` and the default constructor",
      ...
      Loud, Handled, el("C8"), Universal, "TirEmitter.orderBody's paramfulPrimary; CtorFunnel.delegationOnlyNilary", notYetC),
    Difference(cId(21), "constructor overload resolution, against Scala's textual-anteriority rule",
      ...
      el("C8"), Universal, "TirEmitter.orderBody's delegation-topological sort", notYetC),
    Difference(cId(22), "Java's THREE-PHASE applicability (strict, then loose, then varargs) has no Scala counterpart",
      ...
      Silent, Open, Predicted, Universal,
      "no symbol; two narrow FACES are closed — Visibility.decide restores javac's candidate set, TirEmitter.numericOverloadAscription closes exact-match-vs-widening", notYetC),
    Difference(cId(23), "most-specific tie-break: Scala prefers a NON-GENERIC alternative and Java does not",
      ...
      Silent, Open, Predicted, Universal, "no symbol — a sub-case of JS-C22 that needs its own test", notYetC),
    Difference(cId(25), "`override` is mandatory in Scala and absent in Java",
      ...
      Loud, Handled, NoTwin, Universal, "SpoonTir.overridesInherited setting Flags.isOverride; TirEmitter.mods", notYetC),
    Difference(cId(27), "covariant return types",
      "JLS 8.4.8.3", "UNCITED — the same rule",
      NoImpact, NonDiff("the same rule on both sides"), NoTwin, NoFix, "no symbol — a shared rule", notYetC),
    Difference(cId(28), "bridge methods",
      "JLS 13.1", "UNCITED — a source-to-source port never sees one",
      NoImpact, NonDiff("bridges are a binary artefact and this engine emits source"), NoTwin, NoFix, "no symbol", notYetC),
    Difference(cId(29), "a static NESTED class and an INNER class are different types, and only one is path-dependent",
      ...
      Mixed, Handled, NoTwin, Universal,
      "TirEmitter.tpe's cascade — nestedPath for a static nested type, a projection for an inner one, with namedInner opting out at `extends`/`new`", notYetC),
    Difference(cId(30), "method-LOCAL named classes",
      ...
      el("T9"), Universal, "SpoonTir.stmtKind's refusal arm, through SpoonTir.unsupported", notYetC),
    Difference(cId(31), "anonymous class construction and capture",
      "JLS 15.9.5", "UNCITED — the same construct, with a synthesised name",
      Silent, Handled, el("T1"), Universal, "SpoonTir.anonClass", notYetC),
    Difference(cId(32), "a captured local must be EFFECTIVELY FINAL in Java",
      ...
      NoImpact, NonDiff("Scala accepts strictly more"), NoTwin, NoFix, "no symbol", notYetC),
    Difference(cId(33), "the interface default-method diamond is resolved by JLS's two rules, not by trait linearization",
      ...
      el("T7"), Universal, "TirEmitter.diamondOverrides, whose `sup` is the superclass and whose mixins are all in the tail", notYetC),
    Difference(cId(34), "interface statics are not inherited AT ALL, not even by an implementor",
      ...
      Loud, Handled, el("T14"), Universal, "TirEmitter's companion re-export lists", notYetC),
    Difference(cId(35), "interface `private` methods",
      "JLS 9.4", "UNCITED — a private trait member is the image",
      NoImpact, NonDiff("a direct counterpart exists"), NoTwin, NoFix, "no symbol", notYetC),
    Difference(cId(36), "an interface field is implicitly `public static final`",
      ...
      Loud, Handled, el("T14"), Universal, "SpoonTir.fieldFlags with SpoonTir.declaringStaticType's interface edge", notYetC),
    Difference(cId(37), "`name()`, `values()` and `valueOf` are SYNTHESISED on every Java enum",
      "JLS 8.9.3", "UNCITED — a Scala 3 `enum` has a different, non-identical surface",
      Loud, Handled, el("T11"), Universal, "TirEmitter.enumDef's nameM, values and valueOf", notYetC),
    Difference(cId(38), "a promoted enum constructor parameter IS a member, and `name` collides with `Enum.name()`",
      ...
      Loud, Handled, el("T11"), Universal, "TirEmitter.enumDef's hasName", notYetC),
    Difference(cId(39), "`ordinal()` is part of every Java enum's surface, mentioned or not",
      ...
      Loud, Handled, el("T13"), Universal, "TirEmitter.enumDef's hasOrdinal", notYetC),
    Difference(cId(40), "enum constants with PER-CONSTANT class bodies",
      ...
      el("T8"), Universal, "SpoonTir.enumCase, which collects only CtField and CtMethod", notYetC),
    Difference(cId(42), "`EnumMap`/`EnumSet` GUARANTEE declaration-order iteration",
      ...
      "CollectionsTransform.typeMap has no EnumMap/EnumSet key, so the types pass through — and no boundary check can see an iteration-ORDER regression", notYetC),
    Difference(cId(43), "Java `record`",
      ...
      Predicted, Universal, "SpoonTir.classDef's CtClass arms take it, and SpoonTir.typeFlags has no isRecord", notYetC),
    Difference(cId(44), "`sealed` / `non-sealed` / `permits`",
      "JLS 8.1.1.2, 9.1.1.4", "UNCITED — Scala has no `non-sealed` and no explicit `permits`",
      Silent, Open, Predicted, Universal,
      "SpoonTir.typeFlags never populates Flags.isSealed, so a sealed hierarchy ships as an ordinary open class", notYetC),
    Difference(cId(45), "a `final` field's JMM safe-publication guarantee",
      "JLS 17.5", "UNCITED — a `val` carries the same guarantee",
      NoImpact, Handled, NoTwin, Universal, "SpoonTir.fieldFlags — isFinal/isMutable off the FINAL modifier", notYetC),
    Difference(cId(46), "Java has TWO name namespaces and Scala has one",
      "JLS 6.5", "UNCITED — one namespace, resolved innermost-first",
      Loud, Handled, NoTwin, Universal, "TirEmitter.resolveMemberClashes and TirEmitter.resolveFieldShadowing; MemberRenamer for policy renames", notYetC),
    Difference(cId(47), "package-private becomes `private[pkg]`",
      "JLS 6.6.1", "UNCITED — a qualified `private` is the image",
      Loud, Handled, el("T12"), Universal, "Visibility.decide's isPackagePrivate branch; TirEmitter.vis", notYetC),
    Difference(cId(48), "Java's `protected` also grants SAME-PACKAGE access, and accessibility is an input to overload resolution",
      ...
      Loud, Handled, el("T12"), Universal, "Visibility.decide's isProtected branch, with overrideTarget and qualifierResolves", notYetC),
    Difference(cId(49), "Java's `private` reaches the whole enclosing TOP-LEVEL class; Scala's reaches only the template",
      ...
      "TirEmitter.privateQualifier — Visibility.decide returns a bare private and the emitter qualifies it at a nested owner", notYetC),
    Difference(cId(50), "Java's default access is package-private and Scala's is public — INVERTED",
      ...
      Silent, Handled, el("T12"), Universal, "Visibility.decide — the same branch as JS-C47", notYetC),
  )
