...
object Differences:
  ...
  val expressions: List[Difference] = List(
    Difference(eId(1), "`==`/`!=` are REFERENCE identity in Java; Scala's `==` calls `equals`",
      ...
      Silent, Handled, Rule44, Universal, "SpoonTir.referenceIdentity"),
    Difference(eId(2), "postfix/prefix `++`/`--` USED AS A VALUE yields the value before the update",
      ...
      Silent, Handled, Rule44, Universal, "SpoonTir.exprNoCast POSTINC/PREINC -> Tree.IncDec; TirEmitter's Tree.IncDec arm"),
    Difference(eId(3), "compound assignment narrows implicitly to the left-hand type — STATEMENT position",
      ...
      Silent, Handled, Predicted, Universal, "SpoonTir.stmtKind's CtOperatorAssignment arm, via SpoonTir.primRank"),
    Difference(eId(4), "compound assignment narrows implicitly to the left-hand type — EXPRESSION position",
      ...
      "SpoonTir.exprNoCast's CtOperatorAssignment arm — no primRank, no narrowing, and no coerce either"),
    Difference(eId(5), "the conditional operator's type is computed, not the syntactic lub of its branches",
      ...
      Predicted, Universal, "Tir.Tree.If's `tpe` field; TirEmitter's `case Tree.If(c, th, el, _, _)` drops it"),
    Difference(eId(6), "a primitive cast is a CONVERSION in Java and an assertion in Scala once a phase has retyped the value",
      ...
      el("K17"), Universal, "TirEmitter's Tree.Typed arm and TirEmitter.castTarget, which realigns only wildcard arguments"),
    Difference(eId(7), "binary numeric promotion does not reach a GENERIC call boundary",
      ...
      el("X2"), Parameterised, "TestFrameworkTransform.promote"),
    Difference(eId(8), "`&`/`|`/`^` on `boolean` are non-short-circuit and `&&`/`||` are not",
      ...
      NoImpact, NonDiff("verified same bytecode both sides"), NoTwin, NoFix, "SpoonTir.opText BITAND/BITOR/BITXOR vs AND/OR"),
    Difference(eId(9), "a shift distance is masked to the operand's width",
      ...
      NoImpact, NonDiff("shared JVM instructions"), NoTwin, NoFix, "SpoonTir.opText SL/SR/USR"),
    Difference(eId(10), "integer division and remainder, and the floating-point edge cases",
      ...
      NoImpact, NonDiff("shared JVM instructions"), NoTwin, NoFix, "SpoonTir.opText DIV/MOD"),
    Difference(eId(11), "an array store evaluates its operands before the bounds check",
      ...
      NoImpact, NonDiff("verified on both compilers"), NoTwin, NoFix, "SpoonTir.exprNoCast's CtArrayWrite arm -> Tree.ArrayAccess"),
    Difference(eId(12), "the receiver's null check happens after the arguments are evaluated",
      ...
      NoImpact, NonDiff("the order is preserved by construction"), NoTwin, NoFix, "SpoonTir.invocation"),
    Difference(eId(13), "`instanceof` on `null` is false and never throws",
      "JLS 15.20.2", "SLS 12.1 — `isInstanceOf` is false for `null`",
      NoImpact, Handled, NoTwin, Universal, "SpoonTir.exprNoCast INSTANCEOF -> Tree.InstanceOf"),
    Difference(eId(14), "string concatenation with a NON-`String` left operand",
      ...
      Loud, Handled, Predicted, Universal, "SpoonTir.stringify, gated on isStringConcat && !isStringTyped(left)"),
    Difference(eId(15), "an assignment IS an expression, with the assigned value",
      ...
      Universal, "SpoonTir.exprNoCast's CtAssignment arm -> Tree.Block(List(Assign), lhs)"),
  )
  ...
  val statements: List[Difference] = List(
    Difference(sId(1), "an unlabelled `break`/`continue` binds LEXICALLY to the innermost enclosing loop or switch",
      ...
      Silent, Handled, el("F1"), Universal, "Jumps.breaksOut/continuesIn/jumpsTo; TirEmitter.loopWithJumps"),
    Difference(sId(2), "a LABEL sits on any statement, not only on a loop",
      ...
      Silent, Handled, el("F1"), Universal, "Tir.Tree.Labeled; TirEmitter's Tree.Labeled arm and labelNeedsBoundary"),
    Difference(sId(3), "a `boundary` the emitter INTERPOSES steals the enclosing loop's un-annotated jumps",
      ...
      Silent, Handled, el("F2"), Universal, "TirEmitter.interposes feeding TirEmitter.loopWithJumps"),
    Difference(sId(4), "switch FALLTHROUGH runs the next case's statements",
      "JLS 14.11.3", "UNCITED — a `match` arm never falls through",
      Silent, Handled, el("F3"), Universal, "SpoonTir.switchStmt's tail-duplication closures"),
    Difference(sId(5), "a `switch` with no `default` FALLS OUT; a `match` with no `case _` throws",
      ...
      Silent, Handled, Rule44, Universal, "SpoonTir.switchStmt's synthesised fall-out arm"),
    Difference(sId(6), "an unlabelled `break` in the MIDDLE of a case ends the CASE",
      ...
      Silent, Handled, el("F3"), Universal, "TirEmitter.matchStr and TirEmitter.caseNeedsBoundary"),
    Difference(sId(7), "only an UNLABELLED trailing `break` terminates a case; a labelled one leaves the LOOP",
      ...
      Silent, Handled, Rule44, Universal, "SpoonTir.switchStmt's `case (b: CtBreak) :: _ if b.getTargetLabel == null`"),
    Difference(sId(8), "a `null` selector throws NPE IMPLICITLY — a classic switch has no `case null` to opt out with",
      ...
      Silent, Handled, el("F6"), Universal, "TirEmitter.matchStr's synthesised `case null` throw; SwitchNullCheck"),
    Difference(sId(9), "switch EXPRESSIONS and `yield`",
      ...
      Predicted, Universal, "SpoonTir.exprNoCast's refusal arm; SpoonTir.stmtKind's refusal arm"),
    Difference(sId(10), "pattern and record switch, with sealed exhaustiveness",
      ...
      Predicted, Universal, "SpoonTir.switchStmt reads getCaseExpressions through `expr`"),
    Difference(sId(11), "a translated CATCH swallows a translated JUMP — `boundary.Break` is a `RuntimeException`",
      ...
      Silent, Handled, el("F4"), Universal, "TirEmitter.tryStr's guard, via Jumps.catchesBreak; BreakCatchCheck"),
    Difference(sId(12), "a `finally` completing abruptly DISCARDS the try's own abrupt completion",
      ...
      Predicted, Universal, "TirEmitter.tryStr's `fl`"),
    Difference(sId(13), "try-with-resources closes on ANY completion, in reverse order, before this try's own catch",
      ...
      Silent, Handled, el("F5"), Universal, "TirEmitter.tryStr -> resourceStr; TryResourceCheck"),
    Difference(sId(14), "multi-catch `A | B`",
      "JLS 14.20", "UNCITED — a union type in the pattern",
      NoImpact, Handled, Predicted, Universal, "SpoonTir.tryStmt's getMultiTypes reduce to OrType"),
    Difference(sId(15), "the enhanced-for expression is evaluated ONCE, and arrays and `Iterable` differ",
      ...
      NoImpact, Handled, NoTwin, Universal, "SpoonTir's CtForEach -> Tree.ForEach; TirEmitter emits the iterable once"),
    Difference(sId(16), "the enhanced-for BINDING may be reassigned in the body, and may be declared at a supertype",
      ...
      Loud, Handled, el("K7"), Universal, "TirEmitter's Tree.ForEach arm — widenedBinding and reassignsBinding"),
    Difference(sId(17), "the classic `for`'s UPDATE runs on `continue`, and its `ForInit` scopes to the loop",
      ...
      Silent, Handled, Predicted, Universal, "TirEmitter's Tree.For arm — the update is outside the per-iteration boundary"),
    Difference(sId(18), "`do`-`while` — Scala 3 removed it",
      "JLS 14.13", "UNCITED — Scala 3 has no `do`-`while`; `while ({ body; cond }) ()` is the image",
      Silent, Handled, Predicted, Universal, "SpoonTir's CtDo -> Tree.DoWhile; TirEmitter's Tree.DoWhile arm"),
    Difference(sId(19), "definite assignment for LOCALS — Java rejects a read before assignment",
      ...
      el("C10"), Universal, "TirEmitter.valDefStr's fieldOfAClass gate and defaultFor"),
    Difference(sId(20), "redeclaring a local in a nested block",
      "JLS 14.4.3", "UNCITED — Scala permits the shadowing Java rejects",
      NoImpact, NonDiff("javac rejects the input shape, so it never reaches the engine"), NoTwin, NoFix, "SpoonTir.defineLocal"),
    Difference(sId(21), "`return` inside a lambda body returns from the LAMBDA, not from the method",
      ...
      Mixed, Handled, Rule44, Universal, "TirEmitter's Tree.Lambda arm and returnsIn -> a nested `def`"),
    Difference(sId(22), "`synchronized` as a statement",
      "JLS 14.19", "UNCITED — `.synchronized`, with the same monitor bytecode",
      NoImpact, Handled, NoTwin, Universal, "SpoonTir's CtSynchronized -> Tree.Synchronized"),
    ...
    Difference(sId(24), "`throw null`",
      ...
      NoImpact, NonDiff("shared JVM instruction"), NoTwin, NoFix, "SpoonTir's CtThrow -> Tree.Throw"),
    Difference(sId(25), "Java REJECTS unreachable code and Scala allows it — composed with `break`",
      ...
      Universal, "TirEmitter.endsInInfiniteLoop, each arm excluding a loop that breaks out"),
  )
  ...
  val classes: List[Difference] = List(
    Difference(cId(1), "a Java `static` is INHERITED by every subclass; a Scala companion inherits nothing",
      ...
      Loud, Handled, el("T14"), Universal, "SpoonTir.staticCallQualifier, reading the minter's owner rather than the written name"),
    Difference(cId(2), "interface constants are inherited through `implements`",
      ...
      Loud, Handled, el("T14"), Universal, "SpoonTir.declaringStaticType's BFS over the whole inheritance closure"),
    Difference(cId(3), "static HIDING is resolved by the STATIC type of the qualifier",
      ...
      Silent, Handled, el("T14"), Universal, "SpoonTir.declaringStaticType (nearest declarer wins); TirEmitter.staticOwnersOf excludes the class's own static names"),
    ...
    Difference(cId(5), "a static nested constant reached through a SUBCLASS's name, and `import static`",
      ...
      NoImpact, Handled, el("T14"), Universal, "SpoonTir.nestedPath and SpoonTir.staticFieldAccess"),
    ...
    Difference(cId(7), "JLS's class-initialisation TRIGGER list vs Scala's uniform accessor trigger",
      ...
      NoImpact, NonDiff("no observable difference except through JS-C08"), NoTwin, NoFix, "no symbol — see JS-C08"),
    Difference(cId(8), "a CONSTANT VARIABLE is inlined, so reading it never triggers class initialisation",
      ...
      Silent, Handled, Rule44, Universal, "TirEmitter.isJavaConstant and its `inline val` rendering"),
    ...
    Difference(cId(10), "circular class initialisation delivers DEFAULT values on the same thread",
      ...
      NoImpact, NonDiff("shared JVM mechanism"), NoTwin, NoFix, "no symbol — a JVM fact"),
    ...
    Difference(cId(13), "JLS 12.5's instance-creation sequence, whole",
      ...
      el("C12"), Universal, "CtorFunnel; SpoonTir.classDef's step-4 sort; TirEmitter.orderBody"),
    Difference(cId(14), "the superclass constructor runs before this class's field initialisers",
      ...
      NoImpact, Handled, el("C1"), Universal, "CtorFunnel's primary promotion; TirEmitter.lowerCtors splicing plan.primaryBody"),
    Difference(cId(15), "a virtual call from a constructor sees UNINITIALISED subclass state",
      ...
      NoImpact, NonDiff("an identical hazard on both sides"), NoTwin, NoFix, "no symbol — a JVM fact"),
    Difference(cId(16), "instance initialiser blocks",
      "JLS 8.6", "UNCITED — a statement in the class body is the image",
      Silent, Handled, NoTwin, Universal, "SpoonTir.classDef's CtAnonymousExecutable harvest; TirEmitter.isInitBlock"),
    Difference(cId(17), "double-brace initialisation is an anonymous subclass plus an instance initialiser",
      ...
      el("T1"), Universal, "SpoonTir.anonClass's non-static CtAnonymousExecutable arm"),
    ...
    Difference(cId(19), "`this(...)` / `super(...)` in a secondary constructor — Scala's secondary constructors cannot call `super`",
      ...
      Loud, Handled, el("C3"), Universal, "CtorFunnel's promotion; TirEmitter.orderBody's delegateTarget post-order; OmissionCheck counts the residue"),
    Difference(cId(20), "the implicit `super()` and the default constructor",
      ...
      Loud, Handled, el("C8"), Universal, "TirEmitter.orderBody's paramfulPrimary; CtorFunnel.delegationOnlyNilary"),
    Difference(cId(21), "constructor overload resolution, against Scala's textual-anteriority rule",
      ...
      el("C8"), Universal, "TirEmitter.orderBody's delegation-topological sort"),
    Difference(cId(22), "Java's THREE-PHASE applicability (strict, then loose, then varargs) has no Scala counterpart",
      ...
      Silent, Open, Predicted, Universal,
      "no symbol; two narrow FACES are closed — Visibility.decide restores javac's candidate set, TirEmitter.numericOverloadAscription closes exact-match-vs-widening"),
    Difference(cId(23), "most-specific tie-break: Scala prefers a NON-GENERIC alternative and Java does not",
      ...
      Silent, Open, Predicted, Universal, "no symbol — a sub-case of JS-C22 that needs its own test"),
    Difference(cId(25), "`override` is mandatory in Scala and absent in Java",
      ...
      Loud, Handled, NoTwin, Universal, "SpoonTir.overridesInherited setting Flags.isOverride; TirEmitter.mods"),
    Difference(cId(27), "covariant return types",
      "JLS 8.4.8.3", "UNCITED — the same rule",
      NoImpact, NonDiff("the same rule on both sides"), NoTwin, NoFix, "no symbol — a shared rule"),
    Difference(cId(28), "bridge methods",
      "JLS 13.1", "UNCITED — a source-to-source port never sees one",
      NoImpact, NonDiff("bridges are a binary artefact and this engine emits source"), NoTwin, NoFix, "no symbol"),
    Difference(cId(29), "a static NESTED class and an INNER class are different types, and only one is path-dependent",
      ...
      Mixed, Handled, NoTwin, Universal,
      "TirEmitter.tpe's cascade — nestedPath for a static nested type, a projection for an inner one, with namedInner opting out at `extends`/`new`"),
    Difference(cId(30), "method-LOCAL named classes",
      ...
      el("T9"), Universal, "SpoonTir.stmtKind's refusal arm, through SpoonTir.unsupported"),
    Difference(cId(31), "anonymous class construction and capture",
      "JLS 15.9.5", "UNCITED — the same construct, with a synthesised name",
      Silent, Handled, el("T1"), Universal, "SpoonTir.anonClass"),
    Difference(cId(32), "a captured local must be EFFECTIVELY FINAL in Java",
      ...
      NoImpact, NonDiff("Scala accepts strictly more"), NoTwin, NoFix, "no symbol"),
    Difference(cId(33), "the interface default-method diamond is resolved by JLS's two rules, not by trait linearization",
      ...
      el("T7"), Universal, "TirEmitter.diamondOverrides, whose `sup` is the superclass and whose mixins are all in the tail"),
    Difference(cId(34), "interface statics are not inherited AT ALL, not even by an implementor",
      ...
      Loud, Handled, el("T14"), Universal, "TirEmitter's companion re-export lists"),
    Difference(cId(35), "interface `private` methods",
      "JLS 9.4", "UNCITED — a private trait member is the image",
      NoImpact, NonDiff("a direct counterpart exists"), NoTwin, NoFix, "no symbol"),
    Difference(cId(36), "an interface field is implicitly `public static final`",
      ...
      Loud, Handled, el("T14"), Universal, "SpoonTir.fieldFlags with SpoonTir.declaringStaticType's interface edge"),
    Difference(cId(37), "`name()`, `values()` and `valueOf` are SYNTHESISED on every Java enum",
      ...
      Loud, Handled, el("T11"), Universal, "TirEmitter.enumDef's nameM, values and valueOf"),
    Difference(cId(38), "a promoted enum constructor parameter IS a member, and `name` collides with `Enum.name()`",
      ...
      Loud, Handled, el("T11"), Universal, "TirEmitter.enumDef's hasName"),
    Difference(cId(39), "`ordinal()` is part of every Java enum's surface, mentioned or not",
      ...
      Loud, Handled, el("T13"), Universal, "TirEmitter.enumDef's hasOrdinal"),
    Difference(cId(40), "enum constants with PER-CONSTANT class bodies",
      ...
      el("T8"), Universal, "SpoonTir.enumCase, which collects only CtField and CtMethod"),
    Difference(cId(42), "`EnumMap`/`EnumSet` GUARANTEE declaration-order iteration",
      ...
      "CollectionsTransform.typeMap has no EnumMap/EnumSet key, so the types pass through — and no boundary check can see an iteration-ORDER regression"),
    Difference(cId(43), "Java `record`",
      ...
      Predicted, Universal, "SpoonTir.classDef's CtClass arms take it, and SpoonTir.typeFlags has no isRecord"),
    Difference(cId(44), "`sealed` / `non-sealed` / `permits`",
      "JLS 8.1.1.2, 9.1.1.4", "UNCITED — Scala has no `non-sealed` and no explicit `permits`",
      Silent, Open, Predicted, Universal,
      "SpoonTir.typeFlags never populates Flags.isSealed, so a sealed hierarchy ships as an ordinary open class"),
    Difference(cId(45), "a `final` field's JMM safe-publication guarantee",
      "JLS 17.5", "UNCITED — a `val` carries the same guarantee",
      NoImpact, Handled, NoTwin, Universal, "SpoonTir.fieldFlags — isFinal/isMutable off the FINAL modifier"),
    Difference(cId(46), "Java has TWO name namespaces and Scala has one",
      "JLS 6.5", "UNCITED — one namespace, resolved innermost-first",
      Loud, Handled, NoTwin, Universal, "TirEmitter.resolveMemberClashes and TirEmitter.resolveFieldShadowing; MemberRenamer for policy renames"),
    Difference(cId(47), "package-private becomes `private[pkg]`",
      "JLS 6.6.1", "UNCITED — a qualified `private` is the image",
      Loud, Handled, el("T12"), Universal, "Visibility.decide's isPackagePrivate branch; TirEmitter.vis"),
    Difference(cId(48), "Java's `protected` also grants SAME-PACKAGE access, and accessibility is an input to overload resolution",
      ...
      Loud, Handled, el("T12"), Universal, "Visibility.decide's isProtected branch, with overrideTarget and qualifierResolves"),
    ...
    Difference(cId(50), "Java's default access is package-private and Scala's is public — INVERTED",
      ...
      Silent, Handled, el("T12"), Universal, "Visibility.decide — the same branch as JS-C47"),
  )
  ...
  val generics: List[Difference] = List(
    Difference(gId(1), "use-site wildcards, with the matching bound grammar",
      ...
      NoImpact, Handled, el("G2"), Universal, "SpoonTir.tpe's CtWildcardReference branch"),
    Difference(gId(2), "CAPTURE CONVERSION relates two uses of one wildcard in a single expression",
      ...
      Loud, Open, el("G24"), Universal, "no rewrite exists; the fix is local (bind to a named local) and none is synthesised"),
    Difference(gId(3), "`? super Object` has exactly one inhabitant",
      ...
      Loud, Handled, el("G23"), Universal, "SpoonTir.tpe's `!w.isUpper && isObj` branch"),
    Difference(gId(4), "a captured wildcard on ITERATION has no nameable type",
      ...
      Loud, Handled, el("K7"), Universal, "TirEmitter.widenedBinding in the Tree.ForEach arm"),
    Difference(gId(5), "a wildcard in an `extends` clause takes the parameter's DECLARED bound",
      ...
      Loud, Handled, el("G7"), Universal, "SpoonTir.erasureOfFormal and the formal-bound fill in SpoonTir.tpe"),
    Difference(gId(6), "a de-wildcarded raw PARENT and its overrides must agree",
      ...
      Loud, Handled, el("G6"), Universal, "SpoonTir's parent-instantiation fill"),
    Difference(gId(7), "a RAW type erases the REFERENCE's generics, not the class's",
      ...
      Loud, Handled, el("G2"), Universal, "SpoonTir.tpe's empty-actuals branch"),
    Difference(gId(8), "the same raw Java type renders DIFFERENTLY per scope, and correctly so",
      ...
      Silent, Handled, el("G3"), Universal, "SpoonTir.uncheckedGeneric's ownCallee/inOverridingMember save-and-restore"),
    Difference(gId(9), "UNCHECKED CONVERSION at a raw type is legal in Java and is not in Scala",
      ...
      Loud, Handled, el("G12"), Universal, "SpoonTir.uncheckedGeneric emitting Tree.Typed, deciding on RENDERED types"),
    ...
    Difference(gId(11), "a partially-nameable F-BOUNDED raw fill",
      ...
      "SpoonTir.erasureOfFormal's cycle cut is the whole of the F-bound machinery"),
    Difference(gId(12), "a diamond's inference variable has NO NAMEABLE type",
      ...
      Loud, Handled, el("G2"), Universal, "Symbol.UnresolvedTypeVarPrefix; TirEmitter.isUnresolvedTypeVar"),
    Difference(gId(13), "Java arrays are COVARIANT, with a runtime `ArrayStoreException`; Scala's are invariant",
      ...
      Loud, Handled, el("G1"), Universal, "SpoonTir.coerce's arrayCov clause"),
    Difference(gId(14), "a primitive at a generic slot boxes to the WRAPPER, not to the formal",
      ...
      el("G16"), Universal, "SpoonTir.coerce's boxing predicate and boxedPrimitive"),
    ...
    Difference(gId(16), "Scala's `Array[T]` needs a `ClassTag` to be CONSTRUCTED",
      ...
      Loud, Open, Predicted, Universal, "RewriteTrace's scaladoc names this as the canonical hand-built rewrite; nothing synthesises it"),
    ...
    Difference(gId(18), "under `noClasspath` a REFERENCE erases and a DECLARATION does not",
      ...
      Loud, Handled, el("G14"), Universal, "SpoonTir.isExternalCallee driving coerceArgsFixed's erasure cast"),
    ...
    Difference(gId(21), "`instanceof` is restricted to REIFIABLE types, and SE16 added a pattern binding",
      ...
      Predicted, Universal, "Tir.Tree.InstanceOf; no CtTypePattern arm anywhere"),
    ...
    Difference(gId(24), "unboxing `null` throws NPE",
      ...
      NoImpact, Handled, el("G23"), Universal, "SpoonTir.coerce's unbox clause, via wrapperOf/valueMethod"),
    ...
    Difference(gId(31), "a POLY EXPRESSION must never be cast as a whole",
      ...
      el("G12"), Universal, "SpoonTir.uncheckedGeneric's `bad` list, and the second one in SpoonTir.appliedCtorArgs"),
    Difference(gId(32), "a callee's OWN type variables never resolve at the caller",
      ...
      Loud, Handled, el("G12"), Universal, "SpoonTir.uncheckedGeneric's tpResolvable/calleeBounded decline"),
    Difference(gId(33), "SAM conversion eligibility",
      ...
      Loud, Handled, Predicted, Universal, "TirEmitter.samAscribed"),
    ...
    Difference(gId(37), "`T...` is an `Array[T]`, materialised at the port's OWN call sites",
      ...
      Loud, Handled, el("K6.5"), Universal, "SpoonTir.varargPack -> Tree.NewArray for an in-program callee"),
    Difference(gId(38), "a vararg slot ALREADY holding an array must not be re-packed — and a primitive array never forwards",
      ...
      Silent, Handled, el("K6.5"), Universal, "SpoonTir.varargPack's passesArray with componentAgrees"),
    ...
    Difference(gId(42), "a generic vararg's COMPONENT element type has four layered sources",
      ...
      Loud, Handled, el("G1"), Universal, "SpoonTir.varargPack's elemRef, in its four documented steps"),
