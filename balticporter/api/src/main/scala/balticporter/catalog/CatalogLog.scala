...
object Lowering:
  ...
  /** Enter the obligation scope for ONE node of `kind` at `dispatch`, then run its lowering. On exit, every row the catalog attaches to that (kind, dispatch) the body did not consult is recorded as a
    * HOLE. Costs nothing where nothing attaches. `subject` joins scopes by NODE IDENTITY (a node lowered as both statement and expression) rather than kind.
    */
...
/** THE EMITTER'S HALF — the same wrapper at the OTHER end of the pipeline, keyed on the `Tree` kind. Most `JS-S` rows discharge only here: a decision about TEXT taken while rendering. Same as
  * [[Lowering]] (wrapper at the dispatch, scopes joined by node identity); no [[Dispatch]] — the TIR has already resolved the position question.
  */
object Rendering:

  /** Enter the obligation scope for ONE `Tree` node, then render it. `kind` is the node's `productPrefix`, the same name `EmissionFieldCoverageSpec` derives from the class files.
    */
...
/** THE FOURTH SURFACE — a TYPE, at both ends of the pipeline. Neither [[Lowering]] nor [[Rendering]] can enter one (`CtTypeReference` is not a statement/expression; `TypeRepr` is not a `Tree`).
  * Frontend chooses the IMAGE, emitter chooses the TEXT — two [[Attaches]] cases, no [[Dispatch]]. The emitter's origin is [[CatalogLog.currentOrigin]] (the node being rendered FOR), since a
  * `TypeRepr` is a shared value with no origin of its own.
  */
object Typing:

  /** the FRONTEND's type-reference dispatch -- `SpoonTir.tpe`. `kind` is resolved by `SpoonKinds.refNameOf`'s most-specific rule, so a `CtWildcardReference` is not silently read as the
    * `CtTypeParameterReference` its implementation extends.
    */
...
/** WHICH of the frontend's two term dispatches an obligation attaches at. Not decoration: java gives the SAME node kind two different meanings by position (`i += f` discards its value as a statement,
  * JLS 14.8, and yields it as an expression, JLS 15.26.2), so a single kind-keyed attachment could not tell `JS-E03` from `JS-E04`.
  */
enum Dispatch:
  ...
  /** the node reached as a MEMBER DECLARATION -- neither Statement nor Expression. A `CtField` enters neither dispatch (walked from its type's member list), which made a field's INITIALISER
    * assignment-conversion slot invisible until named explicitly. Not folded into [[Statement]]: `CatalogCoverageSpec` derives legal dispatches from Spoon's own hierarchy and would reject that claim.
    */
  case Declaration

  /** both of the two POSITIONAL dispatches owe the consult. Not [[Declaration]]: a kind reached as a declaration is reached at exactly one place, so a row that means it says so.
    */
  case Either

/** WHERE a row's obligation is discharged. ONE value per row, never a list (`DifferenceTakesNoParameterSpec`'s no-parameter rule); a row discharging at two surfaces uses [[Attaches.Both]] instead.
  */
enum Attaches:
  /** the frontend's lowering dispatch owes a consult for every node of `kind` at `dispatch`. `kind` is Spoon's INTERFACE name, the key `SpoonKinds` registers.
    */
  case Lowered(kind: String, dispatch: Dispatch)

  /** the EMITTER's rendering dispatch owes a consult for every `Tree` node of `kind` — the node's `productPrefix`, which is the same name `EmissionFieldCoverageSpec` derives from the class files. See
    * [[Rendering]] for why this case carries no [[Dispatch]].
    */
  case Rendered(kind: String)

  /** the FRONTEND's TYPE-REFERENCE dispatch owes a consult for every reference of `kind` — `SpoonTir.tpe`. `kind` is Spoon's reference-INTERFACE name, the key `SpoonKinds.references` registers. See
    * [[Typing]] for why the type surface is two cases and not one, and why neither carries a [[Dispatch]].
    */
  case LoweredType(kind: String)

  /** the EMITTER's TYPE dispatch owes a consult for every `TypeRepr` of `kind` — `TirEmitter.tpe`. `kind` is the `TypeRepr` case's `productPrefix`, derived from the class files by
    * `EmissionFieldCoverageSpec` exactly as the `Tree` kinds are, so a row naming a case the algebra does not have is caught by a spec rather than by silence.
    */
  case RenderedType(kind: String)

  /** a PHASE decides this row, and cites it per declaration. `phase` is the phase's `name`, so the citation and the phase that owes it can be joined without reading either.
    */
  case Cited(phase: String)

  /** the row discharges at more than one place, and every one owes it — either MORE THAN ONE KIND at one surface (a loop's four `Tree` kinds all through `TirEmitter.loopWithJumps`) or TWO SURFACES
    * (`JS-S18`'s frontend mapping and emitter image). A product of enum cases (not a `List`, which is per-library policy's shape). `Differences.mechanised` requires every leaf.
    */
  ...
  /** there is NOTHING to discharge — the row records a checked non-difference, or a difference the translation satisfies by construction with no site-level decision to take. `why` is what was
    * checked. Distinct from [[Unmechanised]] on purpose: one says the surface is missing, the other says no surface is owed, and collapsing them would hide the first inside the second.
    */
...
final class Obligations private[catalog] (log: CatalogLog, owed: List[DiffId], private[catalog] val subject: AnyRef = null):
  ...
  /** mark `id` considered HERE — reached from [[CatalogLog.markSeen]] for the enclosing scope of a delegated node, and from [[consult]] for this one.
    */
  private[catalog] def see(id: DiffId): Unit = if !seen.contains(id) then seen ::= id

  /** CONSULT a difference at this site. `f` returns `Some(fix)` when it APPLIES here. Consulting discharges the obligation — FIRING is not required, since "does not apply here" is the answer at most
    * sites. Both are recorded: `consulted` says the branch is live, `fired` says it did something.
    */
...
final class CatalogLog(val fatal: Boolean = false):
  ...
  /** the [[Obligations]] handed to a dispatch nothing attaches to. Owes nothing, settles nothing, and still RECORDS — an arm may consult a row the catalog attaches elsewhere, and a consult that
    * vanished because its kind had no attachment would be a coverage number that lies low.
    */
  ...
  /** …and WHERE it is, which a `TypeRepr` cannot answer for itself — it is a value shared across every position naming it. The node it is being rendered FOR has an origin, and that is the line a
    * type-surface finding wants ([[Typing]]). Restored by the same caller as `currentSubject`.
    */
  private var origin: Origin = Origin.synthetic

  /** the innermost live scope's origin — `Origin.synthetic` outside every scope, which is honest: nothing is being lowered or rendered, so there is no site.
    */
  ...
  /** DISCHARGE `id` in every live scope that is lowering THIS SAME NODE.
    *
    * Innermost first, stopping at the first scope whose subject is a different node — so a consult inside a CHILD (`if (x) y += 1`, the assignment's consult under the `CtIf`'s scope) discharges
  ...
  /** the first site at which this row's consult FIRED, so a lane's finding names a line somebody can open. `scala.None` for a row that was consulted and never applied — which is the normal state of
    * most rows at most sites and is not a defect.
    */
  ...
  /** the holes, one per ROW — never one per site. A compound assignment in every method of a library would otherwise produce a thousand findings saying one thing.
    */
  def undischarged: List[CatalogLog.Hole] = holes.values.toList.sortBy(_.id.toString)

  /** the artifact's rows: EVERY catalog row, reached or not, so `catalog.tsv` answers "which branches does this port never touch" and not only "which did it".
    */
  def rows: List[CatalogLog.Row] =
    Differences.all.map(d => CatalogLog.Row(d.id, d.status, d.attaches, consulted(d.id), fired(d.id), declarations(d.id)))
...
object CatalogLog:
  ...
  final case class Row(id: DiffId, status: Status, attaches: Attaches, consulted: Int, fired: Int, declarations: Int)

  /** a hole the registry ALREADY declares — an `Open` or `Absent` row cannot be consulted (rule (ii)), so it is undischarged by construction and is the work list rather than a defect.
    */
  def knownHole(id: DiffId): Boolean =
    Differences.byId.get(id).exists(d => d.status.isOpen || d.status.isInstanceOf[Status.Absent])

  /** the log a caller that does not want one still has to pass. Not a global: each call makes a new one, so nothing accumulates across two runs.
    */
