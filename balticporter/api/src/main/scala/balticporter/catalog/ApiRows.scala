...
/** what the PLATFORM provides. This is the (a) FACT half of an [[ApiRow]] — true of every port, and no manifest may contradict it, because asserting otherwise is how a port silently declares a gap
  * closed.
  */
enum Availability:
  case Full
  ...
  /** present and INERT: it links and does nothing useful. The most dangerous state, because it fails at run time rather than at link time
    */
  case Stub(why: String)
  case Absent

/** what a port should DO about the row on that platform. This is the RECOMMENDATION half, and it is a default rather than a fact — another port may ship its own shim, vendor a subset, or accept the
  * refusal. The mechanism that lets a port say so (`PortManifest.verdictOverrides`) belongs to the chunk that parameterises `PortabilityCheck`; until then a verdict is advice this registry states and
  * nothing reads.
  */
enum Verdict:
  ...
  /** Does a port TARGETING THIS PLATFORM have to do something about the row? `Keep` is the only "nothing to do" — the criterion a portability rule derives through, not `isInstanceOf[Refuse]`.
    * `java.lang.Thread`/`java.util.concurrent.` are `MapTo` on Scala.js and reported as violations; reading only `Refuse` would have deleted both.
    */
  def actionable: Boolean = this != Keep

  /** …and WHICH KIND of work. A `Depend` is a BUILD-GRAPH fact — the API exists, in an artifact the port's build does not name — and conflating that with "no faithful target exists" is what makes a
    * `WeakReference` finding unanswerable: the reader is told to remove a call that a one-line `libraryDependencies` entry makes correct.
    */
  def dependency: Option[ArtifactDep] = this match
    case Depend(d) => Some(d)
    case _         => scala.None

/** How a dependency's ARTIFACT NAME is built — the `%` / `%%` / `%%%` question, as a fact about the artifact rather than about the port.
  */
enum CrossKind:
  ...
  /** cross-built per scala version AND per PLATFORM: `%%%`, which needs the crossproject plugin — which a port declaring a platform-cross dependency necessarily has.
    */
  case Platform

/** a build-graph coordinate a [[Verdict.Depend]] names — every field a literal or enum case, never a PLATFORM SET (`verdict`'s per-platform map already states it; two artifacts for one need are two
  * `Depend` verdicts, not a dep with a set — `ApiRowCarriesNoPolicySpec`).
  * @param resolver
  *   WHERE the artifact resolves from when not default (a fact about the ARTIFACT, like [[cross]]); `None` is the ordinary set. ONE URL, since `SbtGen.Dep` derives the name from it.
  */
final case class ArtifactDep(org: String, name: String, rev: String, cross: CrossKind = CrossKind.Scala, resolver: Option[String] = scala.None):
...
/** ONE ROW of the library and platform halves — `JS-L`/`JS-P`. Unlike a [[Difference]] (a rule of two LANGUAGES, true forever), this states what an IMPLEMENTATION shipped, true as of a version
  * ([[asOf]]). May carry per-platform MAPS but NOT a `RuleScope`/predicate/target set — WHICH platforms a port cares about is the manifest's (`ApiRowCarriesNoPolicySpec`). @param fqn a PREFIX or
  * exact member @param asOf versions the claim anchors to, empty means `why`=UNSTATED.
  */
final case class ApiRow(
  ...
  /** The verdict THIS PORT reads for `p` — the row's own recommendation unless overridden. [[by]] is the (a) FACT (no manifest may contradict it); [[verdict]] is a RECOMMENDATION, the half
...
/** The library and platform halves of the catalog. Every row is transcribed from a source-tree survey of the two javalibs at [[JsNative]]'s versions; where the survey did not reach a platform the row
  * carries `UNSTATED` in `why`. Read by `PortabilityCheck.rowOf` (`isDependency`/`stillAsks`, through the port-overridable [[ApiRow.verdictOn]]) and `DependencyCheck.requirements` (per-platform
  * `ArtifactDep`). `catalog(uncited)` counts the rest.
  */
object ApiRows:
  ...
  private val MultiArchSpi = ArtifactDep(
    "com.kubuszok",
    "multiarch-serviceloader",
    "0.4.0-12-gc168b2f-SNAPSHOT",
    CrossKind.Platform,
    Some("https://central.sonatype.com/repository/maven-snapshots")
  )

  private def row(id: DiffId, fqn: String, exact: Boolean, jvm: (Availability, Verdict), js: (Availability, Verdict), nat: (Availability, Verdict), asOf: Map[String, String], why: String): ApiRow =
    ApiRow(
      id,
      fqn,
      exact,
  ...
  private def mapped(id: DiffId, fqn: String, exact: Boolean, target: String, js: Availability, nat: Availability, why: String): ApiRow =
  ...
  val library: List[ApiRow] = List(
    ...
    mapped(
      ...
      true,
      "balticporter.runtime.JavaCollections",
      Full,
      Full,
    ...
    mapped(
      ...
      true,
      "balticporter.runtime.JavaCollections.asList",
      Full,
      Full,
    ...
    row(
      ...
      "java.util.Iterator",
      false,
      (Full, Shim("balticporter.runtime.JavaIterator")),
      (Full, Shim("balticporter.runtime.JavaIterator")),
      (Full, Shim("balticporter.runtime.JavaIterator")),
      JsNative,
    ...
    row(
      ...
      "java.util.EnumMap",
      true,
      (Full, Shim("engine runtime")),
      (Absent, Shim("engine runtime")),
      (Absent, Shim("engine runtime")),
      JsNative,
    ...
    row(
      ...
      true,
      (Full, Keep),
      (Absent, Refuse("JS's WeakMap requires object keys and cannot enumerate")),
      (Full, Keep),
      JsNative,
    ...
    mapped(
      ...
      "java.util.stream.",
      false,
      "the Scala collection operations, via chain collapse",
      Absent,
      Full,
    ...
    everywhere(
      ...
      "java.lang.Integer",
      true,
      "the static parse/valueOf/radix surface is implemented identically on both non-JVM backends"
    ),
    ...
    row(
      ...
      true,
      (Full, Keep),
      (Partial("toString omits the trailing zero the JVM prints"), Keep),
      (Full, Keep),
      JsNative,
    ...
    row(
      ...
      true,
      (Full, Keep),
      (Absent, Refuse("no bit-for-bit-reproducible target")),
      (Absent, Refuse("no bit-for-bit-reproducible target")),
      JsNative,
    ...
    row(
      ...
      true,
      (Full, Keep),
      (Partial("exit, getProperty and the other environment statics are browser-appropriate stubs"), Keep),
      (Full, Keep),
      JsNative,
    ...
    row(
      ...
      "java.lang.Iterable",
      false,
      (Full, Shim("balticporter.runtime.JavaIterable")),
      (Full, Shim("balticporter.runtime.JavaIterable")),
      (Full, Shim("balticporter.runtime.JavaIterable")),
      JsNative,
    ...
    row(
      ...
      false,
      (Full, Keep),
      (Stub("the working classes live only in an unpublished ext-dummies module"), Depend(ScalaJavaTime)),
      (Absent, Depend(ScalaJavaTime)),
      TimeLib,
    ...
    everywhere(
      ...
      true,
      "not a platform gap at all — a static-inheritance naming question, recorded so a matrix reader does not misfile it; UNSTATED because no version bears on it",
      Unstated
    ),
    row(
      l(65),
      ...
      true,
      (Full, Keep),
      (Stub("a working class exists only in the unpublished ext-dummies module; the default is ROOT"), Depend(ScalaJavaLocale)),
      (Absent, Depend(ScalaJavaLocale)),
      LocaleLib,
    ...
    row(
      ...
      true,
      (Full, Keep),
      (Stub("an ext-dummies placeholder with almost no surface"), Depend(ScalaJavaLocale)),
      (Absent, Depend(ScalaJavaLocale)),
      LocaleLib,
    ...
    row(
      ...
      true,
      (Full, Keep),
      (Stub("an ext-dummies placeholder"), Depend(ScalaJavaLocale)),
      (Absent, Depend(ScalaJavaLocale)),
      LocaleLib,
    ...
    row(
      ...
      true,
      (Full, Keep),
      (Absent, Refuse("no implementation anywhere surveyed")),
      (Absent, Refuse("no implementation anywhere surveyed")),
      LocaleLib,
    ...
    row(
      ...
      true,
      (Full, Keep),
      (Absent, Refuse("no cross-platform collation")),
      (Absent, Refuse("no cross-platform collation")),
      LocaleLib,
    ...
    everywhere(
      ...
      "java.util.Date",
      true,
      "the one row in this area where the plain JDK type works on all three platforms with no dependency; UNSTATED — the survey anchored no version",
      Unstated
    ),
    row(
      l(74),
    ...
    row(
      ...
      true,
      (Full, Keep),
      (Partial("negative-value hex output differs from the JVM"), Keep),
      (Partial("independently ported; conversion coverage is not guaranteed to match JS"), Keep),
      Unstated,
    ...
    row(
      ...
      false,
      (Full, Keep),
      (Partial("compiled to the JS RegExp engine, with documented limitations"), Keep),
      (Partial("RE2-backed, and not a drop-in replacement for java.util.regex"), Keep),
      Unstated,
  ...
  val platform: List[ApiRow] = List(
    row(
      ...
      false,
      (Full, Keep),
      (Absent, Shim("a per-backend Files trait")),
      (Full, Keep),
      JsNative,
    ...
    row(
      ...
      false,
      (Full, Keep),
      (Absent, Refuse("no bytecode-level serialization machinery on Scala.js")),
      (Partial("present, rarely used"), Keep),
      JsNative,
    ...
    row(
      ...
      false,
      (Full, Keep),
      (Partial("stdout via console.log, no real stdin"), Shim("stdout only")),
      (Full, Keep),
      JsNative,
    ...
    everywhere(
      ...
      "java.nio.charset.",
      false,
      "real, non-dummy implementations including the per-charset classes exist in both core javalibs"
    ),
    row(
      p(7),
      "java.nio.channels.",
      false,
      (Full, Keep),
      (Absent, Refuse("the package does not exist in the Scala.js javalib")),
      (Partial("FileChannel and FileLock are real; the socket channels are not"), Keep),
      JsNative,
    ...
    row(
      ...
      true,
      (Full, Keep),
      (Absent, Refuse("no java.nio.channels package at all")),
      (Absent, Refuse("verified absent from the Native javalib")),
      JsNative,
    ...
    row(
      ...
      false,
      (Full, Keep),
      (Absent, Refuse("a browser sandbox has no raw sockets")),
      (Full, Keep),
      JsNative,
    ...
    row(
      ...
      false,
      (Full, Keep),
      (Partial("DNS resolution is a JS-runtime concern, not exposed with the JVM's shape"), MapTo("the JS runtime's own DNS facility")),
      (Full, Keep),
      JsNative,
    ...
    row(
      ...
      false,
      (Full, Keep),
      (Partial("the class exists and is effectively unusable without ICU-like tables"), Refuse("needs ICU-like tables")),
      (Partial("the class exists and is effectively unusable without ICU-like tables"), Refuse("needs ICU-like tables")),
      JsNative,
    ...
    row(
      ...
      false,
      (Full, Keep),
      (Absent, MapTo("a per-backend concurrency trait over ExecutionContext")),
      (Full, Keep),
      JsNative,
    ...
    row(
      ...
      false,
      (Full, Keep),
      (Partial("only Semaphore exists, and only because acquire never really blocks"), MapTo("a per-backend concurrency trait over the JS microtask queue")),
      (Full, Keep),
      JsNative,
    ...
    row(
      ...
      true,
      (Full, Keep),
      (Stub("a private-ctor singleton: getId is hardcoded, run() is empty, currentThread() is always the same instance"), MapTo("a per-backend concurrency trait whose yield is a documented no-op")),
      (Full, Keep),
      JsNative,
    ...
    row(
      ...
      false,
      (Full, Keep),
      (Absent, Refuse("no process spawning in a browser")),
      (Full, Keep),
      JsNative,
    ...
    row(
      ...
      false,
      (Full, Keep),
      (Stub("present but reduced; no real process control in a browser"), Shim("a reduced Runtime")),
      (Full, Keep),
      JsNative,
    ...
    row(
      ...
      true,
      (Full, Keep),
      (Partial("works for any Scala.js object; general reflection is explicitly out"), Keep),
      (Partial("works, and is needed for exception messages; full introspection does not"), Keep),
      JsNative,
    ...
    row(
      ...
      false,
      (Full, Keep),
      (Absent, Refuse("no reflective dispatch; it breaks the Scala.js linker")),
      (Stub("Array/AccessibleObject/Executable and the exception types are present; Method/Field/Constructor dispatch is not"), Refuse("no reflective dispatch; it breaks the Scala Native link")),
      JsNative,
    ...
    row(
      ...
      false,
      (Full, Keep),
      (Absent, Depend(MultiArchSpi)),
      (Partial(
         "real, but `load` is a LINK-TIME intrinsic that only accepts a literal classOf, so no Class-taking wrapper can call it"
       ),
       Depend(MultiArchSpi)
      ),
      JsNative,
    ...
    row(
      ...
      false,
      (Full, Keep),
      (Absent, Depend(CrossCrypto)),
      (Absent, Depend(NativeCrypto)),
      JsNative,
    ...
    row(
      ...
      false,
      (Full, Keep),
      (Absent, Refuse("no zlib in a browser without a JS dependency")),
      (Full, Keep),
      JsNative,
    ...
    row(
      ...
      false,
      (Full, Keep),
      (Absent, Refuse("the javax.* stack is JVM-only")),
      (Absent, Refuse("the javax.* stack is JVM-only")),
      JsNative,
    ...
    row(
      ...
      true,
      (Partial("deprecated but functional"), Keep),
      (Stub("never invoked; no GC hook is exposed the same way"), Refuse("finalizers are never invoked on Scala.js")),
      (Partial("present via the GC; the timing is not equivalent"), Refuse("present but not timing-equivalent to the JVM")),
      JsNative,
    ...
    row(
      ...
      true,
      (Full, Keep),
      (Partial("a link-time-configured properties table, never the live host's"), MapTo("link-time-configured values")),
      (Full, Keep),
      Map("scala-js" -> "1.22.0"),
    ...
    row(
      ...
      true,
      (Full, Keep),
      (Stub("getenv() is always an empty map and getenv(name) is always null"), Refuse("always empty, so a read silently returns nothing")),
      (Full, Keep),
      JsNative,
