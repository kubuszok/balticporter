...
/** Resolved AST (RAST) for Dart — the JSON interchange format between the Dart analyzer exporter and the Scala frontend. Mirrors the TS RAST but with Dart-specific constructs: mixins, extensions,
  * named parameters, factory constructors, late fields, cascade expressions, pattern matching.
  */
...
object DartRast:
  given fileCodec: JsonValueCodec[DartRastFile] = JsonCodecMaker.make(
    CodecMakerConfig.withDiscriminatorFieldName(None).withAllowRecursiveTypes(true)
  )
