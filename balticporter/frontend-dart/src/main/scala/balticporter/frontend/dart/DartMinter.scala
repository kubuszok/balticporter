...
private class MintContext(subs: Substitutions, catalog: CatalogLog):
  ...
  private def mintTopLevel(node: DartRastNode, file: DartRastFile): List[ClassDef] =
    ...
    node.kind match
      ...
      case "ExtensionDeclaration" =>
        ...
        val sym        = freshSym(moduleName, moduleName, Flags(isModule = true), SymId.None, TypeRepr.NoType, origin)
      ...
      case "EnumDeclaration" =>
        ...
        val enumMembers = node.children.filter(_.kind == "EnumConstantDeclaration").map { ec =>
          ...
          val eSym    = freshSym(eName, s"$name.$eName", Flags(isStatic = true, isEnum = true), sym, TypeRepr.NoType, eOrigin)
          ValDef(eSym, TypeTree(TypeRepr.NoType, eOrigin), None, eOrigin)
        }
  ...
  private def isClassMember(node: DartRastNode): Boolean =
    node.kind match
      case "MethodDeclaration" | "FieldDeclaration" | "ConstructorDeclaration" | "GetterDeclaration" | "SetterDeclaration" | "FunctionDeclaration" | "TopLevelVariableDeclaration" => true
      case _                                                                                                                                                                       => false

  private def mintMember(node: DartRastNode, owner: SymId, ownerName: String, file: DartRastFile): Statement =
    val origin = Origin(file.path, node.pos._1, node.pos._2)
    val name   = nameOf(node)
    node.kind match
      case "MethodDeclaration" | "FunctionDeclaration" =>
        val isStatic = node.flags.contains("static")
        val sym      = freshSym(name, s"$ownerName.$name", Flags(isStatic = isStatic), owner, TypeRepr.NoType, origin)
        val params   = mintParams(node, file, sym)
        val retType  = resolveTypeTree(node, file, origin)
        val body     = node.children.find(c => c.kind == "BlockFunctionBody" || c.kind == "ExpressionFunctionBody").map(c => mintBody(c, file))
      ...
      case "ConstructorDeclaration" =>
        ...
        val sym       = freshSym(ctorName, s"$ownerName.$ctorName", flags, owner, TypeRepr.NoType, origin)
        val params    = mintParams(node, file, sym)
      ...
      case "SetterDeclaration" =>
        val sym    = freshSym(name + "_=", s"$ownerName.${name}_=", Flags(), owner, TypeRepr.NoType, origin)
        val params = mintParams(node, file, sym)
        val body   = node.children.find(c => c.kind == "BlockFunctionBody" || c.kind == "ExpressionFunctionBody").map(c => mintBody(c, file))
        DefDef(sym, List(params), TypeTree(TypeRepr.NoType, origin), body, origin)
  ...
  private def mintSuperConstructor(node: DartRastNode, file: DartRastFile, origin: Origin): Option[TypeTree] =
    node.children.find(_.kind == "ExtendsClause").flatMap { ext =>
      ext.children.headOption.map { superType =>
        TypeTree(resolveTypeRepr(superType, file), origin)
      }
    }
  ...
  private def mintStatement(node: DartRastNode, file: DartRastFile): Statement =
    ...
    node.kind match
      ...
      case "ForStatement" =>
        ...
        val body  = parts.lastOption.map(c => mintExpr(c, file)).getOrElse(Literal(Constant.UnitC, TypeRepr.NoType, origin))
        For(Nil, None, Nil, body, TypeRepr.NoType, origin)
      case "ForEachStatement" | "ForEachPartsOfDeclaration" =>
        val body     = node.children.lastOption.map(c => mintExpr(c, file)).getOrElse(Literal(Constant.UnitC, TypeRepr.NoType, origin))
        ...
        val binding = ValDef(bindSym, TypeTree(TypeRepr.NoType, origin), None, origin)
      ...
      case "SwitchStatement" =>
        ...
        val cases     = node.children.tail.filter(c => c.kind == "SwitchCase" || c.kind == "SwitchDefault").map { c =>
          val pattern = c.children.headOption.map(p => mintExpr(p, file)).getOrElse(Ident(SymId.None, TypeRepr.NoType, origin))
          val body    = c.children.lastOption.map(b => mintExpr(b, file)).getOrElse(Literal(Constant.UnitC, TypeRepr.NoType, origin))
          CaseDef(List(TypePattern(SymId.None, TypeTree(TypeRepr.NoType, origin), TypeRepr.NoType, origin)), None, body, false)
        }
      ...
      case "TryStatement" =>
        val tryBody  = node.children.headOption.map(c => mintExpr(c, file)).getOrElse(Literal(Constant.UnitC, TypeRepr.NoType, origin))
        val catchSym = freshSym("e", "e", Flags(isParam = true), SymId.None, TypeRepr.NoType, origin)
        val catches  = node.children.filter(_.kind == "CatchClause").map { cc =>
          val body = cc.children.lastOption.map(b => mintExpr(b, file)).getOrElse(Literal(Constant.UnitC, TypeRepr.NoType, origin))
          CatchCase(ValDef(catchSym, TypeTree(TypeRepr.NoType, origin), None, origin), body)
        }
        val finallyBlock = node.children.find(_.kind == "FinallyClause").flatMap(_.children.headOption).map(c => mintExpr(c, file))
        Try(Nil, tryBody, catches, finallyBlock, TypeRepr.NoType, origin)
      case _ =>
        Unportable(
          Literal(Constant.UnitC, TypeRepr.NoType, origin),
          UnportableKind.FrontendBlindSpot,
  ...
  private def mintVariableDecl(node: DartRastNode, file: DartRastFile): Statement =
    ...
    if decls.length == 1 then
      ...
      val sym     = freshSym(name, name, Flags(isMutable = !isFinal), SymId.None, TypeRepr.NoType, origin)
    ...
    node.kind match
      ...
      case "IntegerLiteral" =>
        Literal(
          Constant.IntC(
            node.value
              .map {
                case balticporter.frontend.ts.RastValue.Num(v) => v.toInt
                case balticporter.frontend.ts.RastValue.Num(v) => v.toInt
                case _                                         => 0
              }
              .getOrElse(0)
      ...
      case "DoubleLiteral" =>
        Literal(
          Constant.DoubleC(
            node.value
              .map {
                case balticporter.frontend.ts.RastValue.Num(v) => v
                case _                                         => 0.0
              }
              .getOrElse(0.0)
          ),
      ...
      case "SimpleStringLiteral" | "AdjacentStrings" | "StringInterpolation" =>
        Literal(
          Constant.StringC(
            node.value
              .map {
                case balticporter.frontend.ts.RastValue.Str(v) => v
                case _                                         => ""
              }
              .getOrElse("")
          ),
      ...
      case "BooleanLiteral" =>
        val v = node.value.exists {
          case balticporter.frontend.ts.RastValue.Bool(b) => b
          case _                                          => false
        }
        Literal(Constant.BoolC(v), TypeRepr.NoType, origin)
      case "NullLiteral" =>
      ...
      case "BinaryExpression" =>
        ...
        Apply(Select(left, SymId.None, TypeRepr.NoType, origin), List(right), SymId.None, resolveTypeRepr(node, file), origin)
      ...
      case "ListLiteral" =>
        ...
        NewArray(TypeTree(resolveTypeRepr(node, file), origin), Nil, Some(elems), resolveTypeRepr(node, file), origin)
      ...
      case "FunctionExpression" =>
        ...
        val body = node.children.find(c => c.kind == "BlockFunctionBody" || c.kind == "ExpressionFunctionBody").map(c => mintBody(c, file)).getOrElse(Literal(Constant.UnitC, TypeRepr.NoType, origin))
      ...
      case "PrefixExpression" | "PostfixExpression" =>
        ...
        Apply(Select(operand, SymId.None, TypeRepr.NoType, origin), Nil, SymId.None, resolveTypeRepr(node, file), origin)
      case "AssignmentExpression" =>
        val target = mintExpr(node.children.head, file)
        val value  = mintExpr(node.children.last, file)
        Assign(target, value, TypeRepr.NoType, origin)
      ...
      case _ =>
        Unportable(
          ...
          MarkerState.Open,
          None,
          s"Dart expr: ${node.kind}",
          resolveTypeRepr(node, file),
          origin
        )

  private def nameOf(node: DartRastNode): String =
    node.children.find(c => c.kind == "SimpleIdentifier" || c.kind == "Identifier").flatMap(_.text).orElse(node.text).getOrElse("$anon")
  ...
  private def resolveTypeRepr(node: DartRastNode, file: DartRastFile): TypeRepr =
    node.`type`.flatMap(file.types.get).map(dartTypeToRepr(_, file)).getOrElse(TypeRepr.NoType)
  ...
  private def findInit(node: DartRastNode, file: DartRastFile): Option[Term] =
    node.children
      .find(c =>
        c.kind != "SimpleIdentifier" && c.kind != "TypeName" &&
