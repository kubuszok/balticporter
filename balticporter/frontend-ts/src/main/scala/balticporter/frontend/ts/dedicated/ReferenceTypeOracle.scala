...
/** Extracts method signatures from hand-ported Scala reference files to serve as a type oracle for the Terser compress emitter.
  *
  * JavaScript has no types, so the RAST types everything as `Any`. The hand-ported ssg-js reference HAS types for every parameter and field. This oracle reads those signatures and provides a lookup
  * by `(objectName, methodName)` for the emitter to use.
  */
object ReferenceTypeOracle:
  ...
  /** Type oracle: maps `(objectName, methodName)` to the reference signature.
    *
    * Lookups try exact match first, then case-insensitive on the method name to handle differences like `isNumberOrBigint` vs `isNumberOrBigInt`.
    */
  final case class TypeOracle(
    ...
    /** Look up the type for a parameter by object, method, and param name. Falls back to `Any` if not found. Parameter lookup is also case-insensitive on the param name.
      */
  ...
  /** Parse a reference Scala file and extract all public method signatures from the top-level object.
    *
    * Handles both single-line and multi-line signatures. A multi-line signature is recognized when a `def` line has unbalanced parentheses; subsequent lines are accumulated until parentheses balance
    * and a return type plus `=` are found.
    */
  ...
  /** Join multi-line method signatures into single logical lines.
    *
    * A line starting with `  def ` or `  private def ` that has unbalanced parentheses gets subsequent lines appended (whitespace-normalized) until parens balance and `: RetType =` is found.
    */
  ...
  /** Parse a comma-separated parameter list, handling nested generics and union types.
    */
  ...
  /** Map from emitter object name to the reference file's object name. The emitter uses names like `CompressCommon` while the reference uses `Common`.
    */
  ...
  /** Build a type oracle from all reference compress module files.
    *
    * @param referenceRoot
    *   path to the compress module directory, e.g. `/path/to/ssg/ssg-js/src/main/scala/ssg/js/compress/`
    */
  def buildFromDirectory(referenceRoot: java.nio.file.Path): TypeOracle =
    ...
    for (objName, fileName) <- referenceObjectToFile do
      ...
      resolved.foreach { fp =>
        ...
        for (_, sig) <- sigs do allMethods((objName, sig.methodName)) = sig
      }
  ...
  /** Build a type oracle from classpath resources (for testing).
    *
    * This uses a hardcoded mapping that covers the most important methods across all compress modules. Built by reading the reference files.
    */
  def buildHardcoded(): TypeOracle =
    ...
    inf(
      "isModified",
      List(
        ParamSig("compressor", "CompressorLike"),
        ParamSig("tw", "TreeWalker"),
        ParamSig("node", "AstNode"),
        ParamSig("value", "AstNode"),
        ParamSig("level", "Int"),
        ParamSig("immutable", "Boolean")
      ),
      "Boolean"
    )
    ...
    inf(
      "bitwiseNegate",
      List(ParamSig("node", "AstNode"), ParamSig("compressor", "CompressorLike"), ParamSig("in32BitContext", "Boolean")),
      "AstNode"
    )
    ...
    inf(
      ...
      List(ParamSig("cls", "AstClass"), ParamSig("visitor", "(AstNode, () => Unit) => Boolean")),
      "Unit"
    )
    inf(
      "negate",
      List(ParamSig("node", "AstNode"), ParamSig("compressor", "CompressorLike"), ParamSig("firstInStatement", "Boolean")),
      "AstNode"
    )
    ...
    com(
      "bestOf",
      List(ParamSig("compressor", "CompressorLike"), ParamSig("ast1", "AstNode"), ParamSig("ast2", "AstNode")),
      "AstNode"
    )
    ...
    com(
      "hasBreakOrContinue",
      List(ParamSig("loop", "AstNode & AstIterationStatement"), ParamSig("parent", "AstNode | Null")),
      "Boolean"
    )
    com(
      "requiresSequenceToMaintainBinding",
      List(ParamSig("parent", "AstNode"), ParamSig("orig", "AstNode"), ParamSig("value", "AstNode")),
      "Boolean"
    )
    com(
      "maintainThisBinding",
    ...
    com(
      "retainTopFunc",
      List(ParamSig("fn", "AstNode"), ParamSig("hasTopFlag", "Boolean"), ParamSig("topRetainCheck", "Any => Boolean")),
      "Boolean"
    )
    ...
    dsf(
      "trim",
      List(
        ParamSig("nodes", "ArrayBuffer[AstNode]"),
        ParamSig("compressor", "CompressorLike"),
        ParamSig("firstInStatement", "Boolean")
      ),
      "ArrayBuffer[AstNode]"
    )
    ...
    rv(
      "resetVariables",
      List(ParamSig("state", "ReduceVarsState"), ParamSig("compressor", "CompressorLike"), ParamSig("scope", "AstScope")),
      "Unit"
    )
    ...
    tb(
      ...
      List(ParamSig("compressor", "CompressorLike"), ParamSig("stat", "AstNode"), ParamSig("target", "ArrayBuffer[AstNode]")),
      "Unit"
    )
    ...
    inl(
      ...
      List(ParamSig("dd", "SymbolDef"), ParamSig("fixedValue", "AstNode | Null")),
      "Boolean"
    )
    ...
    ci(
      "flattenObject",
      List(ParamSig("node", "AstPropAccess"), ParamSig("key", "String"), ParamSig("compressor", "CompressorLike")),
      "AstNode | Null"
    )
    ...
    ci(
      "processExpression",
      List(ParamSig("node", "AstScope"), ParamSig("insert", "Boolean"), ParamSig("compressor", "CompressorLike")),
      "Unit"
    )
    ...
    ci(
      "isNullishCheck",
      List(ParamSig("check", "AstNode"), ParamSig("checkSubject", "AstNode"), ParamSig("compressor", "CompressorLike")),
      "Boolean"
    )
  ...
  /** Build an oracle and remap keys from reference object names to emitter object names, for direct lookup by the emitter.
    */
  ...
  /** Convert a snake_case JS method name to camelCase for lookup. Must match the emitter's snakeToCamel.
    */
