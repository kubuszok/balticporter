...
/** Generates a Terser ast.d.ts from the DEFNODE hierarchy in an existing RAST.
  ...
  *
  * The RAST must be an existing ast.rast.json (from a prior ts-export-terser run). The output is a .d.ts file placed alongside Terser's lib/ast.js so that subsequent TS exports resolve `this.x`
  * accesses to typed fields.
  */
object GenAstDts:
  def main(args: Array[String]): Unit =
    ...
    if outPath == "/dev/stdout" then println(dts)
    else
