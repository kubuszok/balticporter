...
/** JavaScript frontend — uses the TypeScript compiler's `allowJs` mode to type-check plain JavaScript sources. The exporter is the same Node.js `export.ts` with `--allowJs` added to the tsconfig.
  *
  * This enables semantic analysis of JavaScript codebases like Terser that are too dynamic for a pure syntax parser but have enough structure for the TS checker to infer useful types. Unresolvable
  * sites produce `any` types, which the minter records as `Unportable`.
  */
