...
/** Extracts semantic actions from Mermaid jison grammar files and maps them to Scala Db method calls for cross-checking against the hand-ported parsers.
  *
  ...
  *
  * The semantic actions call `yy.*` methods which map to the diagram's Db object (e.g., `yy.addVertex($1, $2)` → `db.addVertex(id, label)`).
  */
object JisonActionExtractor:
  ...
  /** Parse a jison grammar file and extract semantic actions.
    *
    * @param source
    *   the jison grammar file contents
    * @param diagramType
    *   name of the diagram (e.g., "flowchart", "er")
    */
  def extract(source: String, diagramType: String): GrammarSummary =
    ...
    val methodCounts = dbCalls.groupBy(_.methodName).map { case (name, calls) => name -> calls.size }
  ...
  /** Generate a Scala method call from a jison `yy.*` call.
    *
    * Translates `$1`, `$2`, etc. to Scala-friendly parameter names and maps the method name to its Scala equivalent.
    */
  ...
  def formatSummary(summary: GrammarSummary): String =
    ...
    if summary.constants.nonEmpty then
      sb.append(s"\nConstants:\n")
      for (prefix, values) <- summary.constants do sb.append(s"  $prefix: ${values.take(5).mkString(", ")}${if values.size > 5 then "..." else ""}\n")
    sb.toString

  /** Analyze all jison grammars in a directory.
    *
    * @param jisonDir
    *   root directory containing jison files
    */
  ...
  def formatCombinedTable(summaries: List[GrammarSummary]): String =
    ...
    for s <- summaries do
      sb.append(
        f"${s.diagramType}%-25s ${s.productionCount}%6d ${s.actionCount}%8d ${s.dbCalls.size}%9d ${s.dbMethodCounts.size}%8d\n"
      )
  ...
  private def extractProductions(lines: List[String]): List[Production] =
    ...
    for line <- lines do
      ...
      // New rule definition
        ...
        !inAction && trimmed.endsWith(":") && !trimmed.contains("'")
      then currentRule = trimmed.stripSuffix(":").trim
        ...
        if actionStart >= 0 then
          ...
          while i < trimmed.length do
            ...
            if inAction && !(ch == '{' && braceDepth == 1) then actionBuf.append(ch)
            i += 1
        else if trimmed.startsWith("|") then currentAlt = trimmed.stripPrefix("|").trim
        else if trimmed.startsWith(";") then currentRule = ""
      else
  ...
  private def extractCallArgs(source: String, startIdx: Int): List[String] =
    ...
    while i < source.length && depth > 0 do
      ...
      if inString then
        current.append(ch)
        if ch == stringChar && (i == 0 || source(i - 1) != '\\') then inString = false
      else
        ch match
          case '\'' | '"' =>
            inString = true
          ...
          case ')' | ']' | '}' =>
            ...
            else if current.toString.trim.nonEmpty then args += current.toString.trim
          case ',' if depth == 1 =>
            if current.toString.trim.nonEmpty then args += current.toString.trim
            current.clear()
          case _ =>
            current.append(ch)
      i += 1
  ...
  private def extractConstants(source: String): Map[String, List[String]] =
    ...
    for m <- pattern.findAllMatchIn(source) do
      ...
      val value  = m.group(2)
      if prefix != "getLogger" && prefix != "lex" then result.getOrElseUpdate(prefix, mutable.Set.empty) += value
