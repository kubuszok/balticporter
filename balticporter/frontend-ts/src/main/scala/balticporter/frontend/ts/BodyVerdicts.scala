...
/** Justification table for reference-derived method bodies — each body that the RAST pipeline did NOT produce gets a machine-classified or human-reviewed verdict explaining why.
  *
...
object BodyVerdicts:
  ...
  def formatSummary(verdicts: List[Verdict]): String =
    ...
    sb.append("By status:\n")
    for (status, count) <- byStatus.toList.sortBy(-_._2) do sb.append(f"  $status%-15s $count%4d\n")
    sb.append("By category:\n")
    for (category, count) <- byCategory.toList.sortBy(-_._2) do sb.append(f"  $category%-30s $count%4d\n")
    sb.toString
