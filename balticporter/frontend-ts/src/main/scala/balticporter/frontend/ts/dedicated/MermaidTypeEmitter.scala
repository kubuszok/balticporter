...
/** RAST-based emitter for Mermaid type definition modules.
  *
  * Handles `*Types.ts`, `config.type.ts`, and `defaultConfig.ts` files. These define TypeScript interfaces and type aliases that become Scala case classes and type aliases.
  *
  * Translation:
...
object MermaidTypeEmitter {
  ...
  /** Emits Scala types from a RAST type file.
    ...
    * @param objectName
    *   enclosing object name, or empty for top-level
    * @param pkg
    *   e.g. "pie" (the diagram's package)
    * @param extraPkg
    *   additional package segments (e.g. List("common"))
    * @param extraImports
    *   additional imports
    * @return
  ...
  def emitTypes(
    ...
    val needsMutable = interfaces.exists(i =>
      i.fields.exists(f =>
  ...
  /** An interface that just declares DiagramDB methods (getConfig, clear, etc.) should be skipped — those live on the Db class itself.
    */
  ...
  private def emitCaseClass(sb: StringBuilder, iface: InterfaceDecl): Unit = {
    ...
    val params = dataFields.map { f =>
      val tpe =
        if (f.optional && !f.scalaType.startsWith("Option["))
          ...
          f.scalaType
      val default =
        if (f.optional && !tpe.contains("= None"))
  ...
  private def extractTypeAliases(rast: RastFile): List[TypeAliasDecl] =
    rast.nodes.flatMap { node =>
      if (node.kind == "TypeAliasDeclaration") {
        val name       = nameOf(node)
        val isExported = node.flags.contains("ExportKeyword")
        val targetNode = node.children.find(c => c.kind != "Identifier" && c.kind != "TypeParameter")
