...
/** Dedicated emitter for roughjs filler files: FillerInterface, ScanLineHachure,
  * Filler factory, HachureFiller, HatchFiller, ZigZagFiller, ZigZagLineFiller,
  * DashedFiller, DotFiller. Produces Scala matching the hand-port's structure. */
object RoughFillersEmitter {

  def emitAll(): Map[String, String] = Map(
    "FillerInterface" -> fillerInterface,
    "ScanLineHachure" -> scanLineHachure,
    "Filler" -> filler,
  ...
  private val pkg = "package ssg.graphs.commons.rough.fillers\n\n"

  private val fillerInterface = pkg +
    """trait PatternFiller {
  ...
  private val scanLineHachure = pkg +
  ...
  private val zigZagLineFiller = pkg +
    """import scala.collection.mutable.ArrayBuffer
      |
