...
/** Coverage analysis over `ParityDerive` output — per-module RAST vs reference breakdown, regression detection, and TSV reporting.
  *
...
object CoverageLane:
  ...
  def analyze(bodies: List[ParityDerive.BodyEntry], moduleName: String): CoverageReport =
    ...
    val byReason = bodies.filter(_.source == "reference").groupBy(b => if b.why.isEmpty then "unclassified" else b.why).map { case (k, v) => k -> v.size }
  ...
  def formatDetailedReport(report: CoverageReport): String =
    ...
    if report.byReason.nonEmpty then
      ...
      for (reason, count) <- report.byReason.toList.sortBy(-_._2) do sb.append(f"  $reason%-40s $count%4d\n")
    sb.toString
