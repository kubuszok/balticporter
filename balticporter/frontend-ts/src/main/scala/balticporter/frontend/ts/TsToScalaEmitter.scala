...
/** Direct TypeScript → Scala source emitter for RAST v1.
  *
  * Reads the resolved AST produced by the Node.js exporter and emits Scala 3 source text. This is the path-data-parser proof of concept: enough lowering to produce compiling Scala from the 4 helper
  * libraries.
  *
  * Each TS module becomes a Scala object. Interfaces with only typed fields become case classes. Functions become methods on the module object.
  */
object TsToScalaEmitter:
  ...
  final case class EmitConfig(
    ...
    /** Map from TS type alias name to Scala type name. `Point` → `Point` means the emitter uses `Point` wherever the TS type alias appears.
      */
    tupleTypeOverrides: Map[String, String] = Map.empty,
    /** Map from TS type alias name to field names for tuple element access. `Point` → Map(0 → "x", 1 → "y") means `p[0]` → `p.x`.
      */
  ...
  def emit(files: List[RastFile], config: EmitConfig): Map[String, String] =
    ...
    files.flatMap { f =>
      ...
      if config.skipIndex && rawName == "index" then None
        ...
        for {
          replacements <- config.postProcess.get(fileName)
  ...
  private class EmitContext(config: EmitConfig):
    ...
    def emitFile(file: RastFile, fileName: String): String =
      ...
      val variables   = file.nodes.filter(n => n.kind == "FirstStatement" || n.kind == "VariableStatement")
      ...
      for (iface <- interfaces)
        ...
        if isExported(iface) then emitCaseClass(iface, file)
    ...
    private def emitCaseClass(node: RastNode, file: RastFile, indent: String = ""): Unit =
      ...
      val props  = node.children.filter(c => c.kind == "PropertySignature" || c.kind == "PropertyDeclaration")
    ...
    private def emitFunction(node: RastNode, file: RastFile, indent: String): Unit =
      ...
      val paramList      = params.map { p =>
        ...
        if hasArrayBinding then s"${escapedName}: (Double, Double)"
        else s"$escapedName: $pType"
      }
    ...
    private def collectMutatedArrays(body: RastNode): Set[String] =
      ...
      def walk(n: RastNode): Unit =
          ...
          fn.foreach { f =>
            if f.kind == "PropertyAccessExpression" then
              ...
              if method.contains("push") then
                val obj = f.children.headOption.flatMap(_.text).orElse(f.children.headOption.map(c => nameOf(c)))
        ...
        if n.kind == "BinaryExpression" && n.operator.exists(o => o == "PlusEqualsToken" || o == "FirstCompoundAssignment") then n.children.headOption.flatMap(_.text).foreach(result += _)
    ...
    private def emitVariableStatement(node: RastNode, file: RastFile, indent: String): Unit =
      ...
      for (d <- decls)
        ...
        if hasArrayBinding then
          ...
          val names    = pattern.children.filter(_.kind == "BindingElement").flatMap(_.children.find(_.kind == "Identifier").flatMap(_.text))
          val initExpr = d.children.find(c => c.kind != "ArrayBindingPattern" && !c.kind.contains("Type") && !c.kind.contains("Keyword")).map(emitExpr(_, file)).getOrElse("???")
          ...
          val tpeFixed = if tpe == "Double" then
            ...
            // For vars: only Int if initialized with integer AND name suggests index/counter
              ...
              if nm.contains("index") || nm == "i" || nm == "j" || nm == "pos" ||
                nm.contains("count") || nm.contains("length")
              then "Int"
              else tpe
            else tpe
          else tpe
          val init      = findInitializer(d, file)
          val initFixed = if isMutatedArray && (init == "Vector.empty" || init.startsWith("ArrayBuffer")) then tpe.replace("ArrayBuffer[", "ArrayBuffer.empty[").stripSuffix("]") + "]"
          else if isMutatedArray && init.startsWith("Vector(") then init.replace("Vector(", "ArrayBuffer(")
          else init
    ...
    private def emitStatement(node: RastNode, file: RastFile, indent: String): Unit =
      node.kind match
        case "ReturnStatement" =>
          ...
          val converted = if currentMutatedArrays.contains(expr.trim) then s"$expr.toVector"
          else expr
        ...
        case "IfStatement" =>
          ...
          regexMatch match
            case Some((obj, regex)) =>
              ...
              else sb.append(s"$indent  case None => ()\n")
              sb.append(s"$indent}\n")
            case None =>
              ...
              val condFixed     = if isOptionalRef then s"$cond.isDefined"
    ...
    private def emitForOf(node: RastNode, file: RastFile, indent: String): Unit =
      val binding  = node.children.find(c => c.kind == "VariableDeclarationList" || c.kind == "VariableDeclaration")
      val iterable = node.children.find(c => c.kind != "VariableDeclarationList" && c.kind != "VariableDeclaration" && c.kind != "Block")
      val body     = node.children.find(_.kind == "Block")

      val bindingName = binding
        .map { b =>
          val decls = if b.kind == "VariableDeclarationList" then b.children.filter(_.kind == "VariableDeclaration")
          else List(b)
      ...
      val destructured = binding.flatMap { b =>
        val decl = if b.kind == "VariableDeclarationList" then b.children.find(_.kind == "VariableDeclaration")
        else Some(b)
      ...
      if children.length >= 4 then
        ...
        if initChild.kind == "VariableDeclarationList" then
          ...
          for (d <- decls)
            ...
            if declaredVars.contains(n) then sb.append(s"$indent$n = $initExpr\n")
            else
              val kw = if isConst then "val" else "var"
              sb.append(s"$indent$kw $n: Int = $initExpr\n")
              declaredVars += n
        else emitStatement(initChild, file, indent)
        val cond = emitExpr(children(1), file)
        sb.append(s"${indent}while ($cond) {\n")
        // Emit the body block's statements WITHOUT the outer braces
        val bodyNode = children(3)
        if bodyNode.kind == "Block" then bodyNode.children.foreach(c => emitStatement(c, file, indent + "  "))
        else emitStatement(bodyNode, file, indent + "  ")
        ...
        sb.append(s"${indent}  $update\n")
        sb.append(s"$indent}\n")
      else sb.append(s"$indent// TODO: for loop with ${children.length} children\n")
        ...
        while i < clauses.length do
          clauses(i).kind match
            case "CaseClause" =>
              ...
              if stmts.isEmpty && i + 1 < clauses.length && clauses(i + 1).kind == "CaseClause" then
                ...
                if i < clauses.length && clauses(i).kind == "CaseClause" then
                  ...
                  clauses(i).children.drop(1).filter(_.kind != "BreakStatement").foreach(s => emitStatement(s, file, indent + "    "))
                else sb.append(s"$indent  case ${values.mkString(" | ")} =>\n")
              else
            ...
            case "DefaultClause" =>
              ...
              clauses(i).children.filter(_.kind != "BreakStatement").foreach(s => emitStatement(s, file, indent + "    "))
            case _ => ()
          i += 1
    ...
    private def emitExpr(node: RastNode, file: RastFile): String =
      node.kind match
        ...
        case "BinaryExpression" =>
          ...
          val isTypeofCheck = lhs.kind == "TypeOfExpression" && node.operator.exists(o => o == "EqualsEqualsEqualsToken" || o == "EqualsEqualsToken")
          ...
          if op == "=" && lhs.kind == "ArrayLiteralExpression" then
            ...
            else names.zipWithIndex.map { case (n, i) => s"$n = $rAccess($i)" }.mkString("; ")
          // R15: arr[arr.length] = x → arr += x
            ...
            else if op == "=" || op == "+=" || op == "-=" then s"$left $op $right"
            else if op == "&&" || op == "||" then
          ...
          fn match
            case s if s.endsWith(".push") =>
              ...
              def wrapJsNum(a: String): String =
                if isStringBuffer && a.matches(""".*\(\d+\)""") && !a.startsWith("jsNum") && !a.contains("\"") then s"jsNum($a)"
                else a
            ...
            case s if s.endsWith(".splice") =>
              ...
              if args.length == 2 then s"{ val _removed = $obj.take(${args(0)} + ${args(1)}); $obj.remove(${args.mkString(", ")}); _removed }"
        ...
        case "PropertyAccessExpression" =>
          ...
          if obj == "Math" then
            prop match
              case "PI"                                                                                                                  => "math.Pi"
              case "sqrt" | "pow" | "cos" | "sin" | "atan2" | "abs" | "floor" | "ceil" | "max" | "min" | "log" | "tan" | "asin" | "acos" =>
                s"math.$prop"
          ...
          // RegExp.$1 → _m.group(1)
            ...
            s"_m.group($groupNum)"
          else if prop == "length" then s"$obj.length"
          else if prop == "push" then s"$obj.push"
          else if prop == "forEach" then s"$obj.foreach"
          else if prop == "splice" then s"$obj.remove"
          else if prop == "filter" then s"$obj.filter"
          else if scalaKeywords.contains(prop) then s"$obj.`$prop`"
          else s"$obj.$prop"
        ...
        case "ElementAccessExpression" =>
          ...
          val fieldOverride = config.tupleFieldOverrides
            .find { case (typeName, _) =>
          ...
          if callReturnsTuple && idx.matches("\\d+") then
            ...
            val accessor = fieldOverride.getOrElse(s"_${idx.toInt + 1}")
            s"$obj.$accessor"
          else if isOptional then s"$obj.get($idx)"
          else s"$obj($idx)"
        ...
        case "ArrayLiteralExpression" =>
          ...
          val tupleOverrideName = node.`type`
            .flatMap(file.types.get)
            .flatMap { t =>
              config.tupleTypeOverrides
                .find { case (_, scalaName) =>
          ...
          if node.children.isEmpty then "Vector.empty"
          else if allSpread && node.children.length == 1 then emitExpr(node.children.head.children.head, file)
          else if isTuple then
            val elems       = node.children.map(c => if c.kind == "SpreadElement" then emitExpr(c.children.head, file) else emitExpr(c, file))
            ...
            val prefix     = if nonSpread.nonEmpty then s"Vector(${nonSpread.map(emitExpr(_, file)).mkString(", ")})"
            else "Vector.empty"
            ...
            val elems = node.children.map { c =>
              ...
              val isArrayVar = currentMutatedArrays.contains(eName) && c.`type`.flatMap(file.types.get).exists(t => t.kind == "array" || t.text.contains("[]"))
        ...
        case "ConditionalExpression" =>
          ...
          val condFixed =
            if condType == "number" && !cond.contains("==") && !cond.contains("!=") && !cond.contains("<") && !cond.contains(">")
            then s"($cond) != 0"
            else cond
    ...
    private def emitObjectLiteral(node: RastNode, file: RastFile): String =
      ...
      else if fieldNames == Set("type", "text") || fieldNames == Set("`type`", "text") then s"PathToken(${kvMap.getOrElse("`type`", kvMap.getOrElse("type", "???"))}, ${kvMap("text")})"
      else
    ...
    private def emitArrowFunction(node: RastNode, file: RastFile): String =
      ...
      if paramList.length == 1 then s"${paramList.head} => $bodyStr"
      else s"(${paramList.mkString(", ")}) => $bodyStr"
    ...
    private def isUsedAsIndex(name: String, body: RastNode): Boolean =
      ...
      def walk(n: RastNode): Unit =
        if !found && n.kind == "ElementAccessExpression" && n.children.length >= 2 then
          ...
          if idx.text.contains(name) || idx.children.exists(_.text.contains(name)) then found = true
        n.children.foreach(walk)
      walk(body)
      found
    ...
    private def isTupleType(rt: RastType, file: RastFile): Boolean =
      ...
      else if rt.kind == "reference" then rt.target.flatMap(file.types.get).exists(t => isTupleType(t, file))
      else false
    ...
    private def rastTypeToScala(rt: RastType, file: RastFile): String =
      rt.kind match
        ...
        case "any"                => "Any"
        case "array"              =>
          val elem = rt.elementType.flatMap(file.types.get).map(rastTypeToScala(_, file)).getOrElse("Any")
          s"Vector[$elem]"
        case "union" =>
        ...
        case "function" =>
          val params = rt.parameters.getOrElse(Nil).map(p => file.types.get(p.`type`).map(rastTypeToScala(_, file)).getOrElse("Any"))
          val ret    = rt.returnType.flatMap(file.types.get).map(rastTypeToScala(_, file)).getOrElse("Unit")
        ...
        case "reference"      =>
          ...
          if isTuple then
            val typeArgs = rt.typeArguments.getOrElse(Nil).flatMap(file.types.get).map(rastTypeToScala(_, file))
        ...
        case _ =>
          if rt.text.contains("[]") then s"Vector[${rt.text.replace("[]", "").trim}]"
          else if rt.text.contains("[key:") then resolveObjectType(rt, file)
          else typeAliasMap.getOrElse(rt.text, rt.text)
      ...
      node.`type`.flatMap(file.types.get) match
        ...
        case _ =>
          ...
          val fromName = nameId.flatMap(_.`type`).flatMap(file.types.get).flatMap { rt =>
            if rt.kind == "function" then rt.returnType.flatMap(file.types.get).map(rastTypeToScala(_, file))
            else None
          }
          fromName.getOrElse {
    ...
    private def nameOf(node: RastNode): String =
      node.children.find(_.kind == "Identifier").flatMap(_.text).orElse(node.text).getOrElse("$anon")
