...
class JisonActionExtractorSpec extends munit.FunSuite:
  ...
  private val jisonRoot: java.nio.file.Path =
    ...
    if cpRef != null && cpRef.getProtocol == "file" then java.nio.file.Path.of(cpRef.toURI).getParent.getParent.getParent // up to jison/
      ...
      candidates.map(_.resolve("original-src/mermaid/packages/mermaid/src/diagrams")).find(java.nio.file.Files.exists(_)).getOrElse(java.nio.file.Path.of("nonexistent"))
  ...
  test("sankey: extract 1 action with yy.findOrCreateNode + yy.addLink"):
    ...
    if source.isEmpty then println("SKIP: ssg original-src not found")
    else
  ...
  test("batch: analyze all 17 jison grammars"):
    if !java.nio.file.Files.exists(jisonRoot) then println("SKIP: ssg original-src not found at " + jisonRoot)
    else
