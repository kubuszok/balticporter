...
class JisonActionExtractorSpec extends munit.FunSuite:
  ...
  // Classpath snapshot first (self-contained); ssg's vendored upstream mermaid sources as a fallback.
  private val jisonRoot: java.nio.file.Path =
    val cpRef = getClass.getResource("/reference/jison/flowchart/parser/flow.jison")
    if cpRef != null && cpRef.getProtocol == "file" then java.nio.file.Path.of(cpRef.toURI).getParent.getParent.getParent // up to jison/
    else ConsumerCheckout.originalSrc("mermaid/packages/mermaid/src/diagrams").getOrElse(java.nio.file.Path.of("nonexistent"))

  private def loadJison(path: String): String =
    val fullPath = jisonRoot.resolve(path)
    if java.nio.file.Files.exists(fullPath) then new String(java.nio.file.Files.readAllBytes(fullPath))
    else ""
  ...
  test("sankey: extract 1 action with yy.findOrCreateNode + yy.addLink"):
    ...
    assume(source.nonEmpty, s"sankey/parser/sankey.jison not found under $jisonRoot")
    ...
    assert(
      summary.dbMethodCounts.contains("findOrCreateNode"),
      s"should find findOrCreateNode, got: ${summary.dbMethodCounts.keys.mkString(", ")}"
    )
    assert(summary.dbMethodCounts.contains("addLink"), s"should find addLink, got: ${summary.dbMethodCounts.keys.mkString(", ")}")

  test("er: extract entity and relationship actions"):
    val source = loadJison("er/parser/erDiagram.jison")
    assume(source.nonEmpty, s"er/parser/erDiagram.jison not found under $jisonRoot")
    val summary = balticporter.corpus.mermaid.JisonActionExtractor.extract(source, "er")
    println(balticporter.corpus.mermaid.JisonActionExtractor.formatSummary(summary))
    assert(
      summary.dbMethodCounts.contains("addEntity"),
      s"should find addEntity, got: ${summary.dbMethodCounts.keys.mkString(", ")}"
    )
    assert(summary.dbMethodCounts.contains("addRelationship"), s"should find addRelationship")
    // ER has Cardinality and Identification enums
    assert(
      summary.constants.contains("Cardinality") || summary.constants.contains("Identification"),
  ...
  test("flowchart: extract vertex and link actions"):
    ...
    assume(source.nonEmpty, s"flowchart/parser/flow.jison not found under $jisonRoot")
    val summary = balticporter.corpus.mermaid.JisonActionExtractor.extract(source, "flowchart")
    println(balticporter.corpus.mermaid.JisonActionExtractor.formatSummary(summary))
    assert(
      summary.dbMethodCounts.contains("addVertex"),
      s"should find addVertex, got: ${summary.dbMethodCounts.keys.mkString(", ")}"
    )
    assert(summary.dbMethodCounts.contains("setDirection"), s"should find setDirection")
    assert(summary.productionCount >= 20, s"Expected >= 20 productions, got ${summary.productionCount}")

  test("sequence: extract signal and actor actions"):
    val source = loadJison("sequence/parser/sequenceDiagram.jison")
    assume(source.nonEmpty, s"sequence/parser/sequenceDiagram.jison not found under $jisonRoot")
    ...
    assert(
      summary.constants.contains("LINETYPE"),
      s"should find LINETYPE constants, got: ${summary.constants.keys.mkString(", ")}"
    )
    assert(summary.dbCalls.size >= 5, s"Expected >= 5 db calls, got ${summary.dbCalls.size}")
  ...
  test("batch: analyze all 17 jison grammars"):
    assume(java.nio.file.Files.exists(jisonRoot), s"ssg original-src not found at $jisonRoot")
    ...
    assert(summaries.size >= 15, s"Expected >= 15 jison files, got ${summaries.size}")

    // Verify we extract meaningful data from at least some grammars
    val totalDbCalls = summaries.map(_.dbCalls.size).sum
    assert(totalDbCalls >= 50, s"Expected >= 50 total db calls, got $totalDbCalls")
    ...
    if unmapped.nonEmpty then println(s"Unmapped methods: ${unmapped.toList.sorted.mkString(", ")}")
