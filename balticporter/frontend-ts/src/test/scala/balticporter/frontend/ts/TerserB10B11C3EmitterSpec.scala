...
class TerserB10B11C3EmitterSpec extends munit.FunSuite:
  ...
  test("B10: Token constants object with keyword/operator/punctuation sets"):
    ...
    val scala = dedicated.TerserB10B11C3Emitter.emitTokenConstants(rast)
  ...
  test("B10: Tokenizer skeleton with class structure"):
    val rast = loadRast("/rast/terser/lib/parse.rast.json")
    val scala = dedicated.TerserB10B11C3Emitter.emitTokenizerSkeleton(rast)
  ...
  test("B11: CompressorOptions case class with all optimizer flags"):
    ...
    val scala = dedicated.TerserB10B11C3Emitter.emitCompressorOptions(rast)
  ...
  test("B11: MinifyOptions top-level configuration"):
    ...
    val scala = dedicated.TerserB10B11C3Emitter.emitMinifyOptions(rast)
  ...
  test("C3: extract DEFMETHOD entries from compress/index RAST"):
    val rast = loadRast("/rast/terser/lib/compress/index.rast.json")
    val (name, count, methods) = dedicated.TerserB10B11C3Emitter.extractCompressModuleSummary(rast, "index")
  ...
  test("C3: extract DEFMETHOD entries from compress/inference RAST"):
    ...
    val (_, count, methods) = dedicated.TerserB10B11C3Emitter.extractCompressModuleSummary(rast, "inference")
    println(s"[C3] compress/inference: $count DEFMETHODs found: ${methods.take(5).mkString(", ")}${if (methods.size > 5) "..." else ""}")
  ...
  test("C3: extract DEFMETHOD entries from compress/evaluate RAST"):
    ...
    val (_, count, methods) = dedicated.TerserB10B11C3Emitter.extractCompressModuleSummary(rast, "evaluate")
  ...
  test("C3: emit compress module skeleton from RAST"):
    val rast = loadRast("/rast/terser/lib/compress/inference.rast.json")
    val scala = dedicated.TerserB10B11C3Emitter.emitCompressModuleSkeleton(rast, "inference", "Inference")
  ...
  test("C3: emit compress skeleton for drop-side-effect-free"):
    ...
    val scala = dedicated.TerserB10B11C3Emitter.emitCompressModuleSkeleton(rast, "drop-side-effect-free", "DropSideEffectFree")
  ...
  test("write all B10/B11/C3 emitted files"):
    ...
    val allFiles = List(
      ("Token",             dedicated.TerserB10B11C3Emitter.emitTokenConstants(parseRast)),
      ("TokenizerSkeleton", dedicated.TerserB10B11C3Emitter.emitTokenizerSkeleton(parseRast)),
      ("CompressorOptions", dedicated.TerserB10B11C3Emitter.emitCompressorOptions(compressRast)),
      ("MinifyOptions",     dedicated.TerserB10B11C3Emitter.emitMinifyOptions(minifyRast)),
    )
    ...
    val moduleFiles = compressModules.map { (mod, obj, path) =>
      ...
      (obj, dedicated.TerserB10B11C3Emitter.emitCompressModuleSkeleton(rast, mod, obj))
    }
