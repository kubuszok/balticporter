...
class DefmethodBodyTranslatorSpec extends munit.FunSuite:
  ...
  test("equivalent-to.js: extract prototype assignments"):
    ...
    assert(entries.forall(_.methodName == "shallow_cmp"), s"All should be shallow_cmp, got ${entries.map(_.methodName).distinct}")
  ...
  test("equivalent-to.js: function body translates this.value === other.value"):
    ...
    assert(
      result.scalaBody.contains("this.value") || result.scalaBody.contains("value"),
  ...
  test("size.js: simple return-literal bodies translate"):
    ...
    assert(
      astNode.isDefined,
      s"Should find AST_Node._size, methods found: ${entries.filter(_.className == "AST_Node").map(_.methodName)}"
    )
  ...
  test("scope.js: return_false references translate to false"):
    ...
    val isBlockScope = entries.find(e => e.className == "AST_Node" && e.methodName == "is_block_scope")
  ...
  test("scope.js: simple field access translates"):
    ...
    val definition = entries.find(e => e.className == "AST_Symbol" && e.methodName == "definition")
  ...
  test("scope.js: instanceof translates correctly"):
    ...
    val findVariable = entries.find(e => e.className == "AST_Scope" && e.methodName == "find_variable")
    ...
    assert(
      result.scalaBody.contains("isInstanceOf[AstSymbol]"),
  ...
  test("scope.js: this.thedef.global translates property chain"):
    ...
    val global = entries.find(e => e.className == "AST_Symbol" && e.methodName == "global")
    ...
    assert(result.scalaBody.contains("this.thedef.global"), s"Should access this.thedef.global, got: ${result.scalaBody}")
  ...
  test("scope.js: translation statistics"):
    ...
    assert(stats.fullyTranslated >= 10, s"Expected >= 10 fully translated, got ${stats.fullyTranslated}")
  ...
  test("equivalent-to.js: translation statistics"):
    ...
    assert(stats.fullyTranslated >= 40, s"Expected >= 40 fully translated, got ${stats.fullyTranslated}")
  ...
  test("size.js: translation statistics"):
    ...
    assert(stats.fullyTranslated >= 20, s"Expected >= 20 fully translated, got ${stats.fullyTranslated}")
  ...
  test("equivalent-to.js: emit shallow_cmp family"):
    val rast    = loadRast("/rast/terser/lib/equivalent-to.rast.json")
    val entries = dedicated.DefmethodBodyTranslator.extractPrototypeAssignments(rast)
    val output  = dedicated.DefmethodBodyTranslator.emitDefmethodFamily("shallow_cmp", entries, hierarchy)
  ...
  test("scope.js: emit is_block_scope family"):
    ...
    val output = dedicated.DefmethodBodyTranslator.emitDefmethodFamily("is_block_scope", isBlockScope, hierarchy)
    assert(output.contains("def isBlockScope"), s"Should emit isBlockScope")
    assert(output.contains("false") || output.contains("true"), s"Should contain boolean returns")
  ...
  test("scope.js: show sample translated bodies"):
    ...
    for (cls, method) <- samples do
      ...
      entry.foreach { e =>
        ...
        if result.refusalReasons.nonEmpty then println(s"    Refusals: ${result.refusalReasons.mkString(", ")}")
      }
