...
class MermaidTypeEmitterSpec extends munit.FunSuite:
  ...
  test("pieTypes.ts -> case classes from RAST"):
    ...
    assert(scala.contains("case class PieStyleOptions") || scala.contains("PieStyleOptions"), "should emit PieStyleOptions")
    // D3Section interface -> case class
    assert(scala.contains("case class D3Section") || scala.contains("D3Section"), "should emit D3Section")
    // Sections type alias
    ...
    assert(scala.contains("PieDB") && scala.contains("method declarations"), "should skip PieDB interface (all functions)")
