...
class LiqpParserRewriteSpec extends munit.FunSuite:
  ...
  //
  // The cache asked three questions (do the classes exist, are these the coordinates, is this the
  // rewrite policy) and not the fourth: is this the GRAMMAR those classes were compiled from. The
  // generated tree is untracked and regenerated by `./mvnw generate-sources`, so a grammar change
  // produces new `.java` under an unchanged key beside a `parserClasses` directory that still has
  // `.class` files in it — and `hasParserClasses` is an EXISTENCE test. The port then resolves
  // against a parser it no longer has: an import that resolves WRONGLY rather than failing, which
  // is the exact failure this cache's own doc says the fingerprint exists to prevent, arriving one
  // input further in. Nothing reports it — the port compiles and every count is flat.
  // -----------------------------------------------------------------------------------------
